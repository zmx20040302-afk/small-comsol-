% Generate a bounded thermal-actuator CSV suitable for local surrogate training.
...
for DV = DV_values
  for htc_s = htc_s_values
    for htc_us = htc_us_values
      ...
      fprintf('THERMAL_ACTUATOR_SAFE_SAMPLE=%d/%d\n', index, size(rows, 1));
      index = index + 1;
    end
  end
end
...
output_csv = fullfile(result_dir, 'thermal_actuator_safe_training.csv');
...
fprintf('THERMAL_ACTUATOR_SAFE_TRAINING_CSV=%s\n', output_csv);
