"""Train a declared-scope COMSOL parallel-wire electromagnetic-force surrogate."""
...
TRAINING = ROOT / "generated/training_runs/parallel_wires_force_training.csv"
HOLDOUT = ROOT / "generated/training_runs/parallel_wires_force_holdout.csv"
MODEL = ROOT / "generated/models/parallel_wires_force_surrogate.joblib"
INPUTS = ["current_A"]
OUTPUTS = ["force_x_wire2_N"]
SCOPE = "二维平行圆导线静磁场模型；导线半径 r_wire=0.2 m、间距、方向、外域、网格和力计算区域固定，两个导线由同一同向电流 I0=0.25-3 A 驱动，输出第二导线 x 向电磁力。"
...
def main() -> int:
    ...
    candidates = {"linear_regression": LinearRegression(), "quadratic_regression": Pipeline([("poly", PolynomialFeatures(degree=2)), ("linear", LinearRegression())])}
    fitted, scores = {}, {}
    ...
    card = write_general_surrogate_model_card(model_name="平行载流导线电磁力代理模型", model_path=MODEL, training_csv=TRAINING, holdout_csv=HOLDOUT, input_columns=INPUTS, output_columns=OUTPUTS, physical_scope=SCOPE, validation=validation, output_dir=MODEL.parent)
    register_general_surrogate(model_name="平行载流导线电磁力代理模型", model_path=MODEL, card=card, validation=validation, registry_path=ROOT / "generated/models/surrogate_registry.json")
