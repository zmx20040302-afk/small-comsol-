"""Train a declared-scope D1-to-concentration porous-diffusion surrogate."""
...
TRAINING = ROOT / "generated/training_runs/effective_diffusivity_1d_training.csv"
HOLDOUT = ROOT / "generated/training_runs/effective_diffusivity_1d_holdout.csv"
MODEL = ROOT / "generated/models/effective_diffusivity_1d_surrogate.joblib"
INPUTS, OUTPUTS = ["D1_m2_s"], ["concentration_x0p2mm_t100ms_mol_m3"]
SCOPE = "一维等效多孔扩散瞬态模型；固定孔隙率 epsilon=0.383、传质系数、初始浓度、几何、网格和 100 ms 观测时刻，仅改变等效扩散系数 D1=1e-6-4e-6 m^2/s，输出 x=0.2 mm 的浓度。"
...
def main() -> int:
    ...
    selected_name = min(scores, key=lambda name: float(scores[name]["mean_relative_error_percent"]))
    selected_metrics = scores[selected_name]
    passed = float(selected_metrics["max_relative_error_percent"]) <= 5.0
    validation = {"kind":"effective_diffusivity_1d_independent_comsol_validation","created_at":datetime.now(timezone.utc).isoformat(),"training_csv":str(TRAINING.resolve()),"holdout_csv":str(HOLDOUT.resolve()),"training_rows":len(training),"holdout_rows":len(holdout),"candidate_metrics":scores,"selected_model":selected_name,"selection_reason":"Selected by independent COMSOL holdout error for a nonlinear transient diffusion response.","per_output_relative_validation":{OUTPUTS[0]:{**selected_metrics,"threshold_relative_error_percent":5.0,"passed":passed}},"passed":passed,"scope":SCOPE}
    ...
    card = write_general_surrogate_model_card(model_name="一维等效多孔扩散浓度代理模型",model_path=MODEL,training_csv=TRAINING,holdout_csv=HOLDOUT,input_columns=INPUTS,output_columns=OUTPUTS,physical_scope=SCOPE,validation=validation,output_dir=MODEL.parent)
    registry = register_general_surrogate(model_name="一维等效多孔扩散浓度代理模型",model_path=MODEL,card=card,validation=validation,registry_path=ROOT / "generated/models/surrogate_registry.json")
