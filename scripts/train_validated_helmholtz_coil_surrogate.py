"""Train a declared-scope COMSOL Helmholtz-coil center-field surrogate."""
...
TRAINING = ROOT / "generated/training_runs/helmholtz_coil_training.csv"
HOLDOUT = ROOT / "generated/training_runs/helmholtz_coil_holdout.csv"
MODEL = ROOT / "generated/models/helmholtz_coil_surrogate.joblib"
INPUTS = ["coil_current_mA"]
OUTPUTS = ["center_By_T"]
SCOPE = "三维亥姆霍兹双圆线圈磁场模型；两个线圈同向等电流、几何、材料、网格、无限元域与中心测点固定，仅改变线圈电流 I0=0.05-0.6 mA，输出中心带符号 By。"
...
def main() -> int:
    ...
    validation = {"kind": "helmholtz_coil_center_field_independent_comsol_validation", "created_at": datetime.now(timezone.utc).isoformat(), "training_csv": str(TRAINING.resolve()), "holdout_csv": str(HOLDOUT.resolve()), "training_rows": len(training), "holdout_rows": len(holdout), "candidate_metrics": scores, "selected_model": selected_name, "selection_reason": "Selected by independent COMSOL holdout error for the signed center magnetic field.", "per_output_relative_validation": {OUTPUTS[0]: {**selected_metrics, "threshold_relative_error_percent": 1.0, "passed": passed}}, "passed": passed, "scope": SCOPE}
    ...
    card = write_general_surrogate_model_card(model_name="亥姆霍兹线圈中心磁场代理模型", model_path=MODEL, training_csv=TRAINING, holdout_csv=HOLDOUT, input_columns=INPUTS, output_columns=OUTPUTS, physical_scope=SCOPE, validation=validation, output_dir=MODEL.parent)
    register_general_surrogate(model_name="亥姆霍兹线圈中心磁场代理模型", model_path=MODEL, card=card, validation=validation, registry_path=ROOT / "generated/models/surrogate_registry.json")
