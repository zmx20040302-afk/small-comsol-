...
mphstart(getenv_or_default('COMSOL_MPH_HOST', 'localhost'), port_or_default());

model = mphopen(fullfile(builder_dir, 'busbar_joule_heat_baseline.mph'));
voltages_mv = [10 20 30];
heat_transfer_coefficients = [2 5 10];
output_path = fullfile(builder_dir, 'busbar_operating_conditions.csv');
error_path = fullfile(builder_dir, 'busbar_operating_conditions.error.txt');
...
try
  for voltage_mv = voltages_mv
    for htc_w_m2k = heat_transfer_coefficients
      model.param.set('Vtot', sprintf('%g[mV]', voltage_mv));
      model.param.set('htc', sprintf('%g[W/(m^2*K)]', htc_w_m2k));
      ...
      fprintf('Completed Vtot=%g mV, htc=%g W/(m^2*K)\\n', voltage_mv, htc_w_m2k);
    end
  end
  disp(result);
catch errorInfo
...
function value = port_or_default()
value = str2double(getenv('COMSOL_MPH_PORT'));
if isnan(value), value = 2036; end
end
