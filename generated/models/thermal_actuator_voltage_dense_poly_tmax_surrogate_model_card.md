...
## 物理适用范围
微执行器焦耳热分布参数案例；DV=2-6.6 V，htc_s=15000-25000 W/(m^2*K)，htc_us=300-500 W/(m^2*K)。模型只用于该已验证工作区内的最高温度预测，不外推至更高电压或改变后的几何、材料与边界条件。

## 训练与独立验证
- 训练数据：D:\桌面\codex\comsol1\comsol_training_small_model\generated\training_runs\thermal_actuator_voltage_dense_extended_training.csv，63 行
