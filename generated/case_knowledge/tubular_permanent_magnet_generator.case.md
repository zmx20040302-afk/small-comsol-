# tubular_permanent_magnet_generator

- Case directory: `D:\桌面\codex\案例下载\电气\管式永磁发电机`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:20:25.950707+00:00`
...
- `Mg_h` = `18[mm]` - Magnet height
- `S_r` = `20[mm]` - Shaft radius
- `M_w` = `10[mm]` - Magnet width
- `A_g` = `2[mm]` - Air gap between stator and slider
- `St_h` = `8[mm]` - Stator tooth height
- `Sl_h` = `23[mm]` - Slider tooth height
- `P_P` = `2*Mg_h+2*Sl_ch` - Pole pitch
- `Sl_ch` = `15[mm]` - Slider iron height
- `Sl_tw` = `4[mm]` - Slider tooth width
- `C_h` = `(P_P-3*St_h)/3` - Coil height
- `St_w` = `4[mm]` - Stator tooth width
- `C_w` = `10[mm]` - Coil width
- `St_ith` = `5[mm]` - Stator back-iron thickness
- `Nturns` = `100` - Number of coil turns in each winding
- `speed` = `1[m/s]` - Speed of the slider
- `filling_factor` = `0.8` - Filling factor

## 内容级读取结果

- 模型树证据数量: 34
- 理论关键词: 结构力学, 流体, 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r1 / layername, geom1 / r1 / layer, geom1 / r1 / layertop, geom1 / r2 / size
- physics: Magnetic Fields
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: r1 / size, r1 / pos, r1 / layername, r1 / layer, r1 / layertop, r2 / size

### 文件内容证据

- `tubular_permanent_magnet_generator.java`: 参数=16, 几何=8, 物理场=0, 研究=0, 结果=0
- `models.acdc.tubular_permanent_magnet_generator.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `tubular_permanent_magnet_generator.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `tubular_permanent_magnet_generator.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `tubular_permanent_magnet_generator_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.tubular_permanent_magnet_generator.pdf`: models.acdc.tubular_permanent_magnet_generator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Moving Mesh. Note: for rotational electrical machines, the Rotating Machinery,

## PDF Summary and Physics Judgement

- Simple summary: tubular_permanent_magnet_generator 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: Moving Mesh. Note: for rotational electrical machines, the Rotating Machinery,
- PDF: linear motor, this can be modeled using a 2D axisymmetric geometry and applying
- PDF: model results show the magnetic fields in the system with the key result being the
...
- PDF keywords: 结构力学, 流体, 电磁, models.acdc.tubular_permanent_magnet_generator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Moving Mesh. Note: for rotational electrical machines, the Rotating Machinery,
- `Mg_h` -> Magnet height; MATLAB: `model.param.set('Mg_h', value, description)`
- `S_r` -> Shaft radius; MATLAB: `model.param.set('S_r', value, description)`
- `M_w` -> Magnet width; MATLAB: `model.param.set('M_w', value, description)`
- `A_g` -> Air gap between stator and slider; MATLAB: `model.param.set('A_g', value, description)`
- `St_h` -> Stator tooth height; MATLAB: `model.param.set('St_h', value, description)`
- `Sl_h` -> Slider tooth height; MATLAB: `model.param.set('Sl_h', value, description)`
- `P_P` -> Pole pitch; MATLAB: `model.param.set('P_P', value, description)`
- `Sl_ch` -> Slider iron height; MATLAB: `model.param.set('Sl_ch', value, description)`
- `Sl_tw` -> Slider tooth width; MATLAB: `model.param.set('Sl_tw', value, description)`
- `C_h` -> Coil height; MATLAB: `model.param.set('C_h', value, description)`
- `St_w` -> Stator tooth width; MATLAB: `model.param.set('St_w', value, description)`
- `C_w` -> Coil width; MATLAB: `model.param.set('C_w', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；geom1 / r1 / size；geom1 / r1 / pos；geom1 / r1 / layername；geom1 / r1 / layer
- 已识别物理场证据：Magnetic Fields
- 已识别参数：Mg_h、S_r、M_w、A_g、St_h、Sl_h、P_P、Sl_ch、Sl_tw、C_h、St_w、C_w
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 结构力学, 流体, 电磁?
...
- Sweep `Mg_h` around the learned value `18[mm]` and export target outputs to CSV.
- Sweep `S_r` around the learned value `20[mm]` and export target outputs to CSV.
- Sweep `M_w` around the learned value `10[mm]` and export target outputs to CSV.
- Sweep `A_g` around the learned value `2[mm]` and export target outputs to CSV.
- Sweep `St_h` around the learned value `8[mm]` and export target outputs to CSV.
- Sweep `Sl_h` around the learned value `23[mm]` and export target outputs to CSV.
- Sweep `P_P` around the learned value `2*Mg_h+2*Sl_ch` and export target outputs to CSV.
- Sweep `Sl_ch` around the learned value `15[mm]` and export target outputs to CSV.
...
- Summary: 已学习 tubular_permanent_magnet_generator 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=16；内容证据=34 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 34 条；几何=8，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：tubular_permanent_magnet_generator.m, tubular_permanent_magnet_generator.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 tubular_permanent_magnet_generator，训练阶段为 modeling_logic_learning_ready。
...
- `tubular_permanent_magnet_generator.mph` (comsol_mph, 14349667 bytes)
- `tubular_permanent_magnet_generator.m` (matlab_livelink, 61063 bytes)
- `tubular_permanent_magnet_generator.java` (comsol_java, 58119 bytes)
- `tubular_permanent_magnet_generator_summary.json` (json, 2279 bytes)
- `models.acdc.tubular_permanent_magnet_generator.pdf` (pdf_document, 1756185 bytes)
