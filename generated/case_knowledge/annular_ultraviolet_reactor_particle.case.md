...
- Case directory: `D:\桌面\codex\案例下载\电气\带粒子追踪的环形紫外反应器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T15:20:33.371456+00:00`
...
- `r_lamp` = `1[cm]` - Lamp radius
- `r_reac` = `5[cm]` - Reactor radius
- `L_reac` = `100[cm]` - Reactor length
- `L_lamp` = `80[cm]` - Lamp length
- `d_lamp` = `L_reac-L_lamp` - Lamp displacement
- `mid_lamp` = `d_lamp+L_lamp/2` - Lamp midplane location
- `P` = `40[W]` - Total source power
- `r_inl` = `3[cm]` - Inlet radius
- `L_inl` = `30[cm]` - Inlet length
- `z_inl` = `5[cm]` - Inlet z-coordinate
- `k_inact` = `0.1[cm^2/mJ]` - Inactivation rate constant

## 内容级读取结果

- 模型树证据数量: 41
- 理论关键词: 结构力学, 传热, 流体
- geometry: cyl1 / Cylinder, cyl2 / Cylinder, cyl3 / Cylinder, uni1 / Union, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl2 / r, geom1 / cyl2 / h, geom1 / cyl2 / pos, geom1 / cyl2 / axis, geom1 / cyl3 / r, geom1 / cyl3 / h
- physics: Particle Tracing, Turbulent Flow, Ray Optics
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: cyl1 / r, cyl1 / h, cyl2 / r, cyl2 / h, cyl2 / pos, cyl2 / axis, cyl3 / r, cyl3 / h, cyl3 / pos, cyl3 / axis

### 文件内容证据

- `annular_ultraviolet_reactor_particle.java`: 参数=11, 几何=14, 物理场=0, 研究=0, 结果=0
- `models.roptics.annular_ultraviolet_reactor_particle.pdf`: 参数=0, 几何=0, 物理场=3, 研究=2, 结果=0
- `annular_ultraviolet_reactor_particle_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `annular_ultraviolet_reactor_particle.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `annular_ultraviolet_reactor_particle.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.roptics.annular_ultraviolet_reactor_particle.pdf`: models.roptics.annular_ultraviolet_reactor_particle.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Turbulent Flow、Particle Tracing、Ray Optics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The model consists of three studies. In the first study, the fluence rate in the annular region

## PDF Summary and Physics Judgement

- Simple summary: annular_ultraviolet_reactor_particle 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Turbulent Flow，并采用 Time Dependent 研究。
- Primary physics: `Turbulent Flow`
- Study type: `Time Dependent`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Turbulent Flow，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Time Dependent 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: The model consists of three studies. In the first study, the fluence rate in the annular region
- PDF: surrounding a UV lamp is solved for, using the Geometrical Optics physics interface. The
- PDF: In the second study, the velocity profile of water flowing through the reactor is solved for,
- 脚本/文本识别到的物理场: Particle Tracing, Turbulent Flow, Ray Optics
...
- PDF keywords: 结构力学, 传热, 流体, models.roptics.annular_ultraviolet_reactor_particle.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Turbulent Flow、Particle Tracing、Ray Optics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The model consists of three studies. In the first study, the fluence rate in the annular region
- `r_lamp` -> Lamp radius; MATLAB: `model.param.set('r_lamp', value, description)`
- `r_reac` -> Reactor radius; MATLAB: `model.param.set('r_reac', value, description)`
- `L_reac` -> Reactor length; MATLAB: `model.param.set('L_reac', value, description)`
- `L_lamp` -> Lamp length; MATLAB: `model.param.set('L_lamp', value, description)`
- `d_lamp` -> Lamp displacement; MATLAB: `model.param.set('d_lamp', value, description)`
- `mid_lamp` -> Lamp midplane location; MATLAB: `model.param.set('mid_lamp', value, description)`
- `P` -> Total source power; MATLAB: `model.param.set('P', value, description)`
- `r_inl` -> Inlet radius; MATLAB: `model.param.set('r_inl', value, description)`
- `L_inl` -> Inlet length; MATLAB: `model.param.set('L_inl', value, description)`
- `z_inl` -> Inlet z-coordinate; MATLAB: `model.param.set('z_inl', value, description)`
- `k_inact` -> Inactivation rate constant; MATLAB: `model.param.set('k_inact', value, description)`
- `Cylinder` -> 圆柱体、管道或三维钻孔; MATLAB: `model.component('comp1').geom('geom1').create(tag, 'Cylinder')`
...
- 已识别几何证据：cyl1 / Cylinder；cyl2 / Cylinder；cyl3 / Cylinder；uni1 / Union；geom1 / cyl1 / r；geom1 / cyl1 / h
- 已识别物理场证据：Particle Tracing；Turbulent Flow；Ray Optics
- 已识别参数：r_lamp、r_reac、L_reac、L_lamp、d_lamp、mid_lamp、P、r_inl、L_inl、z_inl、k_inact
...
- Transfer recognized physics interfaces to similar requirements: Particle Tracing, Turbulent Flow, Ray Optics.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 结构力学, 传热, 流体?
...
- Sweep `r_lamp` around the learned value `1[cm]` and export target outputs to CSV.
- Sweep `r_reac` around the learned value `5[cm]` and export target outputs to CSV.
- Sweep `L_reac` around the learned value `100[cm]` and export target outputs to CSV.
- Sweep `L_lamp` around the learned value `80[cm]` and export target outputs to CSV.
- Sweep `d_lamp` around the learned value `L_reac-L_lamp` and export target outputs to CSV.
- Sweep `mid_lamp` around the learned value `d_lamp+L_lamp/2` and export target outputs to CSV.
- Sweep `P` around the learned value `40[W]` and export target outputs to CSV.
- Sweep `r_inl` around the learned value `3[cm]` and export target outputs to CSV.
...
- Summary: 已学习 annular_ultraviolet_reactor_particle 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=41 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 41 条；几何=14，物理场=3，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：annular_ultraviolet_reactor_particle.m, annular_ultraviolet_reactor_particle.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 annular_ultraviolet_reactor_particle，训练阶段为 modeling_logic_learning_ready。
...
- `annular_ultraviolet_reactor_particle.mph` (comsol_mph, 170974248 bytes)
- `annular_ultraviolet_reactor_particle.m` (matlab_livelink, 40147 bytes)
- `annular_ultraviolet_reactor_particle.java` (comsol_java, 35770 bytes)
- `annular_ultraviolet_reactor_particle_summary.json` (json, 2075 bytes)
- `models.roptics.annular_ultraviolet_reactor_particle.pdf` (pdf_document, 748029 bytes)
