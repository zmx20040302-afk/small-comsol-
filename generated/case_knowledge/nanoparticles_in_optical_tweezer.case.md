# nanoparticles_in_optical_tweezer

- Case directory: `D:\桌面\codex\案例下载\电气\光镊中的纳米颗粒`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:28:49.586123+00:00`
...
- `lam0` = `1064[nm]` - Free space wavelength
- `n_medium` = `1.33` - Refractive index of medium (water)
- `n_particle` = `1.45` - Refractive index of SiO2 particle
- `k` = `2*pi*n_medium/lam0` - Propagation constant in medium
- `w0` = `0.37[um]` - Beam waist radius
- `E0` = `16.2e6[V/m]` - Electric field amplitude, adapted to give 100 mW power

## 内容级读取结果

- 模型树证据数量: 19
- 理论关键词: 电磁, 传热, 结构力学
- geometry: sph1 / Sphere, geom1 / sph1 / r, sph2 / Sphere, geom1 / sph1 / pos, geom1 / sph1 / layername
- physics: Particle Tracing, Electromagnetic Waves
- mesh: mesh1, mesh3
- studies: Frequency Domain
- boundary_conditions: sph1 / r, sph1 / pos, sph1 / layername

### 文件内容证据

- `nanoparticles_in_optical_tweezer.java`: 参数=6, 几何=5, 物理场=2, 研究=1, 结果=0
- `nanoparticles_in_optical_tweezer.m`: 参数=6, 几何=2, 物理场=2, 研究=1, 结果=0
- `Nanoparticles in Optical Tweezer.pptx`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `nanoparticles_in_optical_tweezer.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `nanoparticles_in_optical_tweezer_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: nanoparticles_in_optical_tweezer 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- 脚本/文本识别到的物理场: Particle Tracing, Electromagnetic Waves
...
- PDF keywords: 电磁, 传热, 结构力学
- `lam0` -> Free space wavelength; MATLAB: `model.param.set('lam0', value, description)`
- `n_medium` -> Refractive index of medium (water); MATLAB: `model.param.set('n_medium', value, description)`
- `n_particle` -> Refractive index of SiO2 particle; MATLAB: `model.param.set('n_particle', value, description)`
- `k` -> Propagation constant in medium; MATLAB: `model.param.set('k', value, description)`
- `w0` -> Beam waist radius; MATLAB: `model.param.set('w0', value, description)`
- `E0` -> Electric field amplitude, adapted to give 100 mW power; MATLAB: `model.param.set('E0', value, description)`
...
- 已识别几何证据：sph1 / Sphere；geom1 / sph1 / r；sph2 / Sphere；geom1 / sph1 / pos；geom1 / sph1 / layername
- 已识别物理场证据：Particle Tracing；Electromagnetic Waves
- 已识别参数：lam0、n_medium、n_particle、k、w0、E0
...
- Transfer recognized physics interfaces to similar requirements: Particle Tracing, Electromagnetic Waves.
...
- Sweep `lam0` around the learned value `1064[nm]` and export target outputs to CSV.
- Sweep `n_medium` around the learned value `1.33` and export target outputs to CSV.
- Sweep `n_particle` around the learned value `1.45` and export target outputs to CSV.
- Sweep `k` around the learned value `2*pi*n_medium/lam0` and export target outputs to CSV.
- Sweep `w0` around the learned value `0.37[um]` and export target outputs to CSV.
- Sweep `E0` around the learned value `16.2e6[V/m]` and export target outputs to CSV.
...
- Summary: 已学习 nanoparticles_in_optical_tweezer 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=19 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 19 条；几何=5，物理场=2，材料=0，网格=2，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：nanoparticles_in_optical_tweezer.m, nanoparticles_in_optical_tweezer.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 nanoparticles_in_optical_tweezer，训练阶段为 modeling_logic_learning_ready。
...
- `nanoparticles_in_optical_tweezer.mph` (comsol_mph, 459444431 bytes)
- `nanoparticles_in_optical_tweezer.m` (matlab_livelink, 73348 bytes)
- `nanoparticles_in_optical_tweezer.java` (comsol_java, 78981 bytes)
- `nanoparticles_in_optical_tweezer_summary.json` (json, 2215 bytes)
- `Nanoparticles in Optical Tweezer.pptx` (presentation_document, 30451077 bytes)
