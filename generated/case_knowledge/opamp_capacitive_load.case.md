...
- Case directory: `D:\桌面\codex\案例下载\电气\带电容负载的运算放大器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:02:35.644634+00:00`
...
- `OPAMP_RIN` = `100[kohm]` - Op-amp input resistance
- `OPAMP_GAIN` = `1e5` - Op-amp gain
- `CLOAD` = `10[nF]` - Capacitive load
- `R1` = `470[ohm]` - Feedback resistance 1
- `R2` = `4700[ohm]` - Feedback resistance 2
- `OPAMP_P` = `100[Hz]` - Op-amp pole frequency
- `OPAMP_ROUT` = `100[ohm]` - Op-amp output resistance
...
- `opamp_capacitive_load.java`: 参数=7, 几何=0, 物理场=1, 研究=0, 结果=0
- `models.acdc.opamp_capacitive_load.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `opamp_capacitive_load.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `opamp_capacitive_load.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `opamp_capacitive_load_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.opamp_capacitive_load.pdf`: models.acdc.opamp_capacitive_load.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；COMSOL Multiphysics. The latter is partially based on the SPICE format originally

## PDF Summary and Physics Judgement

- Simple summary: opamp_capacitive_load 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Circuit，并采用 Time Dependent 研究。
- Primary physics: `Circuit`
- Study type: `Time Dependent`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Circuit，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Time Dependent 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: COMSOL Multiphysics. The latter is partially based on the SPICE format originally
- PDF: Results and Discussion
- PDF: 2 In the Select Physics tree, select AC/DC > Electrical Circuit (cir).
- 脚本/文本识别到的物理场: cir / Circuit
...
- PDF keywords: 电磁, models.acdc.opamp_capacitive_load.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；COMSOL Multiphysics. The latter is partially based on the SPICE format originally
- `OPAMP_RIN` -> Op-amp input resistance; MATLAB: `model.param.set('OPAMP_RIN', value, description)`
- `OPAMP_GAIN` -> Op-amp gain; MATLAB: `model.param.set('OPAMP_GAIN', value, description)`
- `CLOAD` -> Capacitive load; MATLAB: `model.param.set('CLOAD', value, description)`
- `R1` -> Feedback resistance 1; MATLAB: `model.param.set('R1', value, description)`
- `R2` -> Feedback resistance 2; MATLAB: `model.param.set('R2', value, description)`
- `OPAMP_P` -> Op-amp pole frequency; MATLAB: `model.param.set('OPAMP_P', value, description)`
- `OPAMP_ROUT` -> Op-amp output resistance; MATLAB: `model.param.set('OPAMP_ROUT', value, description)`
...
- 已识别物理场证据：cir / Circuit
- 已识别参数：OPAMP_RIN、OPAMP_GAIN、CLOAD、R1、R2、OPAMP_P、OPAMP_ROUT
...
- Transfer recognized physics interfaces to similar requirements: cir / Circuit.
...
- Sweep `OPAMP_RIN` around the learned value `100[kohm]` and export target outputs to CSV.
- Sweep `OPAMP_GAIN` around the learned value `1e5` and export target outputs to CSV.
- Sweep `CLOAD` around the learned value `10[nF]` and export target outputs to CSV.
- Sweep `R1` around the learned value `470[ohm]` and export target outputs to CSV.
- Sweep `R2` around the learned value `4700[ohm]` and export target outputs to CSV.
- Sweep `OPAMP_P` around the learned value `100[Hz]` and export target outputs to CSV.
- Sweep `OPAMP_ROUT` around the learned value `100[ohm]` and export target outputs to CSV.
...
- Summary: 已学习 opamp_capacitive_load 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=9 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 9 条；几何=0，物理场=1，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：opamp_capacitive_load.m, opamp_capacitive_load.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 opamp_capacitive_load，训练阶段为 modeling_logic_learning_ready。
...
- `opamp_capacitive_load.mph` (comsol_mph, 569246 bytes)
- `opamp_capacitive_load.m` (matlab_livelink, 19536 bytes)
- `opamp_capacitive_load.java` (comsol_java, 12657 bytes)
- `opamp_capacitive_load_summary.json` (json, 870 bytes)
- `models.acdc.opamp_capacitive_load.pdf` (pdf_document, 186643 bytes)
