...
- Created at: `2026-08-22T06:48:21.866495+00:00`
...
- `del_sat` = `1/sqrt(pi*mu0_const*sig*f)`
- `sig` = `1e7[S/m]`
- `f` = `25[kHz]`
- `mur0` = `1000`
- `vel` = `10[mm/s]`
...
- `induction_heating_curie_movement_25khz.java`: 参数=8, 几何=1, 物理场=0, 研究=0, 结果=0
- `models.acdc.induction_heating_curie_movement.pdf`: 参数=0, 几何=0, 物理场=2, 研究=3, 结果=0
- `induction_heating_curie_movement_25khz_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `induction_heating_curie_movement_25khz.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `induction_heating_curie_movement_25khz.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: induction_heating_curie_movement_25khz 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF keywords: 传热, 结构力学, 流体, 电磁, models.acdc.induction_heating_curie_movement.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The problem is set up in a 3D modeling space and the geometry sequence is imported
- `del_sat` -> 案例参数/扫描变量; MATLAB: `model.param.set('del_sat', value, description)`
- `sig` -> 案例参数/扫描变量; MATLAB: `model.param.set('sig', value, description)`
- `f` -> 案例参数/扫描变量; MATLAB: `model.param.set('f', value, description)`
- `mur0` -> 案例参数/扫描变量; MATLAB: `model.param.set('mur0', value, description)`
- `vel` -> 案例参数/扫描变量; MATLAB: `model.param.set('vel', value, description)`
- `t` -> 案例参数/扫描变量; MATLAB: `model.param.set('t', value, description)`
- `Tc` -> 案例参数/扫描变量; MATLAB: `model.param.set('Tc', value, description)`
- `J0` -> 案例参数/扫描变量; MATLAB: `model.param.set('J0', value, description)`
...
- 已识别参数：del_sat、sig、f、mur0、vel、t、Tc、J0
...
- Sweep `f` around the learned value `25[kHz]` and export target outputs to CSV.
- Sweep `mur0` around the learned value `1000` and export target outputs to CSV.
- Sweep `vel` around the learned value `10[mm/s]` and export target outputs to CSV.
...
- Sweep `J0` around the learned value `14[A/mm^2]` and export target outputs to CSV.
...
- Summary: 已学习 induction_heating_curie_movement_25khz 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=15 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 15 条；几何=1，物理场=2，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：induction_heating_curie_movement_25khz.m, induction_heating_curie_movement_25khz.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 induction_heating_curie_movement_25khz，训练阶段为 modeling_logic_learning_ready。
...
- `induction_heating_curie_movement_25khz.mph` (comsol_mph, 720933819 bytes)
- `induction_heating_curie_movement_25khz.m` (matlab_livelink, 84915 bytes)
- `induction_heating_curie_movement_25khz.java` (comsol_java, 85904 bytes)
- `induction_heating_curie_movement_25khz_summary.json` (json, 2624 bytes)
