...
- Created at: `2026-08-20T10:23:26.666699+00:00`
...
- `solenoid.java`: 参数=8, 几何=0, 物理场=0, 研究=0, 结果=0
- `solenoid.m`: 参数=8, 几何=0, 物理场=0, 研究=0, 结果=0
- `solenoid.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `solenoid_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: solenoid 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF keywords: 电磁
- `m0` -> 案例参数/扫描变量; MATLAB: `model.param.set('m0', value, description)`
- `k0` -> 案例参数/扫描变量; MATLAB: `model.param.set('k0', value, description)`
- `u0` -> 案例参数/扫描变量; MATLAB: `model.param.set('u0', value, description)`
- `I0` -> 案例参数/扫描变量; MATLAB: `model.param.set('I0', value, description)`
- `Zpos` -> 案例参数/扫描变量; MATLAB: `model.param.set('Zpos', value, description)`
- `Rcoil` -> 案例参数/扫描变量; MATLAB: `model.param.set('Rcoil', value, description)`
...
- 已识别参数：m0、k0、u0、I0、Zpos、Rcoil、V0、N_turns
...
- Sweep `m0` around the learned value `250[g]` and export target outputs to CSV.
- Sweep `k0` around the learned value `20[N/cm]` and export target outputs to CSV.
- Sweep `u0` around the learned value `2[cm]` and export target outputs to CSV.
- Sweep `I0` around the learned value `1[A]` and export target outputs to CSV.
- Sweep `Zpos` around the learned value `0[m]` and export target outputs to CSV.
- Sweep `Rcoil` around the learned value `1[ohm]` and export target outputs to CSV.
- Sweep `V0` around the learned value `10[V]` and export target outputs to CSV.
- Sweep `N_turns` around the learned value `250` and export target outputs to CSV.
...
- Summary: 已学习 solenoid 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=8 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 8 条；几何=0，物理场=0，材料=0，网格=0，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：solenoid.m, solenoid.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 solenoid，训练阶段为 modeling_logic_learning_ready。
...
- `solenoid.mph` (comsol_mph, 44350114 bytes)
- `solenoid.m` (matlab_livelink, 62085 bytes)
- `solenoid.java` (comsol_java, 66863 bytes)
- `solenoid_summary.json` (json, 1359 bytes)
