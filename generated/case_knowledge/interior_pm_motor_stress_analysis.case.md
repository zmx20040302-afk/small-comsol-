...
- Case directory: `D:\桌面\codex\案例下载\电气\内置式永磁电机的电磁和机械分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:32:24.023179+00:00`
...
- `mag_h` = `2.5[mm]` - Magnet height
- `d_s` = `10[mm]` - Shaft diameter
- `d_r` = `60[mm]` - Rotor diameter
- `d_st` = `100[mm]` - Stator diameter
- `airgap` = `0.7[mm]` - Size of airgap
- `d_cont` = `60.7[mm]` - Diameter of the stator-rotor continuity interface
- `L` = `40[mm]` - Out-of-plane thickness of motor
- `w_rot` = `20000[rpm]` - Rotational speed
- `tl` = `mag_h/100` - Thickness of thin layer
- `f_el` = `w_rot*Np/2` - Electrical frequency
- `t_ramp` = `1/f_el/20` - Time for ramping speed
- `N_tsteps` = `72` - Number of time steps
- `Ipk` = `5[A]` - Phase current peak
- `init_ang` = `200[deg]` - Initial current angle for peak torque
...
- `interior_pm_motor_stress_analysis.java`: 参数=19, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.acdc.interior_pm_motor_stress_analysis.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `models.sme.interior_pm_motor_stress_analysis.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `interior_pm_motor_stress_analysis.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `interior_pm_motor_stress_analysis.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `interior_pm_motor_stress_analysis_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.interior_pm_motor_stress_analysis.pdf`: models.acdc.interior_pm_motor_stress_analysis.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；a mechanical and electromagnetic analysis of an IPM motor is demonstrated. The results
- `models.sme.interior_pm_motor_stress_analysis.pdf`: models.sme.interior_pm_motor_stress_analysis.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；a mechanical and electromagnetic analysis of an IPM motor is demonstrated. The results

## PDF Summary and Physics Judgement

- Simple summary: interior_pm_motor_stress_analysis 主要研究结构受力、变形或强度响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: a mechanical and electromagnetic analysis of an IPM motor is demonstrated. The results
- PDF: Figure 1: Geometry of the motor.
- PDF: Moving Mesh
...
- PDF keywords: 结构力学, 电磁, 流体, models.acdc.interior_pm_motor_stress_analysis.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；a mechanical and electromagnetic analysis of an IPM motor is demonstrated. The results, models.sme.interior_pm_motor_stress_analysis.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；a mechanical and electromagnetic analysis of an IPM motor is demonstrated. The results
...
- `mag_h` -> Magnet height; MATLAB: `model.param.set('mag_h', value, description)`
- `d_s` -> Shaft diameter; MATLAB: `model.param.set('d_s', value, description)`
- `d_r` -> Rotor diameter; MATLAB: `model.param.set('d_r', value, description)`
- `d_st` -> Stator diameter; MATLAB: `model.param.set('d_st', value, description)`
- `airgap` -> Size of airgap; MATLAB: `model.param.set('airgap', value, description)`
- `d_cont` -> Diameter of the stator-rotor continuity interface; MATLAB: `model.param.set('d_cont', value, description)`
- `L` -> Out-of-plane thickness of motor; MATLAB: `model.param.set('L', value, description)`
- `w_rot` -> Rotational speed; MATLAB: `model.param.set('w_rot', value, description)`
- `tl` -> Thickness of thin layer; MATLAB: `model.param.set('tl', value, description)`
- `f_el` -> Electrical frequency; MATLAB: `model.param.set('f_el', value, description)`
...
- 已识别参数：Np、Ns、mag_h、d_s、d_r、d_st、airgap、d_cont、L、w_rot、tl、f_el
...
- Sweep `mag_h` around the learned value `2.5[mm]` and export target outputs to CSV.
- Sweep `d_s` around the learned value `10[mm]` and export target outputs to CSV.
- Sweep `d_r` around the learned value `60[mm]` and export target outputs to CSV.
- Sweep `d_st` around the learned value `100[mm]` and export target outputs to CSV.
- Sweep `airgap` around the learned value `0.7[mm]` and export target outputs to CSV.
- Sweep `d_cont` around the learned value `60.7[mm]` and export target outputs to CSV.
...
- Summary: 已学习 interior_pm_motor_stress_analysis 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=19；内容证据=22 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 22 条；几何=0，物理场=1，材料=0，网格=0，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：interior_pm_motor_stress_analysis.m, interior_pm_motor_stress_analysis.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 interior_pm_motor_stress_analysis，训练阶段为 modeling_logic_learning_ready。
...
- `interior_pm_motor_stress_analysis.mph` (comsol_mph, 589388417 bytes)
- `interior_pm_motor_stress_analysis.m` (matlab_livelink, 102106 bytes)
- `interior_pm_motor_stress_analysis.java` (comsol_java, 96714 bytes)
- `interior_pm_motor_stress_analysis_summary.json` (json, 2410 bytes)
- `models.acdc.interior_pm_motor_stress_analysis.pdf` (pdf_document, 2298368 bytes)
- `models.sme.interior_pm_motor_stress_analysis.pdf` (pdf_document, 2298734 bytes)
