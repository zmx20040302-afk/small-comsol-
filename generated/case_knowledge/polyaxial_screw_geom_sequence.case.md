...
- `thread_l` = `11[mm]` - geometry input parameter
- `thread_r` = `1.75[mm]` - geometry input parameter
- `sagittal_angle` = `10[deg]` - geometry input parameter
- `transverse_angle` = `0[deg]` - geometry input parameter
- `head_r` = `3.5[mm]` - geometry input parameter
- `head_h` = `8[mm]` - geometry input parameter
- `hole_d` = `3.5[mm]` - geometry input parameter

## 内容级读取结果

- 模型树证据数量: 39
- 理论关键词: 传热, 流体, 电磁, 多孔介质/裂隙
- geometry: part1 / Part, wp1 / WorkPlane, pol1 / Polygon, fil1 / Fillet, boxsel1 / BoxSelection, part1 / wp1 / quickplane, part1 / wp1 / unite, part1 / wp1 / showworkplane, rev1 / Revolve, part1 / rev1 / angtype, part1 / rev1 / origfaces, part1 / rev1 / selrevolve
- physics: Electromagnetic Waves, Heat Transfer
- studies: Time Dependent, Frequency Domain
- boundary_conditions: wp1 / quickplane, wp1 / unite, wp1 / showworkplane, pol1 / source, pol1 / table, fil1 / radius, boxsel1 / entitydim, boxsel1 / xmin, boxsel1 / xmax, boxsel1 / ymin, boxsel1 / ymax, rev1 / angtype

### 文件内容证据

- `polyaxial_screw_geom_sequence.java`: 参数=7, 几何=13, 物理场=0, 研究=0, 结果=0
- `polyaxial_screw_geom_sequence.m`: 参数=7, 几何=8, 物理场=0, 研究=0, 结果=0
- `models.rf.mri_implant_heating.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `polyaxial_screw_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `polyaxial_screw_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: polyaxial_screw_geom_sequence 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF keywords: 传热, 流体, 电磁, 多孔介质/裂隙, models.rf.mri_implant_heating.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Electromagnetic Waves，研究类型线索为 Time Dependent、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；surroundings. As a result, the RF power absorption by the tissue, and thus the SAR, are
- `thread_l` -> geometry input parameter; MATLAB: `model.param.set('thread_l', value, description)`
- `thread_r` -> geometry input parameter; MATLAB: `model.param.set('thread_r', value, description)`
- `sagittal_angle` -> geometry input parameter; MATLAB: `model.param.set('sagittal_angle', value, description)`
- `transverse_angle` -> geometry input parameter; MATLAB: `model.param.set('transverse_angle', value, description)`
- `head_r` -> geometry input parameter; MATLAB: `model.param.set('head_r', value, description)`
- `head_h` -> geometry input parameter; MATLAB: `model.param.set('head_h', value, description)`
- `hole_d` -> geometry input parameter; MATLAB: `model.param.set('hole_d', value, description)`
...
- 已识别几何证据：part1 / Part；wp1 / WorkPlane；pol1 / Polygon；fil1 / Fillet；boxsel1 / BoxSelection；part1 / wp1 / quickplane
- 已识别物理场证据：Electromagnetic Waves；Heat Transfer
- 已识别参数：thread_l、thread_r、sagittal_angle、transverse_angle、head_r、head_h、hole_d
...
- Sweep `thread_l` around the learned value `11[mm]` and export target outputs to CSV.
- Sweep `thread_r` around the learned value `1.75[mm]` and export target outputs to CSV.
- Sweep `sagittal_angle` around the learned value `10[deg]` and export target outputs to CSV.
- Sweep `transverse_angle` around the learned value `0[deg]` and export target outputs to CSV.
- Sweep `head_r` around the learned value `3.5[mm]` and export target outputs to CSV.
- Sweep `head_h` around the learned value `8[mm]` and export target outputs to CSV.
- Sweep `hole_d` around the learned value `3.5[mm]` and export target outputs to CSV.
...
- Summary: 已学习 polyaxial_screw_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=39 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 39 条；几何=13，物理场=2，材料=0，网格=0，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：polyaxial_screw_geom_sequence.m, polyaxial_screw_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 polyaxial_screw_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `polyaxial_screw_geom_sequence.mph` (comsol_mph, 449984 bytes)
- `polyaxial_screw_geom_sequence.m` (matlab_livelink, 11964 bytes)
- `polyaxial_screw_geom_sequence.java` (comsol_java, 12385 bytes)
- `polyaxial_screw_geom_sequence_summary.json` (json, 500 bytes)
