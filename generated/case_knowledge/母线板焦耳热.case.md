...
- Created at: `2026-07-06T09:40:22.758879+00:00`
...
- physics: ec / ConductiveMedia, ht / HeatTransfer, Heat Transfer
- mesh: mesh1
...
- `models.mph.busbar_geom.zh_CN.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mph.busbar_box.pdf`: models.mph.busbar_box.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.busbar_geom.pdf`: models.mph.busbar_geom.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.busbar_geom.zh_CN.pdf`: models.mph.busbar_geom.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 母线板焦耳热 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- 脚本/文本识别到的物理场: ec / ConductiveMedia, ht / HeatTransfer, Heat Transfer
...
- 已识别物理场证据：ec / ConductiveMedia；ht / HeatTransfer；Heat Transfer
...
- Transfer recognized physics interfaces to similar requirements: ec / ConductiveMedia, ht / HeatTransfer, Heat Transfer.
...
- Summary: 已学习 母线板焦耳热 的 15 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=26 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 26 条；几何=7，物理场=3，材料=0，网格=1，研究=3，结果=0。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 母线板焦耳热，训练阶段为 modeling_logic_learning_ready。
...
- `母线板焦耳热` score=0.2986; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=14
- `管式反应器代理模型 App` score=0.1767; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `热执行器代理模型 App` score=0.149; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42

### Official Documentation Checks

- `IntroductionToLiveLinkForMATLAB.pdf` page 12; module=LiveLink_for_MATLAB; score=0.1275
- `IntroductionToLiveLinkForMATLAB.pdf` page 3; module=LiveLink_for_MATLAB; score=0.1223
- `IntroductionToCOMSOLMultiphysics.pdf` page 170; module=COMSOL_Multiphysics; score=0.1174
