# Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution

- Case directory: `D:\桌面\codex\案例下载\电气\相场法模拟点蚀与力学辅助腐蚀`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:24:32.658140+00:00`
...
- `cseq` = `71.44 [mol/L]` - Equilibrium concentration in the solid phase
- `cleq` = `0.57 [mol/L]` - Equilibrium concentration in the liquid phase
- `T` = `310.15 [K]` - Absolute temperature
- `Vm` = `1/cseq` - Molar volume of Mg alloy
- `tfinal` = `24[h]` - Final computational time
- `DMg_l` = `10^-10[m^2/s]` - Diffusion coefficient Mg2+ in the liquid phase
- `DMg_s` = `10^-13[m^2/s]` - Diffusion coefficient Mg2+ in the solid phase
- `gamma` = `0.5 [J/m^2]` - Interfacial energy
- `thickness_h` = `4 [um]` - Interface thickness
- `omega` = `3/4*gamma/thickness_h` - Height of the double well potential
- `kappa` = `3/2*gamma*thickness_h` - Phase field gradient energy coefficient
- `A` = `6e7[J/m^3]` - Free energy density curvature parameter
- `L0` = `2.3e-10 [m^3/(J*s)]` - Interfacial mobility coefficient without mechanical loading
- `max_time_step` = `50 [s]` - Maximum time step
- `sigmaY` = `138 [MPa]` - Yield strength
- `lambda` = `38 [GPa]`
- `mu` = `16.30 [GPa]`
- `E` = `mu*(lambda*3+2*mu)/(lambda+mu)` - Youngs modulus
- `epsilon_yield` = `sigmaY/E` - Initial yield strain
- `prescribed_disp` = `500[um]*epsilon_inf/100` - Prescribed tensile displacement
- `epsilon_inf` = `0.096` - Perscribed percent strain
- `eta` = `1e-6` - Damage tolerance
- `Qp_coeff` = `A/(omega*cseq^2)` - Coefficient for normalizing phase field potential
- `l_int` = `sqrt(2)*thickness_h` - Interface lenght
...
- `Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution.m`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion.pdf`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion.pdf`: Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics；核心内容可概括为： Tensile loading used to study；Geometric

## PDF Summary and Physics Judgement

- Simple summary: Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF:  Tensile loading used to study
- PDF: Geometric
- PDF:  Axisymmetric 2D geometry
- PDF: Results
...
- PDF keywords: 传热, 结构力学, Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics；核心内容可概括为： Tensile loading used to study；Geometric
- `cseq` -> Equilibrium concentration in the solid phase; MATLAB: `model.param.set('cseq', value, description)`
- `cleq` -> Equilibrium concentration in the liquid phase; MATLAB: `model.param.set('cleq', value, description)`
- `T` -> Absolute temperature; MATLAB: `model.param.set('T', value, description)`
- `Vm` -> Molar volume of Mg alloy; MATLAB: `model.param.set('Vm', value, description)`
- `tfinal` -> Final computational time; MATLAB: `model.param.set('tfinal', value, description)`
- `DMg_l` -> Diffusion coefficient Mg2+ in the liquid phase; MATLAB: `model.param.set('DMg_l', value, description)`
- `DMg_s` -> Diffusion coefficient Mg2+ in the solid phase; MATLAB: `model.param.set('DMg_s', value, description)`
- `gamma` -> Interfacial energy; MATLAB: `model.param.set('gamma', value, description)`
- `thickness_h` -> Interface thickness; MATLAB: `model.param.set('thickness_h', value, description)`
- `omega` -> Height of the double well potential; MATLAB: `model.param.set('omega', value, description)`
- `kappa` -> Phase field gradient energy coefficient; MATLAB: `model.param.set('kappa', value, description)`
- `A` -> Free energy density curvature parameter; MATLAB: `model.param.set('A', value, description)`
...
- 已识别参数：cseq、cleq、T、Vm、tfinal、DMg_l、DMg_s、gamma、thickness_h、omega、kappa、A
...
- Sweep `cseq` around the learned value `71.44 [mol/L]` and export target outputs to CSV.
- Sweep `cleq` around the learned value `0.57 [mol/L]` and export target outputs to CSV.
- Sweep `T` around the learned value `310.15 [K]` and export target outputs to CSV.
- Sweep `Vm` around the learned value `1/cseq` and export target outputs to CSV.
- Sweep `tfinal` around the learned value `24[h]` and export target outputs to CSV.
- Sweep `DMg_l` around the learned value `10^-10[m^2/s]` and export target outputs to CSV.
- Sweep `DMg_s` around the learned value `10^-13[m^2/s]` and export target outputs to CSV.
- Sweep `gamma` around the learned value `0.5 [J/m^2]` and export target outputs to CSV.
...
- Summary: 已学习 Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=24；内容证据=25 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 25 条；几何=0，物理场=1，材料=0，网格=0，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution.m, Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution，训练阶段为 modeling_logic_learning_ready。
...
- `Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution.mph` (comsol_mph, 2250173 bytes)
- `Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution.m` (matlab_livelink, 43635 bytes)
- `Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution.java` (comsol_java, 46319 bytes)
- `Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion_Cleared_Solution_summary.json` (json, 2346 bytes)
- `Phase_Field_Modeling_of_Pitting_and_Mechanically_Assisted_Corrosion.pdf` (pdf_document, 3150947 bytes)
