...
- Case directory: `D:\桌面\codex\案例下载\电气\含压电材料的多层壳`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:15:50.881217+00:00`
...
- 模型树证据数量: 20
- 理论关键词: 电磁, 结构力学, 声学
- geometry: wp1 / WorkPlane, r1 / Rectangle, arr1 / Array, geom1 / wp1 / unite, geom1 / arr1 / fullsize, geom1 / arr1 / displ
- physics: lshell / LayeredShell, Electric Currents
- materials: mat1, mat2, lmat1, llmat1
- mesh: mesh1
- studies: Stationary
- boundary_conditions: pzm1 / 1, pzm2 / 2, wp1 / unite, r1 / size, arr1 / fullsize, arr1 / displ

### 文件内容证据

- `piezoelectric_layered.java`: 参数=0, 几何=6, 物理场=1, 研究=0, 结果=0
- `models.acdc.piezoelectric_layered.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `models.compmat.piezoelectric_layered.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `models.mems.piezoelectric_layered.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `piezoelectric_layered.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoelectric_layered.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoelectric_layered_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.piezoelectric_layered.pdf`: models.acdc.piezoelectric_layered.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electric Currents，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the first case, the pole axis is normal to the shell surface, which results in a change in
- `models.compmat.piezoelectric_layered.pdf`: models.compmat.piezoelectric_layered.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electric Currents，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the first case, the pole axis is normal to the shell surface, which results in a change in
- `models.mems.piezoelectric_layered.pdf`: models.mems.piezoelectric_layered.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electric Currents，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the first case, the pole axis is normal to the shell surface, which results in a change in

## PDF Summary and Physics Judgement

- Simple summary: piezoelectric_layered 主要研究声压、模态或频域声学响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF: the first case, the pole axis is normal to the shell surface, which results in a change in
- PDF: are electric conductors, and the middle one is a piezoelectric material. The geometry of
- PDF: Figure 1: Layered shell geometry representation in 3D.
- 脚本/文本识别到的物理场: lshell / LayeredShell, Electric Currents
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 结构力学, 声学
...
- PDF keywords: 电磁, 结构力学, 声学, models.acdc.piezoelectric_layered.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electric Currents，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the first case, the pole axis is normal to the shell surface, which results in a change in, models.compmat.piezoelectric_layered.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electric Currents，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the first case, the pole axis is normal to the shell surface, which results in a change in, models.mems.piezoelectric_layered.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electric Currents，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the first case, the pole axis is normal to the shell surface, which results in a change in
...
- 已识别几何证据：wp1 / WorkPlane；r1 / Rectangle；arr1 / Array；geom1 / wp1 / unite；geom1 / arr1 / fullsize；geom1 / arr1 / displ
- 已识别物理场证据：lshell / LayeredShell；Electric Currents
...
- Transfer recognized physics interfaces to similar requirements: lshell / LayeredShell, Electric Currents.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 结构力学, 声学?
...
- Summary: 已学习 piezoelectric_layered 的 7 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=20 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=6，物理场=2，材料=4，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：piezoelectric_layered.m, piezoelectric_layered.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 piezoelectric_layered，训练阶段为 modeling_logic_learning_ready。
...
- `piezoelectric_layered.mph` (comsol_mph, 9626774 bytes)
- `piezoelectric_layered.m` (matlab_livelink, 27566 bytes)
- `piezoelectric_layered.java` (comsol_java, 24926 bytes)
- `piezoelectric_layered_summary.json` (json, 1607 bytes)
- `models.acdc.piezoelectric_layered.pdf` (pdf_document, 793026 bytes)
- `models.compmat.piezoelectric_layered.pdf` (pdf_document, 792040 bytes)
- `models.mems.piezoelectric_layered.pdf` (pdf_document, 791988 bytes)
