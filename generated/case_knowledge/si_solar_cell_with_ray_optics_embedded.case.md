# si_solar_cell_with_ray_optics_embedded

- Case directory: `D:\桌面\codex\案例下载\电气\基于射线光学模拟硅太阳能电池`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:33:54.284168+00:00`
...
- `R` = `0` - Reflectance of a perfect anti-reflective coating
- `ld` = `0.5[um]` - Junction depth
- `R_series` = `0[ohm]` - Series resistance
- `AU` = `1.496E11[m]` - Astronomical units
- `Area` = `1[cm^2]` - Cell exposed surface area in cm^2
- `tmax` = `1[m]/c_const` - Simulation time
- `lcell` = `10[um]` - Simulation thickness
- `H0` = `1353[W/m^2]` - Reference irradiance (solar constant)
- `R_shunt` = `1e12[ohm]` - Shunt tesistance
- `p_doping` = `1e19[1/cm^3]` - Peak emitter doping, p-type
- `T` = `300[K]` - Cell temperature
- `aziCell` = `5` - Cell azimuth angle in degree
- `Tsun` = `5777[K]` - Effective temperature of the Sun
- `n_doping` = `5e15[1/cm^3]` - Uniform base doping, n-type
- `nbrsec` = `60*60*24` - The number of second in one day
- `altitude` = `620[m]` - Altitude in  m
- `Rsun` = `695700000[m]` - Radius of the Sun in m
- `tiltCell` = `8` - Cell tilt angle in degree
- `Hsun` = `sigma_const*Tsun^4` - Blackbody irradiance of the Sun
...
- `si_solar_cell_with_ray_optics_embedded.java`: 参数=19, 几何=0, 物理场=0, 研究=0, 结果=0
- `applications.si_solar_cell_with_ray_optics.pdf`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `si_solar_cell_with_ray_optics_embedded.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `si_solar_cell_with_ray_optics_embedded_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `si_solar_cell_with_ray_optics_embedded.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `applications.si_solar_cell_with_ray_optics.pdf`: applications.si_solar_cell_with_ray_optics.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics；核心内容可概括为：Application created in COMSOL Multiphysics 6.4；Cell results are only available after the Cell button has been

## PDF Summary and Physics Judgement

- Simple summary: si_solar_cell_with_ray_optics_embedded 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF: Cell results are only available after the Cell button has been
- PDF: pressed. Once the model has been computed, a variety of cell-related results are displayed.
- PDF: changed, or if there is no solution stored in the application file. The Results ribbon section
...
- PDF keywords: 传热, 电磁, applications.si_solar_cell_with_ray_optics.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics；核心内容可概括为：Application created in COMSOL Multiphysics 6.4；Cell results are only available after the Cell button has been
- `R` -> Reflectance of a perfect anti-reflective coating; MATLAB: `model.param.set('R', value, description)`
- `ld` -> Junction depth; MATLAB: `model.param.set('ld', value, description)`
- `R_series` -> Series resistance; MATLAB: `model.param.set('R_series', value, description)`
- `AU` -> Astronomical units; MATLAB: `model.param.set('AU', value, description)`
- `Area` -> Cell exposed surface area in cm^2; MATLAB: `model.param.set('Area', value, description)`
- `tmax` -> Simulation time; MATLAB: `model.param.set('tmax', value, description)`
- `lcell` -> Simulation thickness; MATLAB: `model.param.set('lcell', value, description)`
- `H0` -> Reference irradiance (solar constant); MATLAB: `model.param.set('H0', value, description)`
- `R_shunt` -> Shunt tesistance; MATLAB: `model.param.set('R_shunt', value, description)`
- `p_doping` -> Peak emitter doping, p-type; MATLAB: `model.param.set('p_doping', value, description)`
- `T` -> Cell temperature; MATLAB: `model.param.set('T', value, description)`
- `aziCell` -> Cell azimuth angle in degree; MATLAB: `model.param.set('aziCell', value, description)`
...
- 已识别物理场证据：Ray Optics
- 已识别参数：R、ld、R_series、AU、Area、tmax、lcell、H0、R_shunt、p_doping、T、aziCell
...
- Sweep `R` around the learned value `0` and export target outputs to CSV.
- Sweep `ld` around the learned value `0.5[um]` and export target outputs to CSV.
- Sweep `R_series` around the learned value `0[ohm]` and export target outputs to CSV.
- Sweep `AU` around the learned value `1.496E11[m]` and export target outputs to CSV.
- Sweep `Area` around the learned value `1[cm^2]` and export target outputs to CSV.
- Sweep `tmax` around the learned value `1[m]/c_const` and export target outputs to CSV.
- Sweep `lcell` around the learned value `10[um]` and export target outputs to CSV.
- Sweep `H0` around the learned value `1353[W/m^2]` and export target outputs to CSV.
...
- Summary: 已学习 si_solar_cell_with_ray_optics_embedded 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=19；内容证据=20 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=0，物理场=1，材料=0，网格=0，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：si_solar_cell_with_ray_optics_embedded.m, si_solar_cell_with_ray_optics_embedded.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 si_solar_cell_with_ray_optics_embedded，训练阶段为 modeling_logic_learning_ready。
...
- `si_solar_cell_with_ray_optics_embedded.mph` (comsol_mph, 10018714 bytes)
- `si_solar_cell_with_ray_optics_embedded.m` (matlab_livelink, 63632 bytes)
- `si_solar_cell_with_ray_optics_embedded.java` (comsol_java, 59016 bytes)
- `si_solar_cell_with_ray_optics_embedded_summary.json` (json, 2382 bytes)
- `applications.si_solar_cell_with_ray_optics.pdf` (pdf_document, 248226 bytes)
