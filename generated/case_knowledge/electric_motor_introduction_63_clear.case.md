# electric_motor_introduction_63_clear

- Case directory: `D:\桌面\codex\案例下载\电气\电动机和发电机的设计分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:27:03.565292+00:00`
...
- `init_ang` = `32[deg]` - Initial mechanical angle
- `Np` = `10` - Number of poles
- `w_rot` = `60[rpm]` - Rotational speed
- `I0` = `10[A]` - Peak current
- `f_el` = `w_rot*Np/2` - Electrical frequency
- `Ns` = `12` - Number of slots
- `nturn` = `10` - Number of turns
- `coil_fillingfactor` = `0.8` - Slot filling factor
- `N_slots` = `12` - geometry input parameter
- `N_modeled_slots` = `12` - geometry input parameter
- `diam_stator` = `35[mm]` - geometry input parameter
- `th_backiron` = `1[mm]` - geometry input parameter
- `h_tooth` = `7[mm]` - geometry input parameter
- `h_shoe` = `1[mm]` - geometry input parameter
- `w_tooth` = `3[mm]` - geometry input parameter
- `w_shoe` = `4.5[mm]` - geometry input parameter
- `f_shoe` = `0.5[mm]` - geometry input parameter
- `extra_external_air` = `6[mm]` - geometry input parameter
- `diam_internal_air` = `18.5[mm]` - geometry input parameter
- `slot_filling_type` = `2` - geometry input parameter
- `smallest_detail` = `0.25[mm]` - geometry input parameter

## 内容级读取结果

- 模型树证据数量: 26
- 理论关键词: 电磁
- geometry: extStat1 / Part, intRot1 / Part
- mesh: mesh1

### 文件内容证据

- `electric_motor_introduction_63_clear.m`: 参数=20, 几何=2, 物理场=0, 研究=0, 结果=0
- `electric_motor_introduction_63_clear.java`: 参数=20, 几何=2, 物理场=0, 研究=0, 结果=0
- `electric_motor_introduction_63_clear.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `electric_motor_introduction_63_clear_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: electric_motor_introduction_63_clear 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `nturn` -> Number of turns; MATLAB: `model.param.set('nturn', value, description)`
- `coil_fillingfactor` -> Slot filling factor; MATLAB: `model.param.set('coil_fillingfactor', value, description)`
- `N_slots` -> geometry input parameter; MATLAB: `model.param.set('N_slots', value, description)`
- `N_modeled_slots` -> geometry input parameter; MATLAB: `model.param.set('N_modeled_slots', value, description)`
...
- 已识别几何证据：extStat1 / Part；intRot1 / Part
- 已识别参数：L、iron_th、init_ang、Np、w_rot、I0、f_el、Ns、nturn、coil_fillingfactor、N_slots、N_modeled_slots
...
- Sweep `init_ang` around the learned value `32[deg]` and export target outputs to CSV.
- Sweep `Np` around the learned value `10` and export target outputs to CSV.
- Sweep `w_rot` around the learned value `60[rpm]` and export target outputs to CSV.
...
- Summary: 已学习 electric_motor_introduction_63_clear 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=23；内容证据=26 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 26 条；几何=2，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：electric_motor_introduction_63_clear.m, electric_motor_introduction_63_clear.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 electric_motor_introduction_63_clear，训练阶段为 modeling_logic_learning_ready。
...
- `electric_motor_introduction_63_clear.mph` (comsol_mph, 1350064 bytes)
- `electric_motor_introduction_63_clear.m` (matlab_livelink, 150493 bytes)
- `electric_motor_introduction_63_clear.java` (comsol_java, 161669 bytes)
- `electric_motor_introduction_63_clear_summary.json` (json, 2148 bytes)
