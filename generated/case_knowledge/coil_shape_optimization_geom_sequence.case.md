# coil_shape_optimization_geom_sequence

- Case directory: `D:\桌面\codex\案例下载\电气\线圈形状优化`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:00:39.820156+00:00`
...
- `R1` = `45[cm]` - Domain radius
- `infR` = `10[cm]` - Infinite domain radius
- `coilWidth` = `1[cm]` - Coil width
- `coilHeight` = `2[cm]` - Coil height
- `R2` = `6[cm]` - Coil inner radius
- `nCoil` = `3` - Number of coils
- `coilSpace` = `1.5[cm]` - Coil spacing
- `objHeight` = `33[cm]` - Objective domain height
- `objR` = `2[cm]` - Objective domain radius
- `objOuter` = `6.5[cm]` - Outer objective domain
- `objSpace` = `1.5[cm]` - Objective domain spacing
- `deformR` = `13[cm]` - Deforming domain width
- `deformHeight` = `40[cm]` - Deforming domain height
- `tTube` = `2[mm]` - Tube thickness
- `objInner` = `objHeight-2*objSpace-2*objOuter` - Inner objective domain

## 内容级读取结果

- 模型树证据数量: 46
- 理论关键词: 优化/参数扫描, 电磁, 结构力学
- geometry: c1 / Circle, r1 / Rectangle, r2 / Rectangle, dif1 / Difference, geom1 / c1 / r, geom1 / c1 / angle, geom1 / c1 / rot, geom1 / c1 / layername, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r1 / selresult, geom1 / r2 / size
- physics: Optimization, Magnetic Fields
- mesh: mesh1
- studies: Optimization, Frequency Domain
- boundary_conditions: c1 / r, c1 / angle, c1 / rot, c1 / layername, r1 / size, r1 / pos, r1 / selresult, r2 / size, r2 / pos, r2 / selresult, dif1 / selresult

### 文件内容证据

- `coil_shape_optimization_geom_sequence.m`: 参数=15, 几何=15, 物理场=1, 研究=1, 结果=0
- `coil_shape_optimization_geom_sequence.java`: 参数=15, 几何=12, 物理场=1, 研究=1, 结果=0
- `models.acdc.coil_shape_optimization.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `models.opt.coil_shape_optimization.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `Shape Optimization of Coils.pptx`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `coil_shape_optimization_geom_sequence.mph`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `coil_shape_optimization_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.coil_shape_optimization.pdf`: models.acdc.coil_shape_optimization.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Optimization，研究类型线索为 Frequency Domain、Optimization；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This model demonstrates how to design a coil geometry using gradient-based shape
- `models.opt.coil_shape_optimization.pdf`: models.opt.coil_shape_optimization.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Optimization，研究类型线索为 Frequency Domain、Optimization；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This model demonstrates how to design a coil geometry using gradient-based shape

## PDF Summary and Physics Judgement

- Simple summary: coil_shape_optimization_geom_sequence 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Frequency Domain 研究。
- Primary physics: `Solid Mechanics`
- Study type: `Frequency Domain`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Solid Mechanics，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Frequency Domain 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: This model demonstrates how to design a coil geometry using gradient-based shape
- PDF: use the optimization result as inspiration for a 3D design.
- PDF: the effect of having coils with partial turns. The result of the shape optimization is shown
- 脚本/文本识别到的物理场: Optimization, Magnetic Fields
- 识别到的研究类型: Optimization, Frequency Domain
- 理论关键词: 优化/参数扫描, 电磁, 结构力学

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 优化/参数扫描, 电磁, 结构力学, models.acdc.coil_shape_optimization.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Optimization，研究类型线索为 Frequency Domain、Optimization；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This model demonstrates how to design a coil geometry using gradient-based shape, models.opt.coil_shape_optimization.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Optimization，研究类型线索为 Frequency Domain、Optimization；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This model demonstrates how to design a coil geometry using gradient-based shape
- `R1` -> Domain radius; MATLAB: `model.param.set('R1', value, description)`
- `infR` -> Infinite domain radius; MATLAB: `model.param.set('infR', value, description)`
- `coilWidth` -> Coil width; MATLAB: `model.param.set('coilWidth', value, description)`
- `coilHeight` -> Coil height; MATLAB: `model.param.set('coilHeight', value, description)`
- `R2` -> Coil inner radius; MATLAB: `model.param.set('R2', value, description)`
- `nCoil` -> Number of coils; MATLAB: `model.param.set('nCoil', value, description)`
- `coilSpace` -> Coil spacing; MATLAB: `model.param.set('coilSpace', value, description)`
- `objHeight` -> Objective domain height; MATLAB: `model.param.set('objHeight', value, description)`
- `objR` -> Objective domain radius; MATLAB: `model.param.set('objR', value, description)`
- `objOuter` -> Outer objective domain; MATLAB: `model.param.set('objOuter', value, description)`
- `objSpace` -> Objective domain spacing; MATLAB: `model.param.set('objSpace', value, description)`
- `deformR` -> Deforming domain width; MATLAB: `model.param.set('deformR', value, description)`
...
- 已识别几何证据：c1 / Circle；r1 / Rectangle；r2 / Rectangle；dif1 / Difference；geom1 / c1 / r；geom1 / c1 / angle
- 已识别物理场证据：Optimization；Magnetic Fields
- 已识别参数：R1、infR、coilWidth、coilHeight、R2、nCoil、coilSpace、objHeight、objR、objOuter、objSpace、deformR
...
- Use the learned study types as solver starting points: Optimization, Frequency Domain.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 优化/参数扫描, 电磁, 结构力学?
...
- Sweep `R1` around the learned value `45[cm]` and export target outputs to CSV.
- Sweep `infR` around the learned value `10[cm]` and export target outputs to CSV.
- Sweep `coilWidth` around the learned value `1[cm]` and export target outputs to CSV.
- Sweep `coilHeight` around the learned value `2[cm]` and export target outputs to CSV.
- Sweep `R2` around the learned value `6[cm]` and export target outputs to CSV.
- Sweep `nCoil` around the learned value `3` and export target outputs to CSV.
- Sweep `coilSpace` around the learned value `1.5[cm]` and export target outputs to CSV.
- Sweep `objHeight` around the learned value `33[cm]` and export target outputs to CSV.
...
- Summary: 已学习 coil_shape_optimization_geom_sequence 的 7 个文件；训练阶段=modeling_logic_learning_ready；参数数量=15；内容证据=46 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 46 条；几何=15，物理场=2，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：coil_shape_optimization_geom_sequence.m, coil_shape_optimization_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 coil_shape_optimization_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `coil_shape_optimization_geom_sequence.mph` (comsol_mph, 410067 bytes)
- `coil_shape_optimization_geom_sequence.m` (matlab_livelink, 13481 bytes)
- `coil_shape_optimization_geom_sequence.java` (comsol_java, 14113 bytes)
- `coil_shape_optimization_geom_sequence_summary.json` (json, 1050 bytes)
- `Shape Optimization of Coils.pptx` (presentation_document, 7361022 bytes)
- `models.acdc.coil_shape_optimization.pdf` (pdf_document, 466131 bytes)
- `models.opt.coil_shape_optimization.pdf` (pdf_document, 465883 bytes)
