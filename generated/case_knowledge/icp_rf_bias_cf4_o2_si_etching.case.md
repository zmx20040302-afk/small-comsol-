...
- Case directory: `D:\桌面\codex\案例下载\电气\带射频偏压的 CF4O2 电感耦合等离子体反应器模型（用于硅刻蚀）`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:25:54.949612+00:00`
...
- `Coil_chamber_height` = `7[cm]` - Coil chamber height
- `Coil_height` = `0.9[cm]` - Coil height
- `Coil_width` = `1.2[cm]` - Coil width
- `Coils_spacing` = `3.5[cm]` - Spacing between coils
- `Dh` = `2[cm]+0.2[cm]` - Dielectric block height
- `Dw` = `3[cm]` - Dielectric block width
- `First_coil_r` = `2[cm]` - First coil radial position
- `First_coil_z` = `7[mm]` - First coil's distance above the window
- `Hc` = `13[cm]` - Plasma chamber height
- `Inh` = `3[cm]` - Inlet block height
- `Inw` = `1[cm]` - Inlet block width
- `MeshRef_h` = `1.5[cm]` - Mesh refinement height
- `MeshRef_r` = `Ww-0.15[cm]` - Mesh refinement radial coordinate
- `p0` = `15[mTorr]` - Pressure
- `Pw` = `100[W]` - Input power
- `Qf` = `50` - Mass flow in SCCM
- `recF` = `0.005` - F recombination
- `recO` = `0.005` - O recombination
- `Vrf` = `0[V]` - RF bias voltage
- `Wc` = `25[cm]` - Plasma chamber width
- `Wh` = `0.2[cm]` - Wafer height
- `Whh` = `2[cm]` - Wafer holder height
- `Whw` = `15[cm]+0.2[cm]` - Wafer holder width
- `Window_thickness` = `2[cm]` - Window thickness
- `Ww` = `15[cm]` - Wafer width
- `Wz` = `2[cm]` - Wafer z-coordinate
- `xinlet` = `1e-6` - Mole fraction of remaining species at the inlet
- `xO2` = `0.2` - Oxygen mole fraction

## 内容级读取结果

- 模型树证据数量: 43
- 理论关键词: 流体, 电磁, 传热, 结构力学, 优化/参数扫描
- geometry: r1 / Rectangle, r2 / Rectangle, geom1 / r1 / size, geom1 / r2 / size
- physics: Electromagnetic Waves, Heat Transfer, Laminar Flow, Magnetic Fields, Particle Tracing
- mesh: mesh1
- studies: Stationary, Frequency Domain, Parametric Sweep
- boundary_conditions: r1 / size, r2 / size

### 文件内容证据

- `icp_rf_bias_cf4_o2_si_etching.java`: 参数=20, 几何=4, 物理场=0, 研究=0, 结果=0
- `models.plasma.icp_rf_bias_cf4_o2_si_etching.pdf`: 参数=0, 几何=0, 物理场=4, 研究=3, 结果=0
- `icp_rf_bias_cf4_o2_si_etching.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `icp_rf_bias_cf4_o2_si_etching_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `icp_rf_bias_cf4_o2_si_etching.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.icp_rf_bias_cf4_o2_si_etching.pdf`: models.plasma.icp_rf_bias_cf4_o2_si_etching.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This study presents a computational model for silicon etching in an inductively coupled

## PDF Summary and Physics Judgement

- Simple summary: icp_rf_bias_cf4_o2_si_etching 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
- Primary physics: `Laminar Flow`
- Study type: `Stationary`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Laminar Flow，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Stationary 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: This study presents a computational model for silicon etching in an inductively coupled
- PDF: The model integrates multiple physics interfaces, including Magnetic Fields, Laminar
- PDF: In particular, this model assumes that the heavy species only exist in the Base geometry.
- 脚本/文本识别到的物理场: Electromagnetic Waves, Heat Transfer, Laminar Flow, Magnetic Fields, Particle Tracing
- 识别到的研究类型: Stationary, Frequency Domain, Parametric Sweep
- 理论关键词: 流体, 电磁, 传热, 结构力学, 优化/参数扫描

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 流体, 电磁, 传热, 结构力学, 优化/参数扫描, models.plasma.icp_rf_bias_cf4_o2_si_etching.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This study presents a computational model for silicon etching in an inductively coupled
- `Coil_chamber_height` -> Coil chamber height; MATLAB: `model.param.set('Coil_chamber_height', value, description)`
- `Coil_height` -> Coil height; MATLAB: `model.param.set('Coil_height', value, description)`
- `Coil_width` -> Coil width; MATLAB: `model.param.set('Coil_width', value, description)`
- `Coils_spacing` -> Spacing between coils; MATLAB: `model.param.set('Coils_spacing', value, description)`
- `Dh` -> Dielectric block height; MATLAB: `model.param.set('Dh', value, description)`
- `Dw` -> Dielectric block width; MATLAB: `model.param.set('Dw', value, description)`
- `First_coil_r` -> First coil radial position; MATLAB: `model.param.set('First_coil_r', value, description)`
- `First_coil_z` -> First coil's distance above the window; MATLAB: `model.param.set('First_coil_z', value, description)`
- `Hc` -> Plasma chamber height; MATLAB: `model.param.set('Hc', value, description)`
- `Inh` -> Inlet block height; MATLAB: `model.param.set('Inh', value, description)`
- `Inw` -> Inlet block width; MATLAB: `model.param.set('Inw', value, description)`
- `MeshRef_h` -> Mesh refinement height; MATLAB: `model.param.set('MeshRef_h', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；geom1 / r1 / size；geom1 / r2 / size
- 已识别物理场证据：Electromagnetic Waves；Heat Transfer；Laminar Flow；Magnetic Fields；Particle Tracing
- 已识别参数：Coil_chamber_height、Coil_height、Coil_width、Coils_spacing、Dh、Dw、First_coil_r、First_coil_z、Hc、Inh、Inw、MeshRef_h
...
- 本案例已提取 28 个参数，可作为约束 JSON 和参数扫描变量。
...
- 识别到物理场证据：Laminar Flow
- 识别到物理场证据：Magnetic Fields
- 识别到物理场证据：Particle Tracing
...
- Transfer recognized physics interfaces to similar requirements: Electromagnetic Waves, Heat Transfer, Laminar Flow, Magnetic Fields, Particle Tracing.
- Reuse geometry construction patterns when new requirements share shape, import, array, sweep, or boolean operations.
- Use the learned study types as solver starting points: Stationary, Frequency Domain, Parametric Sweep.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 电磁, 传热, 结构力学, 优化/参数扫描?
...
- Sweep `Coil_chamber_height` around the learned value `7[cm]` and export target outputs to CSV.
- Sweep `Coil_height` around the learned value `0.9[cm]` and export target outputs to CSV.
- Sweep `Coil_width` around the learned value `1.2[cm]` and export target outputs to CSV.
- Sweep `Coils_spacing` around the learned value `3.5[cm]` and export target outputs to CSV.
- Sweep `Dh` around the learned value `2[cm]+0.2[cm]` and export target outputs to CSV.
- Sweep `Dw` around the learned value `3[cm]` and export target outputs to CSV.
- Sweep `First_coil_r` around the learned value `2[cm]` and export target outputs to CSV.
- Sweep `First_coil_z` around the learned value `7[mm]` and export target outputs to CSV.
- Sweep heat source, heat-transfer coefficient, thermal conductivity, and ambient temperature.
- Sweep inlet velocity, viscosity, density, channel size, and pressure boundary values.
...
- Summary: 已学习 icp_rf_bias_cf4_o2_si_etching 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=28；内容证据=43 条。
...
- 28 extracted parameters with values/descriptions.
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 43 条；几何=4，物理场=5，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：icp_rf_bias_cf4_o2_si_etching.m, icp_rf_bias_cf4_o2_si_etching.java
  - Judgement: 脚本中的 COMSOL API 调用可用于恢复模型树顺序，并辅助生成 LiveLink MATLAB 自动建模脚本。
- **4. 参数与约束提取**
  - Evidence: 从文本、MATLAB 或 Java 证据中提取到 28 个参数。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 icp_rf_bias_cf4_o2_si_etching，训练阶段为 modeling_logic_learning_ready。
...
- `icp_rf_bias_cf4_o2_si_etching.mph` (comsol_mph, 92870327 bytes)
- `icp_rf_bias_cf4_o2_si_etching.m` (matlab_livelink, 341733 bytes)
- `icp_rf_bias_cf4_o2_si_etching.java` (comsol_java, 337053 bytes)
- `icp_rf_bias_cf4_o2_si_etching_summary.json` (json, 3585 bytes)
- `models.plasma.icp_rf_bias_cf4_o2_si_etching.pdf` (pdf_document, 837011 bytes)
