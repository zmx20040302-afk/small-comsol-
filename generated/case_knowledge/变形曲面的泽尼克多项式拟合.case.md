...
- Created at: `2026-07-06T09:37:38.129169+00:00`
...
- `deformed_mirror_zernike_fitting.java`: 参数=4, 几何=3, 物理场=1, 研究=3, 结果=0
- `deformed_mirror_zernike_fitting.mph`: 参数=4, 几何=3, 物理场=1, 研究=3, 结果=0
- `deformed_mirror_zernike_fitting.m`: 参数=4, 几何=3, 物理场=1, 研究=2, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: 变形曲面的泽尼克多项式拟合 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- `Rc` -> 案例参数/扫描变量; MATLAB: `model.param.set('Rc', value, description)`
- `Hc` -> 案例参数/扫描变量; MATLAB: `model.param.set('Hc', value, description)`
- `xc` -> 案例参数/扫描变量; MATLAB: `model.param.set('xc', value, description)`
- `yc` -> 案例参数/扫描变量; MATLAB: `model.param.set('yc', value, description)`
...
- Sweep `Rc` around the learned value `2[cm]` and export target outputs to CSV.
- Sweep `Hc` around the learned value `1[cm]` and export target outputs to CSV.
- Sweep `xc` around the learned value `0` and export target outputs to CSV.
- Sweep `yc` around the learned value `0[cm]` and export target outputs to CSV.
...
- Summary: 已学习 变形曲面的泽尼克多项式拟合 的 3 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=14 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 14 条；几何=3，物理场=1，材料=0，网格=1，研究=3，结果=0。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 变形曲面的泽尼克多项式拟合，训练阶段为 modeling_logic_learning_ready。
...
- `管式反应器代理模型 App` score=0.1786; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `热执行器代理模型 App` score=0.1526; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42
- `热执行器` score=0.1358; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=30

### Official Documentation Checks

- `IntroductionToCOMSOLMultiphysics.pdf` page 254; module=COMSOL_Multiphysics; score=0.1199
- `IntroductionToLiveLinkForMATLAB.pdf` page 5; module=LiveLink_for_MATLAB; score=0.1114
