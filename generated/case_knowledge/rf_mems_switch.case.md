...
- Case directory: `D:\桌面\codex\案例下载\电气\RF MEMS 开关的吸合`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:10:07.119830+00:00`
...
- `V0` = `1[mV]` - Initial voltage
- `Vstep` = `5[V]` - Voltage step
- `insheight` = `100[nm]` - Insulator height
- `airheight` = `900[nm]` - Air height
- `en` = `1e15[Pa/m]` - Spring stiffness
- `tn` = `5e5[Pa]` - Contact force
...
- boundary_conditions: wp1 / unite, r1 / size, r1 / pos, r2 / size, r2 / pos, r3 / size

### 文件内容证据

- `rf_mems_switch.java`: 参数=6, 几何=5, 物理场=0, 研究=0, 结果=0
- `models.mems.rf_mems_switch.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `rf_mems_switch.pptx`: 参数=0, 几何=0, 物理场=2, 研究=0, 结果=0
- `rf_mems_switch.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rf_mems_switch.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rf_mems_switch_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.rf_mems_switch.pdf`: models.mems.rf_mems_switch.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；across the switch, causing the bridge to collapse onto the dielectric layer with a resulting

## PDF Summary and Physics Judgement

- Simple summary: rf_mems_switch 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: across the switch, causing the bridge to collapse onto the dielectric layer with a resulting
- PDF: Electromechanics multiphysics interface to handle both conducting and
- PDF: Figure 1 shows the device geometry. The switch consists of a square polysilicon plate
...
- `Vstep` -> Voltage step; MATLAB: `model.param.set('Vstep', value, description)`
- `insheight` -> Insulator height; MATLAB: `model.param.set('insheight', value, description)`
- `airheight` -> Air height; MATLAB: `model.param.set('airheight', value, description)`
- `en` -> Spring stiffness; MATLAB: `model.param.set('en', value, description)`
- `tn` -> Contact force; MATLAB: `model.param.set('tn', value, description)`
...
- 已识别参数：V0、Vstep、insheight、airheight、en、tn
...
- Sweep `V0` around the learned value `1[mV]` and export target outputs to CSV.
- Sweep `Vstep` around the learned value `5[V]` and export target outputs to CSV.
- Sweep `insheight` around the learned value `100[nm]` and export target outputs to CSV.
- Sweep `airheight` around the learned value `900[nm]` and export target outputs to CSV.
- Sweep `en` around the learned value `1e15[Pa/m]` and export target outputs to CSV.
- Sweep `tn` around the learned value `5e5[Pa]` and export target outputs to CSV.
...
- Summary: 已学习 rf_mems_switch 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=22 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：rf_mems_switch.m, rf_mems_switch.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 rf_mems_switch，训练阶段为 modeling_logic_learning_ready。
...
- `rf_mems_switch.mph` (comsol_mph, 82336033 bytes)
- `rf_mems_switch.m` (matlab_livelink, 56938 bytes)
- `rf_mems_switch.java` (comsol_java, 55930 bytes)
- `rf_mems_switch_summary.json` (json, 1366 bytes)
- `rf_mems_switch.pptx` (presentation_document, 2205149 bytes)
- `models.mems.rf_mems_switch.pdf` (pdf_document, 981855 bytes)
