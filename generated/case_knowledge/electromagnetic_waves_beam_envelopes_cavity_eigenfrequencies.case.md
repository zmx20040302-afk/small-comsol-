# electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies

- Case directory: `D:\桌面\codex\案例下载\电气\利用“电磁波，波束包络”接口为腔特征频率建模`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:45:32.629865+00:00`
...
- `nTransv` = `1`
- `kxm` = `mLong*pi/length`
- `kyn` = `nTransv*pi/width`
- `fmn` = `c_const/(2*pi)*sqrt(kxm^2+kyn^2)`

## 内容级读取结果

- 模型树证据数量: 23
- 理论关键词: 电磁, 声学, 结构力学
- geometry: r1 / Rectangle, geom1 / r1 / size, geom1 / r1 / base, geom1 / r1 / layertop
- physics: Electromagnetic Waves
- mesh: mesh1, mesh2
- studies: Eigenfrequency, Stationary, Eigenvalue, Frequency Domain
- boundary_conditions: r1 / size, r1 / base, r1 / layertop

### 文件内容证据

- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies.java`: 参数=9, 几何=4, 物理场=0, 研究=0, 结果=0
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies.pptx`: 参数=0, 几何=0, 物理场=1, 研究=4, 结果=0
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies 主要研究声压、模态或频域声学响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenvalue 研究。
...
- 识别到的研究类型: Eigenfrequency, Stationary, Eigenvalue, Frequency Domain
- 理论关键词: 电磁, 声学, 结构力学

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 电磁, 声学, 结构力学
- `wl` -> 案例参数/扫描变量; MATLAB: `model.param.set('wl', value, description)`
- `length` -> 案例参数/扫描变量; MATLAB: `model.param.set('length', value, description)`
- `width` -> 案例参数/扫描变量; MATLAB: `model.param.set('width', value, description)`
- `E0` -> 案例参数/扫描变量; MATLAB: `model.param.set('E0', value, description)`
- `mLong` -> 案例参数/扫描变量; MATLAB: `model.param.set('mLong', value, description)`
- `nTransv` -> 案例参数/扫描变量; MATLAB: `model.param.set('nTransv', value, description)`
- `kxm` -> 案例参数/扫描变量; MATLAB: `model.param.set('kxm', value, description)`
- `kyn` -> 案例参数/扫描变量; MATLAB: `model.param.set('kyn', value, description)`
- `fmn` -> 案例参数/扫描变量; MATLAB: `model.param.set('fmn', value, description)`
...
- 已识别几何证据：r1 / Rectangle；geom1 / r1 / size；geom1 / r1 / base；geom1 / r1 / layertop
- 已识别物理场证据：Electromagnetic Waves
- 已识别参数：wl、length、width、E0、mLong、nTransv、kxm、kyn、fmn
...
- Use the learned study types as solver starting points: Eigenfrequency, Stationary, Eigenvalue, Frequency Domain.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 声学, 结构力学?
...
- Sweep `length` around the learned value `50*wl` and export target outputs to CSV.
- Sweep `width` around the learned value `length/10` and export target outputs to CSV.
- Sweep `E0` around the learned value `1[V/m]` and export target outputs to CSV.
- Sweep `mLong` around the learned value `101` and export target outputs to CSV.
- Sweep `nTransv` around the learned value `1` and export target outputs to CSV.
- Sweep `kxm` around the learned value `mLong*pi/length` and export target outputs to CSV.
- Sweep `kyn` around the learned value `nTransv*pi/width` and export target outputs to CSV.
...
- Summary: 已学习 electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=4，物理场=1，材料=0，网格=2，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies.m, electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies，训练阶段为 modeling_logic_learning_ready。
...
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies.mph` (comsol_mph, 86964408 bytes)
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies.m` (matlab_livelink, 33613 bytes)
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies.java` (comsol_java, 21700 bytes)
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies_summary.json` (json, 1352 bytes)
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies.pptx` (presentation_document, 766331 bytes)
