...
- Created at: `2026-08-20T10:24:29.286407+00:00`
...
- `transformer.m`: 参数=10, 几何=0, 物理场=0, 研究=0, 结果=0
- `transformer.java`: 参数=10, 几何=0, 物理场=0, 研究=0, 结果=0
- `transformer.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `transformer_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: transformer 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `I1_DC` -> 案例参数/扫描变量; MATLAB: `model.param.set('I1_DC', value, description)`
- `I2_DC` -> 案例参数/扫描变量; MATLAB: `model.param.set('I2_DC', value, description)`
- `V_max` -> 案例参数/扫描变量; MATLAB: `model.param.set('V_max', value, description)`
...
- `R_load` -> 案例参数/扫描变量; MATLAB: `model.param.set('R_load', value, description)`
...
- 已识别参数：I1_DC、I2_DC、V_max、f0、T0、w0、Periods、R1、R2、R_load
...
- Sweep `I1_DC` around the learned value `0[A]` and export target outputs to CSV.
- Sweep `I2_DC` around the learned value `0[A]` and export target outputs to CSV.
- Sweep `V_max` around the learned value `7.5[V]` and export target outputs to CSV.
...
- Sweep `R1` around the learned value `0.25[ohm]` and export target outputs to CSV.
...
- Summary: 已学习 transformer 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=10 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：transformer.m, transformer.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 transformer，训练阶段为 modeling_logic_learning_ready。
...
- `transformer.mph` (comsol_mph, 84055891 bytes)
- `transformer.m` (matlab_livelink, 88551 bytes)
- `transformer.java` (comsol_java, 94770 bytes)
- `transformer_summary.json` (json, 1439 bytes)
