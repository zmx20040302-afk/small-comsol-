...
- Case directory: `D:\桌面\codex\案例下载\电气\P-N 结 - 一维`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T15:37:20.442046+00:00`
...
- `Va` = `bias*sweep` - Applied voltage
- `Tl` = `300[K]` - Lattice temperature
- `Na` = `1e15[1/cm^3]` - Acceptor concentration
- `Nd` = `1e15[1/cm^3]` - Donor concentration
- `epsilonr_param` = `11.8` - Relative permittivity
- `Eg0_param` = `1.12[V]` - Band gap
- `chi0_param` = `4.05[V]` - Electron affinity
- `Ni` = `1.25e10[1/cm^3]` - Intrinsic carrier concentration
- `Nc_param` = `Ni*exp(Eg0_param*e_const/(2*k_B_const*Tl))` - Conduction band density of states
- `Nv_param` = `Ni*exp(Eg0_param*e_const/(2*k_B_const*Tl))` - Valence band density of states
- `taun_param` = `1e-6[s]` - Electron lifetime
- `taup_param` = `taun_param` - Hole lifetime
- `bias` = `-4[V]` - Device bias
- `sweep` = `1` - Sweep parameter for gradually turning on bias
...
- `pn_junction_1d.java`: 参数=14, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.semicond.pn_junction_1d.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `pn_junction_1d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pn_junction_1d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pn_junction_1d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.pn_junction_1d.pdf`: models.semicond.pn_junction_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；indirect band-gap semiconductor such as silicon, which is the material used in this model.

## PDF Summary and Physics Judgement

- Simple summary: pn_junction_1d 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: indirect band-gap semiconductor such as silicon, which is the material used in this model.
- PDF: discretization and the Finite Volume discretization. The results of these two computation
...
- PDF keywords: 传热, 电磁, models.semicond.pn_junction_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；indirect band-gap semiconductor such as silicon, which is the material used in this model.
- `Va` -> Applied voltage; MATLAB: `model.param.set('Va', value, description)`
- `Tl` -> Lattice temperature; MATLAB: `model.param.set('Tl', value, description)`
- `Na` -> Acceptor concentration; MATLAB: `model.param.set('Na', value, description)`
- `Nd` -> Donor concentration; MATLAB: `model.param.set('Nd', value, description)`
- `epsilonr_param` -> Relative permittivity; MATLAB: `model.param.set('epsilonr_param', value, description)`
- `Eg0_param` -> Band gap; MATLAB: `model.param.set('Eg0_param', value, description)`
- `chi0_param` -> Electron affinity; MATLAB: `model.param.set('chi0_param', value, description)`
- `Ni` -> Intrinsic carrier concentration; MATLAB: `model.param.set('Ni', value, description)`
- `Nc_param` -> Conduction band density of states; MATLAB: `model.param.set('Nc_param', value, description)`
- `Nv_param` -> Valence band density of states; MATLAB: `model.param.set('Nv_param', value, description)`
- `taun_param` -> Electron lifetime; MATLAB: `model.param.set('taun_param', value, description)`
- `taup_param` -> Hole lifetime; MATLAB: `model.param.set('taup_param', value, description)`
...
- 已识别参数：Va、Tl、Na、Nd、epsilonr_param、Eg0_param、chi0_param、Ni、Nc_param、Nv_param、taun_param、taup_param
...
- Sweep `Va` around the learned value `bias*sweep` and export target outputs to CSV.
- Sweep `Tl` around the learned value `300[K]` and export target outputs to CSV.
- Sweep `Na` around the learned value `1e15[1/cm^3]` and export target outputs to CSV.
- Sweep `Nd` around the learned value `1e15[1/cm^3]` and export target outputs to CSV.
- Sweep `epsilonr_param` around the learned value `11.8` and export target outputs to CSV.
- Sweep `Eg0_param` around the learned value `1.12[V]` and export target outputs to CSV.
- Sweep `chi0_param` around the learned value `4.05[V]` and export target outputs to CSV.
- Sweep `Ni` around the learned value `1.25e10[1/cm^3]` and export target outputs to CSV.
...
- Summary: 已学习 pn_junction_1d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=14；内容证据=15 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 15 条；几何=0，物理场=0，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pn_junction_1d.m, pn_junction_1d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pn_junction_1d，训练阶段为 modeling_logic_learning_ready。
...
- `pn_junction_1d.mph` (comsol_mph, 5746911 bytes)
- `pn_junction_1d.m` (matlab_livelink, 105051 bytes)
- `pn_junction_1d.java` (comsol_java, 110078 bytes)
- `pn_junction_1d_summary.json` (json, 1707 bytes)
- `models.semicond.pn_junction_1d.pdf` (pdf_document, 605523 bytes)
