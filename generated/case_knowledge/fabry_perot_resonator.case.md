...
- Case directory: `D:\桌面\codex\案例下载\电气\法布里-珀罗谐振腔`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:30:08.091102+00:00`
...
- `h_cav` = `2[mm]` - Height of domain
- `l_in` = `5[cm]` - Length before cavity
- `l_out` = `5[cm]` - Length after cavity
- `l_total` = `l_cav+l_in+l_out` - Total length
...
- `fabry_perot_resonator.java`: 参数=6, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.woptics.fabry_perot_resonator.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `fabry_perot_resonator.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `fabry_perot_resonator.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `fabry_perot_resonator_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.fabry_perot_resonator.pdf`: models.woptics.fabry_perot_resonator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；will study the transmission properties of a Fabry–Perot resonator and compare this to the

## PDF Summary and Physics Judgement

- Simple summary: fabry_perot_resonator 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electromagnetic Waves，并采用 Frequency Domain 研究。
...
- PDF: will study the transmission properties of a Fabry–Perot resonator and compare this to the
- PDF: Figure 1: Geometry configuration of a Fabry–Perot resonator. The resonator consists of curved
- PDF: geometry consists of the cavity separating the two mirrors and two exterior domains, one
...
- PDF keywords: 流体, 电磁, models.woptics.fabry_perot_resonator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；will study the transmission properties of a Fabry–Perot resonator and compare this to the
- `lda0` -> Wavelength; MATLAB: `model.param.set('lda0', value, description)`
- `f0` -> Frequency; MATLAB: `model.param.set('f0', value, description)`
- `h_cav` -> Height of domain; MATLAB: `model.param.set('h_cav', value, description)`
- `l_in` -> Length before cavity; MATLAB: `model.param.set('l_in', value, description)`
- `l_out` -> Length after cavity; MATLAB: `model.param.set('l_out', value, description)`
- `l_total` -> Total length; MATLAB: `model.param.set('l_total', value, description)`
...
- 已识别参数：lda0、f0、h_cav、l_in、l_out、l_total
...
- Sweep `h_cav` around the learned value `2[mm]` and export target outputs to CSV.
- Sweep `l_in` around the learned value `5[cm]` and export target outputs to CSV.
- Sweep `l_out` around the learned value `5[cm]` and export target outputs to CSV.
- Sweep `l_total` around the learned value `l_cav+l_in+l_out` and export target outputs to CSV.
...
- Summary: 已学习 fabry_perot_resonator 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=9 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 9 条；几何=0，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：fabry_perot_resonator.m, fabry_perot_resonator.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 fabry_perot_resonator，训练阶段为 modeling_logic_learning_ready。
...
- `fabry_perot_resonator.mph` (comsol_mph, 81082494 bytes)
- `fabry_perot_resonator.m` (matlab_livelink, 26415 bytes)
- `fabry_perot_resonator.java` (comsol_java, 22163 bytes)
- `fabry_perot_resonator_summary.json` (json, 1859 bytes)
- `models.woptics.fabry_perot_resonator.pdf` (pdf_document, 1282836 bytes)
