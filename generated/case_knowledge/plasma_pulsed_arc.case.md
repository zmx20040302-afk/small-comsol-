...
- Case directory: `D:\桌面\codex\案例下载\电气\等离子体脉冲电弧`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T16:07:43.259304+00:00`
...
- 模型树证据数量: 24
- 理论关键词: 流体, 电磁, 传热
- geometry: r1 / Rectangle, r2 / Rectangle, pol1 / Polygon, fil1 / Fillet, pt1 / Point, geom1 / r1 / size, geom1 / r2 / size, geom1 / r2 / pos, geom1 / pol1 / type, geom1 / pol1 / source, geom1 / fil1 / radius
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: r1 / size, r2 / size, r2 / pos, pol1 / type, pol1 / source, fil1 / radius

### 文件内容证据

- `plasma_pulsed_arc.java`: 参数=4, 几何=11, 物理场=0, 研究=0, 结果=0
- `models.plasma.plasma_pulsed_arc.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `plasma_pulsed_arc.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `plasma_pulsed_arc.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `plasma_pulsed_arc_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.plasma_pulsed_arc.pdf`: models.plasma.plasma_pulsed_arc.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；a better description of the physics). To make the model more stable use a mesh coarse

## PDF Summary and Physics Judgement

- Simple summary: plasma_pulsed_arc 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: a better description of the physics). To make the model more stable use a mesh coarse
- PDF: Results and Discussion
- PDF: and temperature are higher. In the results shown the temperature of the subtract still has
...
- PDF keywords: 流体, 电磁, 传热, models.plasma.plasma_pulsed_arc.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；a better description of the physics). To make the model more stable use a mesh coarse
...
- Sweep `delta` around the learned value `80[A]` and export target outputs to CSV.
...
- Summary: 已学习 plasma_pulsed_arc 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=24 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 24 条；几何=11，物理场=0，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：plasma_pulsed_arc.m, plasma_pulsed_arc.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 plasma_pulsed_arc，训练阶段为 modeling_logic_learning_ready。
...
- `plasma_pulsed_arc.mph` (comsol_mph, 46864409 bytes)
- `plasma_pulsed_arc.m` (matlab_livelink, 93467 bytes)
- `plasma_pulsed_arc.java` (comsol_java, 94502 bytes)
- `plasma_pulsed_arc_summary.json` (json, 2924 bytes)
- `models.plasma.plasma_pulsed_arc.pdf` (pdf_document, 378193 bytes)
