...
- Case directory: `D:\桌面\codex\案例下载\电气\MESFET 的直流特性`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:49:02.822242+00:00`
...
- `L` = `1[um]` - Gate length
- `Wd` = `4[um]` - Device width
- `Hd` = `0.5[um]` - Device height
- `Ws` = `1[um]` - Source width
- `Wdd` = `1[um]` - Drain width
- `Vg` = `0[V]` - Gate voltage
- `Vd` = `0[V]` - Drain voltage
- `Vs` = `0[V]` - Source voltage
- `Nd` = `1e16[1/cm^3]` - Doping

## 内容级读取结果

- 模型树证据数量: 27
- 理论关键词: 电磁
- geometry: r1 / Rectangle, pt1 / Point, geom1 / r1 / size, geom1 / r1 / pos, geom1 / pt1 / p
- physics: semi / Semiconductor, semi2 / Semiconductor
- materials: mat1
- mesh: mesh1
- studies: Stationary
- boundary_conditions: r1 / size, r1 / pos, pt1 / p, mc1 / 5, mc2 / 3, mc3 / 7, adm1 / 1, tar1 / 1

### 文件内容证据

- `mesfet.java`: 参数=9, 几何=4, 物理场=2, 研究=0, 结果=0
- `mesfet.m`: 参数=9, 几何=5, 物理场=0, 研究=0, 结果=0
- `models.semicond.mesfet.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `mesfet.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mesfet_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.mesfet.pdf`: models.semicond.mesfet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry is composed of a block of 4 by 0.5 μm. The Schottky contact (gate) has a

## PDF Summary and Physics Judgement

- Simple summary: mesfet 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Semiconductor，并采用 Stationary 研究。
- Primary physics: `Semiconductor`
- Study type: `Stationary`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Semiconductor，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Stationary 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: The geometry is composed of a block of 4 by 0.5 μm. The Schottky contact (gate) has a
- PDF: Figure 1 shows the model’s geometry.
- PDF: Figure 1: Geometry of the modeled MESFET.
- 脚本/文本识别到的物理场: semi / Semiconductor, semi2 / Semiconductor
- 识别到的研究类型: Stationary
- 理论关键词: 电磁
...
- PDF keywords: 电磁, models.semicond.mesfet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry is composed of a block of 4 by 0.5 μm. The Schottky contact (gate) has a
- `L` -> Gate length; MATLAB: `model.param.set('L', value, description)`
- `Wd` -> Device width; MATLAB: `model.param.set('Wd', value, description)`
- `Hd` -> Device height; MATLAB: `model.param.set('Hd', value, description)`
- `Ws` -> Source width; MATLAB: `model.param.set('Ws', value, description)`
- `Wdd` -> Drain width; MATLAB: `model.param.set('Wdd', value, description)`
- `Vg` -> Gate voltage; MATLAB: `model.param.set('Vg', value, description)`
- `Vd` -> Drain voltage; MATLAB: `model.param.set('Vd', value, description)`
- `Vs` -> Source voltage; MATLAB: `model.param.set('Vs', value, description)`
...
- 已识别几何证据：r1 / Rectangle；pt1 / Point；geom1 / r1 / size；geom1 / r1 / pos；geom1 / pt1 / p
- 已识别物理场证据：semi / Semiconductor；semi2 / Semiconductor
- 已识别参数：L、Wd、Hd、Ws、Wdd、Vg、Vd、Vs、Nd
...
- Transfer recognized physics interfaces to similar requirements: semi / Semiconductor, semi2 / Semiconductor.
...
- Sweep `Wd` around the learned value `4[um]` and export target outputs to CSV.
- Sweep `Hd` around the learned value `0.5[um]` and export target outputs to CSV.
- Sweep `Ws` around the learned value `1[um]` and export target outputs to CSV.
- Sweep `Wdd` around the learned value `1[um]` and export target outputs to CSV.
- Sweep `Vg` around the learned value `0[V]` and export target outputs to CSV.
- Sweep `Vd` around the learned value `0[V]` and export target outputs to CSV.
- Sweep `Vs` around the learned value `0[V]` and export target outputs to CSV.
...
- Summary: 已学习 mesfet 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=27 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 27 条；几何=5，物理场=2，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mesfet.m, mesfet.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mesfet，训练阶段为 modeling_logic_learning_ready。
...
- `mesfet.mph` (comsol_mph, 19135838 bytes)
- `mesfet.m` (matlab_livelink, 24388 bytes)
- `mesfet.java` (comsol_java, 24116 bytes)
- `mesfet_summary.json` (json, 1659 bytes)
- `models.semicond.mesfet.pdf` (pdf_document, 182608 bytes)
