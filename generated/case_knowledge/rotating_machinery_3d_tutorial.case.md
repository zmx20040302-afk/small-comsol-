...
- Created at: `2026-08-22T05:37:59.585326+00:00`
...
- `omega` = `3000[rpm]` - Rotational velocity
...
- `rotating_machinery_3d_tutorial.java`: 参数=5, 几何=18, 物理场=0, 研究=0, 结果=0
- `models.acdc.rotating_machinery_3d_tutorial.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `rotating_machinery_3d_tutorial.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rotating_machinery_3d_tutorial.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rotating_machinery_3d_tutorial_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: rotating_machinery_3d_tutorial 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF keywords: 流体, 结构力学, 电磁, models.acdc.rotating_machinery_3d_tutorial.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electric Currents，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；representing an optional lamination. Figure 1 shows the geometry with the rotor and
...
- `omega` -> Rotational velocity; MATLAB: `model.param.set('omega', value, description)`
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 结构力学, 电磁?
...
- Sweep `omega` around the learned value `3000[rpm]` and export target outputs to CSV.
...
- Summary: 已学习 rotating_machinery_3d_tutorial 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=40 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 40 条；几何=18，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：rotating_machinery_3d_tutorial.m, rotating_machinery_3d_tutorial.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 rotating_machinery_3d_tutorial，训练阶段为 modeling_logic_learning_ready。
...
- `rotating_machinery_3d_tutorial.mph` (comsol_mph, 125543292 bytes)
- `rotating_machinery_3d_tutorial.m` (matlab_livelink, 60511 bytes)
- `rotating_machinery_3d_tutorial.java` (comsol_java, 60420 bytes)
- `rotating_machinery_3d_tutorial_summary.json` (json, 1321 bytes)
