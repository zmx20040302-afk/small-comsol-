...
- Case directory: `D:\桌面\codex\案例下载\电气\偏压谐振器的吸合和释放分析 - 二维`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:12:24.747211+00:00`
...
- `biased_resonator_2d_pull_in_pull_out.java`: 参数=6, 几何=11, 物理场=0, 研究=0, 结果=0
- `models.mems.biased_resonator_2d_pull_in_pull_out.pdf`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `biased_resonator_2d_pull_in_pull_out.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `biased_resonator_2d_pull_in_pull_out.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `biased_resonator_2d_pull_in_pull_out_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.biased_resonator_2d_pull_in_pull_out.pdf`: models.mems.biased_resonator_2d_pull_in_pull_out.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry, fabrication, and operation of the device are discussed for the Stationary

## PDF Summary and Physics Judgement

- Simple summary: biased_resonator_2d_pull_in_pull_out 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF: Analysis of a Biased Resonator — 2D model.
- PDF: continuously ramped up eventually exceeding the pull-in voltage). The result of the
...
- PDF keywords: 结构力学, 电磁, models.mems.biased_resonator_2d_pull_in_pull_out.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry, fabrication, and operation of the device are discussed for the Stationary
...
- Summary: 已学习 biased_resonator_2d_pull_in_pull_out 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=29 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 29 条；几何=11，物理场=2，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：biased_resonator_2d_pull_in_pull_out.m, biased_resonator_2d_pull_in_pull_out.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 biased_resonator_2d_pull_in_pull_out，训练阶段为 modeling_logic_learning_ready。
...
- `biased_resonator_2d_pull_in_pull_out.mph` (comsol_mph, 15853015 bytes)
- `biased_resonator_2d_pull_in_pull_out.m` (matlab_livelink, 45734 bytes)
- `biased_resonator_2d_pull_in_pull_out.java` (comsol_java, 43526 bytes)
- `biased_resonator_2d_pull_in_pull_out_summary.json` (json, 1320 bytes)
- `models.mems.biased_resonator_2d_pull_in_pull_out.pdf` (pdf_document, 550260 bytes)
