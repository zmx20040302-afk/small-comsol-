# schmidt_cassegrain_telescope_geom_sequence

- Case directory: `D:\桌面\codex\案例下载\电气\施密特-卡塞格林望远镜`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:33:59.693020+00:00`
...
- `nix` = `0.0` - Global optical axis, x-component
- `niy` = `0.0` - Global optical axis, y-component
- `niz` = `-1.0` - Global optical axis, z-component
- `R1_corr` = `0` - Corrector radius of curvature, surface 1
- `R2_corr` = `-56118.2800[mm]` - Corrector radius of curvature, surface 2
- `A04_corr` = `6.431005e-10` - Corrector 4th order aspheric term, surface 2
- `A06_corr` = `3.113976e-16` - Corrector 6th order aspheric term, surface 2
- `Tc_corr` = `4.0000[mm]` - Corrector central thickness
- `d0_corr` = `210.0000[mm]` - Corrector outer diameter
- `R_sec` = `252.6581[mm]` - Secondary mirror radius of curvature
- `d0_sec` = `60.0000[mm]` - Secondary mirror outer diameter
- `dc_sec` = `53.9204[mm]` - Secondary mirror clear diameter
- `Tc_sec` = `7.0000[mm]` - Secondary mirror central thickness
- `z_sec` = `7.0000[mm]` - Secondary mirror z-coordinate, relative to corrector surface 2
- `R_prim` = `-812.8000[mm]` - Primary mirror radius of curvature
- `d0_prim` = `215.0000[mm]` - Primary mirror outer diameter
- `d1_prim` = `206.1674[mm]` - Primary mirror surface diameter
- `dh_prim` = `50.0000[mm]` - Primary mirror hole diameter
- `Tc_prim` = `20.0000[mm]` - Primary mirror central thickness
- `z_prim` = `-303.8701[mm]` - Primary mirror z-coordinate, relative to secondary mirror
- `d_img` = `40.0000[mm]` - Image plane diameter
- `z_img` = `200.0000[mm]` - Image plane z-coordinate, relative to primary mirror vertex
- `d_obs` = `70.0000[mm]` - Central obstruction diameter
- `z_obs` = `-10.0000[mm]` - Central obstruction z-coordinate, relative to corrector surface 1
...
- `schmidt_cassegrain_telescope_geom_sequence.m`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `schmidt_cassegrain_telescope_geom_sequence.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.roptics.schmidt_cassegrain_telescope.pdf`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `schmidt_cassegrain_telescope_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `schmidt_cassegrain_telescope_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.roptics.schmidt_cassegrain_telescope.pdf`: models.roptics.schmidt_cassegrain_telescope.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；plate. Although this results in some residual off-axis coma and focal plane curvature, the

## PDF Summary and Physics Judgement

- Simple summary: schmidt_cassegrain_telescope_geom_sequence 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics。
...
- PDF: plate. Although this results in some residual off-axis coma and focal plane curvature, the
- PDF: The telescope geometry is constructed using parts from the Ray Optics Module Part
- PDF: placed in a COMSOL geometry sequence does not affect the ray trace. However, the
...
- PDF keywords: 结构力学, models.roptics.schmidt_cassegrain_telescope.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；plate. Although this results in some residual off-axis coma and focal plane curvature, the
...
- `R1_corr` -> Corrector radius of curvature, surface 1; MATLAB: `model.param.set('R1_corr', value, description)`
- `R2_corr` -> Corrector radius of curvature, surface 2; MATLAB: `model.param.set('R2_corr', value, description)`
- `A04_corr` -> Corrector 4th order aspheric term, surface 2; MATLAB: `model.param.set('A04_corr', value, description)`
- `A06_corr` -> Corrector 6th order aspheric term, surface 2; MATLAB: `model.param.set('A06_corr', value, description)`
- `Tc_corr` -> Corrector central thickness; MATLAB: `model.param.set('Tc_corr', value, description)`
- `d0_corr` -> Corrector outer diameter; MATLAB: `model.param.set('d0_corr', value, description)`
- `R_sec` -> Secondary mirror radius of curvature; MATLAB: `model.param.set('R_sec', value, description)`
- `d0_sec` -> Secondary mirror outer diameter; MATLAB: `model.param.set('d0_sec', value, description)`
- `dc_sec` -> Secondary mirror clear diameter; MATLAB: `model.param.set('dc_sec', value, description)`
...
- 已识别物理场证据：Ray Optics
- 已识别参数：nix、niy、niz、R1_corr、R2_corr、A04_corr、A06_corr、Tc_corr、d0_corr、R_sec、d0_sec、dc_sec
...
- Sweep `nix` around the learned value `0.0` and export target outputs to CSV.
- Sweep `niy` around the learned value `0.0` and export target outputs to CSV.
- Sweep `niz` around the learned value `-1.0` and export target outputs to CSV.
- Sweep `R1_corr` around the learned value `0` and export target outputs to CSV.
- Sweep `R2_corr` around the learned value `-56118.2800[mm]` and export target outputs to CSV.
- Sweep `A04_corr` around the learned value `6.431005e-10` and export target outputs to CSV.
- Sweep `A06_corr` around the learned value `3.113976e-16` and export target outputs to CSV.
- Sweep `Tc_corr` around the learned value `4.0000[mm]` and export target outputs to CSV.
...
- Summary: 已学习 schmidt_cassegrain_telescope_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=24；内容证据=26 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 26 条；几何=0，物理场=1，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：schmidt_cassegrain_telescope_geom_sequence.m, schmidt_cassegrain_telescope_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 schmidt_cassegrain_telescope_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `schmidt_cassegrain_telescope_geom_sequence.mph` (comsol_mph, 5200826 bytes)
- `schmidt_cassegrain_telescope_geom_sequence.m` (matlab_livelink, 17710 bytes)
- `schmidt_cassegrain_telescope_geom_sequence.java` (comsol_java, 18170 bytes)
- `schmidt_cassegrain_telescope_geom_sequence_summary.json` (json, 1356 bytes)
- `models.roptics.schmidt_cassegrain_telescope.pdf` (pdf_document, 945099 bytes)
