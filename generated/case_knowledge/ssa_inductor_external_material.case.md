# ssa_inductor_external_material

- Case directory: `D:\桌面\codex\案例下载\电气\调用由外部函数定义 HBBH 曲线的材料`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:16:18.513959+00:00`
...
- `freq` = `10[kHz]` - Operating Frequency
- `csigma` = `5e7[S/m]` - Coil, Wires Conductivity
- `cIdc` = `1[A]` - DC Current Bias
- `cIac` = `1[A]` - AC Current Perturbation

## 内容级读取结果

- 模型树证据数量: 31
- 理论关键词: 电磁
- geometry: c1 / Circle, r1 / Rectangle, r2 / Rectangle, fil1 / Fillet, geom1 / c1 / r, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r2 / pos, geom1 / fil1 / radius, geom1 / fin / repairtoltype
- mesh: mesh1
- boundary_conditions: c1 / r, r1 / size, r1 / pos, r2 / size, r2 / pos, fil1 / radius, fin / repairtoltype

### 文件内容证据

- `ssa_inductor_external_material.java`: 参数=12, 几何=11, 物理场=0, 研究=0, 结果=0
- `ssa_inductor_external_material.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ssa_inductor_external_material.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ssa_inductor_external_material_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: ssa_inductor_external_material 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `freq` -> Operating Frequency; MATLAB: `model.param.set('freq', value, description)`
- `csigma` -> Coil, Wires Conductivity; MATLAB: `model.param.set('csigma', value, description)`
...
- 已识别几何证据：c1 / Circle；r1 / Rectangle；r2 / Rectangle；fil1 / Fillet；geom1 / c1 / r；geom1 / r1 / size
- 已识别参数：dr、il、cr、fr、coor、coir、cwc、cn、freq、csigma、cIdc、cIac
...
- Summary: 已学习 ssa_inductor_external_material 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=11，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：ssa_inductor_external_material.m, ssa_inductor_external_material.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 ssa_inductor_external_material，训练阶段为 modeling_logic_learning_ready。
...
- `ssa_inductor_external_material.mph` (comsol_mph, 21286379 bytes)
- `ssa_inductor_external_material.m` (matlab_livelink, 26609 bytes)
- `ssa_inductor_external_material.java` (comsol_java, 23534 bytes)
- `ssa_inductor_external_material_summary.json` (json, 1167 bytes)
