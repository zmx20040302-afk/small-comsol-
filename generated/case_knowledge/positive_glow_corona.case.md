# positive_glow_corona

- Case directory: `D:\桌面\codex\案例下载\电气\正辉光电晕放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:53:53.557488+00:00`
...
- `ri` = `0.1[cm]` - Inner Radius
- `ro` = `2.1[cm]` - Outer Radius
- `t` = `5[us]` - Simulation Starting Time
- `V0` = `rm1(t/1[us])*1[kV]` - Applied Voltage
...
- `positive_glow_corona.java`: 参数=4, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.edis.positive_glow_corona.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `positive_glow_corona.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `positive_glow_corona.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `positive_glow_corona_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.positive_glow_corona.pdf`: models.edis.positive_glow_corona.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This case study investigates a positive glow corona between coaxial cylindrical electrodes.

## PDF Summary and Physics Judgement

- Simple summary: positive_glow_corona 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: This case study investigates a positive glow corona between coaxial cylindrical electrodes.
...
- PDF keywords: 电磁, 结构力学, 流体, models.edis.positive_glow_corona.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This case study investigates a positive glow corona between coaxial cylindrical electrodes.
- `ri` -> Inner Radius; MATLAB: `model.param.set('ri', value, description)`
- `ro` -> Outer Radius; MATLAB: `model.param.set('ro', value, description)`
- `t` -> Simulation Starting Time; MATLAB: `model.param.set('t', value, description)`
...
- Sweep `ri` around the learned value `0.1[cm]` and export target outputs to CSV.
- Sweep `ro` around the learned value `2.1[cm]` and export target outputs to CSV.
- Sweep `t` around the learned value `5[us]` and export target outputs to CSV.
- Sweep `V0` around the learned value `rm1(t/1[us])*1[kV]` and export target outputs to CSV.
...
- Summary: 已学习 positive_glow_corona 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=11 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 11 条；几何=2，物理场=1，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：positive_glow_corona.m, positive_glow_corona.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 positive_glow_corona，训练阶段为 modeling_logic_learning_ready。
...
- `positive_glow_corona.mph` (comsol_mph, 3396437 bytes)
- `positive_glow_corona.m` (matlab_livelink, 44995 bytes)
- `positive_glow_corona.java` (comsol_java, 39823 bytes)
- `positive_glow_corona_summary.json` (json, 1043 bytes)
- `models.edis.positive_glow_corona.pdf` (pdf_document, 705724 bytes)
