...
- Case directory: `D:\桌面\codex\案例下载\电气\空气中的大气压电晕放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:39:14.600378+00:00`
...
- `corona_discharge_air_1d.java`: 参数=1, 几何=2, 物理场=0, 研究=0, 结果=0
- `models.plasma.corona_discharge_air_1d.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `corona_discharge_air_1d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `corona_discharge_air_1d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `corona_discharge_air_1d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.corona_discharge_air_1d.pdf`: models.plasma.corona_discharge_air_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This example presents a study of a coaxial DC corona discharge in dry air at atmospheric

## PDF Summary and Physics Judgement

- Simple summary: corona_discharge_air_1d 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: This example presents a study of a coaxial DC corona discharge in dry air at atmospheric
- PDF: Figure 1 shows a cross section of the model geometry. The discharge is assumed to be
- PDF: resulting positive ions generate more electrons through secondary emission at the cathode
...
- PDF keywords: 传热, 流体, 电磁, 结构力学, 优化/参数扫描, models.plasma.corona_discharge_air_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This example presents a study of a coaxial DC corona discharge in dry air at atmospheric
...
- Sweep `V0` around the learned value `45[kV]` and export target outputs to CSV.
...
- Summary: 已学习 corona_discharge_air_1d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=1；内容证据=8 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 8 条；几何=2，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：corona_discharge_air_1d.m, corona_discharge_air_1d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 corona_discharge_air_1d，训练阶段为 modeling_logic_learning_ready。
...
- `corona_discharge_air_1d.mph` (comsol_mph, 12202403 bytes)
- `corona_discharge_air_1d.m` (matlab_livelink, 52413 bytes)
- `corona_discharge_air_1d.java` (comsol_java, 47158 bytes)
- `corona_discharge_air_1d_summary.json` (json, 945 bytes)
- `models.plasma.corona_discharge_air_1d.pdf` (pdf_document, 736847 bytes)
