...
- Case directory: `D:\桌面\codex\案例下载\电气\氙的瞬态负迁移率和负微分电导率效应`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:19:57.434776+00:00`
...
- `P0` = `10[atm]` - Gas pressure
- `Ngas` = `P0/(k_B_const*Tgas)` - Gas density
- `Beta` = `1e-7` - Ionization degree
- `ne` = `Beta*Ngas+eps` - Electron density

## 内容级读取结果

- 模型树证据数量: 9
- 理论关键词: 传热, 流体, 电磁, 优化/参数扫描
- physics: be / BoltzmannEquation
- studies: Time Dependent, Stationary, Parametric Sweep

### 文件内容证据

- `transient_negative_mobility.java`: 参数=5, 几何=0, 物理场=1, 研究=0, 结果=0
- `models.plasma.transient_negative_mobility.pdf`: 参数=0, 几何=0, 物理场=0, 研究=3, 结果=0
- `transient_negative_mobility_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `transient_negative_mobility.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `transient_negative_mobility.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.transient_negative_mobility.pdf`: models.plasma.transient_negative_mobility.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；becomes negative during the relaxation of the EEDF. The model results here presented

## PDF Summary and Physics Judgement

- Simple summary: transient_negative_mobility 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: becomes negative during the relaxation of the EEDF. The model results here presented
- PDF: agree well with the results from Ref. 1.
- PDF: approximated version of the stationary Boltzmann equation is solved. Here, to study the
- 脚本/文本识别到的物理场: be / BoltzmannEquation
- 识别到的研究类型: Time Dependent, Stationary, Parametric Sweep
- 理论关键词: 传热, 流体, 电磁, 优化/参数扫描

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 传热, 流体, 电磁, 优化/参数扫描, models.plasma.transient_negative_mobility.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；becomes negative during the relaxation of the EEDF. The model results here presented
...
- `Ngas` -> Gas density; MATLAB: `model.param.set('Ngas', value, description)`
- `Beta` -> Ionization degree; MATLAB: `model.param.set('Beta', value, description)`
- `ne` -> Electron density; MATLAB: `model.param.set('ne', value, description)`
...
- 已识别物理场证据：be / BoltzmannEquation
- 已识别参数：Tgas、P0、Ngas、Beta、ne
...
- Transfer recognized physics interfaces to similar requirements: be / BoltzmannEquation.
- Use the learned study types as solver starting points: Time Dependent, Stationary, Parametric Sweep.
...
- Sweep `P0` around the learned value `10[atm]` and export target outputs to CSV.
- Sweep `Ngas` around the learned value `P0/(k_B_const*Tgas)` and export target outputs to CSV.
- Sweep `Beta` around the learned value `1e-7` and export target outputs to CSV.
- Sweep `ne` around the learned value `Beta*Ngas+eps` and export target outputs to CSV.
...
- Summary: 已学习 transient_negative_mobility 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=9 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：transient_negative_mobility.m, transient_negative_mobility.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 transient_negative_mobility，训练阶段为 modeling_logic_learning_ready。
...
- `transient_negative_mobility.mph` (comsol_mph, 7355602 bytes)
- `transient_negative_mobility.m` (matlab_livelink, 61323 bytes)
- `transient_negative_mobility.java` (comsol_java, 67819 bytes)
- `transient_negative_mobility_summary.json` (json, 1940 bytes)
- `models.plasma.transient_negative_mobility.pdf` (pdf_document, 398862 bytes)
