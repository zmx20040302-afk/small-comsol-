...
- Case directory: `D:\桌面\codex\案例下载\电气\压电陶瓷管`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:20:07.541691+00:00`
...
- mesh: mesh1, map1
- studies: Stationary
- boundary_conditions: bndl1 / 1, roll1 / 2, gnd1 / 1, 4, pot1 / 4, r1 / size, r1 / pos

### 文件内容证据

- `piezoceramic_tube.java`: 参数=0, 几何=3, 物理场=3, 研究=0, 结果=0
- `models.mems.piezoceramic_tube.pdf`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `piezoceramic_tube.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoceramic_tube.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoceramic_tube_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.piezoceramic_tube.pdf`: models.mems.piezoceramic_tube.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the Piezoelectricity multiphysics interface. It models a radially polarized piezoelectric tube,

## PDF Summary and Physics Judgement

- Simple summary: piezoceramic_tube 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF: the Piezoelectricity multiphysics interface. It models a radially polarized piezoelectric tube,
- PDF: GEOMETRY
- PDF: 0.62 mm, respectively. It is represented in an axisymmetric geometry by a single off-axis
...
- PDF keywords: 结构力学, 电磁, 流体, models.mems.piezoceramic_tube.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the Piezoelectricity multiphysics interface. It models a radially polarized piezoelectric tube,
...
- Summary: 已学习 piezoceramic_tube 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=17 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 17 条；几何=3，物理场=4，材料=1，网格=2，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：piezoceramic_tube.m, piezoceramic_tube.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 piezoceramic_tube，训练阶段为 modeling_logic_learning_ready。
...
- `piezoceramic_tube.mph` (comsol_mph, 5794318 bytes)
- `piezoceramic_tube.m` (matlab_livelink, 43324 bytes)
- `piezoceramic_tube.java` (comsol_java, 42574 bytes)
- `piezoceramic_tube_summary.json` (json, 1141 bytes)
- `models.mems.piezoceramic_tube.pdf` (pdf_document, 836524 bytes)
