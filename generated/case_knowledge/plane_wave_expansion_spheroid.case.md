# plane_wave_expansion_spheroid

- Case directory: `D:\桌面\codex\案例下载\电气\快速高效的二维轴对称模型平面波散射分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:32:26.602971+00:00`
...
- `a` = `2[um]` - Spheroid short axis
- `b` = `5[um]` - Spheroid long axis
- `theta` = `pi/4` - Incident angle
- `E0` = `1[V/m]` - Incident field strength
- `I0` = `E0^2/(2*Z0_const)` - Incident power density
- `t_PML` = `2[um]` - PML layer thickness
- `H` = `4*b` - Simulation domain height
- `L` = `5*a` - Simulation domain width
- `m` = `1` - Expansion term index
- `N` = `4` - Total number of terms in the expansion

## 内容级读取结果

- 模型树证据数量: 23
- 理论关键词: 未自动识别
- geometry: r1 / Rectangle, e1 / Ellipse, geom1 / r1 / size, geom1 / r1 / base, geom1 / r1 / layername, geom1 / r1 / layerright, geom1 / r1 / layertop
- mesh: mesh1
- boundary_conditions: r1 / size, r1 / base, r1 / layername, r1 / layerright, r1 / layertop

### 文件内容证据

- `plane_wave_expansion_spheroid.java`: 参数=10, 几何=7, 物理场=0, 研究=0, 结果=0
- `plane_wave_expansion_spheroid.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `plane_wave_expansion_spheroid.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `plane_wave_expansion_spheroid_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: plane_wave_expansion_spheroid 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `a` -> Spheroid short axis; MATLAB: `model.param.set('a', value, description)`
- `b` -> Spheroid long axis; MATLAB: `model.param.set('b', value, description)`
- `theta` -> Incident angle; MATLAB: `model.param.set('theta', value, description)`
- `E0` -> Incident field strength; MATLAB: `model.param.set('E0', value, description)`
- `I0` -> Incident power density; MATLAB: `model.param.set('I0', value, description)`
- `t_PML` -> PML layer thickness; MATLAB: `model.param.set('t_PML', value, description)`
- `H` -> Simulation domain height; MATLAB: `model.param.set('H', value, description)`
- `L` -> Simulation domain width; MATLAB: `model.param.set('L', value, description)`
- `m` -> Expansion term index; MATLAB: `model.param.set('m', value, description)`
- `N` -> Total number of terms in the expansion; MATLAB: `model.param.set('N', value, description)`
...
- 已识别几何证据：r1 / Rectangle；e1 / Ellipse；geom1 / r1 / size；geom1 / r1 / base；geom1 / r1 / layername；geom1 / r1 / layerright
- 已识别参数：a、b、theta、E0、I0、t_PML、H、L、m、N
...
- Sweep `a` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `b` around the learned value `5[um]` and export target outputs to CSV.
- Sweep `theta` around the learned value `pi/4` and export target outputs to CSV.
- Sweep `E0` around the learned value `1[V/m]` and export target outputs to CSV.
- Sweep `I0` around the learned value `E0^2/(2*Z0_const)` and export target outputs to CSV.
- Sweep `t_PML` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `H` around the learned value `4*b` and export target outputs to CSV.
- Sweep `L` around the learned value `5*a` and export target outputs to CSV.
...
- Summary: 已学习 plane_wave_expansion_spheroid 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=7，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：plane_wave_expansion_spheroid.m, plane_wave_expansion_spheroid.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 plane_wave_expansion_spheroid，训练阶段为 modeling_logic_learning_ready。
...
- `plane_wave_expansion_spheroid.mph` (comsol_mph, 95935262 bytes)
- `plane_wave_expansion_spheroid.m` (matlab_livelink, 37334 bytes)
- `plane_wave_expansion_spheroid.java` (comsol_java, 33244 bytes)
- `plane_wave_expansion_spheroid_summary.json` (json, 1155 bytes)
