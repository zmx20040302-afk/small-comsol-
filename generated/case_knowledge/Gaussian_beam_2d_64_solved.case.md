# Gaussian_beam_2d_64_solved

- Case directory: `D:\桌面\codex\案例下载\电气\背景高斯光束定义：近轴近似 vs. 平面波展开`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:24:57.203611+00:00`
...
- `y_scale2` = `5`
- `L_PML` = `lda0`
- `a` = `x_scale*lda0+2*L_PML`
- `b` = `y_scale*lda0+2*L_PML`
- `b2` = `y_scale2*lda0+2*L_PML`
- `Nk` = `251`
- `ktm` = `1.5`
- `mesh_size` = `lda0/mesh_N`
- `mesh_N` = `10`
- `E0` = `1[V/m]`
- `P0` = `0.5*sqrt(pi/2)*w0*E0^2/Z0_const`
- `I0` = `P0/w0*sqrt(2/pi)`
...
- 模型树证据数量: 29
- 理论关键词: 未自动识别
- geometry: r1 / Rectangle, geom1 / r1 / size, geom1 / r1 / base, geom1 / r1 / layername, geom1 / r1 / layerleft, geom1 / r1 / layerright
- mesh: mesh1, mesh2
- boundary_conditions: r1 / size, r1 / base, r1 / layername, r1 / layerleft, r1 / layerright

### 文件内容证据

- `Gaussian_beam_2d_64_solved.java`: 参数=16, 几何=6, 物理场=0, 研究=0, 结果=0
- `Gaussian_beam_2d_64_solved.m`: 参数=16, 几何=0, 物理场=0, 研究=0, 结果=0
- `Gaussian_beam_2d_64_solved.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `Gaussian_beam_2d_64_solved_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `Gaussian_beam.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `Gaussian_beam.pdf`: Gaussian_beam.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：▪ This model supplements our blog series on；Gaussian beam modeling.

## PDF Summary and Physics Judgement

- Simple summary: Gaussian_beam_2d_64_solved 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF keywords: Gaussian_beam.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：▪ This model supplements our blog series on；Gaussian beam modeling.
- `lda0` -> 案例参数/扫描变量; MATLAB: `model.param.set('lda0', value, description)`
- `w0` -> 案例参数/扫描变量; MATLAB: `model.param.set('w0', value, description)`
- `x_scale` -> 案例参数/扫描变量; MATLAB: `model.param.set('x_scale', value, description)`
- `y_scale` -> 案例参数/扫描变量; MATLAB: `model.param.set('y_scale', value, description)`
- `y_scale2` -> 案例参数/扫描变量; MATLAB: `model.param.set('y_scale2', value, description)`
- `L_PML` -> 案例参数/扫描变量; MATLAB: `model.param.set('L_PML', value, description)`
- `a` -> 案例参数/扫描变量; MATLAB: `model.param.set('a', value, description)`
- `b` -> 案例参数/扫描变量; MATLAB: `model.param.set('b', value, description)`
- `b2` -> 案例参数/扫描变量; MATLAB: `model.param.set('b2', value, description)`
- `Nk` -> 案例参数/扫描变量; MATLAB: `model.param.set('Nk', value, description)`
- `ktm` -> 案例参数/扫描变量; MATLAB: `model.param.set('ktm', value, description)`
- `mesh_size` -> 案例参数/扫描变量; MATLAB: `model.param.set('mesh_size', value, description)`
...
- 已识别几何证据：r1 / Rectangle；geom1 / r1 / size；geom1 / r1 / base；geom1 / r1 / layername；geom1 / r1 / layerleft；geom1 / r1 / layerright
- 已识别参数：lda0、w0、x_scale、y_scale、y_scale2、L_PML、a、b、b2、Nk、ktm、mesh_size
...
- Sweep `w0` around the learned value `1*lda0` and export target outputs to CSV.
- Sweep `x_scale` around the learned value `10` and export target outputs to CSV.
- Sweep `y_scale` around the learned value `10` and export target outputs to CSV.
- Sweep `y_scale2` around the learned value `5` and export target outputs to CSV.
- Sweep `L_PML` around the learned value `lda0` and export target outputs to CSV.
- Sweep `a` around the learned value `x_scale*lda0+2*L_PML` and export target outputs to CSV.
- Sweep `b` around the learned value `y_scale*lda0+2*L_PML` and export target outputs to CSV.
...
- Summary: 已学习 Gaussian_beam_2d_64_solved 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=16；内容证据=29 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 29 条；几何=6，物理场=0，材料=0，网格=2，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：Gaussian_beam_2d_64_solved.m, Gaussian_beam_2d_64_solved.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 Gaussian_beam_2d_64_solved，训练阶段为 modeling_logic_learning_ready。
...
- `Gaussian_beam_2d_64_solved.mph` (comsol_mph, 124042638 bytes)
- `Gaussian_beam_2d_64_solved.m` (matlab_livelink, 73333 bytes)
- `Gaussian_beam_2d_64_solved.java` (comsol_java, 77810 bytes)
- `Gaussian_beam_2d_64_solved_summary.json` (json, 2455 bytes)
- `Gaussian_beam.pdf` (pdf_document, 4054710 bytes)
