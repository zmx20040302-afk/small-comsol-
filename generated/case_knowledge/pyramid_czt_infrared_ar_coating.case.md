# pyramid_czt_infrared_ar_coating

- Case directory: `D:\桌面\codex\案例下载\电气\用于红外波长的微结构抗反射涂层`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:33:15.891075+00:00`
...
- `lda0` = `3[um]` - Operating wavelength
- `w_CZT` = `5[um]` - Width of the CZT substrate
- `alpha` = `0[deg]` - Angle of incidence
- `w_pillar` = `100[nm]` - Width of microstructure
- `h_Pillar` = `3[um]` - Height of the microstructure
- `pitch` = `2.4[um]` - Pitch of the structure

## 内容级读取结果

- 模型树证据数量: 32
- 理论关键词: 结构力学, 优化/参数扫描
- geometry: r1 / Rectangle, r5 / Rectangle, pare1 / PartitionEdges, r2 / Rectangle, ls1 / LineSegment, geom1 / r1 / size, geom1 / r1 / base, geom1 / r5 / size, geom1 / r5 / base, geom1 / r5 / pos, geom1 / pare1 / param, geom1 / r2 / size
- physics: Optimization
- mesh: mesh1
- studies: Optimization
- boundary_conditions: r1 / size, r1 / base, r5 / size, r5 / base, r5 / pos, pare1 / param, r2 / size, r2 / base, r2 / pos

### 文件内容证据

- `pyramid_czt_infrared_ar_coating.java`: 参数=6, 几何=14, 物理场=0, 研究=0, 结果=0
- `rectangular_si_infrared_ar_coating_optimization.pptx`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `pyramid_czt_infrared_ar_coating_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pyramid_czt_infrared_ar_coating.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pyramid_czt_infrared_ar_coating.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pyramid_czt_infrared_ar_coating.pptx`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: pyramid_czt_infrared_ar_coating 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Optimization 研究。
- Primary physics: `Solid Mechanics`
- Study type: `Optimization`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Solid Mechanics，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Optimization 确认。

### Judgement Evidence

- 脚本/文本识别到的物理场: Optimization
...
- PDF keywords: 结构力学, 优化/参数扫描
- `lda0` -> Operating wavelength; MATLAB: `model.param.set('lda0', value, description)`
- `w_CZT` -> Width of the CZT substrate; MATLAB: `model.param.set('w_CZT', value, description)`
- `alpha` -> Angle of incidence; MATLAB: `model.param.set('alpha', value, description)`
- `w_pillar` -> Width of microstructure; MATLAB: `model.param.set('w_pillar', value, description)`
- `h_Pillar` -> Height of the microstructure; MATLAB: `model.param.set('h_Pillar', value, description)`
- `pitch` -> Pitch of the structure; MATLAB: `model.param.set('pitch', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r5 / Rectangle；pare1 / PartitionEdges；r2 / Rectangle；ls1 / LineSegment；geom1 / r1 / size
- 已识别物理场证据：Optimization
- 已识别参数：lda0、w_CZT、alpha、w_pillar、h_Pillar、pitch
...
- Transfer recognized physics interfaces to similar requirements: Optimization.
...
- Sweep `lda0` around the learned value `3[um]` and export target outputs to CSV.
- Sweep `w_CZT` around the learned value `5[um]` and export target outputs to CSV.
- Sweep `alpha` around the learned value `0[deg]` and export target outputs to CSV.
- Sweep `w_pillar` around the learned value `100[nm]` and export target outputs to CSV.
- Sweep `h_Pillar` around the learned value `3[um]` and export target outputs to CSV.
- Sweep `pitch` around the learned value `2.4[um]` and export target outputs to CSV.
...
- Summary: 已学习 pyramid_czt_infrared_ar_coating 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=32 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 32 条；几何=14，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pyramid_czt_infrared_ar_coating.m, pyramid_czt_infrared_ar_coating.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pyramid_czt_infrared_ar_coating，训练阶段为 modeling_logic_learning_ready。
...
- `pyramid_czt_infrared_ar_coating.mph` (comsol_mph, 18617434 bytes)
- `pyramid_czt_infrared_ar_coating.m` (matlab_livelink, 47322 bytes)
- `pyramid_czt_infrared_ar_coating.java` (comsol_java, 45119 bytes)
- `pyramid_czt_infrared_ar_coating_summary.json` (json, 1367 bytes)
- `pyramid_czt_infrared_ar_coating.pptx` (presentation_document, 1177251 bytes)
- `rectangular_si_infrared_ar_coating_optimization.pptx` (presentation_document, 1013670 bytes)
