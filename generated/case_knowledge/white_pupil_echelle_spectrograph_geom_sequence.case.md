# white_pupil_echelle_spectrograph_geom_sequence

- Case directory: `D:\桌面\codex\案例下载\电气\白瞳阶梯光栅光谱仪`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T07:47:48.909554+00:00`
...
- `B` = `100.0[mm]` - Collimated beam diameter
- `d_mag` = `2.0` - White pupil demagnification constant (i.e, f_1/f_2)
- `f_1` = `800[mm]` - Primary mirror (OAP 1) focal length
- `f_2` = `f_1/d_mag` - Secondary mirror (OAP 2) focal length
- `f_clear` = `0.95` - Mirror clear aperture fraction
- `d0_OAP_1` = `185.0[mm]` - Full diameter of OAP 1
- `dc_OAP_1` = `f_clear*d0_OAP_1` - Clear diameter of OAP 1
- `Tc_OAP_1` = `d0_OAP_1/6` - Center thickness OAP 1
- `d0_OAP_2` = `125.0[mm]` - Full diameter of OAP 2
- `dc_OAP_2` = `f_clear*d0_OAP_2` - Clear diameter of OAP 2
- `Tc_OAP_2` = `d0_OAP_2/6` - Center thickness OAP 2
- `theta_i` = `7.50[deg]` - OAP off axis angle
- `dx_1` = `f_1*tan(theta_i)` - Entrance pupil off axis distance
- `dx_2` = `f_2*tan(theta_i)` - Exit pupil off axis distance
- `dx_1_nom` = `dx_1-f_1*tan(2*gamma_ech)` - Primary mirror off axis distance
- `dx_2_nom` = `dx_2+f_1*tan(2*gamma_ech)` - Secondary mirror off axis distance
- `R_num` = `4.0` - Echelle grating R#
- `theta_B` = `atan(R_num)` - Echelle grating blaze angle
- `gamma_ech` = `-0.65[deg]` - Echelle grating out of plane angle
- `W_ech` = `1.05*B` - Echelle grating width
- `D_ech` = `R_num*W_ech` - Echelle grating depth
- `H_ech` = `D_ech/10` - Echelle grating height
- `lam_xdp` = `525.0[nm]` - Cross-disp. grating mid-wavelength
- `T_xdp` = `725.0[mm^-1]` - Cross-disp. grating line frequency
- `sigma_xdp` = `1/T_xdp` - Cross-disp. grating line spacing
- `theta_xdp` = `asin(lam_xdp/(2*sigma_xdp))` - Cross-disp. grating blaze angle
- `W_xdp` = `1.50*B/d_mag` - Cross-disp. grating width
- `D_xdp` = `1.25*B/d_mag` - Cross-disp. grating depth
- `H_xdp` = `W_xdp/10` - Cross-disp. grating height
- `Z_xdp` = `-75.0[mm]` - Cross-disp. grating displacement
...
- `white_pupil_echelle_spectrograph_geom_sequence.m`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `white_pupil_echelle_spectrograph_geom_sequence.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.roptics.white_pupil_echelle_spectrograph.pdf`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `white_pupil_echelle_spectrograph_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `white_pupil_echelle_spectrograph_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.roptics.white_pupil_echelle_spectrograph.pdf`: models.roptics.white_pupil_echelle_spectrograph.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；fully parameterized geometry. Two

## PDF Summary and Physics Judgement

- Simple summary: white_pupil_echelle_spectrograph_geom_sequence 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics。
...
- PDF: fully parameterized geometry. Two
- PDF: based on the grating geometry and the specified blaze angle (θB). That is,
- PDF: grating geometry and the properties of the incident ray. That is,
...
- PDF keywords: 结构力学, models.roptics.white_pupil_echelle_spectrograph.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；fully parameterized geometry. Two
- `B` -> Collimated beam diameter; MATLAB: `model.param.set('B', value, description)`
- `d_mag` -> White pupil demagnification constant (i.e, f_1/f_2); MATLAB: `model.param.set('d_mag', value, description)`
- `f_1` -> Primary mirror (OAP 1) focal length; MATLAB: `model.param.set('f_1', value, description)`
- `f_2` -> Secondary mirror (OAP 2) focal length; MATLAB: `model.param.set('f_2', value, description)`
- `f_clear` -> Mirror clear aperture fraction; MATLAB: `model.param.set('f_clear', value, description)`
- `d0_OAP_1` -> Full diameter of OAP 1; MATLAB: `model.param.set('d0_OAP_1', value, description)`
- `dc_OAP_1` -> Clear diameter of OAP 1; MATLAB: `model.param.set('dc_OAP_1', value, description)`
- `Tc_OAP_1` -> Center thickness OAP 1; MATLAB: `model.param.set('Tc_OAP_1', value, description)`
- `d0_OAP_2` -> Full diameter of OAP 2; MATLAB: `model.param.set('d0_OAP_2', value, description)`
- `dc_OAP_2` -> Clear diameter of OAP 2; MATLAB: `model.param.set('dc_OAP_2', value, description)`
- `Tc_OAP_2` -> Center thickness OAP 2; MATLAB: `model.param.set('Tc_OAP_2', value, description)`
- `theta_i` -> OAP off axis angle; MATLAB: `model.param.set('theta_i', value, description)`
...
- 已识别物理场证据：Ray Optics
- 已识别参数：B、d_mag、f_1、f_2、f_clear、d0_OAP_1、dc_OAP_1、Tc_OAP_1、d0_OAP_2、dc_OAP_2、Tc_OAP_2、theta_i
...
- 本案例已提取 43 个参数，可作为约束 JSON 和参数扫描变量。
...
- Sweep `B` around the learned value `100.0[mm]` and export target outputs to CSV.
- Sweep `d_mag` around the learned value `2.0` and export target outputs to CSV.
- Sweep `f_1` around the learned value `800[mm]` and export target outputs to CSV.
- Sweep `f_2` around the learned value `f_1/d_mag` and export target outputs to CSV.
- Sweep `f_clear` around the learned value `0.95` and export target outputs to CSV.
- Sweep `d0_OAP_1` around the learned value `185.0[mm]` and export target outputs to CSV.
- Sweep `dc_OAP_1` around the learned value `f_clear*d0_OAP_1` and export target outputs to CSV.
- Sweep `Tc_OAP_1` around the learned value `d0_OAP_1/6` and export target outputs to CSV.
...
- Summary: 已学习 white_pupil_echelle_spectrograph_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=43；内容证据=31 条。
...
- 43 extracted parameters with values/descriptions.
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：white_pupil_echelle_spectrograph_geom_sequence.m, white_pupil_echelle_spectrograph_geom_sequence.java
  - Judgement: 脚本中的 COMSOL API 调用可用于恢复模型树顺序，并辅助生成 LiveLink MATLAB 自动建模脚本。
- **4. 参数与约束提取**
  - Evidence: 从文本、MATLAB 或 Java 证据中提取到 43 个参数。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 white_pupil_echelle_spectrograph_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `white_pupil_echelle_spectrograph_geom_sequence.mph` (comsol_mph, 11083644 bytes)
- `white_pupil_echelle_spectrograph_geom_sequence.m` (matlab_livelink, 51914 bytes)
- `white_pupil_echelle_spectrograph_geom_sequence.java` (comsol_java, 54449 bytes)
- `white_pupil_echelle_spectrograph_geom_sequence_summary.json` (json, 2884 bytes)
- `models.roptics.white_pupil_echelle_spectrograph.pdf` (pdf_document, 1454319 bytes)
