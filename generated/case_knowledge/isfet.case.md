...
- Case directory: `D:\桌面\codex\案例下载\电气\离子敏场效应晶体管 (ISFET) 仿真`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:53:50.630505+00:00`
...
- `DC` = `1e-9[m^2/s]` - Diffusion coefficient, cation
- `DA` = `DC` - Diffusion coefficient, anion
...
- `cC_bulk` = `c0+cOH_bulk` - Bulk cation concentration
- `cA_bulk` = `c0+cH_bulk` - Bulk anion concentration
- `zC` = `+1` - Cation charge
- `zA` = `-1` - Anion charge
- `Istr_bulk` = `0.5*(zC^2*cC_bulk+zA^2*cA_bulk+cOH_bulk+cH_bulk)` - Bulk ionic strength
...
- 模型树证据数量: 32
- 理论关键词: 传热, 电磁, 结构力学, 流体, 优化/参数扫描
- physics: Electrostatics, Global ODEs and DAEs
- studies: Stationary

### 文件内容证据

- `isfet.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.semicond.isfet.pdf`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `isfet.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `isfet.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `isfet_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.isfet.pdf`: models.semicond.isfet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics、Global ODEs and DAEs，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the basic steps to set up the coupling between semiconductor physics and

## PDF Summary and Physics Judgement

- Simple summary: isfet 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: the basic steps to set up the coupling between semiconductor physics and
- PDF: and the electrolyte model. It also shows the technique of using a simple global equation
- PDF: geometry for convenience in making some graphs.) The semiconductor domain is set up
- 脚本/文本识别到的物理场: Electrostatics, Global ODEs and DAEs
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 电磁, 结构力学, 流体, 优化/参数扫描
...
- PDF keywords: 传热, 电磁, 结构力学, 流体, 优化/参数扫描, models.semicond.isfet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics、Global ODEs and DAEs，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the basic steps to set up the coupling between semiconductor physics and
- `Vd` -> Drain voltage; MATLAB: `model.param.set('Vd', value, description)`
- `Vga` -> Gate voltage (applied); MATLAB: `model.param.set('Vga', value, description)`
- `pHb` -> pH of bulk electrolyte; MATLAB: `model.param.set('pHb', value, description)`
- `T0` -> Temperature; MATLAB: `model.param.set('T0', value, description)`
- `V_therm` -> Thermal voltage; MATLAB: `model.param.set('V_therm', value, description)`
- `WAg` -> Work function of Ag; MATLAB: `model.param.set('WAg', value, description)`
- `Eeq` -> Equilibrium potential of reference electrode; MATLAB: `model.param.set('Eeq', value, description)`
- `DC` -> Diffusion coefficient, cation; MATLAB: `model.param.set('DC', value, description)`
- `DA` -> Diffusion coefficient, anion; MATLAB: `model.param.set('DA', value, description)`
- `DH` -> Diffusion coefficient, H+; MATLAB: `model.param.set('DH', value, description)`
- `DOH` -> Diffusion coefficient, OH-; MATLAB: `model.param.set('DOH', value, description)`
- `c0` -> Concentration parameter; MATLAB: `model.param.set('c0', value, description)`
...
- 已识别物理场证据：Electrostatics；Global ODEs and DAEs
- 已识别参数：Vd、Vga、pHb、T0、V_therm、WAg、Eeq、DC、DA、DH、DOH、c0
...
- Transfer recognized physics interfaces to similar requirements: Electrostatics, Global ODEs and DAEs.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 电磁, 结构力学, 流体, 优化/参数扫描?
...
- Sweep `Vga` around the learned value `0[V]` and export target outputs to CSV.
- Sweep `pHb` around the learned value `3` and export target outputs to CSV.
- Sweep `T0` around the learned value `25[degC]` and export target outputs to CSV.
- Sweep `V_therm` around the learned value `R_const*T0/F_const` and export target outputs to CSV.
- Sweep `WAg` around the learned value `4.6[V]` and export target outputs to CSV.
- Sweep `Eeq` around the learned value `0.2[V]` and export target outputs to CSV.
- Sweep `DC` around the learned value `1e-9[m^2/s]` and export target outputs to CSV.
...
- Summary: 已学习 isfet 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=29；内容证据=32 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 32 条；几何=0，物理场=2，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：isfet.m, isfet.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 isfet，训练阶段为 modeling_logic_learning_ready。
...
- `isfet.mph` (comsol_mph, 40948015 bytes)
- `isfet.m` (matlab_livelink, 80888 bytes)
- `isfet.java` (comsol_java, 78548 bytes)
- `isfet_summary.json` (json, 2631 bytes)
- `models.semicond.isfet.pdf` (pdf_document, 643487 bytes)
