...
- Case directory: `D:\桌面\codex\案例下载\电气\用于石油勘探的电磁探测方法仿真`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:29:12.767591+00:00`
...
- 模型树证据数量: 40
- 理论关键词: 电磁, 多孔介质/裂隙
- geometry: sph1 / Sphere, cyl1 / Cylinder, int1 / Intersection, sph2 / Sphere, blk1 / Block, ls1 / LineSegment, ls2 / LineSegment, geom1 / sph1 / r, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / sph2 / r, geom1 / blk1 / size
- physics: Electromagnetic Waves
- mesh: mesh1
- studies: Frequency Domain
- boundary_conditions: sph1 / r, cyl1 / r, cyl1 / h, sph2 / r, blk1 / size, blk1 / base, blk1 / pos, ls1 / specify1, ls1 / coord1, ls1 / specify2, ls1 / coord2, ls2 / specify1

### 文件内容证据

- `marine_csem.java`: 参数=0, 几何=20, 物理场=0, 研究=0, 结果=0
- `models.rf.marine_csem.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `marine_csem.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `marine_csem.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `marine_csem_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.rf.marine_csem.pdf`: models.rf.marine_csem.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the transmitted energy is reflected/guided by the resistive reservoir and results in a higher

## PDF Summary and Physics Judgement

- Simple summary: marine_csem 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electromagnetic Waves，并采用 Frequency Domain 研究。
...
- PDF: the transmitted energy is reflected/guided by the resistive reservoir and results in a higher
...
- PDF keywords: 电磁, 多孔介质/裂隙, models.rf.marine_csem.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the transmitted energy is reflected/guided by the resistive reservoir and results in a higher
...
- 已识别几何证据：sph1 / Sphere；cyl1 / Cylinder；int1 / Intersection；sph2 / Sphere；blk1 / Block；ls1 / LineSegment
...
- Summary: 已学习 marine_csem 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=40 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 40 条；几何=22，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：marine_csem.m, marine_csem.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 marine_csem，训练阶段为 modeling_logic_learning_ready。
...
- `marine_csem.mph` (comsol_mph, 18255122 bytes)
- `marine_csem.m` (matlab_livelink, 24945 bytes)
- `marine_csem.java` (comsol_java, 21668 bytes)
- `marine_csem_summary.json` (json, 897 bytes)
- `models.rf.marine_csem.pdf` (pdf_document, 768015 bytes)
