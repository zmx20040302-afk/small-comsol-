...
- Case directory: `D:\桌面\codex\案例下载\COMSOL\心脏电信号`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-07-06T09:39:34.218495+00:00`
...
- 模型树证据数量: 34
- 理论关键词: 电磁
- geometry: sph1 / Sphere, elp1 / Ellipsoid, blk1 / Block, dif1 / Difference, blk2 / Block, dif2 / Difference, uni1 / Union, sca1 / Scale, dif3 / Difference, geom1 / sph1 / r, geom1 / elp1 / semiaxes, geom1 / blk1 / size
- physics: g / GeneralFormPDE
- mesh: mesh1
- studies: std1 / time / Transient, std1
- boundary_conditions: sph1 / r, elp1 / semiaxes, blk1 / size, blk1 / base, blk1 / pos, blk2 / size, blk2 / base, blk2 / pos, sca1 / keep, sca1 / isotropic, gfeq1 / Ga

### 文件内容证据

- `heart_electrical.mph`: 参数=0, 几何=19, 物理场=1, 研究=2, 结果=0
- `heart_electrical.java`: 参数=0, 几何=19, 物理场=1, 研究=2, 结果=0
- `heart_electrical.m`: 参数=0, 几何=19, 物理场=1, 研究=1, 结果=0
- `heart_electrical.zh_CN.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mph.heart_electrical.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mph.heart_electrical.zh_CN.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mph.heart_electrical.pdf`: models.mph.heart_electrical.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.heart_electrical.zh_CN.pdf`: models.mph.heart_electrical.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 心脏电信号 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 GeneralFormPDE，并采用 Transient 研究。
- Primary physics: `GeneralFormPDE`
- Study type: `Transient`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 GeneralFormPDE，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Transient 确认。
...
- PDF keywords: 电磁, models.mph.heart_electrical.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.heart_electrical.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
...
- 已识别几何证据：sph1 / Sphere；elp1 / Ellipsoid；blk1 / Block；dif1 / Difference；blk2 / Block；dif2 / Difference
...
- Summary: 已学习 心脏电信号 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=34 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 34 条；几何=19，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：heart_electrical.java, heart_electrical.m
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 心脏电信号，训练阶段为 modeling_logic_learning_ready。
...
- `心脏电信号` score=0.2313; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=41
- `管式反应器代理模型 App` score=0.1796; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `热执行器代理模型 App` score=0.153; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42

### Official Documentation Checks

- `IntroductionToCOMSOLMultiphysics.pdf` page 262; module=COMSOL_Multiphysics; score=0.1091
- `LiveLinkForSimulinkUsersGuide.pdf` page 20; module=LiveLink_for_Simulink; score=0.1046
- `LiveLinkForMATLABUsersGuide.pdf` page 198; module=LiveLink_for_MATLAB; score=0.1044
...
- `heart_electrical.java` (comsol_java, 12509 bytes)
- `heart_electrical.m` (matlab_livelink, 14059 bytes)
- `heart_electrical.mph` (comsol_mph, 132694182 bytes)
- `heart_electrical.zh_CN.mph` (comsol_mph, 132096555 bytes)
- `models.mph.heart_electrical.pdf` (pdf_document, 1039826 bytes)
- `models.mph.heart_electrical.zh_CN.pdf` (pdf_document, 1340140 bytes)
