# sector_generator_3d_geom_sequence

- Case directory: `D:\桌面\codex\案例下载\电气\发电机三维仿真`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:29:12.804831+00:00`
...
- `p1x` = `(40[cm]-6[cm]-(10.5[cm]-1.75[cm]))*cos(22.5[deg])`
- `p1y` = `(40[cm]-6[cm]-(10.5[cm]-1.75[cm]))*sin(22.5[deg])`
- `p2y` = `9[cm]/2`
- `rad` = `p1y-p2y`
...
- 模型树证据数量: 14
- 理论关键词: 结构力学, 流体, 电磁
- geometry: wp1 / WorkPlane, pi1 / PartInstance, geom1 / wp1 / unite
- physics: Magnetic Fields
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: wp1 / unite, pi1 / rot, pi1 / selkeepnoncontr

### 文件内容证据

- `sector_generator_3d_geom_sequence.java`: 参数=4, 几何=3, 物理场=0, 研究=0, 结果=0
- `sector_generator_3d_geom_sequence.m`: 参数=4, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.acdc.sector_generator_3d.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `sector_generator_3d_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `sector_generator_3d_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.sector_generator_3d.pdf`: models.acdc.sector_generator_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；being studied is a simplified electric generator in 3D, based on the geometry used in the

## PDF Summary and Physics Judgement

- Simple summary: sector_generator_3d_geom_sequence 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: being studied is a simplified electric generator in 3D, based on the geometry used in the
- PDF: The complete geometry is shown in Figure 1. Parts of the stator have been removed to
- PDF: Figure 1: Geometry of the generator.
...
- PDF keywords: 结构力学, 流体, 电磁, models.acdc.sector_generator_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；being studied is a simplified electric generator in 3D, based on the geometry used in the
- `p1x` -> 案例参数/扫描变量; MATLAB: `model.param.set('p1x', value, description)`
- `p1y` -> 案例参数/扫描变量; MATLAB: `model.param.set('p1y', value, description)`
- `p2y` -> 案例参数/扫描变量; MATLAB: `model.param.set('p2y', value, description)`
- `rad` -> 案例参数/扫描变量; MATLAB: `model.param.set('rad', value, description)`
...
- 已识别几何证据：wp1 / WorkPlane；pi1 / PartInstance；geom1 / wp1 / unite
- 已识别物理场证据：Magnetic Fields
- 已识别参数：p1x、p1y、p2y、rad
...
- Sweep `p1x` around the learned value `(40[cm]-6[cm]-(10.5[cm]-1.75[cm]))*cos(22.5[deg])` and export target outputs to CSV.
- Sweep `p1y` around the learned value `(40[cm]-6[cm]-(10.5[cm]-1.75[cm]))*sin(22.5[deg])` and export target outputs to CSV.
- Sweep `p2y` around the learned value `9[cm]/2` and export target outputs to CSV.
- Sweep `rad` around the learned value `p1y-p2y` and export target outputs to CSV.
...
- Summary: 已学习 sector_generator_3d_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=14 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 14 条；几何=3，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：sector_generator_3d_geom_sequence.m, sector_generator_3d_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 sector_generator_3d_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `sector_generator_3d_geom_sequence.mph` (comsol_mph, 3377169 bytes)
- `sector_generator_3d_geom_sequence.m` (matlab_livelink, 18666 bytes)
- `sector_generator_3d_geom_sequence.java` (comsol_java, 19699 bytes)
- `sector_generator_3d_geom_sequence_summary.json` (json, 827 bytes)
- `models.acdc.sector_generator_3d.pdf` (pdf_document, 3925014 bytes)
