...
- Case directory: `D:\桌面\codex\案例下载\COMSOL\电传感器批处理扫描`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-07-06T09:42:30.525997+00:00`
...
- 模型树证据数量: 28
- 理论关键词: 电磁
- geometry: wp1 / WorkPlane, r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, uni1 / Union, e1 / Ellipse, e2 / Ellipse, geom1 / wp1 / unite, geom1 / wp1 / quickz
...
- `electric_sensor_batch_sweep.java`: 参数=0, 几何=9, 物理场=2, 研究=3, 结果=0
- `electric_sensor_batch_sweep.mph`: 参数=0, 几何=9, 物理场=2, 研究=3, 结果=0
- `electric_sensor_batch_sweep.m`: 参数=0, 几何=9, 物理场=2, 研究=2, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: 电传感器批处理扫描 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electrostatics，并采用 Stationary 研究。
...
- Summary: 已学习 电传感器批处理扫描 的 3 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=9，物理场=2，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：electric_sensor_batch_sweep.java, electric_sensor_batch_sweep.m
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 电传感器批处理扫描，训练阶段为 modeling_logic_learning_ready。
...
- `热执行器代理模型 App` score=0.1571; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42
- `热执行器` score=0.1267; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=30

### Official Documentation Checks

- `IntroductionToCOMSOLMultiphysics.pdf` page 262; module=COMSOL_Multiphysics; score=0.109
- `LiveLinkForSimulinkUsersGuide.pdf` page 20; module=LiveLink_for_Simulink; score=0.1069
- `LiveLinkForMATLABUsersGuide.pdf` page 198; module=LiveLink_for_MATLAB; score=0.1068
...
- `electric_sensor_batch_sweep.java` (comsol_java, 24747 bytes)
- `electric_sensor_batch_sweep.m` (matlab_livelink, 25538 bytes)
- `electric_sensor_batch_sweep.mph` (comsol_mph, 13567973 bytes)
