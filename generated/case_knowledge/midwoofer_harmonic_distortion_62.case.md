# midwoofer_harmonic_distortion_62

- Case directory: `D:\桌面\codex\案例下载\电气\使用 KLIPPEL 测量分析中低音扬声器的谐波失真`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:16:57.144821+00:00`
...
- `f0` = `50[Hz]` - Frequency
- `T0` = `1/f0` - Period
- `NT` = `9` - Number of periods solved
- `N0` = `3` - Number of harmoincs resolved
- `dt` = `1/(20*f0*N0)` - Store time step resolution
- `V0` = `sqrt(2)*Vrms` - Voltage amplitude
- `Vrms` = `2[V]` - Voltrage rms
- `a` = `sqrt(Sd/pi)` - Effective piston radius
- `Sd` = `0.005411[m^2]` - Diaphragm area
- `fmax` = `100[Hz]` - Maximum frequency
...
- `midwoofer_harmonic_distortion_62.m`: 参数=10, 几何=0, 物理场=0, 研究=0, 结果=0
- `midwoofer_harmonic_distortion_62.java`: 参数=10, 几何=0, 物理场=0, 研究=0, 结果=0
- `midwoofer_harmonic_distortion_62.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `midwoofer_harmonic_distortion_62_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: midwoofer_harmonic_distortion_62 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Pressure Acoustics。
...
- `NT` -> Number of periods solved; MATLAB: `model.param.set('NT', value, description)`
- `N0` -> Number of harmoincs resolved; MATLAB: `model.param.set('N0', value, description)`
- `dt` -> Store time step resolution; MATLAB: `model.param.set('dt', value, description)`
- `V0` -> Voltage amplitude; MATLAB: `model.param.set('V0', value, description)`
- `Vrms` -> Voltrage rms; MATLAB: `model.param.set('Vrms', value, description)`
- `a` -> Effective piston radius; MATLAB: `model.param.set('a', value, description)`
- `Sd` -> Diaphragm area; MATLAB: `model.param.set('Sd', value, description)`
- `fmax` -> Maximum frequency; MATLAB: `model.param.set('fmax', value, description)`
...
- 已识别参数：f0、T0、NT、N0、dt、V0、Vrms、a、Sd、fmax
...
- Sweep `NT` around the learned value `9` and export target outputs to CSV.
- Sweep `N0` around the learned value `3` and export target outputs to CSV.
- Sweep `dt` around the learned value `1/(20*f0*N0)` and export target outputs to CSV.
- Sweep `V0` around the learned value `sqrt(2)*Vrms` and export target outputs to CSV.
- Sweep `Vrms` around the learned value `2[V]` and export target outputs to CSV.
- Sweep `a` around the learned value `sqrt(Sd/pi)` and export target outputs to CSV.
...
- Summary: 已学习 midwoofer_harmonic_distortion_62 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=10 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 10 条；几何=0，物理场=0，材料=0，网格=0，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：midwoofer_harmonic_distortion_62.m, midwoofer_harmonic_distortion_62.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 midwoofer_harmonic_distortion_62，训练阶段为 modeling_logic_learning_ready。
...
- `midwoofer_harmonic_distortion_62.mph` (comsol_mph, 107092296 bytes)
- `midwoofer_harmonic_distortion_62.m` (matlab_livelink, 77180 bytes)
- `midwoofer_harmonic_distortion_62.java` (comsol_java, 84051 bytes)
- `midwoofer_harmonic_distortion_62_summary.json` (json, 1910 bytes)
