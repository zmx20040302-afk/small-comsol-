...
- Case directory: `D:\桌面\codex\案例下载\电气\磁流体动力学泵`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:13:21.382121+00:00`
...
- `N` = `10` - number or turns in coils
- `I0` = `1[A]` - coil current magnitude
- `I1` = `I0*exp(-i*120[deg])` - phase 1
- `I2` = `I0` - phase 2
- `I3` = `I0*exp(i*120[deg])` - phase 3

## 内容级读取结果

- 模型树证据数量: 39
- 理论关键词: 电磁, 流体, 结构力学, 多孔介质/裂隙
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, r4 / Rectangle, copy1 / Copy, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r1 / layername, geom1 / r1 / layerright, geom1 / r1 / layerbottom, geom1 / r2 / size, geom1 / r2 / pos
- physics: Laminar Flow, Magnetic Fields
- mesh: mesh1
- studies: Stationary, Frequency Domain
- boundary_conditions: r1 / size, r1 / pos, r1 / layername, r1 / layerright, r1 / layerbottom, r2 / size, r2 / pos, r3 / size, r3 / pos, r4 / size, r4 / base, r4 / pos

### 文件内容证据

- `magnetohydrodynamics_pump.java`: 参数=5, 几何=17, 物理场=0, 研究=0, 结果=0
- `models.acdc.magnetohydrodynamics_pump.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `magnetohydrodynamics_pump_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetohydrodynamics_pump.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetohydrodynamics_pump.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.magnetohydrodynamics_pump.pdf`: models.acdc.magnetohydrodynamics_pump.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Laminar Flow、Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The model is set up in a 2D axisymmetric geometry using the Magnetic Fields and Laminar

## PDF Summary and Physics Judgement

- Simple summary: magnetohydrodynamics_pump 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- PDF: The model is set up in a 2D axisymmetric geometry using the Magnetic Fields and Laminar
- PDF: Flow physics interfaces, coupled via the Magnetohydrodynamics multiphysics interface.
- PDF: The model coupling relies on separate study types for the two physics interfaces, where the
- 脚本/文本识别到的物理场: Laminar Flow, Magnetic Fields
...
- PDF keywords: 电磁, 流体, 结构力学, 多孔介质/裂隙, models.acdc.magnetohydrodynamics_pump.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Laminar Flow、Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The model is set up in a 2D axisymmetric geometry using the Magnetic Fields and Laminar
- `N` -> number or turns in coils; MATLAB: `model.param.set('N', value, description)`
- `I0` -> coil current magnitude; MATLAB: `model.param.set('I0', value, description)`
- `I1` -> phase 1; MATLAB: `model.param.set('I1', value, description)`
- `I2` -> phase 2; MATLAB: `model.param.set('I2', value, description)`
- `I3` -> phase 3; MATLAB: `model.param.set('I3', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；r4 / Rectangle；copy1 / Copy；geom1 / r1 / size
- 已识别物理场证据：Laminar Flow；Magnetic Fields
- 已识别参数：N、I0、I1、I2、I3
...
- Transfer recognized physics interfaces to similar requirements: Laminar Flow, Magnetic Fields.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 流体, 结构力学, 多孔介质/裂隙?
...
- Sweep `N` around the learned value `10` and export target outputs to CSV.
- Sweep `I0` around the learned value `1[A]` and export target outputs to CSV.
- Sweep `I1` around the learned value `I0*exp(-i*120[deg])` and export target outputs to CSV.
- Sweep `I2` around the learned value `I0` and export target outputs to CSV.
- Sweep `I3` around the learned value `I0*exp(i*120[deg])` and export target outputs to CSV.
...
- Summary: 已学习 magnetohydrodynamics_pump 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=39 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 39 条；几何=17，物理场=2，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：magnetohydrodynamics_pump.m, magnetohydrodynamics_pump.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 magnetohydrodynamics_pump，训练阶段为 modeling_logic_learning_ready。
...
- `magnetohydrodynamics_pump.mph` (comsol_mph, 9655699 bytes)
- `magnetohydrodynamics_pump.m` (matlab_livelink, 44228 bytes)
- `magnetohydrodynamics_pump.java` (comsol_java, 40700 bytes)
- `magnetohydrodynamics_pump_summary.json` (json, 1708 bytes)
- `models.acdc.magnetohydrodynamics_pump.pdf` (pdf_document, 2090184 bytes)
