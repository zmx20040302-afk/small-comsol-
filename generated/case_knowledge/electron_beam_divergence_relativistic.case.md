...
- Case directory: `D:\桌面\codex\案例下载\电气\相对论发散电子束`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:29:57.813550+00:00`
...
- `r0beam` = `0.01[m]` - Initial beam radius
- `r0` = `0.05[m]` - Domain radius
- `L` = `0.2[m]` - Domain length
- `Ibeam` = `3[A]` - Beam current magnitude
- `v0beam` = `1e8[m/s]` - Initial beam velocity
- `gamma` = `1/sqrt(1-(v0beam/c_const)^2)` - Relativistic factor
- `K` = `e_const*Ibeam/(2*pi*epsilon0_const*me_const*(v0beam*gamma)^3)` - Generalized perveance
- `hmax` = `0.01` - Maximum mesh element size

## 内容级读取结果

- 模型树证据数量: 24
- 理论关键词: 流体, 电磁, 结构力学
- geometry: cyl1 / Cylinder, wp1 / WorkPlane, c1 / Circle, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / wp1 / planetype, geom1 / wp1 / unite
- physics: Electrostatics, Magnetic Fields, Particle Tracing
- mesh: mesh1
- boundary_conditions: cyl1 / r, cyl1 / h, wp1 / planetype, wp1 / unite, c1 / r

### 文件内容证据

- `electron_beam_divergence_relativistic.java`: 参数=8, 几何=7, 物理场=0, 研究=0, 结果=0
- `models.acdc.electron_beam_divergence_relativistic.pdf`: 参数=0, 几何=0, 物理场=3, 研究=0, 结果=0
- `models.particle.electron_beam_divergence_relativistic.pdf`: 参数=0, 几何=0, 物理场=3, 研究=0, 结果=0
- `electron_beam_divergence_relativistic.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `electron_beam_divergence_relativistic.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `electron_beam_divergence_relativistic_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.electron_beam_divergence_relativistic.pdf`: models.acdc.electron_beam_divergence_relativistic.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics、Magnetic Fields、Particle Tracing；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion
- `models.particle.electron_beam_divergence_relativistic.pdf`: models.particle.electron_beam_divergence_relativistic.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics、Magnetic Fields、Particle Tracing；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion

## PDF Summary and Physics Judgement

- Simple summary: electron_beam_divergence_relativistic 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics。
...
- PDF: from the beam waist as a function of beam radius is compared to the result of Equation 2
- PDF: using a Global Evaluation. The results agree to within a few percentage points. The sources
- 脚本/文本识别到的物理场: Electrostatics, Magnetic Fields, Particle Tracing
- 理论关键词: 流体, 电磁, 结构力学

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 流体, 电磁, 结构力学, models.acdc.electron_beam_divergence_relativistic.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics、Magnetic Fields、Particle Tracing；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion, models.particle.electron_beam_divergence_relativistic.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics、Magnetic Fields、Particle Tracing；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion
- `r0beam` -> Initial beam radius; MATLAB: `model.param.set('r0beam', value, description)`
- `r0` -> Domain radius; MATLAB: `model.param.set('r0', value, description)`
- `L` -> Domain length; MATLAB: `model.param.set('L', value, description)`
- `Ibeam` -> Beam current magnitude; MATLAB: `model.param.set('Ibeam', value, description)`
- `v0beam` -> Initial beam velocity; MATLAB: `model.param.set('v0beam', value, description)`
- `gamma` -> Relativistic factor; MATLAB: `model.param.set('gamma', value, description)`
- `K` -> Generalized perveance; MATLAB: `model.param.set('K', value, description)`
- `hmax` -> Maximum mesh element size; MATLAB: `model.param.set('hmax', value, description)`
...
- 已识别几何证据：cyl1 / Cylinder；wp1 / WorkPlane；c1 / Circle；geom1 / cyl1 / r；geom1 / cyl1 / h；geom1 / wp1 / planetype
- 已识别物理场证据：Electrostatics；Magnetic Fields；Particle Tracing
- 已识别参数：r0beam、r0、L、Ibeam、v0beam、gamma、K、hmax
...
- Transfer recognized physics interfaces to similar requirements: Electrostatics, Magnetic Fields, Particle Tracing.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 电磁, 结构力学?
...
- Sweep `r0beam` around the learned value `0.01[m]` and export target outputs to CSV.
- Sweep `r0` around the learned value `0.05[m]` and export target outputs to CSV.
- Sweep `L` around the learned value `0.2[m]` and export target outputs to CSV.
- Sweep `Ibeam` around the learned value `3[A]` and export target outputs to CSV.
- Sweep `v0beam` around the learned value `1e8[m/s]` and export target outputs to CSV.
- Sweep `gamma` around the learned value `1/sqrt(1-(v0beam/c_const)^2)` and export target outputs to CSV.
- Sweep `K` around the learned value `e_const*Ibeam/(2*pi*epsilon0_const*me_const*(v0beam*gamma)^3)` and export target outputs to CSV.
- Sweep `hmax` around the learned value `0.01` and export target outputs to CSV.
...
- Summary: 已学习 electron_beam_divergence_relativistic 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=24 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 24 条；几何=7，物理场=3，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：electron_beam_divergence_relativistic.m, electron_beam_divergence_relativistic.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 electron_beam_divergence_relativistic，训练阶段为 modeling_logic_learning_ready。
...
- `electron_beam_divergence_relativistic.mph` (comsol_mph, 125737372 bytes)
- `electron_beam_divergence_relativistic.m` (matlab_livelink, 40395 bytes)
- `electron_beam_divergence_relativistic.java` (comsol_java, 35207 bytes)
- `electron_beam_divergence_relativistic_summary.json` (json, 1537 bytes)
- `models.acdc.electron_beam_divergence_relativistic.pdf` (pdf_document, 616490 bytes)
- `models.particle.electron_beam_divergence_relativistic.pdf` (pdf_document, 616364 bytes)
