...
- `wm0` = `1000[rpm]*2*pi` - Angular rotational speed
...
- 模型树证据数量: 23
- 理论关键词: 流体
- geometry: c1 / Circle, c2 / Circle, dif1 / Difference, rot1 / Rotate, r1 / Rectangle, geom1 / c1 / r, geom1 / c1 / angle, geom1 / c2 / r, geom1 / c2 / angle, geom1 / rot1 / rot, geom1 / rot1 / pos
- mesh: mesh1
- boundary_conditions: c1 / r, c1 / angle, c2 / r, c2 / angle, rot1 / rot, rot1 / pos

### 文件内容证据

- `edw_maglev_rotating_transient.java`: 参数=5, 几何=11, 物理场=0, 研究=0, 结果=0
- `edw_maglev_rotating_transient.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `edw_maglev_rotating_transient.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `edw_maglev_rotating_transient_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: edw_maglev_rotating_transient 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- Sweep `wm0` around the learned value `1000[rpm]*2*pi` and export target outputs to CSV.
...
- Summary: 已学习 edw_maglev_rotating_transient 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=11，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：edw_maglev_rotating_transient.m, edw_maglev_rotating_transient.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 edw_maglev_rotating_transient，训练阶段为 modeling_logic_learning_ready。
...
- `edw_maglev_rotating_transient.mph` (comsol_mph, 26615010 bytes)
- `edw_maglev_rotating_transient.m` (matlab_livelink, 48155 bytes)
- `edw_maglev_rotating_transient.java` (comsol_java, 44794 bytes)
- `edw_maglev_rotating_transient_summary.json` (json, 1083 bytes)
