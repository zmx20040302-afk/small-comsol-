# vacancy_electromigration_3D

- Case directory: `D:\桌面\codex\案例下载\电气\互连结构中的空位电迁移`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:52:01.278213+00:00`
...
- `Height` = `15[um]` - Height
- `Thick` = `10[um]` - Thickness
- `Width` = `50[um]` - Width
- `Ri` = `2[um]` - Bottom radius
- `Ro` = `3[um]` - Outer radius
- `HeightCone` = `Thick/2` - Cone height
- `ThickCoat` = `1[um]` - Coating thickness

## 内容级读取结果

- 模型树证据数量: 19
- 理论关键词: 传热, 电磁, 结构力学
- geometry: blk1 / Block, geom1 / blk1 / size, geom1 / blk1 / axis, geom1 / blk1 / selresult, geom1 / blk1 / color
- physics: Heat Transfer, Electric Currents
- mesh: mesh2
- boundary_conditions: blk1 / size, blk1 / axis, blk1 / selresult, blk1 / color

### 文件内容证据

- `vacancy_electromigration_3D.java`: 参数=7, 几何=5, 物理场=2, 研究=0, 结果=0
- `vacancy_electromigration_3D.m`: 参数=7, 几何=0, 物理场=2, 研究=0, 结果=0
- `vacancy_electromigration_3D_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `vacancy_electromigration_3D.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: vacancy_electromigration_3D 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF keywords: 传热, 电磁, 结构力学
- `Height` -> Height; MATLAB: `model.param.set('Height', value, description)`
- `Thick` -> Thickness; MATLAB: `model.param.set('Thick', value, description)`
- `Width` -> Width; MATLAB: `model.param.set('Width', value, description)`
- `Ri` -> Bottom radius; MATLAB: `model.param.set('Ri', value, description)`
- `Ro` -> Outer radius; MATLAB: `model.param.set('Ro', value, description)`
- `HeightCone` -> Cone height; MATLAB: `model.param.set('HeightCone', value, description)`
- `ThickCoat` -> Coating thickness; MATLAB: `model.param.set('ThickCoat', value, description)`
...
- 已识别几何证据：blk1 / Block；geom1 / blk1 / size；geom1 / blk1 / axis；geom1 / blk1 / selresult；geom1 / blk1 / color
- 已识别物理场证据：Heat Transfer；Electric Currents
- 已识别参数：Height、Thick、Width、Ri、Ro、HeightCone、ThickCoat
...
- Sweep `Height` around the learned value `15[um]` and export target outputs to CSV.
- Sweep `Thick` around the learned value `10[um]` and export target outputs to CSV.
- Sweep `Width` around the learned value `50[um]` and export target outputs to CSV.
- Sweep `Ri` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `Ro` around the learned value `3[um]` and export target outputs to CSV.
- Sweep `HeightCone` around the learned value `Thick/2` and export target outputs to CSV.
- Sweep `ThickCoat` around the learned value `1[um]` and export target outputs to CSV.
...
- Summary: 已学习 vacancy_electromigration_3D 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=19 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 19 条；几何=5，物理场=2，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：vacancy_electromigration_3D.m, vacancy_electromigration_3D.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 vacancy_electromigration_3D，训练阶段为 modeling_logic_learning_ready。
...
- `vacancy_electromigration_3D.mph` (comsol_mph, 1065496224 bytes)
- `vacancy_electromigration_3D.m` (matlab_livelink, 85570 bytes)
- `vacancy_electromigration_3D.java` (comsol_java, 90876 bytes)
- `vacancy_electromigration_3D_summary.json` (json, 3182 bytes)
