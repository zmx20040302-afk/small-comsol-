...
- Case directory: `D:\桌面\codex\案例下载\电气\球体之间的电击穿`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:29:55.608800+00:00`
...
- `Vapp` = `-51.8[kV]` - Applied voltage
- `a` = `1.25[cm]` - Sphere diameter
- `hg` = `2.25[cm]` - Gap 1
- `d` = `2*hg-2*a` - Distance between spheres

## 内容级读取结果

- 模型树证据数量: 29
- 理论关键词: 电磁, 传热, 流体
- geometry: sph1 / Sphere, rot1 / Rotate, sph2 / Sphere, blk1 / Block, dif1 / Difference, geom1 / sph1 / r, geom1 / sph1 / rot, geom1 / rot1 / axis, geom1 / sph2 / r, geom1 / sph2 / pos, geom1 / blk1 / size, geom1 / blk1 / pos
- physics: es / Electrostatics, Electrostatics
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: gnd1 / 14, 15, 16, 17, 18, 19, 20, 21, sph1 / r, sph1 / rot, rot1 / axis, sph2 / r, sph2 / pos, blk1 / size, blk1 / pos

### 文件内容证据

- `breakdown_between_spheres.java`: 参数=4, 几何=12, 物理场=2, 研究=0, 结果=0
- `models.edis.breakdown_between_spheres.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `models.plasma.breakdown_between_spheres.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `breakdown_between_spheres.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `breakdown_between_spheres.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `breakdown_between_spheres_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.breakdown_between_spheres.pdf`: models.edis.breakdown_between_spheres.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；parameters including geometry, applied voltage, fill gas, pressure and temperature. A fully
- `models.plasma.breakdown_between_spheres.pdf`: models.plasma.breakdown_between_spheres.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；parameters including geometry, applied voltage, fill gas, pressure and temperature. A fully

## PDF Summary and Physics Judgement

- Simple summary: breakdown_between_spheres 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: parameters including geometry, applied voltage, fill gas, pressure and temperature. A fully
- PDF: geometry is complicated. It is possible to estimate whether electrical breakdown will occur
- PDF: The Electrical Breakdown Detection physics interface defines a variable, ebd.bi which
...
- PDF keywords: 电磁, 传热, 流体, models.edis.breakdown_between_spheres.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；parameters including geometry, applied voltage, fill gas, pressure and temperature. A fully, models.plasma.breakdown_between_spheres.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；parameters including geometry, applied voltage, fill gas, pressure and temperature. A fully
- `Vapp` -> Applied voltage; MATLAB: `model.param.set('Vapp', value, description)`
- `a` -> Sphere diameter; MATLAB: `model.param.set('a', value, description)`
- `hg` -> Gap 1; MATLAB: `model.param.set('hg', value, description)`
- `d` -> Distance between spheres; MATLAB: `model.param.set('d', value, description)`
...
- 已识别几何证据：sph1 / Sphere；rot1 / Rotate；sph2 / Sphere；blk1 / Block；dif1 / Difference；geom1 / sph1 / r
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 传热, 流体?
...
- Sweep `Vapp` around the learned value `-51.8[kV]` and export target outputs to CSV.
- Sweep `a` around the learned value `1.25[cm]` and export target outputs to CSV.
- Sweep `hg` around the learned value `2.25[cm]` and export target outputs to CSV.
- Sweep `d` around the learned value `2*hg-2*a` and export target outputs to CSV.
...
- Summary: 已学习 breakdown_between_spheres 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=29 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 29 条；几何=12，物理场=2，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：breakdown_between_spheres.m, breakdown_between_spheres.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 breakdown_between_spheres，训练阶段为 modeling_logic_learning_ready。
...
- `breakdown_between_spheres.mph` (comsol_mph, 121535887 bytes)
- `breakdown_between_spheres.m` (matlab_livelink, 23482 bytes)
- `breakdown_between_spheres.java` (comsol_java, 19547 bytes)
- `breakdown_between_spheres_summary.json` (json, 1298 bytes)
- `models.edis.breakdown_between_spheres.pdf` (pdf_document, 907564 bytes)
- `models.plasma.breakdown_between_spheres.pdf` (pdf_document, 907729 bytes)
