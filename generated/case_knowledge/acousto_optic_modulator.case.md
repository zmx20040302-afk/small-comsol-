...
- Case directory: `D:\桌面\codex\案例下载\电气\声光调制器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:25:36.690780+00:00`
...
- `speed` = `6.17e3[m/s]` - Acoustic speed in solid
- `wl` = `0.5[\u00b5m]` - Acoustic wavelength
- `freq_disp` = `speed/wl` - Acoustic frequency
- `na` = `1` - Refractive index, air
- `N` = `1.445` - Refractive index, SiO2
...
- `f0` = `c_const/lam0` - Optical frequency
- `alpha` = `0` - Angle of incidence
- `a` = `0.5[um]` - Width
- `b` = `3[um]` - Height
...
- 模型树证据数量: 24
- 理论关键词: 结构力学, 声学, 传热, 电磁
- geometry: r1 / Rectangle, geom1 / r1 / size, geom1 / r1 / layername, geom1 / r1 / layertop
- physics: Solid Mechanics, Ray Optics, Electromagnetic Waves
- mesh: mesh1
- studies: Frequency Domain
- boundary_conditions: r1 / size, r1 / layername, r1 / layertop

### 文件内容证据

- `acousto_optic_modulator.java`: 参数=12, 几何=4, 物理场=0, 研究=0, 结果=0
- `models.woptics.acousto_optic_modulator.pdf`: 参数=0, 几何=0, 物理场=3, 研究=1, 结果=0
- `acousto_optic_modulator.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `acousto_optic_modulator.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `acousto_optic_modulator_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.acousto_optic_modulator.pdf`: models.woptics.acousto_optic_modulator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Ray Optics、Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This example demonstrates the basic physics principles for an AOM, with just a single

## PDF Summary and Physics Judgement

- Simple summary: acousto_optic_modulator 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- PDF: This example demonstrates the basic physics principles for an AOM, with just a single
- PDF: This model uses the Solid Mechanics interface with a Frequency Domain study, to model
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: Solid Mechanics, Ray Optics, Electromagnetic Waves
...
- PDF keywords: 结构力学, 声学, 传热, 电磁, models.woptics.acousto_optic_modulator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Ray Optics、Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This example demonstrates the basic physics principles for an AOM, with just a single
- `speed` -> Acoustic speed in solid; MATLAB: `model.param.set('speed', value, description)`
- `wl` -> Acoustic wavelength; MATLAB: `model.param.set('wl', value, description)`
- `freq_disp` -> Acoustic frequency; MATLAB: `model.param.set('freq_disp', value, description)`
- `na` -> Refractive index, air; MATLAB: `model.param.set('na', value, description)`
- `N` -> Refractive index, SiO2; MATLAB: `model.param.set('N', value, description)`
- `B1` -> First stress optical coefficient; MATLAB: `model.param.set('B1', value, description)`
- `B2` -> Second stress optical coefficient; MATLAB: `model.param.set('B2', value, description)`
- `lam0` -> Vacuum wavelength; MATLAB: `model.param.set('lam0', value, description)`
- `f0` -> Optical frequency; MATLAB: `model.param.set('f0', value, description)`
- `alpha` -> Angle of incidence; MATLAB: `model.param.set('alpha', value, description)`
- `a` -> Width; MATLAB: `model.param.set('a', value, description)`
- `b` -> Height; MATLAB: `model.param.set('b', value, description)`
...
- 已识别几何证据：r1 / Rectangle；geom1 / r1 / size；geom1 / r1 / layername；geom1 / r1 / layertop
- 已识别物理场证据：Solid Mechanics；Ray Optics；Electromagnetic Waves
- 已识别参数：speed、wl、freq_disp、na、N、B1、B2、lam0、f0、alpha、a、b
...
- Transfer recognized physics interfaces to similar requirements: Solid Mechanics, Ray Optics, Electromagnetic Waves.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 结构力学, 声学, 传热, 电磁?
...
- Sweep `speed` around the learned value `6.17e3[m/s]` and export target outputs to CSV.
- Sweep `wl` around the learned value `0.5[\u00b5m]` and export target outputs to CSV.
- Sweep `freq_disp` around the learned value `speed/wl` and export target outputs to CSV.
- Sweep `na` around the learned value `1` and export target outputs to CSV.
- Sweep `N` around the learned value `1.445` and export target outputs to CSV.
- Sweep `B1` around the learned value `0.65e-12[m^2/N]` and export target outputs to CSV.
- Sweep `B2` around the learned value `4.2e-12[m^2/N]` and export target outputs to CSV.
...
- Summary: 已学习 acousto_optic_modulator 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=24 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 24 条；几何=4，物理场=3，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：acousto_optic_modulator.m, acousto_optic_modulator.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 acousto_optic_modulator，训练阶段为 modeling_logic_learning_ready。
...
- `acousto_optic_modulator.mph` (comsol_mph, 8425644 bytes)
- `acousto_optic_modulator.m` (matlab_livelink, 48550 bytes)
- `acousto_optic_modulator.java` (comsol_java, 40749 bytes)
- `acousto_optic_modulator_summary.json` (json, 1713 bytes)
- `models.woptics.acousto_optic_modulator.pdf` (pdf_document, 1873105 bytes)
