# photoconductive_antenna

- Case directory: `D:\桌面\codex\案例下载\电气\太赫兹光电导天线 (PCA)`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:39:05.629201+00:00`
...
- `V0` = `50[V]` - Bias voltage
- `lda0` = `800[nm]` - Laser vacuum wavelength
- `w0` = `2.5[um]` - Laser spot size
- `I0` = `1[mW/um^2]` - Laser peak intensity
- `E0` = `sqrt(2*I0/(c_const*epsilon0_const))` - Laser peak field strength
- `tDelay` = `0.5[ps]` - Pulse delay
- `tPulse` = `100[fs]` - Pulse duration
- `fMax` = `5[THz]` - Maximum frequency to resolve
- `dt` = `1/(60*fMax)` - Time step
- `dPulse` = `1[um]` - Pulse depth
- `dSubstrate` = `5[um]` - Substrate thickness

## 内容级读取结果

- 模型树证据数量: 23
- 理论关键词: 传热, 电磁
- geometry: r1 / Rectangle, geom1 / r1 / size, r2 / Rectangle, geom1 / r1 / base, geom1 / r2 / size, geom1 / r2 / base
- mesh: mesh1, mesh2
- boundary_conditions: r1 / size, r1 / base, r2 / size, r2 / base

### 文件内容证据

- `photoconductive_antenna.java`: 参数=11, 几何=6, 物理场=0, 研究=0, 结果=0
- `photoconductive_antenna.m`: 参数=11, 几何=2, 物理场=0, 研究=0, 结果=0
- `photoconductive_antenna.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `photoconductive_antenna_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: photoconductive_antenna 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF keywords: 传热, 电磁
- `V0` -> Bias voltage; MATLAB: `model.param.set('V0', value, description)`
- `lda0` -> Laser vacuum wavelength; MATLAB: `model.param.set('lda0', value, description)`
- `w0` -> Laser spot size; MATLAB: `model.param.set('w0', value, description)`
- `I0` -> Laser peak intensity; MATLAB: `model.param.set('I0', value, description)`
- `E0` -> Laser peak field strength; MATLAB: `model.param.set('E0', value, description)`
- `tDelay` -> Pulse delay; MATLAB: `model.param.set('tDelay', value, description)`
- `tPulse` -> Pulse duration; MATLAB: `model.param.set('tPulse', value, description)`
- `fMax` -> Maximum frequency to resolve; MATLAB: `model.param.set('fMax', value, description)`
- `dt` -> Time step; MATLAB: `model.param.set('dt', value, description)`
- `dPulse` -> Pulse depth; MATLAB: `model.param.set('dPulse', value, description)`
- `dSubstrate` -> Substrate thickness; MATLAB: `model.param.set('dSubstrate', value, description)`
...
- 已识别几何证据：r1 / Rectangle；geom1 / r1 / size；r2 / Rectangle；geom1 / r1 / base；geom1 / r2 / size；geom1 / r2 / base
- 已识别参数：V0、lda0、w0、I0、E0、tDelay、tPulse、fMax、dt、dPulse、dSubstrate
...
- Sweep `V0` around the learned value `50[V]` and export target outputs to CSV.
- Sweep `lda0` around the learned value `800[nm]` and export target outputs to CSV.
- Sweep `w0` around the learned value `2.5[um]` and export target outputs to CSV.
- Sweep `I0` around the learned value `1[mW/um^2]` and export target outputs to CSV.
- Sweep `E0` around the learned value `sqrt(2*I0/(c_const*epsilon0_const))` and export target outputs to CSV.
- Sweep `tDelay` around the learned value `0.5[ps]` and export target outputs to CSV.
- Sweep `tPulse` around the learned value `100[fs]` and export target outputs to CSV.
- Sweep `fMax` around the learned value `5[THz]` and export target outputs to CSV.
...
- Summary: 已学习 photoconductive_antenna 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=6，物理场=0，材料=0，网格=2，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：photoconductive_antenna.m, photoconductive_antenna.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 photoconductive_antenna，训练阶段为 modeling_logic_learning_ready。
...
- `photoconductive_antenna.mph` (comsol_mph, 1730891 bytes)
- `photoconductive_antenna.m` (matlab_livelink, 56369 bytes)
- `photoconductive_antenna.java` (comsol_java, 59913 bytes)
- `photoconductive_antenna_summary.json` (json, 2166 bytes)
