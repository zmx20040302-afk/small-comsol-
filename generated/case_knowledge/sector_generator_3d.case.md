...
- Created at: `2026-08-22T05:29:42.528926+00:00`
...
- `d_wire` = `3[mm]` - Diameter of wire in the winding
- `N` = `100` - Number of turns in the winding
- `rpm` = `60[rpm]` - Angular velocity of the rotor
...
- `sector_generator_3d.java`: 参数=4, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.acdc.sector_generator_3d.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `sector_generator_3d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `sector_generator_3d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `sector_generator_3d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: sector_generator_3d 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF keywords: 流体, 结构力学, 电磁, models.acdc.sector_generator_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；being studied is a simplified electric generator in 3D, based on the geometry used in the
- `d_wire` -> Diameter of wire in the winding; MATLAB: `model.param.set('d_wire', value, description)`
- `N` -> Number of turns in the winding; MATLAB: `model.param.set('N', value, description)`
- `rpm` -> Angular velocity of the rotor; MATLAB: `model.param.set('rpm', value, description)`
...
- Sweep `d_wire` around the learned value `3[mm]` and export target outputs to CSV.
- Sweep `N` around the learned value `100` and export target outputs to CSV.
- Sweep `rpm` around the learned value `60[rpm]` and export target outputs to CSV.
...
- Summary: 已学习 sector_generator_3d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=12 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 12 条；几何=3，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：sector_generator_3d.m, sector_generator_3d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 sector_generator_3d，训练阶段为 modeling_logic_learning_ready。
...
- `sector_generator_3d.mph` (comsol_mph, 73611383 bytes)
- `sector_generator_3d.m` (matlab_livelink, 65272 bytes)
- `sector_generator_3d.java` (comsol_java, 65568 bytes)
- `sector_generator_3d_summary.json` (json, 1910 bytes)
