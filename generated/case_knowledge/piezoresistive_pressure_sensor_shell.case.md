...
- 模型树证据数量: 18
- 理论关键词: 流体, 传热, 结构力学, 电磁
- geometry: wp1 / WorkPlane, sq1 / Square, sq2 / Square, r1 / Rectangle, geom1 / wp1 / unite
- physics: Solid Mechanics, Electric Currents
- mesh: mesh1
- studies: Stationary
- boundary_conditions: wp1 / unite, sq1 / size, sq1 / base, sq1 / pos, sq1 / layerleft, sq1 / layerright, sq1 / layertop, sq2 / size, sq2 / base

### 文件内容证据

- `piezoresistive_pressure_sensor_shell.java`: 参数=0, 几何=5, 物理场=0, 研究=0, 结果=0
...
- `piezoresistive_pressure_sensor_shell.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoresistive_pressure_sensor_shell.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoresistive_pressure_sensor_shell_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: piezoresistive_pressure_sensor_shell 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- 已识别几何证据：wp1 / WorkPlane；sq1 / Square；sq2 / Square；r1 / Rectangle；geom1 / wp1 / unite
...
- Summary: 已学习 piezoresistive_pressure_sensor_shell 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=18 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 18 条；几何=5，物理场=2，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：piezoresistive_pressure_sensor_shell.m, piezoresistive_pressure_sensor_shell.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 piezoresistive_pressure_sensor_shell，训练阶段为 modeling_logic_learning_ready。
...
- `piezoresistive_pressure_sensor_shell.mph` (comsol_mph, 11390380 bytes)
- `piezoresistive_pressure_sensor_shell.m` (matlab_livelink, 62687 bytes)
- `piezoresistive_pressure_sensor_shell.java` (comsol_java, 61265 bytes)
- `piezoresistive_pressure_sensor_shell_summary.json` (json, 1403 bytes)
