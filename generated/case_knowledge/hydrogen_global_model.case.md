...
- Case directory: `D:\桌面\codex\案例下载\电气\氢全局模型与两项玻尔兹曼方程耦合`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:39:50.081984+00:00`
...
- `W` = `2.5[cm]` - Plasma width
- `H` = `3[cm]` - Plasma height
- `Wg` = `5[cm]` - Width for heat
- `Hg` = `6[cm]` - Height for heat
- `p0` = `5[kPa]` - Pressure
- `pw` = `300[W]` - Power
- `Qs` = `500` - Mass flow
- `pd` = `pw/Vol` - Power density
- `Vol` = `(pi*W^2*H)` - Plasma volume
- `f0` = `2.45e9[Hz]` - Excitation frequency
- `w0` = `f0*2*pi` - Angular frequency
- `hl_ions` = `0.05` - Correction factor for ions

## 内容级读取结果

- 模型树证据数量: 19
- 理论关键词: 流体, PDE/方程建模, 传热, 结构力学, 电磁
...
- `hydrogen_global_model.java`: 参数=12, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.plasma.hydrogen_global_model.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `hydrogen_global_model_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `hydrogen_global_model.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `hydrogen_global_model.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.hydrogen_global_model.pdf`: models.plasma.hydrogen_global_model.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；goals are to show how to prepare a global model to study a chemistry and study the

## PDF Summary and Physics Judgement

- Simple summary: hydrogen_global_model 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: goals are to show how to prepare a global model to study a chemistry and study the
- PDF: that need to be done before moving to a space dependent model.
- PDF: The results are in general good agreement with the ones from Ref. 1. However, a detailed
...
- PDF keywords: 流体, PDE/方程建模, 传热, 结构力学, 电磁, models.plasma.hydrogen_global_model.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；goals are to show how to prepare a global model to study a chemistry and study the
- `W` -> Plasma width; MATLAB: `model.param.set('W', value, description)`
- `H` -> Plasma height; MATLAB: `model.param.set('H', value, description)`
- `Wg` -> Width for heat; MATLAB: `model.param.set('Wg', value, description)`
- `Hg` -> Height for heat; MATLAB: `model.param.set('Hg', value, description)`
- `p0` -> Pressure; MATLAB: `model.param.set('p0', value, description)`
- `pw` -> Power; MATLAB: `model.param.set('pw', value, description)`
- `Qs` -> Mass flow; MATLAB: `model.param.set('Qs', value, description)`
- `pd` -> Power density; MATLAB: `model.param.set('pd', value, description)`
- `Vol` -> Plasma volume; MATLAB: `model.param.set('Vol', value, description)`
- `f0` -> Excitation frequency; MATLAB: `model.param.set('f0', value, description)`
- `w0` -> Angular frequency; MATLAB: `model.param.set('w0', value, description)`
- `hl_ions` -> Correction factor for ions; MATLAB: `model.param.set('hl_ions', value, description)`
...
- 已识别参数：W、H、Wg、Hg、p0、pw、Qs、pd、Vol、f0、w0、hl_ions
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, PDE/方程建模, 传热, 结构力学, 电磁?
...
- Sweep `W` around the learned value `2.5[cm]` and export target outputs to CSV.
- Sweep `H` around the learned value `3[cm]` and export target outputs to CSV.
- Sweep `Wg` around the learned value `5[cm]` and export target outputs to CSV.
- Sweep `Hg` around the learned value `6[cm]` and export target outputs to CSV.
- Sweep `p0` around the learned value `5[kPa]` and export target outputs to CSV.
- Sweep `pw` around the learned value `300[W]` and export target outputs to CSV.
- Sweep `Qs` around the learned value `500` and export target outputs to CSV.
- Sweep `pd` around the learned value `pw/Vol` and export target outputs to CSV.
...
- Summary: 已学习 hydrogen_global_model 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=19 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 19 条；几何=2，物理场=2，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：hydrogen_global_model.m, hydrogen_global_model.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 hydrogen_global_model，训练阶段为 modeling_logic_learning_ready。
...
- `hydrogen_global_model.mph` (comsol_mph, 8194076 bytes)
- `hydrogen_global_model.m` (matlab_livelink, 118868 bytes)
- `hydrogen_global_model.java` (comsol_java, 117771 bytes)
- `hydrogen_global_model_summary.json` (json, 3203 bytes)
- `models.plasma.hydrogen_global_model.pdf` (pdf_document, 377255 bytes)
