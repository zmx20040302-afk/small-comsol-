# friis_equation_verification_cleared

- Case directory: `D:\桌面\codex\案例下载\电气\使用 FEM-BEM 耦合验证弗里斯传输方程`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:20:07.726664+00:00`
...
- `d0` = `60[mil]` - Substrate thickness
- `w_line` = `3.2[mm]` - 50 ohm line width
- `w_patch` = `41[mm]` - Patch width
- `l_patch` = `32.4[mm]` - Patch length
- `w_stub` = `4[mm]` - Tuning stub width
- `l_stub` = `9.5[mm]` - Tuning stub length
- `w_sub` = `70[mm]` - Substrate width
- `l_sub` = `70[mm]` - Substrate length
- `freq_min` = `2.4[GHz]` - Minimum frequency
- `freq_max` = `2.5[GHz]` - Maximum frequency
- `lda_min` = `c_const/freq_max` - Minimum wavelength
- `dist` = `lda_min*10` - distance between Tx and Rx antenna

## 内容级读取结果

- 模型树证据数量: 28
- 理论关键词: PDE/方程建模
- geometry: blk1 / Block, blk2 / Block, geom1 / blk1 / size, geom1 / blk1 / base, blk3 / Block, geom1 / blk2 / size, geom1 / blk2 / base, geom1 / blk3 / size, geom1 / blk3 / base
- mesh: mesh1
- boundary_conditions: blk1 / size, blk1 / base, blk2 / size, blk2 / base, blk3 / size, blk3 / base

### 文件内容证据

- `friis_equation_verification_cleared.java`: 参数=12, 几何=9, 物理场=0, 研究=0, 结果=0
- `friis_equation_verification_cleared.m`: 参数=12, 几何=4, 物理场=0, 研究=0, 结果=0
- `friis_equation_verification_cleared_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `friis_equation_verification_cleared.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: friis_equation_verification_cleared 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF keywords: PDE/方程建模
- `d0` -> Substrate thickness; MATLAB: `model.param.set('d0', value, description)`
...
- `dist` -> distance between Tx and Rx antenna; MATLAB: `model.param.set('dist', value, description)`
...
- 已识别几何证据：blk1 / Block；blk2 / Block；geom1 / blk1 / size；geom1 / blk1 / base；blk3 / Block；geom1 / blk2 / size
- 已识别参数：d0、w_line、w_patch、l_patch、w_stub、l_stub、w_sub、l_sub、freq_min、freq_max、lda_min、dist
...
- How do the theory keywords change governing equations, assumptions, or validation targets: PDE/方程建模?
...
- Sweep `d0` around the learned value `60[mil]` and export target outputs to CSV.
- Sweep `w_line` around the learned value `3.2[mm]` and export target outputs to CSV.
- Sweep `w_patch` around the learned value `41[mm]` and export target outputs to CSV.
- Sweep `l_patch` around the learned value `32.4[mm]` and export target outputs to CSV.
- Sweep `w_stub` around the learned value `4[mm]` and export target outputs to CSV.
- Sweep `l_stub` around the learned value `9.5[mm]` and export target outputs to CSV.
- Sweep `w_sub` around the learned value `70[mm]` and export target outputs to CSV.
- Sweep `l_sub` around the learned value `70[mm]` and export target outputs to CSV.
...
- Summary: 已学习 friis_equation_verification_cleared 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=9，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：friis_equation_verification_cleared.m, friis_equation_verification_cleared.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 friis_equation_verification_cleared，训练阶段为 modeling_logic_learning_ready。
...
- `friis_equation_verification_cleared.mph` (comsol_mph, 4951068 bytes)
- `friis_equation_verification_cleared.m` (matlab_livelink, 109620 bytes)
- `friis_equation_verification_cleared.java` (comsol_java, 118489 bytes)
- `friis_equation_verification_cleared_summary.json` (json, 2285 bytes)
