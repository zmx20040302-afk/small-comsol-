...
- Case directory: `D:\桌面\codex\案例下载\电气\差分微带线建模`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:06:03.992208+00:00`
...
- 模型树证据数量: 18
- 理论关键词: 传热, 电磁, 多孔介质/裂隙
- geometry: blk1 / Block, wp1 / WorkPlane, r1 / Rectangle, mir1 / Mirror, geom1 / blk1 / size, geom1 / blk1 / pos, geom1 / blk1 / layername, geom1 / wp1 / quickz, geom1 / wp1 / unite
- physics: Electromagnetic Waves
- mesh: mesh1
- studies: Frequency Domain
- boundary_conditions: blk1 / size, blk1 / pos, blk1 / layername, wp1 / quickz, wp1 / unite, r1 / size

### 文件内容证据

- `microstrip_line_tem_differential.java`: 参数=0, 几何=9, 物理场=0, 研究=0, 结果=0
- `models.rf.microstrip_line_tem_differential.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `microstrip_line_tem_differential.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `microstrip_line_tem_differential.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `microstrip_line_tem_differential_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.rf.microstrip_line_tem_differential.pdf`: models.rf.microstrip_line_tem_differential.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The physics settings include all possible modeling cases such as single-ended line,

## PDF Summary and Physics Judgement

- Simple summary: microstrip_line_tem_differential 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- PDF: The physics settings include all possible modeling cases such as single-ended line,
- PDF: differential line, and a fictitious noise. In each study step, these settings can selectively be
...
- PDF keywords: 传热, 电磁, 多孔介质/裂隙, models.rf.microstrip_line_tem_differential.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The physics settings include all possible modeling cases such as single-ended line,
...
- 已识别几何证据：blk1 / Block；wp1 / WorkPlane；r1 / Rectangle；mir1 / Mirror；geom1 / blk1 / size；geom1 / blk1 / pos
...
- Summary: 已学习 microstrip_line_tem_differential 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=18 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 18 条；几何=9，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：microstrip_line_tem_differential.m, microstrip_line_tem_differential.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 microstrip_line_tem_differential，训练阶段为 modeling_logic_learning_ready。
...
- `microstrip_line_tem_differential.mph` (comsol_mph, 96613974 bytes)
- `microstrip_line_tem_differential.m` (matlab_livelink, 61165 bytes)
- `microstrip_line_tem_differential.java` (comsol_java, 61039 bytes)
- `microstrip_line_tem_differential_summary.json` (json, 1343 bytes)
- `models.rf.microstrip_line_tem_differential.pdf` (pdf_document, 5697631 bytes)
