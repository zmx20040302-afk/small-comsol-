...
- Created at: `2026-08-22T05:38:45.830618+00:00`
...
- `va` = `64[deg/s]` - Rotation angular velocity
...
- 模型树证据数量: 26
- 理论关键词: 电磁, 流体, 结构力学, 声学, 优化/参数扫描
- geometry: wp1 / WorkPlane, r1 / Rectangle, geom1 / wp1 / quickz, geom1 / wp1 / unite
- physics: Solid Mechanics, Electrostatics
- mesh: mesh1
- studies: Eigenfrequency, Frequency Domain
- boundary_conditions: wp1 / quickz, wp1 / unite, r1 / base

### 文件内容证据

- `piezoelectric_rate_gyroscope.java`: 参数=14, 几何=4, 物理场=0, 研究=0, 结果=0
- `models.mems.piezoelectric_rate_gyroscope.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `piezoelectric_rate_gyroscope.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoelectric_rate_gyroscope.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoelectric_rate_gyroscope_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: piezoelectric_rate_gyroscope 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenfrequency 研究。
...
- PDF keywords: 电磁, 流体, 结构力学, 声学, 优化/参数扫描, models.mems.piezoelectric_rate_gyroscope.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Eigenfrequency、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；coupled to an out of plane mode by the Coriolis force and the resulting out of plane
- `va` -> Rotation angular velocity; MATLAB: `model.param.set('va', value, description)`
...
- 已识别几何证据：wp1 / WorkPlane；r1 / Rectangle；geom1 / wp1 / quickz；geom1 / wp1 / unite
- 已识别物理场证据：Solid Mechanics；Electrostatics
- 已识别参数：va、tQz、w_m、l_w、w_w、l_f、w_f、w_tbf、w_sf、l_d、w_d、l_s
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 流体, 结构力学, 声学, 优化/参数扫描?
...
- Sweep `va` around the learned value `64[deg/s]` and export target outputs to CSV.
...
- Summary: 已学习 piezoelectric_rate_gyroscope 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=14；内容证据=26 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 26 条；几何=4，物理场=2，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：piezoelectric_rate_gyroscope.m, piezoelectric_rate_gyroscope.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 piezoelectric_rate_gyroscope，训练阶段为 modeling_logic_learning_ready。
...
- `piezoelectric_rate_gyroscope.mph` (comsol_mph, 716742936 bytes)
- `piezoelectric_rate_gyroscope.m` (matlab_livelink, 55198 bytes)
- `piezoelectric_rate_gyroscope.java` (comsol_java, 53436 bytes)
- `piezoelectric_rate_gyroscope_summary.json` (json, 1837 bytes)
