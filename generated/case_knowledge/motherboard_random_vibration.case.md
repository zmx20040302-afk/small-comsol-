...
- Case directory: `D:\桌面\codex\案例下载\电气\主板的随机振动试验`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T07:09:25.334780+00:00`
...
- `fL` = `20` - Lower frequency limit of input spectrum
- `fU` = `2000` - Upper frequency limit of input spectrum
- `nFd` = `100` - Number of frequency steps per decade in evaluations
- `nF` = `ceil(nFd*log10(fU/fL))` - Total number of frequency steps in evaluations
- `peakFactor` = `3` - Multiplier from RMS to Peak
- `screwArea` = `5[mm^2]` - Stress area in M3 bolt
- `boltForce` = `screwArea*200[MPa]` - Pretension force in bolts
- `friction` = `0.12` - Friction coefficient under bolt head
- `allowedShear` = `boltForce*friction` - Max shear force without sliding
- `accX` = `1[m/s^2]` - X-direction acceleration, dummy parameter
- `accY` = `1[m/s^2]` - Y-direction acceleration, dummy parameter
- `accZ` = `1[m/s^2]` - Z-direction acceleration, dummy parameter
...
- mesh: mesh1
- studies: Eigenvalue, Eigenfrequency
- boundary_conditions: imp1 / type, imp1 / filename

### 文件内容证据

- `motherboard_random_vibration.java`: 参数=12, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.mems.motherboard_random_vibration.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `models.sme.motherboard_random_vibration.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `motherboard_random_vibration.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `motherboard_random_vibration.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `motherboard_random_vibration_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.motherboard_random_vibration.pdf`: models.mems.motherboard_random_vibration.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Eigenvalue、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；GEOMETRY
- `models.sme.motherboard_random_vibration.pdf`: models.sme.motherboard_random_vibration.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Eigenvalue、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；GEOMETRY

## PDF Summary and Physics Judgement

- Simple summary: motherboard_random_vibration 主要研究声压、模态或频域声学响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenvalue 研究。
- Primary physics: `Solid Mechanics`
- Study type: `Eigenvalue`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Solid Mechanics，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Eigenvalue 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: GEOMETRY
- PDF: Figure 1: Motherboard geometry.
- PDF: a ground motion when a modal based study is used. Instead, the Base Excitation feature is
- 脚本/文本识别到的物理场: Solid Mechanics
- 识别到的研究类型: Eigenvalue, Eigenfrequency
- 理论关键词: 结构力学, 声学

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 结构力学, 声学, models.mems.motherboard_random_vibration.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Eigenvalue、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；GEOMETRY, models.sme.motherboard_random_vibration.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Eigenvalue、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；GEOMETRY
- `fL` -> Lower frequency limit of input spectrum; MATLAB: `model.param.set('fL', value, description)`
- `fU` -> Upper frequency limit of input spectrum; MATLAB: `model.param.set('fU', value, description)`
- `nFd` -> Number of frequency steps per decade in evaluations; MATLAB: `model.param.set('nFd', value, description)`
- `nF` -> Total number of frequency steps in evaluations; MATLAB: `model.param.set('nF', value, description)`
- `peakFactor` -> Multiplier from RMS to Peak; MATLAB: `model.param.set('peakFactor', value, description)`
- `screwArea` -> Stress area in M3 bolt; MATLAB: `model.param.set('screwArea', value, description)`
- `boltForce` -> Pretension force in bolts; MATLAB: `model.param.set('boltForce', value, description)`
- `friction` -> Friction coefficient under bolt head; MATLAB: `model.param.set('friction', value, description)`
- `allowedShear` -> Max shear force without sliding; MATLAB: `model.param.set('allowedShear', value, description)`
- `accX` -> X-direction acceleration, dummy parameter; MATLAB: `model.param.set('accX', value, description)`
- `accY` -> Y-direction acceleration, dummy parameter; MATLAB: `model.param.set('accY', value, description)`
- `accZ` -> Z-direction acceleration, dummy parameter; MATLAB: `model.param.set('accZ', value, description)`
...
- 已识别参数：fL、fU、nFd、nF、peakFactor、screwArea、boltForce、friction、allowedShear、accX、accY、accZ
...
- Use the learned study types as solver starting points: Eigenvalue, Eigenfrequency.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 结构力学, 声学?
...
- Sweep `fL` around the learned value `20` and export target outputs to CSV.
- Sweep `fU` around the learned value `2000` and export target outputs to CSV.
- Sweep `nFd` around the learned value `100` and export target outputs to CSV.
- Sweep `nF` around the learned value `ceil(nFd*log10(fU/fL))` and export target outputs to CSV.
- Sweep `peakFactor` around the learned value `3` and export target outputs to CSV.
- Sweep `screwArea` around the learned value `5[mm^2]` and export target outputs to CSV.
- Sweep `boltForce` around the learned value `screwArea*200[MPa]` and export target outputs to CSV.
- Sweep `friction` around the learned value `0.12` and export target outputs to CSV.
...
- Summary: 已学习 motherboard_random_vibration 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=21 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 21 条；几何=3，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：motherboard_random_vibration.m, motherboard_random_vibration.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 motherboard_random_vibration，训练阶段为 modeling_logic_learning_ready。
...
- `motherboard_random_vibration.mph` (comsol_mph, 537542358 bytes)
- `motherboard_random_vibration.m` (matlab_livelink, 82910 bytes)
- `motherboard_random_vibration.java` (comsol_java, 88108 bytes)
- `motherboard_random_vibration_summary.json` (json, 1834 bytes)
- `models.mems.motherboard_random_vibration.pdf` (pdf_document, 1566297 bytes)
- `models.sme.motherboard_random_vibration.pdf` (pdf_document, 1566503 bytes)
