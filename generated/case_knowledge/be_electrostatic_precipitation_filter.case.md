# be_electrostatic_precipitation_filter

- Case directory: `D:\桌面\codex\案例下载\电气\使用基于边界元的静电接口模拟导线、曲面和实体`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:11:35.644755+00:00`
...
- 模型树证据数量: 18
- 理论关键词: 未自动识别
- geometry: wp1 / WorkPlane, b1 / BezierPolygon, fil1 / Fillet, geom1 / wp1 / quickz, geom1 / wp1 / unite
- physics: Electrostatics
- mesh: mesh1
- boundary_conditions: wp1 / quickz, wp1 / unite, b1 / type, fil1 / radius

### 文件内容证据

- `be_electrostatic_precipitation_filter.java`: 参数=7, 几何=5, 物理场=1, 研究=0, 结果=0
- `be_electrostatic_precipitation_filter_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `be_electrostatic_precipitation_filter.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `be_electrostatic_precipitation_filter.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: be_electrostatic_precipitation_filter 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electrostatics。
- Primary physics: `Electrostatics`
- Study type: `unknown`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Electrostatics，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型需要继续从研究步骤确认。
...
- `plate_width` -> 案例参数/扫描变量; MATLAB: `model.param.set('plate_width', value, description)`
- `plate_height` -> 案例参数/扫描变量; MATLAB: `model.param.set('plate_height', value, description)`
- `plate_spacing` -> 案例参数/扫描变量; MATLAB: `model.param.set('plate_spacing', value, description)`
- `wire_axial_pitch` -> 案例参数/扫描变量; MATLAB: `model.param.set('wire_axial_pitch', value, description)`
- `wire_major_r` -> 案例参数/扫描变量; MATLAB: `model.param.set('wire_major_r', value, description)`
- `wire_offset` -> 案例参数/扫描变量; MATLAB: `model.param.set('wire_offset', value, description)`
- `n_o_turns` -> 案例参数/扫描变量; MATLAB: `model.param.set('n_o_turns', value, description)`
...
- 已识别几何证据：wp1 / WorkPlane；b1 / BezierPolygon；fil1 / Fillet；geom1 / wp1 / quickz；geom1 / wp1 / unite
- 已识别物理场证据：Electrostatics
- 已识别参数：plate_width、plate_height、plate_spacing、wire_axial_pitch、wire_major_r、wire_offset、n_o_turns
...
- Sweep `plate_width` around the learned value `3000[mm]` and export target outputs to CSV.
- Sweep `plate_height` around the learned value `3000[mm]` and export target outputs to CSV.
- Sweep `plate_spacing` around the learned value `500[mm]` and export target outputs to CSV.
- Sweep `wire_axial_pitch` around the learned value `150[mm]` and export target outputs to CSV.
- Sweep `wire_major_r` around the learned value `5[mm]` and export target outputs to CSV.
- Sweep `wire_offset` around the learned value `100[mm]` and export target outputs to CSV.
- Sweep `n_o_turns` around the learned value `20` and export target outputs to CSV.
...
- Summary: 已学习 be_electrostatic_precipitation_filter 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=18 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 18 条；几何=5，物理场=1，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：be_electrostatic_precipitation_filter.m, be_electrostatic_precipitation_filter.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 be_electrostatic_precipitation_filter，训练阶段为 modeling_logic_learning_ready。
...
- `be_electrostatic_precipitation_filter.mph` (comsol_mph, 14221570 bytes)
- `be_electrostatic_precipitation_filter.m` (matlab_livelink, 31950 bytes)
- `be_electrostatic_precipitation_filter.java` (comsol_java, 26641 bytes)
- `be_electrostatic_precipitation_filter_summary.json` (json, 1125 bytes)
