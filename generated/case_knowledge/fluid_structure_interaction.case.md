...
- Case directory: `D:\桌面\codex\案例下载\电气\流-固耦合`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:53:43.820741+00:00`
...
- `U` = `3.33[cm/s]` - Inlet mean velocity at steady state
...
- 模型树证据数量: 21
- 理论关键词: 流体, 结构力学, 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, fil1 / Fillet, geom1 / r1 / size, geom1 / r2 / size, geom1 / r2 / pos, geom1 / fil1 / radius
- physics: spf / LaminarFlow, Solid Mechanics, Laminar Flow
- materials: mat1, mat2
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: r1 / size, r2 / size, r2 / pos, fil1 / radius

### 文件内容证据

- `fluid_structure_interaction.java`: 参数=2, 几何=7, 物理场=1, 研究=0, 结果=0
- `models.mems.fluid_structure_interaction.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `fluid_structure_interaction.pptx`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `fluid_structure_interaction.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `fluid_structure_interaction.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `fluid_structure_interaction_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.fluid_structure_interaction.pdf`: models.mems.fluid_structure_interaction.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Laminar Flow，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；in COMSOL Multiphysics. It illustrates how fluid flow can deform structures and how to

## PDF Summary and Physics Judgement

- Simple summary: fluid_structure_interaction 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Time Dependent 研究。
...
- PDF: in COMSOL Multiphysics. It illustrates how fluid flow can deform structures and how to
- PDF: solve for the flow in a continuously deforming geometry using the arbitrary Lagrangian–
- PDF: The model geometry consists of a horizontal flow channel in the middle of which is an
- 脚本/文本识别到的物理场: spf / LaminarFlow, Solid Mechanics, Laminar Flow
...
- PDF keywords: 流体, 结构力学, 电磁, models.mems.fluid_structure_interaction.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Laminar Flow，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；in COMSOL Multiphysics. It illustrates how fluid flow can deform structures and how to
- `U` -> Inlet mean velocity at steady state; MATLAB: `model.param.set('U', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；fil1 / Fillet；geom1 / r1 / size；geom1 / r2 / size；geom1 / r2 / pos
- 已识别物理场证据：spf / LaminarFlow；Solid Mechanics；Laminar Flow
- 已识别参数：U、H
...
- Transfer recognized physics interfaces to similar requirements: spf / LaminarFlow, Solid Mechanics, Laminar Flow.
...
- Sweep `U` around the learned value `3.33[cm/s]` and export target outputs to CSV.
...
- Summary: 已学习 fluid_structure_interaction 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=21 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 21 条；几何=7，物理场=3，材料=2，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：fluid_structure_interaction.m, fluid_structure_interaction.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 fluid_structure_interaction，训练阶段为 modeling_logic_learning_ready。
...
- `fluid_structure_interaction.mph` (comsol_mph, 76297612 bytes)
- `fluid_structure_interaction.m` (matlab_livelink, 31946 bytes)
- `fluid_structure_interaction.java` (comsol_java, 29639 bytes)
- `fluid_structure_interaction_summary.json` (json, 1262 bytes)
- `fluid_structure_interaction.pptx` (presentation_document, 3234209 bytes)
- `models.mems.fluid_structure_interaction.pdf` (pdf_document, 2809124 bytes)
