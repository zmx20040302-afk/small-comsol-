...
- Created at: `2026-08-16T11:14:45.589837+00:00`
...
- `rate` = `1[K/s]` - TTT temperature sweep parameter
...
- `CCT_katana.m`: 参数=1, 几何=0, 物理场=0, 研究=0, 结果=0
- `CCT_katana.java`: 参数=1, 几何=0, 物理场=0, 研究=0, 结果=0
- `CCT_katana.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `CCT_katana_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: CCT_katana 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF keywords: 传热
- `rate` -> TTT temperature sweep parameter; MATLAB: `model.param.set('rate', value, description)`
...
- Sweep `rate` around the learned value `1[K/s]` and export target outputs to CSV.
...
- Summary: 已学习 CCT_katana 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=1；内容证据=1 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：CCT_katana.m, CCT_katana.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 CCT_katana，训练阶段为 modeling_logic_learning_ready。
...
- `CCT_katana.mph` (comsol_mph, 1582966 bytes)
- `CCT_katana.m` (matlab_livelink, 28271 bytes)
- `CCT_katana.java` (comsol_java, 31633 bytes)
- `CCT_katana_summary.json` (json, 733 bytes)
