...
- 模型树证据数量: 24
- 理论关键词: 电磁
- geometry: sph1 / Sphere, blk1 / Block, int1 / Intersection, geom1 / sph1 / r, geom1 / blk1 / size, geom1 / blk1 / pos
- physics: ewfd / ElectromagneticWavesFrequencyDomain, Electromagnetic Waves
- mesh: mesh1
- studies: std1 / wave / Wavelength, std1
- boundary_conditions: wave / ftplistmethod, wave / solnum, wave / notsolnum, wave / outputmap, wave / ngenAUX, wave / goalngenAUX, sph1 / r, blk1 / size, blk1 / pos

### 文件内容证据

- `scattering_nanosphere_full_vs_symmetry.java`: 参数=4, 几何=6, 物理场=1, 研究=2, 结果=0
- `scattering_nanosphere_full_vs_symmetry.m`: 参数=4, 几何=6, 物理场=1, 研究=1, 结果=0
- `scattering_nanosphere_full_vs_symmetry_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `scattering_nanosphere_full_vs_symmetry.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: scattering_nanosphere_full_vs_symmetry 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 ElectromagneticWavesFrequencyDomain，并采用 Wavelength 研究。
- Primary physics: `ElectromagneticWavesFrequencyDomain`
- Study type: `Wavelength`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 ElectromagneticWavesFrequencyDomain，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Wavelength 确认。

### Judgement Evidence

- 脚本/文本识别到的物理场: ewfd / ElectromagneticWavesFrequencyDomain, Electromagnetic Waves
- 识别到的研究类型: std1 / wave / Wavelength, std1
- 理论关键词: 电磁
...
- `lda` -> 案例参数/扫描变量; MATLAB: `model.param.set('lda', value, description)`
- `t_air` -> 案例参数/扫描变量; MATLAB: `model.param.set('t_air', value, description)`
- `t_pml` -> 案例参数/扫描变量; MATLAB: `model.param.set('t_pml', value, description)`
...
- 已识别几何证据：sph1 / Sphere；blk1 / Block；int1 / Intersection；geom1 / sph1 / r；geom1 / blk1 / size；geom1 / blk1 / pos
- 已识别物理场证据：ewfd / ElectromagneticWavesFrequencyDomain；Electromagnetic Waves
- 已识别参数：r0、lda、t_air、t_pml
...
- Use the learned study types as solver starting points: std1 / wave / Wavelength, std1.
...
- Sweep `lda` around the learned value `400[nm]` and export target outputs to CSV.
...
- Summary: 已学习 scattering_nanosphere_full_vs_symmetry 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=24 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 24 条；几何=6，物理场=2，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：scattering_nanosphere_full_vs_symmetry.m, scattering_nanosphere_full_vs_symmetry.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 scattering_nanosphere_full_vs_symmetry，训练阶段为 modeling_logic_learning_ready。
...
- `scattering_nanosphere_full_vs_symmetry.mph` (comsol_mph, 59842828 bytes)
- `scattering_nanosphere_full_vs_symmetry.m` (matlab_livelink, 127666 bytes)
- `scattering_nanosphere_full_vs_symmetry.java` (comsol_java, 139998 bytes)
- `scattering_nanosphere_full_vs_symmetry_summary.json` (json, 1851 bytes)
