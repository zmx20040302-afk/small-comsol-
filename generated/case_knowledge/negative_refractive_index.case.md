...
- Case directory: `D:\桌面\codex\案例下载\电气\负折射率建模 (RF)`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:13:46.420421+00:00`
...
- `lda0` = `1[m]` - Wavelength, vacuum
- `f0` = `c_const/lda0` - Frequency
- `e_a` = `1` - Relative permittivity, vacuum
- `mu_a` = `1` - Relative permeability, vacuum
- `e_b` = `-1` - Relative permittivity, metamaterial
- `mu_b` = `-1` - Relative permeability, metamaterial
- `n_a` = `sqrt(e_a*mu_a)` - Refractive index, vacuum
- `n_b` = `-sqrt(e_b*mu_b)` - Refractive index, metamaterial
- `alpha` = `30[deg]` - Angle of incidence

## 内容级读取结果

- 模型树证据数量: 31
- 理论关键词: 多孔介质/裂隙, 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, geom1 / r1 / size, geom1 / r1 / layername, geom1 / r2 / size, geom1 / r2 / pos, geom1 / r2 / layername, geom1 / r2 / layer
- physics: emw / ElectromagneticWaves, Electromagnetic Waves
- materials: mat1, mat2
- mesh: mesh1
- studies: Frequency Domain
- boundary_conditions: port1 / 5, port2 / 2, r1 / size, r1 / layername, r2 / size, r2 / pos, r2 / layername, r2 / layer

### 文件内容证据

- `negative_refractive_index.java`: 参数=9, 几何=8, 物理场=1, 研究=0, 结果=0
- `models.rf.negative_refractive_index.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `negative_refractive_index.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `negative_refractive_index.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `negative_refractive_index_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.rf.negative_refractive_index.pdf`: models.rf.negative_refractive_index.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；model. This can be done by using the Transition Boundary Condition (TBC), which

## PDF Summary and Physics Judgement

- Simple summary: negative_refractive_index 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 ElectromagneticWaves，并采用 Frequency Domain 研究。
- Primary physics: `ElectromagneticWaves`
- Study type: `Frequency Domain`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 ElectromagneticWaves，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Frequency Domain 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: model. This can be done by using the Transition Boundary Condition (TBC), which
- PDF: numerical difficulties. If it is too big, it alters the results significantly. The TBC only needs
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: emw / ElectromagneticWaves, Electromagnetic Waves
...
- PDF keywords: 多孔介质/裂隙, 电磁, models.rf.negative_refractive_index.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；model. This can be done by using the Transition Boundary Condition (TBC), which
- `lda0` -> Wavelength, vacuum; MATLAB: `model.param.set('lda0', value, description)`
- `f0` -> Frequency; MATLAB: `model.param.set('f0', value, description)`
- `e_a` -> Relative permittivity, vacuum; MATLAB: `model.param.set('e_a', value, description)`
- `mu_a` -> Relative permeability, vacuum; MATLAB: `model.param.set('mu_a', value, description)`
- `e_b` -> Relative permittivity, metamaterial; MATLAB: `model.param.set('e_b', value, description)`
- `mu_b` -> Relative permeability, metamaterial; MATLAB: `model.param.set('mu_b', value, description)`
- `n_a` -> Refractive index, vacuum; MATLAB: `model.param.set('n_a', value, description)`
- `n_b` -> Refractive index, metamaterial; MATLAB: `model.param.set('n_b', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；geom1 / r1 / size；geom1 / r1 / layername；geom1 / r2 / size；geom1 / r2 / pos
- 已识别物理场证据：emw / ElectromagneticWaves；Electromagnetic Waves
- 已识别参数：lda0、f0、e_a、mu_a、e_b、mu_b、n_a、n_b、alpha
...
- 识别到物理场证据：emw / ElectromagneticWaves
...
- Transfer recognized physics interfaces to similar requirements: emw / ElectromagneticWaves, Electromagnetic Waves.
...
- Sweep `lda0` around the learned value `1[m]` and export target outputs to CSV.
- Sweep `f0` around the learned value `c_const/lda0` and export target outputs to CSV.
- Sweep `e_a` around the learned value `1` and export target outputs to CSV.
- Sweep `mu_a` around the learned value `1` and export target outputs to CSV.
- Sweep `e_b` around the learned value `-1` and export target outputs to CSV.
- Sweep `mu_b` around the learned value `-1` and export target outputs to CSV.
- Sweep `n_a` around the learned value `sqrt(e_a*mu_a)` and export target outputs to CSV.
- Sweep `n_b` around the learned value `-sqrt(e_b*mu_b)` and export target outputs to CSV.
...
- Summary: 已学习 negative_refractive_index 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=8，物理场=2，材料=2，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：negative_refractive_index.m, negative_refractive_index.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 negative_refractive_index，训练阶段为 modeling_logic_learning_ready。
...
- `negative_refractive_index.mph` (comsol_mph, 2801680 bytes)
- `negative_refractive_index.m` (matlab_livelink, 16969 bytes)
- `negative_refractive_index.java` (comsol_java, 14218 bytes)
- `negative_refractive_index_summary.json` (json, 1534 bytes)
- `models.rf.negative_refractive_index.pdf` (pdf_document, 1034549 bytes)
