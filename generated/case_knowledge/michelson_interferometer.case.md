...
- Case directory: `D:\桌面\codex\案例下载\电气\迈克尔逊干涉仪`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:11:38.859890+00:00`
...
- `lam` = `632.8[nm]` - Wavelength of He-Ne laser
- `delta_d` = `8000*lam` - Optical path length difference
- `th` = `0.12[in]` - Thickness of mirrors
- `dia` = `0.5[in]` - Diameter of mirrors
- `d1` = `2.335[in]` - Distance between mirror M1 and the center of the beam splitter
- `d2` = `d1-delta_d` - Distance between mirror M2 and the center of the beam splitter
- `dE` = `12[in]` - Distance between the beam splitter and the screen
- `n_int` = `3.9641` - Refractive index of the beam-splitter interface
- `n_coat` = `1.2354` - Refractive index of the anti-reflective coating

## 内容级读取结果

- 模型树证据数量: 30
- 理论关键词: 传热, 电磁
- geometry: pi1 / PartInstance, cyl1 / Cylinder, cyl2 / Cylinder, geom1 / pi1 / selkeepnoncontr, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / pos, geom1 / cyl1 / axis, geom1 / cyl2 / r, geom1 / cyl2 / h, geom1 / cyl2 / pos
- physics: Ray Optics
- mesh: mesh1
- boundary_conditions: pi1 / selkeepnoncontr, cyl1 / r, cyl1 / h, cyl1 / pos, cyl1 / axis, cyl2 / r, cyl2 / h, cyl2 / pos

### 文件内容证据

- `michelson_interferometer.java`: 参数=9, 几何=11, 物理场=0, 研究=0, 结果=0
- `models.roptics.michelson_interferometer.pdf`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `michelson_interferometer.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `michelson_interferometer.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `michelson_interferometer_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.roptics.michelson_interferometer.pdf`: models.roptics.michelson_interferometer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Changing, even slightly, the optical path length of either beam results in a change of the

## PDF Summary and Physics Judgement

- Simple summary: michelson_interferometer 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF: Changing, even slightly, the optical path length of either beam results in a change of the
- PDF: Figure 1: An illuminated Michelson interferometer arrangement with the resulting
...
- PDF keywords: 传热, 电磁, models.roptics.michelson_interferometer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Changing, even slightly, the optical path length of either beam results in a change of the
- `lam` -> Wavelength of He-Ne laser; MATLAB: `model.param.set('lam', value, description)`
- `delta_d` -> Optical path length difference; MATLAB: `model.param.set('delta_d', value, description)`
- `th` -> Thickness of mirrors; MATLAB: `model.param.set('th', value, description)`
- `dia` -> Diameter of mirrors; MATLAB: `model.param.set('dia', value, description)`
- `d1` -> Distance between mirror M1 and the center of the beam splitter; MATLAB: `model.param.set('d1', value, description)`
- `d2` -> Distance between mirror M2 and the center of the beam splitter; MATLAB: `model.param.set('d2', value, description)`
- `dE` -> Distance between the beam splitter and the screen; MATLAB: `model.param.set('dE', value, description)`
- `n_int` -> Refractive index of the beam-splitter interface; MATLAB: `model.param.set('n_int', value, description)`
- `n_coat` -> Refractive index of the anti-reflective coating; MATLAB: `model.param.set('n_coat', value, description)`
...
- 已识别几何证据：pi1 / PartInstance；cyl1 / Cylinder；cyl2 / Cylinder；geom1 / pi1 / selkeepnoncontr；geom1 / cyl1 / r；geom1 / cyl1 / h
- 已识别物理场证据：Ray Optics
- 已识别参数：lam、delta_d、th、dia、d1、d2、dE、n_int、n_coat
...
- Sweep `lam` around the learned value `632.8[nm]` and export target outputs to CSV.
- Sweep `delta_d` around the learned value `8000*lam` and export target outputs to CSV.
- Sweep `th` around the learned value `0.12[in]` and export target outputs to CSV.
- Sweep `dia` around the learned value `0.5[in]` and export target outputs to CSV.
- Sweep `d1` around the learned value `2.335[in]` and export target outputs to CSV.
- Sweep `d2` around the learned value `d1-delta_d` and export target outputs to CSV.
- Sweep `dE` around the learned value `12[in]` and export target outputs to CSV.
- Sweep `n_int` around the learned value `3.9641` and export target outputs to CSV.
...
- Summary: 已学习 michelson_interferometer 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=30 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 30 条；几何=11，物理场=1，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：michelson_interferometer.m, michelson_interferometer.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 michelson_interferometer，训练阶段为 modeling_logic_learning_ready。
...
- `michelson_interferometer.mph` (comsol_mph, 7472455 bytes)
- `michelson_interferometer.m` (matlab_livelink, 35272 bytes)
- `michelson_interferometer.java` (comsol_java, 34984 bytes)
- `michelson_interferometer_summary.json` (json, 1309 bytes)
- `models.roptics.michelson_interferometer.pdf` (pdf_document, 685634 bytes)
