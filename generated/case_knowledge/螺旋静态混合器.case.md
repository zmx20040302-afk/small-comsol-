...
- Created at: `2026-07-06T09:44:20.025535+00:00`
...
- 模型树证据数量: 42
- 理论关键词: 流体
- geometry: part1 / Part, wp1 / WorkPlane, r1 / Rectangle, boxsel1 / BoxSelection, part1 / wp1 / unite, part1 / ext1 / crossfaces, part1 / boxsel1 / entitydim, part1 / boxsel1 / zmin, part1 / boxsel1 / zmax, part1 / boxsel1 / conditio ## helical_static_mixer.m function out = model % % helical_static_mixer.m % % Model exported on Jun 13 2026, 17:53 by COMSOL 6.4.0.293. import com.comsol, part1 / boxsel1 / zmi ## helical_static_mixer_parameters.txt th_bl_i 1.0[mm]
- physics: spf / LaminarFlow, tds / DilutedSpecies
- mesh: mesh1
- studies: std1 / stat / Stationary, std1, Stationary
- boundary_conditions: wp1 / unite, r1 / size, r1 / base, ext1 / crossfaces, boxsel1 / entitydim, boxsel1 / zmin, boxsel1 / zmax, boxsel1 / conditio ## helical_static_mixer.m function out = model % % helical_static_mixer.m % % Model exported on Jun 13 2026, 17:53 by COMSOL 6.4.0.293. import com.comsol.model.*, boxsel1 / zmi ## helical_static_mixer_parameters.txt th_bl_i 1.0[mm]

### 文件内容证据

- `helical_static_mixer.mph`: 参数=16, 几何=11, 物理场=2, 研究=3, 结果=0
- `helical_static_mixer.java`: 参数=16, 几何=9, 物理场=2, 研究=3, 结果=0
- `helical_static_mixer.m`: 参数=16, 几何=7, 物理场=2, 研究=2, 结果=0
...
- `applications.helical_static_mixer.pdf`: applications.helical_static_mixer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `applications.helical_static_mixer.zh_CN.pdf`: applications.helical_static_mixer.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 螺旋静态混合器 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- 脚本/文本识别到的物理场: spf / LaminarFlow, tds / DilutedSpecies
...
- PDF keywords: 流体, applications.helical_static_mixer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., applications.helical_static_mixer.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `th_bl_i` -> Blade thickness; MATLAB: `model.param.set('th_bl_i', value, description)`
- `l_bl_i` -> Blade length; MATLAB: `model.param.set('l_bl_i', value, description)`
- `r_mx_i` -> Mixer radius; MATLAB: `model.param.set('r_mx_i', value, description)`
- `n_bl` -> Number of blades; MATLAB: `model.param.set('n_bl', value, description)`
- `il_mx` -> Mixer inlet length; MATLAB: `model.param.set('il_mx', value, description)`
- `ol_mx` -> Mixer outlet length; MATLAB: `model.param.set('ol_mx', value, description)`
- `l_mx` -> Mixer length; MATLAB: `model.param.set('l_mx', value, description)`
- `rho_l` -> Liquid density; MATLAB: `model.param.set('rho_l', value, description)`
- `visc_l` -> Liquid viscosity; MATLAB: `model.param.set('visc_l', value, description)`
- `u_av` -> Average velocity; MATLAB: `model.param.set('u_av', value, description)`
- `c_in` -> Inlet concentration; MATLAB: `model.param.set('c_in', value, description)`
- `D1` -> Diffusivity; MATLAB: `model.param.set('D1', value, description)`
...
- 已识别几何证据：part1 / Part；wp1 / WorkPlane；r1 / Rectangle；boxsel1 / BoxSelection；part1 / wp1 / unite；part1 / ext1 / crossfaces
- 已识别物理场证据：spf / LaminarFlow；tds / DilutedSpecies
- 已识别参数：th_bl_i、l_bl_i、r_mx_i、n_bl、il_mx、ol_mx、l_mx、rho_l、visc_l、u_av、c_in、D1
...
- Transfer recognized physics interfaces to similar requirements: spf / LaminarFlow, tds / DilutedSpecies.
...
- Sweep `th_bl_i` around the learned value `1.0[mm]` and export target outputs to CSV.
- Sweep `l_bl_i` around the learned value `24[mm]` and export target outputs to CSV.
- Sweep `r_mx_i` around the learned value `5.0[mm]` and export target outputs to CSV.
- Sweep `n_bl` around the learned value `5` and export target outputs to CSV.
- Sweep `il_mx` around the learned value `12[mm]` and export target outputs to CSV.
- Sweep `ol_mx` around the learned value `18[mm]` and export target outputs to CSV.
- Sweep `l_mx` around the learned value `n_bl*l_bl_i+il_mx+ol_mx` and export target outputs to CSV.
- Sweep `rho_l` around the learned value `1.0e3[kg/m^3]` and export target outputs to CSV.
...
- Summary: 已学习 螺旋静态混合器 的 8 个文件；训练阶段=modeling_logic_learning_ready；参数数量=16；内容证据=42 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 42 条；几何=11，物理场=2，材料=0，网格=1，研究=3，结果=0。
