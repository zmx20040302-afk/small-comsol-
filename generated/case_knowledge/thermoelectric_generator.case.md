...
- Case directory: `D:\桌面\codex\案例下载\电气\热电发电机`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:21:19.944453+00:00`
...
- `T0` = `100[degC]` - Input Temperature
- `L_element` = `6[mm]` - Element length

## 内容级读取结果

- 模型树证据数量: 18
- 理论关键词: 传热, 电磁
- geometry: blk1 / Block, geom1 / blk1 / size, geom1 / blk1 / layername, geom1 / blk1 / layertop
- physics: ht / HeatTransfer, ec / ConductiveMedia
- materials: mat1, mat2
- mesh: mesh1
- boundary_conditions: temp1 / 10, temp2 / 3, gnd1 / 3, fp1 / 10, blk1 / size, blk1 / layername, blk1 / layertop

### 文件内容证据

- `thermoelectric_generator.java`: 参数=2, 几何=4, 物理场=2, 研究=0, 结果=0
- `thermoelectric_generator.m`: 参数=2, 几何=0, 物理场=0, 研究=0, 结果=0
- `thermoelectric_generator.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `thermoelectric_generator_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: thermoelectric_generator 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- 脚本/文本识别到的物理场: ht / HeatTransfer, ec / ConductiveMedia
- 理论关键词: 传热, 电磁
...
- PDF keywords: 传热, 电磁
- `T0` -> Input Temperature; MATLAB: `model.param.set('T0', value, description)`
- `L_element` -> Element length; MATLAB: `model.param.set('L_element', value, description)`
...
- 已识别几何证据：blk1 / Block；geom1 / blk1 / size；geom1 / blk1 / layername；geom1 / blk1 / layertop
- 已识别物理场证据：ht / HeatTransfer；ec / ConductiveMedia
- 已识别参数：T0、L_element
...
- Transfer recognized physics interfaces to similar requirements: ht / HeatTransfer, ec / ConductiveMedia.
...
- Sweep `T0` around the learned value `100[degC]` and export target outputs to CSV.
- Sweep `L_element` around the learned value `6[mm]` and export target outputs to CSV.
...
- Summary: 已学习 thermoelectric_generator 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=18 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 18 条；几何=4，物理场=2，材料=2，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：thermoelectric_generator.m, thermoelectric_generator.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 thermoelectric_generator，训练阶段为 modeling_logic_learning_ready。
...
- `thermoelectric_generator.mph` (comsol_mph, 4097414 bytes)
- `thermoelectric_generator.m` (matlab_livelink, 29538 bytes)
- `thermoelectric_generator.java` (comsol_java, 28256 bytes)
- `thermoelectric_generator_summary.json` (json, 2153 bytes)
