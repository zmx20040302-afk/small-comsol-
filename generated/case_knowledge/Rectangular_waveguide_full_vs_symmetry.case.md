...
- 模型树证据数量: 27
- 理论关键词: 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, geom1 / r1 / size, geom1 / r1 / base, geom1 / r1 / layername, geom1 / r1 / layerleft, geom1 / r1 / layerbottom, geom1 / r2 / size, geom1 / r2 / base, geom1 / r2 / layername
- physics: emw / ElectromagneticWaves, emw2 / ElectromagneticWaves, Electromagnetic Waves
- materials: mat1, mat2
- mesh: mesh1
- boundary_conditions: r1 / size, r1 / base, r1 / layername, r1 / layerleft, r1 / layerbottom, r2 / size, r2 / base, r2 / layername

### 文件内容证据

- `Rectangular_waveguide_full_vs_symmetry.java`: 参数=3, 几何=10, 物理场=2, 研究=0, 结果=0
- `Rectangular_waveguide_full_vs_symmetry.m`: 参数=3, 几何=10, 物理场=0, 研究=0, 结果=0
- `Rectangular_waveguide_full_vs_symmetry_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `Rectangular_waveguide_full_vs_symmetry.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: Rectangular_waveguide_full_vs_symmetry 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 ElectromagneticWaves。
- Primary physics: `ElectromagneticWaves`
- Study type: `unknown`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 ElectromagneticWaves，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型需要继续从研究步骤确认。
...
- PDF keywords: 电磁
- `port_w` -> 案例参数/扫描变量; MATLAB: `model.param.set('port_w', value, description)`
- `port_h` -> 案例参数/扫描变量; MATLAB: `model.param.set('port_h', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；geom1 / r1 / size；geom1 / r1 / base；geom1 / r1 / layername；geom1 / r1 / layerleft
- 已识别物理场证据：emw / ElectromagneticWaves；emw2 / ElectromagneticWaves；Electromagnetic Waves
- 已识别参数：port_w、port_h、f0
...
- Sweep `port_w` around the learned value `2[cm]` and export target outputs to CSV.
- Sweep `port_h` around the learned value `1[cm]` and export target outputs to CSV.
- Sweep `f0` around the learned value `20[GHz]` and export target outputs to CSV.
...
- Summary: 已学习 Rectangular_waveguide_full_vs_symmetry 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=27 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 27 条；几何=10，物理场=3，材料=2，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：Rectangular_waveguide_full_vs_symmetry.m, Rectangular_waveguide_full_vs_symmetry.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 Rectangular_waveguide_full_vs_symmetry，训练阶段为 modeling_logic_learning_ready。
...
- `Rectangular_waveguide_full_vs_symmetry.mph` (comsol_mph, 10953746 bytes)
- `Rectangular_waveguide_full_vs_symmetry.m` (matlab_livelink, 40784 bytes)
- `Rectangular_waveguide_full_vs_symmetry.java` (comsol_java, 43543 bytes)
- `Rectangular_waveguide_full_vs_symmetry_summary.json` (json, 1321 bytes)
