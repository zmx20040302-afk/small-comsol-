...
- Case directory: `D:\桌面\codex\案例下载\电气\单导线之间的自感和互感`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:39:39.076161+00:00`
...
- `M` = `(mu0_const*pi*R2^2)/(2*R1)` - Analytic mutual inductance
...
- 模型树证据数量: 31
- 理论关键词: 电磁, 多孔介质/裂隙
- geometry: c1 / Circle, c2 / Circle, c3 / Circle, geom1 / c1 / r, geom1 / c1 / angle, geom1 / c1 / rot, geom1 / c1 / layername, geom1 / c2 / r, geom1 / c2 / pos, geom1 / c3 / r, geom1 / c3 / pos
- physics: Magnetic Fields
- materials: mat1, mat2
- mesh: mesh1
- studies: Stationary, Frequency Domain
- boundary_conditions: c1 / r, c1 / angle, c1 / rot, c1 / layername, c2 / r, c2 / pos, c3 / r, c3 / pos

### 文件内容证据

- `mutual_inductance.java`: 参数=6, 几何=11, 物理场=0, 研究=0, 结果=0
- `models.acdc.mutual_inductance.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `mutual_inductance.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mutual_inductance.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mutual_inductance_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.mutual_inductance.pdf`: models.acdc.mutual_inductance.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；accuracy of the model.

## PDF Summary and Physics Judgement

- Simple summary: mutual_inductance 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Magnetic Fields，并采用 Stationary 研究。
...
- PDF: accuracy of the model.
- PDF: When solving the Magnetic Fields interface in a Stationary study step, these quantities are
- PDF: computed automatically for the coils present in the model. Cycling the feed over the coils
...
- PDF keywords: 电磁, 多孔介质/裂隙, models.acdc.mutual_inductance.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；accuracy of the model.
...
- 已识别几何证据：c1 / Circle；c2 / Circle；c3 / Circle；geom1 / c1 / r；geom1 / c1 / angle；geom1 / c1 / rot
...
- Sweep `M` around the learned value `(mu0_const*pi*R2^2)/(2*R1)` and export target outputs to CSV.
...
- Summary: 已学习 mutual_inductance 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=11，物理场=1，材料=2，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mutual_inductance.m, mutual_inductance.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mutual_inductance，训练阶段为 modeling_logic_learning_ready。
...
- `mutual_inductance.mph` (comsol_mph, 10137878 bytes)
- `mutual_inductance.m` (matlab_livelink, 33050 bytes)
- `mutual_inductance.java` (comsol_java, 30977 bytes)
- `mutual_inductance_summary.json` (json, 1147 bytes)
- `models.acdc.mutual_inductance.pdf` (pdf_document, 1402680 bytes)
