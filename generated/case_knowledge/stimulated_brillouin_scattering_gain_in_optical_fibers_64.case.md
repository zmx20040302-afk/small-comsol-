# stimulated_brillouin_scattering_gain_in_optical_fibers_64

- Case directory: `D:\桌面\codex\案例下载\电气\光纤中的受激布里渊散射增益`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:34:00.877615+00:00`
...
- `lam0` = `1550[nm]` - optical wavelength (caption Figure 3)
- `fsweep` = `10.0[GHz]` - Sweep Frequency
- `r1` = `2[um]` - core radius
- `r2` = `62.5[um]` - outer radius
- `n_core` = `1.47125` - core index
- `n_clad` = `1.4556` - cladding index
- `cp_core` = `5560[m/s]` - core pressure wave speed
- `cp_clad` = `5940[m/s]` - cladding pressure wave speed
- `rho_core` = `2200[kg/m^3]` - core density
- `rho_clad` = `2200[kg/m^3]` - cladding density

## 内容级读取结果

- 模型树证据数量: 30
- 理论关键词: 流体, 声学
- geometry: c1 / Circle, c2 / Circle, ls1 / LineSegment, geom1 / c1 / r, geom1 / c2 / r, geom1 / ls1 / specify1, geom1 / ls1 / specify2, i1 / Interval, geom1 / ls1 / coord2, geom2 / i1 / coord
- physics: Pressure Acoustics
- mesh: mesh1, mesh2
- studies: Eigenvalue
- boundary_conditions: c1 / r, c2 / r, ls1 / specify1, ls1 / specify2, ls1 / coord2, i1 / coord

### 文件内容证据

- `stimulated_brillouin_scattering_gain_in_optical_fibers_64.java`: 参数=10, 几何=10, 物理场=0, 研究=0, 结果=0
- `stimulated_brillouin_scattering_gain_in_optical_fibers_64.m`: 参数=10, 几何=7, 物理场=0, 研究=0, 结果=0
- `stimulated_brillouin_scattering_gain_in_optical_fibers_63.pptx`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `stimulated_brillouin_scattering_gain_in_optical_fibers_64.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `stimulated_brillouin_scattering_gain_in_optical_fibers_64_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: stimulated_brillouin_scattering_gain_in_optical_fibers_64 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Pressure Acoustics，并采用 Eigenvalue 研究。
...
- 脚本/文本识别到的物理场: Pressure Acoustics
- 识别到的研究类型: Eigenvalue
- 理论关键词: 流体, 声学
...
- PDF keywords: 流体, 声学
- `lam0` -> optical wavelength (caption Figure 3); MATLAB: `model.param.set('lam0', value, description)`
- `fsweep` -> Sweep Frequency; MATLAB: `model.param.set('fsweep', value, description)`
- `r1` -> core radius; MATLAB: `model.param.set('r1', value, description)`
- `r2` -> outer radius; MATLAB: `model.param.set('r2', value, description)`
- `n_core` -> core index; MATLAB: `model.param.set('n_core', value, description)`
- `n_clad` -> cladding index; MATLAB: `model.param.set('n_clad', value, description)`
- `cp_core` -> core pressure wave speed; MATLAB: `model.param.set('cp_core', value, description)`
- `cp_clad` -> cladding pressure wave speed; MATLAB: `model.param.set('cp_clad', value, description)`
- `rho_core` -> core density; MATLAB: `model.param.set('rho_core', value, description)`
- `rho_clad` -> cladding density; MATLAB: `model.param.set('rho_clad', value, description)`
...
- 已识别几何证据：c1 / Circle；c2 / Circle；ls1 / LineSegment；geom1 / c1 / r；geom1 / c2 / r；geom1 / ls1 / specify1
- 已识别物理场证据：Pressure Acoustics
- 已识别参数：lam0、fsweep、r1、r2、n_core、n_clad、cp_core、cp_clad、rho_core、rho_clad
...
- Transfer recognized physics interfaces to similar requirements: Pressure Acoustics.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 声学?
...
- Sweep `lam0` around the learned value `1550[nm]` and export target outputs to CSV.
- Sweep `fsweep` around the learned value `10.0[GHz]` and export target outputs to CSV.
- Sweep `r1` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `r2` around the learned value `62.5[um]` and export target outputs to CSV.
- Sweep `n_core` around the learned value `1.47125` and export target outputs to CSV.
- Sweep `n_clad` around the learned value `1.4556` and export target outputs to CSV.
- Sweep `cp_core` around the learned value `5560[m/s]` and export target outputs to CSV.
- Sweep `cp_clad` around the learned value `5940[m/s]` and export target outputs to CSV.
...
- Summary: 已学习 stimulated_brillouin_scattering_gain_in_optical_fibers_64 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=30 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 30 条；几何=10，物理场=1，材料=0，网格=2，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：stimulated_brillouin_scattering_gain_in_optical_fibers_64.m, stimulated_brillouin_scattering_gain_in_optical_fibers_64.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 stimulated_brillouin_scattering_gain_in_optical_fibers_64，训练阶段为 modeling_logic_learning_ready。
...
- `stimulated_brillouin_scattering_gain_in_optical_fibers_64.mph` (comsol_mph, 681163645 bytes)
- `stimulated_brillouin_scattering_gain_in_optical_fibers_64.m` (matlab_livelink, 40707 bytes)
- `stimulated_brillouin_scattering_gain_in_optical_fibers_64.java` (comsol_java, 43451 bytes)
- `stimulated_brillouin_scattering_gain_in_optical_fibers_64_summary.json` (json, 1707 bytes)
- `stimulated_brillouin_scattering_gain_in_optical_fibers_63.pptx` (presentation_document, 14609883 bytes)
