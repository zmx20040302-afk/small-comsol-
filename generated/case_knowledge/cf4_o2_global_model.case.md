...
- Case directory: `D:\桌面\codex\案例下载\电气\CF4O2 等离子体反应器的全局模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:29:29.000201+00:00`
...
- `cf4_o2_global_model.java`: 参数=12, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.plasma.cf4_o2_global_model.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `cf4_o2_global_model.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `cf4_o2_global_model.m`: 参数=2, 几何=0, 物理场=0, 研究=0, 结果=0
- `cf4_o2_global_model_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.cf4_o2_global_model.pdf`: models.plasma.cf4_o2_global_model.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；global model. The plasma chemistry is based on Ref. 1 and the electron impact reactions

## PDF Summary and Physics Judgement

- Simple summary: cf4_o2_global_model 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: global model. The plasma chemistry is based on Ref. 1 and the electron impact reactions
- PDF: reduced. These advantages make a global model a good first approach to study a plasma
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: plas / ColdPlasma
...
- PDF keywords: 传热, 流体, 结构力学, 电磁, models.plasma.cf4_o2_global_model.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；global model. The plasma chemistry is based on Ref. 1 and the electron impact reactions
...
- `Vp` -> Potential drop; MATLAB: `model.param.set('Vp', value, description)`
...
- 已识别物理场证据：plas / ColdPlasma
- 已识别参数：R0、L0、p0、Tgas、xO2、Qfeed、pw、lambda_diff、hc、recO、recF、Vp
...
- Transfer recognized physics interfaces to similar requirements: plas / ColdPlasma.
...
- Sweep `R0` around the learned value `8.0[cm]` and export target outputs to CSV.
- Sweep `L0` around the learned value `7.5[cm]` and export target outputs to CSV.
- Sweep `p0` around the learned value `8[mTorr]` and export target outputs to CSV.
...
- Sweep `pw` around the learned value `120[W]` and export target outputs to CSV.
...
- Summary: 已学习 cf4_o2_global_model 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=19 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 19 条；几何=2，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：cf4_o2_global_model.m, cf4_o2_global_model.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 cf4_o2_global_model，训练阶段为 modeling_logic_learning_ready。
...
- `cf4_o2_global_model.mph` (comsol_mph, 15350390 bytes)
- `cf4_o2_global_model.m` (matlab_livelink, 182159 bytes)
- `cf4_o2_global_model.java` (comsol_java, 164085 bytes)
- `cf4_o2_global_model_summary.json` (json, 1773 bytes)
- `models.plasma.cf4_o2_global_model.pdf` (pdf_document, 262867 bytes)
