# solidly_mounted_resonator_2d

- Case directory: `D:\桌面\codex\案例下载\电气\固态装配谐振器（二维）`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T15:26:56.163356+00:00`
...
- `solidly_mounted_resonator_2d.java`: 参数=9, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mems.solidly_mounted_resonator_2d.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `solidly_mounted_resonator_2d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `solidly_mounted_resonator_2d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `solidly_mounted_resonator_2d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.solidly_mounted_resonator_2d.pdf`: models.mems.solidly_mounted_resonator_2d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Eigenfrequency、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry of the SMR model and its key components are shown in Figure 1.

## PDF Summary and Physics Judgement

- Simple summary: solidly_mounted_resonator_2d 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenfrequency 研究。
...
- PDF: The geometry of the SMR model and its key components are shown in Figure 1.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 电磁, 声学, 结构力学?
...
- Summary: 已学习 solidly_mounted_resonator_2d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=14 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 14 条；几何=0，物理场=2，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：solidly_mounted_resonator_2d.m, solidly_mounted_resonator_2d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 solidly_mounted_resonator_2d，训练阶段为 modeling_logic_learning_ready。
...
- `solidly_mounted_resonator_2d.mph` (comsol_mph, 48777770 bytes)
- `solidly_mounted_resonator_2d.m` (matlab_livelink, 54261 bytes)
- `solidly_mounted_resonator_2d.java` (comsol_java, 52558 bytes)
- `solidly_mounted_resonator_2d_summary.json` (json, 2079 bytes)
- `models.mems.solidly_mounted_resonator_2d.pdf` (pdf_document, 1736342 bytes)
