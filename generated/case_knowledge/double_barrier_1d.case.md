...
- Case directory: `D:\桌面\codex\案例下载\电气\双势垒 - 一维`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:17:48.699246+00:00`
...
- `lmbd0` = `1[eV]` - Eigenvalue scale
- `x0` = `0.32` - Fraction of Al in AlGaAs
- `mw` = `0.067*me_const` - Effective mass in well regions
- `mb` = `(0.067+0.083*x0)*me_const` - Effective mass in barrier regions
- `Vb` = `1.247[eV]*x0*0.67` - Potential energy in barrier regions
- `lw` = `10[nm]` - Length of well
- `lb` = `5[nm]` - Length of barrier
- `L` = `3*lw+2*lb` - Length of modeling domain
- `reE1anal` = `0.03095108621779908[eV]` - Analytical energy for quasi-bound state, real part
- `reE2anal` = `0.122061164410767[eV]` - Analytical energy for quasi-bound state, real part
- `reE3anal` = `0.25894903907423716[eV]` - Analytical energy for quasi-bound state, real part
- `reE_anal` = `reE1anal` - Sweeping parameter for quasi-bound states
- `imE_anal` = `imE1anal` - Sweeping parameter for quasi-bound states
- `T3` = `h_const/reE3anal` - Time scale for time dependent study
- `tau3` = `hbar_const/imE3anal` - Decay time constant
- `E1anal` = `0.0309510912373888[eV]` - Analytical energy for transmission=1 solution
- `E2anal` = `0.12206193797930533[eV]` - Analytical energy for transmission=1 solution
- `E3anal` = `0.2591443427675162[eV]` - Analytical energy for transmission=1 solution
- `E_anal` = `E1anal` - Sweeping parameter for transmission=1 solutions
- `E0` = `1[eV]` - Energy for stationary study
- `psi0` = `1` - Incoming wave amplitude

## 内容级读取结果

- 模型树证据数量: 25
- 理论关键词: 电磁, 优化/参数扫描
- studies: Stationary, Time Dependent, Eigenvalue, Parametric Sweep

### 文件内容证据

- `double_barrier_1d.java`: 参数=20, 几何=0, 物理场=0, 研究=3, 结果=0
- `models.semicond.double_barrier_1d.pdf`: 参数=0, 几何=0, 物理场=0, 研究=4, 结果=0
- `double_barrier_1d.mph`: 参数=0, 几何=0, 物理场=0, 研究=3, 结果=0
- `double_barrier_1d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `double_barrier_1d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.double_barrier_1d.pdf`: models.semicond.double_barrier_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁、优化/参数扫描，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；and transmission versus energy curves. The numerical results agree very well with analytical

## PDF Summary and Physics Judgement

- Simple summary: double_barrier_1d 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Optimization，并采用 Eigenvalue 研究。
- Primary physics: `Optimization`
- Study type: `Eigenvalue`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Optimization，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Eigenvalue 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: and transmission versus energy curves. The numerical results agree very well with analytical
- PDF: To keep the relevance to semiconductor physics without losing generality, the model is
- PDF: Schrödinger Equation physics interface applies the
- 识别到的研究类型: Stationary, Time Dependent, Eigenvalue, Parametric Sweep
- 理论关键词: 电磁, 优化/参数扫描

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 电磁, 优化/参数扫描, models.semicond.double_barrier_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁、优化/参数扫描，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；and transmission versus energy curves. The numerical results agree very well with analytical
- `lmbd0` -> Eigenvalue scale; MATLAB: `model.param.set('lmbd0', value, description)`
- `x0` -> Fraction of Al in AlGaAs; MATLAB: `model.param.set('x0', value, description)`
- `mw` -> Effective mass in well regions; MATLAB: `model.param.set('mw', value, description)`
- `mb` -> Effective mass in barrier regions; MATLAB: `model.param.set('mb', value, description)`
- `Vb` -> Potential energy in barrier regions; MATLAB: `model.param.set('Vb', value, description)`
- `lw` -> Length of well; MATLAB: `model.param.set('lw', value, description)`
- `lb` -> Length of barrier; MATLAB: `model.param.set('lb', value, description)`
- `L` -> Length of modeling domain; MATLAB: `model.param.set('L', value, description)`
- `reE1anal` -> Analytical energy for quasi-bound state, real part; MATLAB: `model.param.set('reE1anal', value, description)`
- `reE2anal` -> Analytical energy for quasi-bound state, real part; MATLAB: `model.param.set('reE2anal', value, description)`
- `reE3anal` -> Analytical energy for quasi-bound state, real part; MATLAB: `model.param.set('reE3anal', value, description)`
- `reE_anal` -> Sweeping parameter for quasi-bound states; MATLAB: `model.param.set('reE_anal', value, description)`
...
- 已识别参数：lmbd0、x0、mw、mb、Vb、lw、lb、L、reE1anal、reE2anal、reE3anal、reE_anal
...
- 本案例已提取 21 个参数，可作为约束 JSON 和参数扫描变量。
...
- Use the learned study types as solver starting points: Stationary, Time Dependent, Eigenvalue, Parametric Sweep.
...
- Sweep `lmbd0` around the learned value `1[eV]` and export target outputs to CSV.
- Sweep `x0` around the learned value `0.32` and export target outputs to CSV.
- Sweep `mw` around the learned value `0.067*me_const` and export target outputs to CSV.
- Sweep `mb` around the learned value `(0.067+0.083*x0)*me_const` and export target outputs to CSV.
- Sweep `Vb` around the learned value `1.247[eV]*x0*0.67` and export target outputs to CSV.
- Sweep `lw` around the learned value `10[nm]` and export target outputs to CSV.
- Sweep `lb` around the learned value `5[nm]` and export target outputs to CSV.
- Sweep `L` around the learned value `3*lw+2*lb` and export target outputs to CSV.
...
- Summary: 已学习 double_barrier_1d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=21；内容证据=25 条。
...
- 21 extracted parameters with values/descriptions.
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 25 条；几何=0，物理场=0，材料=0，网格=0，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：double_barrier_1d.m, double_barrier_1d.java
  - Judgement: 脚本中的 COMSOL API 调用可用于恢复模型树顺序，并辅助生成 LiveLink MATLAB 自动建模脚本。
- **4. 参数与约束提取**
  - Evidence: 从文本、MATLAB 或 Java 证据中提取到 21 个参数。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 double_barrier_1d，训练阶段为 modeling_logic_learning_ready。
...
- `double_barrier_1d.mph` (comsol_mph, 9276468 bytes)
- `double_barrier_1d.m` (matlab_livelink, 54396 bytes)
- `double_barrier_1d.java` (comsol_java, 51466 bytes)
- `double_barrier_1d_summary.json` (json, 1835 bytes)
- `models.semicond.double_barrier_1d.pdf` (pdf_document, 676824 bytes)
