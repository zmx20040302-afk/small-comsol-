...
- Case directory: `D:\桌面\codex\案例下载\电气\Lombardi 表面迁移率`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:52:22.379452+00:00`
...
- `Vgs` = `1[V]` - Gate-source voltage
- `cp` = `0` - Continuation parameter

## 内容级读取结果

- 模型树证据数量: 41
- 理论关键词: 电磁, 传热, 声学
- geometry: r1 / Rectangle, ls1 / LineSegment, ls2 / LineSegment, ls3 / LineSegment, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r1 / layername, geom1 / r1 / layerbottom, geom1 / r1 / layertop, geom1 / ls1 / specify1, geom1 / ls1 / coord1, geom1 / ls1 / specify2
- mesh: mesh1
- studies: Stationary
- boundary_conditions: r1 / size, r1 / pos, r1 / layername, r1 / layerbottom, r1 / layertop, ls1 / specify1, ls1 / coord1, ls1 / specify2, ls1 / coord2, ls2 / specify1, ls2 / coord1, ls2 / specify2

### 文件内容证据

- `lombardi_surface_mobility.java`: 参数=3, 几何=20, 物理场=0, 研究=0, 结果=0
- `models.semicond.lombardi_surface_mobility.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `lombardi_surface_mobility.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `lombardi_surface_mobility.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `lombardi_surface_mobility_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.lombardi_surface_mobility.pdf`: models.semicond.lombardi_surface_mobility.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁、声学，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；upon detection of such mobility models by the Semiconductor physics interface. The

## PDF Summary and Physics Judgement

- Simple summary: lombardi_surface_mobility 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: upon detection of such mobility models by the Semiconductor physics interface. The
- PDF: surface mobility model adds surface scattering resulting from these effects to an existing
- PDF: located at the top right and top left of the geometry, respectively; see Figure 1. The gate
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 传热, 声学
...
- PDF keywords: 电磁, 传热, 声学, models.semicond.lombardi_surface_mobility.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁、声学，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；upon detection of such mobility models by the Semiconductor physics interface. The
...
- `cp` -> Continuation parameter; MATLAB: `model.param.set('cp', value, description)`
...
- 已识别几何证据：r1 / Rectangle；ls1 / LineSegment；ls2 / LineSegment；ls3 / LineSegment；geom1 / r1 / size；geom1 / r1 / pos
- 已识别参数：Vds、Vgs、cp
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 传热, 声学?
...
- Sweep `Vgs` around the learned value `1[V]` and export target outputs to CSV.
- Sweep `cp` around the learned value `0` and export target outputs to CSV.
...
- Summary: 已学习 lombardi_surface_mobility 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=41 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 41 条；几何=20，物理场=0，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：lombardi_surface_mobility.m, lombardi_surface_mobility.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 lombardi_surface_mobility，训练阶段为 modeling_logic_learning_ready。
...
- `lombardi_surface_mobility.mph` (comsol_mph, 11112436 bytes)
- `lombardi_surface_mobility.m` (matlab_livelink, 48122 bytes)
- `lombardi_surface_mobility.java` (comsol_java, 45420 bytes)
- `lombardi_surface_mobility_summary.json` (json, 1159 bytes)
- `models.semicond.lombardi_surface_mobility.pdf` (pdf_document, 1238651 bytes)
