# Inductive_Heating_with_nonlinear_BH

- Case directory: `D:\桌面\codex\案例下载\电气\零件（具有温度非线性 BH 曲线）的感应加热`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:31:30.135699+00:00`
...
- boundary_conditions: wp1 / unite, sq1 / size

### 文件内容证据

- `Inductive_Heating_with_nonlinear_BH.java`: 参数=8, 几何=3, 物理场=0, 研究=0, 结果=0
- `Inductive_Heating_with_nonlinear_BH.m`: 参数=8, 几何=0, 物理场=0, 研究=0, 结果=0
- `Inductive_Heating_with_nonlinear_BH_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `Inductive_Heating_with_nonlinear_BH.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: Inductive_Heating_with_nonlinear_BH 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- `AirSize` -> 案例参数/扫描变量; MATLAB: `model.param.set('AirSize', value, description)`
- `CoilRad` -> 案例参数/扫描变量; MATLAB: `model.param.set('CoilRad', value, description)`
- `BendRad` -> 案例参数/扫描变量; MATLAB: `model.param.set('BendRad', value, description)`
- `CoilSpacing` -> 案例参数/扫描变量; MATLAB: `model.param.set('CoilSpacing', value, description)`
...
- 已识别参数：Width、Th、Rad、Length、AirSize、CoilRad、BendRad、CoilSpacing
...
- Sweep `Width` around the learned value `50[mm]` and export target outputs to CSV.
- Sweep `Th` around the learned value `5[mm]` and export target outputs to CSV.
- Sweep `Rad` around the learned value `3[mm]` and export target outputs to CSV.
- Sweep `Length` around the learned value `100[mm]` and export target outputs to CSV.
- Sweep `AirSize` around the learned value `150[mm]` and export target outputs to CSV.
- Sweep `CoilRad` around the learned value `10[mm]` and export target outputs to CSV.
- Sweep `BendRad` around the learned value `50[mm]` and export target outputs to CSV.
- Sweep `CoilSpacing` around the learned value `30[mm]` and export target outputs to CSV.
...
- Summary: 已学习 Inductive_Heating_with_nonlinear_BH 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=16 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 16 条；几何=3，物理场=0，材料=0，网格=3，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：Inductive_Heating_with_nonlinear_BH.m, Inductive_Heating_with_nonlinear_BH.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 Inductive_Heating_with_nonlinear_BH，训练阶段为 modeling_logic_learning_ready。
...
- `Inductive_Heating_with_nonlinear_BH.mph` (comsol_mph, 2370820 bytes)
- `Inductive_Heating_with_nonlinear_BH.m` (matlab_livelink, 111036 bytes)
- `Inductive_Heating_with_nonlinear_BH.java` (comsol_java, 115243 bytes)
- `Inductive_Heating_with_nonlinear_BH_summary.json` (json, 5964 bytes)
