# cross_grating_echelle_spectrograph_geom_sequence

- Case directory: `D:\桌面\codex\案例下载\电气\交叉光栅阶梯光栅光谱仪`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T06:57:29.202019+00:00`
...
- `lam_mid` = `525[nm]` - Middle wavelength
- `T_xdp` = `500[1/mm]` - Cross-dispersion line density
- `sigma_xdp` = `1/T_xdp` - Cross-dispersion line spacing
- `theta_xdp` = `asin(0.5*lam_mid/sigma_xdp)` - Cross-dispersion angle of incidence
- `theta_B` = `63.43[deg]` - Echelle blaze angle
- `dtheta` = `1.5[deg]` - Echelle in-plane angle
- `gamma` = `10.0[deg]` - Echelle out-of-plane angle
- `R1_doub` = `183.6850[mm]` - Radius of curvature, surface 1
- `R2_doub` = `43.2490[mm]` - Radius of curvature, surface 2
- `R3_doub` = `-64.1000[mm]` - Radius of curvature, surface 3
- `Tc1_doub` = `1.5[mm]` - Center thickness, element 1
- `Tc2_doub` = `3.5[mm]` - Center thickness, element 2
- `d0_doub` = `22.5[mm]` - Lens diameter
- `BFL_doub` = `97.4495[mm]` - Back focal length
...
- `cross_grating_echelle_spectrograph_geom_sequence.m`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `cross_grating_echelle_spectrograph_geom_sequence.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.roptics.cross_grating_echelle_spectrograph.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `cross_grating_echelle_spectrograph_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `cross_grating_echelle_spectrograph_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.roptics.cross_grating_echelle_spectrograph.pdf`: models.roptics.cross_grating_echelle_spectrograph.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics，研究类型线索为 Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry for the model, including the camera objective, can be inserted from a

## PDF Summary and Physics Judgement

- Simple summary: cross_grating_echelle_spectrograph_geom_sequence 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Parametric Sweep 研究。
...
- PDF: The geometry for the model, including the camera objective, can be inserted from a
- PDF: predefined sequence. Full instructions for creating the geometry sequence are given in
- PDF: Appendix — Geometry Instructions . An overview of the objective lens can be found in the
...
- PDF keywords: 结构力学, 优化/参数扫描, models.roptics.cross_grating_echelle_spectrograph.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics，研究类型线索为 Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry for the model, including the camera objective, can be inserted from a
- `lam_mid` -> Middle wavelength; MATLAB: `model.param.set('lam_mid', value, description)`
- `T_xdp` -> Cross-dispersion line density; MATLAB: `model.param.set('T_xdp', value, description)`
- `sigma_xdp` -> Cross-dispersion line spacing; MATLAB: `model.param.set('sigma_xdp', value, description)`
- `theta_xdp` -> Cross-dispersion angle of incidence; MATLAB: `model.param.set('theta_xdp', value, description)`
- `theta_B` -> Echelle blaze angle; MATLAB: `model.param.set('theta_B', value, description)`
- `dtheta` -> Echelle in-plane angle; MATLAB: `model.param.set('dtheta', value, description)`
- `gamma` -> Echelle out-of-plane angle; MATLAB: `model.param.set('gamma', value, description)`
- `R1_doub` -> Radius of curvature, surface 1; MATLAB: `model.param.set('R1_doub', value, description)`
- `R2_doub` -> Radius of curvature, surface 2; MATLAB: `model.param.set('R2_doub', value, description)`
- `R3_doub` -> Radius of curvature, surface 3; MATLAB: `model.param.set('R3_doub', value, description)`
- `Tc1_doub` -> Center thickness, element 1; MATLAB: `model.param.set('Tc1_doub', value, description)`
- `Tc2_doub` -> Center thickness, element 2; MATLAB: `model.param.set('Tc2_doub', value, description)`
...
- 已识别物理场证据：Ray Optics
- 已识别参数：lam_mid、T_xdp、sigma_xdp、theta_xdp、theta_B、dtheta、gamma、R1_doub、R2_doub、R3_doub、Tc1_doub、Tc2_doub
...
- 本案例已提取 45 个参数，可作为约束 JSON 和参数扫描变量。
...
- Sweep `lam_mid` around the learned value `525[nm]` and export target outputs to CSV.
- Sweep `T_xdp` around the learned value `500[1/mm]` and export target outputs to CSV.
- Sweep `sigma_xdp` around the learned value `1/T_xdp` and export target outputs to CSV.
- Sweep `theta_xdp` around the learned value `asin(0.5*lam_mid/sigma_xdp)` and export target outputs to CSV.
- Sweep `theta_B` around the learned value `63.43[deg]` and export target outputs to CSV.
- Sweep `dtheta` around the learned value `1.5[deg]` and export target outputs to CSV.
- Sweep `gamma` around the learned value `10.0[deg]` and export target outputs to CSV.
- Sweep `R1_doub` around the learned value `183.6850[mm]` and export target outputs to CSV.
...
- Summary: 已学习 cross_grating_echelle_spectrograph_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=45；内容证据=32 条。
...
- 45 extracted parameters with values/descriptions.
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 32 条；几何=0，物理场=1，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：cross_grating_echelle_spectrograph_geom_sequence.m, cross_grating_echelle_spectrograph_geom_sequence.java
  - Judgement: 脚本中的 COMSOL API 调用可用于恢复模型树顺序，并辅助生成 LiveLink MATLAB 自动建模脚本。
- **4. 参数与约束提取**
  - Evidence: 从文本、MATLAB 或 Java 证据中提取到 45 个参数。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 cross_grating_echelle_spectrograph_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `cross_grating_echelle_spectrograph_geom_sequence.mph` (comsol_mph, 16509862 bytes)
- `cross_grating_echelle_spectrograph_geom_sequence.m` (matlab_livelink, 39835 bytes)
- `cross_grating_echelle_spectrograph_geom_sequence.java` (comsol_java, 41802 bytes)
- `cross_grating_echelle_spectrograph_geom_sequence_summary.json` (json, 2430 bytes)
- `models.roptics.cross_grating_echelle_spectrograph.pdf` (pdf_document, 1331136 bytes)
