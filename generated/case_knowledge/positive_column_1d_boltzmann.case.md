...
- Case directory: `D:\桌面\codex\案例下载\电气\与两项玻尔兹曼方程耦合的直流辉光放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T03:59:59.764551+00:00`
...
- `positive_column_1d_boltzmann.java`: 参数=0, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.plasma.positive_column_1d_boltzmann.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `positive_column_1d_boltzmann_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `positive_column_1d_boltzmann.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `positive_column_1d_boltzmann.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.positive_column_1d_boltzmann.pdf`: models.plasma.positive_column_1d_boltzmann.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；fluorescent lamps. DC discharges are attractive to study because the solution is time

## PDF Summary and Physics Judgement

- Simple summary: positive_column_1d_boltzmann 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: fluorescent lamps. DC discharges are attractive to study because the solution is time
- PDF: distribution function over electron scattering cross sections. Simulation results from this
- PDF: model are compared with results from the model DC Glow Discharge, 1D where the
...
- PDF keywords: 流体, 电磁, PDE/方程建模, 传热, 结构力学, models.plasma.positive_column_1d_boltzmann.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；fluorescent lamps. DC discharges are attractive to study because the solution is time
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 电磁, PDE/方程建模, 传热, 结构力学?
...
- Summary: 已学习 positive_column_1d_boltzmann 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=10 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：positive_column_1d_boltzmann.m, positive_column_1d_boltzmann.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 positive_column_1d_boltzmann，训练阶段为 modeling_logic_learning_ready。
...
- `positive_column_1d_boltzmann.mph` (comsol_mph, 28110616 bytes)
- `positive_column_1d_boltzmann.m` (matlab_livelink, 81306 bytes)
- `positive_column_1d_boltzmann.java` (comsol_java, 81554 bytes)
- `positive_column_1d_boltzmann_summary.json` (json, 1256 bytes)
- `models.plasma.positive_column_1d_boltzmann.pdf` (pdf_document, 400179 bytes)
