# GaussianBeamOnPeriodicStructure

- Case directory: `D:\桌面\codex\案例下载\电气\周期性结构中的高斯光束`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:40:05.985118+00:00`
...
- `N` = `50` - Number of unit cells. Use an even number.
- `tPml` = `lam0/5` - PML thickness
- `k0` = `2*pi/lam0` - Wave vector in free space
- `xWidth` = `N*d` - Simulation width w/o PMLs
- `xPt` = `1[nm]` - For mesh on the port
- `AOI` = `25[deg]` - Beam angle of incidence
- `w0` = `2.0*lam0` - Beam waist
- `tAir` = `15*d` - Thickness of air domain
- `tSub` = `10*d` - Thickness of substrate
- `E0` = `1[V/m]` - Unit magnitude E-field
- `I0` = `E0^2/2/Z0_const` - Intensity
- `meshMax` = `lam0/8` - Maximum element size air
- `meshMaxSub` = `meshMax/nb` - Maximum element size substrate

## 内容级读取结果

- 模型树证据数量: 22
- 理论关键词: 电磁, 结构力学
- geometry: pch1 / ParameterCheck, geom1 / pch1 / condition
- mesh: mesh1
- boundary_conditions: pch1 / condition

### 文件内容证据

- `GaussianBeamOnPeriodicStructure.java`: 参数=18, 几何=2, 物理场=0, 研究=0, 结果=0
- `GaussianBeamOnPeriodicStructure_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `GaussianBeamOnPeriodicStructure.pptx`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `GaussianBeamOnPeriodicStructure.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `GaussianBeamOnPeriodicStructure.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: GaussianBeamOnPeriodicStructure 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics。
...
- `N` -> Number of unit cells. Use an even number.; MATLAB: `model.param.set('N', value, description)`
- `tPml` -> PML thickness; MATLAB: `model.param.set('tPml', value, description)`
- `k0` -> Wave vector in free space; MATLAB: `model.param.set('k0', value, description)`
- `xWidth` -> Simulation width w/o PMLs; MATLAB: `model.param.set('xWidth', value, description)`
- `xPt` -> For mesh on the port; MATLAB: `model.param.set('xPt', value, description)`
- `AOI` -> Beam angle of incidence; MATLAB: `model.param.set('AOI', value, description)`
- `w0` -> Beam waist; MATLAB: `model.param.set('w0', value, description)`
...
- 已识别几何证据：pch1 / ParameterCheck；geom1 / pch1 / condition
- 已识别参数：na、nb、d、lam0、f0、N、tPml、k0、xWidth、xPt、AOI、w0
...
- Sweep `N` around the learned value `50` and export target outputs to CSV.
- Sweep `tPml` around the learned value `lam0/5` and export target outputs to CSV.
- Sweep `k0` around the learned value `2*pi/lam0` and export target outputs to CSV.
...
- Summary: 已学习 GaussianBeamOnPeriodicStructure 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=18；内容证据=22 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 22 条；几何=2，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：GaussianBeamOnPeriodicStructure.m, GaussianBeamOnPeriodicStructure.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 GaussianBeamOnPeriodicStructure，训练阶段为 modeling_logic_learning_ready。
...
- `GaussianBeamOnPeriodicStructure.mph` (comsol_mph, 38446237 bytes)
- `GaussianBeamOnPeriodicStructure.m` (matlab_livelink, 56888 bytes)
- `GaussianBeamOnPeriodicStructure.java` (comsol_java, 60233 bytes)
- `GaussianBeamOnPeriodicStructure_summary.json` (json, 1796 bytes)
- `GaussianBeamOnPeriodicStructure.pptx` (presentation_document, 9330576 bytes)
