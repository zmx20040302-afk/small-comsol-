...
- Case directory: `D:\桌面\codex\案例下载\电气\InSb p 沟道场效应晶体管的密度梯度分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T15:36:30.350048+00:00`
...
- `Lgate` = `40[nm]` - Gate length
- `Lgate2cap` = `135[nm]` - Gate to cap length
- `Lcap` = `100[nm]` - Cap length
- `Hcap` = `16[nm]` - Cap height
- `Htopbar` = `10[nm]` - Top barrier height
- `Hwell` = `5[nm]` - Well height
- `Hbotbar` = `150[nm]` - Bottom barrier height
- `Hstack` = `Htopbar+Hwell+Hbotbar` - Total height
- `Ltot` = `Lgate+2*(Lgate2cap+Lcap)` - Total length
- `d0` = `1[um]` - Out-of-plane thickness
...
- `insb_pfet_density_gradient.java`: 参数=10, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.semicond.insb_pfet_density_gradient.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `insb_pfet_density_gradient.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `insb_pfet_density_gradient.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `insb_pfet_density_gradient_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.insb_pfet_density_gradient.pdf`: models.semicond.insb_pfet_density_gradient.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；mobility model. The hole density profile and the Id–Vg curve obtained from the 2D

## PDF Summary and Physics Judgement

- Simple summary: insb_pfet_density_gradient 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: mobility model. The hole density profile and the Id–Vg curve obtained from the 2D
- PDF: model.
- PDF: contacts. (See Fig. 6 in Ref. 1, though the gate is not recessed in this model.)
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 流体, 电磁
...
- PDF keywords: 传热, 流体, 电磁, models.semicond.insb_pfet_density_gradient.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；mobility model. The hole density profile and the Id–Vg curve obtained from the 2D
- `Lgate` -> Gate length; MATLAB: `model.param.set('Lgate', value, description)`
- `Lgate2cap` -> Gate to cap length; MATLAB: `model.param.set('Lgate2cap', value, description)`
- `Lcap` -> Cap length; MATLAB: `model.param.set('Lcap', value, description)`
- `Hcap` -> Cap height; MATLAB: `model.param.set('Hcap', value, description)`
- `Htopbar` -> Top barrier height; MATLAB: `model.param.set('Htopbar', value, description)`
- `Hwell` -> Well height; MATLAB: `model.param.set('Hwell', value, description)`
- `Hbotbar` -> Bottom barrier height; MATLAB: `model.param.set('Hbotbar', value, description)`
- `Hstack` -> Total height; MATLAB: `model.param.set('Hstack', value, description)`
- `Ltot` -> Total length; MATLAB: `model.param.set('Ltot', value, description)`
- `d0` -> Out-of-plane thickness; MATLAB: `model.param.set('d0', value, description)`
...
- 已识别参数：Lgate、Lgate2cap、Lcap、Hcap、Htopbar、Hwell、Hbotbar、Hstack、Ltot、d0
...
- Sweep `Lgate` around the learned value `40[nm]` and export target outputs to CSV.
- Sweep `Lgate2cap` around the learned value `135[nm]` and export target outputs to CSV.
- Sweep `Lcap` around the learned value `100[nm]` and export target outputs to CSV.
- Sweep `Hcap` around the learned value `16[nm]` and export target outputs to CSV.
- Sweep `Htopbar` around the learned value `10[nm]` and export target outputs to CSV.
- Sweep `Hwell` around the learned value `5[nm]` and export target outputs to CSV.
- Sweep `Hbotbar` around the learned value `150[nm]` and export target outputs to CSV.
- Sweep `Hstack` around the learned value `Htopbar+Hwell+Hbotbar` and export target outputs to CSV.
...
- Summary: 已学习 insb_pfet_density_gradient 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=11 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 11 条；几何=0，物理场=0，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：insb_pfet_density_gradient.m, insb_pfet_density_gradient.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 insb_pfet_density_gradient，训练阶段为 modeling_logic_learning_ready。
...
- `insb_pfet_density_gradient.mph` (comsol_mph, 23336964 bytes)
- `insb_pfet_density_gradient.m` (matlab_livelink, 81319 bytes)
- `insb_pfet_density_gradient.java` (comsol_java, 76952 bytes)
- `insb_pfet_density_gradient_summary.json` (json, 2189 bytes)
- `models.semicond.insb_pfet_density_gradient.pdf` (pdf_document, 452645 bytes)
