...
- 模型树证据数量: 31
- 理论关键词: 传热, 结构力学, 电磁
- geometry: wp6 / WorkPlane, r1 / Rectangle, fil1 / Fillet, geom1 / wp6 / unite
- physics: Heat Transfer, Solid Mechanics, Electric Currents
- mesh: mesh1
- boundary_conditions: wp6 / unite, r1 / size, r1 / pos, fil1 / radius

### 文件内容证据

- `bond_wire_led_cmph_kernel.m`: 参数=19, 几何=4, 物理场=1, 研究=0, 结果=0
- `bond_wire_led_cmph_kernel.java`: 参数=19, 几何=4, 物理场=1, 研究=0, 结果=0
- `Modeling Thermal Expansion.pdf`: 参数=0, 几何=0, 物理场=3, 研究=0, 结果=0
- `bond_wire_led_cmph_kernel.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `bond_wire_led_cmph_kernel_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: bond_wire_led_cmph_kernel 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- 已识别几何证据：wp6 / WorkPlane；r1 / Rectangle；fil1 / Fillet；geom1 / wp6 / unite
...
- Summary: 已学习 bond_wire_led_cmph_kernel 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=19；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=4，物理场=3，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：bond_wire_led_cmph_kernel.m, bond_wire_led_cmph_kernel.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 bond_wire_led_cmph_kernel，训练阶段为 modeling_logic_learning_ready。
...
- `bond_wire_led_cmph_kernel.mph` (comsol_mph, 152925054 bytes)
- `bond_wire_led_cmph_kernel.m` (matlab_livelink, 64112 bytes)
- `bond_wire_led_cmph_kernel.java` (comsol_java, 67608 bytes)
- `bond_wire_led_cmph_kernel_summary.json` (json, 2923 bytes)
