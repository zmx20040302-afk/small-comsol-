...
- Case directory: `D:\桌面\codex\案例下载\电气\硅纳米线环栅元件的表面陷阱`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:20:51.132523+00:00`
...
- `Rw` = `10[nm]` - Nanowire radius
- `L` = `100[nm]` - Gate length
- `t_ox` = `2[nm]` - Oxide thickness
- `t_np` = `Rw` - Additional length
- `Na_0` = `1e17[1/cm^3]` - Background acceptor concentration
- `Nd_0` = `1e20[1/cm^3]` - Maximum donor concentration
- `Nt_0` = `1e10[1/cm^2]` - Trap density
- `Vd` = `500[mV]` - Drain voltage
- `Vg` = `-500[mV]` - Gate voltage
- `ramp` = `1` - Ramp parameter

## 内容级读取结果

- 模型树证据数量: 24
- 理论关键词: 电磁, 传热
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, geom1 / r1 / size, geom1 / r2 / size, geom1 / r3 / size, geom1 / r3 / pos
- materials: mat1
- mesh: mesh1
- studies: Stationary
- boundary_conditions: r1 / size, r2 / size, r3 / size, r3 / pos

### 文件内容证据

- `nanowire_traps.java`: 参数=10, 几何=7, 物理场=0, 研究=0, 结果=0
- `models.semicond.nanowire_traps.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `nanowire_traps.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `nanowire_traps.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `nanowire_traps_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.nanowire_traps.pdf`: models.semicond.nanowire_traps.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；distinct discrete traps (denoted by the superscript i), giving the following result:

## PDF Summary and Physics Judgement

- Simple summary: nanowire_traps 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: distinct discrete traps (denoted by the superscript i), giving the following result:
- PDF: Finally the charge resulting from the occupied traps must also be computed. In general the
- PDF: The total charge density, Q, that results from the traps is given by:
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 传热
...
- PDF keywords: 电磁, 传热, models.semicond.nanowire_traps.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；distinct discrete traps (denoted by the superscript i), giving the following result:
- `Rw` -> Nanowire radius; MATLAB: `model.param.set('Rw', value, description)`
- `L` -> Gate length; MATLAB: `model.param.set('L', value, description)`
- `t_ox` -> Oxide thickness; MATLAB: `model.param.set('t_ox', value, description)`
- `t_np` -> Additional length; MATLAB: `model.param.set('t_np', value, description)`
- `Na_0` -> Background acceptor concentration; MATLAB: `model.param.set('Na_0', value, description)`
- `Nd_0` -> Maximum donor concentration; MATLAB: `model.param.set('Nd_0', value, description)`
- `Nt_0` -> Trap density; MATLAB: `model.param.set('Nt_0', value, description)`
- `Vd` -> Drain voltage; MATLAB: `model.param.set('Vd', value, description)`
- `Vg` -> Gate voltage; MATLAB: `model.param.set('Vg', value, description)`
- `ramp` -> Ramp parameter; MATLAB: `model.param.set('ramp', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；geom1 / r1 / size；geom1 / r2 / size；geom1 / r3 / size
- 已识别参数：Rw、L、t_ox、t_np、Na_0、Nd_0、Nt_0、Vd、Vg、ramp
...
- Sweep `Rw` around the learned value `10[nm]` and export target outputs to CSV.
- Sweep `L` around the learned value `100[nm]` and export target outputs to CSV.
- Sweep `t_ox` around the learned value `2[nm]` and export target outputs to CSV.
- Sweep `t_np` around the learned value `Rw` and export target outputs to CSV.
- Sweep `Na_0` around the learned value `1e17[1/cm^3]` and export target outputs to CSV.
- Sweep `Nd_0` around the learned value `1e20[1/cm^3]` and export target outputs to CSV.
- Sweep `Nt_0` around the learned value `1e10[1/cm^2]` and export target outputs to CSV.
- Sweep `Vd` around the learned value `500[mV]` and export target outputs to CSV.
...
- Summary: 已学习 nanowire_traps 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=24 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 24 条；几何=7，物理场=0，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：nanowire_traps.m, nanowire_traps.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 nanowire_traps，训练阶段为 modeling_logic_learning_ready。
...
- `nanowire_traps.mph` (comsol_mph, 37333986 bytes)
- `nanowire_traps.m` (matlab_livelink, 55687 bytes)
- `nanowire_traps.java` (comsol_java, 56402 bytes)
- `nanowire_traps_summary.json` (json, 1438 bytes)
- `models.semicond.nanowire_traps.pdf` (pdf_document, 891152 bytes)
