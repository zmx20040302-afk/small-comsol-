...
- Case directory: `D:\桌面\codex\案例下载\电气\静电驱动悬臂梁`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:52:24.345733+00:00`
...
- `V0` = `5[V]` - Bias on cantilever

## 内容级读取结果

- 模型树证据数量: 25
- 理论关键词: 结构力学, 流体, 电磁, 优化/参数扫描
- geometry: blk1 / Block, blk2 / Block, blk3 / Block, blk4 / Block, geom1 / blk1 / size, geom1 / blk1 / pos, geom1 / blk2 / size, geom1 / blk3 / size, geom1 / blk3 / pos, geom1 / blk4 / size
- physics: solid / SolidMechanics, Electrostatics, Solid Mechanics
- materials: mat1
- mesh: mesh1
- studies: Stationary, Parametric Sweep
- boundary_conditions: fix1 / 4, blk1 / size, blk1 / pos, blk2 / size, blk3 / size, blk3 / pos, blk4 / size

### 文件内容证据

- `electrostatically_actuated_cantilever.java`: 参数=1, 几何=10, 物理场=1, 研究=0, 结果=0
- `models.mems.electrostatically_actuated_cantilever.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `electrostatically_actuated_cantilever_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `electrostatically_actuated_cantilever.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `electrostatically_actuated_cantilever.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.electrostatically_actuated_cantilever.pdf`: models.mems.electrostatically_actuated_cantilever.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Electromechanics multiphysics interface to solve the coupled equations for the structural

## PDF Summary and Physics Judgement

- Simple summary: electrostatically_actuated_cantilever 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF: Electromechanics multiphysics interface to solve the coupled equations for the structural
- PDF: frequency capacitance voltage sweep. The model predicts the results of such a test.
- PDF: Figure 1 shows the model geometry. The beam has the following dimensions:
- 脚本/文本识别到的物理场: solid / SolidMechanics, Electrostatics, Solid Mechanics
...
- PDF keywords: 结构力学, 流体, 电磁, 优化/参数扫描, models.mems.electrostatically_actuated_cantilever.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Electromechanics multiphysics interface to solve the coupled equations for the structural
- `V0` -> Bias on cantilever; MATLAB: `model.param.set('V0', value, description)`
...
- 已识别几何证据：blk1 / Block；blk2 / Block；blk3 / Block；blk4 / Block；geom1 / blk1 / size；geom1 / blk1 / pos
- 已识别物理场证据：solid / SolidMechanics；Electrostatics；Solid Mechanics
- 已识别参数：V0
...
- Transfer recognized physics interfaces to similar requirements: solid / SolidMechanics, Electrostatics, Solid Mechanics.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 结构力学, 流体, 电磁, 优化/参数扫描?
...
- Sweep `V0` around the learned value `5[V]` and export target outputs to CSV.
...
- Summary: 已学习 electrostatically_actuated_cantilever 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=1；内容证据=25 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 25 条；几何=10，物理场=3，材料=1，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：electrostatically_actuated_cantilever.m, electrostatically_actuated_cantilever.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 electrostatically_actuated_cantilever，训练阶段为 modeling_logic_learning_ready。
...
- `electrostatically_actuated_cantilever.mph` (comsol_mph, 13127895 bytes)
- `electrostatically_actuated_cantilever.m` (matlab_livelink, 28829 bytes)
- `electrostatically_actuated_cantilever.java` (comsol_java, 27553 bytes)
- `electrostatically_actuated_cantilever_summary.json` (json, 1172 bytes)
- `models.mems.electrostatically_actuated_cantilever.pdf` (pdf_document, 699394 bytes)
