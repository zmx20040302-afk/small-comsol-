...
- `core_tLam` = `2.5[cm]`
- `coil_r_in` = `14.5[cm]`
- `coil_h` = `19.2[cm]` - From 2D model
- `coil_wPrm` = `3.7[cm]` - From 2D model
- `coil_wScn` = `2.8[cm]` - From 2D model
- `coil_rSp` = `1[cm]` - 0 for packed coils
- `coil_hSp` = `2[cm]` - 0 for packed coils
- `carp_wTot` = `170[cm]`
- `core_wTot` = `3*core_wLeg+2*core_wWindow`
- `core_hTot` = `2*core_wLeg+core_hWindow`
- `core_posLeg` = `core_wWindow+core_wLeg`
- `core_posWindow` = `(core_wLeg+core_wWindow)/2`
- `coil_rTot` = `coil_r_in+coil_wPrm+coil_rSp+coil_wScn`
- `coil_hTot` = `2*coil_h+coil_hSp`
- `coil_zPos` = `-coil_hSp/2-coil_h`
- `tol` = `0.1[mm]`
- `check_aPrm` = `(coil_wPrm*coil_h) >= (coil_nPrm*coil_aPrm)`
- `check_aScn` = `(coil_wScn*coil_h) >= (coil_nScn*coil_aScn)`
...
- `three_phase_power_transformer_3d.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `Computation_losses_in_three_phase_power_transformer.pptx`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `three_phase_power_transformer_3d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `three_phase_power_transformer_3d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `three_phase_power_transformer_3d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: three_phase_power_transformer_3d 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `core_wLeg` -> 案例参数/扫描变量; MATLAB: `model.param.set('core_wLeg', value, description)`
- `core_wWindow` -> 案例参数/扫描变量; MATLAB: `model.param.set('core_wWindow', value, description)`
- `core_hWindow` -> 案例参数/扫描变量; MATLAB: `model.param.set('core_hWindow', value, description)`
- `core_tLam` -> 案例参数/扫描变量; MATLAB: `model.param.set('core_tLam', value, description)`
- `coil_r_in` -> 案例参数/扫描变量; MATLAB: `model.param.set('coil_r_in', value, description)`
- `coil_h` -> From 2D model; MATLAB: `model.param.set('coil_h', value, description)`
- `coil_wPrm` -> From 2D model; MATLAB: `model.param.set('coil_wPrm', value, description)`
- `coil_wScn` -> From 2D model; MATLAB: `model.param.set('coil_wScn', value, description)`
- `coil_rSp` -> 0 for packed coils; MATLAB: `model.param.set('coil_rSp', value, description)`
- `coil_hSp` -> 0 for packed coils; MATLAB: `model.param.set('coil_hSp', value, description)`
- `carp_wTot` -> 案例参数/扫描变量; MATLAB: `model.param.set('carp_wTot', value, description)`
- `core_wTot` -> 案例参数/扫描变量; MATLAB: `model.param.set('core_wTot', value, description)`
...
- 已识别参数：core_wLeg、core_wWindow、core_hWindow、core_tLam、coil_r_in、coil_h、coil_wPrm、coil_wScn、coil_rSp、coil_hSp、carp_wTot、core_wTot
...
- Sweep `core_wLeg` around the learned value `21[cm]` and export target outputs to CSV.
- Sweep `core_wWindow` around the learned value `33[cm]` and export target outputs to CSV.
- Sweep `core_hWindow` around the learned value `60[cm]` and export target outputs to CSV.
- Sweep `core_tLam` around the learned value `2.5[cm]` and export target outputs to CSV.
- Sweep `coil_r_in` around the learned value `14.5[cm]` and export target outputs to CSV.
- Sweep `coil_h` around the learned value `19.2[cm]` and export target outputs to CSV.
- Sweep `coil_wPrm` around the learned value `3.7[cm]` and export target outputs to CSV.
- Sweep `coil_wScn` around the learned value `2.8[cm]` and export target outputs to CSV.
...
- Summary: 已学习 three_phase_power_transformer_3d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=21；内容证据=22 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 22 条；几何=0，物理场=0，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：three_phase_power_transformer_3d.m, three_phase_power_transformer_3d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 three_phase_power_transformer_3d，训练阶段为 modeling_logic_learning_ready。
...
- `three_phase_power_transformer_3d.mph` (comsol_mph, 95576097 bytes)
- `three_phase_power_transformer_3d.m` (matlab_livelink, 148698 bytes)
- `three_phase_power_transformer_3d.java` (comsol_java, 151874 bytes)
- `three_phase_power_transformer_3d_summary.json` (json, 4016 bytes)
