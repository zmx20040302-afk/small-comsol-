# General_FF_MultInhomLayers

- Case directory: `D:\桌面\codex\案例下载\电气\基板对远场辐射的影响`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:08:09.293820+00:00`
...
- `f0` = `1[GHz]` - Operating frequency
- `wl0` = `c_const/f0` - Operating wavelength
- `I0` = `1[A]` - Electric current magnitude
- `V0` = `1[V]` - Magnetic currnet magnitude
- `rSphere` = `wl0/2` - Radius of 3D sphere
- `tPml` = `wl0/5` - PML thickness in 3D
- `rLoop` = `wl0/20` - Current loop radius
- `tSlab` = `wl0/8` - Slab thickness
- `rSphere2D` = `wl0*10`
...
- 模型树证据数量: 25
- 理论关键词: 传热, 电磁, 多孔介质/裂隙
- geometry: sph1 / Sphere, wp1 / WorkPlane, geom1 / sph1 / r, geom1 / sph1 / layername, geom1 / wp1 / unite, geom1 / wp1 / selresult, geom1 / wp1 / selresultshow, c1 / Circle
- mesh: mesh1
- boundary_conditions: sph1 / r, sph1 / layername, wp1 / unite, wp1 / selresult, wp1 / selresultshow, c1 / type, c1 / r

### 文件内容证据

- `General_FF_MultInhomLayers.java`: 参数=9, 几何=8, 物理场=0, 研究=0, 结果=0
- `General_FF_MultInhomLayers.m`: 参数=9, 几何=7, 物理场=0, 研究=0, 结果=0
- `FFwithSubstrate.pptx`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `General_FF_MultInhomLayers_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `General_FF_MultInhomLayers.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: General_FF_MultInhomLayers 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF keywords: 传热, 电磁, 多孔介质/裂隙
- `f0` -> Operating frequency; MATLAB: `model.param.set('f0', value, description)`
- `wl0` -> Operating wavelength; MATLAB: `model.param.set('wl0', value, description)`
- `I0` -> Electric current magnitude; MATLAB: `model.param.set('I0', value, description)`
- `V0` -> Magnetic currnet magnitude; MATLAB: `model.param.set('V0', value, description)`
- `rSphere` -> Radius of 3D sphere; MATLAB: `model.param.set('rSphere', value, description)`
- `tPml` -> PML thickness in 3D; MATLAB: `model.param.set('tPml', value, description)`
- `rLoop` -> Current loop radius; MATLAB: `model.param.set('rLoop', value, description)`
- `tSlab` -> Slab thickness; MATLAB: `model.param.set('tSlab', value, description)`
- `rSphere2D` -> 案例参数/扫描变量; MATLAB: `model.param.set('rSphere2D', value, description)`
...
- 已识别几何证据：sph1 / Sphere；wp1 / WorkPlane；geom1 / sph1 / r；geom1 / sph1 / layername；geom1 / wp1 / unite；geom1 / wp1 / selresult
- 已识别参数：f0、wl0、I0、V0、rSphere、tPml、rLoop、tSlab、rSphere2D
...
- Sweep `rSphere` around the learned value `wl0/2` and export target outputs to CSV.
- Sweep `tPml` around the learned value `wl0/5` and export target outputs to CSV.
- Sweep `rLoop` around the learned value `wl0/20` and export target outputs to CSV.
- Sweep `tSlab` around the learned value `wl0/8` and export target outputs to CSV.
...
- Summary: 已学习 General_FF_MultInhomLayers 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=25 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 25 条；几何=8，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：General_FF_MultInhomLayers.m, General_FF_MultInhomLayers.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 General_FF_MultInhomLayers，训练阶段为 modeling_logic_learning_ready。
...
- `General_FF_MultInhomLayers.mph` (comsol_mph, 42089779 bytes)
- `General_FF_MultInhomLayers.m` (matlab_livelink, 55205 bytes)
- `General_FF_MultInhomLayers.java` (comsol_java, 58733 bytes)
- `General_FF_MultInhomLayers_summary.json` (json, 1374 bytes)
- `FFwithSubstrate.pptx` (presentation_document, 12090447 bytes)
