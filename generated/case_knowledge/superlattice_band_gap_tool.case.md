...
- Case directory: `D:\桌面\codex\案例下载\电气\超晶格带隙工具`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:01:54.035313+00:00`
...
- `lw` = `5[nm]` - Width of well
- `lb` = `5[nm]` - Width of barrier
- `mew` = `0.063` - Electron effective mass of well material
- `meb` = `0.71` - Electron effective mass of barrier material
- `mhw` = `0.51` - Hole effective mass of well material
- `mhb` = `0.76` - Hole effective mass of barrier material
- `Egw` = `1.424[V]` - Band gap of well
- `Egb` = `2.168[V]` - Band gap of barrier
- `CBO` = `0.283[V]` - Conduction band offset
- `VBO` = `Egb-Egw-CBO` - Valence band offset
- `hmax` = `0.1[nm]` - Max mesh element size

## 内容级读取结果

- 模型树证据数量: 22
- 理论关键词: 传热
- geometry: i1 / Interval, geom1 / i1 / specify, geom1 / i1 / left, geom1 / i1 / len
- physics: schre / SchrodingerEquation
- mesh: mesh1
- studies: Eigenvalue
- boundary_conditions: meff2 / 2, i1 / specify, i1 / left, i1 / len

### 文件内容证据

- `superlattice_band_gap_tool.java`: 参数=11, 几何=4, 物理场=1, 研究=0, 结果=0
- `models.semicond.superlattice_band_gap_tool.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `superlattice_band_gap_tool.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `superlattice_band_gap_tool.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `superlattice_band_gap_tool_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.superlattice_band_gap_tool.pdf`: models.semicond.superlattice_band_gap_tool.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热，研究类型线索为 Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Schrödinger Equation physics interface applies

## PDF Summary and Physics Judgement

- Simple summary: superlattice_band_gap_tool 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Coefficient Form PDE，并采用 Eigenvalue 研究。
...
- PDF: Schrödinger Equation physics interface applies
- PDF: Two Schrödinger Equation physics interfaces are set up to solve for the electron and hole
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: schre / SchrodingerEquation
- 识别到的研究类型: Eigenvalue
- 理论关键词: 传热
...
- PDF keywords: 传热, models.semicond.superlattice_band_gap_tool.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热，研究类型线索为 Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Schrödinger Equation physics interface applies
- `lw` -> Width of well; MATLAB: `model.param.set('lw', value, description)`
- `lb` -> Width of barrier; MATLAB: `model.param.set('lb', value, description)`
- `mew` -> Electron effective mass of well material; MATLAB: `model.param.set('mew', value, description)`
- `meb` -> Electron effective mass of barrier material; MATLAB: `model.param.set('meb', value, description)`
- `mhw` -> Hole effective mass of well material; MATLAB: `model.param.set('mhw', value, description)`
- `mhb` -> Hole effective mass of barrier material; MATLAB: `model.param.set('mhb', value, description)`
- `Egw` -> Band gap of well; MATLAB: `model.param.set('Egw', value, description)`
- `Egb` -> Band gap of barrier; MATLAB: `model.param.set('Egb', value, description)`
- `CBO` -> Conduction band offset; MATLAB: `model.param.set('CBO', value, description)`
- `VBO` -> Valence band offset; MATLAB: `model.param.set('VBO', value, description)`
- `hmax` -> Max mesh element size; MATLAB: `model.param.set('hmax', value, description)`
...
- 已识别几何证据：i1 / Interval；geom1 / i1 / specify；geom1 / i1 / left；geom1 / i1 / len
- 已识别物理场证据：schre / SchrodingerEquation
- 已识别参数：lw、lb、mew、meb、mhw、mhb、Egw、Egb、CBO、VBO、hmax
...
- 识别到物理场证据：schre / SchrodingerEquation
...
- Transfer recognized physics interfaces to similar requirements: schre / SchrodingerEquation.
...
- Sweep `lw` around the learned value `5[nm]` and export target outputs to CSV.
- Sweep `lb` around the learned value `5[nm]` and export target outputs to CSV.
- Sweep `mew` around the learned value `0.063` and export target outputs to CSV.
- Sweep `meb` around the learned value `0.71` and export target outputs to CSV.
- Sweep `mhw` around the learned value `0.51` and export target outputs to CSV.
- Sweep `mhb` around the learned value `0.76` and export target outputs to CSV.
- Sweep `Egw` around the learned value `1.424[V]` and export target outputs to CSV.
- Sweep `Egb` around the learned value `2.168[V]` and export target outputs to CSV.
...
- Summary: 已学习 superlattice_band_gap_tool 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=22 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 22 条；几何=4，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：superlattice_band_gap_tool.m, superlattice_band_gap_tool.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 superlattice_band_gap_tool，训练阶段为 modeling_logic_learning_ready。
...
- `superlattice_band_gap_tool.mph` (comsol_mph, 2402435 bytes)
- `superlattice_band_gap_tool.m` (matlab_livelink, 30501 bytes)
- `superlattice_band_gap_tool.java` (comsol_java, 24159 bytes)
- `superlattice_band_gap_tool_summary.json` (json, 1318 bytes)
- `models.semicond.superlattice_band_gap_tool.pdf` (pdf_document, 303432 bytes)
