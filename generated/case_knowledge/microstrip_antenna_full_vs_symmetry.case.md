# microstrip_antenna_full_vs_symmetry

- Case directory: `D:\桌面\codex\案例下载\电气\利用对称性简化射频与波动光学仿真`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:22:59.920666+00:00`
...
- `l_stub` = `16[mm]` - Tuning stub length
...
- 模型树证据数量: 28
- 理论关键词: 未自动识别
- geometry: blk1 / Block, geom1 / blk1 / size, geom1 / blk1 / base, blk2 / Block, geom1 / blk1 / layerright, geom1 / blk1 / layerbottom, geom1 / blk2 / size, geom1 / blk2 / base, geom1 / blk2 / layername
- physics: Electromagnetic Waves
- mesh: mesh1, mesh3
- boundary_conditions: blk1 / size, blk1 / base, blk1 / layerright, blk1 / layerbottom, blk2 / size, blk2 / base, blk2 / layername

### 文件内容证据

- `microstrip_antenna_full_vs_symmetry.java`: 参数=9, 几何=9, 物理场=0, 研究=0, 结果=0
- `microstrip_antenna_full_vs_symmetry.m`: 参数=9, 几何=3, 物理场=0, 研究=0, 结果=0
- `microstrip_antenna_full_vs_symmetry_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `microstrip_antenna_full_vs_symmetry.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: microstrip_antenna_full_vs_symmetry 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electromagnetic Waves。
...
- 已识别几何证据：blk1 / Block；geom1 / blk1 / size；geom1 / blk1 / base；blk2 / Block；geom1 / blk1 / layerright；geom1 / blk1 / layerbottom
- 已识别物理场证据：Electromagnetic Waves
- 已识别参数：d、w_line、w_patch、l_patch、w_stub、l_stub、w_sub、l_sub、l_PML
...
- Sweep `l_stub` around the learned value `16[mm]` and export target outputs to CSV.
...
- Summary: 已学习 microstrip_antenna_full_vs_symmetry 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=9，物理场=1，材料=0，网格=2，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：microstrip_antenna_full_vs_symmetry.m, microstrip_antenna_full_vs_symmetry.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 microstrip_antenna_full_vs_symmetry，训练阶段为 modeling_logic_learning_ready。
...
- `microstrip_antenna_full_vs_symmetry.mph` (comsol_mph, 636702979 bytes)
- `microstrip_antenna_full_vs_symmetry.m` (matlab_livelink, 92436 bytes)
- `microstrip_antenna_full_vs_symmetry.java` (comsol_java, 99002 bytes)
- `microstrip_antenna_full_vs_symmetry_summary.json` (json, 2117 bytes)
