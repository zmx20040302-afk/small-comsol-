...
- Created at: `2026-07-06T09:43:58.092710+00:00`
...
- `free_convection.java`: 参数=3, 几何=6, 物理场=2, 研究=3, 结果=0
- `free_convection.mph`: 参数=3, 几何=6, 物理场=2, 研究=3, 结果=0
- `free_convection.m`: 参数=3, 几何=6, 物理场=2, 研究=2, 结果=0
...
- `models.mph.free_convection.pdf`: models.mph.free_convection.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.free_convection.zh_CN.pdf`: models.mph.free_convection.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 自然对流传热 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- 脚本/文本识别到的物理场: spf / LaminarFlow, ht / HeatTransferInFluids
...
- PDF keywords: 传热, 流体, models.mph.free_convection.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.free_convection.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `v_in` -> 案例参数/扫描变量; MATLAB: `model.param.set('v_in', value, description)`
- `T_in` -> 案例参数/扫描变量; MATLAB: `model.param.set('T_in', value, description)`
- `T_heat` -> 案例参数/扫描变量; MATLAB: `model.param.set('T_heat', value, description)`
...
- 已识别几何证据：r1 / Rectangle；c1 / Circle；dif1 / Difference；geom1 / r1 / size；geom1 / c1 / r；geom1 / c1 / pos
- 已识别物理场证据：spf / LaminarFlow；ht / HeatTransferInFluids
- 已识别参数：v_in、T_in、T_heat
...
- Transfer recognized physics interfaces to similar requirements: spf / LaminarFlow, ht / HeatTransferInFluids.
...
- Sweep `v_in` around the learned value `5[mm/s]` and export target outputs to CSV.
- Sweep `T_in` around the learned value `20[degC]` and export target outputs to CSV.
- Sweep `T_heat` around the learned value `50[degC]` and export target outputs to CSV.
...
- Summary: 已学习 自然对流传热 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=19 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 19 条；几何=6，物理场=2，材料=1，网格=1，研究=3，结果=0。
