...
- Case directory: `D:\桌面\codex\案例下载\电气\氩气玻尔兹曼分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:27:05.958192+00:00`
...
- `Beta` = `0` - Ionization degree
- `xArs` = `0` - Ars mole fraction
- `wN` = `0` - Reduced frequency
...
- boundary_conditions: bmdl1 / ne, bmdl1 / beta, bmdl1 / x, bmdl1 / Species, bmdl1 / ebar, bmdl1 / TransportPropPG, bmdl1 / RateCoefPG, bmdl1 / MoleConstrainedSpecies, init1 / ebarinit, init1 / Td0, xsec1 / Filepath, eir1 / formula

### 文件内容证据

- `boltzmann_argon.java`: 参数=3, 几何=0, 物理场=1, 研究=0, 结果=0
- `models.plasma.boltzmann_argon.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `boltzmann_argon.m`: 参数=3, 几何=0, 物理场=1, 研究=0, 结果=0
- `boltzmann_argon.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `boltzmann_argon_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.boltzmann_argon.pdf`: models.plasma.boltzmann_argon.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Druyvesteyn, but in some cases they fail to fit the discharge physics. Factors such as

## PDF Summary and Physics Judgement

- Simple summary: boltzmann_argon 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Parametric Sweep 研究。
...
- PDF: Druyvesteyn, but in some cases they fail to fit the discharge physics. Factors such as
- PDF: This tutorial is based on the results obtained in Ref. 1. The Two-term Boltzmann equation
...
- PDF keywords: 结构力学, 流体, 电磁, 优化/参数扫描, models.plasma.boltzmann_argon.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Druyvesteyn, but in some cases they fail to fit the discharge physics. Factors such as
- `Beta` -> Ionization degree; MATLAB: `model.param.set('Beta', value, description)`
- `xArs` -> Ars mole fraction; MATLAB: `model.param.set('xArs', value, description)`
- `wN` -> Reduced frequency; MATLAB: `model.param.set('wN', value, description)`
...
- Sweep `Beta` around the learned value `0` and export target outputs to CSV.
- Sweep `xArs` around the learned value `0` and export target outputs to CSV.
- Sweep `wN` around the learned value `0` and export target outputs to CSV.
...
- Summary: 已学习 boltzmann_argon 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=18 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 18 条；几何=0，物理场=1，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：boltzmann_argon.m, boltzmann_argon.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 boltzmann_argon，训练阶段为 modeling_logic_learning_ready。
...
- `boltzmann_argon.mph` (comsol_mph, 14702094 bytes)
- `boltzmann_argon.m` (matlab_livelink, 32362 bytes)
- `boltzmann_argon.java` (comsol_java, 34465 bytes)
- `boltzmann_argon_summary.json` (json, 1454 bytes)
- `models.plasma.boltzmann_argon.pdf` (pdf_document, 349041 bytes)
