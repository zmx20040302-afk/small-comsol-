...
- Case directory: `D:\桌面\codex\案例下载\电气\正负电晕放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:31:00.931164+00:00`
...
- `rin` = `0.01[cm]` - Electrode radius
- `muiN` = `6e21[1/(V*s*m)]` - Reduced ion mobility
- `rnp` = `2e-6[cm^3/s]` - Ion-ion recombination
- `rei` = `5e-8[cm^3/s]` - Electron-ion recombination
- `P0` = `760[torr]` - Gas pressure
- `T0` = `600[K]` - Gas temperature
- `ni0` = `1e16[m^-3]` - Initial ion number density
- `ne0` = `1e10[m^-3]` - Initial electron number density
- `V0` = `cp*30[kV]` - Applied voltage
- `cp` = `-1` - Corona polarity
...
- `positive_and_negative_corona_discharges.java`: 参数=10, 几何=2, 物理场=0, 研究=0, 结果=0
- `models.plasma.positive_and_negative_corona_discharges.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `positive_and_negative_corona_discharges.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `positive_and_negative_corona_discharges.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `positive_and_negative_corona_discharges_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.positive_and_negative_corona_discharges.pdf`: models.plasma.positive_and_negative_corona_discharges.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This tutorial presents a study of positive and negative corona discharges in dry air at

## PDF Summary and Physics Judgement

- Simple summary: positive_and_negative_corona_discharges 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: This tutorial presents a study of positive and negative corona discharges in dry air at
- PDF: types of models are used: a full self-consistent plasma model and a simplified model. The
- PDF: simulation results obtained with the self-consistent plasma model.
...
- PDF keywords: 传热, 流体, 电磁, 结构力学, models.plasma.positive_and_negative_corona_discharges.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This tutorial presents a study of positive and negative corona discharges in dry air at
- `rin` -> Electrode radius; MATLAB: `model.param.set('rin', value, description)`
- `muiN` -> Reduced ion mobility; MATLAB: `model.param.set('muiN', value, description)`
- `rnp` -> Ion-ion recombination; MATLAB: `model.param.set('rnp', value, description)`
- `rei` -> Electron-ion recombination; MATLAB: `model.param.set('rei', value, description)`
- `P0` -> Gas pressure; MATLAB: `model.param.set('P0', value, description)`
- `T0` -> Gas temperature; MATLAB: `model.param.set('T0', value, description)`
- `ni0` -> Initial ion number density; MATLAB: `model.param.set('ni0', value, description)`
- `ne0` -> Initial electron number density; MATLAB: `model.param.set('ne0', value, description)`
- `V0` -> Applied voltage; MATLAB: `model.param.set('V0', value, description)`
- `cp` -> Corona polarity; MATLAB: `model.param.set('cp', value, description)`
...
- 已识别参数：rin、muiN、rnp、rei、P0、T0、ni0、ne0、V0、cp
...
- Sweep `rin` around the learned value `0.01[cm]` and export target outputs to CSV.
- Sweep `muiN` around the learned value `6e21[1/(V*s*m)]` and export target outputs to CSV.
- Sweep `rnp` around the learned value `2e-6[cm^3/s]` and export target outputs to CSV.
- Sweep `rei` around the learned value `5e-8[cm^3/s]` and export target outputs to CSV.
- Sweep `P0` around the learned value `760[torr]` and export target outputs to CSV.
- Sweep `T0` around the learned value `600[K]` and export target outputs to CSV.
- Sweep `ni0` around the learned value `1e16[m^-3]` and export target outputs to CSV.
- Sweep `ne0` around the learned value `1e10[m^-3]` and export target outputs to CSV.
...
- Summary: 已学习 positive_and_negative_corona_discharges 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=18 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 18 条；几何=2，物理场=1，材料=0，网格=2，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：positive_and_negative_corona_discharges.m, positive_and_negative_corona_discharges.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 positive_and_negative_corona_discharges，训练阶段为 modeling_logic_learning_ready。
...
- `positive_and_negative_corona_discharges.mph` (comsol_mph, 8977275 bytes)
- `positive_and_negative_corona_discharges.m` (matlab_livelink, 71029 bytes)
- `positive_and_negative_corona_discharges.java` (comsol_java, 66812 bytes)
- `positive_and_negative_corona_discharges_summary.json` (json, 1288 bytes)
- `models.plasma.positive_and_negative_corona_discharges.pdf` (pdf_document, 494664 bytes)
