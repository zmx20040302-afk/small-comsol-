# axial_homopolar_induction_bearing

- Case directory: `D:\桌面\codex\案例下载\电气\轴向单极感应轴承 - 三维`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:41:12.423199+00:00`
...
- `airgap_axial` = `1[mm]` - Axial air gap
- `airgap_radial` = `2[mm]` - Radial air gap
- `disc_t` = `2[mm]` - Disc thickness
- `disc_w` = `10[mm]` - Disc width
- `magnet_h` = `disc_t+2*airgap_axial+2*yoke_teeth_h` - Magneth height
- `magnet_w` = `6[mm]` - Magnet width
- `offset` = `-1.5[mm]` - Rotor offset
- `omega` = `50000[rpm]` - Angular rotor speed
- `shaft_d` = `4[mm]` - Shaft diameter
- `shaft_h` = `30[mm]` - Shaft height
- `skin_depth` = `1/sqrt(pi*omega*5.998e7[S/m]*mu0_const)` - Skin depth in copper region
- `yoke_t` = `4[mm]` - Yoke thickness
- `yoke_teeth_h` = `2[mm]` - Yoke teeth height
- `yoke_teeth_w` = `disc_w` - Yoke teeth width
- `yoke_w` = `yoke_teeth_w+magnet_w+4[mm]` - Yoke total width

## 内容级读取结果

- 模型树证据数量: 24
- 理论关键词: 电磁
- geometry: cyl1 / Cylinder, cyl2 / Cylinder, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / pos
- mesh: mesh1
- boundary_conditions: cyl1 / r, cyl1 / h, cyl1 / pos

### 文件内容证据

- `axial_homopolar_induction_bearing.java`: 参数=15, 几何=5, 物理场=0, 研究=0, 结果=0
- `axial_homopolar_induction_bearing.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `axial_homopolar_induction_bearing.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `axial_homopolar_induction_bearing_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: axial_homopolar_induction_bearing 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF keywords: 电磁
- `airgap_axial` -> Axial air gap; MATLAB: `model.param.set('airgap_axial', value, description)`
- `airgap_radial` -> Radial air gap; MATLAB: `model.param.set('airgap_radial', value, description)`
- `disc_t` -> Disc thickness; MATLAB: `model.param.set('disc_t', value, description)`
- `disc_w` -> Disc width; MATLAB: `model.param.set('disc_w', value, description)`
- `magnet_h` -> Magneth height; MATLAB: `model.param.set('magnet_h', value, description)`
- `magnet_w` -> Magnet width; MATLAB: `model.param.set('magnet_w', value, description)`
- `offset` -> Rotor offset; MATLAB: `model.param.set('offset', value, description)`
- `omega` -> Angular rotor speed; MATLAB: `model.param.set('omega', value, description)`
- `shaft_d` -> Shaft diameter; MATLAB: `model.param.set('shaft_d', value, description)`
- `shaft_h` -> Shaft height; MATLAB: `model.param.set('shaft_h', value, description)`
- `skin_depth` -> Skin depth in copper region; MATLAB: `model.param.set('skin_depth', value, description)`
- `yoke_t` -> Yoke thickness; MATLAB: `model.param.set('yoke_t', value, description)`
...
- 已识别几何证据：cyl1 / Cylinder；cyl2 / Cylinder；geom1 / cyl1 / r；geom1 / cyl1 / h；geom1 / cyl1 / pos
- 已识别参数：airgap_axial、airgap_radial、disc_t、disc_w、magnet_h、magnet_w、offset、omega、shaft_d、shaft_h、skin_depth、yoke_t
...
- Sweep `airgap_axial` around the learned value `1[mm]` and export target outputs to CSV.
- Sweep `airgap_radial` around the learned value `2[mm]` and export target outputs to CSV.
- Sweep `disc_t` around the learned value `2[mm]` and export target outputs to CSV.
- Sweep `disc_w` around the learned value `10[mm]` and export target outputs to CSV.
- Sweep `magnet_h` around the learned value `disc_t+2*airgap_axial+2*yoke_teeth_h` and export target outputs to CSV.
- Sweep `magnet_w` around the learned value `6[mm]` and export target outputs to CSV.
- Sweep `offset` around the learned value `-1.5[mm]` and export target outputs to CSV.
- Sweep `omega` around the learned value `50000[rpm]` and export target outputs to CSV.
...
- Summary: 已学习 axial_homopolar_induction_bearing 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=15；内容证据=24 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 24 条；几何=5，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：axial_homopolar_induction_bearing.m, axial_homopolar_induction_bearing.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 axial_homopolar_induction_bearing，训练阶段为 modeling_logic_learning_ready。
...
- `axial_homopolar_induction_bearing.mph` (comsol_mph, 230110900 bytes)
- `axial_homopolar_induction_bearing.m` (matlab_livelink, 56499 bytes)
- `axial_homopolar_induction_bearing.java` (comsol_java, 52452 bytes)
- `axial_homopolar_induction_bearing_summary.json` (json, 1450 bytes)
