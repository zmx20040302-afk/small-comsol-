...
- Case directory: `D:\桌面\codex\案例下载\电气\离子风`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:43:31.276570+00:00`
...
- physics: edis / ElectricDischarge, Laminar Flow
- materials: mat1
- mesh: mesh1
- studies: Stationary
...
- `ionic_wind.java`: 参数=2, 几何=14, 物理场=1, 研究=0, 结果=0
- `models.edis.ionic_wind.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `ionic_wind.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ionic_wind.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ionic_wind_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.ionic_wind.pdf`: models.edis.ionic_wind.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Laminar Flow，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion

## PDF Summary and Physics Judgement

- Simple summary: ionic_wind 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- PDF: Open the corona_discharge_iv_curve model.
- PDF: ADD PHYSICS
- 脚本/文本识别到的物理场: edis / ElectricDischarge, Laminar Flow
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 结构力学, 流体
...
- PDF keywords: 电磁, 结构力学, 流体, models.edis.ionic_wind.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Laminar Flow，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion
...
- 已识别物理场证据：edis / ElectricDischarge；Laminar Flow
- 已识别参数：V0、d
...
- Transfer recognized physics interfaces to similar requirements: edis / ElectricDischarge, Laminar Flow.
...
- Summary: 已学习 ionic_wind 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=14，物理场=2，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：ionic_wind.m, ionic_wind.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 ionic_wind，训练阶段为 modeling_logic_learning_ready。
...
- `ionic_wind.mph` (comsol_mph, 16980045 bytes)
- `ionic_wind.m` (matlab_livelink, 26476 bytes)
- `ionic_wind.java` (comsol_java, 23330 bytes)
- `ionic_wind_summary.json` (json, 996 bytes)
- `models.edis.ionic_wind.pdf` (pdf_document, 277857 bytes)
