...
- `c3_xw` = `26[mm]` - x-Workplane for C3 vertebra
- `c5_xw` = `56[mm]` - x-Workplane for C5 vertebra
- `c3_zw` = `15[mm]` - z-Workplane for C3 vertebra
...
- geometry: cyl1 / Cylinder, wp1 / WorkPlane, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / pos, geom1 / cyl1 / layername, geom1 / cyl1 / layerside, geom1 / cyl1 / layerbottom, geom1 / cyl1 / layertop, c1 / Circle, geom1 / wp1 / quickz, geom1 / wp1 / unite
- physics: Electromagnetic Waves, Heat Transfer
- mesh: mesh1
- studies: Time Dependent, Frequency Domain
- boundary_conditions: cyl1 / r, cyl1 / h, cyl1 / pos, cyl1 / layername, cyl1 / layerside, cyl1 / layerbottom, cyl1 / layertop, wp1 / quickz, wp1 / unite

### 文件内容证据

- `mri_implant_heating_geom_sequence.java`: 参数=4, 几何=12, 物理场=0, 研究=0, 结果=0
- `mri_implant_heating_geom_sequence.m`: 参数=4, 几何=9, 物理场=0, 研究=0, 结果=0
- `models.rf.mri_implant_heating.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `mri_implant_heating_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `mri_implant_heating_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: mri_implant_heating_geom_sequence 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- `c3_xw` -> x-Workplane for C3 vertebra; MATLAB: `model.param.set('c3_xw', value, description)`
- `c5_xw` -> x-Workplane for C5 vertebra; MATLAB: `model.param.set('c5_xw', value, description)`
- `c3_zw` -> z-Workplane for C3 vertebra; MATLAB: `model.param.set('c3_zw', value, description)`
...
- 已识别几何证据：cyl1 / Cylinder；wp1 / WorkPlane；geom1 / cyl1 / r；geom1 / cyl1 / h；geom1 / cyl1 / pos；geom1 / cyl1 / layername
- 已识别物理场证据：Electromagnetic Waves；Heat Transfer
- 已识别参数：c3_xw、c5_xw、c3_zw、d_rod
...
- Sweep `c5_xw` around the learned value `56[mm]` and export target outputs to CSV.
- Sweep `c3_zw` around the learned value `15[mm]` and export target outputs to CSV.
...
- Summary: 已学习 mri_implant_heating_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=30 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 30 条；几何=12，物理场=2，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mri_implant_heating_geom_sequence.m, mri_implant_heating_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mri_implant_heating_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `mri_implant_heating_geom_sequence.mph` (comsol_mph, 727113 bytes)
- `mri_implant_heating_geom_sequence.m` (matlab_livelink, 11079 bytes)
- `mri_implant_heating_geom_sequence.java` (comsol_java, 11230 bytes)
- `mri_implant_heating_geom_sequence_summary.json` (json, 1091 bytes)
