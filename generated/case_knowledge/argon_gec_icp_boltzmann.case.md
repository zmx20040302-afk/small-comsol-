...
- Case directory: `D:\桌面\codex\案例下载\电气\通过两项玻尔兹曼方程耦合模拟 GEC ICP 反应器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:58:06.213635+00:00`
...
- `Psp` = `1500[W]` - Power input
- `mueN` = `4E24[1/(m*V*s)]` - Reduced electron mobility
- `T0` = `300[K]` - Gas temperature
- `p0` = `0.02[torr]` - Gas pressure

## 内容级读取结果

- 模型树证据数量: 29
- 理论关键词: 传热, 流体, PDE/方程建模, 电磁
- geometry: pol1 / Polygon, r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, r4 / Rectangle, geom1 / pol1 / x, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r2 / pos, geom1 / r3 / size, geom1 / r3 / pos
- physics: Electrostatics, Magnetic Fields
- mesh: mesh1
- studies: Frequency Domain
- boundary_conditions: pol1 / x, r1 / size, r1 / pos, r2 / size, r2 / pos, r3 / size, r3 / pos, r4 / size

### 文件内容证据

- `argon_gec_icp_boltzmann.java`: 参数=4, 几何=13, 物理场=0, 研究=0, 结果=0
- `models.plasma.argon_gec_icp_boltzmann.pdf`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `argon_gec_icp_boltzmann_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `argon_gec_icp_boltzmann.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `argon_gec_icp_boltzmann.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.argon_gec_icp_boltzmann.pdf`: models.plasma.argon_gec_icp_boltzmann.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics、Magnetic Fields，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Reference Cell operates as an inductively-coupled plasma in this model.

## PDF Summary and Physics Judgement

- Simple summary: argon_gec_icp_boltzmann 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- PDF: Reference Cell operates as an inductively-coupled plasma in this model.
- PDF: distribution function over electron scattering cross sections. Simulation results from this
- PDF: model are compared with results from the model GEC ICP Reactor, Argon Chemistry
...
- PDF keywords: 传热, 流体, PDE/方程建模, 电磁, models.plasma.argon_gec_icp_boltzmann.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics、Magnetic Fields，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Reference Cell operates as an inductively-coupled plasma in this model.
- `Psp` -> Power input; MATLAB: `model.param.set('Psp', value, description)`
- `mueN` -> Reduced electron mobility; MATLAB: `model.param.set('mueN', value, description)`
...
- 已识别几何证据：pol1 / Polygon；r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；r4 / Rectangle；geom1 / pol1 / x
- 已识别物理场证据：Electrostatics；Magnetic Fields
- 已识别参数：Psp、mueN、T0、p0
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 流体, PDE/方程建模, 电磁?
...
- Sweep `Psp` around the learned value `1500[W]` and export target outputs to CSV.
- Sweep `mueN` around the learned value `4E24[1/(m*V*s)]` and export target outputs to CSV.
...
- Summary: 已学习 argon_gec_icp_boltzmann 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=29 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 29 条；几何=13，物理场=2，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：argon_gec_icp_boltzmann.m, argon_gec_icp_boltzmann.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 argon_gec_icp_boltzmann，训练阶段为 modeling_logic_learning_ready。
...
- `argon_gec_icp_boltzmann.mph` (comsol_mph, 83374781 bytes)
- `argon_gec_icp_boltzmann.m` (matlab_livelink, 91414 bytes)
- `argon_gec_icp_boltzmann.java` (comsol_java, 92164 bytes)
- `argon_gec_icp_boltzmann_summary.json` (json, 1733 bytes)
- `models.plasma.argon_gec_icp_boltzmann.pdf` (pdf_document, 1365815 bytes)
