...
- Created at: `2026-08-16T11:08:39.485661+00:00`
...
- `lda` = `1[um]` - Wavelength
- `n_ar` = `sqrt(n_fs)` - Refractive index of the AR coating
- `n_fs` = `1.5` - Refractive index of the phase shifter
- `n_bs` = `4.85` - Refractive index of the beam splitter coating
- `t_ar` = `lda/(4*n_ar)` - Thickness of the AR coating
- `theta` = `pi/3` - First phase shift
- `alpha` = `pi/4` - Second phase shift
- `beta` = `pi/6` - Third phase shift
- `t1` = `1000*(lda/n_fs)+lda*(theta/(2*pi))/(n_fs-1)` - Thickness of the first phase shift
- `t2` = `1000*(lda/n_fs)+lda*(alpha/(2*pi))/(n_fs-1)` - Thickness of the second phase shift
- `t3` = `1000*(lda/n_fs)+lda*(beta/(2*pi))/(n_fs-1)` - Thickness of the third phase shift
- `E1` = `1[V/m]` - Electric field amplitude of the first input
- `E2` = `2[V/m]` - Electric field amplitude of the second input
- `u11` = `0.5*exp(-j*alpha)*(exp(-j*theta)-1)` - First element of the unitary matrix
- `u12` = `0.5*j*exp(-j*alpha)*(exp(-j*theta)+1)` - Second element of the unitary matrix
- `u21` = `0.5*j*exp(-j*beta)*(exp(-j*theta)+1)` - Third element of the unitary matrix
- `u22` = `0.5*exp(-j*beta)*(1-exp(-j*theta))` - Forth element of the unitary matrix
- `E1p` = `u11*E1+u12*E2` - Predicted field amplitude of the first output
- `E2p` = `u21*E1+u22*E2` - Predicted field amplitude of the second output

## 内容级读取结果

- 模型树证据数量: 26
- 理论关键词: 电磁
- geometry: sq1 / Square, r1 / Rectangle, geom1 / r1 / size, geom1 / r1 / pos
- mesh: mesh1
- boundary_conditions: r1 / size, r1 / pos

### 文件内容证据

- `free_space_mzi.java`: 参数=19, 几何=4, 物理场=0, 研究=0, 结果=0
- `free_space_mzi.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `free_space_mzi.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `free_space_mzi_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: free_space_mzi 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `n_ar` -> Refractive index of the AR coating; MATLAB: `model.param.set('n_ar', value, description)`
- `n_fs` -> Refractive index of the phase shifter; MATLAB: `model.param.set('n_fs', value, description)`
- `n_bs` -> Refractive index of the beam splitter coating; MATLAB: `model.param.set('n_bs', value, description)`
- `t_ar` -> Thickness of the AR coating; MATLAB: `model.param.set('t_ar', value, description)`
- `theta` -> First phase shift; MATLAB: `model.param.set('theta', value, description)`
- `alpha` -> Second phase shift; MATLAB: `model.param.set('alpha', value, description)`
- `beta` -> Third phase shift; MATLAB: `model.param.set('beta', value, description)`
- `t1` -> Thickness of the first phase shift; MATLAB: `model.param.set('t1', value, description)`
- `t2` -> Thickness of the second phase shift; MATLAB: `model.param.set('t2', value, description)`
- `t3` -> Thickness of the third phase shift; MATLAB: `model.param.set('t3', value, description)`
- `E1` -> Electric field amplitude of the first input; MATLAB: `model.param.set('E1', value, description)`
...
- 已识别几何证据：sq1 / Square；r1 / Rectangle；geom1 / r1 / size；geom1 / r1 / pos
- 已识别参数：lda、n_ar、n_fs、n_bs、t_ar、theta、alpha、beta、t1、t2、t3、E1
...
- Sweep `lda` around the learned value `1[um]` and export target outputs to CSV.
- Sweep `n_ar` around the learned value `sqrt(n_fs)` and export target outputs to CSV.
- Sweep `n_fs` around the learned value `1.5` and export target outputs to CSV.
- Sweep `n_bs` around the learned value `4.85` and export target outputs to CSV.
- Sweep `t_ar` around the learned value `lda/(4*n_ar)` and export target outputs to CSV.
- Sweep `theta` around the learned value `pi/3` and export target outputs to CSV.
- Sweep `alpha` around the learned value `pi/4` and export target outputs to CSV.
- Sweep `beta` around the learned value `pi/6` and export target outputs to CSV.
...
- Summary: 已学习 free_space_mzi 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=19；内容证据=26 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 26 条；几何=4，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：free_space_mzi.m, free_space_mzi.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 free_space_mzi，训练阶段为 modeling_logic_learning_ready。
...
- `free_space_mzi.mph` (comsol_mph, 222779695 bytes)
- `free_space_mzi.m` (matlab_livelink, 38688 bytes)
- `free_space_mzi.java` (comsol_java, 36418 bytes)
- `free_space_mzi_summary.json` (json, 1845 bytes)
