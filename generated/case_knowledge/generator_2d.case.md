...
- Case directory: `D:\桌面\codex\案例下载\电气\发电机二维模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:51:32.326736+00:00`
...
- `L` = `0.4[m]` - Length of generator
- `rpm` = `60[1/min]` - Rotational speed of rotor
...
- `generator_2d.java`: 参数=4, 几何=2, 物理场=0, 研究=0, 结果=0
- `models.acdc.generator_2d.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `generator_2d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `generator_2d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `generator_2d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.generator_2d.pdf`: models.acdc.generator_2d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Modeling in COMSOL Multiphysics

## PDF Summary and Physics Judgement

- Simple summary: generator_2d 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Magnetic Fields，并采用 Time Dependent 研究。
...
- PDF: Modeling in COMSOL Multiphysics
- PDF: The COMSOL Multiphysics model of the generator is a time-dependent 2D problem on
- PDF: between the stator and rotor geometries. Thus, there is no Lorentz term in the equation,
...
- PDF keywords: 流体, 电磁, 多孔介质/裂隙, models.acdc.generator_2d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Modeling in COMSOL Multiphysics
- `L` -> Length of generator; MATLAB: `model.param.set('L', value, description)`
- `rpm` -> Rotational speed of rotor; MATLAB: `model.param.set('rpm', value, description)`
...
- Sweep `L` around the learned value `0.4[m]` and export target outputs to CSV.
- Sweep `rpm` around the learned value `60[1/min]` and export target outputs to CSV.
...
- Summary: 已学习 generator_2d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=11 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 11 条；几何=2，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：generator_2d.m, generator_2d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 generator_2d，训练阶段为 modeling_logic_learning_ready。
...
- `generator_2d.mph` (comsol_mph, 30646840 bytes)
- `generator_2d.m` (matlab_livelink, 47271 bytes)
- `generator_2d.java` (comsol_java, 45577 bytes)
- `generator_2d_summary.json` (json, 1414 bytes)
- `models.acdc.generator_2d.pdf` (pdf_document, 1271377 bytes)
