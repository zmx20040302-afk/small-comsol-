...
- Case directory: `D:\桌面\codex\案例下载\电气\非晶硅等离子体增强化学气相沉积的硅烷氩气 ICP 反应器仿真`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:16:13.997711+00:00`
...
- `xSiH4` = `0.05` - SiH4 mole fraction
- `xAr` = `1-xSiH4` - Ar mole fraction
- `p0` = `20[mTorr]` - Pressure
- `Pw` = `50[W]` - Input power
- `Qf` = `5` - Mass flow in SCCM
- `f0` = `13.56e6[Hz]` - Excitation frequency
- `xinletH2` = `1e-4` - Mole fraction at inlet
- `xinlet` = `1e-6` - Mole fraction at inlet
- `Coil_chamber_height` = `7[cm]` - Coil chamber height
- `Coil_height` = `0.9[cm]` - Coil height
- `Coil_width` = `1.2[cm]` - Coil width
- `Coils_spacing` = `4.5[cm]` - Spacing between coils
- `Dh` = `2[cm]+0.2[cm]` - Dielectric block height
- `Dw` = `3[cm]` - Dielectric block width
- `First_coil_r` = `4[cm]` - First coil radial position
- `First_coil_z` = `7[mm]` - First coils distance above the window
- `Hc` = `20[cm]` - Plasma chamber height
- `Inh` = `8[cm]` - Window support block height
- `Inw` = `4[cm]` - Window support  block width
- `recH` = `0.005` - F recombination
- `recSiH3` = `0.005` - O recombination
- `Wc` = `25[cm]` - Plasma chamber width
- `Wh` = `0.2[cm]` - Wafer height
- `Whh` = `2[cm]` - Wafer holder height
- `Whw` = `15[cm]+0.2[cm]` - Wafer holder width
- `Window_thickness` = `1.5[cm]` - Window thickness
- `Ww` = `15[cm]` - Wafer width
- `Wz` = `2[cm]` - Wafer z coordinate
- `InletW` = `0.9[cm]` - Inlet tube width
- `InletH` = `6[cm]` - Inlet tube height

## 内容级读取结果

- 模型树证据数量: 43
- 理论关键词: 流体, 电磁, 传热, 结构力学, 优化/参数扫描
- geometry: r1 / Rectangle, r2 / Rectangle, geom1 / r1 / size
- physics: Heat Transfer, Laminar Flow, Electrostatics, Magnetic Fields
- mesh: mesh1
...
- `icp_pecvd_sih4_ar.java`: 参数=20, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.plasma.icp_pecvd_sih4_ar.pdf`: 参数=0, 几何=0, 物理场=4, 研究=4, 结果=0
- `icp_pecvd_sih4_ar.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `icp_pecvd_sih4_ar.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `icp_pecvd_sih4_ar_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.icp_pecvd_sih4_ar.pdf`: models.plasma.icp_pecvd_sih4_ar.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；plasma (ICP) reactor operating with a silane/argon gas mixture. The study focuses on the

## PDF Summary and Physics Judgement

- Simple summary: icp_pecvd_sih4_ar 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Time Dependent 研究。
- Primary physics: `Laminar Flow`
- Study type: `Time Dependent`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Laminar Flow，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Time Dependent 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: plasma (ICP) reactor operating with a silane/argon gas mixture. The study focuses on the
- PDF: the plasma conductivity, which is set in the Plasma Conductivity Coupling multiphysics node:
- PDF: heating the electrons is set in the Electron Heat Source multiphysics node.
- 脚本/文本识别到的物理场: Heat Transfer, Laminar Flow, Electrostatics, Magnetic Fields
...
- PDF keywords: 流体, 电磁, 传热, 结构力学, 优化/参数扫描, models.plasma.icp_pecvd_sih4_ar.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；plasma (ICP) reactor operating with a silane/argon gas mixture. The study focuses on the
- `xSiH4` -> SiH4 mole fraction; MATLAB: `model.param.set('xSiH4', value, description)`
- `xAr` -> Ar mole fraction; MATLAB: `model.param.set('xAr', value, description)`
- `p0` -> Pressure; MATLAB: `model.param.set('p0', value, description)`
- `Pw` -> Input power; MATLAB: `model.param.set('Pw', value, description)`
- `Qf` -> Mass flow in SCCM; MATLAB: `model.param.set('Qf', value, description)`
- `f0` -> Excitation frequency; MATLAB: `model.param.set('f0', value, description)`
- `xinletH2` -> Mole fraction at inlet; MATLAB: `model.param.set('xinletH2', value, description)`
- `xinlet` -> Mole fraction at inlet; MATLAB: `model.param.set('xinlet', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；geom1 / r1 / size
- 已识别物理场证据：Heat Transfer；Laminar Flow；Electrostatics；Magnetic Fields
- 已识别参数：xSiH4、xAr、p0、Pw、Qf、f0、xinletH2、xinlet、Coil_chamber_height、Coil_height、Coil_width、Coils_spacing
...
- Transfer recognized physics interfaces to similar requirements: Heat Transfer, Laminar Flow, Electrostatics, Magnetic Fields.
...
- Sweep `xSiH4` around the learned value `0.05` and export target outputs to CSV.
- Sweep `xAr` around the learned value `1-xSiH4` and export target outputs to CSV.
- Sweep `p0` around the learned value `20[mTorr]` and export target outputs to CSV.
- Sweep `Pw` around the learned value `50[W]` and export target outputs to CSV.
- Sweep `Qf` around the learned value `5` and export target outputs to CSV.
- Sweep `f0` around the learned value `13.56e6[Hz]` and export target outputs to CSV.
- Sweep `xinletH2` around the learned value `1e-4` and export target outputs to CSV.
- Sweep `xinlet` around the learned value `1e-6` and export target outputs to CSV.
...
- Summary: 已学习 icp_pecvd_sih4_ar 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=30；内容证据=43 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 43 条；几何=3，物理场=4，材料=0，网格=1，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：icp_pecvd_sih4_ar.m, icp_pecvd_sih4_ar.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 icp_pecvd_sih4_ar，训练阶段为 modeling_logic_learning_ready。
...
- `icp_pecvd_sih4_ar.mph` (comsol_mph, 83517606 bytes)
- `icp_pecvd_sih4_ar.m` (matlab_livelink, 315623 bytes)
- `icp_pecvd_sih4_ar.java` (comsol_java, 330092 bytes)
- `icp_pecvd_sih4_ar_summary.json` (json, 3312 bytes)
- `models.plasma.icp_pecvd_sih4_ar.pdf` (pdf_document, 888322 bytes)
