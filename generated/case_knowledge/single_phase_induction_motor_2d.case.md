# single_phase_induction_motor_2d

- Case directory: `D:\桌面\codex\案例下载\电气\单相感应电机：TEAM 研讨会问题 30`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:35:29.249469+00:00`
...
- `n0` = `2045.175` - Number of turns
- `L` = `1[m]` - Length of the motor
- `RPM` = `0[rad/s]` - Motor rpm

## 内容级读取结果

- 模型树证据数量: 20
- 理论关键词: 未自动识别
- geometry: c1 / Circle, c2 / Circle, c3 / Circle, geom1 / c1 / r, geom1 / c2 / r
- mesh: mesh1
- boundary_conditions: c1 / r, c2 / r

### 文件内容证据

- `single_phase_induction_motor_2d.java`: 参数=12, 几何=5, 物理场=0, 研究=0, 结果=0
- `single_phase_induction_motor_2d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `single_phase_induction_motor_2d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `single_phase_induction_motor_2d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: single_phase_induction_motor_2d 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- 已识别几何证据：c1 / Circle；c2 / Circle；c3 / Circle；geom1 / c1 / r；geom1 / c2 / r
- 已识别参数：r1、r2、r3、r4、r5、win_angle、airgap、f0、w0、n0、L、RPM
...
- Summary: 已学习 single_phase_induction_motor_2d 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=20 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=5，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：single_phase_induction_motor_2d.m, single_phase_induction_motor_2d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 single_phase_induction_motor_2d，训练阶段为 modeling_logic_learning_ready。
...
- `single_phase_induction_motor_2d.mph` (comsol_mph, 24052300 bytes)
- `single_phase_induction_motor_2d.m` (matlab_livelink, 43279 bytes)
- `single_phase_induction_motor_2d.java` (comsol_java, 37626 bytes)
- `single_phase_induction_motor_2d_summary.json` (json, 1375 bytes)
