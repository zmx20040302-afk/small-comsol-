...
- Case directory: `D:\桌面\codex\案例下载\电气\弯曲的阶跃折射率光纤`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T03:09:09.182283+00:00`
...
- `lda0` = `1.55[um]` - Wavelength
- `f0` = `c_const/lda0` - Frequency
- `nClad` = `1.4378` - Refractive index, cladding
- `nCore` = `1.4457` - Refractive index, core
- `aCore` = `8[um]` - Core radius
- `aClad` = `40[um]` - Cladding radius
- `Rb` = `3[mm]` - Bend radius
- `aSquare` = `100[um]` - Side length of square
- `tPML` = `20[um]` - PML thickness
- `dr` = `aSquare/2-tPML` - Distance from core center to PML boundary
- `ldaPML` = `lda0/sqrt(nClad^2-(nCore*Rb/(Rb+dr))^2)` - Radial wavelength in PML

## 内容级读取结果

- 模型树证据数量: 31
- 理论关键词: 流体, 电磁
- geometry: c1 / Circle, c2 / Circle, sq1 / Square, geom1 / c1 / r, geom1 / c2 / r, geom2 / c1 / r, geom2 / c1 / pos, geom2 / sq1 / size, geom2 / sq1 / base
- physics: Magnetic Fields, Electromagnetic Waves
- mesh: mesh1, mesh2
- studies: Eigenvalue, Frequency Domain
- boundary_conditions: c1 / r, c2 / r, c1 / pos, sq1 / size, sq1 / base

### 文件内容证据

- `step_index_fiber_bend.java`: 参数=11, 几何=9, 物理场=0, 研究=0, 结果=0
- `models.woptics.step_index_fiber_bend.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `step_index_fiber_bend.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `step_index_fiber_bend.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `step_index_fiber_bend_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.step_index_fiber_bend.pdf`: models.woptics.step_index_fiber_bend.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Electromagnetic Waves，研究类型线索为 Eigenvalue、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；with analytically. A circular step-index waveguide is a basic shape where benchmark results

## PDF Summary and Physics Judgement

- Simple summary: step_index_fiber_bend 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Magnetic Fields，并采用 Eigenvalue 研究。
- Primary physics: `Magnetic Fields`
- Study type: `Eigenvalue`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Magnetic Fields，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Eigenvalue 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: with analytically. A circular step-index waveguide is a basic shape where benchmark results
- PDF: constant angular velocity. As a result, the wavelength and the propagation constant varies
- PDF: The second mode analysis is performed for a 2D axisymmetric geometry. In this case, the
- 脚本/文本识别到的物理场: Magnetic Fields, Electromagnetic Waves
- 识别到的研究类型: Eigenvalue, Frequency Domain
- 理论关键词: 流体, 电磁

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 流体, 电磁, models.woptics.step_index_fiber_bend.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Electromagnetic Waves，研究类型线索为 Eigenvalue、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；with analytically. A circular step-index waveguide is a basic shape where benchmark results
- `lda0` -> Wavelength; MATLAB: `model.param.set('lda0', value, description)`
- `f0` -> Frequency; MATLAB: `model.param.set('f0', value, description)`
- `nClad` -> Refractive index, cladding; MATLAB: `model.param.set('nClad', value, description)`
- `nCore` -> Refractive index, core; MATLAB: `model.param.set('nCore', value, description)`
- `aCore` -> Core radius; MATLAB: `model.param.set('aCore', value, description)`
- `aClad` -> Cladding radius; MATLAB: `model.param.set('aClad', value, description)`
- `Rb` -> Bend radius; MATLAB: `model.param.set('Rb', value, description)`
- `aSquare` -> Side length of square; MATLAB: `model.param.set('aSquare', value, description)`
- `tPML` -> PML thickness; MATLAB: `model.param.set('tPML', value, description)`
- `dr` -> Distance from core center to PML boundary; MATLAB: `model.param.set('dr', value, description)`
- `ldaPML` -> Radial wavelength in PML; MATLAB: `model.param.set('ldaPML', value, description)`
...
- 已识别几何证据：c1 / Circle；c2 / Circle；sq1 / Square；geom1 / c1 / r；geom1 / c2 / r；geom2 / c1 / r
- 已识别物理场证据：Magnetic Fields；Electromagnetic Waves
- 已识别参数：lda0、f0、nClad、nCore、aCore、aClad、Rb、aSquare、tPML、dr、ldaPML
...
- Use the learned study types as solver starting points: Eigenvalue, Frequency Domain.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 电磁?
...
- Sweep `nClad` around the learned value `1.4378` and export target outputs to CSV.
- Sweep `nCore` around the learned value `1.4457` and export target outputs to CSV.
- Sweep `aCore` around the learned value `8[um]` and export target outputs to CSV.
- Sweep `aClad` around the learned value `40[um]` and export target outputs to CSV.
- Sweep `Rb` around the learned value `3[mm]` and export target outputs to CSV.
- Sweep `aSquare` around the learned value `100[um]` and export target outputs to CSV.
...
- Summary: 已学习 step_index_fiber_bend 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=9，物理场=2，材料=0，网格=2，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：step_index_fiber_bend.m, step_index_fiber_bend.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 step_index_fiber_bend，训练阶段为 modeling_logic_learning_ready。
...
- `step_index_fiber_bend.mph` (comsol_mph, 9290613 bytes)
- `step_index_fiber_bend.m` (matlab_livelink, 31804 bytes)
- `step_index_fiber_bend.java` (comsol_java, 28905 bytes)
- `step_index_fiber_bend_summary.json` (json, 1610 bytes)
- `models.woptics.step_index_fiber_bend.pdf` (pdf_document, 1996659 bytes)
