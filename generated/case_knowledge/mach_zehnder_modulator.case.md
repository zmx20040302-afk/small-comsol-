...
- Case directory: `D:\桌面\codex\案例下载\电气\马赫-曾德尔调制器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T08:30:34.556619+00:00`
...
- `n_clad` = `2.2` - Cladding refractive index
- `n_core` = `2.22` - Core refractive index
- `hx` = `lda0/(6*n_core)/(1-cos(alpha))` - Maximum element size in x-direction
- `hy` = `lda0/(6*n_core)/sin(alpha)` - Maximum element size in y-direction
- `r13` = `30[pm/V]` - Electro-optic coefficient
- `V0` = `100[V]` - Applied voltage
- `epsr` = `35` - Low-frequency relative permittivity

## 内容级读取结果

- 模型树证据数量: 15
- 理论关键词: 结构力学, 电磁, 优化/参数扫描
- physics: Electrostatics, Electromagnetic Waves
- mesh: mesh1
- studies: Stationary, Frequency Domain, Parametric Sweep

### 文件内容证据

- `mach_zehnder_modulator.java`: 参数=9, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.woptics.mach_zehnder_modulator.pdf`: 参数=0, 几何=0, 物理场=2, 研究=3, 结果=0
- `mach_zehnder_modulator.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mach_zehnder_modulator.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mach_zehnder_modulator_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.mach_zehnder_modulator.pdf`: models.woptics.mach_zehnder_modulator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics、Electromagnetic Waves，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the applied electric field. A refractive index change results in a change of the phase of the

## PDF Summary and Physics Judgement

- Simple summary: mach_zehnder_modulator 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF: the applied electric field. A refractive index change results in a change of the phase of the
- PDF: optical waveguide modulator. The model is implemented in a 2D geometry, but could be
- PDF: Thus, you must make sure that the phase variation is well resolved by the mesh. For
- 脚本/文本识别到的物理场: Electrostatics, Electromagnetic Waves
...
- PDF keywords: 结构力学, 电磁, 优化/参数扫描, models.woptics.mach_zehnder_modulator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics、Electromagnetic Waves，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the applied electric field. A refractive index change results in a change of the phase of the
...
- `hx` -> Maximum element size in x-direction; MATLAB: `model.param.set('hx', value, description)`
- `hy` -> Maximum element size in y-direction; MATLAB: `model.param.set('hy', value, description)`
- `r13` -> Electro-optic coefficient; MATLAB: `model.param.set('r13', value, description)`
- `V0` -> Applied voltage; MATLAB: `model.param.set('V0', value, description)`
- `epsr` -> Low-frequency relative permittivity; MATLAB: `model.param.set('epsr', value, description)`
...
- 已识别物理场证据：Electrostatics；Electromagnetic Waves
- 已识别参数：lda0、f0、n_clad、n_core、hx、hy、r13、V0、epsr
...
- Transfer recognized physics interfaces to similar requirements: Electrostatics, Electromagnetic Waves.
...
- Sweep `n_clad` around the learned value `2.2` and export target outputs to CSV.
- Sweep `n_core` around the learned value `2.22` and export target outputs to CSV.
- Sweep `hx` around the learned value `lda0/(6*n_core)/(1-cos(alpha))` and export target outputs to CSV.
- Sweep `hy` around the learned value `lda0/(6*n_core)/sin(alpha)` and export target outputs to CSV.
- Sweep `r13` around the learned value `30[pm/V]` and export target outputs to CSV.
- Sweep `V0` around the learned value `100[V]` and export target outputs to CSV.
...
- Summary: 已学习 mach_zehnder_modulator 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=15 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 15 条；几何=0，物理场=2，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mach_zehnder_modulator.m, mach_zehnder_modulator.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mach_zehnder_modulator，训练阶段为 modeling_logic_learning_ready。
...
- `mach_zehnder_modulator.mph` (comsol_mph, 243331309 bytes)
- `mach_zehnder_modulator.m` (matlab_livelink, 53759 bytes)
- `mach_zehnder_modulator.java` (comsol_java, 50461 bytes)
- `mach_zehnder_modulator_summary.json` (json, 1803 bytes)
- `models.woptics.mach_zehnder_modulator.pdf` (pdf_document, 2068556 bytes)
