...
- `pz_theory` = `pz/(j*2*pi*f0)*sym_factor` - Scalar components for theory
- `E_coeff` = `Z*((c_const/n)/(4*pi))` - Coefficient for E-field theory variables
- `sim_r` = `15*wl` - Exterior of simulation
- `dipole_sim_r` = `wl/2` - Radius for Full-Wave simulation
- `mesh_r` = `wl/10` - Included for mesh refinement near source
- `sym_factor` = `8` - Symmetry factor

## 内容级读取结果

- 模型树证据数量: 35
- 理论关键词: 电磁, 传热
- geometry: sph1 / Sphere, pt1 / Point, sph2 / Sphere, del1 / Delete, geom1 / sph1 / r, geom1 / sph1 / layername, geom1 / sph1 / layer, geom1 / sph2 / r, pard1 / PartitionDomains, geom1 / pard1 / partitionwith
- physics: Electromagnetic Waves
- mesh: mesh1
- studies: Frequency Domain
- boundary_conditions: sph1 / r, sph1 / layername, sph1 / layer, sph2 / r, pard1 / partitionwith

### 文件内容证据

- `rf_to_beam_envelopes_63.java`: 参数=17, 几何=9, 物理场=0, 研究=0, 结果=0
- `rf_to_beam_envelopes_63.m`: 参数=17, 几何=8, 物理场=0, 研究=0, 结果=0
- `rf_to_beam_envelopes.pptx`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `half_wavelength_dipole_antenna.pptx`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rf_to_beam_envelopes_63.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rf_to_beam_envelopes_63_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: rf_to_beam_envelopes_63 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- 已识别几何证据：sph1 / Sphere；pt1 / Point；sph2 / Sphere；del1 / Delete；geom1 / sph1 / r；geom1 / sph1 / layername
...
- Summary: 已学习 rf_to_beam_envelopes_63 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=17；内容证据=35 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 35 条；几何=10，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：rf_to_beam_envelopes_63.m, rf_to_beam_envelopes_63.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 rf_to_beam_envelopes_63，训练阶段为 modeling_logic_learning_ready。
...
- `rf_to_beam_envelopes_63.mph` (comsol_mph, 10840249 bytes)
- `rf_to_beam_envelopes_63.m` (matlab_livelink, 62124 bytes)
- `rf_to_beam_envelopes_63.java` (comsol_java, 66081 bytes)
- `rf_to_beam_envelopes_63_summary.json` (json, 1481 bytes)
