# electromagnetic_plunger

- Case directory: `D:\桌面\codex\案例下载\电气\电磁柱塞`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:16:53.631723+00:00`
...
- `D_scale` = `1` - Damping scale factor

## 内容级读取结果

- 模型树证据数量: 20
- 理论关键词: 电磁
- geometry: b1 / BezierPolygon
- mesh: mesh1

### 文件内容证据

- `electromagnetic_plunger.java`: 参数=18, 几何=1, 物理场=0, 研究=0, 结果=0
- `electromagnetic_plunger.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `electromagnetic_plunger.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `electromagnetic_plunger_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: electromagnetic_plunger 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF keywords: 电磁
- `k` -> Spring constant; MATLAB: `model.param.set('k', value, description)`
- `dt` -> Pulse length; MATLAB: `model.param.set('dt', value, description)`
- `tau` -> Rise/Decay time; MATLAB: `model.param.set('tau', value, description)`
...
- `Dia_wire` -> Diameter of a wire; MATLAB: `model.param.set('Dia_wire', value, description)`
- `I0_wire` -> Current through the wire; MATLAB: `model.param.set('I0_wire', value, description)`
...
- 已识别几何证据：b1 / BezierPolygon
- 已识别参数：k、dt、tau、R_p、Len_p、z0_p、Tip_R_p、Cut_angle、Nturn、Dia_wire、I0_wire、Guider_width
...
- Sweep `k` around the learned value `40[N/m]` and export target outputs to CSV.
- Sweep `dt` around the learned value `0.4[s]` and export target outputs to CSV.
- Sweep `tau` around the learned value `0.01[s]` and export target outputs to CSV.
...
- Sweep `z0_p` around the learned value `24[mm]` and export target outputs to CSV.
...
- Summary: 已学习 electromagnetic_plunger 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=18；内容证据=20 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=1，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：electromagnetic_plunger.m, electromagnetic_plunger.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 electromagnetic_plunger，训练阶段为 modeling_logic_learning_ready。
...
- `electromagnetic_plunger.mph` (comsol_mph, 69164904 bytes)
- `electromagnetic_plunger.m` (matlab_livelink, 73894 bytes)
- `electromagnetic_plunger.java` (comsol_java, 72412 bytes)
- `electromagnetic_plunger_summary.json` (json, 1622 bytes)
