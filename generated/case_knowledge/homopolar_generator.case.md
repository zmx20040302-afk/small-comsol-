...
- Case directory: `D:\桌面\codex\案例下载\电气\单极发电机`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:21:41.329584+00:00`
...
- `Bz0` = `1[Wb/m^2]` - Constant magnetic flux density
- `RPM` = `1200[rpm]` - Disc speed

## 内容级读取结果

- 模型树证据数量: 38
- 理论关键词: 电磁, 流体
- geometry: cyl1 / Cylinder, cyl2 / Cylinder, cyl3 / Cylinder, cyl4 / Cylinder, wp1 / WorkPlane, r1 / Rectangle, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / pos, geom1 / cyl2 / r, geom1 / cyl2 / h, geom1 / cyl2 / pos
...
- `homopolar_generator.java`: 参数=2, 几何=19, 物理场=0, 研究=0, 结果=0
- `models.acdc.homopolar_generator.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `homopolar_generator.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `homopolar_generator.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `homopolar_generator_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.homopolar_generator.pdf`: models.acdc.homopolar_generator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion

## PDF Summary and Physics Judgement

- Simple summary: homopolar_generator 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Magnetic Fields，并采用 Stationary 研究。
...
- PDF: 2 In the Select Physics tree, select AC/DC > Electromagnetic Fields > Vector Formulations >
- PDF: 4 Click Study.
...
- PDF keywords: 电磁, 流体, models.acdc.homopolar_generator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion
- `Bz0` -> Constant magnetic flux density; MATLAB: `model.param.set('Bz0', value, description)`
- `RPM` -> Disc speed; MATLAB: `model.param.set('RPM', value, description)`
...
- 已识别几何证据：cyl1 / Cylinder；cyl2 / Cylinder；cyl3 / Cylinder；cyl4 / Cylinder；wp1 / WorkPlane；r1 / Rectangle
...
- Sweep `Bz0` around the learned value `1[Wb/m^2]` and export target outputs to CSV.
- Sweep `RPM` around the learned value `1200[rpm]` and export target outputs to CSV.
...
- Summary: 已学习 homopolar_generator 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=38 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 38 条；几何=19，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：homopolar_generator.m, homopolar_generator.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 homopolar_generator，训练阶段为 modeling_logic_learning_ready。
...
- `homopolar_generator.mph` (comsol_mph, 17461484 bytes)
- `homopolar_generator.m` (matlab_livelink, 27300 bytes)
- `homopolar_generator.java` (comsol_java, 23727 bytes)
- `homopolar_generator_summary.json` (json, 886 bytes)
- `models.acdc.homopolar_generator.pdf` (pdf_document, 1849485 bytes)
