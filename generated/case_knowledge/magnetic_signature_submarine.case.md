...
- `gB` = `-5e-5[T]` - Geomagnetic field
- `dw` = `100[m]` - Water domain width
- `dl` = `120[m]` - Water domain length

## 内容级读取结果

- 模型树证据数量: 22
- 理论关键词: 电磁, 传热, 声学, 多孔介质/裂隙
- geometry: blk1 / Block, imp1 / Import, geom1 / blk1 / size, geom1 / blk1 / base, geom1 / blk1 / pos, geom1 / imp1 / type
- physics: mfnc / MagnetostaticsNoCurrents, Electric Currents, Electrostatics, Magnetic Fields
- materials: mat1, mat2
- mesh: mesh1
- studies: Stationary
- boundary_conditions: zsp1 / 3, blk1 / size, blk1 / base, blk1 / pos, imp1 / type

### 文件内容证据

- `magnetic_signature_submarine.java`: 参数=3, 几何=6, 物理场=1, 研究=0, 结果=0
- `models.acdc.magnetic_signature_submarine.pdf`: 参数=0, 几何=0, 物理场=3, 研究=1, 结果=0
- `magnetic_signature_submarine.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetic_signature_submarine.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetic_signature_submarine_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: magnetic_signature_submarine 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- 脚本/文本识别到的物理场: mfnc / MagnetostaticsNoCurrents, Electric Currents, Electrostatics, Magnetic Fields
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 传热, 声学, 多孔介质/裂隙
...
- `gB` -> Geomagnetic field; MATLAB: `model.param.set('gB', value, description)`
- `dw` -> Water domain width; MATLAB: `model.param.set('dw', value, description)`
- `dl` -> Water domain length; MATLAB: `model.param.set('dl', value, description)`
...
- 已识别几何证据：blk1 / Block；imp1 / Import；geom1 / blk1 / size；geom1 / blk1 / base；geom1 / blk1 / pos；geom1 / imp1 / type
- 已识别物理场证据：mfnc / MagnetostaticsNoCurrents；Electric Currents；Electrostatics；Magnetic Fields
- 已识别参数：gB、dw、dl
...
- Transfer recognized physics interfaces to similar requirements: mfnc / MagnetostaticsNoCurrents, Electric Currents, Electrostatics, Magnetic Fields.
...
- Sweep `gB` around the learned value `-5e-5[T]` and export target outputs to CSV.
- Sweep `dw` around the learned value `100[m]` and export target outputs to CSV.
- Sweep `dl` around the learned value `120[m]` and export target outputs to CSV.
...
- Summary: 已学习 magnetic_signature_submarine 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=22 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 22 条；几何=6，物理场=4，材料=2，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：magnetic_signature_submarine.m, magnetic_signature_submarine.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 magnetic_signature_submarine，训练阶段为 modeling_logic_learning_ready。
...
- `magnetic_signature_submarine.mph` (comsol_mph, 39562528 bytes)
- `magnetic_signature_submarine.m` (matlab_livelink, 19240 bytes)
- `magnetic_signature_submarine.java` (comsol_java, 14527 bytes)
- `magnetic_signature_submarine_summary.json` (json, 863 bytes)
