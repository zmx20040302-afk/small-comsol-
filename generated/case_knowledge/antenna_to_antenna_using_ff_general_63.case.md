...
- `antenna_to_antenna_using_ff_general_63.java`: 参数=20, 几何=2, 物理场=0, 研究=0, 结果=0
- `antenna_to_antenna_using_ff_general_63.m`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `rf_to_beam_envelopes.pptx`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `half_wavelength_dipole_antenna.pptx`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `antenna_to_antenna_using_ff_general_63.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `antenna_to_antenna_using_ff_general_63_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: antenna_to_antenna_using_ff_general_63 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- Summary: 已学习 antenna_to_antenna_using_ff_general_63 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=31；内容证据=38 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 38 条；几何=2，物理场=1，材料=0，网格=2，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：antenna_to_antenna_using_ff_general_63.m, antenna_to_antenna_using_ff_general_63.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 antenna_to_antenna_using_ff_general_63，训练阶段为 modeling_logic_learning_ready。
...
- `antenna_to_antenna_using_ff_general_63.mph` (comsol_mph, 474405425 bytes)
- `antenna_to_antenna_using_ff_general_63.m` (matlab_livelink, 40224 bytes)
- `antenna_to_antenna_using_ff_general_63.java` (comsol_java, 42309 bytes)
- `antenna_to_antenna_using_ff_general_63_summary.json` (json, 2421 bytes)
