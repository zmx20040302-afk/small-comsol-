...
- Case directory: `D:\桌面\codex\案例下载\电气\飞机上电线的雷电感应电压`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T08:27:21.174763+00:00`
...
- 模型树证据数量: 34
- 理论关键词: 电磁, 多孔介质/裂隙
- geometry: imp1 / Import, sel1 / ExplicitSelection, difsel1 / DifferenceSelection, wp1 / WorkPlane, r1 / Rectangle, geom1 / imp1 / type, geom1 / imp1 / selresult, geom1 / imp1 / selresultshow, geom1 / sel1 / selshow, geom1 / difsel1 / entitydim, geom1 / difsel1 / add, geom1 / difsel1 / subtract
- physics: Electric Currents, Electromagnetic Waves
- mesh: mesh1
- studies: Time Dependent
- boundary_conditions: imp1 / type, imp1 / selresult, imp1 / selresultshow, sel1 / selshow, difsel1 / entitydim, difsel1 / add, difsel1 / subtract, wp1 / quickplane, wp1 / quicky, wp1 / unite, wp1 / selresult, wp1 / selresultshow

### 文件内容证据

- `lightning_induced_voltage_airplane.java`: 参数=0, 几何=17, 物理场=0, 研究=0, 结果=0
- `models.rf.lightning_induced_voltage_airplane.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `lightning_induced_voltage_airplane.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `lightning_induced_voltage_airplane_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `lightning_induced_voltage_airplane.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.rf.lightning_induced_voltage_airplane.pdf`: models.rf.lightning_induced_voltage_airplane.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；systems. Both of these factors have resulted in increased emphasis on lightning protection.

## PDF Summary and Physics Judgement

- Simple summary: lightning_induced_voltage_airplane 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electric Currents，并采用 Time Dependent 研究。
...
- PDF: systems. Both of these factors have resulted in increased emphasis on lightning protection.
- PDF: accurate results as a realistic lightning strike can be modeled numerically.
- PDF: Lightning current is a highly nonlinear phenomenon that requires a transient 3D study.
- 脚本/文本识别到的物理场: Electric Currents, Electromagnetic Waves
...
- PDF keywords: 电磁, 多孔介质/裂隙, models.rf.lightning_induced_voltage_airplane.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；systems. Both of these factors have resulted in increased emphasis on lightning protection.
...
- 已识别几何证据：imp1 / Import；sel1 / ExplicitSelection；difsel1 / DifferenceSelection；wp1 / WorkPlane；r1 / Rectangle；geom1 / imp1 / type
- 已识别物理场证据：Electric Currents；Electromagnetic Waves
...
- Transfer recognized physics interfaces to similar requirements: Electric Currents, Electromagnetic Waves.
...
- Summary: 已学习 lightning_induced_voltage_airplane 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=34 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 34 条；几何=17，物理场=2，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：lightning_induced_voltage_airplane.m, lightning_induced_voltage_airplane.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 lightning_induced_voltage_airplane，训练阶段为 modeling_logic_learning_ready。
...
- `lightning_induced_voltage_airplane.mph` (comsol_mph, 2397404465 bytes)
- `lightning_induced_voltage_airplane.m` (matlab_livelink, 32145 bytes)
- `lightning_induced_voltage_airplane.java` (comsol_java, 31260 bytes)
- `lightning_induced_voltage_airplane_summary.json` (json, 922 bytes)
- `models.rf.lightning_induced_voltage_airplane.pdf` (pdf_document, 1648440 bytes)
