...
- `newtonian_telescope_geom_sequence.m`: 参数=16, 几何=1, 物理场=0, 研究=0, 结果=0
- `newtonian_telescope_geom_sequence.java`: 参数=15, 几何=1, 物理场=0, 研究=0, 结果=0
- `models.roptics.newtonian_telescope_structural_analysis.pdf`: 参数=0, 几何=0, 物理场=3, 研究=2, 结果=0
- `newtonian_telescope_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `newtonian_telescope_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: newtonian_telescope_geom_sequence 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- `d_pupil` -> Entrance pupil diameter; MATLAB: `model.param.set('d_pupil', value, description)`
- `f` -> Primary mirror focal length; MATLAB: `model.param.set('f', value, description)`
- `k` -> Primary mirror conic constant; MATLAB: `model.param.set('k', value, description)`
- `F` -> Primary mirror focal ratio; MATLAB: `model.param.set('F', value, description)`
- `f_image` -> Image plane position (relative to optical axis); MATLAB: `model.param.set('f_image', value, description)`
- `d_sec` -> Secondary mirror diameter (projected); MATLAB: `model.param.set('d_sec', value, description)`
- `delta_sec` -> Secondary mirror offset (relative to optical axis); MATLAB: `model.param.set('delta_sec', value, description)`
- `d_image` -> Image plane diameter; MATLAB: `model.param.set('d_image', value, description)`
- `d_clear` -> Primary mirror clear diameter (use full mirror surface); MATLAB: `model.param.set('d_clear', value, description)`
...
- 已识别参数：nix、niy、niz、d_pupil、f、k、F、f_image、d_sec、delta_sec、d_image、d_clear
...
- Sweep `d_pupil` around the learned value `250[mm]` and export target outputs to CSV.
- Sweep `f` around the learned value `1000[mm]` and export target outputs to CSV.
- Sweep `k` around the learned value `-1.0` and export target outputs to CSV.
- Sweep `F` around the learned value `f/d_pupil` and export target outputs to CSV.
- Sweep `f_image` around the learned value `200[mm]` and export target outputs to CSV.
...
- Summary: 已学习 newtonian_telescope_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=16；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=1，物理场=3，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：newtonian_telescope_geom_sequence.m, newtonian_telescope_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 newtonian_telescope_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `newtonian_telescope_geom_sequence.mph` (comsol_mph, 2524032 bytes)
- `newtonian_telescope_geom_sequence.m` (matlab_livelink, 11594 bytes)
- `newtonian_telescope_geom_sequence.java` (comsol_java, 11811 bytes)
- `newtonian_telescope_geom_sequence_summary.json` (json, 1058 bytes)
