...
- Case directory: `D:\桌面\codex\案例下载\电气\微波腔等离子体反应器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T15:32:07.524643+00:00`
...
- `Pin` = `500[W]` - Input power
- `p0` = `25[kPa]` - Pressure
- `Qs` = `400` - Mass flow
- `f0` = `2.45[GHz]` - Excitation frequency
- `R0` = `11.4[cm]` - Chamber radius
- `H0` = `38.7[cm]` - Chamber height
- `Slw` = `1.[cm]` - Substrate lift width
- `Slh` = `12.2[cm]` - Substrate lift height
- `Shw` = `2.5[cm]` - Substrate holder width
- `Shh` = `2[cm]` - Substrate holder height
- `Bhw` = `R0-Br` - Bell jar holder width
- `Bhh` = `3[cm]` - Bell jar holder height
- `Br` = `7.3[cm]` - Bell jar radius
- `Bz` = `12.8[cm]` - Bell jar height
- `Bth` = `0.7[cm]` - Bell jar thickness
- `Cxw` = `3[cm]` - External coaxial width
- `Cxh` = `15[cm]` - External coaxial height
- `Th` = `4[cm]` - Transition height
- `Tw` = `Th` - Transition width
- `Cxiw` = `0.5[cm]` - Inner coax width
- `Cxos` = `0.5[cm]` - Inner coax offset
- `Dw` = `6[cm]` - Diamond side
- `Dcut` = `1.2[cm]` - Diamond Cut
- `dielh` = `6.5[cm]` - Dielectric height
- `dielw` = `1.7[cm]` - Dielectric width

## 内容级读取结果

- 模型树证据数量: 48
- 理论关键词: 流体, 电磁, 传热, 结构力学
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, r4 / Rectangle, geom1 / r1 / size, geom1 / r2 / size, geom1 / r3 / size, geom1 / r3 / pos, geom1 / r4 / size, geom1 / r4 / pos
- physics: Heat Transfer, Laminar Flow, Electrostatics, Electromagnetic Waves
- mesh: mesh1
- studies: Stationary, Frequency Domain
- boundary_conditions: r1 / size, r2 / size, r3 / size, r3 / pos, r4 / size, r4 / pos

### 文件内容证据

- `microwave_cavity_plasma.java`: 参数=20, 几何=10, 物理场=0, 研究=0, 结果=0
- `microwave_cavity_plasma.m`: 参数=10, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.plasma.microwave_cavity_plasma.pdf`: 参数=0, 几何=0, 物理场=4, 研究=2, 结果=0
- `microwave_cavity_plasma.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `microwave_cavity_plasma_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.microwave_cavity_plasma.pdf`: models.plasma.microwave_cavity_plasma.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Electrostatics，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；plasma approximation that is set in the Plasma Conductivity Coupling multiphysics feature:

## PDF Summary and Physics Judgement

- Simple summary: microwave_cavity_plasma 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- PDF: plasma approximation that is set in the Plasma Conductivity Coupling multiphysics feature:
- PDF: responsible to heat the electrons is set in the Electron Heat Source multiphysics feature.
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: Heat Transfer, Laminar Flow, Electrostatics, Electromagnetic Waves
...
- PDF keywords: 流体, 电磁, 传热, 结构力学, models.plasma.microwave_cavity_plasma.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Electrostatics，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；plasma approximation that is set in the Plasma Conductivity Coupling multiphysics feature:
- `Pin` -> Input power; MATLAB: `model.param.set('Pin', value, description)`
- `p0` -> Pressure; MATLAB: `model.param.set('p0', value, description)`
- `Qs` -> Mass flow; MATLAB: `model.param.set('Qs', value, description)`
- `f0` -> Excitation frequency; MATLAB: `model.param.set('f0', value, description)`
- `R0` -> Chamber radius; MATLAB: `model.param.set('R0', value, description)`
- `H0` -> Chamber height; MATLAB: `model.param.set('H0', value, description)`
- `Slw` -> Substrate lift width; MATLAB: `model.param.set('Slw', value, description)`
- `Slh` -> Substrate lift height; MATLAB: `model.param.set('Slh', value, description)`
- `Shw` -> Substrate holder width; MATLAB: `model.param.set('Shw', value, description)`
- `Shh` -> Substrate holder height; MATLAB: `model.param.set('Shh', value, description)`
- `Bhw` -> Bell jar holder width; MATLAB: `model.param.set('Bhw', value, description)`
- `Bhh` -> Bell jar holder height; MATLAB: `model.param.set('Bhh', value, description)`
...
- 已识别物理场证据：Heat Transfer；Laminar Flow；Electrostatics；Electromagnetic Waves
- 已识别参数：Pin、p0、Qs、f0、R0、H0、Slw、Slh、Shw、Shh、Bhw、Bhh
...
- Transfer recognized physics interfaces to similar requirements: Heat Transfer, Laminar Flow, Electrostatics, Electromagnetic Waves.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 电磁, 传热, 结构力学?
...
- Sweep `Pin` around the learned value `500[W]` and export target outputs to CSV.
- Sweep `p0` around the learned value `25[kPa]` and export target outputs to CSV.
- Sweep `Qs` around the learned value `400` and export target outputs to CSV.
- Sweep `f0` around the learned value `2.45[GHz]` and export target outputs to CSV.
- Sweep `R0` around the learned value `11.4[cm]` and export target outputs to CSV.
- Sweep `H0` around the learned value `38.7[cm]` and export target outputs to CSV.
- Sweep `Slw` around the learned value `1.[cm]` and export target outputs to CSV.
- Sweep `Slh` around the learned value `12.2[cm]` and export target outputs to CSV.
...
- Summary: 已学习 microwave_cavity_plasma 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=25；内容证据=48 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 48 条；几何=10，物理场=4，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：microwave_cavity_plasma.m, microwave_cavity_plasma.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 microwave_cavity_plasma，训练阶段为 modeling_logic_learning_ready。
...
- `microwave_cavity_plasma.mph` (comsol_mph, 81672400 bytes)
- `microwave_cavity_plasma.m` (matlab_livelink, 154856 bytes)
- `microwave_cavity_plasma.java` (comsol_java, 158970 bytes)
- `microwave_cavity_plasma_summary.json` (json, 2976 bytes)
- `models.plasma.microwave_cavity_plasma.pdf` (pdf_document, 1293338 bytes)
