# slot_waveguide_3d_propagation

- Case directory: `D:\桌面\codex\案例下载\电气\肋和槽构型的硅波导中的波传播三维模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:26:25.581246+00:00`
...
- `w_slab` = `180[nm]` - Width of slab
- `w_slot` = `50[nm]` - Width of slot
- `h_slot` = `300[nm]` - Height of slot
- `lda0` = `1.55[um]` - Operating wavelength
- `f0` = `c_const/lda0` - Operating freqeuncy
- `n_slab` = `3.48` - Refractive index of slab
- `n_slot` = `1.44` - Refractive index of slot
...
- 模型树证据数量: 17
- 理论关键词: 未自动识别
- geometry: wp1 / WorkPlane, r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, geom1 / wp1 / quickplane, geom1 / wp1 / unite
- mesh: mesh1
- boundary_conditions: wp1 / quickplane, wp1 / unite

### 文件内容证据

- `slot_waveguide_3d_propagation.java`: 参数=8, 几何=6, 物理场=0, 研究=0, 结果=0
- `slot_waveguide_3d_propagation.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `slot_waveguide_3d_propagation.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `slot_waveguide_3d_propagation_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: slot_waveguide_3d_propagation 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `w_slab` -> Width of slab; MATLAB: `model.param.set('w_slab', value, description)`
- `w_slot` -> Width of slot; MATLAB: `model.param.set('w_slot', value, description)`
- `h_slot` -> Height of slot; MATLAB: `model.param.set('h_slot', value, description)`
- `lda0` -> Operating wavelength; MATLAB: `model.param.set('lda0', value, description)`
- `f0` -> Operating freqeuncy; MATLAB: `model.param.set('f0', value, description)`
- `n_slab` -> Refractive index of slab; MATLAB: `model.param.set('n_slab', value, description)`
- `n_slot` -> Refractive index of slot; MATLAB: `model.param.set('n_slot', value, description)`
- `n_clad` -> Refractive index of cladding; MATLAB: `model.param.set('n_clad', value, description)`
...
- 已识别几何证据：wp1 / WorkPlane；r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；geom1 / wp1 / quickplane；geom1 / wp1 / unite
- 已识别参数：w_slab、w_slot、h_slot、lda0、f0、n_slab、n_slot、n_clad
...
- Sweep `w_slab` around the learned value `180[nm]` and export target outputs to CSV.
- Sweep `w_slot` around the learned value `50[nm]` and export target outputs to CSV.
- Sweep `h_slot` around the learned value `300[nm]` and export target outputs to CSV.
...
- Sweep `n_slab` around the learned value `3.48` and export target outputs to CSV.
- Sweep `n_slot` around the learned value `1.44` and export target outputs to CSV.
- Sweep `n_clad` around the learned value `1.44` and export target outputs to CSV.
...
- Summary: 已学习 slot_waveguide_3d_propagation 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=17 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 17 条；几何=6，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：slot_waveguide_3d_propagation.m, slot_waveguide_3d_propagation.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 slot_waveguide_3d_propagation，训练阶段为 modeling_logic_learning_ready。
...
- `slot_waveguide_3d_propagation.mph` (comsol_mph, 36892669 bytes)
- `slot_waveguide_3d_propagation.m` (matlab_livelink, 24714 bytes)
- `slot_waveguide_3d_propagation.java` (comsol_java, 20312 bytes)
- `slot_waveguide_3d_propagation_summary.json` (json, 1153 bytes)
