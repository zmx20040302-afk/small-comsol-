# vaporization_during_electrosurgery

- Case directory: `D:\桌面\codex\案例下载\电气\Vaporization During Electrosurgery`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:21:49.723227+00:00`
...
- `por` = `Vf0_l+Vf0_g` - Porosity
- `perm` = `4.57e-11[m^2]` - Permeability
- `rhos` = `1520[kg/m^3]` - Density of solid matrix
- `cps` = `3523[J/(kg*K)]` - Heat capacity of solid matrix
- `ks` = `.28[W/(m*K)]` - Thermal conuductivity of solid matrix
- `Kevap` = `10[1/s]` - Evaporation rate constant
- `Kcond` = `100[1/s]` - Condensation rate constant
- `Kvap` = `40000[1/s]` - Vaporization rate constant
- `R_cont` = `1e-6[K*m^2/W]` - Thermal contact resistance
- `Vf0_l` = `.74` - Initial blood volume fraction
- `Vf0_g` = `.1*Vf0_l` - Initial air volume fraction
- `sl0` = `Vf0_l/por` - Initial blood saturation
- `Jden` = `200000[A/m^2]` - Applied current density

## 内容级读取结果

- 模型树证据数量: 32
- 理论关键词: 传热, 电磁, 多孔介质/裂隙, 流体
- geometry: r1 / Rectangle, r2 / Rectangle, fil1 / Fillet, dif1 / Difference, geom1 / r1 / size, geom1 / r2 / size, geom1 / r2 / pos, geom1 / fil1 / radius, geom1 / dif1 / keepsubtract, geom1 / dif1 / intbnd
- physics: Heat Transfer, Electric Currents
- mesh: mesh1
- boundary_conditions: r1 / size, r2 / size, r2 / pos, fil1 / radius, dif1 / keepsubtract, dif1 / intbnd

### 文件内容证据

- `vaporization_during_electrosurgery.m`: 参数=13, 几何=10, 物理场=0, 研究=0, 结果=0
- `vaporization_during_electrosurgery.java`: 参数=13, 几何=10, 物理场=0, 研究=0, 结果=0
- `vaporization_during_electrosurgery.pptx`: 参数=0, 几何=0, 物理场=2, 研究=0, 结果=0
- `vaporization_during_electrosurgery.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `vaporization_during_electrosurgery_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: vaporization_during_electrosurgery 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF keywords: 传热, 电磁, 多孔介质/裂隙, 流体
- `por` -> Porosity; MATLAB: `model.param.set('por', value, description)`
- `perm` -> Permeability; MATLAB: `model.param.set('perm', value, description)`
- `rhos` -> Density of solid matrix; MATLAB: `model.param.set('rhos', value, description)`
- `cps` -> Heat capacity of solid matrix; MATLAB: `model.param.set('cps', value, description)`
- `ks` -> Thermal conuductivity of solid matrix; MATLAB: `model.param.set('ks', value, description)`
- `Kevap` -> Evaporation rate constant; MATLAB: `model.param.set('Kevap', value, description)`
- `Kcond` -> Condensation rate constant; MATLAB: `model.param.set('Kcond', value, description)`
- `Kvap` -> Vaporization rate constant; MATLAB: `model.param.set('Kvap', value, description)`
- `R_cont` -> Thermal contact resistance; MATLAB: `model.param.set('R_cont', value, description)`
- `Vf0_l` -> Initial blood volume fraction; MATLAB: `model.param.set('Vf0_l', value, description)`
- `Vf0_g` -> Initial air volume fraction; MATLAB: `model.param.set('Vf0_g', value, description)`
- `sl0` -> Initial blood saturation; MATLAB: `model.param.set('sl0', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；fil1 / Fillet；dif1 / Difference；geom1 / r1 / size；geom1 / r2 / size
- 已识别物理场证据：Heat Transfer；Electric Currents
- 已识别参数：por、perm、rhos、cps、ks、Kevap、Kcond、Kvap、R_cont、Vf0_l、Vf0_g、sl0
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 电磁, 多孔介质/裂隙, 流体?
...
- Sweep `por` around the learned value `Vf0_l+Vf0_g` and export target outputs to CSV.
- Sweep `perm` around the learned value `4.57e-11[m^2]` and export target outputs to CSV.
- Sweep `rhos` around the learned value `1520[kg/m^3]` and export target outputs to CSV.
- Sweep `cps` around the learned value `3523[J/(kg*K)]` and export target outputs to CSV.
- Sweep `ks` around the learned value `.28[W/(m*K)]` and export target outputs to CSV.
- Sweep `Kevap` around the learned value `10[1/s]` and export target outputs to CSV.
- Sweep `Kcond` around the learned value `100[1/s]` and export target outputs to CSV.
- Sweep `Kvap` around the learned value `40000[1/s]` and export target outputs to CSV.
...
- Summary: 已学习 vaporization_during_electrosurgery 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=13；内容证据=32 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 32 条；几何=10，物理场=2，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：vaporization_during_electrosurgery.m, vaporization_during_electrosurgery.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 vaporization_during_electrosurgery，训练阶段为 modeling_logic_learning_ready。
...
- `vaporization_during_electrosurgery.mph` (comsol_mph, 399079580 bytes)
- `vaporization_during_electrosurgery.m` (matlab_livelink, 128882 bytes)
- `vaporization_during_electrosurgery.java` (comsol_java, 142887 bytes)
- `vaporization_during_electrosurgery_summary.json` (json, 4469 bytes)
- `vaporization_during_electrosurgery.pptx` (presentation_document, 17930384 bytes)
