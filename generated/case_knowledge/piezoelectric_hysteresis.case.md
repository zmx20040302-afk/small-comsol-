...
- Created at: `2026-08-22T07:05:30.522393+00:00`
...
- physics: solid / SolidMechanics, Solid Mechanics, Electrostatics
- materials: mat1
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: blk1 / size

### 文件内容证据

- `piezoelectric_hysteresis.java`: 参数=14, 几何=2, 物理场=1, 研究=0, 结果=0
...
- `piezoelectric_hysteresis.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoelectric_hysteresis.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoelectric_hysteresis_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: piezoelectric_hysteresis 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- 脚本/文本识别到的物理场: solid / SolidMechanics, Solid Mechanics, Electrostatics
...
- `W` -> Actuator width; MATLAB: `model.param.set('W', value, description)`
...
- 已识别物理场证据：solid / SolidMechanics；Solid Mechanics；Electrostatics
...
- Transfer recognized physics interfaces to similar requirements: solid / SolidMechanics, Solid Mechanics, Electrostatics.
...
- Summary: 已学习 piezoelectric_hysteresis 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=14；内容证据=24 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 24 条；几何=2，物理场=3，材料=1，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：piezoelectric_hysteresis.m, piezoelectric_hysteresis.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 piezoelectric_hysteresis，训练阶段为 modeling_logic_learning_ready。
...
- `piezoelectric_hysteresis.mph` (comsol_mph, 118328416 bytes)
- `piezoelectric_hysteresis.m` (matlab_livelink, 46752 bytes)
- `piezoelectric_hysteresis.java` (comsol_java, 43583 bytes)
- `piezoelectric_hysteresis_summary.json` (json, 1466 bytes)
