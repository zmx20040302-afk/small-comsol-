...
- Case directory: `D:\桌面\codex\案例下载\电气\浮栅 EEPROM 器件的编程过程`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:05:59.616313+00:00`
...
- `Qgate` = `0[C]` - Gate charge
- `Vcontrol` = `0.1[V]` - Control voltage
- `Vd` = `10[mV]` - Drain voltage
- `Vmax` = `23.7[V]` - Transient voltage maximum
- `A_fn` = `1.23e-6[A/V^2]` - A tunneling coefficient
- `B_fn` = `237[MV/cm]` - B tunneling coefficient

## 内容级读取结果

- 模型树证据数量: 28
- 理论关键词: 电磁, 传热
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, uni1 / Union, r4 / Rectangle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r2 / pos, geom1 / r3 / size, geom1 / r3 / pos, geom1 / uni1 / intbnd
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: r1 / size, r1 / pos, r2 / size, r2 / pos, r3 / size, r3 / pos, uni1 / intbnd

### 文件内容证据

- `eeprom.java`: 参数=6, 几何=12, 物理场=0, 研究=0, 结果=0
- `models.semicond.eeprom.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `eeprom.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `eeprom.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `eeprom_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.eeprom.pdf`: models.semicond.eeprom.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Erasable Programmable Read-Only Memory (EEPROM) device. A stationary study

## PDF Summary and Physics Judgement

- Simple summary: eeprom 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: Erasable Programmable Read-Only Memory (EEPROM) device. A stationary study
- PDF: Figure 1: Model geometry showing key components of the EEPROM. The tunnel barrier is
- PDF: EEPROM devices). The geometry of the floating gate is such that the electric field is
...
- PDF keywords: 电磁, 传热, models.semicond.eeprom.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Erasable Programmable Read-Only Memory (EEPROM) device. A stationary study
- `Qgate` -> Gate charge; MATLAB: `model.param.set('Qgate', value, description)`
- `Vcontrol` -> Control voltage; MATLAB: `model.param.set('Vcontrol', value, description)`
- `Vd` -> Drain voltage; MATLAB: `model.param.set('Vd', value, description)`
- `Vmax` -> Transient voltage maximum; MATLAB: `model.param.set('Vmax', value, description)`
- `A_fn` -> A tunneling coefficient; MATLAB: `model.param.set('A_fn', value, description)`
- `B_fn` -> B tunneling coefficient; MATLAB: `model.param.set('B_fn', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；uni1 / Union；r4 / Rectangle；geom1 / r1 / size
- 已识别参数：Qgate、Vcontrol、Vd、Vmax、A_fn、B_fn
...
- Sweep `Qgate` around the learned value `0[C]` and export target outputs to CSV.
- Sweep `Vcontrol` around the learned value `0.1[V]` and export target outputs to CSV.
- Sweep `Vd` around the learned value `10[mV]` and export target outputs to CSV.
- Sweep `Vmax` around the learned value `23.7[V]` and export target outputs to CSV.
- Sweep `A_fn` around the learned value `1.23e-6[A/V^2]` and export target outputs to CSV.
- Sweep `B_fn` around the learned value `237[MV/cm]` and export target outputs to CSV.
...
- Summary: 已学习 eeprom 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=12，物理场=0，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：eeprom.m, eeprom.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 eeprom，训练阶段为 modeling_logic_learning_ready。
...
- `eeprom.mph` (comsol_mph, 79368773 bytes)
- `eeprom.m` (matlab_livelink, 67687 bytes)
- `eeprom.java` (comsol_java, 63767 bytes)
- `eeprom_summary.json` (json, 1416 bytes)
- `models.semicond.eeprom.pdf` (pdf_document, 1237738 bytes)
