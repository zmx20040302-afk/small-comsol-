...
- geometry: blk1 / Block, wp1 / WorkPlane, r1 / Rectangle, geom1 / blk1 / size, geom1 / blk1 / pos, geom1 / blk1 / layername, geom1 / blk1 / layer, geom1 / blk1 / layerfront, geom1 / blk1 / layerback, geom1 / blk1 / layerbottom, geom1 / wp1 / quickplane, geom1 / wp1 / quicky
- physics: Solid Mechanics, Electrostatics
- mesh: mesh1
- studies: Stationary, Eigenfrequency, Frequency Domain, Parametric Sweep
- boundary_conditions: blk1 / size, blk1 / pos, blk1 / layername, blk1 / layer, blk1 / layerfront, blk1 / layerback, blk1 / layerbottom, wp1 / quickplane, wp1 / quicky, wp1 / unite

### 文件内容证据

- `biased_resonator_3d_modes.java`: 参数=1, 几何=13, 物理场=0, 研究=0, 结果=0
...
- `biased_resonator_3d_modes_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=3, 结果=0
- `biased_resonator_3d_modes.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `biased_resonator_3d_modes.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: biased_resonator_3d_modes 主要研究结构受力、变形或强度响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenfrequency 研究。
...
- 已识别几何证据：blk1 / Block；wp1 / WorkPlane；r1 / Rectangle；geom1 / blk1 / size；geom1 / blk1 / pos；geom1 / blk1 / layername
...
- 本案例已提取 1 个参数，可作为约束 JSON 和参数扫描变量。
...
- Summary: 已学习 biased_resonator_3d_modes 的 8 个文件；训练阶段=modeling_logic_learning_ready；参数数量=1；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=13，物理场=2，材料=0，网格=1，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：biased_resonator_3d_modes.m, biased_resonator_3d_modes.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 biased_resonator_3d_modes，训练阶段为 modeling_logic_learning_ready。
...
- `biased_resonator_3d_modes.mph` (comsol_mph, 331794569 bytes)
- `biased_resonator_3d_modes.m` (matlab_livelink, 70996 bytes)
- `biased_resonator_3d_modes.java` (comsol_java, 72479 bytes)
- `biased_resonator_3d_modes_summary.json` (json, 1579 bytes)
