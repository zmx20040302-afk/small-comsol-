# current_voltage_switch

- Case directory: `D:\桌面\codex\案例下载\电气\将电流-电压开关应用于模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:42:34.159779+00:00`
...
- 模型树证据数量: 12
- 理论关键词: 电磁
- geometry: blk1 / Block, geom1 / fin / repairtoltype
- physics: ec / ConductiveMedia, ev / Events, Electric Currents
- mesh: mesh1, ftet1
- boundary_conditions: term1 / 1, gnd1 / 6, fin / repairtoltype, ge1 / DependentVariableQuantity, cucns1 / sigma_mat

### 文件内容证据

- `current_voltage_switch.java`: 参数=0, 几何=2, 物理场=3, 研究=0, 结果=0
- `current_voltage_switch_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `current_voltage_switch.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `current_voltage_switch.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: current_voltage_switch 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electric Currents。
- Primary physics: `Electric Currents`
- Study type: `unknown`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Electric Currents，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型需要继续从研究步骤确认。

### Judgement Evidence

- 脚本/文本识别到的物理场: ec / ConductiveMedia, ev / Events, Electric Currents
- 理论关键词: 电磁
...
- 已识别几何证据：blk1 / Block；geom1 / fin / repairtoltype
- 已识别物理场证据：ec / ConductiveMedia；ev / Events；Electric Currents
...
- Transfer recognized physics interfaces to similar requirements: ec / ConductiveMedia, ev / Events, Electric Currents.
...
- Summary: 已学习 current_voltage_switch 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=12 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 12 条；几何=2，物理场=3，材料=0，网格=2，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：current_voltage_switch.m, current_voltage_switch.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 current_voltage_switch，训练阶段为 modeling_logic_learning_ready。
...
- `current_voltage_switch.mph` (comsol_mph, 5857227 bytes)
- `current_voltage_switch.m` (matlab_livelink, 21739 bytes)
- `current_voltage_switch.java` (comsol_java, 19175 bytes)
- `current_voltage_switch_summary.json` (json, 865 bytes)
