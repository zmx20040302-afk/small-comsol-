...
- Case directory: `D:\桌面\codex\案例下载\电气\电容耦合等离子体反应器中的离子能量分布函数`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:09:07.028784+00:00`
...
- 模型树证据数量: 26
- 理论关键词: 传热, 结构力学, 流体, 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, uni1 / Union, cha1 / Chamfer, fil1 / Fillet, geom1 / r1 / size, geom1 / r2 / size, geom1 / r2 / pos, geom1 / uni1 / intbnd, geom1 / cha1 / dist
- physics: Particle Tracing
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: r1 / size, r2 / size, r2 / pos, uni1 / intbnd, cha1 / dist

### 文件内容证据

- `ccp_ion_energy_distribution_function.java`: 参数=7, 几何=10, 物理场=0, 研究=0, 结果=0
- `models.plasma.ccp_ion_energy_distribution_function.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `ccp_ion_energy_distribution_function.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ccp_ion_energy_distribution_function.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ccp_ion_energy_distribution_function_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.ccp_ion_energy_distribution_function.pdf`: models.plasma.ccp_ion_energy_distribution_function.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Particle Tracing，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Multiphysics Plasma Module. The reactor is the Plasmalab System 100 parallel plate,

## PDF Summary and Physics Judgement

- Simple summary: ccp_ion_energy_distribution_function 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: Multiphysics Plasma Module. The reactor is the Plasmalab System 100 parallel plate,
- PDF: Two physics interfaces are used in two consecutive steps. First, the
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: Particle Tracing
...
- PDF keywords: 传热, 结构力学, 流体, 电磁, models.plasma.ccp_ion_energy_distribution_function.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Particle Tracing，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Multiphysics Plasma Module. The reactor is the Plasmalab System 100 parallel plate,
- `r0` -> 案例参数/扫描变量; MATLAB: `model.param.set('r0', value, description)`
- `h0` -> 案例参数/扫描变量; MATLAB: `model.param.set('h0', value, description)`
- `r1` -> 案例参数/扫描变量; MATLAB: `model.param.set('r1', value, description)`
- `h1` -> 案例参数/扫描变量; MATLAB: `model.param.set('h1', value, description)`
- `Prf` -> 案例参数/扫描变量; MATLAB: `model.param.set('Prf', value, description)`
- `f0` -> 案例参数/扫描变量; MATLAB: `model.param.set('f0', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；uni1 / Union；cha1 / Chamfer；fil1 / Fillet；geom1 / r1 / size
- 已识别物理场证据：Particle Tracing
- 已识别参数：r0、h0、r1、h1、Prf、f0、p0
...
- Transfer recognized physics interfaces to similar requirements: Particle Tracing.
...
- Sweep `r0` around the learned value `160[mm]` and export target outputs to CSV.
- Sweep `h0` around the learned value `4.5[cm]` and export target outputs to CSV.
- Sweep `r1` around the learned value `110[mm]` and export target outputs to CSV.
- Sweep `h1` around the learned value `40[mm]` and export target outputs to CSV.
...
- Summary: 已学习 ccp_ion_energy_distribution_function 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=26 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 26 条；几何=10，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：ccp_ion_energy_distribution_function.m, ccp_ion_energy_distribution_function.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 ccp_ion_energy_distribution_function，训练阶段为 modeling_logic_learning_ready。
...
- `ccp_ion_energy_distribution_function.mph` (comsol_mph, 77240503 bytes)
- `ccp_ion_energy_distribution_function.m` (matlab_livelink, 67064 bytes)
- `ccp_ion_energy_distribution_function.java` (comsol_java, 70645 bytes)
- `ccp_ion_energy_distribution_function_summary.json` (json, 1962 bytes)
- `models.plasma.ccp_ion_energy_distribution_function.pdf` (pdf_document, 756526 bytes)
