...
- Case directory: `D:\桌面\codex\案例下载\电气\电感耦合等离子体 (ICP) 炬`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T15:22:45.008973+00:00`
...
- physics: Heat Transfer, Laminar Flow, Magnetic Fields
- mesh: mesh1
- studies: Stationary
...
- `icp_torch.java`: 参数=20, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.plasma.icp_torch.pdf`: 参数=0, 几何=0, 物理场=3, 研究=1, 结果=0
- `icp_torch.m`: 参数=4, 几何=0, 物理场=0, 研究=0, 结果=0
- `icp_torch.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `icp_torch_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.icp_torch.pdf`: models.plasma.icp_torch.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Magnetic Fields，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Figure 1 displays the geometry of the to-be-modeled inductive plasma torch.

## PDF Summary and Physics Judgement

- Simple summary: icp_torch 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- PDF: Figure 1 displays the geometry of the to-be-modeled inductive plasma torch.
- PDF: Figure 1: Geometry of an inductively coupled plasma torch. The torch is composed of three
- PDF: Figure 2 shows the geometry of the model.
- 脚本/文本识别到的物理场: Heat Transfer, Laminar Flow, Magnetic Fields
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 流体, 电磁
...
- PDF keywords: 传热, 流体, 电磁, models.plasma.icp_torch.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Magnetic Fields，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Figure 1 displays the geometry of the to-be-modeled inductive plasma torch.
- `T0` -> Ambient temperature; MATLAB: `model.param.set('T0', value, description)`
- `Pext` -> Coil excitation power; MATLAB: `model.param.set('Pext', value, description)`
- `f0` -> Coil excitation frequency; MATLAB: `model.param.set('f0', value, description)`
- `r_3` -> Axial length: Computational domain; MATLAB: `model.param.set('r_3', value, description)`
- `L_3` -> Height: Computational domain and sheath tube; MATLAB: `model.param.set('L_3', value, description)`
- `d_1` -> Thickness: Carrier tube; MATLAB: `model.param.set('d_1', value, description)`
- `L_0` -> Height: Carrier tube and central tube; MATLAB: `model.param.set('L_0', value, description)`
- `r_1` -> Inner radius: Carrier tube; MATLAB: `model.param.set('r_1', value, description)`
- `d_2` -> Thickness: Central tube; MATLAB: `model.param.set('d_2', value, description)`
- `r_2` -> Inner radius: Central tube; MATLAB: `model.param.set('r_2', value, description)`
- `d_3` -> Thickness: Sheath tube; MATLAB: `model.param.set('d_3', value, description)`
- `r_0` -> Inner radius: Sheath tube; MATLAB: `model.param.set('r_0', value, description)`
...
- 已识别物理场证据：Heat Transfer；Laminar Flow；Magnetic Fields
- 已识别参数：T0、Pext、f0、r_3、L_3、d_1、L_0、r_1、d_2、r_2、d_3、r_0
...
- Transfer recognized physics interfaces to similar requirements: Heat Transfer, Laminar Flow, Magnetic Fields.
...
- Sweep `Pext` around the learned value `11[kW]` and export target outputs to CSV.
- Sweep `f0` around the learned value `3[MHz]` and export target outputs to CSV.
- Sweep `r_3` around the learned value `125[mm]` and export target outputs to CSV.
- Sweep `L_3` around the learned value `200[mm]` and export target outputs to CSV.
- Sweep `d_1` around the learned value `2[mm]` and export target outputs to CSV.
- Sweep `L_0` around the learned value `50[mm]` and export target outputs to CSV.
- Sweep `r_1` around the learned value `3.7[mm]` and export target outputs to CSV.
...
- Summary: 已学习 icp_torch 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=31；内容证据=39 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 39 条；几何=3，物理场=3，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：icp_torch.m, icp_torch.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 icp_torch，训练阶段为 modeling_logic_learning_ready。
...
- `icp_torch.mph` (comsol_mph, 6838190 bytes)
- `icp_torch.m` (matlab_livelink, 82672 bytes)
- `icp_torch.java` (comsol_java, 86801 bytes)
- `icp_torch_summary.json` (json, 3509 bytes)
- `models.plasma.icp_torch.pdf` (pdf_document, 734711 bytes)
