# rays_in_dispersive_prism

- Case directory: `D:\桌面\codex\案例下载\电气\光在具有频率相关折射率的介质中的传播`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:23:16.132484+00:00`
...
- 模型树证据数量: 31
- 理论关键词: 未自动识别
- geometry: tet1 / Tetrahedron, blk1 / Block, geom1 / blk1 / size, geom1 / blk1 / pos
- physics: gop / GeometricalOptics
- materials: mat1, mat2
- mesh: mesh1
- studies: std1 / rtrac / RayTracing, std1
- boundary_conditions: blk1 / size, blk1 / pos, relg1 / L0, relg1 / FDistributionFunction, relg1 / nuNval, rtrac / timestepspec, rtrac / llist, rtrac / notlistsolnum, rtrac / notsolnum, rtrac / listsolnum, rtrac / solnum, st1 / study

### 文件内容证据

- `rays_in_dispersive_prism.java`: 参数=0, 几何=4, 物理场=1, 研究=2, 结果=0
- `rays_in_dispersive_prism.m`: 参数=0, 几何=4, 物理场=1, 研究=1, 结果=0
- `rays_in_dispersive_prism.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rays_in_dispersive_prism_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `dispersive_prism.pptx`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: rays_in_dispersive_prism 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 GeometricalOptics，并采用 RayTracing 研究。
- Primary physics: `GeometricalOptics`
- Study type: `RayTracing`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 GeometricalOptics，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 RayTracing 确认。

### Judgement Evidence

- 脚本/文本识别到的物理场: gop / GeometricalOptics
- 识别到的研究类型: std1 / rtrac / RayTracing, std1
...
- 已识别几何证据：tet1 / Tetrahedron；blk1 / Block；geom1 / blk1 / size；geom1 / blk1 / pos
- 已识别物理场证据：gop / GeometricalOptics
...
- Transfer recognized physics interfaces to similar requirements: gop / GeometricalOptics.
- Reuse geometry construction patterns when new requirements share shape, import, array, sweep, or boolean operations.
- Use the learned study types as solver starting points: std1 / rtrac / RayTracing, std1.
...
- Summary: 已学习 rays_in_dispersive_prism 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=4，物理场=1，材料=2，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：rays_in_dispersive_prism.m, rays_in_dispersive_prism.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 rays_in_dispersive_prism，训练阶段为 modeling_logic_learning_ready。
...
- `rays_in_dispersive_prism.mph` (comsol_mph, 1837513 bytes)
- `rays_in_dispersive_prism.m` (matlab_livelink, 86533 bytes)
- `rays_in_dispersive_prism.java` (comsol_java, 93810 bytes)
- `rays_in_dispersive_prism_summary.json` (json, 1290 bytes)
- `dispersive_prism.pptx` (presentation_document, 9190516 bytes)
