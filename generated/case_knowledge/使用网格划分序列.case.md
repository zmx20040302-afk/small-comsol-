...
- Created at: `2026-07-06T09:37:09.967233+00:00`
...
- `meshing_sequence.mph`: 参数=0, 几何=20, 物理场=0, 研究=0, 结果=0
- `meshing_sequence_geom_sequence.m`: 参数=0, 几何=18, 物理场=0, 研究=0, 结果=0
- `meshing_sequence_geom_sequence.mph`: 参数=0, 几何=18, 物理场=0, 研究=0, 结果=0
...
- `models.mph.meshing_sequence.pdf`: models.mph.meshing_sequence.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.meshing_sequence.zh_CN.pdf`: models.mph.meshing_sequence.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 使用网格划分序列 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics。
...
- PDF keywords: 结构力学, models.mph.meshing_sequence.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.meshing_sequence.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
...
- `diameter` -> 案例参数/扫描变量; MATLAB: `model.param.set('diameter', value, description)`
- `clearance` -> 案例参数/扫描变量; MATLAB: `model.param.set('clearance', value, description)`
- `distance_ball_x` -> 案例参数/扫描变量; MATLAB: `model.param.set('distance_ball_x', value, description)`
- `distance_ball_y` -> 案例参数/扫描变量; MATLAB: `model.param.set('distance_ball_y', value, description)`
- `distance_ball_z` -> 案例参数/扫描变量; MATLAB: `model.param.set('distance_ball_z', value, description)`
- `array_displacement_x` -> 案例参数/扫描变量; MATLAB: `model.param.set('array_displacement_x', value, description)`
- `array_displacement_y` -> 案例参数/扫描变量; MATLAB: `model.param.set('array_displacement_y', value, description)`
- `array_size_x` -> 案例参数/扫描变量; MATLAB: `model.param.set('array_size_x', value, description)`
- `array_size_y` -> 案例参数/扫描变量; MATLAB: `model.param.set('array_size_y', value, description)`
...
- 已识别几何证据：geom1 / sph1 / selresult；geom1 / sph1 / selresultshow；blk1 / Block；blk2 / Block；sph1 / Sphere；dif1 / Difference
- 已识别参数：length、height、width、diameter、clearance、distance_ball_x、distance_ball_y、distance_ball_z、array_displacement_x、array_displacement_y、array_size_x、array_size_y
...
- Sweep `length` around the learned value `2.5[mm]` and export target outputs to CSV.
- Sweep `height` around the learned value `0.5[mm]` and export target outputs to CSV.
- Sweep `width` around the learned value `2[mm]` and export target outputs to CSV.
- Sweep `diameter` around the learned value `0.4[mm]` and export target outputs to CSV.
- Sweep `clearance` around the learned value `0.3[mm]` and export target outputs to CSV.
- Sweep `distance_ball_x` around the learned value `0.25[mm]` and export target outputs to CSV.
- Sweep `distance_ball_y` around the learned value `0.25[mm]` and export target outputs to CSV.
- Sweep `distance_ball_z` around the learned value `0.15[mm]` and export target outputs to CSV.
...
- `distance_ball_y`
- `distance_ball_z`
- `array_displacement_x`
- `array_displacement_y`
- `array_size_x`
- `array_size_y`
- `length_baseplate`
- `width_baseplate`
- `height_baseplate_layer`
- `height_baseplate`
