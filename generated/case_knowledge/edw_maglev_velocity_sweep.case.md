# edw_maglev_velocity_sweep

- Case directory: `D:\桌面\codex\案例下载\电气\电动轮磁悬浮二维模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:19:32.417905+00:00`
...
- `ro` = `50[mm]` - Outer radius of Halbach rotor
- `ri` = `34[mm]` - Inner radius of Halbach rotor
- `slip` = `30[m/s]` - Slip speed
- `v_x` = `0[m/s]` - Translational velocity of Halbach rotor
- `wm0` = `2*pi*(slip+v_x)/ro` - Angular rotational speed
- `Br` = `1.42 [T]` - Remnant flux density

## 内容级读取结果

- 模型树证据数量: 26
- 理论关键词: 流体
- geometry: c1 / Circle, c2 / Circle, dif1 / Difference, rot1 / Rotate, r1 / Rectangle, geom1 / c1 / r, geom1 / c1 / angle, geom1 / c2 / r, geom1 / c2 / angle, geom1 / rot1 / rot, geom1 / rot1 / pos, geom1 / r1 / size
- mesh: mesh1
- boundary_conditions: c1 / r, c1 / angle, c2 / r, c2 / angle, rot1 / rot, rot1 / pos, r1 / size

### 文件内容证据

- `edw_maglev_velocity_sweep.java`: 参数=6, 几何=12, 物理场=0, 研究=0, 结果=0
- `edw_maglev_velocity_sweep.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `edw_maglev_velocity_sweep.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `edw_maglev_velocity_sweep_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: edw_maglev_velocity_sweep 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF keywords: 流体
- `ro` -> Outer radius of Halbach rotor; MATLAB: `model.param.set('ro', value, description)`
- `ri` -> Inner radius of Halbach rotor; MATLAB: `model.param.set('ri', value, description)`
- `slip` -> Slip speed; MATLAB: `model.param.set('slip', value, description)`
- `v_x` -> Translational velocity of Halbach rotor; MATLAB: `model.param.set('v_x', value, description)`
- `wm0` -> Angular rotational speed; MATLAB: `model.param.set('wm0', value, description)`
- `Br` -> Remnant flux density; MATLAB: `model.param.set('Br', value, description)`
...
- 已识别几何证据：c1 / Circle；c2 / Circle；dif1 / Difference；rot1 / Rotate；r1 / Rectangle；geom1 / c1 / r
- 已识别参数：ro、ri、slip、v_x、wm0、Br
...
- Sweep `ri` around the learned value `34[mm]` and export target outputs to CSV.
- Sweep `slip` around the learned value `30[m/s]` and export target outputs to CSV.
- Sweep `v_x` around the learned value `0[m/s]` and export target outputs to CSV.
- Sweep `wm0` around the learned value `2*pi*(slip+v_x)/ro` and export target outputs to CSV.
- Sweep `Br` around the learned value `1.42 [T]` and export target outputs to CSV.
...
- Summary: 已学习 edw_maglev_velocity_sweep 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=26 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 26 条；几何=12，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：edw_maglev_velocity_sweep.m, edw_maglev_velocity_sweep.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 edw_maglev_velocity_sweep，训练阶段为 modeling_logic_learning_ready。
...
- `edw_maglev_velocity_sweep.mph` (comsol_mph, 6233719 bytes)
- `edw_maglev_velocity_sweep.m` (matlab_livelink, 41117 bytes)
- `edw_maglev_velocity_sweep.java` (comsol_java, 37579 bytes)
- `edw_maglev_velocity_sweep_summary.json` (json, 1031 bytes)
