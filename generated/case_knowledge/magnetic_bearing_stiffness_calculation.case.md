# magnetic_bearing_stiffness_calculation

- Case directory: `D:\桌面\codex\案例下载\电气\轴向磁轴承磁刚度三维模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:29:39.719698+00:00`
...
- geometry: wp1 / WorkPlane, r1 / Rectangle, fil1 / Fillet, wp2 / WorkPlane, geom2 / wp1 / quickplane, geom2 / wp1 / unite
- physics: Magnetic Fields
- mesh: mesh2
- boundary_conditions: wp1 / quickplane, wp1 / unite, r1 / pos, r1 / layerbottom, r1 / layertop, fil1 / radius

### 文件内容证据

- `magnetic_bearing_stiffness_calculation.java`: 参数=7, 几何=6, 物理场=0, 研究=0, 结果=0
- `magnetic_bearing_stiffness_calculation.pptx`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `magnetic_bearing_stiffness_calculation.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetic_bearing_stiffness_calculation.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetic_bearing_stiffness_calculation_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: magnetic_bearing_stiffness_calculation 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics。
...
- `X0` -> 案例参数/扫描变量; MATLAB: `model.param.set('X0', value, description)`
...
- 已识别几何证据：wp1 / WorkPlane；r1 / Rectangle；fil1 / Fillet；wp2 / WorkPlane；geom2 / wp1 / quickplane；geom2 / wp1 / unite
- 已识别物理场证据：Magnetic Fields
- 已识别参数：R1、R2、R3、R4、h0、Br、X0
...
- Sweep `X0` around the learned value `0[mm]` and export target outputs to CSV.
...
- Summary: 已学习 magnetic_bearing_stiffness_calculation 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=21 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 21 条；几何=6，物理场=1，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：magnetic_bearing_stiffness_calculation.m, magnetic_bearing_stiffness_calculation.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 magnetic_bearing_stiffness_calculation，训练阶段为 modeling_logic_learning_ready。
...
- `magnetic_bearing_stiffness_calculation.mph` (comsol_mph, 648560589 bytes)
- `magnetic_bearing_stiffness_calculation.m` (matlab_livelink, 43182 bytes)
- `magnetic_bearing_stiffness_calculation.java` (comsol_java, 41937 bytes)
- `magnetic_bearing_stiffness_calculation_summary.json` (json, 1172 bytes)
- `magnetic_bearing_stiffness_calculation.pptx` (presentation_document, 2135182 bytes)
