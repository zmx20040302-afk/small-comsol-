# charge_transport_in_polyethylene

- Case directory: `D:\桌面\codex\案例下载\电气\聚乙烯中的电荷传输动力学`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:17:36.927516+00:00`
...
- `d` = `150[um]` - Thickness
- `E` = `20[kV/mm]` - Initial electric field
- `V0` = `d*E` - Applied voltage

## 内容级读取结果

- 模型树证据数量: 14
- 理论关键词: 电磁, 传热, 流体, 优化/参数扫描
- geometry: i1 / Interval, geom1 / i1 / coord
- physics: edis / ElectricDischarge
- materials: mat1
- mesh: mesh1, edg1
...
- `charge_transport_in_polyethylene.java`: 参数=3, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.edis.charge_transport_in_polyethylene.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `charge_transport_in_polyethylene.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `charge_transport_in_polyethylene.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `charge_transport_in_polyethylene_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.charge_transport_in_polyethylene.pdf`: models.edis.charge_transport_in_polyethylene.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Time Dependent、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The study explores charge transport dynamics in polyethylene, a common solid dielectric

## PDF Summary and Physics Judgement

- Simple summary: charge_transport_in_polyethylene 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: The study explores charge transport dynamics in polyethylene, a common solid dielectric
- PDF: Results and Discussion
- PDF: 2 In the Select Physics tree, select Electric Discharge > Electric Discharge (edis).
- 脚本/文本识别到的物理场: edis / ElectricDischarge
...
- PDF keywords: 电磁, 传热, 流体, 优化/参数扫描, models.edis.charge_transport_in_polyethylene.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Time Dependent、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The study explores charge transport dynamics in polyethylene, a common solid dielectric
- `d` -> Thickness; MATLAB: `model.param.set('d', value, description)`
- `E` -> Initial electric field; MATLAB: `model.param.set('E', value, description)`
...
- 已识别物理场证据：edis / ElectricDischarge
- 已识别参数：d、E、V0
...
- Transfer recognized physics interfaces to similar requirements: edis / ElectricDischarge.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 传热, 流体, 优化/参数扫描?
...
- Sweep `d` around the learned value `150[um]` and export target outputs to CSV.
- Sweep `E` around the learned value `20[kV/mm]` and export target outputs to CSV.
- Sweep `V0` around the learned value `d*E` and export target outputs to CSV.
...
- Summary: 已学习 charge_transport_in_polyethylene 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=14 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 14 条；几何=2，物理场=1，材料=1，网格=2，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：charge_transport_in_polyethylene.m, charge_transport_in_polyethylene.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 charge_transport_in_polyethylene，训练阶段为 modeling_logic_learning_ready。
...
- `charge_transport_in_polyethylene.mph` (comsol_mph, 14831324 bytes)
- `charge_transport_in_polyethylene.m` (matlab_livelink, 24863 bytes)
- `charge_transport_in_polyethylene.java` (comsol_java, 21279 bytes)
- `charge_transport_in_polyethylene_summary.json` (json, 1011 bytes)
- `models.edis.charge_transport_in_polyethylene.pdf` (pdf_document, 564462 bytes)
