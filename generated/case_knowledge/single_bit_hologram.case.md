...
- Case directory: `D:\桌面\codex\案例下载\电气\单比特全息图`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:12:22.839700+00:00`
...
- `wl0` = `1[um]` - Wavelength
- `k0` = `2*pi/wl0` - Wave number
- `L` = `30[um]` - Air domain size
- `H` = `30[um]` - Air domain height
- `L_h` = `30[um]` - Hologram material size
- `n1` = `1.35` - Hologram material index
- `dn` = `0.01` - Modulation coefficient
- `a` = `10[um]` - Hologram half length
- `w_o` = `10[um]` - 1/e^2 intensity waist radius for object beam
- `w_r` = `10[um]` - 1/e^2 intensity waist radius for reference beam
- `threshold` = `.4` - Exposure threshold

## 内容级读取结果

- 模型树证据数量: 21
- 理论关键词: 电磁
- geometry: r1 / Rectangle, geom1 / r1 / size, geom1 / r1 / base
- physics: Electromagnetic Waves
- materials: mat1
- mesh: mesh1
- studies: Stationary, Frequency Domain
- boundary_conditions: r1 / size, r1 / base

### 文件内容证据

- `single_bit_hologram.java`: 参数=11, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.woptics.single_bit_hologram.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `single_bit_hologram.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `single_bit_hologram.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `single_bit_hologram_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.single_bit_hologram.pdf`: models.woptics.single_bit_hologram.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion

## PDF Summary and Physics Judgement

- Simple summary: single_bit_hologram 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electromagnetic Waves，并采用 Stationary 研究。
- Primary physics: `Electromagnetic Waves`
- Study type: `Stationary`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Electromagnetic Waves，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Stationary 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: Results and Discussion
- PDF: intensity pattern in the rightmost figure shows diagonal stripes that result in the striped
...
- PDF keywords: 电磁, models.woptics.single_bit_hologram.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion
- `wl0` -> Wavelength; MATLAB: `model.param.set('wl0', value, description)`
- `k0` -> Wave number; MATLAB: `model.param.set('k0', value, description)`
- `L` -> Air domain size; MATLAB: `model.param.set('L', value, description)`
- `H` -> Air domain height; MATLAB: `model.param.set('H', value, description)`
- `L_h` -> Hologram material size; MATLAB: `model.param.set('L_h', value, description)`
- `n1` -> Hologram material index; MATLAB: `model.param.set('n1', value, description)`
- `dn` -> Modulation coefficient; MATLAB: `model.param.set('dn', value, description)`
- `a` -> Hologram half length; MATLAB: `model.param.set('a', value, description)`
- `w_o` -> 1/e^2 intensity waist radius for object beam; MATLAB: `model.param.set('w_o', value, description)`
- `w_r` -> 1/e^2 intensity waist radius for reference beam; MATLAB: `model.param.set('w_r', value, description)`
- `threshold` -> Exposure threshold; MATLAB: `model.param.set('threshold', value, description)`
...
- 已识别几何证据：r1 / Rectangle；geom1 / r1 / size；geom1 / r1 / base
- 已识别物理场证据：Electromagnetic Waves
- 已识别参数：wl0、k0、L、H、L_h、n1、dn、a、w_o、w_r、threshold
...
- Sweep `wl0` around the learned value `1[um]` and export target outputs to CSV.
- Sweep `k0` around the learned value `2*pi/wl0` and export target outputs to CSV.
- Sweep `L` around the learned value `30[um]` and export target outputs to CSV.
- Sweep `H` around the learned value `30[um]` and export target outputs to CSV.
- Sweep `L_h` around the learned value `30[um]` and export target outputs to CSV.
- Sweep `n1` around the learned value `1.35` and export target outputs to CSV.
- Sweep `dn` around the learned value `0.01` and export target outputs to CSV.
- Sweep `a` around the learned value `10[um]` and export target outputs to CSV.
...
- Summary: 已学习 single_bit_hologram 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=21 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 21 条；几何=3，物理场=1，材料=1，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：single_bit_hologram.m, single_bit_hologram.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 single_bit_hologram，训练阶段为 modeling_logic_learning_ready。
...
- `single_bit_hologram.mph` (comsol_mph, 89717261 bytes)
- `single_bit_hologram.m` (matlab_livelink, 30563 bytes)
- `single_bit_hologram.java` (comsol_java, 22396 bytes)
- `single_bit_hologram_summary.json` (json, 1150 bytes)
- `models.woptics.single_bit_hologram.pdf` (pdf_document, 1795223 bytes)
