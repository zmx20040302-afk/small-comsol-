...
- Case directory: `D:\桌面\codex\案例下载\电气\菲涅尔棱体`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:05:10.100114+00:00`
...
- `delta` = `45[deg]` - Phase delay between s- and p-polarizations
- `n1` = `1` - Refractive index, air
- `n2` = `1.51` - Refractive index, glass
- `n12` = `n1/n2` - Refractive index ratio
- `theta` = `0.84855[rad]` - Angle of incidence
...
- physics: ge / GlobalEquations, gop / GeometricalOptics, Global ODEs and DAEs, Ray Optics
- materials: mat1
- mesh: mesh1
- studies: Stationary
- boundary_conditions: matd2 / 1, 6, wp1 / unite, pol1 / source

### 文件内容证据

- `fresnel_rhomb.java`: 参数=5, 几何=4, 物理场=2, 研究=0, 结果=0
- `models.roptics.fresnel_rhomb.pdf`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `fresnel_rhomb.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `fresnel_rhomb.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `fresnel_rhomb_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.roptics.fresnel_rhomb.pdf`: models.roptics.fresnel_rhomb.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Global ODEs and DAEs、Ray Optics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The model geometry is an uncoated glass prism of refractive index n1 = 1.51 surrounded

## PDF Summary and Physics Judgement

- Simple summary: fresnel_rhomb 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: The model geometry is an uncoated glass prism of refractive index n1 = 1.51 surrounded
- PDF: by air (n2 = 1). The model uses two physics interfaces and two studies.
- PDF: The first physics interface is an instance of the Global ODEs and DAEs interface. This
- 脚本/文本识别到的物理场: ge / GlobalEquations, gop / GeometricalOptics, Global ODEs and DAEs, Ray Optics
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 电磁
...
- PDF keywords: 传热, 电磁, models.roptics.fresnel_rhomb.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Global ODEs and DAEs、Ray Optics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The model geometry is an uncoated glass prism of refractive index n1 = 1.51 surrounded
- `delta` -> Phase delay between s- and p-polarizations; MATLAB: `model.param.set('delta', value, description)`
- `n1` -> Refractive index, air; MATLAB: `model.param.set('n1', value, description)`
- `n2` -> Refractive index, glass; MATLAB: `model.param.set('n2', value, description)`
- `n12` -> Refractive index ratio; MATLAB: `model.param.set('n12', value, description)`
...
- 已识别物理场证据：ge / GlobalEquations；gop / GeometricalOptics；Global ODEs and DAEs；Ray Optics
- 已识别参数：delta、n1、n2、n12、theta
...
- Transfer recognized physics interfaces to similar requirements: ge / GlobalEquations, gop / GeometricalOptics, Global ODEs and DAEs, Ray Optics.
...
- Sweep `delta` around the learned value `45[deg]` and export target outputs to CSV.
- Sweep `n1` around the learned value `1` and export target outputs to CSV.
- Sweep `n2` around the learned value `1.51` and export target outputs to CSV.
- Sweep `n12` around the learned value `n1/n2` and export target outputs to CSV.
- Sweep `theta` around the learned value `0.84855[rad]` and export target outputs to CSV.
...
- Summary: 已学习 fresnel_rhomb 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=19 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 19 条；几何=4，物理场=4，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：fresnel_rhomb.m, fresnel_rhomb.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 fresnel_rhomb，训练阶段为 modeling_logic_learning_ready。
...
- `fresnel_rhomb.mph` (comsol_mph, 3121757 bytes)
- `fresnel_rhomb.m` (matlab_livelink, 16751 bytes)
- `fresnel_rhomb.java` (comsol_java, 13140 bytes)
- `fresnel_rhomb_summary.json` (json, 1203 bytes)
- `models.roptics.fresnel_rhomb.pdf` (pdf_document, 405411 bytes)
