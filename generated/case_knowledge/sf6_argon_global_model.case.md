# sf6_argon_global_model

- Case directory: `D:\桌面\codex\案例下载\电气\SF6氩等离子体全局模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:16:29.566182+00:00`
...
- `Qfeed` = `100` - Feed mass flow in SCCM
- `Temp` = `315[K]` - Gas temperature
- `pA` = `20[mTorr]` - Gas pressure
- `Pinp` = `2000[W]` - Power input from source
- `abs_to_inp` = `0.5` - Power absorption ratio
- `Pabs` = `Pinp*abs_to_inp` - Power absorbed by plasma
- `xAr` = `0.1` - Argon mole fraction
- `width` = `19[cm]` - Chamber radius
- `height` = `17[cm]` - Chamber height
- `lambda_diff` = `((pi/height)^2+(2.405/width)^2)^-0.5` - Diffusion length for heat
...
- physics: plas / ColdPlasma, Heat Transfer
- mesh: mesh1
- studies: Stationary
...
- `sf6_argon_global_model.java`: 参数=10, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.plasma.sf6_argon_global_model.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `sf6_argon_global_model.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `sf6_argon_global_model.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `sf6_argon_global_model_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.sf6_argon_global_model.pdf`: models.plasma.sf6_argon_global_model.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；global model. The main goal is to show how to prepare a model with a mixture of different

## PDF Summary and Physics Judgement

- Simple summary: sf6_argon_global_model 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: global model. The main goal is to show how to prepare a model with a mixture of different
- PDF: to study a plasma reactor, especially when complex chemistries are involved or the
- PDF: The present study also solves the gas heat equation
- 脚本/文本识别到的物理场: plas / ColdPlasma, Heat Transfer
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 流体, 结构力学, 电磁
...
- PDF keywords: 传热, 流体, 结构力学, 电磁, models.plasma.sf6_argon_global_model.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；global model. The main goal is to show how to prepare a model with a mixture of different
- `Qfeed` -> Feed mass flow in SCCM; MATLAB: `model.param.set('Qfeed', value, description)`
- `Temp` -> Gas temperature; MATLAB: `model.param.set('Temp', value, description)`
- `pA` -> Gas pressure; MATLAB: `model.param.set('pA', value, description)`
- `Pinp` -> Power input from source; MATLAB: `model.param.set('Pinp', value, description)`
- `abs_to_inp` -> Power absorption ratio; MATLAB: `model.param.set('abs_to_inp', value, description)`
- `Pabs` -> Power absorbed by plasma; MATLAB: `model.param.set('Pabs', value, description)`
- `xAr` -> Argon mole fraction; MATLAB: `model.param.set('xAr', value, description)`
- `width` -> Chamber radius; MATLAB: `model.param.set('width', value, description)`
- `height` -> Chamber height; MATLAB: `model.param.set('height', value, description)`
- `lambda_diff` -> Diffusion length for heat; MATLAB: `model.param.set('lambda_diff', value, description)`
...
- 已识别物理场证据：plas / ColdPlasma；Heat Transfer
- 已识别参数：Qfeed、Temp、pA、Pinp、abs_to_inp、Pabs、xAr、width、height、lambda_diff
...
- Transfer recognized physics interfaces to similar requirements: plas / ColdPlasma, Heat Transfer.
...
- Sweep `Qfeed` around the learned value `100` and export target outputs to CSV.
- Sweep `Temp` around the learned value `315[K]` and export target outputs to CSV.
- Sweep `pA` around the learned value `20[mTorr]` and export target outputs to CSV.
- Sweep `Pinp` around the learned value `2000[W]` and export target outputs to CSV.
- Sweep `abs_to_inp` around the learned value `0.5` and export target outputs to CSV.
- Sweep `Pabs` around the learned value `Pinp*abs_to_inp` and export target outputs to CSV.
- Sweep `xAr` around the learned value `0.1` and export target outputs to CSV.
- Sweep `width` around the learned value `19[cm]` and export target outputs to CSV.
...
- Summary: 已学习 sf6_argon_global_model 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=17 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 17 条；几何=2，物理场=2，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：sf6_argon_global_model.m, sf6_argon_global_model.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 sf6_argon_global_model，训练阶段为 modeling_logic_learning_ready。
...
- `sf6_argon_global_model.mph` (comsol_mph, 20438450 bytes)
- `sf6_argon_global_model.m` (matlab_livelink, 165858 bytes)
- `sf6_argon_global_model.java` (comsol_java, 158548 bytes)
- `sf6_argon_global_model_summary.json` (json, 1706 bytes)
- `models.plasma.sf6_argon_global_model.pdf` (pdf_document, 308667 bytes)
