...
- Case directory: `D:\桌面\codex\案例下载\电气\基于基本单元模型计算 SAW 速度`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:10:29.742193+00:00`
...
- `c_0` = `3997.8[m/s]` - Expected free SAW velocity
- `f_0` = `0.98*c_0/lambdaD` - Initial guess for eigenfrequency

## 内容级读取结果

- 模型树证据数量: 17
- 理论关键词: 流体, 声学, 传热, 结构力学, 电磁, 优化/参数扫描
- geometry: r1 / Rectangle
- physics: Solid Mechanics, Electrostatics, Optimization
- mesh: mesh1, mesh2, mesh3
- studies: Eigenfrequency, Optimization

### 文件内容证据

- `saw_velocity_calculation.java`: 参数=8, 几何=1, 物理场=0, 研究=1, 结果=0
- `models.mems.saw_velocity_calculation.pdf`: 参数=0, 几何=0, 物理场=3, 研究=2, 结果=0
- `SAW_Velocity_Calculations_from_a_Unit_Cell_Model.pptx`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `saw_velocity_calculation.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `saw_velocity_calculation.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `saw_velocity_calculation_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.saw_velocity_calculation.pdf`: models.mems.saw_velocity_calculation.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics、Optimization，研究类型线索为 Eigenfrequency、Optimization；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Eigenfrequency study. This tutorial investigates unit cells of 128 YX-cut LiNbO3 crystal

## PDF Summary and Physics Judgement

- Simple summary: saw_velocity_calculation 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Eigenfrequency 研究。
...
- PDF: Eigenfrequency study. This tutorial investigates unit cells of 128 YX-cut LiNbO3 crystal
- PDF: SAW phenomenon, the Eigenfrequency study is performed to solve for two
- PDF: The geometric parameters are summarized in the first table in the Modeling Instructions
- 脚本/文本识别到的物理场: Solid Mechanics, Electrostatics, Optimization
- 识别到的研究类型: Eigenfrequency, Optimization
- 理论关键词: 流体, 声学, 传热, 结构力学, 电磁, 优化/参数扫描

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 流体, 声学, 传热, 结构力学, 电磁, 优化/参数扫描, models.mems.saw_velocity_calculation.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics、Optimization，研究类型线索为 Eigenfrequency、Optimization；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Eigenfrequency study. This tutorial investigates unit cells of 128 YX-cut LiNbO3 crystal
...
- `c_0` -> Expected free SAW velocity; MATLAB: `model.param.set('c_0', value, description)`
- `f_0` -> Initial guess for eigenfrequency; MATLAB: `model.param.set('f_0', value, description)`
...
- 已识别参数：lambdaD、h_el、l_el、W、h_sub、L_PML、c_0、f_0
...
- Use the learned study types as solver starting points: Eigenfrequency, Optimization.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 声学, 传热, 结构力学, 电磁, 优化/参数扫描?
...
- Sweep `c_0` around the learned value `3997.8[m/s]` and export target outputs to CSV.
- Sweep `f_0` around the learned value `0.98*c_0/lambdaD` and export target outputs to CSV.
...
- Summary: 已学习 saw_velocity_calculation 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=17 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 17 条；几何=1，物理场=3，材料=0，网格=3，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：saw_velocity_calculation.m, saw_velocity_calculation.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 saw_velocity_calculation，训练阶段为 modeling_logic_learning_ready。
...
- `saw_velocity_calculation.mph` (comsol_mph, 32601798 bytes)
- `saw_velocity_calculation.m` (matlab_livelink, 97462 bytes)
- `saw_velocity_calculation.java` (comsol_java, 98883 bytes)
- `saw_velocity_calculation_summary.json` (json, 3283 bytes)
- `SAW_Velocity_Calculations_from_a_Unit_Cell_Model.pptx` (presentation_document, 10242065 bytes)
- `models.mems.saw_velocity_calculation.pdf` (pdf_document, 1150304 bytes)
