# surface_micromachined_accelerometer_geom_subsequence

- Case directory: `D:\桌面\codex\案例下载\电气\表面微机械加速度计`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:45:03.424515+00:00`
...
- `l_PM` = `440[um]` - geometry input parameter
- `w_PM` = `100[um]` - geometry input parameter
- `t_PM` = `2[um]` - geometry input parameter
- `l_f` = `114[um]` - geometry input parameter
- `w_f` = `4[um]` - geometry input parameter
- `n_st` = `3` - geometry input parameter
- `n_f` = `7` - geometry input parameter
- `g_f` = `1[um]` - geometry input parameter
- `g_st` = `3[um]` - geometry input parameter
- `x_st` = `14[um]` - geometry input parameter
- `x_f` = `173[um]` - geometry input parameter
- `w_eh` = `2[um]` - geometry input parameter
- `p_eh` = `20[um]` - geometry input parameter

## 内容级读取结果

- 模型树证据数量: 31
- 理论关键词: 流体, 结构力学, 电磁
- geometry: part1 / Part, part2 / Part, part3 / Part, wp1 / WorkPlane, r1 / Rectangle, r2 / Rectangle, arr1 / Array, part1 / wp1 / unite
- physics: Solid Mechanics, Electrostatics
- studies: Stationary
- boundary_conditions: wp1 / unite, r1 / size, r2 / size, r2 / pos, arr1 / type, arr1 / linearsize, arr1 / displ

### 文件内容证据

- `surface_micromachined_accelerometer_geom_subsequence.java`: 参数=13, 几何=8, 物理场=0, 研究=0, 结果=0
- `surface_micromachined_accelerometer_geom_subsequence.m`: 参数=13, 几何=8, 物理场=0, 研究=0, 结果=0
- `models.mems.surface_micromachined_accelerometer.pdf`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `surface_micromachined_accelerometer_geom_subsequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `surface_micromachined_accelerometer_geom_subsequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: surface_micromachined_accelerometer_geom_subsequence 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- `l_PM` -> geometry input parameter; MATLAB: `model.param.set('l_PM', value, description)`
- `w_PM` -> geometry input parameter; MATLAB: `model.param.set('w_PM', value, description)`
- `t_PM` -> geometry input parameter; MATLAB: `model.param.set('t_PM', value, description)`
- `l_f` -> geometry input parameter; MATLAB: `model.param.set('l_f', value, description)`
- `w_f` -> geometry input parameter; MATLAB: `model.param.set('w_f', value, description)`
- `n_st` -> geometry input parameter; MATLAB: `model.param.set('n_st', value, description)`
- `n_f` -> geometry input parameter; MATLAB: `model.param.set('n_f', value, description)`
- `g_f` -> geometry input parameter; MATLAB: `model.param.set('g_f', value, description)`
- `g_st` -> geometry input parameter; MATLAB: `model.param.set('g_st', value, description)`
- `x_st` -> geometry input parameter; MATLAB: `model.param.set('x_st', value, description)`
- `x_f` -> geometry input parameter; MATLAB: `model.param.set('x_f', value, description)`
- `w_eh` -> geometry input parameter; MATLAB: `model.param.set('w_eh', value, description)`
...
- 已识别几何证据：part1 / Part；part2 / Part；part3 / Part；wp1 / WorkPlane；r1 / Rectangle；r2 / Rectangle
- 已识别物理场证据：Solid Mechanics；Electrostatics
- 已识别参数：l_PM、w_PM、t_PM、l_f、w_f、n_st、n_f、g_f、g_st、x_st、x_f、w_eh
...
- Sweep `l_PM` around the learned value `440[um]` and export target outputs to CSV.
- Sweep `w_PM` around the learned value `100[um]` and export target outputs to CSV.
- Sweep `t_PM` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `l_f` around the learned value `114[um]` and export target outputs to CSV.
- Sweep `w_f` around the learned value `4[um]` and export target outputs to CSV.
- Sweep `n_st` around the learned value `3` and export target outputs to CSV.
- Sweep `n_f` around the learned value `7` and export target outputs to CSV.
- Sweep `g_f` around the learned value `1[um]` and export target outputs to CSV.
...
- Summary: 已学习 surface_micromachined_accelerometer_geom_subsequence 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=13；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=8，物理场=2，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：surface_micromachined_accelerometer_geom_subsequence.m, surface_micromachined_accelerometer_geom_subsequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 surface_micromachined_accelerometer_geom_subsequence，训练阶段为 modeling_logic_learning_ready。
...
- `surface_micromachined_accelerometer_geom_subsequence.mph` (comsol_mph, 1767694 bytes)
- `surface_micromachined_accelerometer_geom_subsequence.m` (matlab_livelink, 20730 bytes)
- `surface_micromachined_accelerometer_geom_subsequence.java` (comsol_java, 22044 bytes)
- `surface_micromachined_accelerometer_geom_subsequence_summary.json` (json, 538 bytes)
