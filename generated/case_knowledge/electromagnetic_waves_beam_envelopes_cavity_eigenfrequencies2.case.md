# electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2

- Case directory: `D:\桌面\codex\案例下载\电气\利用“电磁波，波束包络”接口为腔特征频率建模`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:45:59.050170+00:00`
...
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2.java`: 参数=10, 几何=1, 物理场=0, 研究=0, 结果=0
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies.pptx`: 参数=0, 几何=0, 物理场=1, 研究=4, 结果=0
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2 主要研究声压、模态或频域声学响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenvalue 研究。
...
- `width_protrusion` -> 案例参数/扫描变量; MATLAB: `model.param.set('width_protrusion', value, description)`
...
- 已识别参数：wl、length、width、E0、mLong、nTransv、kxm、kyn、fmn、width_protrusion
...
- Summary: 已学习 electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=18 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 18 条；几何=1，物理场=1，材料=0，网格=2，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2.m, electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2，训练阶段为 modeling_logic_learning_ready。
...
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2.mph` (comsol_mph, 115875445 bytes)
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2.m` (matlab_livelink, 38885 bytes)
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2.java` (comsol_java, 24349 bytes)
- `electromagnetic_waves_beam_envelopes_cavity_eigenfrequencies2_summary.json` (json, 1390 bytes)
