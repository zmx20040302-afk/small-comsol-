...
- Case directory: `D:\桌面\codex\案例下载\电气\金属-绝缘体-金属 (MIM) 二极管`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:36:33.302135+00:00`
...
- mesh: mesh1
- studies: Stationary
- boundary_conditions: mc1 / 1, mc2 / 2, mc3 / 1, i1 / coord

### 文件内容证据

- `mim_diode.java`: 参数=2, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.semicond.mim_diode.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `mim_diode.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mim_diode.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mim_diode_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.mim_diode.pdf`: models.semicond.mim_diode.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；In contrast, classical physics treats the layer as an impenetrable barrier, despite its extreme

## PDF Summary and Physics Judgement

- Simple summary: mim_diode 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Semiconductor，并采用 Stationary 研究。
...
- PDF: In contrast, classical physics treats the layer as an impenetrable barrier, despite its extreme
- PDF: This model shows how to model a simple MIM diode using the WKB tunneling model.
...
- PDF keywords: 电磁, models.semicond.mim_diode.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；In contrast, classical physics treats the layer as an impenetrable barrier, despite its extreme
- `V0` -> Voltage; MATLAB: `model.param.set('V0', value, description)`
- `wf` -> Metal work function; MATLAB: `model.param.set('wf', value, description)`
...
- Sweep `wf` around the learned value `1.03 [V]` and export target outputs to CSV.
...
- Summary: 已学习 mim_diode 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=12 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 12 条；几何=2，物理场=1，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mim_diode.m, mim_diode.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mim_diode，训练阶段为 modeling_logic_learning_ready。
...
- `mim_diode.mph` (comsol_mph, 3743746 bytes)
- `mim_diode.m` (matlab_livelink, 31183 bytes)
- `mim_diode.java` (comsol_java, 26797 bytes)
- `mim_diode_summary.json` (json, 1213 bytes)
- `models.semicond.mim_diode.pdf` (pdf_document, 162335 bytes)
