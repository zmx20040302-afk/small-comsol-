# mutual_inductance_coil_group

- Case directory: `D:\桌面\codex\案例下载\电气\单导线和螺旋线圈的自感和互感`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:14:14.498937+00:00`
...
- `r_wire` = `1[mm]` - Radius, wire
- `R1` = `100[mm]` - Radius, outer coil
- `R2` = `10[mm]` - Radius, inner coil
- `M` = `20*(mu0_const*pi*R2^2)/(2*R1)` - Analytic mutual inductance
- `I1` = `1[A]` - Current, outer coil
- `I2` = `0[A]` - Current, inner coil

## 内容级读取结果

- 模型树证据数量: 35
- 理论关键词: 电磁, 结构力学, 多孔介质/裂隙
- geometry: c1 / Circle, c2 / Circle, c3 / Circle, arr1 / Array, geom1 / c1 / r, geom1 / c1 / angle, geom1 / c1 / rot, geom1 / c1 / layername, geom1 / c2 / r, geom1 / c2 / pos, geom1 / c3 / r, geom1 / c3 / pos
- physics: Magnetic Fields
- materials: mat1
- mesh: mesh1
- studies: Stationary, Frequency Domain
- boundary_conditions: c1 / r, c1 / angle, c1 / rot, c1 / layername, c2 / r, c2 / pos, c3 / r, c3 / pos, arr1 / fullsize, arr1 / displ

### 文件内容证据

- `mutual_inductance_coil_group.java`: 参数=6, 几何=14, 物理场=0, 研究=0, 结果=0
- `models.acdc.mutual_inductance_coil_group (1).pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `models.acdc.mutual_inductance_coil_group.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `mutual_inductance_coil_group.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mutual_inductance_coil_group.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mutual_inductance_coil_group_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.mutual_inductance_coil_group (1).pdf`: models.acdc.mutual_inductance_coil_group (1).pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；domain model. Each turn of the secondary coil is modeled explicitly. Static and AC results
- `models.acdc.mutual_inductance_coil_group.pdf`: models.acdc.mutual_inductance_coil_group.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Created in COMSOL Multiphysics 5.6；domain model. Each turn of the secondary coil is modeled explicitly. Static and AC results

## PDF Summary and Physics Judgement

- Simple summary: mutual_inductance_coil_group 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF: domain model. Each turn of the secondary coil is modeled explicitly. Static and AC results
- PDF: Results and Discussion
- PDF: approach is generally less accurate, since the result depends more strongly on the mesh
...
- PDF keywords: 电磁, 结构力学, 多孔介质/裂隙, models.acdc.mutual_inductance_coil_group (1).pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；domain model. Each turn of the secondary coil is modeled explicitly. Static and AC results, models.acdc.mutual_inductance_coil_group.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Created in COMSOL Multiphysics 5.6；domain model. Each turn of the secondary coil is modeled explicitly. Static and AC results
- `r_wire` -> Radius, wire; MATLAB: `model.param.set('r_wire', value, description)`
- `R1` -> Radius, outer coil; MATLAB: `model.param.set('R1', value, description)`
- `R2` -> Radius, inner coil; MATLAB: `model.param.set('R2', value, description)`
- `M` -> Analytic mutual inductance; MATLAB: `model.param.set('M', value, description)`
- `I1` -> Current, outer coil; MATLAB: `model.param.set('I1', value, description)`
- `I2` -> Current, inner coil; MATLAB: `model.param.set('I2', value, description)`
...
- 已识别几何证据：c1 / Circle；c2 / Circle；c3 / Circle；arr1 / Array；geom1 / c1 / r；geom1 / c1 / angle
- 已识别物理场证据：Magnetic Fields
- 已识别参数：r_wire、R1、R2、M、I1、I2
...
- Sweep `r_wire` around the learned value `1[mm]` and export target outputs to CSV.
- Sweep `R1` around the learned value `100[mm]` and export target outputs to CSV.
- Sweep `R2` around the learned value `10[mm]` and export target outputs to CSV.
- Sweep `M` around the learned value `20*(mu0_const*pi*R2^2)/(2*R1)` and export target outputs to CSV.
- Sweep `I1` around the learned value `1[A]` and export target outputs to CSV.
- Sweep `I2` around the learned value `0[A]` and export target outputs to CSV.
...
- Summary: 已学习 mutual_inductance_coil_group 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=35 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 35 条；几何=14，物理场=1，材料=1，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mutual_inductance_coil_group.m, mutual_inductance_coil_group.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mutual_inductance_coil_group，训练阶段为 modeling_logic_learning_ready。
...
- `mutual_inductance_coil_group.mph` (comsol_mph, 15040573 bytes)
- `mutual_inductance_coil_group.m` (matlab_livelink, 32826 bytes)
- `mutual_inductance_coil_group.java` (comsol_java, 31340 bytes)
- `mutual_inductance_coil_group_summary.json` (json, 1592 bytes)
- `models.acdc.mutual_inductance_coil_group (1).pdf` (pdf_document, 1190175 bytes)
- `models.acdc.mutual_inductance_coil_group.pdf` (pdf_document, 1330513 bytes)
