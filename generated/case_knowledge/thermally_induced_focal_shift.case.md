...
- Case directory: `D:\桌面\codex\案例下载\电气\大功率激光聚焦系统中的热致焦移`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T03:56:04.496502+00:00`
...
- `Irms` = `1[W]` - Root mean square power of the source
- `T0` = `293.15` - Room temperature
- `lam` = `1064[nm]` - Fiber laser's wavelength
- `d` = `50[mm]` - Diameter of the plano-convex lens
- `R` = `68.8[mm]` - Radius of curvature of plano-convex lens
- `Te` = `3.0[mm]` - Edge thickness of the plano-convex lens
- `Tc` = `Te+R-sqrt(R^2-(d/2)^2)` - Center thickness of the plano-convex lens
- `dis` = `1.5*R` - Distance between the collimating and focusing lens
- `bfl` = `145.0[mm]` - Back focal length of the plano-convex lens
- `fiber_pos` = `-(dis/2+Tc+bfl)` - Position of the optical fiber
- `fiber_NA` = `0.1` - Numerical aperture of the optical fiber
- `theta` = `asin(fiber_NA)` - Half cone angle of the released rays
- `n0` = `1.45` - Real part of the refractive index at T=T0 and lambda=lam
- `dndT` = `10e-6[1/K]` - Thermal variation of the real part of the refractive index

## 内容级读取结果

- 模型树证据数量: 20
- 理论关键词: 传热, 结构力学, 电磁, 优化/参数扫描
- geometry: pi1 / PartInstance
- physics: Heat Transfer, Solid Mechanics, Ray Optics
- mesh: mesh1
- studies: Parametric Sweep

### 文件内容证据

- `thermally_induced_focal_shift.java`: 参数=14, 几何=1, 物理场=0, 研究=0, 结果=0
- `models.roptics.thermally_induced_focal_shift.pdf`: 参数=0, 几何=0, 物理场=3, 研究=1, 结果=0
- `thermally_induced_focal_shift.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `thermally_induced_focal_shift.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `thermally_induced_focal_shift_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.roptics.thermally_induced_focal_shift.pdf`: models.roptics.thermally_induced_focal_shift.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Solid Mechanics、Ray Optics，研究类型线索为 Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4； Stress-optical effects resulting from thermal stress

## PDF Summary and Physics Judgement

- Simple summary: thermally_induced_focal_shift 主要研究结构受力、变形或强度响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Parametric Sweep 研究。
- Primary physics: `Heat Transfer`
- Study type: `Parametric Sweep`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Heat Transfer，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Parametric Sweep 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF:  Stress-optical effects resulting from thermal stress
- PDF: relay in which the rays can deposit energy. The lenses are heated as a result. The
- PDF: The resulting thermally induced focal shift is then computed.
- 脚本/文本识别到的物理场: Heat Transfer, Solid Mechanics, Ray Optics
...
- PDF keywords: 传热, 结构力学, 电磁, 优化/参数扫描, models.roptics.thermally_induced_focal_shift.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Solid Mechanics、Ray Optics，研究类型线索为 Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4； Stress-optical effects resulting from thermal stress
- `Irms` -> Root mean square power of the source; MATLAB: `model.param.set('Irms', value, description)`
- `T0` -> Room temperature; MATLAB: `model.param.set('T0', value, description)`
- `lam` -> Fiber laser's wavelength; MATLAB: `model.param.set('lam', value, description)`
- `d` -> Diameter of the plano-convex lens; MATLAB: `model.param.set('d', value, description)`
- `R` -> Radius of curvature of plano-convex lens; MATLAB: `model.param.set('R', value, description)`
- `Te` -> Edge thickness of the plano-convex lens; MATLAB: `model.param.set('Te', value, description)`
- `Tc` -> Center thickness of the plano-convex lens; MATLAB: `model.param.set('Tc', value, description)`
- `dis` -> Distance between the collimating and focusing lens; MATLAB: `model.param.set('dis', value, description)`
- `bfl` -> Back focal length of the plano-convex lens; MATLAB: `model.param.set('bfl', value, description)`
- `fiber_pos` -> Position of the optical fiber; MATLAB: `model.param.set('fiber_pos', value, description)`
- `fiber_NA` -> Numerical aperture of the optical fiber; MATLAB: `model.param.set('fiber_NA', value, description)`
- `theta` -> Half cone angle of the released rays; MATLAB: `model.param.set('theta', value, description)`
...
- 已识别几何证据：pi1 / PartInstance
- 已识别物理场证据：Heat Transfer；Solid Mechanics；Ray Optics
- 已识别参数：Irms、T0、lam、d、R、Te、Tc、dis、bfl、fiber_pos、fiber_NA、theta
...
- Transfer recognized physics interfaces to similar requirements: Heat Transfer, Solid Mechanics, Ray Optics.
...
- Sweep `Irms` around the learned value `1[W]` and export target outputs to CSV.
- Sweep `T0` around the learned value `293.15` and export target outputs to CSV.
- Sweep `lam` around the learned value `1064[nm]` and export target outputs to CSV.
- Sweep `d` around the learned value `50[mm]` and export target outputs to CSV.
- Sweep `R` around the learned value `68.8[mm]` and export target outputs to CSV.
- Sweep `Te` around the learned value `3.0[mm]` and export target outputs to CSV.
- Sweep `Tc` around the learned value `Te+R-sqrt(R^2-(d/2)^2)` and export target outputs to CSV.
- Sweep `dis` around the learned value `1.5*R` and export target outputs to CSV.
...
- Summary: 已学习 thermally_induced_focal_shift 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=14；内容证据=20 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=1，物理场=3，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：thermally_induced_focal_shift.m, thermally_induced_focal_shift.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 thermally_induced_focal_shift，训练阶段为 modeling_logic_learning_ready。
...
- `thermally_induced_focal_shift.mph` (comsol_mph, 25578684 bytes)
- `thermally_induced_focal_shift.m` (matlab_livelink, 66266 bytes)
- `thermally_induced_focal_shift.java` (comsol_java, 61578 bytes)
- `thermally_induced_focal_shift_summary.json` (json, 2889 bytes)
- `models.roptics.thermally_induced_focal_shift.pdf` (pdf_document, 1117799 bytes)
