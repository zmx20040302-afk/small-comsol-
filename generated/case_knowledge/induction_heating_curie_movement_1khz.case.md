...
- Created at: `2026-08-22T06:47:45.571462+00:00`
...
- `induction_heating_curie_movement_1khz.java`: 参数=8, 几何=1, 物理场=0, 研究=0, 结果=0
- `models.acdc.induction_heating_curie_movement.pdf`: 参数=0, 几何=0, 物理场=2, 研究=3, 结果=0
- `induction_heating_curie_movement_1khz_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `induction_heating_curie_movement_1khz.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `induction_heating_curie_movement_1khz.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: induction_heating_curie_movement_1khz 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- Sweep `J0` around the learned value `20[A/mm^2]` and export target outputs to CSV.
...
- Summary: 已学习 induction_heating_curie_movement_1khz 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=15 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：induction_heating_curie_movement_1khz.m, induction_heating_curie_movement_1khz.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 induction_heating_curie_movement_1khz，训练阶段为 modeling_logic_learning_ready。
...
- `induction_heating_curie_movement_1khz.mph` (comsol_mph, 342151060 bytes)
- `induction_heating_curie_movement_1khz.m` (matlab_livelink, 82991 bytes)
- `induction_heating_curie_movement_1khz.java` (comsol_java, 83890 bytes)
- `induction_heating_curie_movement_1khz_summary.json` (json, 2620 bytes)
