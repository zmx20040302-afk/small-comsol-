# gyroscope_mixed_formulation

- Case directory: `D:\桌面\codex\案例下载\电气\使用混合公式模拟微机械陀螺仪`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:34:55.743965+00:00`
...
- `Vcomb` = `20[V]` - Comb drive voltage
- `V_ac` = `1.5[V]` - AC voltage
- `Vbase` = `5[V]` - Base voltage
- `Q` = `500` - Quality factor
- `t_anchor` = `2[um]` - Anchor layer thickness
- `mf` = `10` - Mesh factor
- `Omega` = `0[deg/s]` - Angular speed
...
- `gyroscope_mixed_formulation.java`: 参数=7, 几何=2, 物理场=0, 研究=0, 结果=0
- `models.mems.gyroscope_mixed_formulation.pdf`: 参数=0, 几何=0, 物理场=2, 研究=3, 结果=0
- `gyroscope_mixed_formulation.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `gyroscope_mixed_formulation.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `gyroscope_mixed_formulation_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.gyroscope_mixed_formulation.pdf`: models.mems.gyroscope_mixed_formulation.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the relevant physics are added automatically. The original device is loosely based on Ref. 1.

## PDF Summary and Physics Judgement

- Simple summary: gyroscope_mixed_formulation 主要研究声压、模态或频域声学响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenfrequency 研究。
...
- PDF: the relevant physics are added automatically. The original device is loosely based on Ref. 1.
- PDF: For convenience, the geometry is imported into the model. The structural part (excluding
- PDF: of domain or boundary-dependent features and mesh.
...
- PDF keywords: 电磁, 结构力学, 声学, models.mems.gyroscope_mixed_formulation.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the relevant physics are added automatically. The original device is loosely based on Ref. 1.
- `Vcomb` -> Comb drive voltage; MATLAB: `model.param.set('Vcomb', value, description)`
- `V_ac` -> AC voltage; MATLAB: `model.param.set('V_ac', value, description)`
- `Vbase` -> Base voltage; MATLAB: `model.param.set('Vbase', value, description)`
- `Q` -> Quality factor; MATLAB: `model.param.set('Q', value, description)`
- `t_anchor` -> Anchor layer thickness; MATLAB: `model.param.set('t_anchor', value, description)`
- `mf` -> Mesh factor; MATLAB: `model.param.set('mf', value, description)`
- `Omega` -> Angular speed; MATLAB: `model.param.set('Omega', value, description)`
...
- 已识别参数：Vcomb、V_ac、Vbase、Q、t_anchor、mf、Omega
...
- Sweep `Vcomb` around the learned value `20[V]` and export target outputs to CSV.
- Sweep `V_ac` around the learned value `1.5[V]` and export target outputs to CSV.
- Sweep `Vbase` around the learned value `5[V]` and export target outputs to CSV.
- Sweep `Q` around the learned value `500` and export target outputs to CSV.
- Sweep `t_anchor` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `mf` around the learned value `10` and export target outputs to CSV.
- Sweep `Omega` around the learned value `0[deg/s]` and export target outputs to CSV.
...
- Summary: 已学习 gyroscope_mixed_formulation 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=16 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 16 条；几何=2，物理场=2，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：gyroscope_mixed_formulation.m, gyroscope_mixed_formulation.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 gyroscope_mixed_formulation，训练阶段为 modeling_logic_learning_ready。
...
- `gyroscope_mixed_formulation.mph` (comsol_mph, 600138033 bytes)
- `gyroscope_mixed_formulation.m` (matlab_livelink, 78447 bytes)
- `gyroscope_mixed_formulation.java` (comsol_java, 79318 bytes)
- `gyroscope_mixed_formulation_summary.json` (json, 1945 bytes)
- `models.mems.gyroscope_mixed_formulation.pdf` (pdf_document, 5036567 bytes)
