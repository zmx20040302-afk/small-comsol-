...
- `Size` = `20[cm]`
- `Factor` = `1000`
- `sigma_stab` = `1/(pi*f0*mu0_const*(Factor*Size)^2)`
- `I0` = `1[kA]`
- `Rwire` = `0.5[cm]`
...
- 模型树证据数量: 30
- 理论关键词: 电磁
- geometry: wp1 / WorkPlane, r1 / Rectangle, fil1 / Fillet, ext1 / Extrude, geom1 / wp1 / unite, geom1 / ext1 / selresult, cyl1 / Cylinder, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / pos, geom1 / cyl1 / axis, geom1 / cyl1 / layername
- physics: Magnetic Fields
- mesh: mesh1
- boundary_conditions: wp1 / unite, r1 / size, r1 / pos, fil1 / radius, ext1 / selresult, cyl1 / r, cyl1 / h, cyl1 / pos, cyl1 / axis, cyl1 / layername

### 文件内容证据

- `Symmetric_Coil_Inductive_Heating.java`: 参数=6, 几何=12, 物理场=0, 研究=0, 结果=0
- `Symmetric_Coil_Inductive_Heating.m`: 参数=6, 几何=6, 物理场=0, 研究=0, 结果=0
- `Symmetric_Coil_Inductive_Heating_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `Symmetric_Coil_Inductive_Heating.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: Symmetric_Coil_Inductive_Heating 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Magnetic Fields。
...
- `Size` -> 案例参数/扫描变量; MATLAB: `model.param.set('Size', value, description)`
...
- `Rwire` -> 案例参数/扫描变量; MATLAB: `model.param.set('Rwire', value, description)`
...
- 已识别几何证据：wp1 / WorkPlane；r1 / Rectangle；fil1 / Fillet；ext1 / Extrude；geom1 / wp1 / unite；geom1 / ext1 / selresult
- 已识别物理场证据：Magnetic Fields
- 已识别参数：f0、Size、Factor、sigma_stab、I0、Rwire
...
- Sweep `f0` around the learned value `450[kHz]` and export target outputs to CSV.
- Sweep `Size` around the learned value `20[cm]` and export target outputs to CSV.
- Sweep `Factor` around the learned value `1000` and export target outputs to CSV.
- Sweep `sigma_stab` around the learned value `1/(pi*f0*mu0_const*(Factor*Size)^2)` and export target outputs to CSV.
- Sweep `I0` around the learned value `1[kA]` and export target outputs to CSV.
- Sweep `Rwire` around the learned value `0.5[cm]` and export target outputs to CSV.
...
- Summary: 已学习 Symmetric_Coil_Inductive_Heating 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=30 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 30 条；几何=12，物理场=1，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：Symmetric_Coil_Inductive_Heating.m, Symmetric_Coil_Inductive_Heating.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 Symmetric_Coil_Inductive_Heating，训练阶段为 modeling_logic_learning_ready。
...
- `Symmetric_Coil_Inductive_Heating.mph` (comsol_mph, 56126563 bytes)
- `Symmetric_Coil_Inductive_Heating.m` (matlab_livelink, 41143 bytes)
- `Symmetric_Coil_Inductive_Heating.java` (comsol_java, 43358 bytes)
- `Symmetric_Coil_Inductive_Heating_summary.json` (json, 1300 bytes)
