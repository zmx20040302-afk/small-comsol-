...
- Case directory: `D:\桌面\codex\案例下载\COMSOL\顶盖驱动方腔流`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-25T09:35:50.413866+00:00`
...
- `xR` = `1-0.1328` - x-coordinate for right vortex
- `yR` = `0.1484` - y-coordinate for right vortex
- `xL` = `0.0781` - x-coordinate for left vortex
- `yL` = `0.0781` - y-coordinate for left vortex
- `xC` = `0.6172` - x-coordinate for center vortex
- `yC` = `0.7344` - y-coordinate for center vortex
...
- 理论关键词: 结构力学, 流体
- geometry: sq1 / Square
- physics: spf / LaminarFlow, Laminar Flow
- materials: mat1
- mesh: mesh1, map1
- studies: std1 / stat / Stationary, std1, Stationary
- boundary_conditions: wallbc2 / [3], prpc1 / [1], wallbc2 / 3, prpc1 / 1, wallbc2 / SlidingWall, wallbc2 / uvw, dis1 / type, dis1 / elemcount, dis1 / elemratio, dis1 / growthrate, dis1 / symmetric, stat / useparam

### 文件内容证据

- `lid_driven_cavity.mph`: 参数=7, 几何=1, 物理场=1, 研究=3, 结果=0
- `lid_driven_cavity.java`: 参数=7, 几何=1, 物理场=2, 研究=0, 结果=0
- `lid_driven_cavity.m`: 参数=7, 几何=1, 物理场=1, 研究=0, 结果=0
- `lid_driven_cavity_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: lid_driven_cavity 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- 脚本/文本识别到的物理场: spf / LaminarFlow, Laminar Flow
...
- `xR` -> x-coordinate for right vortex; MATLAB: `model.param.set('xR', value, description)`
- `yR` -> y-coordinate for right vortex; MATLAB: `model.param.set('yR', value, description)`
- `xL` -> x-coordinate for left vortex; MATLAB: `model.param.set('xL', value, description)`
- `yL` -> y-coordinate for left vortex; MATLAB: `model.param.set('yL', value, description)`
- `xC` -> x-coordinate for center vortex; MATLAB: `model.param.set('xC', value, description)`
- `yC` -> y-coordinate for center vortex; MATLAB: `model.param.set('yC', value, description)`
...
- 已识别几何证据：sq1 / Square
- 已识别物理场证据：spf / LaminarFlow；Laminar Flow
- 已识别参数：Re、xR、yR、xL、yL、xC、yC
...
- Transfer recognized physics interfaces to similar requirements: spf / LaminarFlow, Laminar Flow.
...
- Sweep `Re` around the learned value `10000` and export target outputs to CSV.
- Sweep `xR` around the learned value `1-0.1328` and export target outputs to CSV.
- Sweep `yR` around the learned value `0.1484` and export target outputs to CSV.
- Sweep `xL` around the learned value `0.0781` and export target outputs to CSV.
- Sweep `yL` around the learned value `0.0781` and export target outputs to CSV.
- Sweep `xC` around the learned value `0.6172` and export target outputs to CSV.
- Sweep `yC` around the learned value `0.7344` and export target outputs to CSV.
...
- Summary: 已学习 lid_driven_cavity 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=1，物理场=2，材料=1，网格=2，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：lid_driven_cavity.m, lid_driven_cavity.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 lid_driven_cavity，训练阶段为 modeling_logic_learning_ready。
...
- `lid_driven_cavity.mph` (comsol_mph, 4339257 bytes)
- `lid_driven_cavity.m` (matlab_livelink, 19172 bytes)
- `lid_driven_cavity.java` (comsol_java, 19980 bytes)
- `lid_driven_cavity_summary.json` (json, 1015 bytes)
