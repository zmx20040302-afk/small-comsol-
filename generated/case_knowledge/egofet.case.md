...
- Case directory: `D:\桌面\codex\案例下载\电气\电解质栅极有机场效应晶体管`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:23:50.860642+00:00`
...
- `w_dom` = `30[um]` - Domain width
- `h_semi` = `500[nm]` - Organic semiconductor thickness
- `h_electrolyte` = `10[um]` - Electrolyte thickness
- `V_dr` = `0[V]` - Drain electrode Voltage
- `V_g` = `0[V]` - Gate electrode voltage
- `D_h` = `1e-6[m^2/s]` - Diffusion coefficient for holes
- `D_p` = `15e-10[m^2/s]` - Diffusion coefficient for positive ions
- `D_n` = `D_p` - Diffusion coefficient for negative ions
- `T` = `300[K]` - Temperature
- `d` = `1[cm]` - depth
- `z_h` = `+1` - charge of hole
- `z_p` = `+1` - charge of positive ions
- `z_n` = `-1` - charge of negative ions
- `ch0` = `1e-3[mol/m^3]` - Holes initial concentration
- `cp0` = `1[mol/m^3]` - Positive ions initial concentration
- `cn0` = `1[mol/m^3]` - Negative ions initial concentration
...
- physics: tcc / TransportOfChargeCarriers, Electrostatics
- mesh: mesh1
- studies: Stationary
...
- `egofet.java`: 参数=16, 几何=5, 物理场=1, 研究=0, 结果=0
- `models.semicond.egofet.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `egofet.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `egofet.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `egofet_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.egofet.pdf`: models.semicond.egofet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；based on a general Drift–Diffusion model.

## PDF Summary and Physics Judgement

- Simple summary: egofet 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: based on a general Drift–Diffusion model.
- PDF: The materials and geometry of this model are a simplified version of the device described
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: tcc / TransportOfChargeCarriers, Electrostatics
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 电磁, 流体
...
- PDF keywords: 传热, 电磁, 流体, models.semicond.egofet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；based on a general Drift–Diffusion model.
- `w_dom` -> Domain width; MATLAB: `model.param.set('w_dom', value, description)`
- `h_semi` -> Organic semiconductor thickness; MATLAB: `model.param.set('h_semi', value, description)`
- `h_electrolyte` -> Electrolyte thickness; MATLAB: `model.param.set('h_electrolyte', value, description)`
- `V_dr` -> Drain electrode Voltage; MATLAB: `model.param.set('V_dr', value, description)`
- `V_g` -> Gate electrode voltage; MATLAB: `model.param.set('V_g', value, description)`
- `D_h` -> Diffusion coefficient for holes; MATLAB: `model.param.set('D_h', value, description)`
- `D_p` -> Diffusion coefficient for positive ions; MATLAB: `model.param.set('D_p', value, description)`
- `D_n` -> Diffusion coefficient for negative ions; MATLAB: `model.param.set('D_n', value, description)`
- `T` -> Temperature; MATLAB: `model.param.set('T', value, description)`
- `d` -> depth; MATLAB: `model.param.set('d', value, description)`
- `z_h` -> charge of hole; MATLAB: `model.param.set('z_h', value, description)`
- `z_p` -> charge of positive ions; MATLAB: `model.param.set('z_p', value, description)`
...
- 已识别物理场证据：tcc / TransportOfChargeCarriers；Electrostatics
- 已识别参数：w_dom、h_semi、h_electrolyte、V_dr、V_g、D_h、D_p、D_n、T、d、z_h、z_p
...
- 识别到物理场证据：tcc / TransportOfChargeCarriers
...
- Transfer recognized physics interfaces to similar requirements: tcc / TransportOfChargeCarriers, Electrostatics.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 电磁, 流体?
...
- Sweep `w_dom` around the learned value `30[um]` and export target outputs to CSV.
- Sweep `h_semi` around the learned value `500[nm]` and export target outputs to CSV.
- Sweep `h_electrolyte` around the learned value `10[um]` and export target outputs to CSV.
- Sweep `V_dr` around the learned value `0[V]` and export target outputs to CSV.
- Sweep `V_g` around the learned value `0[V]` and export target outputs to CSV.
- Sweep `D_h` around the learned value `1e-6[m^2/s]` and export target outputs to CSV.
- Sweep `D_p` around the learned value `15e-10[m^2/s]` and export target outputs to CSV.
- Sweep `D_n` around the learned value `D_p` and export target outputs to CSV.
...
- Summary: 已学习 egofet 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=16；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=5，物理场=2，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：egofet.m, egofet.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 egofet，训练阶段为 modeling_logic_learning_ready。
...
- `egofet.mph` (comsol_mph, 119553932 bytes)
- `egofet.m` (matlab_livelink, 37764 bytes)
- `egofet.java` (comsol_java, 34982 bytes)
- `egofet_summary.json` (json, 1252 bytes)
- `models.semicond.egofet.pdf` (pdf_document, 320559 bytes)
