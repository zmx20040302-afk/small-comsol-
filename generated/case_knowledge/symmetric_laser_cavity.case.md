# symmetric_laser_cavity

- Case directory: `D:\桌面\codex\案例下载\电气\对称激光腔的横模`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:20:58.894520+00:00`
...
- `wlref` = `1[um]` - Reference wavelength
- `d` = `1[m]` - Cavity length
- `R` = `1.5*d` - Mirror radius
- `g` = `1-d/R` - Stability parameter
- `q` = `floor(2*d/wlref)` - Longitudinal mode number
- `n` = `0` - Transverse mode number
- `fqn` = `c_const/(2*d)*(q+1+1/pi*(1+2*n)*atan(sqrt((1-g)/(1+g))))` - Mode frequency
- `wlqn` = `c_const/fqn` - Mode wavelength
- `w0` = `sqrt(d*wlqn/(2*pi)*sqrt((1+g)/(1-g)))` - Spot radius
- `z0` = `pi*w0^2/wlqn` - Rayleigh range
- `wR` = `sqrt(d*wlqn/pi*sqrt(1/(1-g^2)))` - Spot radius at the mirrors
- `h0` = `10*wR` - Cavity height
- `k0` = `2*pi*fqn/c_const` - Wave number

## 内容级读取结果

- 模型树证据数量: 32
- 理论关键词: 电磁, 声学, 优化/参数扫描
- geometry: c1 / Circle, r1 / Rectangle, int1 / Intersection, mir1 / Mirror, geom1 / c1 / r, geom1 / c1 / pos, geom1 / r1 / size, geom1 / mir1 / keep
- physics: Global ODEs and DAEs, Electromagnetic Waves
- mesh: mesh1
- studies: Stationary, Eigenfrequency, Frequency Domain, Parametric Sweep
- boundary_conditions: c1 / r, c1 / pos, r1 / size, mir1 / keep

### 文件内容证据

- `symmetric_laser_cavity.java`: 参数=13, 几何=8, 物理场=0, 研究=0, 结果=0
- `models.woptics.symmetric_laser_cavity.pdf`: 参数=0, 几何=0, 物理场=2, 研究=4, 结果=0
- `symmetric_laser_cavity.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `symmetric_laser_cavity.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `symmetric_laser_cavity_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.symmetric_laser_cavity.pdf`: models.woptics.symmetric_laser_cavity.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Global ODEs and DAEs、Electromagnetic Waves，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Notice that in 3D geometry, the square root expression in the amplitude

## PDF Summary and Physics Judgement

- Simple summary: symmetric_laser_cavity 主要研究声压、模态或频域声学响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Pressure Acoustics，并采用 Eigenfrequency 研究。
- Primary physics: `Pressure Acoustics`
- Study type: `Eigenfrequency`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Pressure Acoustics，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Eigenfrequency 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: Notice that in 3D geometry, the square root expression in the amplitude
- PDF: Equation 3). If an eigenfrequency study would be used for solving the cavity problem, the
- PDF: stationary study solving for both the electric field and the frequencies that self-consistently
...
- PDF keywords: 电磁, 声学, 优化/参数扫描, models.woptics.symmetric_laser_cavity.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Global ODEs and DAEs、Electromagnetic Waves，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Notice that in 3D geometry, the square root expression in the amplitude
- `wlref` -> Reference wavelength; MATLAB: `model.param.set('wlref', value, description)`
- `d` -> Cavity length; MATLAB: `model.param.set('d', value, description)`
- `R` -> Mirror radius; MATLAB: `model.param.set('R', value, description)`
- `g` -> Stability parameter; MATLAB: `model.param.set('g', value, description)`
- `q` -> Longitudinal mode number; MATLAB: `model.param.set('q', value, description)`
- `n` -> Transverse mode number; MATLAB: `model.param.set('n', value, description)`
- `fqn` -> Mode frequency; MATLAB: `model.param.set('fqn', value, description)`
- `wlqn` -> Mode wavelength; MATLAB: `model.param.set('wlqn', value, description)`
- `w0` -> Spot radius; MATLAB: `model.param.set('w0', value, description)`
- `z0` -> Rayleigh range; MATLAB: `model.param.set('z0', value, description)`
- `wR` -> Spot radius at the mirrors; MATLAB: `model.param.set('wR', value, description)`
- `h0` -> Cavity height; MATLAB: `model.param.set('h0', value, description)`
...
- 已识别几何证据：c1 / Circle；r1 / Rectangle；int1 / Intersection；mir1 / Mirror；geom1 / c1 / r；geom1 / c1 / pos
- 已识别物理场证据：Global ODEs and DAEs；Electromagnetic Waves
- 已识别参数：wlref、d、R、g、q、n、fqn、wlqn、w0、z0、wR、h0
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 声学, 优化/参数扫描?
...
- Sweep `wlref` around the learned value `1[um]` and export target outputs to CSV.
- Sweep `d` around the learned value `1[m]` and export target outputs to CSV.
- Sweep `R` around the learned value `1.5*d` and export target outputs to CSV.
- Sweep `g` around the learned value `1-d/R` and export target outputs to CSV.
- Sweep `q` around the learned value `floor(2*d/wlref)` and export target outputs to CSV.
- Sweep `n` around the learned value `0` and export target outputs to CSV.
- Sweep `fqn` around the learned value `c_const/(2*d)*(q+1+1/pi*(1+2*n)*atan(sqrt((1-g)/(1+g))))` and export target outputs to CSV.
- Sweep `wlqn` around the learned value `c_const/fqn` and export target outputs to CSV.
...
- Summary: 已学习 symmetric_laser_cavity 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=13；内容证据=32 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 32 条；几何=8，物理场=2，材料=0，网格=1，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：symmetric_laser_cavity.m, symmetric_laser_cavity.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 symmetric_laser_cavity，训练阶段为 modeling_logic_learning_ready。
...
- `symmetric_laser_cavity.mph` (comsol_mph, 7112230 bytes)
- `symmetric_laser_cavity.m` (matlab_livelink, 27721 bytes)
- `symmetric_laser_cavity.java` (comsol_java, 25368 bytes)
- `symmetric_laser_cavity_summary.json` (json, 1227 bytes)
- `models.woptics.symmetric_laser_cavity.pdf` (pdf_document, 929629 bytes)
