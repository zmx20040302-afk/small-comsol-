...
- Case directory: `D:\桌面\codex\案例下载\电气\氯放电全局模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:54:24.384308+00:00`
...
- `chlorine_global_model.java`: 参数=5, 几何=2, 物理场=0, 研究=0, 结果=0
- `models.plasma.chlorine_global_model.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `chlorine_global_model.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `chlorine_global_model.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `chlorine_global_model_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.chlorine_global_model.pdf`: models.plasma.chlorine_global_model.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Time Dependent、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；In this study, model results of chlorine and electron density for absorbed powers between

## PDF Summary and Physics Judgement

- Simple summary: chlorine_global_model 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: In this study, model results of chlorine and electron density for absorbed powers between
- PDF: Model results of several relevant quantities such as atomic chlorine density, electron
- PDF: This study uses the LEA and a Maxwellian EEDF to computed the electron impact rate
...
- PDF keywords: 传热, 流体, 结构力学, 电磁, 优化/参数扫描, models.plasma.chlorine_global_model.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Time Dependent、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；In this study, model results of chlorine and electron density for absorbed powers between
- `Rad` -> 案例参数/扫描变量; MATLAB: `model.param.set('Rad', value, description)`
- `L` -> 案例参数/扫描变量; MATLAB: `model.param.set('L', value, description)`
- `Pabs` -> 案例参数/扫描变量; MATLAB: `model.param.set('Pabs', value, description)`
- `Qfeed` -> 案例参数/扫描变量; MATLAB: `model.param.set('Qfeed', value, description)`
...
- Sweep `Rad` around the learned value `15[cm]` and export target outputs to CSV.
- Sweep `L` around the learned value `14[cm]` and export target outputs to CSV.
- Sweep `Pabs` around the learned value `300[W]` and export target outputs to CSV.
- Sweep `Qfeed` around the learned value `20` and export target outputs to CSV.
- Sweep `p0` around the learned value `0.01[Torr]` and export target outputs to CSV.
...
- Summary: 已学习 chlorine_global_model 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=11 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 11 条；几何=2，物理场=0，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：chlorine_global_model.m, chlorine_global_model.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 chlorine_global_model，训练阶段为 modeling_logic_learning_ready。
...
- `chlorine_global_model.mph` (comsol_mph, 40025594 bytes)
- `chlorine_global_model.m` (matlab_livelink, 130929 bytes)
- `chlorine_global_model.java` (comsol_java, 139368 bytes)
- `chlorine_global_model_summary.json` (json, 1483 bytes)
- `models.plasma.chlorine_global_model.pdf` (pdf_document, 337679 bytes)
