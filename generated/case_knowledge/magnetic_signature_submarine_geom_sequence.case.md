# magnetic_signature_submarine_geom_sequence

- Case directory: `D:\桌面\codex\案例下载\电气\潜艇的磁信号`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:28:51.065986+00:00`
...
- `a` = `7[m]` - First semi axis of the hull
- `b` = `3.5[m]` - Seconds semi axis of the hull
- `g2` = `6.5[m]` - Length of transition section of the hull
- `g3` = `6.5[m]` - Length of the cone section of the hull
- `L` = `42[m]` - Length of the deck
- `alpha` = `18[deg]` - Angle of the cone section of the hull
- `s_deck` = `1.2[m]` - Width of the flat section of the hull
- `c_deck` = `4[m]` - Flat height of the hull
- `theta` = `60[deg]` - Semi angle of the non axi-symmetric hull
- `p_z2` = `-b*sin(theta)+s_deck` - First parameter used in the definition of the transition area
- `p_y2` = `c_deck-(b*cos(theta))` - Second parameter used in the definition of the transition area
- `p_a` = `(-tan(theta)*p_z2-2*p_y2)/(p_z2)^3` - Third parameter used in the definition of the transition area
- `p_b` = `(-tan(theta)-3*p_a*(p_z2)^2)/2/p_z2` - fourth parameter used in the definition of the transition area
- `a0` = `0.2969` - Sail NACA profile parameter 0
- `a1` = `0.12651673270629464` - Sail NACA profile parameter 1
- `a2` = `0.34981592496061949` - Sail NACA profile parameter 2
- `a3` = `0.28392704804012290` - Sail NACA profile parameter 3
- `a4` = `0.10449439037320877` - Sail NACA profile parameter 4
- `s_x1` = `19[m]` - x-coordinate of the sail at the deck height
- `s_l1` = `13[m]` - Sail length at the deck height
- `s_x2` = `19.2[m]` - x-coordinate of the sail at the top
- `s_l2` = `12.3[m]` - Sail length at the top
- `s_w2` = `2[m]` - Sail width at the top
- `s_h` = `3.5[m]` - Sail height
- `r_x1` = `58.9[m]` - x-coordinate of the rudder at the center
- `r_l1` = `2.6[m]` - Rudder length at the center
- `r_w1` = `0.4[m]` - Rudder width at the center
- `r_x2` = `59.15[m]` - x-coordinate of the rudder at the exterior
- `r_l2` = `2.35[m]` - Rudder length at the exterior
- `r_w2` = `0.3[m]` - Rudder width at the exterior
- `r_h` = `3.5[m]` - Rudder height
- `b_x1` = `11.0[m]` - x-coordinate of the bow plane at the interior
- `b_l1` = `2.6[m]` - Bow plane length at the interior
- `b_w1` = `0.265[m]` - Bow plane width at the interior
- `b_h1` = `1.2[m]` - Bow plane interior plane
- `b_x2` = `11.25[m]` - x-coordinate of the bow plane at the exterior
- `b_l2` = `2.35[m]` - Bow plane length at the exterior
- `b_w2` = `0.222[m]` - Bow plane width at the exterior
- `b_h2` = `3.5[m]` - Bow plane exterior plane
- `b_d` = `3.8675[m]` - Bow plane offset from submarine center

## 内容级读取结果

- 模型树证据数量: 38
- 理论关键词: 电磁, 传热, 声学, 多孔介质/裂隙
- physics: Electric Currents, Electrostatics, Magnetic Fields
- studies: Stationary

### 文件内容证据

- `magnetic_signature_submarine_geom_sequence.m`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetic_signature_submarine_geom_sequence.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.acdc.magnetic_signature_submarine.pdf`: 参数=0, 几何=0, 物理场=3, 研究=1, 结果=0
- `magnetic_signature_submarine_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetic_signature_submarine_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.magnetic_signature_submarine.pdf`: models.acdc.magnetic_signature_submarine.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electric Currents、Electrostatics、Magnetic Fields，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；simulate using standard finite element analysis because volume meshes of thin extended

## PDF Summary and Physics Judgement

- Simple summary: magnetic_signature_submarine_geom_sequence 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: simulate using standard finite element analysis because volume meshes of thin extended
- PDF: metal as 2D faces embedded in a 3D geometry. Thus it is only necessary to create
- PDF: comparatively inexpensive 2D face meshes in addition to the 3D volume mesh used for the
- 脚本/文本识别到的物理场: Electric Currents, Electrostatics, Magnetic Fields
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 传热, 声学, 多孔介质/裂隙
...
- PDF keywords: 电磁, 传热, 声学, 多孔介质/裂隙, models.acdc.magnetic_signature_submarine.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electric Currents、Electrostatics、Magnetic Fields，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；simulate using standard finite element analysis because volume meshes of thin extended
- `a` -> First semi axis of the hull; MATLAB: `model.param.set('a', value, description)`
- `b` -> Seconds semi axis of the hull; MATLAB: `model.param.set('b', value, description)`
- `g2` -> Length of transition section of the hull; MATLAB: `model.param.set('g2', value, description)`
- `g3` -> Length of the cone section of the hull; MATLAB: `model.param.set('g3', value, description)`
- `L` -> Length of the deck; MATLAB: `model.param.set('L', value, description)`
- `alpha` -> Angle of the cone section of the hull; MATLAB: `model.param.set('alpha', value, description)`
- `s_deck` -> Width of the flat section of the hull; MATLAB: `model.param.set('s_deck', value, description)`
- `c_deck` -> Flat height of the hull; MATLAB: `model.param.set('c_deck', value, description)`
- `theta` -> Semi angle of the non axi-symmetric hull; MATLAB: `model.param.set('theta', value, description)`
- `p_z2` -> First parameter used in the definition of the transition area; MATLAB: `model.param.set('p_z2', value, description)`
- `p_y2` -> Second parameter used in the definition of the transition area; MATLAB: `model.param.set('p_y2', value, description)`
- `p_a` -> Third parameter used in the definition of the transition area; MATLAB: `model.param.set('p_a', value, description)`
...
- 已识别物理场证据：Electric Currents；Electrostatics；Magnetic Fields
- 已识别参数：a、b、g2、g3、L、alpha、s_deck、c_deck、theta、p_z2、p_y2、p_a
...
- 本案例已提取 40 个参数，可作为约束 JSON 和参数扫描变量。
...
- Transfer recognized physics interfaces to similar requirements: Electric Currents, Electrostatics, Magnetic Fields.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 传热, 声学, 多孔介质/裂隙?
...
- Sweep `a` around the learned value `7[m]` and export target outputs to CSV.
- Sweep `b` around the learned value `3.5[m]` and export target outputs to CSV.
- Sweep `g2` around the learned value `6.5[m]` and export target outputs to CSV.
- Sweep `g3` around the learned value `6.5[m]` and export target outputs to CSV.
- Sweep `L` around the learned value `42[m]` and export target outputs to CSV.
- Sweep `alpha` around the learned value `18[deg]` and export target outputs to CSV.
- Sweep `s_deck` around the learned value `1.2[m]` and export target outputs to CSV.
- Sweep `c_deck` around the learned value `4[m]` and export target outputs to CSV.
...
- Summary: 已学习 magnetic_signature_submarine_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=40；内容证据=38 条。
...
- 40 extracted parameters with values/descriptions.
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 38 条；几何=0，物理场=3，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：magnetic_signature_submarine_geom_sequence.m, magnetic_signature_submarine_geom_sequence.java
  - Judgement: 脚本中的 COMSOL API 调用可用于恢复模型树顺序，并辅助生成 LiveLink MATLAB 自动建模脚本。
- **4. 参数与约束提取**
  - Evidence: 从文本、MATLAB 或 Java 证据中提取到 40 个参数。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 magnetic_signature_submarine_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `magnetic_signature_submarine_geom_sequence.mph` (comsol_mph, 3892773 bytes)
- `magnetic_signature_submarine_geom_sequence.m` (matlab_livelink, 77777 bytes)
- `magnetic_signature_submarine_geom_sequence.java` (comsol_java, 84329 bytes)
- `magnetic_signature_submarine_geom_sequence_summary.json` (json, 1676 bytes)
- `models.acdc.magnetic_signature_submarine.pdf` (pdf_document, 984905 bytes)
