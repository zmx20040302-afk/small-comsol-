...
- Case directory: `D:\桌面\codex\案例下载\电气\全息页面数据存储仿真`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:22:34.876804+00:00`
...
- `wl0` = `1e-6 [m]` - Wavelength
- `k0` = `2*pi/wl0` - Wave number
- `n` = `1.35` - Hologram material index
- `wl` = `wl0/n` - Wavelength in material
- `Lx` = `160e-6[m]` - Air domain size
- `Ly` = `40e-6[m]` - Air domain height
- `dn` = `0.01` - Modulation coefficient
- `w_o` = `10e-6[m]` - 1/e^2 intensity waist radius for object beam
- `w_r` = `10e-6[m]` - 1/e^2 intensity waist radius for reference beam
- `threshold` = `.35` - Holographic material threshold
- `NA` = `1.22/2*wl0/w_o` - NA in air
- `N` = `8` - Data bit number
- `f_by_d` = `N/NA` - f by d
- `w_o_1` = `f_by_d*wl0/2` - 1st order position
- `d` = `40[um]` - Pixel half size (effective pixel size+margin)
- `f` = `f_by_d*d` - Focal length
- `R` = `N*d` - Half of SML size
- `xR` = `pi*w_r^2/wl` - Rayleight range for reference beam
- `R_r` = `Lx/2*(1+(xR/(Lx/2))^2)` - Wavefront curvature

## 内容级读取结果

- 模型树证据数量: 27
- 理论关键词: 未自动识别
- geometry: r1 / Rectangle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r1 / layername
- mesh: mesh1
- boundary_conditions: r1 / size, r1 / pos, r1 / layername

### 文件内容证据

- `page_hologram.java`: 参数=19, 几何=4, 物理场=0, 研究=0, 结果=0
- `page_hologram.m`: 参数=19, 几何=1, 物理场=0, 研究=0, 结果=0
- `page_hologram.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `page_hologram_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `page_hologram.pptx`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: page_hologram 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `n` -> Hologram material index; MATLAB: `model.param.set('n', value, description)`
- `wl` -> Wavelength in material; MATLAB: `model.param.set('wl', value, description)`
- `Lx` -> Air domain size; MATLAB: `model.param.set('Lx', value, description)`
- `Ly` -> Air domain height; MATLAB: `model.param.set('Ly', value, description)`
...
- `threshold` -> Holographic material threshold; MATLAB: `model.param.set('threshold', value, description)`
- `NA` -> NA in air; MATLAB: `model.param.set('NA', value, description)`
- `N` -> Data bit number; MATLAB: `model.param.set('N', value, description)`
...
- 已识别几何证据：r1 / Rectangle；geom1 / r1 / size；geom1 / r1 / pos；geom1 / r1 / layername
- 已识别参数：wl0、k0、n、wl、Lx、Ly、dn、w_o、w_r、threshold、NA、N
...
- Sweep `wl0` around the learned value `1e-6 [m]` and export target outputs to CSV.
- Sweep `k0` around the learned value `2*pi/wl0` and export target outputs to CSV.
- Sweep `n` around the learned value `1.35` and export target outputs to CSV.
- Sweep `wl` around the learned value `wl0/n` and export target outputs to CSV.
- Sweep `Lx` around the learned value `160e-6[m]` and export target outputs to CSV.
- Sweep `Ly` around the learned value `40e-6[m]` and export target outputs to CSV.
- Sweep `dn` around the learned value `0.01` and export target outputs to CSV.
- Sweep `w_o` around the learned value `10e-6[m]` and export target outputs to CSV.
...
- Summary: 已学习 page_hologram 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=19；内容证据=27 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 27 条；几何=4，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：page_hologram.m, page_hologram.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 page_hologram，训练阶段为 modeling_logic_learning_ready。
...
- `page_hologram.mph` (comsol_mph, 319575850 bytes)
- `page_hologram.m` (matlab_livelink, 34941 bytes)
- `page_hologram.java` (comsol_java, 27481 bytes)
- `page_hologram_summary.json` (json, 1506 bytes)
- `page_hologram.pptx` (presentation_document, 2834317 bytes)
