...
- Created at: `2026-08-20T12:33:26.882604+00:00`
...
- geometry: blk1 / Block, blk2 / Block, wp1 / WorkPlane, r1 / Rectangle, r2 / Rectangle, geom1 / blk1 / size, geom1 / blk1 / pos, geom1 / blk2 / size, geom1 / wp1 / planetype, geom1 / wp1 / unite
- physics: Electromagnetic Waves
- mesh: mesh1
- studies: Frequency Domain
- boundary_conditions: blk1 / size, blk1 / pos, blk2 / size, wp1 / planetype, wp1 / unite, r1 / base, r2 / base

### 文件内容证据

- `gcpw_numeric_tem_port.java`: 参数=3, 几何=10, 物理场=0, 研究=0, 结果=0
- `models.rf.gcpw_numeric_tem_port.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `gcpw_numeric_tem_port.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `gcpw_numeric_tem_port.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `gcpw_numeric_tem_port_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: gcpw_numeric_tem_port 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- 已识别几何证据：blk1 / Block；blk2 / Block；wp1 / WorkPlane；r1 / Rectangle；r2 / Rectangle；geom1 / blk1 / size
...
- Summary: 已学习 gcpw_numeric_tem_port 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=10，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：gcpw_numeric_tem_port.m, gcpw_numeric_tem_port.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 gcpw_numeric_tem_port，训练阶段为 modeling_logic_learning_ready。
...
- `gcpw_numeric_tem_port.mph` (comsol_mph, 130230856 bytes)
- `gcpw_numeric_tem_port.m` (matlab_livelink, 26992 bytes)
- `gcpw_numeric_tem_port.java` (comsol_java, 25921 bytes)
- `gcpw_numeric_tem_port_summary.json` (json, 1147 bytes)
