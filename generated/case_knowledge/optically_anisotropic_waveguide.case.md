# optically_anisotropic_waveguide

- Case directory: `D:\桌面\codex\案例下载\电气\光学各向异性波导`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T16:10:19.079502+00:00`
...
- `height` = `4000[nm]` - Height of waveguide
- `n_core_o` = `sqrt(epsr_o)` - Core refractive index, ordinary
- `n_core_eo` = `sqrt(epsr_eo)` - Core refractive index, extraordinary
- `eps_clad` = `2.05` - Cladding relative permittivity
- `k0_1` = `2*pi/lda0` - Wavenumber in vacuum
- `theta` = `0[deg]` - x-y plane rotation
- `phi` = `45[deg]` - y-z plane rotation
- `zeta` = `90[deg]` - x-z plane rotation
- `epsr_eo` = `2.19` - Core relative permittivity, extraordinary
- `epsr_o` = `2.31` - Core relative permittivity, ordinary

## 内容级读取结果

- 模型树证据数量: 32
- 理论关键词: 电磁, 结构力学, 多孔介质/裂隙, 优化/参数扫描
- geometry: sq1 / Square, r1 / Rectangle, geom1 / sq1 / size, geom1 / sq1 / base, geom1 / r1 / size, geom1 / r1 / base
- physics: ewfd / ElectromagneticWavesFrequencyDomain, Electromagnetic Waves
- materials: mat1, mat2
- mesh: mesh1, size1, ftri1
- studies: Frequency Domain, Parametric Sweep
- boundary_conditions: size1 / 2, sq1 / size, sq1 / base, r1 / size, r1 / base

### 文件内容证据

- `optically_anisotropic_waveguide.java`: 参数=12, 几何=6, 物理场=1, 研究=0, 结果=0
- `models.woptics.optically_anisotropic_waveguide.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `optically_anisotropic_waveguide.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `optically_anisotropic_waveguide.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `optically_anisotropic_waveguide_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.optically_anisotropic_waveguide.pdf`: models.woptics.optically_anisotropic_waveguide.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；from 4 μm to 0.5 μm. For each geometry, the effective index is solved for and plotted in

## PDF Summary and Physics Judgement

- Simple summary: optically_anisotropic_waveguide 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Frequency Domain 研究。
...
- PDF: from 4 μm to 0.5 μm. For each geometry, the effective index is solved for and plotted in
- PDF: different steps of the parametric sweep. The results agree well with the results reported in
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: ewfd / ElectromagneticWavesFrequencyDomain, Electromagnetic Waves
...
- PDF keywords: 电磁, 结构力学, 多孔介质/裂隙, 优化/参数扫描, models.woptics.optically_anisotropic_waveguide.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；from 4 μm to 0.5 μm. For each geometry, the effective index is solved for and plotted in
...
- `height` -> Height of waveguide; MATLAB: `model.param.set('height', value, description)`
- `n_core_o` -> Core refractive index, ordinary; MATLAB: `model.param.set('n_core_o', value, description)`
- `n_core_eo` -> Core refractive index, extraordinary; MATLAB: `model.param.set('n_core_eo', value, description)`
- `eps_clad` -> Cladding relative permittivity; MATLAB: `model.param.set('eps_clad', value, description)`
- `k0_1` -> Wavenumber in vacuum; MATLAB: `model.param.set('k0_1', value, description)`
- `theta` -> x-y plane rotation; MATLAB: `model.param.set('theta', value, description)`
- `phi` -> y-z plane rotation; MATLAB: `model.param.set('phi', value, description)`
- `zeta` -> x-z plane rotation; MATLAB: `model.param.set('zeta', value, description)`
- `epsr_eo` -> Core relative permittivity, extraordinary; MATLAB: `model.param.set('epsr_eo', value, description)`
- `epsr_o` -> Core relative permittivity, ordinary; MATLAB: `model.param.set('epsr_o', value, description)`
...
- 已识别几何证据：sq1 / Square；r1 / Rectangle；geom1 / sq1 / size；geom1 / sq1 / base；geom1 / r1 / size；geom1 / r1 / base
- 已识别物理场证据：ewfd / ElectromagneticWavesFrequencyDomain；Electromagnetic Waves
- 已识别参数：lda0、f0、height、n_core_o、n_core_eo、eps_clad、k0_1、theta、phi、zeta、epsr_eo、epsr_o
...
- 识别到物理场证据：ewfd / ElectromagneticWavesFrequencyDomain
...
- Transfer recognized physics interfaces to similar requirements: ewfd / ElectromagneticWavesFrequencyDomain, Electromagnetic Waves.
...
- Sweep `lda0` around the learned value `1.55[um]` and export target outputs to CSV.
- Sweep `f0` around the learned value `c_const/lda0` and export target outputs to CSV.
- Sweep `height` around the learned value `4000[nm]` and export target outputs to CSV.
- Sweep `n_core_o` around the learned value `sqrt(epsr_o)` and export target outputs to CSV.
- Sweep `n_core_eo` around the learned value `sqrt(epsr_eo)` and export target outputs to CSV.
- Sweep `eps_clad` around the learned value `2.05` and export target outputs to CSV.
- Sweep `k0_1` around the learned value `2*pi/lda0` and export target outputs to CSV.
...
- Summary: 已学习 optically_anisotropic_waveguide 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=32 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 32 条；几何=6，物理场=2，材料=2，网格=3，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：optically_anisotropic_waveguide.m, optically_anisotropic_waveguide.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 optically_anisotropic_waveguide，训练阶段为 modeling_logic_learning_ready。
...
- `optically_anisotropic_waveguide.mph` (comsol_mph, 203183416 bytes)
- `optically_anisotropic_waveguide.m` (matlab_livelink, 29956 bytes)
- `optically_anisotropic_waveguide.java` (comsol_java, 27641 bytes)
- `optically_anisotropic_waveguide_summary.json` (json, 1194 bytes)
- `models.woptics.optically_anisotropic_waveguide.pdf` (pdf_document, 868404 bytes)
