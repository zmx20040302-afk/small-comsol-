...
- Case directory: `D:\桌面\codex\案例下载\电气\PIN 二极管中的辐射效应`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:02:35.349290+00:00`
...
- `L` = `300[um]` - Device length
- `V0` = `1000[V]` - Applied voltage
- `Nds` = `1e20[cm^-3]` - Surface concentration of N-plus and P-plus doping
- `dj` = `25[um]` - Junction depth of N-plus and P-plus doping
- `Npi` = `2e12[cm^-3]` - Intrinsic doping concentration
- `tau` = `100[us]` - Carrier lifetime
- `mup0` = `480[cm^2/V/s]` - Low field, low doping, hole mobility
- `mun0` = `1350[cm^2/V/s]` - Low field, low doping, electron mobility
- `cp` = `1` - Continuation parameter for mobility model
- `RadSi` = `0` - Radiation dose rate in Rad(Si)/s
- `t` = `0[s]` - Time parameter
- `tp` = `0.5[ns]` - Radiation pulse duration
- `gR` = `4.03e13*RadSi[1/cm^3/s]*pw1(t/tp)` - Generation rate due to radiation dose
- `index` = `1` - Parameter for solution indexing
- `area` = `1[mm^2]` - Cross-section area of the 1D model

## 内容级读取结果

- 模型树证据数量: 23
- 理论关键词: 传热, 电磁, 流体, 优化/参数扫描
- geometry: i1 / Interval, geom1 / i1 / coord
- physics: Electrostatics
- mesh: mesh1
- studies: Stationary, Time Dependent, Parametric Sweep
- boundary_conditions: i1 / coord

### 文件内容证据

- `pin_radiation_effects.java`: 参数=15, 几何=2, 物理场=0, 研究=0, 结果=0
- `models.semicond.pin_radiation_effects.pdf`: 参数=0, 几何=0, 物理场=1, 研究=3, 结果=0
- `pin_radiation_effects.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pin_radiation_effects.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pin_radiation_effects_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.pin_radiation_effects.pdf`: models.semicond.pin_radiation_effects.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；separated carriers, and charge transport with field-dependent mobilities, results in the

## PDF Summary and Physics Judgement

- Simple summary: pin_radiation_effects 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: separated carriers, and charge transport with field-dependent mobilities, results in the
- PDF: employed in the study settings with detailed discussions in the Modeling Instructions
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: Electrostatics
...
- PDF keywords: 传热, 电磁, 流体, 优化/参数扫描, models.semicond.pin_radiation_effects.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；separated carriers, and charge transport with field-dependent mobilities, results in the
- `L` -> Device length; MATLAB: `model.param.set('L', value, description)`
- `V0` -> Applied voltage; MATLAB: `model.param.set('V0', value, description)`
- `Nds` -> Surface concentration of N-plus and P-plus doping; MATLAB: `model.param.set('Nds', value, description)`
- `dj` -> Junction depth of N-plus and P-plus doping; MATLAB: `model.param.set('dj', value, description)`
- `Npi` -> Intrinsic doping concentration; MATLAB: `model.param.set('Npi', value, description)`
- `tau` -> Carrier lifetime; MATLAB: `model.param.set('tau', value, description)`
- `mup0` -> Low field, low doping, hole mobility; MATLAB: `model.param.set('mup0', value, description)`
- `mun0` -> Low field, low doping, electron mobility; MATLAB: `model.param.set('mun0', value, description)`
- `cp` -> Continuation parameter for mobility model; MATLAB: `model.param.set('cp', value, description)`
- `RadSi` -> Radiation dose rate in Rad(Si)/s; MATLAB: `model.param.set('RadSi', value, description)`
- `t` -> Time parameter; MATLAB: `model.param.set('t', value, description)`
- `tp` -> Radiation pulse duration; MATLAB: `model.param.set('tp', value, description)`
...
- 已识别几何证据：i1 / Interval；geom1 / i1 / coord
- 已识别物理场证据：Electrostatics
- 已识别参数：L、V0、Nds、dj、Npi、tau、mup0、mun0、cp、RadSi、t、tp
...
- Transfer recognized physics interfaces to similar requirements: Electrostatics.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 电磁, 流体, 优化/参数扫描?
...
- Sweep `L` around the learned value `300[um]` and export target outputs to CSV.
- Sweep `V0` around the learned value `1000[V]` and export target outputs to CSV.
- Sweep `Nds` around the learned value `1e20[cm^-3]` and export target outputs to CSV.
- Sweep `dj` around the learned value `25[um]` and export target outputs to CSV.
- Sweep `Npi` around the learned value `2e12[cm^-3]` and export target outputs to CSV.
- Sweep `tau` around the learned value `100[us]` and export target outputs to CSV.
- Sweep `mup0` around the learned value `480[cm^2/V/s]` and export target outputs to CSV.
- Sweep `mun0` around the learned value `1350[cm^2/V/s]` and export target outputs to CSV.
...
- Summary: 已学习 pin_radiation_effects 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=15；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=2，物理场=1，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pin_radiation_effects.m, pin_radiation_effects.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pin_radiation_effects，训练阶段为 modeling_logic_learning_ready。
...
- `pin_radiation_effects.mph` (comsol_mph, 22172644 bytes)
- `pin_radiation_effects.m` (matlab_livelink, 91613 bytes)
- `pin_radiation_effects.java` (comsol_java, 87605 bytes)
- `pin_radiation_effects_summary.json` (json, 1595 bytes)
- `models.semicond.pin_radiation_effects.pdf` (pdf_document, 1369600 bytes)
