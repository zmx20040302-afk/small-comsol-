...
- `n` = `0.47` - 0.47
- `E0` = `1[V/m]`
- `Pinc` = `E0^2/2/Z0_const`

## 内容级读取结果

- 模型树证据数量: 32
- 理论关键词: 传热, 电磁
- geometry: sph1 / Sphere, cyl1 / Cylinder, dif1 / Difference, geom1 / sph1 / r, geom1 / sph1 / layername, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / pos, geom1 / cyl1 / layername, geom1 / cyl1 / layerside, geom1 / cyl1 / layerbottom, geom1 / cyl1 / layertop
- physics: Electromagnetic Waves
- mesh: mesh1
- studies: Frequency Domain
- boundary_conditions: sph1 / r, sph1 / layername, cyl1 / r, cyl1 / h, cyl1 / pos, cyl1 / layername, cyl1 / layerside, cyl1 / layerbottom, cyl1 / layertop

### 文件内容证据

- `half_wavelength_dipole_antenna_63.m`: 参数=8, 几何=12, 物理场=0, 研究=0, 结果=0
- `half_wavelength_dipole_antenna_63.java`: 参数=8, 几何=12, 物理场=0, 研究=0, 结果=0
- `rf_to_beam_envelopes.pptx`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `half_wavelength_dipole_antenna.pptx`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `half_wavelength_dipole_antenna_63.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `half_wavelength_dipole_antenna_63_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: half_wavelength_dipole_antenna_63 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- `g` -> 案例参数/扫描变量; MATLAB: `model.param.set('g', value, description)`
- `n` -> 0.47; MATLAB: `model.param.set('n', value, description)`
- `E0` -> 案例参数/扫描变量; MATLAB: `model.param.set('E0', value, description)`
- `Pinc` -> 案例参数/扫描变量; MATLAB: `model.param.set('Pinc', value, description)`
...
- 已识别几何证据：sph1 / Sphere；cyl1 / Cylinder；dif1 / Difference；geom1 / sph1 / r；geom1 / sph1 / layername；geom1 / cyl1 / r
- 已识别物理场证据：Electromagnetic Waves
- 已识别参数：f0、wl0、l、d、g、n、E0、Pinc
...
- Sweep `l` around the learned value `n*wl0` and export target outputs to CSV.
- Sweep `d` around the learned value `l/100` and export target outputs to CSV.
- Sweep `g` around the learned value `d` and export target outputs to CSV.
- Sweep `n` around the learned value `0.47` and export target outputs to CSV.
- Sweep `E0` around the learned value `1[V/m]` and export target outputs to CSV.
- Sweep `Pinc` around the learned value `E0^2/2/Z0_const` and export target outputs to CSV.
...
- Summary: 已学习 half_wavelength_dipole_antenna_63 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=32 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：half_wavelength_dipole_antenna_63.m, half_wavelength_dipole_antenna_63.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 half_wavelength_dipole_antenna_63，训练阶段为 modeling_logic_learning_ready。
...
- `half_wavelength_dipole_antenna_63.mph` (comsol_mph, 92745394 bytes)
- `half_wavelength_dipole_antenna_63.m` (matlab_livelink, 40267 bytes)
- `half_wavelength_dipole_antenna_63.java` (comsol_java, 42828 bytes)
- `half_wavelength_dipole_antenna_63_summary.json` (json, 1310 bytes)
