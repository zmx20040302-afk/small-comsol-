# piezoelectric_rate_gyroscope_geom_sequence

- Case directory: `D:\桌面\codex\案例下载\电气\压电速率陀螺仪`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:38:19.018055+00:00`
...
- `tQz` = `0.5[mm]` - Quartz thickness
- `w_m` = `1[mm]` - Mount width
- `l_w` = `(l_f-w_m)/2-w_tbf` - Wing length
- `w_w` = `0.25[mm]` - Wing width
- `l_f` = `4[mm]` - Frame length
- `w_f` = `3[mm]` - Frame width
- `w_tbf` = `0.4[mm]` - Frame top/bottom thickness
- `w_sf` = `0.25[mm]` - Frame side thickness
- `l_d` = `6[mm]` - Drive tine length
- `w_d` = `0.4[mm]` - Drive tine width
- `l_s` = `5.5[mm]` - Sense tine length
- `w_s` = `0.4[mm]` - Sense tine width
- `gap` = `1[mm]` - Gap between tines

## 内容级读取结果

- 模型树证据数量: 33
- 理论关键词: 电磁, 结构力学, 流体, 声学, 优化/参数扫描
- geometry: wp1 / WorkPlane, r1 / Rectangle, r2 / Rectangle, dif1 / Difference, sq1 / Square, geom1 / wp1 / quickz, geom1 / wp1 / unite
- physics: Solid Mechanics, Electrostatics
- mesh: mesh1
- studies: Eigenfrequency, Frequency Domain
- boundary_conditions: wp1 / quickz, wp1 / unite, r1 / size, r1 / base, r2 / size, r2 / base, sq1 / size, sq1 / base

### 文件内容证据

- `piezoelectric_rate_gyroscope_geom_sequence.m`: 参数=13, 几何=7, 物理场=0, 研究=0, 结果=0
- `piezoelectric_rate_gyroscope_geom_sequence.java`: 参数=13, 几何=7, 物理场=0, 研究=0, 结果=0
- `models.mems.piezoelectric_rate_gyroscope.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `piezoelectric_rate_gyroscope_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoelectric_rate_gyroscope_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.piezoelectric_rate_gyroscope.pdf`: models.mems.piezoelectric_rate_gyroscope.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Eigenfrequency、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；coupled to an out of plane mode by the Coriolis force and the resulting out of plane

## PDF Summary and Physics Judgement

- Simple summary: piezoelectric_rate_gyroscope_geom_sequence 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenfrequency 研究。
...
- PDF: coupled to an out of plane mode by the Coriolis force and the resulting out of plane
- PDF: motion is sensed by the direct piezoelectric effect. The geometry of the tuning forks is
- PDF: Figure 1 shows the geometry of the device indicating the key features.
- 脚本/文本识别到的物理场: Solid Mechanics, Electrostatics
- 识别到的研究类型: Eigenfrequency, Frequency Domain
- 理论关键词: 电磁, 结构力学, 流体, 声学, 优化/参数扫描

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 电磁, 结构力学, 流体, 声学, 优化/参数扫描, models.mems.piezoelectric_rate_gyroscope.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Eigenfrequency、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；coupled to an out of plane mode by the Coriolis force and the resulting out of plane
- `tQz` -> Quartz thickness; MATLAB: `model.param.set('tQz', value, description)`
- `w_m` -> Mount width; MATLAB: `model.param.set('w_m', value, description)`
- `l_w` -> Wing length; MATLAB: `model.param.set('l_w', value, description)`
- `w_w` -> Wing width; MATLAB: `model.param.set('w_w', value, description)`
- `l_f` -> Frame length; MATLAB: `model.param.set('l_f', value, description)`
- `w_f` -> Frame width; MATLAB: `model.param.set('w_f', value, description)`
- `w_tbf` -> Frame top/bottom thickness; MATLAB: `model.param.set('w_tbf', value, description)`
- `w_sf` -> Frame side thickness; MATLAB: `model.param.set('w_sf', value, description)`
- `l_d` -> Drive tine length; MATLAB: `model.param.set('l_d', value, description)`
- `w_d` -> Drive tine width; MATLAB: `model.param.set('w_d', value, description)`
- `l_s` -> Sense tine length; MATLAB: `model.param.set('l_s', value, description)`
- `w_s` -> Sense tine width; MATLAB: `model.param.set('w_s', value, description)`
...
- 已识别几何证据：wp1 / WorkPlane；r1 / Rectangle；r2 / Rectangle；dif1 / Difference；sq1 / Square；geom1 / wp1 / quickz
- 已识别物理场证据：Solid Mechanics；Electrostatics
- 已识别参数：tQz、w_m、l_w、w_w、l_f、w_f、w_tbf、w_sf、l_d、w_d、l_s、w_s
...
- Use the learned study types as solver starting points: Eigenfrequency, Frequency Domain.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 结构力学, 流体, 声学, 优化/参数扫描?
...
- Sweep `tQz` around the learned value `0.5[mm]` and export target outputs to CSV.
- Sweep `w_m` around the learned value `1[mm]` and export target outputs to CSV.
- Sweep `l_w` around the learned value `(l_f-w_m)/2-w_tbf` and export target outputs to CSV.
- Sweep `w_w` around the learned value `0.25[mm]` and export target outputs to CSV.
- Sweep `l_f` around the learned value `4[mm]` and export target outputs to CSV.
- Sweep `w_f` around the learned value `3[mm]` and export target outputs to CSV.
- Sweep `w_tbf` around the learned value `0.4[mm]` and export target outputs to CSV.
- Sweep `w_sf` around the learned value `0.25[mm]` and export target outputs to CSV.
...
- Summary: 已学习 piezoelectric_rate_gyroscope_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=13；内容证据=33 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 33 条；几何=7，物理场=2，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：piezoelectric_rate_gyroscope_geom_sequence.m, piezoelectric_rate_gyroscope_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 piezoelectric_rate_gyroscope_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `piezoelectric_rate_gyroscope_geom_sequence.mph` (comsol_mph, 756039 bytes)
- `piezoelectric_rate_gyroscope_geom_sequence.m` (matlab_livelink, 17535 bytes)
- `piezoelectric_rate_gyroscope_geom_sequence.java` (comsol_java, 18594 bytes)
- `piezoelectric_rate_gyroscope_geom_sequence_summary.json` (json, 964 bytes)
- `models.mems.piezoelectric_rate_gyroscope.pdf` (pdf_document, 907889 bytes)
