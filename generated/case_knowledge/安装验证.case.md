...
- Created at: `2026-07-06T09:38:40.697988+00:00`
...
- `applications.installation_verification.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `applications.installation_verification.zh_CN.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `installation_verification.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `installation_verification.zh_CN.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `applications.installation_verification.pdf`: applications.installation_verification.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `applications.installation_verification.zh_CN.pdf`: applications.installation_verification.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 安装验证 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- Summary: 已学习 安装验证 的 4 个文件；训练阶段=needs_comsol_sweep_csv；参数数量=0；内容证据=0 条。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 安装验证，训练阶段为 needs_comsol_sweep_csv。
...
- `管式反应器代理模型 App` score=0.1824; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `集群设置验证` score=0.1514; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=1
- `热执行器代理模型 App` score=0.1352; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42

### Official Documentation Checks

- `LiveLinkForMATLABUsersGuide.pdf` page 198; module=LiveLink_for_MATLAB; score=0.1171
- `LiveLinkForSimulinkUsersGuide.pdf` page 20; module=LiveLink_for_Simulink; score=0.1153
- `COMSOL_ProgrammingReferenceManual.pdf` page 1196; module=COMSOL_Multiphysics; score=0.1115
