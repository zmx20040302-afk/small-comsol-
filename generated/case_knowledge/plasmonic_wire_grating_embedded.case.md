# plasmonic_wire_grating_embedded

- Case directory: `D:\桌面\codex\案例下载\电气\表面等离激元线光栅（波动光学）`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:24:33.903539+00:00`
...
- `nb` = `1.2` - Refractive index, substrate
- `d` = `400[nm]` - Grating constant
- `lam0` = `441[nm]` - Vacuum wavelength
- `f0` = `c_const/lam0` - Frequency
- `alpha_deg` = `45` - Angle of incidence, in degrees
- `alpha` = `alpha_deg[deg]` - Angle of incidence
- `beta` = `asin(na*sin(alpha)/nb)` - Refraction angle
- `r_wire` = `80[nm]` - Wire radius
- `epsilon_prime` = `-1.75` - Relative permittivity, real part
- `epsilon_dprime` = `5.4` - Relative permittivity, imaginary part

## 内容级读取结果

- 模型树证据数量: 33
- 理论关键词: 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, c1 / Circle, uni1 / Union, del1 / Delete, geom1 / r1 / size, geom1 / r2 / size, geom1 / r2 / pos, geom1 / c1 / r, geom1 / c1 / pos, wp1 / WorkPlane, geom2 / wp1 / unite
- physics: Electromagnetic Waves
- mesh: mesh1, mesh2
- studies: Frequency Domain
- boundary_conditions: r1 / size, r2 / size, r2 / pos, c1 / r, c1 / pos, wp1 / unite

### 文件内容证据

- `plasmonic_wire_grating_embedded.java`: 参数=11, 几何=12, 物理场=0, 研究=0, 结果=0
- `plasmonic_wire_grating_embedded.m`: 参数=11, 几何=10, 物理场=0, 研究=0, 结果=0
- `models.woptics.plasmonic_wire_grating.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `applications.plasmonic_wire_grating.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `plasmonic_wire_grating_embedded.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `plasmonic_wire_grating_embedded_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `applications.plasmonic_wire_grating.pdf`: applications.plasmonic_wire_grating.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁；核心内容可概括为：Application created in COMSOL Multiphysics 6.4；of four sections: Geometry Parameters, Material Properties, Wave Properties, and
- `models.woptics.plasmonic_wire_grating.pdf`: models.woptics.plasmonic_wire_grating.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Figure 1: The modeled grating. The model considers a unit cell of a slice through this geometry.

## PDF Summary and Physics Judgement

- Simple summary: plasmonic_wire_grating_embedded 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electromagnetic Waves，并采用 Frequency Domain 研究。
- Primary physics: `Electromagnetic Waves`
- Study type: `Frequency Domain`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Electromagnetic Waves，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Frequency Domain 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: Figure 1: The modeled grating. The model considers a unit cell of a slice through this geometry.
- PDF: Figure 2: The geometric path lengths of two transmitted parallel beams. The optical path
- PDF: length is the geometric path length multiplied by the local refractive index.
...
- PDF keywords: 电磁, applications.plasmonic_wire_grating.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁；核心内容可概括为：Application created in COMSOL Multiphysics 6.4；of four sections: Geometry Parameters, Material Properties, Wave Properties, and, models.woptics.plasmonic_wire_grating.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Figure 1: The modeled grating. The model considers a unit cell of a slice through this geometry.
- `na` -> Refractive index, air; MATLAB: `model.param.set('na', value, description)`
- `nb` -> Refractive index, substrate; MATLAB: `model.param.set('nb', value, description)`
- `d` -> Grating constant; MATLAB: `model.param.set('d', value, description)`
- `lam0` -> Vacuum wavelength; MATLAB: `model.param.set('lam0', value, description)`
- `f0` -> Frequency; MATLAB: `model.param.set('f0', value, description)`
- `alpha_deg` -> Angle of incidence, in degrees; MATLAB: `model.param.set('alpha_deg', value, description)`
- `alpha` -> Angle of incidence; MATLAB: `model.param.set('alpha', value, description)`
- `beta` -> Refraction angle; MATLAB: `model.param.set('beta', value, description)`
- `r_wire` -> Wire radius; MATLAB: `model.param.set('r_wire', value, description)`
- `epsilon_prime` -> Relative permittivity, real part; MATLAB: `model.param.set('epsilon_prime', value, description)`
- `epsilon_dprime` -> Relative permittivity, imaginary part; MATLAB: `model.param.set('epsilon_dprime', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；c1 / Circle；uni1 / Union；del1 / Delete；geom1 / r1 / size
- 已识别物理场证据：Electromagnetic Waves
- 已识别参数：na、nb、d、lam0、f0、alpha_deg、alpha、beta、r_wire、epsilon_prime、epsilon_dprime
...
- Sweep `na` around the learned value `1` and export target outputs to CSV.
- Sweep `nb` around the learned value `1.2` and export target outputs to CSV.
- Sweep `d` around the learned value `400[nm]` and export target outputs to CSV.
- Sweep `lam0` around the learned value `441[nm]` and export target outputs to CSV.
- Sweep `f0` around the learned value `c_const/lam0` and export target outputs to CSV.
- Sweep `alpha_deg` around the learned value `45` and export target outputs to CSV.
- Sweep `alpha` around the learned value `alpha_deg[deg]` and export target outputs to CSV.
- Sweep `beta` around the learned value `asin(na*sin(alpha)/nb)` and export target outputs to CSV.
...
- Summary: 已学习 plasmonic_wire_grating_embedded 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=33 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 33 条；几何=12，物理场=1，材料=0，网格=2，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：plasmonic_wire_grating_embedded.m, plasmonic_wire_grating_embedded.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 plasmonic_wire_grating_embedded，训练阶段为 modeling_logic_learning_ready。
...
- `plasmonic_wire_grating_embedded.mph` (comsol_mph, 16671658 bytes)
- `plasmonic_wire_grating_embedded.m` (matlab_livelink, 44853 bytes)
- `plasmonic_wire_grating_embedded.java` (comsol_java, 48150 bytes)
- `plasmonic_wire_grating_embedded_summary.json` (json, 2053 bytes)
- `applications.plasmonic_wire_grating.pdf` (pdf_document, 282727 bytes)
- `models.woptics.plasmonic_wire_grating.pdf` (pdf_document, 816106 bytes)
