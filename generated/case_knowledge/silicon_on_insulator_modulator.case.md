# silicon_on_insulator_modulator

- Case directory: `D:\桌面\codex\案例下载\电气\绝缘体上硅调制器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:24:02.893521+00:00`
...
- `w_pplus` = `600[nm]`
- `w_nplus` = `600[nm]`
- `w_slab` = `w_nplusplus+w_pplusplus+w_wg+w_nplus+w_pplus`
- `h_box` = `1.7[um]`
- `l_PIN` = `1[mm]`
- `lam0` = `1.55[um]`
- `nSi0` = `3.4757`
- `alpha0` = `0.025[1/cm]`
- `k0` = `(lam0*alpha0*100)/(4*pi)`
- `nplusplus` = `5e20[1/cm^3]`
- `nplus` = `2e18[1/cm^3]`
- `pplus` = `5e17[1/cm^3]`
- `pplusplus` = `5e20[1/cm^3]`

## 内容级读取结果

- 模型树证据数量: 42
- 理论关键词: 电磁, 结构力学
- geometry: r1 / Rectangle, r2 / Rectangle, geom1 / r1 / size, geom1 / r1 / base, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r2 / base, geom1 / r2 / pos
- physics: semi / Semiconductor, ewfd / ElectromagneticWavesFrequencyDomain, Electromagnetic Waves
- mesh: mesh1
- studies: std1 / stat / Stationary, Stationary, std1, Frequency Domain
- boundary_conditions: r1 / size, r1 / base, r1 / pos, r2 / size, r2 / base, r2 / pos

### 文件内容证据

- `silicon_on_insulator_modulator.java`: 参数=20, 几何=8, 物理场=2, 研究=3, 结果=0
- `silicon_on_insulator_modulator.m`: 参数=20, 几何=5, 物理场=2, 研究=2, 结果=0
- `models.semicond.silicon_on_insulator_modulator.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `silicon_on_insulator_modulator.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `silicon_on_insulator_modulator_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.silicon_on_insulator_modulator.pdf`: models.semicond.silicon_on_insulator_modulator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；A cross-section view of the SoI modulator is shown in Figure 1, where the geometry and

## PDF Summary and Physics Judgement

- Simple summary: silicon_on_insulator_modulator 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF: A cross-section view of the SoI modulator is shown in Figure 1, where the geometry and
- PDF: Figure 1: SoI modulator model geometry and net dopant concentration profile.
- PDF: Users interested in the mode analysis study for electromagnetic waveguides may find the
- 脚本/文本识别到的物理场: semi / Semiconductor, ewfd / ElectromagneticWavesFrequencyDomain, Electromagnetic Waves
- 识别到的研究类型: std1 / stat / Stationary, Stationary, std1, Frequency Domain
- 理论关键词: 电磁, 结构力学

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 电磁, 结构力学, models.semicond.silicon_on_insulator_modulator.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；A cross-section view of the SoI modulator is shown in Figure 1, where the geometry and
- `Va` -> 案例参数/扫描变量; MATLAB: `model.param.set('Va', value, description)`
- `Vc` -> 案例参数/扫描变量; MATLAB: `model.param.set('Vc', value, description)`
- `w_wg` -> 案例参数/扫描变量; MATLAB: `model.param.set('w_wg', value, description)`
- `h_wg` -> 案例参数/扫描变量; MATLAB: `model.param.set('h_wg', value, description)`
- `h_line` -> 案例参数/扫描变量; MATLAB: `model.param.set('h_line', value, description)`
- `w_nplusplus` -> 案例参数/扫描变量; MATLAB: `model.param.set('w_nplusplus', value, description)`
- `w_pplusplus` -> 案例参数/扫描变量; MATLAB: `model.param.set('w_pplusplus', value, description)`
- `w_pplus` -> 案例参数/扫描变量; MATLAB: `model.param.set('w_pplus', value, description)`
- `w_nplus` -> 案例参数/扫描变量; MATLAB: `model.param.set('w_nplus', value, description)`
- `w_slab` -> 案例参数/扫描变量; MATLAB: `model.param.set('w_slab', value, description)`
- `h_box` -> 案例参数/扫描变量; MATLAB: `model.param.set('h_box', value, description)`
- `l_PIN` -> 案例参数/扫描变量; MATLAB: `model.param.set('l_PIN', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；geom1 / r1 / size；geom1 / r1 / base；geom1 / r1 / pos；geom1 / r2 / size
- 已识别物理场证据：semi / Semiconductor；ewfd / ElectromagneticWavesFrequencyDomain；Electromagnetic Waves
- 已识别参数：Va、Vc、w_wg、h_wg、h_line、w_nplusplus、w_pplusplus、w_pplus、w_nplus、w_slab、h_box、l_PIN
...
- Transfer recognized physics interfaces to similar requirements: semi / Semiconductor, ewfd / ElectromagneticWavesFrequencyDomain, Electromagnetic Waves.
- Reuse geometry construction patterns when new requirements share shape, import, array, sweep, or boolean operations.
- Use the learned study types as solver starting points: std1 / stat / Stationary, Stationary, std1, Frequency Domain.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 结构力学?
...
- Sweep `Va` around the learned value `1[V]` and export target outputs to CSV.
- Sweep `Vc` around the learned value `0[V]` and export target outputs to CSV.
- Sweep `w_wg` around the learned value `400[nm]` and export target outputs to CSV.
- Sweep `h_wg` around the learned value `150[nm]` and export target outputs to CSV.
- Sweep `h_line` around the learned value `120[nm]` and export target outputs to CSV.
- Sweep `w_nplusplus` around the learned value `600[nm]` and export target outputs to CSV.
- Sweep `w_pplusplus` around the learned value `600[nm]` and export target outputs to CSV.
- Sweep `w_pplus` around the learned value `600[nm]` and export target outputs to CSV.
...
- Summary: 已学习 silicon_on_insulator_modulator 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=20；内容证据=42 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 42 条；几何=8，物理场=3，材料=0，网格=1，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：silicon_on_insulator_modulator.m, silicon_on_insulator_modulator.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 silicon_on_insulator_modulator，训练阶段为 modeling_logic_learning_ready。
...
- `silicon_on_insulator_modulator.mph` (comsol_mph, 59945772 bytes)
- `silicon_on_insulator_modulator.m` (matlab_livelink, 43025 bytes)
- `silicon_on_insulator_modulator.java` (comsol_java, 45694 bytes)
- `silicon_on_insulator_modulator_summary.json` (json, 1837 bytes)
- `models.semicond.silicon_on_insulator_modulator.pdf` (pdf_document, 1476952 bytes)
