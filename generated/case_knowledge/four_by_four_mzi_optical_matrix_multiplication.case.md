# four_by_four_mzi_optical_matrix_multiplication

- Case directory: `D:\桌面\codex\案例下载\电气\基于 MZI 网络的光学酉矩阵乘法装置`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:08:14.952135+00:00`
...
- `lda` = `1.55[um]` - Free space wavelength
- `n_core` = `2.5` - Refractive index of waveguide core
- `n_clad` = `1.5` - Refractive index of waveguide cladding
- `w_core` = `0.2[um]` - Core width
- `w_clad` = `2[um]` - Cladding width
- `s_core` = `0.335[um]` - Coupler core seperation
- `s_port` = `10[um]` - Port core seperation
- `L_s` = `5[um]` - Coupler length
- `L_e` = `50[um]` - Element length
- `L_w` = `10[um]` - Straight waveguide length
- `L_MZI` = `L_w*3+L_e*2` - Total length of a MZI
- `E1` = `1` - 1st input amplitude
- `E2` = `2` - 2nd input amplitude
- `E3` = `3` - 3rd input amplitude
- `E4` = `2` - 4th input amplitude
- `delta_n_1` = `0` - Refractive index change of the 1st phase shifter
- `delta_n_2` = `0` - Refractive index change of the 2nd phase shifter
- `delta_n_3` = `0` - Refractive index change of the 3rd phase shifter
- `delta_n_4` = `0` - Refractive index change of the 4th phase shifter
- `delta_n_5` = `0` - Refractive index change of the 5th phase shifter
- `delta_n_6` = `0` - Refractive index change of the 6th phase shifter
- `delta_n_7` = `0` - Refractive index change of the 7th phase shifter
- `delta_n_8` = `0` - Refractive index change of the 8th phase shifter
- `delta_n_9` = `0` - Refractive index change of the 9th phase shifter
- `delta_n_10` = `0` - Refractive index change of the 10th phase shifter
- `delta_n_11` = `0` - Refractive index change of the 11th phase shifter
- `delta_n_12` = `0` - Refractive index change of the 12th phase shifter
- `delta_n_13` = `0` - Refractive index change of the 13th phase shifter
- `delta_n_14` = `0` - Refractive index change of the 14th phase shifter
- `delta_n_15` = `0` - Refractive index change of the 15th phase shifter
- `delta_n_16` = `0` - Refractive index change of the 16th phase shifter
- `delta_n_17` = `0` - Refractive index change of the 17th phase shifter
- `delta_n_18` = `0` - Refractive index change of the 18th phase shifter
...
- `four_by_four_mzi_optical_matrix_multiplication.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `four_by_four_mzi_optical_matrix_multiplication.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `four_by_four_mzi_optical_matrix_multiplication.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `four_by_four_mzi_optical_matrix_multiplication_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: four_by_four_mzi_optical_matrix_multiplication 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `lda` -> Free space wavelength; MATLAB: `model.param.set('lda', value, description)`
- `n_core` -> Refractive index of waveguide core; MATLAB: `model.param.set('n_core', value, description)`
- `n_clad` -> Refractive index of waveguide cladding; MATLAB: `model.param.set('n_clad', value, description)`
- `w_core` -> Core width; MATLAB: `model.param.set('w_core', value, description)`
- `w_clad` -> Cladding width; MATLAB: `model.param.set('w_clad', value, description)`
- `s_core` -> Coupler core seperation; MATLAB: `model.param.set('s_core', value, description)`
- `s_port` -> Port core seperation; MATLAB: `model.param.set('s_port', value, description)`
- `L_s` -> Coupler length; MATLAB: `model.param.set('L_s', value, description)`
- `L_e` -> Element length; MATLAB: `model.param.set('L_e', value, description)`
- `L_w` -> Straight waveguide length; MATLAB: `model.param.set('L_w', value, description)`
- `L_MZI` -> Total length of a MZI; MATLAB: `model.param.set('L_MZI', value, description)`
- `E1` -> 1st input amplitude; MATLAB: `model.param.set('E1', value, description)`
...
- 已识别参数：lda、n_core、n_clad、w_core、w_clad、s_core、s_port、L_s、L_e、L_w、L_MZI、E1
...
- Sweep `lda` around the learned value `1.55[um]` and export target outputs to CSV.
- Sweep `n_core` around the learned value `2.5` and export target outputs to CSV.
- Sweep `n_clad` around the learned value `1.5` and export target outputs to CSV.
- Sweep `w_core` around the learned value `0.2[um]` and export target outputs to CSV.
- Sweep `w_clad` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `s_core` around the learned value `0.335[um]` and export target outputs to CSV.
- Sweep `s_port` around the learned value `10[um]` and export target outputs to CSV.
- Sweep `L_s` around the learned value `5[um]` and export target outputs to CSV.
...
- Summary: 已学习 four_by_four_mzi_optical_matrix_multiplication 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=33；内容证据=30 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：four_by_four_mzi_optical_matrix_multiplication.m, four_by_four_mzi_optical_matrix_multiplication.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 four_by_four_mzi_optical_matrix_multiplication，训练阶段为 modeling_logic_learning_ready。
...
- `four_by_four_mzi_optical_matrix_multiplication.mph` (comsol_mph, 333077943 bytes)
- `four_by_four_mzi_optical_matrix_multiplication.m` (matlab_livelink, 107899 bytes)
- `four_by_four_mzi_optical_matrix_multiplication.java` (comsol_java, 111416 bytes)
- `four_by_four_mzi_optical_matrix_multiplication_summary.json` (json, 2353 bytes)
