...
- Case directory: `D:\桌面\codex\案例下载\电气\以 Brewster 角入射的高斯光束`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T03:09:56.548523+00:00`
...
- `w0` = `10*lda0` - Spot radius
- `z0` = `pi*w0^2/lda0` - Rayleigh range
- `alpha` = `atan(n2)` - Angle of incidence (at Brewster angle)
- `a` = `6*w0` - Width
- `b` = `n2*a` - Propagation length
- `n2` = `1.5` - Refractive index
- `alpha2` = `asin(sin(alpha)/n2)` - Refracted angle
- `k0` = `2*pi/lda0` - Wave number in air
- `k1x_air` = `k0` - Wave vector in air, first wave, x-component
- `k1y_air` = `0[1/m]` - Wave vector in air, first wave, y-component
- `k2x_air` = `-k0*cos(2*alpha)` - Wave vector in air, second wave, x-component
- `k2y_air` = `-k0*sin(2*alpha)` - Wave vector in air, second wave, y-component
- `k1x_glass` = `k0*n2*cos(alpha-alpha2)` - Wave vector in glass, first wave, x-component
- `k1y_glass` = `k0*n2*sin(alpha-alpha2)` - Wave vector in glass, first wave, y-component
- `k2x_glass` = `-k0*n2*cos(alpha+alpha2)` - Wave vector in glass, second wave, x-component
- `k2y_glass` = `-k0*n2*sin(alpha+alpha2)` - Wave vector in glass, second wave, y-component

## 内容级读取结果

- 模型树证据数量: 25
- 理论关键词: 电磁
- geometry: r1 / Rectangle, ls1 / LineSegment, geom1 / r1 / size
...
- `brewster_interface.java`: 参数=18, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.woptics.brewster_interface.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `brewster_interface.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `brewster_interface.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `brewster_interface_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.brewster_interface.pdf`: models.woptics.brewster_interface.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Equation 1 results in the Brewster angle definition

## PDF Summary and Physics Judgement

- Simple summary: brewster_interface 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electromagnetic Waves，并采用 Frequency Domain 研究。
...
- PDF: Equation 1 results in the Brewster angle definition
- PDF: Results and Discussion
- PDF: First, the results are computed for s-polarization, where the polarization is orthogonal to
...
- PDF keywords: 电磁, models.woptics.brewster_interface.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Equation 1 results in the Brewster angle definition
...
- `alpha` -> Angle of incidence (at Brewster angle); MATLAB: `model.param.set('alpha', value, description)`
- `a` -> Width; MATLAB: `model.param.set('a', value, description)`
- `b` -> Propagation length; MATLAB: `model.param.set('b', value, description)`
- `n2` -> Refractive index; MATLAB: `model.param.set('n2', value, description)`
- `alpha2` -> Refracted angle; MATLAB: `model.param.set('alpha2', value, description)`
- `k0` -> Wave number in air; MATLAB: `model.param.set('k0', value, description)`
- `k1x_air` -> Wave vector in air, first wave, x-component; MATLAB: `model.param.set('k1x_air', value, description)`
- `k1y_air` -> Wave vector in air, first wave, y-component; MATLAB: `model.param.set('k1y_air', value, description)`
...
- 已识别几何证据：r1 / Rectangle；ls1 / LineSegment；geom1 / r1 / size
- 已识别物理场证据：Electromagnetic Waves
- 已识别参数：lda0、f0、w0、z0、alpha、a、b、n2、alpha2、k0、k1x_air、k1y_air
...
- Sweep `w0` around the learned value `10*lda0` and export target outputs to CSV.
- Sweep `z0` around the learned value `pi*w0^2/lda0` and export target outputs to CSV.
- Sweep `alpha` around the learned value `atan(n2)` and export target outputs to CSV.
- Sweep `a` around the learned value `6*w0` and export target outputs to CSV.
- Sweep `b` around the learned value `n2*a` and export target outputs to CSV.
- Sweep `n2` around the learned value `1.5` and export target outputs to CSV.
...
- Summary: 已学习 brewster_interface 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=18；内容证据=25 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 25 条；几何=3，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：brewster_interface.m, brewster_interface.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 brewster_interface，训练阶段为 modeling_logic_learning_ready。
...
- `brewster_interface.mph` (comsol_mph, 31168149 bytes)
- `brewster_interface.m` (matlab_livelink, 28342 bytes)
- `brewster_interface.java` (comsol_java, 25769 bytes)
- `brewster_interface_summary.json` (json, 1479 bytes)
- `models.woptics.brewster_interface.pdf` (pdf_document, 1529888 bytes)
