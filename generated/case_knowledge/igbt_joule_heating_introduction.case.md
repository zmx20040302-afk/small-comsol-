# igbt_joule_heating_introduction

- Case directory: `D:\桌面\codex\案例下载\电气\高功率 IGBT 模块的热分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:46:47.306374+00:00`
...
- `I_col` = `1.8[kA]` - Rated collector current (entire module)
- `th_met` = `4[um]` - Metalization layer thickness (diode and IGBT die)
- `th_igbt` = `140[um]` - IGBT die thickness
- `h_air` = `50[W/(m^2*K)]` - Heat transfer coefficient, air (forced convection)
- `T_air` = `60[degC]` - Ambient temperature, air (forced convection)
- `T_trm` = `70[degC]` - Ambient temperature, terminals (conduction)

## 内容级读取结果

- 模型树证据数量: 20
- 理论关键词: 传热, 电磁, 结构力学
- geometry: imp1 / Import, geom1 / imp1 / type, geom1 / imp1 / filename, geom1 / imp1 / selcadshownamesfromfileobj, geom1 / imp1 / selcadshownamesfromfilebnd
- physics: Heat Transfer, Electric Currents, Global ODEs and DAEs
- mesh: mesh1
- studies: Stationary
- boundary_conditions: imp1 / type, imp1 / filename, imp1 / selcadshownamesfromfileobj, imp1 / selcadshownamesfromfilebnd

### 文件内容证据

- `igbt_joule_heating_introduction.java`: 参数=6, 几何=5, 物理场=1, 研究=0, 结果=0
- `models.acdc.igbt_joule_heating.pdf`: 参数=0, 几何=0, 物理场=3, 研究=1, 结果=0
- `igbt_joule_heating.pptx`: 参数=0, 几何=0, 物理场=2, 研究=0, 结果=0
- `models.acdc.igbt_joule_heating_introduction.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `igbt_joule_heating_introduction.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `igbt_joule_heating_introduction.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `igbt_joule_heating_introduction_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.igbt_joule_heating.pdf`: models.acdc.igbt_joule_heating.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Electric Currents、Global ODEs and DAEs，研究类型线索为 Stationary；核心内容可概括为：Transfer in Solids physics interfaces, respectively. To couple the two physics, the；Electromagnetic Heating multiphysics coupling is used.
- `models.acdc.igbt_joule_heating_introduction.pdf`: models.acdc.igbt_joule_heating_introduction.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Template Multiphysics File

## PDF Summary and Physics Judgement

- Simple summary: igbt_joule_heating_introduction 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: Transfer in Solids physics interfaces, respectively. To couple the two physics, the
- PDF: Electromagnetic Heating multiphysics coupling is used.
- PDF: Using a stationary study, the device is modeled in the “on” state, where strong currents
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: Heat Transfer, Electric Currents, Global ODEs and DAEs
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 电磁, 结构力学
...
- PDF keywords: 传热, 电磁, 结构力学, models.acdc.igbt_joule_heating.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Electric Currents、Global ODEs and DAEs，研究类型线索为 Stationary；核心内容可概括为：Transfer in Solids physics interfaces, respectively. To couple the two physics, the；Electromagnetic Heating multiphysics coupling is used., models.acdc.igbt_joule_heating_introduction.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Template Multiphysics File
- `I_col` -> Rated collector current (entire module); MATLAB: `model.param.set('I_col', value, description)`
- `th_met` -> Metalization layer thickness (diode and IGBT die); MATLAB: `model.param.set('th_met', value, description)`
- `th_igbt` -> IGBT die thickness; MATLAB: `model.param.set('th_igbt', value, description)`
- `h_air` -> Heat transfer coefficient, air (forced convection); MATLAB: `model.param.set('h_air', value, description)`
- `T_air` -> Ambient temperature, air (forced convection); MATLAB: `model.param.set('T_air', value, description)`
- `T_trm` -> Ambient temperature, terminals (conduction); MATLAB: `model.param.set('T_trm', value, description)`
...
- 已识别几何证据：imp1 / Import；geom1 / imp1 / type；geom1 / imp1 / filename；geom1 / imp1 / selcadshownamesfromfileobj；geom1 / imp1 / selcadshownamesfromfilebnd
- 已识别物理场证据：Heat Transfer；Electric Currents；Global ODEs and DAEs
- 已识别参数：I_col、th_met、th_igbt、h_air、T_air、T_trm
...
- Transfer recognized physics interfaces to similar requirements: Heat Transfer, Electric Currents, Global ODEs and DAEs.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 电磁, 结构力学?
...
- Sweep `I_col` around the learned value `1.8[kA]` and export target outputs to CSV.
- Sweep `th_met` around the learned value `4[um]` and export target outputs to CSV.
- Sweep `th_igbt` around the learned value `140[um]` and export target outputs to CSV.
- Sweep `h_air` around the learned value `50[W/(m^2*K)]` and export target outputs to CSV.
- Sweep `T_air` around the learned value `60[degC]` and export target outputs to CSV.
- Sweep `T_trm` around the learned value `70[degC]` and export target outputs to CSV.
...
- Summary: 已学习 igbt_joule_heating_introduction 的 7 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=20 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=5，物理场=3，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：igbt_joule_heating_introduction.m, igbt_joule_heating_introduction.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 igbt_joule_heating_introduction，训练阶段为 modeling_logic_learning_ready。
...
- `igbt_joule_heating_introduction.mph` (comsol_mph, 323606950 bytes)
- `igbt_joule_heating_introduction.m` (matlab_livelink, 38763 bytes)
- `igbt_joule_heating_introduction.java` (comsol_java, 37698 bytes)
- `igbt_joule_heating_introduction_summary.json` (json, 932 bytes)
- `igbt_joule_heating.pptx` (presentation_document, 22922337 bytes)
- `models.acdc.igbt_joule_heating.pdf` (pdf_document, 3243009 bytes)
- `models.acdc.igbt_joule_heating_introduction.pdf` (pdf_document, 99542 bytes)
