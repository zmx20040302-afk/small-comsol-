...
- Case directory: `D:\桌面\codex\案例下载\电气\铁电材料的滞回现象`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:23:09.650528+00:00`
...
- `H0` = `0.01[in]` - Actuator thickness
- `R0` = `0.5[in]` - Actuator radius
- `Vmax` = `1600[V]` - Maximum applied voltage
- `alpha` = `3.6e6[m/F]` - Interdomain coupling
- `a` = `4.4e5[V/m]` - Domain wall density
- `c` = `0.18` - Polarization reversibility
- `k` = `1.9e6[V/m]` - Pinning loss
- `Ps` = `0.49[C/m^2]` - Saturation polarization
...
- physics: es / Electrostatics, Electrostatics
- materials: mat1
- mesh: mesh1, map1, ftri1
- studies: Stationary, Parametric Sweep
- boundary_conditions: ccns1 / 1, gnd1 / 4, pot1 / 2, r1 / size, r2 / size

### 文件内容证据

- `ferroelectric_hysteresis.java`: 参数=9, 几何=4, 物理场=2, 研究=0, 结果=0
- `models.acdc.ferroelectric_hysteresis.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `ferroelectric_hysteresis.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ferroelectric_hysteresis.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ferroelectric_hysteresis_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.ferroelectric_hysteresis.pdf`: models.acdc.ferroelectric_hysteresis.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；electric field can rearrange the domains, resulting in a net polarization in the material. At

## PDF Summary and Physics Judgement

- Simple summary: ferroelectric_hysteresis 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Optimization，并采用 Stationary 研究。
- Primary physics: `Optimization`
- Study type: `Stationary`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Optimization，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Stationary 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: electric field can rearrange the domains, resulting in a net polarization in the material. At
- PDF: Multiphysics. It assumes that the total polarization field is represented by a reversible part
- PDF: Figure 1: Model geometry. The smaller domain represents the upper half of the ferroelectric
...
- PDF keywords: 电磁, 优化/参数扫描, models.acdc.ferroelectric_hysteresis.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；electric field can rearrange the domains, resulting in a net polarization in the material. At
- `t` -> Time parameter; MATLAB: `model.param.set('t', value, description)`
- `H0` -> Actuator thickness; MATLAB: `model.param.set('H0', value, description)`
- `R0` -> Actuator radius; MATLAB: `model.param.set('R0', value, description)`
- `Vmax` -> Maximum applied voltage; MATLAB: `model.param.set('Vmax', value, description)`
...
- 已识别物理场证据：es / Electrostatics；Electrostatics
- 已识别参数：t、H0、R0、Vmax、alpha、a、c、k、Ps
...
- Transfer recognized physics interfaces to similar requirements: es / Electrostatics, Electrostatics.
...
- Sweep `H0` around the learned value `0.01[in]` and export target outputs to CSV.
- Sweep `R0` around the learned value `0.5[in]` and export target outputs to CSV.
- Sweep `Vmax` around the learned value `1600[V]` and export target outputs to CSV.
- Sweep `alpha` around the learned value `3.6e6[m/F]` and export target outputs to CSV.
- Sweep `a` around the learned value `4.4e5[V/m]` and export target outputs to CSV.
- Sweep `c` around the learned value `0.18` and export target outputs to CSV.
- Sweep `k` around the learned value `1.9e6[V/m]` and export target outputs to CSV.
...
- Summary: 已学习 ferroelectric_hysteresis 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=26 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 26 条；几何=4，物理场=2，材料=1，网格=3，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：ferroelectric_hysteresis.m, ferroelectric_hysteresis.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 ferroelectric_hysteresis，训练阶段为 modeling_logic_learning_ready。
...
- `ferroelectric_hysteresis.mph` (comsol_mph, 22804020 bytes)
- `ferroelectric_hysteresis.m` (matlab_livelink, 20779 bytes)
- `ferroelectric_hysteresis.java` (comsol_java, 17481 bytes)
- `ferroelectric_hysteresis_summary.json` (json, 1003 bytes)
- `models.acdc.ferroelectric_hysteresis.pdf` (pdf_document, 493647 bytes)
