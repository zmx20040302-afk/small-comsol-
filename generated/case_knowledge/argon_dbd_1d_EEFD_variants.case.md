# argon_dbd_1d_EEFD_variants

- Case directory: `D:\桌面\codex\案例下载\电气\使用不同 EEDF 计算的介质阻挡放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:31:22.251638+00:00`
...
- `argon_dbd_1d_EEFD_variants.java`: 参数=6, 几何=0, 物理场=0, 研究=0, 结果=0
- `argon_dbd_1d_EEFD_variants.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `argon_dbd_1d_EEFD_variants.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `argon_dbd_1d_EEFD_variants_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: argon_dbd_1d_EEFD_variants 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `Emax` -> 案例参数/扫描变量; MATLAB: `model.param.set('Emax', value, description)`
...
- Sweep `V0` around the learned value `-750[V]` and export target outputs to CSV.
- Sweep `Emax` around the learned value `250[V]` and export target outputs to CSV.
...
- Summary: 已学习 argon_dbd_1d_EEFD_variants 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=6 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：argon_dbd_1d_EEFD_variants.m, argon_dbd_1d_EEFD_variants.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 argon_dbd_1d_EEFD_variants，训练阶段为 modeling_logic_learning_ready。
...
- `argon_dbd_1d_EEFD_variants.mph` (comsol_mph, 23281098 bytes)
- `argon_dbd_1d_EEFD_variants.m` (matlab_livelink, 145148 bytes)
- `argon_dbd_1d_EEFD_variants.java` (comsol_java, 162761 bytes)
- `argon_dbd_1d_EEFD_variants_summary.json` (json, 2307 bytes)
