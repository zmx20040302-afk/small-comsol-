# Three_Phase_Cable_Sub_Section

- Case directory: `D:\桌面\codex\案例下载\电气\使用“磁场公式”接口实现系统激励`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:29:13.668464+00:00`
...
- `dZ` = `0.25[m]` - Length per twist
- `Zstart` = `0.25*dZ`
- `WireRadius` = `0.01[m]`
- `WireCenter` = `0.02[m]`
- `AirRadius` = `0.2[m]`
- `rho_air` = `1e1[ohm*m]` - Resistivity of air
- `dZsection` = `dZ*0.25`
- `Angle` = `360[deg]*dZsection/dZ`
- `I0` = `20[A]`
...
- 模型树证据数量: 38
- 理论关键词: 电磁
- geometry: pc1 / ParametricCurve, wp1 / WorkPlane, c1 / Circle, geom1 / pc1 / parmax, geom1 / pc1 / coord, geom1 / wp1 / planetype, geom1 / wp1 / normalvector, geom1 / wp1 / normalcoord, geom1 / wp1 / unite, swe1 / Sweep, geom1 / swe1 / crossfaces, geom1 / swe1 / manualdir
- physics: Magnetic Fields
- mesh: mesh1
- boundary_conditions: pc1 / parmax, pc1 / coord, wp1 / planetype, wp1 / normalvector, wp1 / normalcoord, wp1 / unite, c1 / r, swe1 / crossfaces, swe1 / manualdir, swe1 / keep, swe1 / includefinal, swe1 / twisting

### 文件内容证据

- `Three_Phase_Cable_Sub_Section.java`: 参数=9, 几何=13, 物理场=0, 研究=0, 结果=0
- `Three_Phase_Cable_Sub_Section.m`: 参数=9, 几何=9, 物理场=0, 研究=0, 结果=0
- `Three_Phase_Cable_Sub_Section_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `Three_Phase_Cable_Sub_Section.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: Three_Phase_Cable_Sub_Section 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Magnetic Fields。
- Primary physics: `Magnetic Fields`
- Study type: `unknown`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Magnetic Fields，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型需要继续从研究步骤确认。
...
- PDF keywords: 电磁
- `dZ` -> Length per twist; MATLAB: `model.param.set('dZ', value, description)`
- `Zstart` -> 案例参数/扫描变量; MATLAB: `model.param.set('Zstart', value, description)`
- `WireRadius` -> 案例参数/扫描变量; MATLAB: `model.param.set('WireRadius', value, description)`
- `WireCenter` -> 案例参数/扫描变量; MATLAB: `model.param.set('WireCenter', value, description)`
- `AirRadius` -> 案例参数/扫描变量; MATLAB: `model.param.set('AirRadius', value, description)`
- `rho_air` -> Resistivity of air; MATLAB: `model.param.set('rho_air', value, description)`
- `dZsection` -> 案例参数/扫描变量; MATLAB: `model.param.set('dZsection', value, description)`
- `Angle` -> 案例参数/扫描变量; MATLAB: `model.param.set('Angle', value, description)`
- `I0` -> 案例参数/扫描变量; MATLAB: `model.param.set('I0', value, description)`
...
- 已识别几何证据：pc1 / ParametricCurve；wp1 / WorkPlane；c1 / Circle；geom1 / pc1 / parmax；geom1 / pc1 / coord；geom1 / wp1 / planetype
- 已识别物理场证据：Magnetic Fields
- 已识别参数：dZ、Zstart、WireRadius、WireCenter、AirRadius、rho_air、dZsection、Angle、I0
...
- Sweep `dZ` around the learned value `0.25[m]` and export target outputs to CSV.
- Sweep `Zstart` around the learned value `0.25*dZ` and export target outputs to CSV.
- Sweep `WireRadius` around the learned value `0.01[m]` and export target outputs to CSV.
- Sweep `WireCenter` around the learned value `0.02[m]` and export target outputs to CSV.
- Sweep `AirRadius` around the learned value `0.2[m]` and export target outputs to CSV.
- Sweep `rho_air` around the learned value `1e1[ohm*m]` and export target outputs to CSV.
- Sweep `dZsection` around the learned value `dZ*0.25` and export target outputs to CSV.
- Sweep `Angle` around the learned value `360[deg]*dZsection/dZ` and export target outputs to CSV.
...
- Summary: 已学习 Three_Phase_Cable_Sub_Section 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=38 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 38 条；几何=15，物理场=1，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：Three_Phase_Cable_Sub_Section.m, Three_Phase_Cable_Sub_Section.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 Three_Phase_Cable_Sub_Section，训练阶段为 modeling_logic_learning_ready。
...
- `Three_Phase_Cable_Sub_Section.mph` (comsol_mph, 36702766 bytes)
- `Three_Phase_Cable_Sub_Section.m` (matlab_livelink, 53667 bytes)
- `Three_Phase_Cable_Sub_Section.java` (comsol_java, 56736 bytes)
- `Three_Phase_Cable_Sub_Section_summary.json` (json, 1877 bytes)
