...
- Created at: `2026-08-21T15:35:22.773254+00:00`
...
- `speed` = `5000[rpm]` - Shaft speed
- `f_el` = `speed*Np/2` - Electrical excitation frequency
- `Ipk` = `50[A]` - Phase current peak
- `init_ang` = `4.625[rad]` - Initial current angle
- `n_cog` = `Ns/gcd(Np,Ns)*2` - Cogging torque harmonic order
- `Nframes` = `n_cog*6` - Number of frames
- `L` = `250[mm]` - Motor axial length
...
- `pmm_steady_state.java`: 参数=7, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.acdc.pmm_steady_state.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `pmm_steady_state.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pmm_steady_state.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pmm_steady_state_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: pmm_steady_state 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF keywords: 电磁, 传热, models.acdc.pmm_steady_state.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry of a distributed wound interior permanent magnet machine is imported and
- `speed` -> Shaft speed; MATLAB: `model.param.set('speed', value, description)`
- `f_el` -> Electrical excitation frequency; MATLAB: `model.param.set('f_el', value, description)`
...
- `n_cog` -> Cogging torque harmonic order; MATLAB: `model.param.set('n_cog', value, description)`
- `Nframes` -> Number of frames; MATLAB: `model.param.set('Nframes', value, description)`
- `L` -> Motor axial length; MATLAB: `model.param.set('L', value, description)`
...
- 已识别参数：speed、f_el、Ipk、init_ang、n_cog、Nframes、L
...
- Sweep `speed` around the learned value `5000[rpm]` and export target outputs to CSV.
- Sweep `f_el` around the learned value `speed*Np/2` and export target outputs to CSV.
- Sweep `Ipk` around the learned value `50[A]` and export target outputs to CSV.
- Sweep `init_ang` around the learned value `4.625[rad]` and export target outputs to CSV.
...
- Sweep `L` around the learned value `250[mm]` and export target outputs to CSV.
...
- Summary: 已学习 pmm_steady_state 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=8 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 8 条；几何=0，物理场=0，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pmm_steady_state.m, pmm_steady_state.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pmm_steady_state，训练阶段为 modeling_logic_learning_ready。
...
- `pmm_steady_state.mph` (comsol_mph, 80144340 bytes)
- `pmm_steady_state.m` (matlab_livelink, 56497 bytes)
- `pmm_steady_state.java` (comsol_java, 55518 bytes)
- `pmm_steady_state_summary.json` (json, 5818 bytes)
