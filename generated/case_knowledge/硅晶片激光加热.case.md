...
- Created at: `2026-07-06T09:43:09.982114+00:00`
...
- `laser_heating_wafer.mph`: 参数=7, 几何=7, 物理场=1, 研究=2, 结果=0
...
- `models.mph.laser_heating_wafer.pdf`: models.mph.laser_heating_wafer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.laser_heating_wafer.zh_CN.pdf`: models.mph.laser_heating_wafer.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 硅晶片激光加热 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Transient 研究。
...
- PDF keywords: models.mph.laser_heating_wafer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.laser_heating_wafer.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `r_wafer` -> 案例参数/扫描变量; MATLAB: `model.param.set('r_wafer', value, description)`
- `thickness` -> 案例参数/扫描变量; MATLAB: `model.param.set('thickness', value, description)`
- `v_rotation` -> 案例参数/扫描变量; MATLAB: `model.param.set('v_rotation', value, description)`
- `period` -> 案例参数/扫描变量; MATLAB: `model.param.set('period', value, description)`
- `r_spot` -> 案例参数/扫描变量; MATLAB: `model.param.set('r_spot', value, description)`
- `emissivity` -> 案例参数/扫描变量; MATLAB: `model.param.set('emissivity', value, description)`
- `p_laser` -> 案例参数/扫描变量; MATLAB: `model.param.set('p_laser', value, description)`
...
- 已识别几何证据：cyl1 / Cylinder；blk1 / Block；int1 / Intersection；geom1 / cyl1 / r；geom1 / cyl1 / h；geom1 / blk1 / size
- 已识别物理场证据：ht / HeatTransfer
- 已识别参数：r_wafer、thickness、v_rotation、period、r_spot、emissivity、p_laser
...
- Transfer recognized physics interfaces to similar requirements: ht / HeatTransfer.
...
- Sweep `r_wafer` around the learned value `1[in]` and export target outputs to CSV.
- Sweep `thickness` around the learned value `275[um]` and export target outputs to CSV.
- Sweep `v_rotation` around the learned value `10[rpm]` and export target outputs to CSV.
- Sweep `period` around the learned value `20[s]` and export target outputs to CSV.
- Sweep `r_spot` around the learned value `2[mm]` and export target outputs to CSV.
- Sweep `emissivity` around the learned value `0.8` and export target outputs to CSV.
- Sweep `p_laser` around the learned value `10[W]` and export target outputs to CSV.
