# disc_resonator_anchor_losses

- Case directory: `D:\桌面\codex\案例下载\电气\氮中的负流注`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:16:36.931397+00:00`
...
- `R_d` = `10[um]` - Disc radius
- `H_d` = `3[um]` - Disc height
- `H_p` = `0.8[um]` - Post height
- `R_p` = `0.8[um]` - Post radius
- `R_s` = `40[um]` - Substrate section radius
- `t_PML` = `wl` - PML thickness
- `wl` = `9.5[um]` - Wavelength of acoustic waves

## 内容级读取结果

- 模型树证据数量: 37
- 理论关键词: 声学, 结构力学, 传热, 流体, 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, c1 / Circle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r3 / size, geom1 / r3 / pos, geom1 / c1 / r, geom1 / c1 / angle, geom1 / c1 / rot
- physics: Solid Mechanics, Electrostatics
- mesh: mesh1
- studies: Eigenvalue, Eigenfrequency, Time Dependent
- boundary_conditions: r1 / size, r1 / pos, r2 / size, r3 / size, r3 / pos, c1 / r, c1 / angle, c1 / rot, c1 / layername, c1 / layer

### 文件内容证据

- `disc_resonator_anchor_losses.java`: 参数=7, 几何=14, 物理场=0, 研究=0, 结果=0
- `models.plasma.streamer_1d.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `models.mems.disc_resonator_anchor_losses.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `disc_resonator_anchor_losses.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `disc_resonator_anchor_losses.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `disc_resonator_anchor_losses_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.disc_resonator_anchor_losses.pdf`: models.mems.disc_resonator_anchor_losses.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Eigenvalue、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the substrate. These waves are radiated away from the resonator resulting in a loss of
- `models.plasma.streamer_1d.pdf`: models.plasma.streamer_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This example presents a study of a negative streamer in atmospheric pressure nitrogen. The

## PDF Summary and Physics Judgement

- Simple summary: disc_resonator_anchor_losses 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Eigenvalue 研究。
...
- PDF: This example presents a study of a negative streamer in atmospheric pressure nitrogen. The
- PDF: results. Furthermore, in Ref. 1, results from a fluid and particle-in-cell model are
- PDF: few mean free paths of the wall resulting in the boundary condition for the electron flux
- 脚本/文本识别到的物理场: Solid Mechanics, Electrostatics
- 识别到的研究类型: Eigenvalue, Eigenfrequency, Time Dependent
- 理论关键词: 声学, 结构力学, 传热, 流体, 电磁

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 声学, 结构力学, 传热, 流体, 电磁, models.mems.disc_resonator_anchor_losses.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Eigenvalue、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the substrate. These waves are radiated away from the resonator resulting in a loss of, models.plasma.streamer_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This example presents a study of a negative streamer in atmospheric pressure nitrogen. The
- `R_d` -> Disc radius; MATLAB: `model.param.set('R_d', value, description)`
- `H_d` -> Disc height; MATLAB: `model.param.set('H_d', value, description)`
- `H_p` -> Post height; MATLAB: `model.param.set('H_p', value, description)`
- `R_p` -> Post radius; MATLAB: `model.param.set('R_p', value, description)`
- `R_s` -> Substrate section radius; MATLAB: `model.param.set('R_s', value, description)`
- `t_PML` -> PML thickness; MATLAB: `model.param.set('t_PML', value, description)`
- `wl` -> Wavelength of acoustic waves; MATLAB: `model.param.set('wl', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；c1 / Circle；geom1 / r1 / size；geom1 / r1 / pos
- 已识别物理场证据：Solid Mechanics；Electrostatics
- 已识别参数：R_d、H_d、H_p、R_p、R_s、t_PML、wl
...
- Use the learned study types as solver starting points: Eigenvalue, Eigenfrequency, Time Dependent.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 声学, 结构力学, 传热, 流体, 电磁?
...
- Sweep `R_d` around the learned value `10[um]` and export target outputs to CSV.
- Sweep `H_d` around the learned value `3[um]` and export target outputs to CSV.
- Sweep `H_p` around the learned value `0.8[um]` and export target outputs to CSV.
- Sweep `R_p` around the learned value `0.8[um]` and export target outputs to CSV.
- Sweep `R_s` around the learned value `40[um]` and export target outputs to CSV.
- Sweep `t_PML` around the learned value `wl` and export target outputs to CSV.
- Sweep `wl` around the learned value `9.5[um]` and export target outputs to CSV.
...
- Summary: 已学习 disc_resonator_anchor_losses 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=37 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 37 条；几何=14，物理场=2，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：disc_resonator_anchor_losses.m, disc_resonator_anchor_losses.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 disc_resonator_anchor_losses，训练阶段为 modeling_logic_learning_ready。
...
- `disc_resonator_anchor_losses.mph` (comsol_mph, 6153513 bytes)
- `disc_resonator_anchor_losses.m` (matlab_livelink, 24170 bytes)
- `disc_resonator_anchor_losses.java` (comsol_java, 20539 bytes)
- `disc_resonator_anchor_losses_summary.json` (json, 1233 bytes)
- `models.mems.disc_resonator_anchor_losses.pdf` (pdf_document, 545485 bytes)
- `models.plasma.streamer_1d.pdf` (pdf_document, 426973 bytes)
