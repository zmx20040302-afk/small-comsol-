...
- Case directory: `D:\桌面\codex\案例下载\电气\氢玻尔兹曼分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:39:20.838894+00:00`
...
- `boltzmann_hydrogen.java`: 参数=7, 几何=0, 物理场=1, 研究=0, 结果=0
- `models.plasma.boltzmann_hydrogen.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `boltzmann_hydrogen.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `boltzmann_hydrogen.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `boltzmann_hydrogen_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.boltzmann_hydrogen.pdf`: models.plasma.boltzmann_hydrogen.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；background gas of molecular and atomic hydrogen. This type of study is normally done to

## PDF Summary and Physics Judgement

- Simple summary: boltzmann_hydrogen 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: background gas of molecular and atomic hydrogen. This type of study is normally done to
- PDF: models. Here, the results are only analyzed to test if the computed electron energy
- PDF: the results that the time-averaged EEDFs are similar to the ones obtained in the HF limit.
...
- PDF keywords: 传热, 流体, 电磁, models.plasma.boltzmann_hydrogen.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；background gas of molecular and atomic hydrogen. This type of study is normally done to
...
- `wsN` -> Reduced angular frequency; MATLAB: `model.param.set('wsN', value, description)`
- `EN` -> Reduced electric field; MATLAB: `model.param.set('EN', value, description)`
...
- 已识别参数：Tgas、p0、Ngas、f0、w0、wsN、EN
...
- Sweep `p0` around the learned value `25000[Pa]` and export target outputs to CSV.
...
- Sweep `wsN` around the learned value `w0/Ngas` and export target outputs to CSV.
- Sweep `EN` around the learned value `50[Td]` and export target outputs to CSV.
...
- Summary: 已学习 boltzmann_hydrogen 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=10 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 10 条；几何=0，物理场=1，材料=0，网格=0，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：boltzmann_hydrogen.m, boltzmann_hydrogen.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 boltzmann_hydrogen，训练阶段为 modeling_logic_learning_ready。
...
- `boltzmann_hydrogen.mph` (comsol_mph, 3927897 bytes)
- `boltzmann_hydrogen.m` (matlab_livelink, 93935 bytes)
- `boltzmann_hydrogen.java` (comsol_java, 114980 bytes)
- `boltzmann_hydrogen_summary.json` (json, 2373 bytes)
- `models.plasma.boltzmann_hydrogen.pdf` (pdf_document, 340960 bytes)
