...
- geometry: c1 / Circle, c2 / Circle, c3 / Circle, c4 / Circle, dif1 / Difference, rot1 / Rotate, c5 / Circle, c6 / Circle, geom1 / c1 / r, geom1 / c2 / r, geom1 / c3 / r, geom1 / c3 / angle
- physics: Magnetic Fields
- mesh: mesh1
- studies: Frequency Domain, Parametric Sweep
- boundary_conditions: c1 / r, c2 / r, c3 / r, c3 / angle, c4 / r, c4 / angle, rot1 / rot, c5 / r, c6 / r, c7 / r

### 文件内容证据

- `three_phase_motor_frequency_geom_sequence.java`: 参数=7, 几何=20, 物理场=0, 研究=0, 结果=0
- `three_phase_motor_frequency_geom_sequence.m`: 参数=7, 几何=16, 物理场=0, 研究=0, 结果=0
- `models.acdc.three_phase_motor_frequency.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `three_phase_motor_frequency_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `three_phase_motor_frequency_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: three_phase_motor_frequency_geom_sequence 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- PDF keywords: 传热, 流体, 电磁, 多孔介质/裂隙, 优化/参数扫描, models.acdc.three_phase_motor_frequency.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Frequency Domain Study of Three-Phase
...
- `airgap` -> Size of air gap; MATLAB: `model.param.set('airgap', value, description)`
...
- 已识别几何证据：c1 / Circle；c2 / Circle；c3 / Circle；c4 / Circle；dif1 / Difference；rot1 / Rotate
- 已识别物理场证据：Magnetic Fields
- 已识别参数：r1、r2、r3、r4、r5、win_angle、airgap
...
- Sweep `r5` around the learned value `5.7[cm]` and export target outputs to CSV.
- Sweep `win_angle` around the learned value `45[deg]` and export target outputs to CSV.
- Sweep `airgap` around the learned value `r3-r2` and export target outputs to CSV.
...
- Summary: 已学习 three_phase_motor_frequency_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=41 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 41 条；几何=20，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：three_phase_motor_frequency_geom_sequence.m, three_phase_motor_frequency_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 three_phase_motor_frequency_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `three_phase_motor_frequency_geom_sequence.mph` (comsol_mph, 343669 bytes)
- `three_phase_motor_frequency_geom_sequence.m` (matlab_livelink, 6869 bytes)
- `three_phase_motor_frequency_geom_sequence.java` (comsol_java, 6729 bytes)
- `three_phase_motor_frequency_geom_sequence_summary.json` (json, 829 bytes)
