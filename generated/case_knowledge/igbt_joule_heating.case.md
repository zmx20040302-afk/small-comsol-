...
- Created at: `2026-08-20T11:49:01.997394+00:00`
...
- 模型树证据数量: 14
- 理论关键词: 传热, 电磁, 结构力学
- geometry: imp1 / Import, geom1 / imp1 / type
- physics: Heat Transfer, Electric Currents, Global ODEs and DAEs
- mesh: mesh1
- studies: Stationary
- boundary_conditions: imp1 / type

### 文件内容证据

- `igbt_joule_heating.java`: 参数=6, 几何=2, 物理场=1, 研究=0, 结果=0
...
- `igbt_joule_heating.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `igbt_joule_heating.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `igbt_joule_heating_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: igbt_joule_heating 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- 已识别几何证据：imp1 / Import；geom1 / imp1 / type
...
- Summary: 已学习 igbt_joule_heating 的 7 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=14 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 14 条；几何=2，物理场=3，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：igbt_joule_heating.m, igbt_joule_heating.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 igbt_joule_heating，训练阶段为 modeling_logic_learning_ready。
...
- `igbt_joule_heating.mph` (comsol_mph, 707864223 bytes)
- `igbt_joule_heating.m` (matlab_livelink, 97985 bytes)
- `igbt_joule_heating.java` (comsol_java, 91492 bytes)
- `igbt_joule_heating_summary.json` (json, 2652 bytes)
