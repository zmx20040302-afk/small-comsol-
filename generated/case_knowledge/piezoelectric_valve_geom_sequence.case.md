# piezoelectric_valve_geom_sequence

- Case directory: `D:\桌面\codex\案例下载\电气\压电阀`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:29:58.858325+00:00`
...
- `t0` = `0.05[mm]` - Piezoelectric layer thickness
- `ID` = `7[mm]` - Disc actuator inner diameter
- `OD` = `50[mm]` - Disc actuator outer diameter
- `n` = `12` - Number of layers in actuator
- `ts` = `0.2[mm]` - Thickness of seal
- `w0` = `0.5[mm]` - Through hole dimension
- `w1` = `ID/2` - Clamp region dimension
- `w2` = `ID/4` - Overall clamp dimension
- `h0` = `5*t0*n` - Base thickness
- `deltaz` = `16[um]` - Contact offset at 0[V]

## 内容级读取结果

- 模型树证据数量: 48
- 理论关键词: 电磁, 结构力学, 流体
- geometry: r1 / Rectangle, arr1 / Array, r2 / Rectangle, fil1 / Fillet, r3 / Rectangle, cha1 / Chamfer, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r1 / layername, geom1 / r1 / layer, geom1 / r1 / layerleft, geom1 / r1 / layerbottom
- physics: Solid Mechanics, Electrostatics
- mesh: mesh1
- studies: Stationary
- boundary_conditions: r1 / size, r1 / pos, r1 / layername, r1 / layer, r1 / layerleft, r1 / layerbottom, arr1 / fullsize, arr1 / displ, r2 / size, r2 / pos, fil1 / radius, r3 / size

### 文件内容证据

- `piezoelectric_valve_geom_sequence.java`: 参数=10, 几何=20, 物理场=0, 研究=0, 结果=0
- `piezoelectric_valve_geom_sequence.m`: 参数=10, 几何=19, 物理场=0, 研究=0, 结果=0
- `models.mems.piezoelectric_valve.pdf`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `piezoelectric_valve_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoelectric_valve_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.piezoelectric_valve.pdf`: models.mems.piezoelectric_valve.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the stacked actuator is considered in the model.

## PDF Summary and Physics Judgement

- Simple summary: piezoelectric_valve_geom_sequence 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF: the stacked actuator is considered in the model.
- PDF: Figure 1 shows both an axisymmetric slice through the geometry and a 3D rendering of
- PDF: the geometry. In this simple valve design a disc actuator compresses a hyperelastic seal
...
- PDF keywords: 电磁, 结构力学, 流体, models.mems.piezoelectric_valve.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the stacked actuator is considered in the model.
...
- `ts` -> Thickness of seal; MATLAB: `model.param.set('ts', value, description)`
- `w0` -> Through hole dimension; MATLAB: `model.param.set('w0', value, description)`
- `w1` -> Clamp region dimension; MATLAB: `model.param.set('w1', value, description)`
- `w2` -> Overall clamp dimension; MATLAB: `model.param.set('w2', value, description)`
- `h0` -> Base thickness; MATLAB: `model.param.set('h0', value, description)`
- `deltaz` -> Contact offset at 0[V]; MATLAB: `model.param.set('deltaz', value, description)`
...
- 已识别几何证据：r1 / Rectangle；arr1 / Array；r2 / Rectangle；fil1 / Fillet；r3 / Rectangle；cha1 / Chamfer
- 已识别物理场证据：Solid Mechanics；Electrostatics
- 已识别参数：t0、ID、OD、n、ts、w0、w1、w2、h0、deltaz
...
- Sweep `t0` around the learned value `0.05[mm]` and export target outputs to CSV.
- Sweep `ID` around the learned value `7[mm]` and export target outputs to CSV.
- Sweep `OD` around the learned value `50[mm]` and export target outputs to CSV.
- Sweep `n` around the learned value `12` and export target outputs to CSV.
- Sweep `ts` around the learned value `0.2[mm]` and export target outputs to CSV.
- Sweep `w0` around the learned value `0.5[mm]` and export target outputs to CSV.
- Sweep `w1` around the learned value `ID/2` and export target outputs to CSV.
- Sweep `w2` around the learned value `ID/4` and export target outputs to CSV.
...
- Summary: 已学习 piezoelectric_valve_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=48 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 48 条；几何=20，物理场=2，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：piezoelectric_valve_geom_sequence.m, piezoelectric_valve_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 piezoelectric_valve_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `piezoelectric_valve_geom_sequence.mph` (comsol_mph, 265037 bytes)
- `piezoelectric_valve_geom_sequence.m` (matlab_livelink, 7187 bytes)
- `piezoelectric_valve_geom_sequence.java` (comsol_java, 7529 bytes)
- `piezoelectric_valve_geom_sequence_summary.json` (json, 841 bytes)
- `models.mems.piezoelectric_valve.pdf` (pdf_document, 901750 bytes)
