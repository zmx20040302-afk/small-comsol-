# bolted_AC_electrical_contact

- Case directory: `D:\桌面\codex\案例下载\电气\螺栓连接处的交流电接触电阻`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:24:17.479760+00:00`
...
- 模型树证据数量: 40
- 理论关键词: 电磁
- geometry: blk1 / Block, blk2 / Block, cyl1 / Cylinder, dif1 / Difference, cyl2 / Cylinder, cyl3 / Cylinder, geom1 / blk1 / size, geom1 / blk1 / pos, geom1 / blk2 / size, geom1 / blk2 / pos, geom1 / cyl1 / r, geom1 / cyl1 / h
- mesh: mesh1
- boundary_conditions: blk1 / size, blk1 / pos, blk2 / size, blk2 / pos, cyl1 / r, cyl1 / h, cyl1 / pos, cyl2 / r, cyl2 / h, cyl2 / pos, cyl2 / layername, cyl2 / layerside

### 文件内容证据

- `bolted_AC_electrical_contact.java`: 参数=0, 几何=20, 物理场=0, 研究=0, 结果=0
- `bolted_AC_electrical_contact.m`: 参数=0, 几何=19, 物理场=0, 研究=0, 结果=0
- `bolted_AC_electrical_contact.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `bolted_AC_electrical_contact_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: bolted_AC_electrical_contact 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- 已识别几何证据：blk1 / Block；blk2 / Block；cyl1 / Cylinder；dif1 / Difference；cyl2 / Cylinder；cyl3 / Cylinder
...
- Summary: 已学习 bolted_AC_electrical_contact 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=40 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 40 条；几何=23，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：bolted_AC_electrical_contact.m, bolted_AC_electrical_contact.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 bolted_AC_electrical_contact，训练阶段为 modeling_logic_learning_ready。
...
- `bolted_AC_electrical_contact.mph` (comsol_mph, 56407941 bytes)
- `bolted_AC_electrical_contact.m` (matlab_livelink, 42965 bytes)
- `bolted_AC_electrical_contact.java` (comsol_java, 45960 bytes)
- `bolted_AC_electrical_contact_summary.json` (json, 1321 bytes)
