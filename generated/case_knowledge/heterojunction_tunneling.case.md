# heterojunction_tunneling

- Case directory: `D:\桌面\codex\案例下载\电气\异质结隧穿`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:38:49.429937+00:00`
...
- `T` = `T0` - Lattice temperature
- `d1` = `0.25[um]` - Layer 1 thickness
- `d3` = `d1` - Layer 3 thickness
- `d2` = `0.078[um]` - Layer 2 thickness
- `N_D1` = `1.5e17[cm^-3]` - Layer 1 n doping
- `N_D2` = `5e15[cm^-3]` - Layer 2 n doping
- `N_D3` = `8e17[cm^-3]` - Layer 3 n doping
- `N_A` = `0[cm^-3]` - P doping
- `Al_frac0` = `0.3` - Mole fraction of Al at heterojunction
- `Va` = `0[V]` - Applied voltage
- `thickness` = `1[m]` - Out of plane thickness
- `Vth` = `k_B_const*T/e_const` - Thermal voltage

## 内容级读取结果

- 模型树证据数量: 29
- 理论关键词: 传热, 电磁, 优化/参数扫描
- geometry: r1 / Rectangle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r1 / layername, geom1 / r1 / layer, geom1 / r1 / layerleft, geom1 / r1 / layerbottom
- mesh: mesh1
- studies: Stationary, Parametric Sweep
- boundary_conditions: r1 / size, r1 / pos, r1 / layername, r1 / layer, r1 / layerleft, r1 / layerbottom

### 文件内容证据

- `heterojunction_tunneling.java`: 参数=13, 几何=7, 物理场=0, 研究=0, 结果=0
- `models.semicond.heterojunction_tunneling.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `heterojunction_tunneling.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `heterojunction_tunneling.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `heterojunction_tunneling_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.heterojunction_tunneling.pdf`: models.semicond.heterojunction_tunneling.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁、优化/参数扫描，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；curves, their temperature dependence, and energy band diagrams all agree well with results

## PDF Summary and Physics Judgement

- Simple summary: heterojunction_tunneling 主要研究量子能级或波函数分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: curves, their temperature dependence, and energy band diagrams all agree well with results
- PDF: contribution to the current density can result from the quantum tunneling of the charge
- PDF: Within the drift–diffusion formulation of the semiconductor physics, the effect of
...
- PDF keywords: 传热, 电磁, 优化/参数扫描, models.semicond.heterojunction_tunneling.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁、优化/参数扫描，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；curves, their temperature dependence, and energy band diagrams all agree well with results
- `T0` -> Reference temperature; MATLAB: `model.param.set('T0', value, description)`
- `T` -> Lattice temperature; MATLAB: `model.param.set('T', value, description)`
- `d1` -> Layer 1 thickness; MATLAB: `model.param.set('d1', value, description)`
- `d3` -> Layer 3 thickness; MATLAB: `model.param.set('d3', value, description)`
- `d2` -> Layer 2 thickness; MATLAB: `model.param.set('d2', value, description)`
- `N_D1` -> Layer 1 n doping; MATLAB: `model.param.set('N_D1', value, description)`
- `N_D2` -> Layer 2 n doping; MATLAB: `model.param.set('N_D2', value, description)`
- `N_D3` -> Layer 3 n doping; MATLAB: `model.param.set('N_D3', value, description)`
- `N_A` -> P doping; MATLAB: `model.param.set('N_A', value, description)`
- `Al_frac0` -> Mole fraction of Al at heterojunction; MATLAB: `model.param.set('Al_frac0', value, description)`
- `Va` -> Applied voltage; MATLAB: `model.param.set('Va', value, description)`
- `thickness` -> Out of plane thickness; MATLAB: `model.param.set('thickness', value, description)`
...
- 已识别几何证据：r1 / Rectangle；geom1 / r1 / size；geom1 / r1 / pos；geom1 / r1 / layername；geom1 / r1 / layer；geom1 / r1 / layerleft
- 已识别参数：T0、T、d1、d3、d2、N_D1、N_D2、N_D3、N_A、Al_frac0、Va、thickness
...
- Sweep `T` around the learned value `T0` and export target outputs to CSV.
- Sweep `d1` around the learned value `0.25[um]` and export target outputs to CSV.
- Sweep `d3` around the learned value `d1` and export target outputs to CSV.
- Sweep `d2` around the learned value `0.078[um]` and export target outputs to CSV.
- Sweep `N_D1` around the learned value `1.5e17[cm^-3]` and export target outputs to CSV.
- Sweep `N_D2` around the learned value `5e15[cm^-3]` and export target outputs to CSV.
- Sweep `N_D3` around the learned value `8e17[cm^-3]` and export target outputs to CSV.
...
- Summary: 已学习 heterojunction_tunneling 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=13；内容证据=29 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 29 条；几何=7，物理场=0，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：heterojunction_tunneling.m, heterojunction_tunneling.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 heterojunction_tunneling，训练阶段为 modeling_logic_learning_ready。
...
- `heterojunction_tunneling.mph` (comsol_mph, 26939937 bytes)
- `heterojunction_tunneling.m` (matlab_livelink, 53488 bytes)
- `heterojunction_tunneling.java` (comsol_java, 49351 bytes)
- `heterojunction_tunneling_summary.json` (json, 1764 bytes)
- `models.semicond.heterojunction_tunneling.pdf` (pdf_document, 606943 bytes)
