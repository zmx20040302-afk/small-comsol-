...
- Case directory: `D:\桌面\codex\案例下载\电气\光子晶体光纤博客模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:07:06.422767+00:00`
...
- `r_clad` = `pitch*4` - Radius of cladding
- `r_hole` = `0.3*pitch` - Radius of hole
- `lda0` = `1.55[um]` - Operating wavelength
- `n_clad` = `1.449` - Refractive index of cladding
- `para` = `0.1` - Normalized wavelength
- `pitch` = `lda0/para` - Pitch distance
- `verticle_disp` = `tan(60[deg])*(pitch/2)` - Verticle distance between arrays

## 内容级读取结果

- 模型树证据数量: 31
- 理论关键词: 未自动识别
- geometry: c1 / Circle, c2 / Circle, arr1 / Array, mir1 / Mirror, mov1 / Move, geom1 / c1 / r, geom1 / c2 / r, geom1 / c2 / pos, geom1 / arr1 / fullsize, geom1 / arr1 / displ, geom1 / mir1 / keep, geom1 / mov1 / keep
- mesh: mesh1
- boundary_conditions: c1 / r, c2 / r, c2 / pos, arr1 / fullsize, arr1 / displ, mir1 / keep, mov1 / keep, mov1 / displx, mov1 / disply

### 文件内容证据

- `photonic_crystal_fiber.java`: 参数=7, 几何=14, 物理场=0, 研究=0, 结果=0
- `photonic_crystal_fiber.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `photonic_crystal_fiber.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `photonic_crystal_fiber_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: photonic_crystal_fiber 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `r_clad` -> Radius of cladding; MATLAB: `model.param.set('r_clad', value, description)`
- `r_hole` -> Radius of hole; MATLAB: `model.param.set('r_hole', value, description)`
...
- `para` -> Normalized wavelength; MATLAB: `model.param.set('para', value, description)`
- `pitch` -> Pitch distance; MATLAB: `model.param.set('pitch', value, description)`
- `verticle_disp` -> Verticle distance between arrays; MATLAB: `model.param.set('verticle_disp', value, description)`
...
- 已识别几何证据：c1 / Circle；c2 / Circle；arr1 / Array；mir1 / Mirror；mov1 / Move；geom1 / c1 / r
- 已识别参数：r_clad、r_hole、lda0、n_clad、para、pitch、verticle_disp
...
- Sweep `r_clad` around the learned value `pitch*4` and export target outputs to CSV.
- Sweep `r_hole` around the learned value `0.3*pitch` and export target outputs to CSV.
- Sweep `lda0` around the learned value `1.55[um]` and export target outputs to CSV.
- Sweep `n_clad` around the learned value `1.449` and export target outputs to CSV.
- Sweep `para` around the learned value `0.1` and export target outputs to CSV.
- Sweep `pitch` around the learned value `lda0/para` and export target outputs to CSV.
- Sweep `verticle_disp` around the learned value `tan(60[deg])*(pitch/2)` and export target outputs to CSV.
...
- Summary: 已学习 photonic_crystal_fiber 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=14，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：photonic_crystal_fiber.m, photonic_crystal_fiber.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 photonic_crystal_fiber，训练阶段为 modeling_logic_learning_ready。
...
- `photonic_crystal_fiber.mph` (comsol_mph, 2112666708 bytes)
- `photonic_crystal_fiber.m` (matlab_livelink, 22710 bytes)
- `photonic_crystal_fiber.java` (comsol_java, 19892 bytes)
- `photonic_crystal_fiber_summary.json` (json, 1051 bytes)
