# second_harmonic_generation_frequency_domain

- Case directory: `D:\桌面\codex\案例下载\电气\频域中的二次谐波产生`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:20:19.943440+00:00`
...
- `lambda1` = `1.064[um]` - Fundamental wavelength
- `f1` = `c_const/lambda1` - Fundamental frequency
- `f2` = `2*f1` - Second harmonic frequency
- `lambda2` = `c_const/f2` - Second harmonic wavelength
- `sim_l` = `lambda1*25` - Simulation length
- `sim_h` = `lambda2/16` - Simulation height
- `d` = `1e-18[C/V^2]` - Nonlinear coefficient
- `L` = `sim_l - 3*lambda1` - Length of nonlinear region
- `I1` = `30[MW/m^2]` - Incident fundamental intensity
- `E1` = `sqrt(2*I1/c_const/epsilon0_const)` - Incident fundamental electric field strength
- `offset` = `1.5*lambda1` - Start of nonlinear region

## 内容级读取结果

- 模型树证据数量: 27
- 理论关键词: 电磁
- geometry: r1 / Rectangle, geom1 / r1 / size, geom1 / r1 / layername, geom1 / r1 / layerleft, geom1 / r1 / layerright, geom1 / r1 / layerbottom
- physics: ewfd1 / ElectromagneticWavesFrequencyDomain, Electromagnetic Waves
- materials: mat1
- mesh: mesh1
- studies: Frequency Domain
- boundary_conditions: r1 / size, r1 / layername, r1 / layerleft, r1 / layerright, r1 / layerbottom

### 文件内容证据

- `second_harmonic_generation_frequency_domain.java`: 参数=11, 几何=6, 物理场=1, 研究=0, 结果=0
- `models.woptics.second_harmonic_generation_frequency_domain.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `second_harmonic_generation_frequency_domain.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `second_harmonic_generation_frequency_domain_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `second_harmonic_generation_frequency_domain.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.second_harmonic_generation_frequency_domain.pdf`: models.woptics.second_harmonic_generation_frequency_domain.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；also possible to mix them, which results in the generation of light in ultraviolet (UV) at

## PDF Summary and Physics Judgement

- Simple summary: second_harmonic_generation_frequency_domain 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 ElectromagneticWavesFrequencyDomain，并采用 Frequency Domain 研究。
- Primary physics: `ElectromagneticWavesFrequencyDomain`
- Study type: `Frequency Domain`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 ElectromagneticWavesFrequencyDomain，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Frequency Domain 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: also possible to mix them, which results in the generation of light in ultraviolet (UV) at
- PDF: harmonic frequency, which is twice the fundamental frequency. The results are compared
- PDF: with analytical results obtained within the Slowly Varying Envelope Approximation
- 脚本/文本识别到的物理场: ewfd1 / ElectromagneticWavesFrequencyDomain, Electromagnetic Waves
...
- PDF keywords: 电磁, models.woptics.second_harmonic_generation_frequency_domain.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；also possible to mix them, which results in the generation of light in ultraviolet (UV) at
- `lambda1` -> Fundamental wavelength; MATLAB: `model.param.set('lambda1', value, description)`
- `f1` -> Fundamental frequency; MATLAB: `model.param.set('f1', value, description)`
- `f2` -> Second harmonic frequency; MATLAB: `model.param.set('f2', value, description)`
- `lambda2` -> Second harmonic wavelength; MATLAB: `model.param.set('lambda2', value, description)`
- `sim_l` -> Simulation length; MATLAB: `model.param.set('sim_l', value, description)`
- `sim_h` -> Simulation height; MATLAB: `model.param.set('sim_h', value, description)`
- `d` -> Nonlinear coefficient; MATLAB: `model.param.set('d', value, description)`
- `L` -> Length of nonlinear region; MATLAB: `model.param.set('L', value, description)`
- `I1` -> Incident fundamental intensity; MATLAB: `model.param.set('I1', value, description)`
- `E1` -> Incident fundamental electric field strength; MATLAB: `model.param.set('E1', value, description)`
- `offset` -> Start of nonlinear region; MATLAB: `model.param.set('offset', value, description)`
...
- 已识别几何证据：r1 / Rectangle；geom1 / r1 / size；geom1 / r1 / layername；geom1 / r1 / layerleft；geom1 / r1 / layerright；geom1 / r1 / layerbottom
- 已识别物理场证据：ewfd1 / ElectromagneticWavesFrequencyDomain；Electromagnetic Waves
- 已识别参数：lambda1、f1、f2、lambda2、sim_l、sim_h、d、L、I1、E1、offset
...
- 识别到物理场证据：ewfd1 / ElectromagneticWavesFrequencyDomain
...
- Transfer recognized physics interfaces to similar requirements: ewfd1 / ElectromagneticWavesFrequencyDomain, Electromagnetic Waves.
...
- Sweep `lambda1` around the learned value `1.064[um]` and export target outputs to CSV.
- Sweep `f1` around the learned value `c_const/lambda1` and export target outputs to CSV.
- Sweep `f2` around the learned value `2*f1` and export target outputs to CSV.
- Sweep `lambda2` around the learned value `c_const/f2` and export target outputs to CSV.
- Sweep `sim_l` around the learned value `lambda1*25` and export target outputs to CSV.
- Sweep `sim_h` around the learned value `lambda2/16` and export target outputs to CSV.
- Sweep `d` around the learned value `1e-18[C/V^2]` and export target outputs to CSV.
- Sweep `L` around the learned value `sim_l - 3*lambda1` and export target outputs to CSV.
...
- Summary: 已学习 second_harmonic_generation_frequency_domain 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=27 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 27 条；几何=6，物理场=2，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：second_harmonic_generation_frequency_domain.m, second_harmonic_generation_frequency_domain.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 second_harmonic_generation_frequency_domain，训练阶段为 modeling_logic_learning_ready。
...
- `second_harmonic_generation_frequency_domain.mph` (comsol_mph, 3095178 bytes)
- `second_harmonic_generation_frequency_domain.m` (matlab_livelink, 28487 bytes)
- `second_harmonic_generation_frequency_domain.java` (comsol_java, 23613 bytes)
- `second_harmonic_generation_frequency_domain_summary.json` (json, 1350 bytes)
- `models.woptics.second_harmonic_generation_frequency_domain.pdf` (pdf_document, 1268990 bytes)
