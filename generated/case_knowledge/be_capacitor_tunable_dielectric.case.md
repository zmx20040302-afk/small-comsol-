...
- `be_capacitor_tunable_dielectric.java`: 参数=0, 几何=16, 物理场=0, 研究=0, 结果=0
- `be_capacitor_tunable_dielectric_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `be_capacitor_tunable_dielectric.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `be_capacitor_tunable_dielectric.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: be_capacitor_tunable_dielectric 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electrostatics。
...
- Summary: 已学习 be_capacitor_tunable_dielectric 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=16，物理场=1，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：be_capacitor_tunable_dielectric.m, be_capacitor_tunable_dielectric.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 be_capacitor_tunable_dielectric，训练阶段为 modeling_logic_learning_ready。
...
- `be_capacitor_tunable_dielectric.mph` (comsol_mph, 3935111 bytes)
- `be_capacitor_tunable_dielectric.m` (matlab_livelink, 34793 bytes)
- `be_capacitor_tunable_dielectric.java` (comsol_java, 28762 bytes)
- `be_capacitor_tunable_dielectric_summary.json` (json, 890 bytes)
