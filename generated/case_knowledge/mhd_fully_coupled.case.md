...
- Created at: `2026-08-20T10:34:21.332737+00:00`
...
- `mhd_fully_coupled.java`: 参数=8, 几何=18, 物理场=0, 研究=0, 结果=0
- `mhd_fully_coupled_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mhd_fully_coupled.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mhd_fully_coupled.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: mhd_fully_coupled 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- Summary: 已学习 mhd_fully_coupled 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=40 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mhd_fully_coupled.m, mhd_fully_coupled.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mhd_fully_coupled，训练阶段为 modeling_logic_learning_ready。
...
- `mhd_fully_coupled.mph` (comsol_mph, 14151351 bytes)
- `mhd_fully_coupled.m` (matlab_livelink, 45168 bytes)
- `mhd_fully_coupled.java` (comsol_java, 45091 bytes)
- `mhd_fully_coupled_summary.json` (json, 1248 bytes)
