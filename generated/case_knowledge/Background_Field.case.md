...
- Created at: `2026-08-16T10:27:56.835920+00:00`
...
- `Factor` = `500`
- `R0` = `15[cm]`
- `sigma_stab` = `1/(pi*f0*mu0_const*(Factor*R0)^2)`

## 内容级读取结果

- 模型树证据数量: 36
- 理论关键词: 电磁
- geometry: hel1 / Helix, sph2 / Sphere, sph3 / Sphere, uni1 / Union, geom1 / hel1 / turns, geom1 / hel1 / rmaj, geom1 / hel1 / rmin, geom1 / hel1 / axialpitch, geom1 / hel1 / radialpitch, geom1 / sph2 / r, geom1 / sph2 / pos, geom1 / sph3 / r
- physics: Magnetic Fields
- materials: mat1, mat2
- mesh: mesh1
- boundary_conditions: hel1 / turns, hel1 / rmaj, hel1 / rmin, hel1 / axialpitch, hel1 / radialpitch, sph2 / r, sph2 / pos, sph3 / r, sph3 / pos, uni1 / intbnd, sph1 / r

### 文件内容证据

- `Background_Field.java`: 参数=5, 几何=16, 物理场=0, 研究=0, 结果=0
- `Background_Field.m`: 参数=5, 几何=14, 物理场=0, 研究=0, 结果=0
- `Background_Field_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `Background_Field.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: Background_Field 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Magnetic Fields。
...
- `B0` -> 案例参数/扫描变量; MATLAB: `model.param.set('B0', value, description)`
...
- 已识别几何证据：hel1 / Helix；sph2 / Sphere；sph3 / Sphere；uni1 / Union；geom1 / hel1 / turns；geom1 / hel1 / rmaj
- 已识别物理场证据：Magnetic Fields
- 已识别参数：f0、B0、Factor、R0、sigma_stab
...
- Sweep `B0` around the learned value `0.1[T]` and export target outputs to CSV.
- Sweep `Factor` around the learned value `500` and export target outputs to CSV.
- Sweep `R0` around the learned value `15[cm]` and export target outputs to CSV.
- Sweep `sigma_stab` around the learned value `1/(pi*f0*mu0_const*(Factor*R0)^2)` and export target outputs to CSV.
...
- Summary: 已学习 Background_Field 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=36 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 36 条；几何=16，物理场=1，材料=2，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：Background_Field.m, Background_Field.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 Background_Field，训练阶段为 modeling_logic_learning_ready。
...
- `Background_Field.mph` (comsol_mph, 149635177 bytes)
- `Background_Field.m` (matlab_livelink, 22817 bytes)
- `Background_Field.java` (comsol_java, 23999 bytes)
- `Background_Field_summary.json` (json, 1298 bytes)
