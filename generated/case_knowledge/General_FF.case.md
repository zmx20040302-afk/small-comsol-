...
- Created at: `2026-08-20T10:08:56.703605+00:00`
...
- `rSphere` = `wl0/4` - Radius of 3D sphere
...
- 模型树证据数量: 23
- 理论关键词: 电磁, 多孔介质/裂隙, 传热
- geometry: sph1 / Sphere, wp1 / WorkPlane, geom1 / sph1 / r, geom1 / sph1 / layername, geom1 / wp1 / unite, c1 / Circle, geom1 / wp1 / selresult, geom1 / wp1 / selresultshow
- mesh: mesh1
- boundary_conditions: sph1 / r, sph1 / layername, wp1 / unite, wp1 / selresult, wp1 / selresultshow

### 文件内容证据

- `General_FF.java`: 参数=9, 几何=8, 物理场=0, 研究=0, 结果=0
- `General_FF.m`: 参数=9, 几何=5, 物理场=0, 研究=0, 结果=0
- `FFwithSubstrate.pptx`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `General_FF_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `General_FF.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: General_FF 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- 已识别几何证据：sph1 / Sphere；wp1 / WorkPlane；geom1 / sph1 / r；geom1 / sph1 / layername；geom1 / wp1 / unite；c1 / Circle
...
- Summary: 已学习 General_FF 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=8，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：General_FF.m, General_FF.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 General_FF，训练阶段为 modeling_logic_learning_ready。
...
- `General_FF.mph` (comsol_mph, 10512813 bytes)
- `General_FF.m` (matlab_livelink, 46544 bytes)
- `General_FF.java` (comsol_java, 49468 bytes)
- `General_FF_summary.json` (json, 1382 bytes)
