...
- Case directory: `D:\桌面\codex\案例下载\电气\哈特曼边界层`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:51:23.590543+00:00`
...
- `d` = `1[cm]` - Half-distance between plates
- `dens` = `1000[kg/m^3]` - Fluid density
- `sigma0` = `1e7[S/m]` - Fluid electric conductivity
- `visc` = `1e-3[Pa*s]` - Fluid viscosity
- `U0` = `0.05[m/s]` - Average Inlet velocity
- `H0` = `Ha/mu0_const/d/sqrt(sigma0/visc)` - Imposed magnetic field
- `B0` = `mu0_const*H0` - Magnetic flux density
- `Re` = `dens*U0*2*d/visc` - Reynolds number
- `Exx` = `-mu0_const*H0*U0` - Induced electric field
- `J0` = `sigma0*Exx` - Induced electric current density
...
- physics: mf / InductionCurrents, Laminar Flow, Magnetic Fields
- materials: mat1
- mesh: mesh1
- studies: Stationary
- boundary_conditions: exvp1 / 1, 2, 3, 4, r1 / size, r1 / pos

### 文件内容证据

- `hartmann_boundary_layer.java`: 参数=11, 几何=3, 物理场=1, 研究=0, 结果=0
- `models.acdc.hartmann_boundary_layer.pdf`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `hartmann_boundary_layer.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `hartmann_boundary_layer.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `hartmann_boundary_layer_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.hartmann_boundary_layer.pdf`: models.acdc.hartmann_boundary_layer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Laminar Flow、Magnetic Fields，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The model is set up in 2D using the Magnetic Fields and Laminar Flow physics interfaces,

## PDF Summary and Physics Judgement

- Simple summary: hartmann_boundary_layer 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- PDF: The model is set up in 2D using the Magnetic Fields and Laminar Flow physics interfaces,
- PDF: coupled via a Magnetohydrodynamics multiphysics node.
- PDF: Results
- 脚本/文本识别到的物理场: mf / InductionCurrents, Laminar Flow, Magnetic Fields
- 识别到的研究类型: Stationary
- 理论关键词: 流体, 电磁, 多孔介质/裂隙
...
- PDF keywords: 流体, 电磁, 多孔介质/裂隙, models.acdc.hartmann_boundary_layer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Laminar Flow、Magnetic Fields，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The model is set up in 2D using the Magnetic Fields and Laminar Flow physics interfaces,
- `Ha` -> Hartmann number; MATLAB: `model.param.set('Ha', value, description)`
- `d` -> Half-distance between plates; MATLAB: `model.param.set('d', value, description)`
- `dens` -> Fluid density; MATLAB: `model.param.set('dens', value, description)`
- `sigma0` -> Fluid electric conductivity; MATLAB: `model.param.set('sigma0', value, description)`
- `visc` -> Fluid viscosity; MATLAB: `model.param.set('visc', value, description)`
- `U0` -> Average Inlet velocity; MATLAB: `model.param.set('U0', value, description)`
- `H0` -> Imposed magnetic field; MATLAB: `model.param.set('H0', value, description)`
- `B0` -> Magnetic flux density; MATLAB: `model.param.set('B0', value, description)`
- `Re` -> Reynolds number; MATLAB: `model.param.set('Re', value, description)`
- `Exx` -> Induced electric field; MATLAB: `model.param.set('Exx', value, description)`
- `J0` -> Induced electric current density; MATLAB: `model.param.set('J0', value, description)`
...
- 已识别物理场证据：mf / InductionCurrents；Laminar Flow；Magnetic Fields
- 已识别参数：Ha、d、dens、sigma0、visc、U0、H0、B0、Re、Exx、J0
...
- Transfer recognized physics interfaces to similar requirements: mf / InductionCurrents, Laminar Flow, Magnetic Fields.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 电磁, 多孔介质/裂隙?
...
- Sweep `Ha` around the learned value `10` and export target outputs to CSV.
- Sweep `d` around the learned value `1[cm]` and export target outputs to CSV.
- Sweep `dens` around the learned value `1000[kg/m^3]` and export target outputs to CSV.
- Sweep `sigma0` around the learned value `1e7[S/m]` and export target outputs to CSV.
- Sweep `visc` around the learned value `1e-3[Pa*s]` and export target outputs to CSV.
- Sweep `U0` around the learned value `0.05[m/s]` and export target outputs to CSV.
- Sweep `H0` around the learned value `Ha/mu0_const/d/sqrt(sigma0/visc)` and export target outputs to CSV.
- Sweep `B0` around the learned value `mu0_const*H0` and export target outputs to CSV.
...
- Summary: 已学习 hartmann_boundary_layer 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=3，物理场=3，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：hartmann_boundary_layer.m, hartmann_boundary_layer.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 hartmann_boundary_layer，训练阶段为 modeling_logic_learning_ready。
...
- `hartmann_boundary_layer.mph` (comsol_mph, 3366228 bytes)
- `hartmann_boundary_layer.m` (matlab_livelink, 27588 bytes)
- `hartmann_boundary_layer.java` (comsol_java, 23361 bytes)
- `hartmann_boundary_layer_summary.json` (json, 1374 bytes)
- `models.acdc.hartmann_boundary_layer.pdf` (pdf_document, 557727 bytes)
