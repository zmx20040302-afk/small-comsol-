...
- Case directory: `D:\桌面\codex\案例下载\电气\微波炉中的旋转加热及相变`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:36:01.023723+00:00`
...
- `wo` = `20[cm]` - Oven width
- `do` = `20[cm]` - Oven depth
- `ho` = `10[cm]` - Oven height
- `wg` = `50[mm]` - Waveguide width
- `dg` = `78[mm]` - Waveguide depth
- `hg` = `18[mm]` - Waveguide height
- `rp` = `8[cm]` - Glass plate radius
- `hp` = `5[mm]` - Glass plate height
- `rot` = `0[deg]` - Initial angular position
- `gpos` = `5[cm]` - Potato shift from center
- `rpot` = `2[cm]` - Potato radius

## 内容级读取结果

- 模型树证据数量: 25
- 理论关键词: 传热, 结构力学, 流体, 电磁, 多孔介质/裂隙, 优化/参数扫描
- geometry: blk1 / Block, blk2 / Block, geom1 / blk1 / size, geom1 / blk1 / pos
- physics: Heat Transfer, Magnetic Fields, Optimization, Electromagnetic Waves
- mesh: mesh1
- studies: Time Dependent, Frequency Domain, Optimization
- boundary_conditions: blk1 / size, blk1 / pos

### 文件内容证据

- `rotating_microwave_oven.java`: 参数=11, 几何=4, 物理场=0, 研究=0, 结果=0
- `models.rf.rotating_microwave_oven.pdf`: 参数=0, 几何=0, 物理场=4, 研究=3, 结果=0
- `rotating_microwave_oven.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rotating_microwave_oven.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rotating_microwave_oven_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.rf.rotating_microwave_oven.pdf`: models.rf.rotating_microwave_oven.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields、Optimization，研究类型线索为 Time Dependent、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；interacts with objects, affecting their internal temperature and cooking results. It also aids

## PDF Summary and Physics Judgement

- Simple summary: rotating_microwave_oven 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: interacts with objects, affecting their internal temperature and cooking results. It also aids
- PDF: second steps, with a mesh adjusted for conformity at each step using the Phase Change
- PDF: MOVING MESH
- 脚本/文本识别到的物理场: Heat Transfer, Magnetic Fields, Optimization, Electromagnetic Waves
- 识别到的研究类型: Time Dependent, Frequency Domain, Optimization
- 理论关键词: 传热, 结构力学, 流体, 电磁, 多孔介质/裂隙, 优化/参数扫描

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 传热, 结构力学, 流体, 电磁, 多孔介质/裂隙, 优化/参数扫描, models.rf.rotating_microwave_oven.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields、Optimization，研究类型线索为 Time Dependent、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；interacts with objects, affecting their internal temperature and cooking results. It also aids
...
- `rot` -> Initial angular position; MATLAB: `model.param.set('rot', value, description)`
- `gpos` -> Potato shift from center; MATLAB: `model.param.set('gpos', value, description)`
...
- 已识别几何证据：blk1 / Block；blk2 / Block；geom1 / blk1 / size；geom1 / blk1 / pos
- 已识别物理场证据：Heat Transfer；Magnetic Fields；Optimization；Electromagnetic Waves
- 已识别参数：wo、do、ho、wg、dg、hg、rp、hp、rot、gpos、rpot
...
- Transfer recognized physics interfaces to similar requirements: Heat Transfer, Magnetic Fields, Optimization, Electromagnetic Waves.
- Reuse geometry construction patterns when new requirements share shape, import, array, sweep, or boolean operations.
- Use the learned study types as solver starting points: Time Dependent, Frequency Domain, Optimization.
...
- Sweep `wo` around the learned value `20[cm]` and export target outputs to CSV.
- Sweep `do` around the learned value `20[cm]` and export target outputs to CSV.
- Sweep `ho` around the learned value `10[cm]` and export target outputs to CSV.
...
- Sweep `rp` around the learned value `8[cm]` and export target outputs to CSV.
- Sweep `hp` around the learned value `5[mm]` and export target outputs to CSV.
...
- Summary: 已学习 rotating_microwave_oven 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=25 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 25 条；几何=4，物理场=4，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：rotating_microwave_oven.m, rotating_microwave_oven.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 rotating_microwave_oven，训练阶段为 modeling_logic_learning_ready。
...
- `rotating_microwave_oven.mph` (comsol_mph, 596842400 bytes)
- `rotating_microwave_oven.m` (matlab_livelink, 83315 bytes)
- `rotating_microwave_oven.java` (comsol_java, 77210 bytes)
- `rotating_microwave_oven_summary.json` (json, 3115 bytes)
- `models.rf.rotating_microwave_oven.pdf` (pdf_document, 4861137 bytes)
