...
- Case directory: `D:\桌面\codex\案例下载\电气\肖特基二极管`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:54:15.323827+00:00`
...
- `V0` = `3[V]*tri1(t/6[ms])` - Anode voltage
- `t` = `0[s]` - Time
...
- physics: semi / Semiconductor
- materials: mat1
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: adm1 / 1, adm2 / 2, adm3 / 3, adm4 / 4, mc1 / 1, mc2 / 5, i1 / specify, i1 / len

### 文件内容证据

- `shockley_diode.java`: 参数=2, 几何=3, 物理场=1, 研究=0, 结果=0
- `models.semicond.shockley_diode.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `shockley_diode.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `shockley_diode.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `shockley_diode_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.shockley_diode.pdf`: models.semicond.shockley_diode.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The I–V curve of a Shockley diode typically has a loop. This means that a stationary study

## PDF Summary and Physics Judgement

- Simple summary: shockley_diode 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Semiconductor，并采用 Time Dependent 研究。
- Primary physics: `Semiconductor`
- Study type: `Time Dependent`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Semiconductor，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Time Dependent 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: The I–V curve of a Shockley diode typically has a loop. This means that a stationary study
- PDF: voltage. This model shows how to model a Shockley diode using a time-dependent study
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: semi / Semiconductor
...
- PDF keywords: 电磁, models.semicond.shockley_diode.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The I–V curve of a Shockley diode typically has a loop. This means that a stationary study
- `V0` -> Anode voltage; MATLAB: `model.param.set('V0', value, description)`
...
- Transfer recognized physics interfaces to similar requirements: semi / Semiconductor.
...
- Sweep `V0` around the learned value `3[V]*tri1(t/6[ms])` and export target outputs to CSV.
...
- Summary: 已学习 shockley_diode 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=18 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 18 条；几何=3，物理场=1，材料=1，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：shockley_diode.m, shockley_diode.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 shockley_diode，训练阶段为 modeling_logic_learning_ready。
...
- `shockley_diode.mph` (comsol_mph, 4773352 bytes)
- `shockley_diode.m` (matlab_livelink, 25159 bytes)
- `shockley_diode.java` (comsol_java, 22741 bytes)
- `shockley_diode_summary.json` (json, 1047 bytes)
- `models.semicond.shockley_diode.pdf` (pdf_document, 202779 bytes)
