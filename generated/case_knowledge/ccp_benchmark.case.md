...
- Case directory: `D:\桌面\codex\案例下载\电气\电容耦合等离子体基准模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:04:00.423948+00:00`
...
- physics: ptp / ColdPlasmaTimePeriodic, Electrostatics
- mesh: mesh1, edg1
- boundary_conditions: gnd1 / 1, mct1 / 2, i1 / coord

### 文件内容证据

- `ccp_benchmark.java`: 参数=1, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.plasma.ccp_benchmark.pdf`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `ccp_benchmark.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ccp_benchmark.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ccp_benchmark_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.ccp_benchmark.pdf`: models.plasma.ccp_benchmark.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The underlying physics of a capacitively coupled plasma is rather complicated, even for

## PDF Summary and Physics Judgement

- Simple summary: ccp_benchmark 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF: The underlying physics of a capacitively coupled plasma is rather complicated, even for
- PDF: rather simple geometric configurations and plasma chemistries. This model benchmarks
- PDF: the Plasma, Time Periodic interface against many different codes, the results of which are
- 脚本/文本识别到的物理场: ptp / ColdPlasmaTimePeriodic, Electrostatics
- 理论关键词: 流体, 传热, 结构力学, 电磁

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 流体, 传热, 结构力学, 电磁, models.plasma.ccp_benchmark.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The underlying physics of a capacitively coupled plasma is rather complicated, even for
...
- 已识别物理场证据：ptp / ColdPlasmaTimePeriodic；Electrostatics
- 已识别参数：p0
...
- Transfer recognized physics interfaces to similar requirements: ptp / ColdPlasmaTimePeriodic, Electrostatics.
...
- Sweep `p0` around the learned value `0.3[torr]` and export target outputs to CSV.
...
- Summary: 已学习 ccp_benchmark 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=1；内容证据=10 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 10 条；几何=2，物理场=2，材料=0，网格=2，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：ccp_benchmark.m, ccp_benchmark.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 ccp_benchmark，训练阶段为 modeling_logic_learning_ready。
...
- `ccp_benchmark.mph` (comsol_mph, 3367072 bytes)
- `ccp_benchmark.m` (matlab_livelink, 43200 bytes)
- `ccp_benchmark.java` (comsol_java, 43255 bytes)
- `ccp_benchmark_summary.json` (json, 1229 bytes)
- `models.plasma.ccp_benchmark.pdf` (pdf_document, 349468 bytes)
