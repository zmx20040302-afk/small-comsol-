...
- Case directory: `D:\桌面\codex\案例下载\电气\干空气玻尔兹曼分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:37:00.266288+00:00`
...
- `boltzmann_dry_air.java`: 参数=2, 几何=0, 物理场=1, 研究=0, 结果=0
- `models.plasma.boltzmann_dry_air.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `boltzmann_dry_air.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `boltzmann_dry_air.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `boltzmann_dry_air_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.boltzmann_dry_air.pdf`: models.plasma.boltzmann_dry_air.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；that can be used in a fluid-type model.

## PDF Summary and Physics Judgement

- Simple summary: boltzmann_dry_air 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF: that can be used in a fluid-type model.
- PDF: interface. The first part of the study is to solve for the EEDF for the conditions that the
- PDF: into the fluid model and see if the results of interest change significantly.
...
- PDF keywords: 传热, 流体, 电磁, models.plasma.boltzmann_dry_air.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；that can be used in a fluid-type model.
- `xN2` -> 案例参数/扫描变量; MATLAB: `model.param.set('xN2', value, description)`
...
- Sweep `xN2` around the learned value `0.8` and export target outputs to CSV.
- Sweep `xO2` around the learned value `1-xN2` and export target outputs to CSV.
...
- Summary: 已学习 boltzmann_dry_air 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=3 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 3 条；几何=0，物理场=1，材料=0，网格=0，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：boltzmann_dry_air.m, boltzmann_dry_air.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 boltzmann_dry_air，训练阶段为 modeling_logic_learning_ready。
...
- `boltzmann_dry_air.mph` (comsol_mph, 2066336 bytes)
- `boltzmann_dry_air.m` (matlab_livelink, 91979 bytes)
- `boltzmann_dry_air.java` (comsol_java, 106283 bytes)
- `boltzmann_dry_air_summary.json` (json, 2821 bytes)
- `models.plasma.boltzmann_dry_air.pdf` (pdf_document, 468941 bytes)
