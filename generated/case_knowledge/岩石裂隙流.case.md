...
- Created at: `2026-07-06T09:38:44.906935+00:00`
...
- `rock_fracture_flow.mph`: 参数=2, 几何=3, 物理场=1, 研究=3, 结果=0
- `rock_fracture_flow.java`: 参数=2, 几何=3, 物理场=1, 研究=3, 结果=0
- `rock_fracture_flow.m`: 参数=2, 几何=3, 物理场=1, 研究=2, 结果=0
...
- `models.mph.rock_fracture_flow.pdf`: models.mph.rock_fracture_flow.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.rock_fracture_flow.zh_CN.pdf`: models.mph.rock_fracture_flow.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 岩石裂隙流 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- 脚本/文本识别到的物理场: cdeq / ConvectionDiffusionEquation
...
- PDF keywords: 传热, 流体, 多孔介质/裂隙, models.mph.rock_fracture_flow.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.rock_fracture_flow.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `rho` -> 案例参数/扫描变量; MATLAB: `model.param.set('rho', value, description)`
- `mu` -> 案例参数/扫描变量; MATLAB: `model.param.set('mu', value, description)`
...
- 已识别物理场证据：cdeq / ConvectionDiffusionEquation
- 已识别参数：rho、mu
...
- 识别到物理场证据：cdeq / ConvectionDiffusionEquation
...
- Transfer recognized physics interfaces to similar requirements: cdeq / ConvectionDiffusionEquation.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 流体, 多孔介质/裂隙?
...
- Sweep `rho` around the learned value `1000[kg/m^3]` and export target outputs to CSV.
- Sweep `mu` around the learned value `0.001[Pa*s]` and export target outputs to CSV.
...
- Summary: 已学习 岩石裂隙流 的 8 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=16 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 16 条；几何=3，物理场=1，材料=0，网格=1，研究=3，结果=0。
