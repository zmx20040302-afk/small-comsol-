# ball_adhesion_decohesion_v63

- Case directory: `D:\桌面\codex\案例下载\电气\压痕球的黏附与剥离`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:56:49.540682+00:00`
...
- `Rs` = `50[mm]` - Sphere radius
- `Rme` = `150[mm]` - Membrane radius
- `tm` = `6[mm]` - Membrane thickness
- `d0` = `5[mm]` - Initial clearance
- `C10` = `1.5[MPa]` - Mooney-Rivlin C10 (rubber)
- `C01` = `0.4[MPa]` - Mooney-Rivlin C01 (rubber)
- `kappa` = `100[MPa]` - Bulk modulus (rubber)
- `kn` = `2e10[N/m^3]` - Adhesive stiffness
- `p_stick` = `0.2[MPa]` - Adhesion pressure
- `sIc` = `2[MPa]` - Adhesive tensile strength
- `GIc` = `1130[J/m^2]` - Tensile energy release rate
- `sIIc` = `3[MPa]` - Adhesive shear strength
- `GIIc` = `1800[J/m^2]` - Shear energy release rate
- `w_max` = `80[mm]` - Max vertical ball displacement
...
- 模型树证据数量: 31
- 理论关键词: 结构力学, 流体
- geometry: c1 / Circle, r1 / Rectangle, geom1 / c1 / r, geom1 / c1 / angle, geom1 / c1 / pos, geom1 / c1 / rot, geom1 / r1 / size, geom1 / r1 / pos
- materials: mat1
- mesh: mesh1
- boundary_conditions: c1 / r, c1 / angle, c1 / pos, c1 / rot, r1 / size, r1 / pos

### 文件内容证据

- `ball_adhesion_decohesion_v63.m`: 参数=15, 几何=8, 物理场=0, 研究=0, 结果=0
- `ball_adhesion_decohesion_v63.java`: 参数=15, 几何=8, 物理场=0, 研究=0, 结果=0
- `ball_adhesion_decohesion.pptx`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ball_adhesion_decohesion_v63.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ball_adhesion_decohesion_v63_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: ball_adhesion_decohesion_v63 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics。
...
- PDF keywords: 结构力学, 流体
- `Rs` -> Sphere radius; MATLAB: `model.param.set('Rs', value, description)`
- `Rme` -> Membrane radius; MATLAB: `model.param.set('Rme', value, description)`
- `tm` -> Membrane thickness; MATLAB: `model.param.set('tm', value, description)`
- `d0` -> Initial clearance; MATLAB: `model.param.set('d0', value, description)`
- `C10` -> Mooney-Rivlin C10 (rubber); MATLAB: `model.param.set('C10', value, description)`
- `C01` -> Mooney-Rivlin C01 (rubber); MATLAB: `model.param.set('C01', value, description)`
- `kappa` -> Bulk modulus (rubber); MATLAB: `model.param.set('kappa', value, description)`
- `kn` -> Adhesive stiffness; MATLAB: `model.param.set('kn', value, description)`
- `p_stick` -> Adhesion pressure; MATLAB: `model.param.set('p_stick', value, description)`
- `sIc` -> Adhesive tensile strength; MATLAB: `model.param.set('sIc', value, description)`
- `GIc` -> Tensile energy release rate; MATLAB: `model.param.set('GIc', value, description)`
- `sIIc` -> Adhesive shear strength; MATLAB: `model.param.set('sIIc', value, description)`
...
- 已识别几何证据：c1 / Circle；r1 / Rectangle；geom1 / c1 / r；geom1 / c1 / angle；geom1 / c1 / pos；geom1 / c1 / rot
- 已识别参数：Rs、Rme、tm、d0、C10、C01、kappa、kn、p_stick、sIc、GIc、sIIc
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 结构力学, 流体?
...
- Sweep `Rs` around the learned value `50[mm]` and export target outputs to CSV.
- Sweep `Rme` around the learned value `150[mm]` and export target outputs to CSV.
- Sweep `tm` around the learned value `6[mm]` and export target outputs to CSV.
- Sweep `d0` around the learned value `5[mm]` and export target outputs to CSV.
- Sweep `C10` around the learned value `1.5[MPa]` and export target outputs to CSV.
- Sweep `C01` around the learned value `0.4[MPa]` and export target outputs to CSV.
- Sweep `kappa` around the learned value `100[MPa]` and export target outputs to CSV.
- Sweep `kn` around the learned value `2e10[N/m^3]` and export target outputs to CSV.
...
- Summary: 已学习 ball_adhesion_decohesion_v63 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=15；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=8，物理场=0，材料=1，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：ball_adhesion_decohesion_v63.m, ball_adhesion_decohesion_v63.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 ball_adhesion_decohesion_v63，训练阶段为 modeling_logic_learning_ready。
...
- `ball_adhesion_decohesion_v63.mph` (comsol_mph, 19946550 bytes)
- `ball_adhesion_decohesion_v63.m` (matlab_livelink, 34657 bytes)
- `ball_adhesion_decohesion_v63.java` (comsol_java, 37138 bytes)
- `ball_adhesion_decohesion_v63_summary.json` (json, 1451 bytes)
- `ball_adhesion_decohesion.pptx` (presentation_document, 700675 bytes)
