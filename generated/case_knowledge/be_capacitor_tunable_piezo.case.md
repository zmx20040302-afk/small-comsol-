...
- `be_capacitor_tunable_piezo.java`: 参数=0, 几何=16, 物理场=0, 研究=0, 结果=0
- `be_capacitor_tunable_piezo_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `be_capacitor_tunable_piezo.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `be_capacitor_tunable_piezo.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: be_capacitor_tunable_piezo 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electrostatics。
...
- Summary: 已学习 be_capacitor_tunable_piezo 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=28 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：be_capacitor_tunable_piezo.m, be_capacitor_tunable_piezo.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 be_capacitor_tunable_piezo，训练阶段为 modeling_logic_learning_ready。
...
- `be_capacitor_tunable_piezo.mph` (comsol_mph, 6454235 bytes)
- `be_capacitor_tunable_piezo.m` (matlab_livelink, 40524 bytes)
- `be_capacitor_tunable_piezo.java` (comsol_java, 35043 bytes)
- `be_capacitor_tunable_piezo_summary.json` (json, 952 bytes)
