...
- Case directory: `D:\桌面\codex\案例下载\电气\CF4O2 等离子体反应器全局模型（用于硅刻蚀）`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:26:04.055960+00:00`
...
- `R0` = `10[cm]` - Radius
- `L0` = `10[cm]` - Length
- `p0` = `0.35[Torr]` - Gas pressure
- `Tgas` = `300[K]` - Gas temperature
- `xO2` = `0.01` - O2 mole fraction
- `Qfeed` = `50` - Feed mass flow in SCCM
- `pw` = `200[W]` - Power absorbed
- `lambda_diff` = `((pi/L0)^2+(2.405/R0)^2)^-0.5` - Diffusion length
- `hl` = `1.0` - Correction factor
- `hc` = `0.4` - Correction factor
- `recO` = `0.005` - O recombination factor
- `recF` = `0.005` - F recombination factor
- `Vp` = `10[V]` - Potential drop
- `Eion` = `15[V]` - Ion energy

## 内容级读取结果

- 模型树证据数量: 26
- 理论关键词: 传热, 流体, 结构力学, 电磁, 优化/参数扫描
- geometry: r1 / Rectangle, geom1 / r1 / size
- physics: plas / ColdPlasma, Solid Mechanics
- mesh: mesh1
- studies: Stationary, Time Dependent, Parametric Sweep
- boundary_conditions: srg1 / 3, 4, srg2 / 2, srg3 / 2, r1 / size

### 文件内容证据

- `cf4_o2_si_etching_global_model.java`: 参数=14, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.plasma.cf4_o2_si_etching_global_model.pdf`: 参数=0, 几何=0, 物理场=0, 研究=3, 结果=0
- `double_walled_nanotube_silicon_electrode.pdf`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `cf4_o2_si_etching_global_model.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `cf4_o2_si_etching_global_model.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `cf4_o2_si_etching_global_model_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `double_walled_nanotube_silicon_electrode.pdf`: double_walled_nanotube_silicon_electrode.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics；核心内容可概括为：Geometry 1；Physics Definition
- `models.plasma.cf4_o2_si_etching_global_model.pdf`: models.plasma.cf4_o2_si_etching_global_model.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This tutorial studies the etching of silicon using a CF4/O2 plasma using a global model.

## PDF Summary and Physics Judgement

- Simple summary: cf4_o2_si_etching_global_model 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: This tutorial studies the etching of silicon using a CF4/O2 plasma using a global model.
- PDF: reduced. These advantages make a global model a good first approach to study a plasma
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: plas / ColdPlasma, Solid Mechanics
...
- PDF keywords: 传热, 流体, 结构力学, 电磁, 优化/参数扫描, double_walled_nanotube_silicon_electrode.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics；核心内容可概括为：Geometry 1；Physics Definition, models.plasma.cf4_o2_si_etching_global_model.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This tutorial studies the etching of silicon using a CF4/O2 plasma using a global model.
- `R0` -> Radius; MATLAB: `model.param.set('R0', value, description)`
- `L0` -> Length; MATLAB: `model.param.set('L0', value, description)`
- `p0` -> Gas pressure; MATLAB: `model.param.set('p0', value, description)`
- `Tgas` -> Gas temperature; MATLAB: `model.param.set('Tgas', value, description)`
- `xO2` -> O2 mole fraction; MATLAB: `model.param.set('xO2', value, description)`
- `Qfeed` -> Feed mass flow in SCCM; MATLAB: `model.param.set('Qfeed', value, description)`
- `pw` -> Power absorbed; MATLAB: `model.param.set('pw', value, description)`
- `lambda_diff` -> Diffusion length; MATLAB: `model.param.set('lambda_diff', value, description)`
- `hl` -> Correction factor; MATLAB: `model.param.set('hl', value, description)`
- `hc` -> Correction factor; MATLAB: `model.param.set('hc', value, description)`
- `recO` -> O recombination factor; MATLAB: `model.param.set('recO', value, description)`
- `recF` -> F recombination factor; MATLAB: `model.param.set('recF', value, description)`
...
- 已识别物理场证据：plas / ColdPlasma；Solid Mechanics
- 已识别参数：R0、L0、p0、Tgas、xO2、Qfeed、pw、lambda_diff、hl、hc、recO、recF
...
- Transfer recognized physics interfaces to similar requirements: plas / ColdPlasma, Solid Mechanics.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 流体, 结构力学, 电磁, 优化/参数扫描?
...
- Sweep `R0` around the learned value `10[cm]` and export target outputs to CSV.
- Sweep `L0` around the learned value `10[cm]` and export target outputs to CSV.
- Sweep `p0` around the learned value `0.35[Torr]` and export target outputs to CSV.
- Sweep `Tgas` around the learned value `300[K]` and export target outputs to CSV.
- Sweep `xO2` around the learned value `0.01` and export target outputs to CSV.
- Sweep `Qfeed` around the learned value `50` and export target outputs to CSV.
- Sweep `pw` around the learned value `200[W]` and export target outputs to CSV.
- Sweep `lambda_diff` around the learned value `((pi/L0)^2+(2.405/R0)^2)^-0.5` and export target outputs to CSV.
...
- Summary: 已学习 cf4_o2_si_etching_global_model 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=14；内容证据=26 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 26 条；几何=2，物理场=2，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：cf4_o2_si_etching_global_model.m, cf4_o2_si_etching_global_model.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 cf4_o2_si_etching_global_model，训练阶段为 modeling_logic_learning_ready。
...
- `cf4_o2_si_etching_global_model.mph` (comsol_mph, 19269978 bytes)
- `cf4_o2_si_etching_global_model.m` (matlab_livelink, 208568 bytes)
- `cf4_o2_si_etching_global_model.java` (comsol_java, 196205 bytes)
- `cf4_o2_si_etching_global_model_summary.json` (json, 3127 bytes)
- `double_walled_nanotube_silicon_electrode.pdf` (pdf_document, 457851 bytes)
- `models.plasma.cf4_o2_si_etching_global_model.pdf` (pdf_document, 267020 bytes)
