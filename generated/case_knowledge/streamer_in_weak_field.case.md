# streamer_in_weak_field

- Case directory: `D:\桌面\codex\案例下载\电气\弱电场中的正流注传播`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:42:12.525532+00:00`
...
- `V0` = `13[kV]` - Applied voltage
- `De` = `1800[cm^2/s]` - Electron diffusion coefficient

## 内容级读取结果

- 模型树证据数量: 28
- 理论关键词: 电磁, 结构力学, 流体
- geometry: r1 / Rectangle, pc1 / ParametricCurve, ls1 / LineSegment, ls2 / LineSegment, csol1 / ConvertToSolid, r2 / Rectangle, dif1 / Difference, geom1 / r1 / size, geom1 / r1 / layername, geom1 / r1 / layerleft, geom1 / r1 / layerbottom, geom1 / pc1 / parmax
- physics: Electrostatics
- mesh: mesh1
- studies: Time Dependent
- boundary_conditions: r1 / size, r1 / layername, r1 / layerleft, r1 / layerbottom, pc1 / parmax, pc1 / coord, r2 / size, r2 / pos

### 文件内容证据

- `streamer_in_weak_field.java`: 参数=2, 几何=15, 物理场=0, 研究=0, 结果=0
- `models.edis.streamer_in_weak_field.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `streamer_in_weak_field_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `streamer_in_weak_field.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `streamer_in_weak_field.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.streamer_in_weak_field.pdf`: models.edis.streamer_in_weak_field.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；pivotal in various scientific and technological realms, ranging from atmospheric physics to

## PDF Summary and Physics Judgement

- Simple summary: streamer_in_weak_field 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: pivotal in various scientific and technological realms, ranging from atmospheric physics to
- PDF: corona discharges, and electrical breakdown in gases. In this numerical modeling study, we
- PDF: This case study investigates a positive streamer between point-plate electrodes. Initially, a
...
- PDF keywords: 电磁, 结构力学, 流体, models.edis.streamer_in_weak_field.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；pivotal in various scientific and technological realms, ranging from atmospheric physics to
- `V0` -> Applied voltage; MATLAB: `model.param.set('V0', value, description)`
- `De` -> Electron diffusion coefficient; MATLAB: `model.param.set('De', value, description)`
...
- 已识别几何证据：r1 / Rectangle；pc1 / ParametricCurve；ls1 / LineSegment；ls2 / LineSegment；csol1 / ConvertToSolid；r2 / Rectangle
...
- Sweep `V0` around the learned value `13[kV]` and export target outputs to CSV.
- Sweep `De` around the learned value `1800[cm^2/s]` and export target outputs to CSV.
...
- Summary: 已学习 streamer_in_weak_field 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=15，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：streamer_in_weak_field.m, streamer_in_weak_field.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 streamer_in_weak_field，训练阶段为 modeling_logic_learning_ready。
...
- `streamer_in_weak_field.mph` (comsol_mph, 103195693 bytes)
- `streamer_in_weak_field.m` (matlab_livelink, 40345 bytes)
- `streamer_in_weak_field.java` (comsol_java, 38777 bytes)
- `streamer_in_weak_field_summary.json` (json, 957 bytes)
- `models.edis.streamer_in_weak_field.pdf` (pdf_document, 1219708 bytes)
