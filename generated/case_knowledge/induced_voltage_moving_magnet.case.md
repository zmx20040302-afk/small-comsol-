...
- Case directory: `D:\桌面\codex\案例下载\电气\移动磁铁在线圈中产生感应电压`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T15:33:58.438120+00:00`
...
- boundary_conditions: r1 / size, r1 / pos, r2 / size, r2 / pos, r3 / size, r3 / pos, r4 / size, r4 / pos, r5 / pos, r6 / size, r6 / pos

### 文件内容证据

- `induced_voltage_moving_magnet.java`: 参数=3, 几何=17, 物理场=0, 研究=0, 结果=0
- `models.acdc.induced_voltage_moving_magnet.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `induced_voltage_moving_magnet_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `induced_voltage_moving_magnet.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `induced_voltage_moving_magnet.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.induced_voltage_moving_magnet.pdf`: models.acdc.induced_voltage_moving_magnet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；magnet is significant, so the application uses a moving and sliding mesh.

## PDF Summary and Physics Judgement

- Simple summary: induced_voltage_moving_magnet 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: magnet is significant, so the application uses a moving and sliding mesh.
- PDF: results in a 2D axisymmetric problem, where the modeling space is a rectangular region in
- PDF: coil corners are not rounded off, which leads to a simpler mesh and a smaller problem size.
...
- PDF keywords: 电磁, 结构力学, models.acdc.induced_voltage_moving_magnet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；magnet is significant, so the application uses a moving and sliding mesh.
- `f0` -> Frequency of an oscillating magnet; MATLAB: `model.param.set('f0', value, description)`
- `T0` -> Time period of an oscillating magnet; MATLAB: `model.param.set('T0', value, description)`
...
- Sweep `f0` around the learned value `4[Hz]` and export target outputs to CSV.
...
- Summary: 已学习 induced_voltage_moving_magnet 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=36 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 36 条；几何=17，物理场=2，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：induced_voltage_moving_magnet.m, induced_voltage_moving_magnet.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 induced_voltage_moving_magnet，训练阶段为 modeling_logic_learning_ready。
...
- `induced_voltage_moving_magnet.mph` (comsol_mph, 13618906 bytes)
- `induced_voltage_moving_magnet.m` (matlab_livelink, 39214 bytes)
- `induced_voltage_moving_magnet.java` (comsol_java, 34463 bytes)
- `induced_voltage_moving_magnet_summary.json` (json, 1070 bytes)
- `models.acdc.induced_voltage_moving_magnet.pdf` (pdf_document, 1184376 bytes)
