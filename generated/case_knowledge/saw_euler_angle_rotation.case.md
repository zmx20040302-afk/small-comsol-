...
- Case directory: `D:\桌面\codex\案例下载\电气\SAW 模型中的欧拉角旋转`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:30:17.726960+00:00`
...
- `alpha_128` = `0[deg]` - Euler angle alpha
- `beta_128` = `-38[deg]` - Euler angle beta
- `gamma_128` = `0[deg]` - Euler angle gamma
- `lambdaG` = `8.97[um]` - Design period
- `c_apr` = `3997.8[m/s]` - Expected free SAW velocity
- `f_apr` = `0.98*c_apr/lambdaG` - Initial guess for eigenfrequency
...
- studies: Eigenfrequency

### 文件内容证据

- `saw_euler_angle_rotation.java`: 参数=6, 几何=0, 物理场=0, 研究=1, 结果=0
- `models.mems.saw_euler_angle_rotation.pdf`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `Euler_Angles_Rotation_in_SAW_Modeling.pptx`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `saw_euler_angle_rotation.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `saw_euler_angle_rotation.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `saw_euler_angle_rotation_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.saw_euler_angle_rotation.pdf`: models.mems.saw_euler_angle_rotation.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；In COMSOL Multiphysics, default material property matrices are defined with respect to

## PDF Summary and Physics Judgement

- Simple summary: saw_euler_angle_rotation 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenfrequency 研究。
...
- PDF: In COMSOL Multiphysics, default material property matrices are defined with respect to
- PDF: geometry window). In general, surface acoustic wave (SAW) devices use different crystal
- PDF: In the ZXZ convention adopted by COMSOL Multiphysics, the Euler angles α, β, and γ
- 脚本/文本识别到的物理场: Solid Mechanics, Electrostatics
- 识别到的研究类型: Eigenfrequency
- 理论关键词: 流体, 声学, 结构力学, 电磁

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 流体, 声学, 结构力学, 电磁, models.mems.saw_euler_angle_rotation.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；In COMSOL Multiphysics, default material property matrices are defined with respect to
- `alpha_128` -> Euler angle alpha; MATLAB: `model.param.set('alpha_128', value, description)`
- `beta_128` -> Euler angle beta; MATLAB: `model.param.set('beta_128', value, description)`
- `gamma_128` -> Euler angle gamma; MATLAB: `model.param.set('gamma_128', value, description)`
- `lambdaG` -> Design period; MATLAB: `model.param.set('lambdaG', value, description)`
- `c_apr` -> Expected free SAW velocity; MATLAB: `model.param.set('c_apr', value, description)`
- `f_apr` -> Initial guess for eigenfrequency; MATLAB: `model.param.set('f_apr', value, description)`
...
- 已识别参数：alpha_128、beta_128、gamma_128、lambdaG、c_apr、f_apr
...
- Use the learned study types as solver starting points: Eigenfrequency.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 声学, 结构力学, 电磁?
...
- Sweep `alpha_128` around the learned value `0[deg]` and export target outputs to CSV.
- Sweep `beta_128` around the learned value `-38[deg]` and export target outputs to CSV.
- Sweep `gamma_128` around the learned value `0[deg]` and export target outputs to CSV.
- Sweep `lambdaG` around the learned value `8.97[um]` and export target outputs to CSV.
- Sweep `c_apr` around the learned value `3997.8[m/s]` and export target outputs to CSV.
- Sweep `f_apr` around the learned value `0.98*c_apr/lambdaG` and export target outputs to CSV.
...
- Summary: 已学习 saw_euler_angle_rotation 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=12 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 12 条；几何=0，物理场=2，材料=0，网格=3，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：saw_euler_angle_rotation.m, saw_euler_angle_rotation.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 saw_euler_angle_rotation，训练阶段为 modeling_logic_learning_ready。
...
- `saw_euler_angle_rotation.mph` (comsol_mph, 108480121 bytes)
- `saw_euler_angle_rotation.m` (matlab_livelink, 93259 bytes)
- `saw_euler_angle_rotation.java` (comsol_java, 95645 bytes)
- `saw_euler_angle_rotation_summary.json` (json, 4924 bytes)
- `Euler_Angles_Rotation_in_SAW_Modeling.pptx` (presentation_document, 10431561 bytes)
- `models.mems.saw_euler_angle_rotation.pdf` (pdf_document, 1734214 bytes)
