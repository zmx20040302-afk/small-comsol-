...
- Case directory: `D:\桌面\codex\案例下载\电气\介质平板波导`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:49:56.362620+00:00`
...
- `lda0` = `1550[nm]` - Wavelength
- `n_core` = `1.5` - Refractive index, core
- `n_cladding` = `1` - Refractive index, cladding
- `h_core` = `1[um]` - Thickness, core
- `h_cladding` = `7[um]` - Thickness, cladding
- `w_slab` = `5[um]` - Slab width
- `k_core` = `2*pi[rad]*n_core/lda0` - Wave number, core
- `k_cladding` = `2*pi[rad]*n_cladding/lda0` - Wave number, cladding
...
- 模型树证据数量: 30
- 理论关键词: 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, geom1 / r1 / size, geom1 / r1 / base, geom1 / r2 / size, geom1 / r2 / base
- physics: ewfd / ElectromagneticWavesFrequencyDomain, Magnetic Fields, Global ODEs and DAEs, Electromagnetic Waves
- materials: mat1, mat2
- mesh: mesh1
- studies: Stationary, Eigenvalue, Frequency Domain
- boundary_conditions: port1 / 1, 3, 5, r1 / size, r1 / base, r2 / size, r2 / base

### 文件内容证据

- `dielectric_slab_waveguide.java`: 参数=9, 几何=6, 物理场=1, 研究=0, 结果=0
- `models.woptics.dielectric_slab_waveguide.pdf`: 参数=0, 几何=0, 物理场=3, 研究=3, 结果=0
- `dielectric_slab_waveguide.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `dielectric_slab_waveguide.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `dielectric_slab_waveguide_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.dielectric_slab_waveguide.pdf`: models.woptics.dielectric_slab_waveguide.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Global ODEs and DAEs、Electromagnetic Waves，研究类型线索为 Stationary、Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；for the fields, and compares to analytic results.

## PDF Summary and Physics Judgement

- Simple summary: dielectric_slab_waveguide 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 ElectromagneticWavesFrequencyDomain，并采用 Eigenvalue 研究。
- Primary physics: `ElectromagneticWavesFrequencyDomain`
- Study type: `Eigenvalue`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 ElectromagneticWavesFrequencyDomain，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Eigenvalue 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: for the fields, and compares to analytic results.
- PDF: COMSOL Multiphysics detects a system of nonlinear equations, the only requirement
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: ewfd / ElectromagneticWavesFrequencyDomain, Magnetic Fields, Global ODEs and DAEs, Electromagnetic Waves
- 识别到的研究类型: Stationary, Eigenvalue, Frequency Domain
- 理论关键词: 电磁

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 电磁, models.woptics.dielectric_slab_waveguide.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Global ODEs and DAEs、Electromagnetic Waves，研究类型线索为 Stationary、Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；for the fields, and compares to analytic results.
- `lda0` -> Wavelength; MATLAB: `model.param.set('lda0', value, description)`
- `n_core` -> Refractive index, core; MATLAB: `model.param.set('n_core', value, description)`
- `n_cladding` -> Refractive index, cladding; MATLAB: `model.param.set('n_cladding', value, description)`
- `h_core` -> Thickness, core; MATLAB: `model.param.set('h_core', value, description)`
- `h_cladding` -> Thickness, cladding; MATLAB: `model.param.set('h_cladding', value, description)`
- `w_slab` -> Slab width; MATLAB: `model.param.set('w_slab', value, description)`
- `k_core` -> Wave number, core; MATLAB: `model.param.set('k_core', value, description)`
- `k_cladding` -> Wave number, cladding; MATLAB: `model.param.set('k_cladding', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；geom1 / r1 / size；geom1 / r1 / base；geom1 / r2 / size；geom1 / r2 / base
- 已识别物理场证据：ewfd / ElectromagneticWavesFrequencyDomain；Magnetic Fields；Global ODEs and DAEs；Electromagnetic Waves
- 已识别参数：lda0、n_core、n_cladding、h_core、h_cladding、w_slab、k_core、k_cladding、f0
...
- Transfer recognized physics interfaces to similar requirements: ewfd / ElectromagneticWavesFrequencyDomain, Magnetic Fields, Global ODEs and DAEs, Electromagnetic Waves.
- Reuse geometry construction patterns when new requirements share shape, import, array, sweep, or boolean operations.
- Use the learned study types as solver starting points: Stationary, Eigenvalue, Frequency Domain.
...
- Sweep `lda0` around the learned value `1550[nm]` and export target outputs to CSV.
- Sweep `n_core` around the learned value `1.5` and export target outputs to CSV.
- Sweep `n_cladding` around the learned value `1` and export target outputs to CSV.
- Sweep `h_core` around the learned value `1[um]` and export target outputs to CSV.
- Sweep `h_cladding` around the learned value `7[um]` and export target outputs to CSV.
- Sweep `w_slab` around the learned value `5[um]` and export target outputs to CSV.
- Sweep `k_core` around the learned value `2*pi[rad]*n_core/lda0` and export target outputs to CSV.
- Sweep `k_cladding` around the learned value `2*pi[rad]*n_cladding/lda0` and export target outputs to CSV.
...
- Summary: 已学习 dielectric_slab_waveguide 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=30 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 30 条；几何=6，物理场=4，材料=2，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：dielectric_slab_waveguide.m, dielectric_slab_waveguide.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 dielectric_slab_waveguide，训练阶段为 modeling_logic_learning_ready。
...
- `dielectric_slab_waveguide.mph` (comsol_mph, 8331372 bytes)
- `dielectric_slab_waveguide.m` (matlab_livelink, 22079 bytes)
- `dielectric_slab_waveguide.java` (comsol_java, 19834 bytes)
- `dielectric_slab_waveguide_summary.json` (json, 1228 bytes)
- `models.woptics.dielectric_slab_waveguide.pdf` (pdf_document, 692075 bytes)
