...
- Case directory: `D:\桌面\codex\案例下载\电气\通过 PVT 法实现 SiC 的外延生长`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:27:33.199268+00:00`
...
- `r_seed` = `30[mm]` - Radius of seed crystal
- `h_seed` = `5[mm]` - Height of seed crystal
- `r_chamber` = `65[mm]` - Radius of chamber
- `h_chamber` = `80[mm]` - Height of chamber
- `t_side` = `6[mm]` - Side thickness of furnace
- `t_cap` = `20[mm]` - Top/Bottom cap thickness of furnace
- `h_melt` = `50[mm]` - Height of melt
- `t_felt` = `40[mm]` - Thickness of air
- `r_coil` = `5[mm]` - Radius of coil wire
- `pr_coil_1` = `150[mm]` - Position of coil section 1, r
- `pz_coil_1` = `-20[mm]` - Position of coil section 1, z
- `n_coil_1` = `4` - Turns of coil
- `d_coil_1` = `25[mm]` - Distance between coil turns
- `r_rod_u` = `10[mm]` - Radius of upper rod
- `r_rod_d` = `5[mm]` - Radius of lower rod
- `r_air` = `300[mm]` - Radius of air domain
- `h_air` = `400[mm]` - Height of air domain
- `pz_air` = `-150[mm]` - Position of air domain
- `freq` = `16[kHz]` - Operating frequency
...
- `SiC_PVT_62_clearsol.java`: 参数=19, 几何=0, 物理场=1, 研究=0, 结果=0
- `SiC_PVT_62_clearsol.m`: 参数=19, 几何=0, 物理场=0, 研究=0, 结果=0
- `SiC_PVT_62_clearsol.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `SiC_PVT_62_clearsol_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: SiC_PVT_62_clearsol 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- `r_seed` -> Radius of seed crystal; MATLAB: `model.param.set('r_seed', value, description)`
- `h_seed` -> Height of seed crystal; MATLAB: `model.param.set('h_seed', value, description)`
- `r_chamber` -> Radius of chamber; MATLAB: `model.param.set('r_chamber', value, description)`
- `h_chamber` -> Height of chamber; MATLAB: `model.param.set('h_chamber', value, description)`
- `t_side` -> Side thickness of furnace; MATLAB: `model.param.set('t_side', value, description)`
- `t_cap` -> Top/Bottom cap thickness of furnace; MATLAB: `model.param.set('t_cap', value, description)`
- `h_melt` -> Height of melt; MATLAB: `model.param.set('h_melt', value, description)`
- `t_felt` -> Thickness of air; MATLAB: `model.param.set('t_felt', value, description)`
- `r_coil` -> Radius of coil wire; MATLAB: `model.param.set('r_coil', value, description)`
- `pr_coil_1` -> Position of coil section 1, r; MATLAB: `model.param.set('pr_coil_1', value, description)`
- `pz_coil_1` -> Position of coil section 1, z; MATLAB: `model.param.set('pz_coil_1', value, description)`
- `n_coil_1` -> Turns of coil; MATLAB: `model.param.set('n_coil_1', value, description)`
...
- 已识别参数：r_seed、h_seed、r_chamber、h_chamber、t_side、t_cap、h_melt、t_felt、r_coil、pr_coil_1、pz_coil_1、n_coil_1
...
- Sweep `r_seed` around the learned value `30[mm]` and export target outputs to CSV.
- Sweep `h_seed` around the learned value `5[mm]` and export target outputs to CSV.
- Sweep `r_chamber` around the learned value `65[mm]` and export target outputs to CSV.
- Sweep `h_chamber` around the learned value `80[mm]` and export target outputs to CSV.
- Sweep `t_side` around the learned value `6[mm]` and export target outputs to CSV.
- Sweep `t_cap` around the learned value `20[mm]` and export target outputs to CSV.
- Sweep `h_melt` around the learned value `50[mm]` and export target outputs to CSV.
- Sweep `t_felt` around the learned value `40[mm]` and export target outputs to CSV.
...
- Summary: 已学习 SiC_PVT_62_clearsol 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=19；内容证据=20 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：SiC_PVT_62_clearsol.m, SiC_PVT_62_clearsol.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 SiC_PVT_62_clearsol，训练阶段为 modeling_logic_learning_ready。
...
- `SiC_PVT_62_clearsol.mph` (comsol_mph, 3815623 bytes)
- `SiC_PVT_62_clearsol.m` (matlab_livelink, 151868 bytes)
- `SiC_PVT_62_clearsol.java` (comsol_java, 161771 bytes)
- `SiC_PVT_62_clearsol_summary.json` (json, 3602 bytes)
