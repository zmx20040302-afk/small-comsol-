# radially_polarized_piezo_63_cleared

- Case directory: `D:\桌面\codex\案例下载\电气\径向极化压电换能器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:44:41.368104+00:00`
...
- 模型树证据数量: 16
- 理论关键词: 结构力学, 电磁
- geometry: wp1 / WorkPlane, r1 / Rectangle, rev1 / Revolve, geom1 / wp1 / quickplane, geom1 / wp1 / unite, geom1 / rev1 / angtype, geom1 / fin / repairtoltype
- physics: solid / SolidMechanics
- materials: mat1
- mesh: mesh1
- boundary_conditions: wp1 / quickplane, wp1 / unite, r1 / size, r1 / pos, rev1 / angtype, fin / repairtoltype

### 文件内容证据

- `radially_polarized_piezo_63_cleared.java`: 参数=0, 几何=7, 物理场=1, 研究=0, 结果=0
- `radially_polarized_piezo_63_cleared.m`: 参数=0, 几何=7, 物理场=0, 研究=0, 结果=0
- `radially_polarized_piezo_63.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `radially_polarized_piezo_63_cleared.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `radially_polarized_piezo_63_cleared_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `radially_polarized_piezo_63.pdf`: radially_polarized_piezo_63.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、电磁；核心内容可概括为：Applied voltage results

## PDF Summary and Physics Judgement

- Simple summary: radially_polarized_piezo_63_cleared 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics。
...
- PDF keywords: 结构力学, 电磁, radially_polarized_piezo_63.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、电磁；核心内容可概括为：Applied voltage results
...
- 已识别几何证据：wp1 / WorkPlane；r1 / Rectangle；rev1 / Revolve；geom1 / wp1 / quickplane；geom1 / wp1 / unite；geom1 / rev1 / angtype
...
- Summary: 已学习 radially_polarized_piezo_63_cleared 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=16 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 16 条；几何=7，物理场=1，材料=1，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：radially_polarized_piezo_63_cleared.m, radially_polarized_piezo_63_cleared.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 radially_polarized_piezo_63_cleared，训练阶段为 modeling_logic_learning_ready。
...
- `radially_polarized_piezo_63_cleared.mph` (comsol_mph, 762852 bytes)
- `radially_polarized_piezo_63_cleared.m` (matlab_livelink, 25109 bytes)
- `radially_polarized_piezo_63_cleared.java` (comsol_java, 26512 bytes)
- `radially_polarized_piezo_63_cleared_summary.json` (json, 1037 bytes)
- `radially_polarized_piezo_63.pdf` (pdf_document, 922042 bytes)
