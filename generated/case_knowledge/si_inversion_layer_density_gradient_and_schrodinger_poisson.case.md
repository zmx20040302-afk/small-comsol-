...
- Case directory: `D:\桌面\codex\案例下载\电气\硅反型层的密度梯度和薛定谔-泊松结果`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T03:56:53.655844+00:00`
...
- `mnDG` = `me_const/3` - Density-gradient effective mass
- `Na0` = `3.8e16[1/cm^3]` - Doping concentration
- `Vg` = `0[V]` - Gate voltage
- `epsrOx` = `3.9` - Oxide dielectric constant
- `dOx` = `3.1[nm]` - Oxide thickness
- `Phi0` = `4.01[V]` - Gate metal work function

## 内容级读取结果

- 模型树证据数量: 17
- 理论关键词: 传热, 电磁, 量子/特征值, 结构力学, 流体, 优化/参数扫描
- geometry: i1 / Interval, geom1 / i1 / coordsource, geom1 / i1 / coordvec
- physics: Electrostatics
- mesh: mesh1
- studies: Stationary, Eigenvalue, Parametric Sweep
- boundary_conditions: i1 / coordsource, i1 / coordvec

### 文件内容证据

- `si_inversion_layer_density_gradient_and_schrodinger_poisson.java`: 参数=7, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.semicond.si_inversion_layer_density_gradient_and_schrodinger_poisson.pdf`: 参数=0, 几何=0, 物理场=1, 研究=3, 结果=0
- `si_inversion_layer_density_gradient_and_schrodinger_poisson_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `si_inversion_layer_density_gradient_and_schrodinger_poisson.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `si_inversion_layer_density_gradient_and_schrodinger_poisson.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.si_inversion_layer_density_gradient_and_schrodinger_poisson.pdf`: models.semicond.si_inversion_layer_density_gradient_and_schrodinger_poisson.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results for a Silicon Inversion Layer

## PDF Summary and Physics Judgement

- Simple summary: si_inversion_layer_density_gradient_and_schrodinger_poisson 主要研究量子能级或波函数分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Eigenvalue 研究。
...
- PDF: Results for a Silicon Inversion Layer
- PDF: 2 | DENSITY-GRADIENT AND SCHRÖDINGER–POISSON RESULTS FOR A SILICON
- PDF: of quantum confinement in the device physics simulation of a silicon inversion layer. This
- 脚本/文本识别到的物理场: Electrostatics
- 识别到的研究类型: Stationary, Eigenvalue, Parametric Sweep
- 理论关键词: 传热, 电磁, 量子/特征值, 结构力学, 流体, 优化/参数扫描

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 传热, 电磁, 量子/特征值, 结构力学, 流体, 优化/参数扫描, models.semicond.si_inversion_layer_density_gradient_and_schrodinger_poisson.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results for a Silicon Inversion Layer
- `T0` -> Temperature; MATLAB: `model.param.set('T0', value, description)`
- `mnDG` -> Density-gradient effective mass; MATLAB: `model.param.set('mnDG', value, description)`
- `Na0` -> Doping concentration; MATLAB: `model.param.set('Na0', value, description)`
- `Vg` -> Gate voltage; MATLAB: `model.param.set('Vg', value, description)`
- `epsrOx` -> Oxide dielectric constant; MATLAB: `model.param.set('epsrOx', value, description)`
- `dOx` -> Oxide thickness; MATLAB: `model.param.set('dOx', value, description)`
- `Phi0` -> Gate metal work function; MATLAB: `model.param.set('Phi0', value, description)`
...
- 已识别几何证据：i1 / Interval；geom1 / i1 / coordsource；geom1 / i1 / coordvec
- 已识别物理场证据：Electrostatics
- 已识别参数：T0、mnDG、Na0、Vg、epsrOx、dOx、Phi0
...
- Use the learned study types as solver starting points: Stationary, Eigenvalue, Parametric Sweep.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 电磁, 量子/特征值, 结构力学, 流体, 优化/参数扫描?
...
- Sweep `mnDG` around the learned value `me_const/3` and export target outputs to CSV.
- Sweep `Na0` around the learned value `3.8e16[1/cm^3]` and export target outputs to CSV.
- Sweep `Vg` around the learned value `0[V]` and export target outputs to CSV.
- Sweep `epsrOx` around the learned value `3.9` and export target outputs to CSV.
- Sweep `dOx` around the learned value `3.1[nm]` and export target outputs to CSV.
- Sweep `Phi0` around the learned value `4.01[V]` and export target outputs to CSV.
...
- Summary: 已学习 si_inversion_layer_density_gradient_and_schrodinger_poisson 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=17 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 17 条；几何=3，物理场=1，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：si_inversion_layer_density_gradient_and_schrodinger_poisson.m, si_inversion_layer_density_gradient_and_schrodinger_poisson.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 si_inversion_layer_density_gradient_and_schrodinger_poisson，训练阶段为 modeling_logic_learning_ready。
...
- `si_inversion_layer_density_gradient_and_schrodinger_poisson.mph` (comsol_mph, 32367358 bytes)
- `si_inversion_layer_density_gradient_and_schrodinger_poisson.m` (matlab_livelink, 48572 bytes)
- `si_inversion_layer_density_gradient_and_schrodinger_poisson.java` (comsol_java, 42180 bytes)
- `si_inversion_layer_density_gradient_and_schrodinger_poisson_summary.json` (json, 1728 bytes)
- `models.semicond.si_inversion_layer_density_gradient_and_schrodinger_poisson.pdf` (pdf_document, 326175 bytes)
