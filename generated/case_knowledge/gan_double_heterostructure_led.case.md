...
- Case directory: `D:\桌面\codex\案例下载\电气\InGaNAlGaN 双异质结 LED`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:43:47.349790+00:00`
...
- `V_n` = `0[V]` - n-contact voltage
- `V_p` = `0[V]` - p-contact voltage
- `InGaN_bg` = `((h_const*c_const)/450[nm])/(1.6e-19[C])` - Band gap energy of InGaN layer
- `I_p` = `1e-6[A]` - p-contact current
- `A_cross` = `200[um]*200[um]` - Cross sectional area

## 内容级读取结果

- 模型树证据数量: 19
- 理论关键词: 电磁, 传热
- geometry: i1 / Interval, i2 / Interval, i3 / Interval, geom1 / i1 / coord, geom1 / i2 / coord, geom1 / i3 / coord
- materials: mat1, mat2
- mesh: mesh1
- studies: Stationary, Frequency Domain
- boundary_conditions: i1 / coord, i2 / coord, i3 / coord

### 文件内容证据

- `gan_double_heterostructure_led.java`: 参数=5, 几何=6, 物理场=0, 研究=0, 结果=0
- `models.semicond.gan_double_heterostructure_led.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `gan_double_heterostructure_led.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `gan_double_heterostructure_led.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `gan_double_heterostructure_led_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.gan_double_heterostructure_led.pdf`: models.semicond.gan_double_heterostructure_led.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；included in the model.

## PDF Summary and Physics Judgement

- Simple summary: gan_double_heterostructure_led 主要研究量子能级或波函数分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: included in the model.
- PDF: Noble Prize in Physics was awarded to Isamu Akasaki, Hiroshi Amano, and Shuji
- PDF: The materials and geometry of this model are based on the device described by Nakamura
...
- PDF keywords: 电磁, 传热, models.semicond.gan_double_heterostructure_led.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；included in the model.
...
- `InGaN_bg` -> Band gap energy of InGaN layer; MATLAB: `model.param.set('InGaN_bg', value, description)`
- `I_p` -> p-contact current; MATLAB: `model.param.set('I_p', value, description)`
- `A_cross` -> Cross sectional area; MATLAB: `model.param.set('A_cross', value, description)`
...
- 已识别几何证据：i1 / Interval；i2 / Interval；i3 / Interval；geom1 / i1 / coord；geom1 / i2 / coord；geom1 / i3 / coord
- 已识别参数：V_n、V_p、InGaN_bg、I_p、A_cross
...
- Sweep `InGaN_bg` around the learned value `((h_const*c_const)/450[nm])/(1.6e-19[C])` and export target outputs to CSV.
- Sweep `I_p` around the learned value `1e-6[A]` and export target outputs to CSV.
- Sweep `A_cross` around the learned value `200[um]*200[um]` and export target outputs to CSV.
...
- Summary: 已学习 gan_double_heterostructure_led 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=19 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 19 条；几何=6，物理场=0，材料=2，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：gan_double_heterostructure_led.m, gan_double_heterostructure_led.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 gan_double_heterostructure_led，训练阶段为 modeling_logic_learning_ready。
...
- `gan_double_heterostructure_led.mph` (comsol_mph, 8820838 bytes)
- `gan_double_heterostructure_led.m` (matlab_livelink, 44762 bytes)
- `gan_double_heterostructure_led.java` (comsol_java, 40851 bytes)
- `gan_double_heterostructure_led_summary.json` (json, 1595 bytes)
- `models.semicond.gan_double_heterostructure_led.pdf` (pdf_document, 511224 bytes)
