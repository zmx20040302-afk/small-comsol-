# magnetic_bearing_force_calculation

- Case directory: `D:\桌面\codex\案例下载\电气\使用虚功和麦克斯韦应力张量计算电磁力`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:18:12.090398+00:00`
...
- mesh: mesh1
- boundary_conditions: r1 / size, r1 / pos, r1 / layername, r1 / layertop, r2 / size, r2 / pos, r2 / layername, r2 / layertop, r3 / size, r3 / pos, r3 / layername

### 文件内容证据

- `magnetic_bearing_force_calculation.java`: 参数=7, 几何=14, 物理场=0, 研究=0, 结果=0
- `magnetic_bearing_force_calculation_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `magnetic_bearing_force_calculation.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetic_bearing_force_calculation.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: magnetic_bearing_force_calculation 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics。
...
- `Z0` -> 案例参数/扫描变量; MATLAB: `model.param.set('Z0', value, description)`
...
- Sweep `Z0` around the learned value `0[mm]` and export target outputs to CSV.
...
- Summary: 已学习 magnetic_bearing_force_calculation 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=34 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 34 条；几何=14，物理场=1，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：magnetic_bearing_force_calculation.m, magnetic_bearing_force_calculation.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 magnetic_bearing_force_calculation，训练阶段为 modeling_logic_learning_ready。
...
- `magnetic_bearing_force_calculation.mph` (comsol_mph, 50148276 bytes)
- `magnetic_bearing_force_calculation.m` (matlab_livelink, 41033 bytes)
- `magnetic_bearing_force_calculation.java` (comsol_java, 38516 bytes)
- `magnetic_bearing_force_calculation_summary.json` (json, 1144 bytes)
