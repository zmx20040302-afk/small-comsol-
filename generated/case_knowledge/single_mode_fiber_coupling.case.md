...
- Case directory: `D:\桌面\codex\案例下载\电气\单模光纤到光纤耦合`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:48:26.460762+00:00`
...
- `h_clad` = `40[um]` - Cladding width
- `h_core` = `4[um]` - Core width
- `l_fiber` = `100[um]` - Fiber length
- `f_lens` = `50[um]` - Focal length
- `t_lens` = `15[um]` - Lens thickness
- `df` = `-4[um]` - Focus adjustment
- `t_PML` = `1[um]` - Perfectly matched layer thickness
- `n_lens` = `1.5` - Refractive index, lens
- `n_clad` = `1.5` - Refractive index, cladding
- `n_core` = `1.51` - Refractive index, core
- `lda0` = `1[um]` - Vacuum wavelength
- `f0` = `c_const/lda0` - Frequency
- `l_dom` = `f_lens*2` - Half of the coupling length
- `lamc` = `2*pi*h_core/2/2.405*sqrt(n_core^2-n_clad^2)` - Single mode critical wavelength
- `V` = `2*pi/lda0*h_core/2*sqrt(n_core^2-n_clad^2)` - Fiber parameter
- `r_lens` = `(n_lens-1)*f_lens` - Lens radius of curvature

## 内容级读取结果

- 模型树证据数量: 38
- 理论关键词: 电磁, 传热, 结构力学, 优化/参数扫描
- geometry: r1 / Rectangle, r2 / Rectangle, geom1 / r1 / size, geom1 / r1 / base, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r2 / base, geom1 / r2 / pos, geom1 / r2 / layername, geom1 / r2 / layertop
- physics: Electromagnetic Waves
- mesh: mesh1
- studies: Frequency Domain, Parametric Sweep
- boundary_conditions: r1 / size, r1 / base, r1 / pos, r2 / size, r2 / base, r2 / pos, r2 / layername, r2 / layertop

### 文件内容证据

- `single_mode_fiber_coupling.java`: 参数=16, 几何=10, 物理场=0, 研究=0, 结果=0
- `models.woptics.single_mode_fiber_coupling.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `FiberCoupling.pptx`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `single_mode_fiber_coupling.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `single_mode_fiber_coupling.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `single_mode_fiber_coupling_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.single_mode_fiber_coupling.pdf`: models.woptics.single_mode_fiber_coupling.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；suppress reflections. The geometry is surrounded by Perfectly Matched Layers to absorb

## PDF Summary and Physics Judgement

- Simple summary: single_mode_fiber_coupling 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- PDF: suppress reflections. The geometry is surrounded by Perfectly Matched Layers to absorb
- PDF: To reduce the necessary number of mesh elements along the optical axis and make efficient
- PDF: propagation constant ewbe.beta_1 is solved for in a Boundary Mode Analysis study step.
...
- PDF keywords: 电磁, 传热, 结构力学, 优化/参数扫描, models.woptics.single_mode_fiber_coupling.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；suppress reflections. The geometry is surrounded by Perfectly Matched Layers to absorb
- `h_clad` -> Cladding width; MATLAB: `model.param.set('h_clad', value, description)`
- `h_core` -> Core width; MATLAB: `model.param.set('h_core', value, description)`
- `l_fiber` -> Fiber length; MATLAB: `model.param.set('l_fiber', value, description)`
- `f_lens` -> Focal length; MATLAB: `model.param.set('f_lens', value, description)`
- `t_lens` -> Lens thickness; MATLAB: `model.param.set('t_lens', value, description)`
- `df` -> Focus adjustment; MATLAB: `model.param.set('df', value, description)`
- `t_PML` -> Perfectly matched layer thickness; MATLAB: `model.param.set('t_PML', value, description)`
- `n_lens` -> Refractive index, lens; MATLAB: `model.param.set('n_lens', value, description)`
- `n_clad` -> Refractive index, cladding; MATLAB: `model.param.set('n_clad', value, description)`
- `n_core` -> Refractive index, core; MATLAB: `model.param.set('n_core', value, description)`
- `lda0` -> Vacuum wavelength; MATLAB: `model.param.set('lda0', value, description)`
...
- 已识别参数：h_clad、h_core、l_fiber、f_lens、t_lens、df、t_PML、n_lens、n_clad、n_core、lda0、f0
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 传热, 结构力学, 优化/参数扫描?
...
- Sweep `h_clad` around the learned value `40[um]` and export target outputs to CSV.
- Sweep `h_core` around the learned value `4[um]` and export target outputs to CSV.
- Sweep `l_fiber` around the learned value `100[um]` and export target outputs to CSV.
- Sweep `f_lens` around the learned value `50[um]` and export target outputs to CSV.
- Sweep `t_lens` around the learned value `15[um]` and export target outputs to CSV.
- Sweep `df` around the learned value `-4[um]` and export target outputs to CSV.
- Sweep `t_PML` around the learned value `1[um]` and export target outputs to CSV.
- Sweep `n_lens` around the learned value `1.5` and export target outputs to CSV.
...
- Summary: 已学习 single_mode_fiber_coupling 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=16；内容证据=38 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 38 条；几何=10，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：single_mode_fiber_coupling.m, single_mode_fiber_coupling.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 single_mode_fiber_coupling，训练阶段为 modeling_logic_learning_ready。
...
- `single_mode_fiber_coupling.mph` (comsol_mph, 144420946 bytes)
- `single_mode_fiber_coupling.m` (matlab_livelink, 32613 bytes)
- `single_mode_fiber_coupling.java` (comsol_java, 28715 bytes)
- `single_mode_fiber_coupling_summary.json` (json, 1483 bytes)
- `FiberCoupling.pptx` (presentation_document, 848292 bytes)
- `models.woptics.single_mode_fiber_coupling.pdf` (pdf_document, 1501711 bytes)
