...
- Case directory: `D:\桌面\codex\案例下载\电气\采用 TSSG 方法制备 SiC 晶片`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:26:39.120863+00:00`
...
- `Ac` = `20[mm]*35[mm]` - Cooling channel cross-sectional area
- `Mt` = `1[kg/min]` - Cooling water mass flow rate
- `Tin` = `20[degC]` - Cooling water inlet temperature
- `rotate_seed` = `20[rpm]` - Seed crystal speed
- `rotate_crucible` = `5[rpm]` - Crucible speed
- `M_sic` = `40[g/mol]` - Molar mass of silicon carbide
- `rho_sic` = `3215[kg/m^3]` - Density of silicon carbide
- `gamma` = `-2.5e-4[N/(m*K)]` - Surface tension temperature derivative
- `M_si` = `28[g/mol]` - Molar mass of silicon
- `Tref` = `293.15[K]` - Reference temperature
- `pref` = `1[atm]` - Reference pressure
- `D` = `4.2e-8[m^2/s]` - Diffusion coefficient of carbon in silicon

## 内容级读取结果

- 模型树证据数量: 37
- 理论关键词: 传热, 流体, 电磁, 多孔介质/裂隙, 优化/参数扫描
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, r4 / Rectangle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r2 / pos, geom1 / r3 / size, geom1 / r3 / pos, r5 / Rectangle, geom1 / r4 / size
- physics: Heat Transfer, Optimization
- mesh: mesh1
- studies: Optimization
- boundary_conditions: r1 / size, r1 / pos, r2 / size, r2 / pos, r3 / size, r3 / pos, r4 / size, r4 / pos

### 文件内容证据

- `SiC_TSSG.java`: 参数=12, 几何=13, 物理场=0, 研究=0, 结果=0
- `SiC_TSSG.m`: 参数=12, 几何=10, 物理场=0, 研究=0, 结果=0
- `Preparation of SiC Wafers Using the TSSG Method.pptx`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `SiC_TSSG.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `SiC_TSSG_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: SiC_TSSG 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Optimization 研究。
...
- 脚本/文本识别到的物理场: Heat Transfer, Optimization
...
- PDF keywords: 传热, 流体, 电磁, 多孔介质/裂隙, 优化/参数扫描
- `Ac` -> Cooling channel cross-sectional area; MATLAB: `model.param.set('Ac', value, description)`
- `Mt` -> Cooling water mass flow rate; MATLAB: `model.param.set('Mt', value, description)`
- `Tin` -> Cooling water inlet temperature; MATLAB: `model.param.set('Tin', value, description)`
- `rotate_seed` -> Seed crystal speed; MATLAB: `model.param.set('rotate_seed', value, description)`
- `rotate_crucible` -> Crucible speed; MATLAB: `model.param.set('rotate_crucible', value, description)`
- `M_sic` -> Molar mass of silicon carbide; MATLAB: `model.param.set('M_sic', value, description)`
- `rho_sic` -> Density of silicon carbide; MATLAB: `model.param.set('rho_sic', value, description)`
- `gamma` -> Surface tension temperature derivative; MATLAB: `model.param.set('gamma', value, description)`
- `M_si` -> Molar mass of silicon; MATLAB: `model.param.set('M_si', value, description)`
- `Tref` -> Reference temperature; MATLAB: `model.param.set('Tref', value, description)`
- `pref` -> Reference pressure; MATLAB: `model.param.set('pref', value, description)`
- `D` -> Diffusion coefficient of carbon in silicon; MATLAB: `model.param.set('D', value, description)`
...
- 已识别物理场证据：Heat Transfer；Optimization
- 已识别参数：Ac、Mt、Tin、rotate_seed、rotate_crucible、M_sic、rho_sic、gamma、M_si、Tref、pref、D
...
- Transfer recognized physics interfaces to similar requirements: Heat Transfer, Optimization.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 流体, 电磁, 多孔介质/裂隙, 优化/参数扫描?
...
- Sweep `Ac` around the learned value `20[mm]*35[mm]` and export target outputs to CSV.
- Sweep `Mt` around the learned value `1[kg/min]` and export target outputs to CSV.
- Sweep `Tin` around the learned value `20[degC]` and export target outputs to CSV.
- Sweep `rotate_seed` around the learned value `20[rpm]` and export target outputs to CSV.
- Sweep `rotate_crucible` around the learned value `5[rpm]` and export target outputs to CSV.
- Sweep `M_sic` around the learned value `40[g/mol]` and export target outputs to CSV.
- Sweep `rho_sic` around the learned value `3215[kg/m^3]` and export target outputs to CSV.
- Sweep `gamma` around the learned value `-2.5e-4[N/(m*K)]` and export target outputs to CSV.
...
- Summary: 已学习 SiC_TSSG 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=37 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 37 条；几何=13，物理场=2，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：SiC_TSSG.m, SiC_TSSG.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 SiC_TSSG，训练阶段为 modeling_logic_learning_ready。
...
- `SiC_TSSG.mph` (comsol_mph, 37474477 bytes)
- `SiC_TSSG.m` (matlab_livelink, 121537 bytes)
- `SiC_TSSG.java` (comsol_java, 129074 bytes)
- `SiC_TSSG_summary.json` (json, 3520 bytes)
- `Preparation of SiC Wafers Using the TSSG Method.pptx` (presentation_document, 15450313 bytes)
