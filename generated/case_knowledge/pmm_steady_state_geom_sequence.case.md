# pmm_steady_state_geom_sequence

- Case directory: `D:\桌面\codex\案例下载\电气\永磁电机的稳定工况`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T15:34:21.140004+00:00`
...
- `Ns` = `60` - Number of slots
- `Nsec` = `gcd(Np,Ns)` - Number of modeled symmetric/antiperiodic sectors
- `stator_core_rout` = `120[mm]` - Stator core outer radius
- `stator_core_th` = `stator_core_rout*0.3` - Stator core radial thickness
- `stator_core_rin` = `stator_core_rout-stator_core_th` - Stator core inner radius
- `N_coil_slot` = `2` - Number of coils per slot
- `airgap` = `2[mm]` - Air gap thickness between stator and rotor
- `slot_pitch_frc` = `0.5` - Stator core slot pitch fraction (0-1)
- `slot_shoe_tip_height` = `1[mm]` - Stator slot shoe tip height
- `slot_shoe_angle` = `20[deg]` - Slot shoe angle
- `slot_opening` = `2.5[mm]` - Slot opening width
- `slot_width` = `stator_core_rin*2*sin(pi/Ns*slot_pitch_frc)` - Stator slot width
- `slot_height` = `stator_core_th*0.7` - Stator slot height
- `slot_shoe_base_height` = `slot_shoe_tip_height+(slot_width-slot_opening)/2*tan(slot_shoe_angle)` - Stator slot shoe base height
- `coil_height` = `(slot_height-slot_shoe_base_height)/N_coil_slot` - Coil height
- `rotor_core_rout` = `stator_core_rin-airgap` - Rotor core outer radius
- `shaft_rout` = `rotor_core_rout*.2` - Shaft radius
- `rotor_bridge` = `1.5[mm]` - Rotor iron thickness between airgap and outmost flux barriers/ air pockets
- `pole_bridge` = `2[mm]` - Rotor iron thickness between poles
- `v_bridge` = `2[mm]` - Rotor iron thickness between magnets placed in v-shape
- `mag_ang` = `20[deg]` - Angle of magnets placed in v-shape
- `magnet_height` = `5[mm]` - Magnet height
- `flux_barrier` = `2[mm]` - Width of flux barrier (inversely proportional to magnet width)

## 内容级读取结果

- 模型树证据数量: 37
- 理论关键词: 传热, 电磁
- geometry: c1 / Circle, geom1 / c1 / r, geom1 / c1 / angle, c2 / Circle, dif1 / Difference, geom1 / c2 / r, geom1 / c2 / angle
- mesh: mesh1
- studies: Stationary
- boundary_conditions: c1 / r, c1 / angle, c2 / r, c2 / angle

### 文件内容证据

- `pmm_steady_state_geom_sequence.java`: 参数=20, 几何=7, 物理场=0, 研究=0, 结果=0
- `pmm_steady_state_geom_sequence.m`: 参数=20, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.acdc.pmm_steady_state.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `pmm_steady_state_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pmm_steady_state_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.pmm_steady_state.pdf`: models.acdc.pmm_steady_state.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry of a distributed wound interior permanent magnet machine is imported and

## PDF Summary and Physics Judgement

- Simple summary: pmm_steady_state_geom_sequence 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: The geometry of a distributed wound interior permanent magnet machine is imported and
- PDF: peak torque production. Following this, overall results such as shaft power, loss per
- PDF: Figure 1: Complete geometry of motor design.
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 电磁
...
- PDF keywords: 传热, 电磁, models.acdc.pmm_steady_state.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry of a distributed wound interior permanent magnet machine is imported and
- `Np` -> Number of poles; MATLAB: `model.param.set('Np', value, description)`
- `Ns` -> Number of slots; MATLAB: `model.param.set('Ns', value, description)`
- `Nsec` -> Number of modeled symmetric/antiperiodic sectors; MATLAB: `model.param.set('Nsec', value, description)`
- `stator_core_rout` -> Stator core outer radius; MATLAB: `model.param.set('stator_core_rout', value, description)`
- `stator_core_th` -> Stator core radial thickness; MATLAB: `model.param.set('stator_core_th', value, description)`
- `stator_core_rin` -> Stator core inner radius; MATLAB: `model.param.set('stator_core_rin', value, description)`
- `N_coil_slot` -> Number of coils per slot; MATLAB: `model.param.set('N_coil_slot', value, description)`
- `airgap` -> Air gap thickness between stator and rotor; MATLAB: `model.param.set('airgap', value, description)`
- `slot_pitch_frc` -> Stator core slot pitch fraction (0-1); MATLAB: `model.param.set('slot_pitch_frc', value, description)`
- `slot_shoe_tip_height` -> Stator slot shoe tip height; MATLAB: `model.param.set('slot_shoe_tip_height', value, description)`
- `slot_shoe_angle` -> Slot shoe angle; MATLAB: `model.param.set('slot_shoe_angle', value, description)`
- `slot_opening` -> Slot opening width; MATLAB: `model.param.set('slot_opening', value, description)`
...
- 已识别几何证据：c1 / Circle；geom1 / c1 / r；geom1 / c1 / angle；c2 / Circle；dif1 / Difference；geom1 / c2 / r
- 已识别参数：Np、Ns、Nsec、stator_core_rout、stator_core_th、stator_core_rin、N_coil_slot、airgap、slot_pitch_frc、slot_shoe_tip_height、slot_shoe_angle、slot_opening
...
- Sweep `Np` around the learned value `10` and export target outputs to CSV.
- Sweep `Ns` around the learned value `60` and export target outputs to CSV.
- Sweep `Nsec` around the learned value `gcd(Np,Ns)` and export target outputs to CSV.
- Sweep `stator_core_rout` around the learned value `120[mm]` and export target outputs to CSV.
- Sweep `stator_core_th` around the learned value `stator_core_rout*0.3` and export target outputs to CSV.
- Sweep `stator_core_rin` around the learned value `stator_core_rout-stator_core_th` and export target outputs to CSV.
- Sweep `N_coil_slot` around the learned value `2` and export target outputs to CSV.
- Sweep `airgap` around the learned value `2[mm]` and export target outputs to CSV.
...
- Summary: 已学习 pmm_steady_state_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=24；内容证据=37 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 37 条；几何=7，物理场=0，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pmm_steady_state_geom_sequence.m, pmm_steady_state_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pmm_steady_state_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `airgap`
- `slot_pitch_frc`
- `slot_shoe_tip_height`
- `slot_shoe_angle`
- `slot_opening`
- `slot_width`
- `slot_height`
- `slot_shoe_base_height`
- `coil_height`
- `rotor_core_rout`
- `shaft_rout`
- `rotor_bridge`
...
- `pmm_steady_state_geom_sequence.mph` (comsol_mph, 2044729 bytes)
- `pmm_steady_state_geom_sequence.m` (matlab_livelink, 16269 bytes)
- `pmm_steady_state_geom_sequence.java` (comsol_java, 16806 bytes)
- `pmm_steady_state_geom_sequence_summary.json` (json, 1566 bytes)
- `models.acdc.pmm_steady_state.pdf` (pdf_document, 1464512 bytes)
