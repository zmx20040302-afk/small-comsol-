# electrostrictive_disc

- Case directory: `D:\桌面\codex\案例下载\电气\电致伸缩盘`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:24:41.622425+00:00`
...
- `H0` = `2[mm]` - Disc thickness
- `R0` = `5[mm]` - Disc radius
- `Q11` = `0.0133[m^4/C^2]` - Electrostrictive coupling coefficient
- `Q12` = `-0.00606[m^4/C^2]` - Electrostrictive coupling coefficient
- `E1` = `105[GPa]` - Young
- `nu1` = `0.4` - Poisson
- `rho1` = `7900[kg/m^3]` - Density
- `Ps` = `0.2589[C/m^2]` - Saturation polarization
- `a` = `0.86207[MV/m]` - Domain wall density
- `Vmax` = `2[MV/m]*H0` - Maximum applied voltage
- `F0` = `-80[MPa]` - F0
...
- boundary_conditions: symp1 / 2, bndl1 / 4, ccnfe1 / 1, r1 / size, r2 / size

### 文件内容证据

- `electrostrictive_disc.java`: 参数=12, 几何=4, 物理场=3, 研究=0, 结果=0
- `electrostrictive_disc.m`: 参数=12, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mems.electrostrictive_disc.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `electrostrictive_disc.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `electrostrictive_disc_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.electrostrictive_disc.pdf`: models.mems.electrostrictive_disc.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；are generated by these materials as a result of an applied electric field. There is very little

## PDF Summary and Physics Judgement

- Simple summary: electrostrictive_disc 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF: are generated by these materials as a result of an applied electric field. There is very little
- PDF: geometry is axisymmetric, and consequently 2D axisymmetry is used. The geometry is
- PDF: approach in the study settings.
...
- `H0` -> Disc thickness; MATLAB: `model.param.set('H0', value, description)`
- `R0` -> Disc radius; MATLAB: `model.param.set('R0', value, description)`
- `Q11` -> Electrostrictive coupling coefficient; MATLAB: `model.param.set('Q11', value, description)`
- `Q12` -> Electrostrictive coupling coefficient; MATLAB: `model.param.set('Q12', value, description)`
- `E1` -> Young; MATLAB: `model.param.set('E1', value, description)`
- `nu1` -> Poisson; MATLAB: `model.param.set('nu1', value, description)`
...
- `F0` -> F0; MATLAB: `model.param.set('F0', value, description)`
...
- 已识别参数：t、H0、R0、Q11、Q12、E1、nu1、rho1、Ps、a、Vmax、F0
...
- Sweep `H0` around the learned value `2[mm]` and export target outputs to CSV.
- Sweep `R0` around the learned value `5[mm]` and export target outputs to CSV.
- Sweep `Q11` around the learned value `0.0133[m^4/C^2]` and export target outputs to CSV.
- Sweep `Q12` around the learned value `-0.00606[m^4/C^2]` and export target outputs to CSV.
- Sweep `E1` around the learned value `105[GPa]` and export target outputs to CSV.
- Sweep `nu1` around the learned value `0.4` and export target outputs to CSV.
- Sweep `rho1` around the learned value `7900[kg/m^3]` and export target outputs to CSV.
...
- Summary: 已学习 electrostrictive_disc 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=29 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 29 条；几何=4，物理场=4，材料=1，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：electrostrictive_disc.m, electrostrictive_disc.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 electrostrictive_disc，训练阶段为 modeling_logic_learning_ready。
...
- `electrostrictive_disc.mph` (comsol_mph, 15646348 bytes)
- `electrostrictive_disc.m` (matlab_livelink, 33643 bytes)
- `electrostrictive_disc.java` (comsol_java, 33911 bytes)
- `electrostrictive_disc_summary.json` (json, 1307 bytes)
- `models.mems.electrostrictive_disc.pdf` (pdf_document, 1017950 bytes)
