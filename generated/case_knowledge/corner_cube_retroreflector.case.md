# corner_cube_retroreflector

- Case directory: `D:\桌面\codex\案例下载\电气\角隅反射镜`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:41:48.978399+00:00`
...
- mesh: mesh1
- boundary_conditions: mir1 / 5, 6, 7, wall1 / 2, pi1 / selkeepnoncontr

### 文件内容证据

- `corner_cube_retroreflector.java`: 参数=0, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.roptics.corner_cube_retroreflector.pdf`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `corner_cube_retroreflector.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `corner_cube_retroreflector.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `corner_cube_retroreflector_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.roptics.corner_cube_retroreflector.pdf`: models.roptics.corner_cube_retroreflector.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；cube retroreflector using the Geometrical Optics interface.

## PDF Summary and Physics Judgement

- Simple summary: corner_cube_retroreflector 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 GeometricalOptics。
- Primary physics: `GeometricalOptics`
- Study type: `unknown`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 GeometricalOptics，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型需要继续从研究步骤确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: cube retroreflector using the Geometrical Optics interface.
- PDF: geometry, shown in Figure 1, is imported as a built-in Part from the Part Library for the
- PDF: Figure 1: Geometry of a typical corner cube retroreflector.
...
- PDF keywords: models.roptics.corner_cube_retroreflector.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；cube retroreflector using the Geometrical Optics interface.
...
- Summary: 已学习 corner_cube_retroreflector 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=8 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 8 条；几何=2，物理场=2，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：corner_cube_retroreflector.m, corner_cube_retroreflector.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 corner_cube_retroreflector，训练阶段为 modeling_logic_learning_ready。
...
- `corner_cube_retroreflector.mph` (comsol_mph, 20093133 bytes)
- `corner_cube_retroreflector.m` (matlab_livelink, 12446 bytes)
- `corner_cube_retroreflector.java` (comsol_java, 9482 bytes)
- `corner_cube_retroreflector_summary.json` (json, 1052 bytes)
- `models.roptics.corner_cube_retroreflector.pdf` (pdf_document, 368241 bytes)
