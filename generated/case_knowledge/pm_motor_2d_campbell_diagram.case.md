...
- Case directory: `D:\桌面\codex\案例下载\电气\永磁电机坎贝尔图`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:06:54.329742+00:00`
...
- `n_shaft` = `12000[rpm]` - Shaft speed nominal
- `n_cog` = `Ns/gcd(Np,Ns)*2` - First cogging harmonic
- `Nframes` = `n_cog*6` - Number of time frames
- `Ipk` = `1[A]` - Phase current
- `init_ang` = `3.45[rad]` - Initial current angle for peak torque
- `f_el` = `n_shaft*Np/2` - Electrical frequency as function of shaft speed sweep
- `L` = `50[mm]` - Axial length
- `freq_min` = `900[Hz]` - Threshold frequency for sweep

## 内容级读取结果

- 模型树证据数量: 19
- 理论关键词: 电磁, 传热, 结构力学, 流体, 声学, 优化/参数扫描
- geometry: pi1 / PartInstance
- physics: Solid Mechanics, Magnetic Fields, Pressure Acoustics
- mesh: mesh1
- studies: Stationary, Eigenfrequency, Frequency Domain, Parametric Sweep

### 文件内容证据

- `pm_motor_2d_campbell_diagram.java`: 参数=10, 几何=1, 物理场=0, 研究=0, 结果=0
- `models.acdc.pm_motor_2d_campbell_diagram.pdf`: 参数=0, 几何=0, 物理场=3, 研究=4, 结果=0
- `models.aco.pm_motor_2d_campbell_diagram.pdf`: 参数=0, 几何=0, 物理场=3, 研究=4, 结果=0
- `pm_motor_2d_campbell_diagram.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pm_motor_2d_campbell_diagram.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pm_motor_2d_campbell_diagram_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.pm_motor_2d_campbell_diagram.pdf`: models.acdc.pm_motor_2d_campbell_diagram.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Magnetic Fields、Pressure Acoustics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the resulting structural vibration and surrounding acoustical emission of the motor. From
- `models.aco.pm_motor_2d_campbell_diagram.pdf`: models.aco.pm_motor_2d_campbell_diagram.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Magnetic Fields、Pressure Acoustics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the resulting structural vibration and surrounding acoustical emission of the motor. From

## PDF Summary and Physics Judgement

- Simple summary: pm_motor_2d_campbell_diagram 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Eigenfrequency 研究。
...
- PDF: the resulting structural vibration and surrounding acoustical emission of the motor. From
- PDF: results. As such a very simple 2D geometry is used for all physics to minimize the level of
- PDF: complexity not directly related to the multiphysics coupling.
- 脚本/文本识别到的物理场: Solid Mechanics, Magnetic Fields, Pressure Acoustics
...
- PDF keywords: 电磁, 传热, 结构力学, 流体, 声学, 优化/参数扫描, models.acdc.pm_motor_2d_campbell_diagram.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Magnetic Fields、Pressure Acoustics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the resulting structural vibration and surrounding acoustical emission of the motor. From, models.aco.pm_motor_2d_campbell_diagram.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Magnetic Fields、Pressure Acoustics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the resulting structural vibration and surrounding acoustical emission of the motor. From
...
- `n_shaft` -> Shaft speed nominal; MATLAB: `model.param.set('n_shaft', value, description)`
- `n_cog` -> First cogging harmonic; MATLAB: `model.param.set('n_cog', value, description)`
- `Nframes` -> Number of time frames; MATLAB: `model.param.set('Nframes', value, description)`
- `Ipk` -> Phase current; MATLAB: `model.param.set('Ipk', value, description)`
- `init_ang` -> Initial current angle for peak torque; MATLAB: `model.param.set('init_ang', value, description)`
- `f_el` -> Electrical frequency as function of shaft speed sweep; MATLAB: `model.param.set('f_el', value, description)`
- `L` -> Axial length; MATLAB: `model.param.set('L', value, description)`
- `freq_min` -> Threshold frequency for sweep; MATLAB: `model.param.set('freq_min', value, description)`
...
- 已识别物理场证据：Solid Mechanics；Magnetic Fields；Pressure Acoustics
- 已识别参数：Np、Ns、n_shaft、n_cog、Nframes、Ipk、init_ang、f_el、L、freq_min
...
- Transfer recognized physics interfaces to similar requirements: Solid Mechanics, Magnetic Fields, Pressure Acoustics.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 传热, 结构力学, 流体, 声学, 优化/参数扫描?
...
- Sweep `Ns` around the learned value `12` and export target outputs to CSV.
- Sweep `n_shaft` around the learned value `12000[rpm]` and export target outputs to CSV.
- Sweep `n_cog` around the learned value `Ns/gcd(Np,Ns)*2` and export target outputs to CSV.
- Sweep `Nframes` around the learned value `n_cog*6` and export target outputs to CSV.
- Sweep `Ipk` around the learned value `1[A]` and export target outputs to CSV.
- Sweep `init_ang` around the learned value `3.45[rad]` and export target outputs to CSV.
- Sweep `f_el` around the learned value `n_shaft*Np/2` and export target outputs to CSV.
...
- Summary: 已学习 pm_motor_2d_campbell_diagram 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=19 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 19 条；几何=1，物理场=3，材料=0，网格=1，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pm_motor_2d_campbell_diagram.m, pm_motor_2d_campbell_diagram.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pm_motor_2d_campbell_diagram，训练阶段为 modeling_logic_learning_ready。
...
- `pm_motor_2d_campbell_diagram.mph` (comsol_mph, 487755345 bytes)
- `pm_motor_2d_campbell_diagram.m` (matlab_livelink, 109323 bytes)
- `pm_motor_2d_campbell_diagram.java` (comsol_java, 115177 bytes)
- `pm_motor_2d_campbell_diagram_summary.json` (json, 5239 bytes)
- `models.acdc.pm_motor_2d_campbell_diagram.pdf` (pdf_document, 1039215 bytes)
- `models.aco.pm_motor_2d_campbell_diagram.pdf` (pdf_document, 1039194 bytes)
