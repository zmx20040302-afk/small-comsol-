...
- Case directory: `D:\桌面\codex\案例下载\COMSOL\积分-偏微分方程`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-07-06T09:43:32.972712+00:00`

## File Summary

- count: 5
- kinds: {'matlab_livelink': 1, 'comsol_mph': 2, 'pdf_document': 2}
- matlab_files: 1
- java_files: 0
- pdf_files: 2
- mph_files: 2
...
- materials: mat1
- mesh: mesh1
- studies: std1 / time / Transient
- boundary_conditions: temp1 / [1], hs1 / Q0, init1 / Tinit, temp1 / T0

### 文件内容证据

- `integro_partial.m`: 参数=7, 几何=1, 物理场=1, 研究=1, 结果=0
- `integro_partial.mph`: 参数=7, 几何=1, 物理场=1, 研究=1, 结果=0
- `integro_partial.zh_CN.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mph.integro_partial.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mph.integro_partial.zh_CN.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mph.integro_partial.pdf`: models.mph.integro_partial.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.integro_partial.zh_CN.pdf`: models.mph.integro_partial.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 积分-偏微分方程 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Transient 研究。
...
- 识别到的研究类型: std1 / time / Transient
- 理论关键词: 传热

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 传热, models.mph.integro_partial.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.integro_partial.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `epsilon` -> 案例参数/扫描变量; MATLAB: `model.param.set('epsilon', value, description)`
- `T_cold` -> 案例参数/扫描变量; MATLAB: `model.param.set('T_cold', value, description)`
- `DT_max` -> 案例参数/扫描变量; MATLAB: `model.param.set('DT_max', value, description)`
- `T_init` -> 案例参数/扫描变量; MATLAB: `model.param.set('T_init', value, description)`
- `D_i` -> 案例参数/扫描变量; MATLAB: `model.param.set('D_i', value, description)`
- `D_o` -> 案例参数/扫描变量; MATLAB: `model.param.set('D_o', value, description)`
...
- 已识别参数：epsilon、T_cold、DT_max、T_init、D_i、D_o、L
...
- Use the learned study types as solver starting points: std1 / time / Transient.
...
- Sweep `epsilon` around the learned value `1` and export target outputs to CSV.
- Sweep `T_cold` around the learned value `300[K]` and export target outputs to CSV.
- Sweep `DT_max` around the learned value `1200[K]` and export target outputs to CSV.
- Sweep `T_init` around the learned value `T_cold` and export target outputs to CSV.
- Sweep `D_i` around the learned value `1[in]` and export target outputs to CSV.
- Sweep `D_o` around the learned value `1.1*D_i` and export target outputs to CSV.
...
- Summary: 已学习 积分-偏微分方程 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=16 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 16 条；几何=1，物理场=1，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：integro_partial.m
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 积分-偏微分方程，训练阶段为 modeling_logic_learning_ready。
...
- `积分-偏微分方程` score=0.2665; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=23
- `管式反应器代理模型 App` score=0.1757; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `热执行器代理模型 App` score=0.1483; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42

### Official Documentation Checks

- `IntroductionToLiveLinkForMATLAB.pdf` page 5; module=LiveLink_for_MATLAB; score=0.1132
...
- `integro_partial.m` (matlab_livelink, 8940 bytes)
- `integro_partial.mph` (comsol_mph, 4021106 bytes)
- `integro_partial.zh_CN.mph` (comsol_mph, 4046231 bytes)
- `models.mph.integro_partial.pdf` (pdf_document, 473570 bytes)
- `models.mph.integro_partial.zh_CN.pdf` (pdf_document, 1291908 bytes)
