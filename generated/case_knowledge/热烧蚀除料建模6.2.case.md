...
- Created at: `2026-07-06T09:42:01.982747+00:00`
...
- `laser_machining_v62.mph`: 参数=0, 几何=2, 物理场=1, 研究=2, 结果=0
- `laser_machining_v62.java`: 参数=0, 几何=2, 物理场=1, 研究=2, 结果=0
- `laser_machining_v62.m`: 参数=0, 几何=2, 物理场=1, 研究=1, 结果=0
- `heat_shield_1D_v62.mph`: 参数=8, 几何=2, 物理场=2, 研究=0, 结果=0
...
- Simple summary: 热烧蚀除料建模6.2 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Transient 研究。
...
- 脚本/文本识别到的物理场: ht / HeatTransfer, dg / DeformedGeometry
...
- PDF keywords: 传热, 流体
- `H_v` -> Heat of Vaporization; MATLAB: `model.param.set('H_v', value, description)`
- `rho` -> Density; MATLAB: `model.param.set('rho', value, description)`
- `Cp` -> Specific Heat; MATLAB: `model.param.set('Cp', value, description)`
- `k` -> Conductivity; MATLAB: `model.param.set('k', value, description)`
- `A` -> Frfequency Factor; MATLAB: `model.param.set('A', value, description)`
- `E_a` -> Activation Energy; MATLAB: `model.param.set('E_a', value, description)`
- `n` -> order of reaction; MATLAB: `model.param.set('n', value, description)`
- `H_r` -> heat of reaction; MATLAB: `model.param.set('H_r', value, description)`
...
- 已识别几何证据：i1 / Interval；geom1 / i1 / p2；r1 / Rectangle；geom1 / r1 / size
- 已识别物理场证据：ht / HeatTransfer；dg / DeformedGeometry
- 已识别参数：H_v、rho、Cp、k、A、E_a、n、H_r
...
- Transfer recognized physics interfaces to similar requirements: ht / HeatTransfer, dg / DeformedGeometry.
...
- Sweep `H_v` around the learned value `500[kJ/kg]` and export target outputs to CSV.
- Sweep `rho` around the learned value `1200[kg/m^3]` and export target outputs to CSV.
- Sweep `Cp` around the learned value `1000[J/kg/K]` and export target outputs to CSV.
- Sweep `k` around the learned value `0.2[W/m/K]` and export target outputs to CSV.
- Sweep `A` around the learned value `20e3[1/s]` and export target outputs to CSV.
- Sweep `E_a` around the learned value `50[kJ/mol]` and export target outputs to CSV.
- Sweep `n` around the learned value `1.4` and export target outputs to CSV.
- Sweep `H_r` around the learned value `500[kJ/kg]` and export target outputs to CSV.
