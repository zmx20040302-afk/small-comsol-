...
- Case directory: `D:\桌面\codex\案例下载\电气\氮气中的双头流注`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:45:55.609103+00:00`
...
- 模型树证据数量: 11
- 理论关键词: 流体, 电磁, 传热, 结构力学
- geometry: r1 / Rectangle, mce1 / MeshControlEdges, geom1 / r1 / layername, geom1 / r1 / layerleft, geom1 / r1 / layerbottom
...
- `streamer_2d.java`: 参数=0, 几何=5, 物理场=0, 研究=0, 结果=0
- `models.plasma.streamer_2d.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `streamer_2d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `streamer_2d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `streamer_2d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.streamer_2d.pdf`: models.plasma.streamer_2d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This example presents a study of a double-headed streamer. An initial seed of electrons is

## PDF Summary and Physics Judgement

- Simple summary: streamer_2d 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: This example presents a study of a double-headed streamer. An initial seed of electrons is
- PDF: speed of the streamer agrees well with simulation results from Ref. 1.
- PDF: The chemistry of a plasma sustained in nitrogen can be very complex and a detailed study
...
- PDF keywords: 流体, 电磁, 传热, 结构力学, models.plasma.streamer_2d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This example presents a study of a double-headed streamer. An initial seed of electrons is
...
- 已识别几何证据：r1 / Rectangle；mce1 / MeshControlEdges；geom1 / r1 / layername；geom1 / r1 / layerleft；geom1 / r1 / layerbottom
...
- Summary: 已学习 streamer_2d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=11 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 11 条；几何=5，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：streamer_2d.m, streamer_2d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 streamer_2d，训练阶段为 modeling_logic_learning_ready。
...
- `streamer_2d.mph` (comsol_mph, 37403918 bytes)
- `streamer_2d.m` (matlab_livelink, 26204 bytes)
- `streamer_2d.java` (comsol_java, 22311 bytes)
- `streamer_2d_summary.json` (json, 822 bytes)
- `models.plasma.streamer_2d.pdf` (pdf_document, 814964 bytes)
