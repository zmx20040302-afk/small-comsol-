# proton_beam_scanning

- Case directory: `D:\桌面\codex\案例下载\电气\质子治疗用笔形束扫描喷嘴`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:37:16.140196+00:00`
...
- `aperture` = `5[cm]` - Quad aperture
- `h_quad` = `40[cm]` - Quad height
- `th_quad_pole` = `6[cm]` - Quad pole width
- `th_quad_coil` = `2[cm]` - Quad coil width
- `h_quad_coil` = `8[cm]` - Quad coil height
- `th_quad_coil_gap` = `5[mm]` - Quad coil-core gap
- `pos_quad_coil` = `12[cm]` - Quad coil position
- `th_quad_brace` = `9[cm]` - Quad core thickness
- `l_quad` = `15[cm]` - Quad length (z)
...
- `proton_beam_scanning.m`: 参数=9, 几何=0, 物理场=0, 研究=0, 结果=0
- `proton_beam_scanning.java`: 参数=9, 几何=0, 物理场=0, 研究=0, 结果=0
- `proton_beam_scanning.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `proton_beam_scanning_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: proton_beam_scanning 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF keywords: 电磁
- `aperture` -> Quad aperture; MATLAB: `model.param.set('aperture', value, description)`
- `h_quad` -> Quad height; MATLAB: `model.param.set('h_quad', value, description)`
- `th_quad_pole` -> Quad pole width; MATLAB: `model.param.set('th_quad_pole', value, description)`
- `th_quad_coil` -> Quad coil width; MATLAB: `model.param.set('th_quad_coil', value, description)`
- `h_quad_coil` -> Quad coil height; MATLAB: `model.param.set('h_quad_coil', value, description)`
- `th_quad_coil_gap` -> Quad coil-core gap; MATLAB: `model.param.set('th_quad_coil_gap', value, description)`
- `pos_quad_coil` -> Quad coil position; MATLAB: `model.param.set('pos_quad_coil', value, description)`
- `th_quad_brace` -> Quad core thickness; MATLAB: `model.param.set('th_quad_brace', value, description)`
- `l_quad` -> Quad length (z); MATLAB: `model.param.set('l_quad', value, description)`
...
- 已识别参数：aperture、h_quad、th_quad_pole、th_quad_coil、h_quad_coil、th_quad_coil_gap、pos_quad_coil、th_quad_brace、l_quad
...
- Sweep `aperture` around the learned value `5[cm]` and export target outputs to CSV.
- Sweep `h_quad` around the learned value `40[cm]` and export target outputs to CSV.
- Sweep `th_quad_pole` around the learned value `6[cm]` and export target outputs to CSV.
- Sweep `th_quad_coil` around the learned value `2[cm]` and export target outputs to CSV.
- Sweep `h_quad_coil` around the learned value `8[cm]` and export target outputs to CSV.
- Sweep `th_quad_coil_gap` around the learned value `5[mm]` and export target outputs to CSV.
- Sweep `pos_quad_coil` around the learned value `12[cm]` and export target outputs to CSV.
- Sweep `th_quad_brace` around the learned value `9[cm]` and export target outputs to CSV.
...
- Summary: 已学习 proton_beam_scanning 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=9 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 9 条；几何=0，物理场=0，材料=0，网格=0，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：proton_beam_scanning.m, proton_beam_scanning.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 proton_beam_scanning，训练阶段为 modeling_logic_learning_ready。
...
- `proton_beam_scanning.mph` (comsol_mph, 7389818 bytes)
- `proton_beam_scanning.m` (matlab_livelink, 127692 bytes)
- `proton_beam_scanning.java` (comsol_java, 135896 bytes)
- `proton_beam_scanning_summary.json` (json, 4500 bytes)
