...
- Case directory: `D:\桌面\codex\案例下载\电气\使用永磁体的轴向磁轴承`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:33:50.163665+00:00`
...
- `R1` = `10[mm]` - Inner radius of inner magnet
- `R2` = `20[mm]` - Outer radius of inner magnet
- `R3` = `22[mm]` - Inner radius of outer magnet
- `R4` = `32[mm]` - Outer radius of outer magnet
- `h0` = `10[mm]` - Magnet height
- `Br` = `1[T]` - Remanent flux density of magnet
- `dZ` = `0[mm]` - Axial displacement

## 内容级读取结果

- 模型树证据数量: 40
- 理论关键词: 电磁, 结构力学, 多孔介质/裂隙, 优化/参数扫描
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r1 / layername, geom1 / r1 / layertop, geom1 / r2 / size, geom1 / r2 / pos, geom1 / r2 / layername, geom1 / r2 / layertop, geom1 / r3 / size
- physics: Magnetic Fields, Optimization
- mesh: mesh1
- studies: Stationary, Parametric Sweep, Optimization
- boundary_conditions: r1 / size, r1 / pos, r1 / layername, r1 / layertop, r2 / size, r2 / pos, r2 / layername, r2 / layertop, r3 / size, r3 / pos, r3 / layername, r3 / layerright

### 文件内容证据

- `axial_magnetic_bearing.java`: 参数=7, 几何=15, 物理场=0, 研究=0, 结果=0
- `models.acdc.axial_magnetic_bearing.pdf`: 参数=0, 几何=0, 物理场=2, 研究=3, 结果=0
- `axial_magnetic_bearing.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `axial_magnetic_bearing.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `axial_magnetic_bearing_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.axial_magnetic_bearing.pdf`: models.acdc.axial_magnetic_bearing.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Optimization，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Multiphysics calculates the total magnetic force on an object by integrating the vector

## PDF Summary and Physics Judgement

- Simple summary: axial_magnetic_bearing 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF: Multiphysics calculates the total magnetic force on an object by integrating the vector
- PDF: direction as well as the coupled stiffness coefficients requires a complete 3D model.
- PDF: Results
- 脚本/文本识别到的物理场: Magnetic Fields, Optimization
- 识别到的研究类型: Stationary, Parametric Sweep, Optimization
- 理论关键词: 电磁, 结构力学, 多孔介质/裂隙, 优化/参数扫描

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 电磁, 结构力学, 多孔介质/裂隙, 优化/参数扫描, models.acdc.axial_magnetic_bearing.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Optimization，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Multiphysics calculates the total magnetic force on an object by integrating the vector
- `R1` -> Inner radius of inner magnet; MATLAB: `model.param.set('R1', value, description)`
- `R2` -> Outer radius of inner magnet; MATLAB: `model.param.set('R2', value, description)`
- `R3` -> Inner radius of outer magnet; MATLAB: `model.param.set('R3', value, description)`
- `R4` -> Outer radius of outer magnet; MATLAB: `model.param.set('R4', value, description)`
- `h0` -> Magnet height; MATLAB: `model.param.set('h0', value, description)`
- `Br` -> Remanent flux density of magnet; MATLAB: `model.param.set('Br', value, description)`
- `dZ` -> Axial displacement; MATLAB: `model.param.set('dZ', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；geom1 / r1 / size；geom1 / r1 / pos；geom1 / r1 / layername
- 已识别物理场证据：Magnetic Fields；Optimization
- 已识别参数：R1、R2、R3、R4、h0、Br、dZ
...
- Use the learned study types as solver starting points: Stationary, Parametric Sweep, Optimization.
...
- Sweep `R1` around the learned value `10[mm]` and export target outputs to CSV.
- Sweep `R2` around the learned value `20[mm]` and export target outputs to CSV.
- Sweep `R3` around the learned value `22[mm]` and export target outputs to CSV.
- Sweep `R4` around the learned value `32[mm]` and export target outputs to CSV.
- Sweep `h0` around the learned value `10[mm]` and export target outputs to CSV.
- Sweep `Br` around the learned value `1[T]` and export target outputs to CSV.
- Sweep `dZ` around the learned value `0[mm]` and export target outputs to CSV.
...
- Summary: 已学习 axial_magnetic_bearing 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=40 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 40 条；几何=15，物理场=2，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：axial_magnetic_bearing.m, axial_magnetic_bearing.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 axial_magnetic_bearing，训练阶段为 modeling_logic_learning_ready。
...
- `axial_magnetic_bearing.mph` (comsol_mph, 30935183 bytes)
- `axial_magnetic_bearing.m` (matlab_livelink, 24363 bytes)
- `axial_magnetic_bearing.java` (comsol_java, 20368 bytes)
- `axial_magnetic_bearing_summary.json` (json, 1073 bytes)
- `models.acdc.axial_magnetic_bearing.pdf` (pdf_document, 1098292 bytes)
