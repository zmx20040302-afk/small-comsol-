...
- Case directory: `D:\桌面\codex\案例下载\电气\偏振光束分束器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:23:01.973556+00:00`
...
- `wl0` = `550[nm]` - Center wavelength
- `dWl` = `500[nm]` - Bandwidth
- `w0` = `20*wl0` - Spot radius
- `nPrism` = `1.673` - Refractive index of the glass prisms
- `nHigh` = `2.3` - Refractive index of the high-index materials
- `nLow` = `1.38` - Refractive index of the low-index material
- `z0` = `pi*nPrism*w0^2/wl0` - Rayleigh range
- `a` = `8*w0/sqrt(1-(8*w0/z0)^2)` - Prism side
- `nLayers` = `7` - Number of dielectric layers
- `dHigh` = `wl0/4/nHigh/cos(alphaHigh)` - Thickness of high-index material layers
- `dLow` = `wl0/4/nLow/cos(alphaLow)` - Thickness of low-index material layers
- `E0` = `1[V/m]` - Peak incident electric field
- `lda0` = `wl0` - Wavelength in vacuum
- `k0` = `2*pi/lda0` - Wave number in vacuum
- `kParallel` = `k0*nPrism/sqrt(2)` - Wave vector component parallel to the boundaries
- `dStack` = `floor(nLayers/2)*(dHigh+dLow)+dHigh` - Thickness of thin-film stack
- `wlCount` = `11` - Number of wavelengths in the sweep
- `alphaHigh` = `asin(nPrism*sin(pi/4)/nHigh)` - Angle in high-index material
- `alphaLow` = `asin(nPrism*sin(pi/4)/nLow)` - Angle in low-index material
- `xCOGu` = `-(a/2+dStack/sqrt(2))/3` - Center of gravity, upper triangle, x-coordinate
- `yCOGu` = `-xCOGu` - Center of gravity, upper triangle, y-coordinate
- `xCOGl` = `-xCOGu` - Center of gravity, lower triangle, x-coordinate
- `yCOGl` = `xCOGu` - Center of gravity, lower triangle, y-coordinate

## 内容级读取结果

- 模型树证据数量: 25
- 理论关键词: 电磁
- physics: Global ODEs and DAEs, Electromagnetic Waves

### 文件内容证据

- `polarizing_beam_splitter.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `applications.polarizing_beam_splitter.pdf`: 参数=0, 几何=0, 物理场=2, 研究=0, 结果=0
- `polarizing_beam_splitter.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `polarizing_beam_splitter.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `polarizing_beam_splitter_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `applications.polarizing_beam_splitter.pdf`: applications.polarizing_beam_splitter.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Global ODEs and DAEs、Electromagnetic Waves；核心内容可概括为：Application created in COMSOL Multiphysics 6.4；simulation parameters are set to the default parameters. Click the Geometry button in

## PDF Summary and Physics Judgement

- Simple summary: polarizing_beam_splitter 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Global ODEs and DAEs。
- Primary physics: `Global ODEs and DAEs`
- Study type: `unknown`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Global ODEs and DAEs，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型需要继续从研究步骤确认。

### Judgement Evidence

- PDF: Application created in COMSOL Multiphysics 6.4
- PDF: simulation parameters are set to the default parameters. Click the Geometry button in
- PDF: the Design section to display the current geometry. If a Spot radius sweep has been
- PDF: performed, it is possible to select the spot radius for which the geometry should be
- 脚本/文本识别到的物理场: Global ODEs and DAEs, Electromagnetic Waves
- 理论关键词: 电磁

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 电磁, applications.polarizing_beam_splitter.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Global ODEs and DAEs、Electromagnetic Waves；核心内容可概括为：Application created in COMSOL Multiphysics 6.4；simulation parameters are set to the default parameters. Click the Geometry button in
- `wl0` -> Center wavelength; MATLAB: `model.param.set('wl0', value, description)`
- `dWl` -> Bandwidth; MATLAB: `model.param.set('dWl', value, description)`
- `w0` -> Spot radius; MATLAB: `model.param.set('w0', value, description)`
- `nPrism` -> Refractive index of the glass prisms; MATLAB: `model.param.set('nPrism', value, description)`
- `nHigh` -> Refractive index of the high-index materials; MATLAB: `model.param.set('nHigh', value, description)`
- `nLow` -> Refractive index of the low-index material; MATLAB: `model.param.set('nLow', value, description)`
- `z0` -> Rayleigh range; MATLAB: `model.param.set('z0', value, description)`
- `a` -> Prism side; MATLAB: `model.param.set('a', value, description)`
- `nLayers` -> Number of dielectric layers; MATLAB: `model.param.set('nLayers', value, description)`
- `dHigh` -> Thickness of high-index material layers; MATLAB: `model.param.set('dHigh', value, description)`
- `dLow` -> Thickness of low-index material layers; MATLAB: `model.param.set('dLow', value, description)`
- `E0` -> Peak incident electric field; MATLAB: `model.param.set('E0', value, description)`
...
- 已识别物理场证据：Global ODEs and DAEs；Electromagnetic Waves
- 已识别参数：wl0、dWl、w0、nPrism、nHigh、nLow、z0、a、nLayers、dHigh、dLow、E0
...
- Transfer recognized physics interfaces to similar requirements: Global ODEs and DAEs, Electromagnetic Waves.
...
- Sweep `wl0` around the learned value `550[nm]` and export target outputs to CSV.
- Sweep `dWl` around the learned value `500[nm]` and export target outputs to CSV.
- Sweep `w0` around the learned value `20*wl0` and export target outputs to CSV.
- Sweep `nPrism` around the learned value `1.673` and export target outputs to CSV.
- Sweep `nHigh` around the learned value `2.3` and export target outputs to CSV.
- Sweep `nLow` around the learned value `1.38` and export target outputs to CSV.
- Sweep `z0` around the learned value `pi*nPrism*w0^2/wl0` and export target outputs to CSV.
- Sweep `a` around the learned value `8*w0/sqrt(1-(8*w0/z0)^2)` and export target outputs to CSV.
...
- Summary: 已学习 polarizing_beam_splitter 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=23；内容证据=25 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 25 条；几何=0，物理场=2，材料=0，网格=0，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：polarizing_beam_splitter.m, polarizing_beam_splitter.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 polarizing_beam_splitter，训练阶段为 modeling_logic_learning_ready。
...
- `polarizing_beam_splitter.mph` (comsol_mph, 92210050 bytes)
- `polarizing_beam_splitter.m` (matlab_livelink, 67678 bytes)
- `polarizing_beam_splitter.java` (comsol_java, 65564 bytes)
- `polarizing_beam_splitter_summary.json` (json, 2463 bytes)
- `applications.polarizing_beam_splitter.pdf` (pdf_document, 376424 bytes)
