# reverse_bias_leakage_current

- Case directory: `D:\桌面\codex\案例下载\电气\反向偏置漏电流`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:22:04.274073+00:00`
...
- `reverse_bias_leakage_current.java`: 参数=2, 几何=3, 物理场=0, 研究=0, 结果=0
- `reverse_bias_leakage_current_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `models.semicond.reverse_bias_leakage_current.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `reverse_bias_leakage_current.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `reverse_bias_leakage_current.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.reverse_bias_leakage_current.pdf`: models.semicond.reverse_bias_leakage_current.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion

## PDF Summary and Physics Judgement

- Simple summary: reverse_bias_leakage_current 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electric Currents，并采用 Stationary 研究。
...
- PDF keywords: 电磁, models.semicond.reverse_bias_leakage_current.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion
- `Ndop` -> 案例参数/扫描变量; MATLAB: `model.param.set('Ndop', value, description)`
...
- Sweep `Ndop` around the learned value `1e19[1/cm^3]` and export target outputs to CSV.
...
- Summary: 已学习 reverse_bias_leakage_current 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=11 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 11 条；几何=3，物理场=1，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：reverse_bias_leakage_current.m, reverse_bias_leakage_current.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 reverse_bias_leakage_current，训练阶段为 modeling_logic_learning_ready。
...
- `reverse_bias_leakage_current.mph` (comsol_mph, 5539860 bytes)
- `reverse_bias_leakage_current.m` (matlab_livelink, 40887 bytes)
- `reverse_bias_leakage_current.java` (comsol_java, 37693 bytes)
- `reverse_bias_leakage_current_summary.json` (json, 1248 bytes)
- `models.semicond.reverse_bias_leakage_current.pdf` (pdf_document, 243874 bytes)
