...
- Case directory: `D:\桌面\codex\案例下载\电气\异质结 - 一维`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:23:49.169297+00:00`
...
- `sign` = `1` - Sign of applied voltage
- `Va` = `0[V]` - Voltage
...
- mesh: mesh1
- studies: Stationary
- boundary_conditions: i1 / coord, i2 / coord

### 文件内容证据

- `heterojunction_1d.java`: 参数=5, 几何=4, 物理场=0, 研究=0, 结果=0
- `models.semicond.heterojunction_1d.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `heterojunction_1d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `heterojunction_1d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `heterojunction_1d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.heterojunction_1d.pdf`: models.semicond.heterojunction_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；curves for each configuration are compared with results from the literature. Several

## PDF Summary and Physics Judgement

- Simple summary: heterojunction_1d 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: curves for each configuration are compared with results from the literature. Several
- PDF: methods for better convergence are demonstrated in the setup of the various study steps.
- PDF: emission model.
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 传热
...
- PDF keywords: 电磁, 传热, models.semicond.heterojunction_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；curves for each configuration are compared with results from the literature. Several
- `sign` -> Sign of applied voltage; MATLAB: `model.param.set('sign', value, description)`
- `Va` -> Voltage; MATLAB: `model.param.set('Va', value, description)`
- `ramp` -> Continuation parameter; MATLAB: `model.param.set('ramp', value, description)`
...
- Sweep `sign` around the learned value `1` and export target outputs to CSV.
...
- Summary: 已学习 heterojunction_1d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=13 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 13 条；几何=4，物理场=0，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：heterojunction_1d.m, heterojunction_1d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 heterojunction_1d，训练阶段为 modeling_logic_learning_ready。
...
- `heterojunction_1d.mph` (comsol_mph, 15950036 bytes)
- `heterojunction_1d.m` (matlab_livelink, 52821 bytes)
- `heterojunction_1d.java` (comsol_java, 47348 bytes)
- `heterojunction_1d_summary.json` (json, 1230 bytes)
- `models.semicond.heterojunction_1d.pdf` (pdf_document, 390764 bytes)
