...
- Case directory: `D:\桌面\codex\案例下载\电气\MOSFET 迁移率模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:01:22.302485+00:00`
...
- `mosfet_mobility.java`: 参数=2, 几何=6, 物理场=0, 研究=0, 结果=0
- `models.semicond.mosfet_mobility.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `mosfet_mobility.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mosfet_mobility.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mosfet_mobility_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.mosfet_mobility.pdf`: models.semicond.mosfet_mobility.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；models are used, the Semiconductor physics interface automatically creates a suggested

## PDF Summary and Physics Judgement

- Simple summary: mosfet_mobility 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: models are used, the Semiconductor physics interface automatically creates a suggested
- PDF: semiconductor material. However, in many devices, the physics demands that you model
- PDF: The geometry and operation of the device are discussed for the DC Characteristics of a
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 传热, 流体, 声学
...
- PDF keywords: 电磁, 传热, 流体, 声学, models.semicond.mosfet_mobility.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；models are used, the Semiconductor physics interface automatically creates a suggested
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 传热, 流体, 声学?
...
- Summary: 已学习 mosfet_mobility 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=14 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 14 条；几何=6，物理场=0，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mosfet_mobility.m, mosfet_mobility.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mosfet_mobility，训练阶段为 modeling_logic_learning_ready。
...
- `mosfet_mobility.mph` (comsol_mph, 21048054 bytes)
- `mosfet_mobility.m` (matlab_livelink, 57930 bytes)
- `mosfet_mobility.java` (comsol_java, 53655 bytes)
- `mosfet_mobility_summary.json` (json, 1313 bytes)
- `models.semicond.mosfet_mobility.pdf` (pdf_document, 181841 bytes)
