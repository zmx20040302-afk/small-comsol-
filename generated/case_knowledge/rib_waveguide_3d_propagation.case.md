...
- `w_core` = `460[nm]` - Width of core
- `h_core` = `200[nm]` - Height of core
- `w_clad` = `1000[nm]` - Width of cladding
- `h_clad` = `1000[nm]` - Height of cladding
- `n_core` = `3.48` - Refractive index of core
- `n_clad_over` = `1.51` - Refractive index of cladding (upper)
- `n_clad_under` = `1.44` - Refractive index of cladding (lower)
...
- 模型树证据数量: 17
- 理论关键词: 未自动识别
- geometry: wp1 / WorkPlane, r1 / Rectangle, r2 / Rectangle, geom1 / wp1 / quickplane, geom1 / wp1 / unite
- mesh: mesh1
- boundary_conditions: wp1 / quickplane, wp1 / unite

### 文件内容证据

- `rib_waveguide_3d_propagation.java`: 参数=9, 几何=5, 物理场=0, 研究=0, 结果=0
- `rib_waveguide_3d_propagation.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rib_waveguide_3d_propagation.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rib_waveguide_3d_propagation_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: rib_waveguide_3d_propagation 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `n_clad_over` -> Refractive index of cladding (upper); MATLAB: `model.param.set('n_clad_over', value, description)`
- `n_clad_under` -> Refractive index of cladding (lower); MATLAB: `model.param.set('n_clad_under', value, description)`
...
- 已识别几何证据：wp1 / WorkPlane；r1 / Rectangle；r2 / Rectangle；geom1 / wp1 / quickplane；geom1 / wp1 / unite
- 已识别参数：w_core、h_core、w_clad、h_clad、n_core、n_clad_over、n_clad_under、lda0、f0
...
- Sweep `w_core` around the learned value `460[nm]` and export target outputs to CSV.
- Sweep `h_core` around the learned value `200[nm]` and export target outputs to CSV.
- Sweep `w_clad` around the learned value `1000[nm]` and export target outputs to CSV.
- Sweep `h_clad` around the learned value `1000[nm]` and export target outputs to CSV.
- Sweep `n_core` around the learned value `3.48` and export target outputs to CSV.
- Sweep `n_clad_over` around the learned value `1.51` and export target outputs to CSV.
- Sweep `n_clad_under` around the learned value `1.44` and export target outputs to CSV.
...
- Summary: 已学习 rib_waveguide_3d_propagation 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=17 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：rib_waveguide_3d_propagation.m, rib_waveguide_3d_propagation.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 rib_waveguide_3d_propagation，训练阶段为 modeling_logic_learning_ready。
...
- `rib_waveguide_3d_propagation.mph` (comsol_mph, 29830889 bytes)
- `rib_waveguide_3d_propagation.m` (matlab_livelink, 24119 bytes)
- `rib_waveguide_3d_propagation.java` (comsol_java, 19561 bytes)
- `rib_waveguide_3d_propagation_summary.json` (json, 1190 bytes)
