# double_headed_streamer_discharge_chemistry

- Case directory: `D:\桌面\codex\案例下载\电气\通过自定义放电化学模拟放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:32:04.560342+00:00`
...
- `mu` = `2.9e5/(P/1[Torr])*1[cm^2/V/s]` - Electron mobility
- `cM` = `P/R_const/300[K]` - Gas molar concentration
- `V0` = `52[kV]` - Applied voltage

## 内容级读取结果

- 模型树证据数量: 18
- 理论关键词: 流体, 电磁, 传热, 结构力学
- geometry: r1 / Rectangle, geom1 / r1 / layername, geom1 / r1 / layerleft, geom1 / r1 / layerbottom
- physics: re / ReactionEng, chem / Chemistry, tcc / TransportOfChargeCarriers, Electrostatics
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: r1 / layername, r1 / layerleft, r1 / layerbottom

### 文件内容证据

- `double_headed_streamer_discharge_chemistry.java`: 参数=4, 几何=4, 物理场=3, 研究=0, 结果=0
- `models.edis.double_headed_streamer_discharge_chemistry.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `double_headed_streamer_discharge_chemistry.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `double_headed_streamer_discharge_chemistry.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `double_headed_streamer_discharge_chemistry_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.double_headed_streamer_discharge_chemistry.pdf`: models.edis.double_headed_streamer_discharge_chemistry.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；results in Ref. 1.

## PDF Summary and Physics Judgement

- Simple summary: double_headed_streamer_discharge_chemistry 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: results in Ref. 1.
- PDF: Results and Discussion
- PDF: The results in this section are for a double-headed streamer propagating in a background
- 脚本/文本识别到的物理场: re / ReactionEng, chem / Chemistry, tcc / TransportOfChargeCarriers, Electrostatics
...
- PDF keywords: 流体, 电磁, 传热, 结构力学, models.edis.double_headed_streamer_discharge_chemistry.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；results in Ref. 1.
- `P` -> Gas pressure; MATLAB: `model.param.set('P', value, description)`
- `mu` -> Electron mobility; MATLAB: `model.param.set('mu', value, description)`
- `cM` -> Gas molar concentration; MATLAB: `model.param.set('cM', value, description)`
...
- 已识别几何证据：r1 / Rectangle；geom1 / r1 / layername；geom1 / r1 / layerleft；geom1 / r1 / layerbottom
- 已识别物理场证据：re / ReactionEng；chem / Chemistry；tcc / TransportOfChargeCarriers；Electrostatics
- 已识别参数：P、mu、cM、V0
...
- 识别到物理场证据：re / ReactionEng
- 识别到物理场证据：chem / Chemistry
...
- Transfer recognized physics interfaces to similar requirements: re / ReactionEng, chem / Chemistry, tcc / TransportOfChargeCarriers, Electrostatics.
...
- Sweep `mu` around the learned value `2.9e5/(P/1[Torr])*1[cm^2/V/s]` and export target outputs to CSV.
- Sweep `cM` around the learned value `P/R_const/300[K]` and export target outputs to CSV.
- Sweep `V0` around the learned value `52[kV]` and export target outputs to CSV.
...
- Summary: 已学习 double_headed_streamer_discharge_chemistry 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=18 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 18 条；几何=4，物理场=4，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：double_headed_streamer_discharge_chemistry.m, double_headed_streamer_discharge_chemistry.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 double_headed_streamer_discharge_chemistry，训练阶段为 modeling_logic_learning_ready。
...
- `double_headed_streamer_discharge_chemistry.mph` (comsol_mph, 14638390 bytes)
- `double_headed_streamer_discharge_chemistry.m` (matlab_livelink, 36306 bytes)
- `double_headed_streamer_discharge_chemistry.java` (comsol_java, 34831 bytes)
- `double_headed_streamer_discharge_chemistry_summary.json` (json, 1185 bytes)
- `models.edis.double_headed_streamer_discharge_chemistry.pdf` (pdf_document, 579154 bytes)
