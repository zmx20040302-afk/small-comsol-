...
- Case directory: `D:\桌面\codex\案例下载\电气\热释电探测器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:19:21.448449+00:00`
...
- `r_el` = `1.4[mm]` - Radius of top electrode
- `r_s` = `0.25[mm]` - Width of standoff
- `r_d` = `1.5[mm]` - Radius of crystal
- `t_d` = `0.025[mm]` - Thickness of crystal
- `t_s` = `0.040[mm]` - Thickness of standoff
- `w_b` = `2[mm]` - Width of laser beam
- `Qmax` = `500[W/m^2]` - Maximum laser power density
- `pulse` = `1[s]` - Duration of laser pulse
- `C_ext` = `100[pF]` - Capacitance of C1
- `R_ext` = `1000[ohm]` - Resistance of R1

## 内容级读取结果

- 模型树证据数量: 23
- 理论关键词: 电磁, 传热, 结构力学, 流体, 优化/参数扫描
- geometry: r1 / Rectangle, r2 / Rectangle, geom1 / r1 / size, geom1 / r2 / size
- physics: Heat Transfer, Solid Mechanics, Electrostatics, Optimization
- mesh: mesh1
- studies: Time Dependent, Optimization
- boundary_conditions: r1 / size, r2 / size

### 文件内容证据

- `pyroelectric_detector.java`: 参数=10, 几何=4, 物理场=0, 研究=0, 结果=0
- `models.mems.pyroelectric_detector.pdf`: 参数=0, 几何=0, 物理场=4, 研究=2, 结果=0
- `pyroelectric_detector.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pyroelectric_detector.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pyroelectric_detector_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.pyroelectric_detector.pdf`: models.mems.pyroelectric_detector.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Solid Mechanics、Electrostatics，研究类型线索为 Time Dependent、Optimization；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Piezoelectricity and Pyroelectricity multiphysics interface and (ii) the Electrical Circuit

## PDF Summary and Physics Judgement

- Simple summary: pyroelectric_detector 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: Piezoelectricity and Pyroelectricity multiphysics interface and (ii) the Electrical Circuit
- PDF: interface. A time-dependent study solves for temperature evolution in the disk and the
- PDF: model geometry is fully parameterized to allow for easy changes in the device structure for
- 脚本/文本识别到的物理场: Heat Transfer, Solid Mechanics, Electrostatics, Optimization
- 识别到的研究类型: Time Dependent, Optimization
- 理论关键词: 电磁, 传热, 结构力学, 流体, 优化/参数扫描

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 电磁, 传热, 结构力学, 流体, 优化/参数扫描, models.mems.pyroelectric_detector.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Solid Mechanics、Electrostatics，研究类型线索为 Time Dependent、Optimization；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Piezoelectricity and Pyroelectricity multiphysics interface and (ii) the Electrical Circuit
- `r_el` -> Radius of top electrode; MATLAB: `model.param.set('r_el', value, description)`
- `r_s` -> Width of standoff; MATLAB: `model.param.set('r_s', value, description)`
- `r_d` -> Radius of crystal; MATLAB: `model.param.set('r_d', value, description)`
- `t_d` -> Thickness of crystal; MATLAB: `model.param.set('t_d', value, description)`
- `t_s` -> Thickness of standoff; MATLAB: `model.param.set('t_s', value, description)`
- `w_b` -> Width of laser beam; MATLAB: `model.param.set('w_b', value, description)`
- `Qmax` -> Maximum laser power density; MATLAB: `model.param.set('Qmax', value, description)`
- `pulse` -> Duration of laser pulse; MATLAB: `model.param.set('pulse', value, description)`
- `C_ext` -> Capacitance of C1; MATLAB: `model.param.set('C_ext', value, description)`
- `R_ext` -> Resistance of R1; MATLAB: `model.param.set('R_ext', value, description)`
...
- 已识别物理场证据：Heat Transfer；Solid Mechanics；Electrostatics；Optimization
- 已识别参数：r_el、r_s、r_d、t_d、t_s、w_b、Qmax、pulse、C_ext、R_ext
...
- Transfer recognized physics interfaces to similar requirements: Heat Transfer, Solid Mechanics, Electrostatics, Optimization.
- Reuse geometry construction patterns when new requirements share shape, import, array, sweep, or boolean operations.
- Use the learned study types as solver starting points: Time Dependent, Optimization.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 传热, 结构力学, 流体, 优化/参数扫描?
...
- Sweep `r_el` around the learned value `1.4[mm]` and export target outputs to CSV.
- Sweep `r_s` around the learned value `0.25[mm]` and export target outputs to CSV.
- Sweep `r_d` around the learned value `1.5[mm]` and export target outputs to CSV.
- Sweep `t_d` around the learned value `0.025[mm]` and export target outputs to CSV.
- Sweep `t_s` around the learned value `0.040[mm]` and export target outputs to CSV.
- Sweep `w_b` around the learned value `2[mm]` and export target outputs to CSV.
- Sweep `Qmax` around the learned value `500[W/m^2]` and export target outputs to CSV.
- Sweep `pulse` around the learned value `1[s]` and export target outputs to CSV.
...
- Summary: 已学习 pyroelectric_detector 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=4，物理场=4，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pyroelectric_detector.m, pyroelectric_detector.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pyroelectric_detector，训练阶段为 modeling_logic_learning_ready。
...
- `pyroelectric_detector.mph` (comsol_mph, 72091432 bytes)
- `pyroelectric_detector.m` (matlab_livelink, 47961 bytes)
- `pyroelectric_detector.java` (comsol_java, 42499 bytes)
- `pyroelectric_detector_summary.json` (json, 2473 bytes)
- `models.mems.pyroelectric_detector.pdf` (pdf_document, 282315 bytes)
