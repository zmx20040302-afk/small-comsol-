...
- Case directory: `D:\桌面\codex\案例下载\电气\全局模型与两项玻尔兹曼方程耦合`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:12:47.466009+00:00`
...
- `boltzmann_global_model_argon.java`: 参数=6, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.plasma.boltzmann_global_model_argon.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `boltzmann_global_model_argon_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `boltzmann_global_model_argon.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `boltzmann_global_model_argon.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.boltzmann_global_model_argon.pdf`: models.plasma.boltzmann_global_model_argon.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；analytic model. Without the spatial derivatives the numerical solution of the equation set

## PDF Summary and Physics Judgement

- Simple summary: boltzmann_global_model_argon 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: analytic model. Without the spatial derivatives the numerical solution of the equation set
- PDF: make a global model a good first approach to study a plasma reactor, especially when
...
- PDF keywords: PDE/方程建模, 传热, 结构力学, 流体, 电磁, models.plasma.boltzmann_global_model_argon.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；analytic model. Without the spatial derivatives the numerical solution of the equation set
- `rad` -> 案例参数/扫描变量; MATLAB: `model.param.set('rad', value, description)`
- `gap` -> 案例参数/扫描变量; MATLAB: `model.param.set('gap', value, description)`
- `Res` -> 案例参数/扫描变量; MATLAB: `model.param.set('Res', value, description)`
- `Tg` -> 案例参数/扫描变量; MATLAB: `model.param.set('Tg', value, description)`
- `P0` -> 案例参数/扫描变量; MATLAB: `model.param.set('P0', value, description)`
- `Vdc` -> 案例参数/扫描变量; MATLAB: `model.param.set('Vdc', value, description)`
...
- How do the theory keywords change governing equations, assumptions, or validation targets: PDE/方程建模, 传热, 结构力学, 流体, 电磁?
...
- Sweep `rad` around the learned value `0.4[cm]` and export target outputs to CSV.
- Sweep `gap` around the learned value `0.4[cm]` and export target outputs to CSV.
- Sweep `Res` around the learned value `1e5[ohm]` and export target outputs to CSV.
- Sweep `Tg` around the learned value `300[K]` and export target outputs to CSV.
- Sweep `P0` around the learned value `100[torr]` and export target outputs to CSV.
- Sweep `Vdc` around the learned value `1000[V]` and export target outputs to CSV.
...
- Summary: 已学习 boltzmann_global_model_argon 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=12 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 12 条；几何=2，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：boltzmann_global_model_argon.m, boltzmann_global_model_argon.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 boltzmann_global_model_argon，训练阶段为 modeling_logic_learning_ready。
...
- `boltzmann_global_model_argon.mph` (comsol_mph, 3827913 bytes)
- `boltzmann_global_model_argon.m` (matlab_livelink, 58621 bytes)
- `boltzmann_global_model_argon.java` (comsol_java, 56518 bytes)
- `boltzmann_global_model_argon_summary.json` (json, 1596 bytes)
- `models.plasma.boltzmann_global_model_argon.pdf` (pdf_document, 326983 bytes)
