...
- Created at: `2026-08-20T10:22:29.741514+00:00`
...
- 模型树证据数量: 40
- 理论关键词: 未自动识别
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, r4 / Rectangle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r2 / pos, geom1 / r3 / size, geom1 / r3 / pos, geom1 / r4 / size, geom1 / r4 / pos
- mesh: mesh1
- boundary_conditions: r1 / size, r1 / pos, r2 / size, r2 / pos, r3 / size, r3 / pos, r4 / size, r4 / pos, r4 / layername, r4 / layerright, r4 / layertop, r5 / size

### 文件内容证据

- `biased_inductor.java`: 参数=8, 几何=18, 物理场=0, 研究=0, 结果=0
- `biased_inductor.m`: 参数=8, 几何=14, 物理场=0, 研究=0, 结果=0
- `biased_inductor.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `biased_inductor_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: biased_inductor 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `I_DC` -> 案例参数/扫描变量; MATLAB: `model.param.set('I_DC', value, description)`
- `f0` -> 案例参数/扫描变量; MATLAB: `model.param.set('f0', value, description)`
- `T0` -> 案例参数/扫描变量; MATLAB: `model.param.set('T0', value, description)`
- `R_coil` -> 案例参数/扫描变量; MATLAB: `model.param.set('R_coil', value, description)`
- `V_AC` -> 案例参数/扫描变量; MATLAB: `model.param.set('V_AC', value, description)`
- `Periods` -> 案例参数/扫描变量; MATLAB: `model.param.set('Periods', value, description)`
- `T_sample` -> 案例参数/扫描变量; MATLAB: `model.param.set('T_sample', value, description)`
- `N_turns` -> 案例参数/扫描变量; MATLAB: `model.param.set('N_turns', value, description)`
...
- 已识别参数：I_DC、f0、T0、R_coil、V_AC、Periods、T_sample、N_turns
...
- Sweep `I_DC` around the learned value `1[A]` and export target outputs to CSV.
...
- Sweep `R_coil` around the learned value `80[mohm]` and export target outputs to CSV.
- Sweep `V_AC` around the learned value `700[mV]` and export target outputs to CSV.
- Sweep `Periods` around the learned value `5` and export target outputs to CSV.
- Sweep `T_sample` around the learned value `T0/100` and export target outputs to CSV.
- Sweep `N_turns` around the learned value `4*20` and export target outputs to CSV.
...
- Summary: 已学习 biased_inductor 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=40 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：biased_inductor.m, biased_inductor.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 biased_inductor，训练阶段为 modeling_logic_learning_ready。
...
- `biased_inductor.mph` (comsol_mph, 25905387 bytes)
- `biased_inductor.m` (matlab_livelink, 37700 bytes)
- `biased_inductor.java` (comsol_java, 40027 bytes)
- `biased_inductor_summary.json` (json, 1196 bytes)
