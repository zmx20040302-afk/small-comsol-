# time_periodic_steady_state

- Case directory: `D:\桌面\codex\案例下载\电气\增量电感的计算和应用示例`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:23:50.795152+00:00`
...
- `V_max` = `10[V]` - Peak-to-Peak Voltage
- `f0` = `50[Hz]` - Frequency
- `T0` = `1/f0` - Time Period
- `dT` = `T0/40` - Output Timestep
- `Length` = `1[m]` - Length of time-space domian
- `TimeSpaceResolution` = `400` - Spatial Resolution
- `Periods` = `50` - Transient periods to solve
- `N` = `20` - Extra points fo smooth FFT plot
- `R1` = `0.25[ohm]` - Primary coil resistance
- `R2` = `R1` - Secondary coil resistance
- `R_load` = `50[ohm]` - Load on secondary
...
- `time_periodic_steady_state.m`: 参数=11, 几何=0, 物理场=0, 研究=0, 结果=0
- `time_periodic_steady_state.java`: 参数=11, 几何=0, 物理场=0, 研究=0, 结果=0
- `time_periodic_steady_state.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `time_periodic_steady_state_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: time_periodic_steady_state 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF keywords: 电磁
- `V_max` -> Peak-to-Peak Voltage; MATLAB: `model.param.set('V_max', value, description)`
- `f0` -> Frequency; MATLAB: `model.param.set('f0', value, description)`
- `T0` -> Time Period; MATLAB: `model.param.set('T0', value, description)`
- `dT` -> Output Timestep; MATLAB: `model.param.set('dT', value, description)`
- `Length` -> Length of time-space domian; MATLAB: `model.param.set('Length', value, description)`
- `TimeSpaceResolution` -> Spatial Resolution; MATLAB: `model.param.set('TimeSpaceResolution', value, description)`
- `Periods` -> Transient periods to solve; MATLAB: `model.param.set('Periods', value, description)`
- `N` -> Extra points fo smooth FFT plot; MATLAB: `model.param.set('N', value, description)`
- `R1` -> Primary coil resistance; MATLAB: `model.param.set('R1', value, description)`
- `R2` -> Secondary coil resistance; MATLAB: `model.param.set('R2', value, description)`
- `R_load` -> Load on secondary; MATLAB: `model.param.set('R_load', value, description)`
...
- 已识别参数：V_max、f0、T0、dT、Length、TimeSpaceResolution、Periods、N、R1、R2、R_load
...
- Sweep `V_max` around the learned value `10[V]` and export target outputs to CSV.
...
- Sweep `dT` around the learned value `T0/40` and export target outputs to CSV.
- Sweep `Length` around the learned value `1[m]` and export target outputs to CSV.
- Sweep `TimeSpaceResolution` around the learned value `400` and export target outputs to CSV.
- Sweep `Periods` around the learned value `50` and export target outputs to CSV.
...
- Summary: 已学习 time_periodic_steady_state 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=11 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：time_periodic_steady_state.m, time_periodic_steady_state.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 time_periodic_steady_state，训练阶段为 modeling_logic_learning_ready。
...
- `time_periodic_steady_state.mph` (comsol_mph, 4678742 bytes)
- `time_periodic_steady_state.m` (matlab_livelink, 57064 bytes)
- `time_periodic_steady_state.java` (comsol_java, 64523 bytes)
- `time_periodic_steady_state_summary.json` (json, 1133 bytes)
