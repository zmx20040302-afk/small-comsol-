# linear_magnetic_gear

- Case directory: `D:\桌面\codex\案例下载\电气\线性磁齿轮`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:34:16.587103+00:00`
...
- `HS_yoke_or` = `16[mm]` - High-speed yoke outer radius
- `HS_yoke_ir` = `6[mm]` - High-speed yoke inner radius
- `HS_yoke_h` = `HS_rotor_pp*HS_rotor_polepair` - High-speed yoke height
- `HS_rotor_pp` = `33[mm]` - High-speed rotor pole pitch
- `HS_pm_h` = `HS_rotor_pp/2`
- `HS_rotor_polepair` = `4` - High-speed rotor pole pair
- `PM_thickness` = `4[mm]` - Permanent magnet thickness
- `Airgap_ir` = `1[mm]` - Airgap between inner rotor and stationary rings
- `Airgap_or` = `1[mm]` - Airgap between outer rotor and stationary rings
- `S_ring_pp` = `8.8[mm]` - Statioary steel pole pitch
- `S_ring_thickness` = `6[mm]` - Stationary steel thickness
- `S_ring_polepair` = `15` - Stationary steel pole pairs
- `LS_rotor_polepair` = `S_ring_polepair-HS_rotor_polepair` - Low-speed rotor pole pairs
- `LS_rotor_pp` = `12[mm]`
- `LS_yoke_h` = `10[mm]` - Low-speed yoke thickness
- `Air_outside` = `20[mm]` - Thickness of air around the device
- `z_disp` = `0[mm]` - Z-axis displacement
- `Br` = `1.4[T]` - Remanent flux density
- `gear_ratio` = `LS_rotor_polepair/HS_rotor_polepair` - Gear ratio
- `z0` = `0[mm]` - Initial position of stationary poles
- `t` = `0[s]` - This is needed for stationary study.
- `freq_HS` = `30[Hz]`
- `freq_LS` = `freq_HS/gear_ratio`
...
- `linear_magnetic_gear.java`: 参数=20, 几何=0, 物理场=0, 研究=1, 结果=0
- `linear_magnetic_gear.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `linear_magnetic_gear.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `linear_magnetic_gear_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: linear_magnetic_gear 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF keywords: 电磁, 结构力学
- `HS_yoke_or` -> High-speed yoke outer radius; MATLAB: `model.param.set('HS_yoke_or', value, description)`
- `HS_yoke_ir` -> High-speed yoke inner radius; MATLAB: `model.param.set('HS_yoke_ir', value, description)`
- `HS_yoke_h` -> High-speed yoke height; MATLAB: `model.param.set('HS_yoke_h', value, description)`
- `HS_rotor_pp` -> High-speed rotor pole pitch; MATLAB: `model.param.set('HS_rotor_pp', value, description)`
- `HS_pm_h` -> 案例参数/扫描变量; MATLAB: `model.param.set('HS_pm_h', value, description)`
- `HS_rotor_polepair` -> High-speed rotor pole pair; MATLAB: `model.param.set('HS_rotor_polepair', value, description)`
- `PM_thickness` -> Permanent magnet thickness; MATLAB: `model.param.set('PM_thickness', value, description)`
- `Airgap_ir` -> Airgap between inner rotor and stationary rings; MATLAB: `model.param.set('Airgap_ir', value, description)`
- `Airgap_or` -> Airgap between outer rotor and stationary rings; MATLAB: `model.param.set('Airgap_or', value, description)`
- `S_ring_pp` -> Statioary steel pole pitch; MATLAB: `model.param.set('S_ring_pp', value, description)`
- `S_ring_thickness` -> Stationary steel thickness; MATLAB: `model.param.set('S_ring_thickness', value, description)`
- `S_ring_polepair` -> Stationary steel pole pairs; MATLAB: `model.param.set('S_ring_polepair', value, description)`
...
- 已识别参数：HS_yoke_or、HS_yoke_ir、HS_yoke_h、HS_rotor_pp、HS_pm_h、HS_rotor_polepair、PM_thickness、Airgap_ir、Airgap_or、S_ring_pp、S_ring_thickness、S_ring_polepair
...
- Sweep `HS_yoke_or` around the learned value `16[mm]` and export target outputs to CSV.
- Sweep `HS_yoke_ir` around the learned value `6[mm]` and export target outputs to CSV.
- Sweep `HS_yoke_h` around the learned value `HS_rotor_pp*HS_rotor_polepair` and export target outputs to CSV.
- Sweep `HS_rotor_pp` around the learned value `33[mm]` and export target outputs to CSV.
- Sweep `HS_pm_h` around the learned value `HS_rotor_pp/2` and export target outputs to CSV.
- Sweep `HS_rotor_polepair` around the learned value `4` and export target outputs to CSV.
- Sweep `PM_thickness` around the learned value `4[mm]` and export target outputs to CSV.
- Sweep `Airgap_ir` around the learned value `1[mm]` and export target outputs to CSV.
...
- Summary: 已学习 linear_magnetic_gear 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=23；内容证据=24 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 24 条；几何=0，物理场=0，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：linear_magnetic_gear.m, linear_magnetic_gear.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 linear_magnetic_gear，训练阶段为 modeling_logic_learning_ready。
...
- `linear_magnetic_gear.mph` (comsol_mph, 35874905 bytes)
- `linear_magnetic_gear.m` (matlab_livelink, 81063 bytes)
- `linear_magnetic_gear.java` (comsol_java, 80082 bytes)
- `linear_magnetic_gear_summary.json` (json, 2526 bytes)
