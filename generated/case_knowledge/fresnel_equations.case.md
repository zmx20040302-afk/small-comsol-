...
- Case directory: `D:\桌面\codex\案例下载\电气\菲涅尔方程 (RF)`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-25T10:26:15.123734+00:00`
...
- `alpha` = `70[deg]` - Angle of incidence
- `beta` = `asin(n_air*sin(alpha)/n_slab)` - Refraction angle
- `alpha_brewster` = `atan(n_slab/n_air)` - Brewster angle, TM only

## 内容级读取结果

- 模型树证据数量: 20
- 理论关键词: 电磁, PDE/方程建模, 结构力学, 优化/参数扫描
- geometry: blk1 / Block, geom1 / blk1 / size, geom1 / blk1 / layername
- physics: emw / ElectromagneticWaves, emw2 / ElectromagneticWaves, Electromagnetic Waves
- materials: mat1, mat2
- mesh: mesh1
...
- `fresnel_equations.java`: 参数=7, 几何=3, 物理场=2, 研究=0, 结果=0
- `models.rf.fresnel_equations.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `fresnel_equations_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `fresnel_equations.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `fresnel_equations.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.rf.fresnel_equations.pdf`: models.rf.fresnel_equations.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；coefficients and compares the results to the Fresnel equations.

## PDF Summary and Physics Judgement

- Simple summary: fresnel_equations 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Frequency Domain 研究。
...
- PDF: coefficients and compares the results to the Fresnel equations.
- PDF: Results and Discussion
- PDF: 2 In the Select Physics tree, select Radio Frequency > Electromagnetic Waves,
- 脚本/文本识别到的物理场: emw / ElectromagneticWaves, emw2 / ElectromagneticWaves, Electromagnetic Waves
...
- PDF keywords: 电磁, PDE/方程建模, 结构力学, 优化/参数扫描, models.rf.fresnel_equations.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；coefficients and compares the results to the Fresnel equations.
...
- `alpha_brewster` -> Brewster angle, TM only; MATLAB: `model.param.set('alpha_brewster', value, description)`
...
- 已识别物理场证据：emw / ElectromagneticWaves；emw2 / ElectromagneticWaves；Electromagnetic Waves
- 已识别参数：n_air、n_slab、lda0、f0、alpha、beta、alpha_brewster
...
- 识别到物理场证据：emw2 / ElectromagneticWaves
...
- Transfer recognized physics interfaces to similar requirements: emw / ElectromagneticWaves, emw2 / ElectromagneticWaves, Electromagnetic Waves.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, PDE/方程建模, 结构力学, 优化/参数扫描?
...
- Sweep `alpha` around the learned value `70[deg]` and export target outputs to CSV.
...
- Summary: 已学习 fresnel_equations 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=20 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=3，物理场=3，材料=2，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：fresnel_equations.m, fresnel_equations.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 fresnel_equations，训练阶段为 modeling_logic_learning_ready。
...
- `fresnel_equations.mph` (comsol_mph, 10913134 bytes)
- `fresnel_equations.m` (matlab_livelink, 43290 bytes)
- `fresnel_equations.java` (comsol_java, 43063 bytes)
- `fresnel_equations_summary.json` (json, 2451 bytes)
- `models.rf.fresnel_equations.pdf` (pdf_document, 1194469 bytes)
