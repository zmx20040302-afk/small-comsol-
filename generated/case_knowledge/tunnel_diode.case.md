...
- Case directory: `D:\桌面\codex\案例下载\电气\隧穿二极管`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:24:18.302691+00:00`
...
- `Na` = `2e19[cm^-3]` - p-doping
- `Nd` = `2e19[cm^-3]` - n-doping
- `V0` = `0[V]` - Applied voltage
- `mstar` = `2*me_const/(1/0.044+1/0.036)` - Effective mass
- `d` = `5[nm]` - Depth of tunneling layer
...
- `tunnel_diode.java`: 参数=6, 几何=1, 物理场=0, 研究=0, 结果=0
- `models.semicond.tunnel_diode.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `tunnel_diode.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `tunnel_diode.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `tunnel_diode_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.tunnel_diode.pdf`: models.semicond.tunnel_diode.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The limits of the energy integration are also obtained from the numerical results.

## PDF Summary and Physics Judgement

- Simple summary: tunnel_diode 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: The limits of the energy integration are also obtained from the numerical results.
- PDF: Results and Discussion
- PDF: 1. S.M. Sze and K.K. Ng, Physics of Semiconductor Devices , 3rd ed., John Wiley & Sons,
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 电磁
...
- PDF keywords: 传热, 电磁, models.semicond.tunnel_diode.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The limits of the energy integration are also obtained from the numerical results.
- `T0` -> Temperature; MATLAB: `model.param.set('T0', value, description)`
- `Na` -> p-doping; MATLAB: `model.param.set('Na', value, description)`
- `Nd` -> n-doping; MATLAB: `model.param.set('Nd', value, description)`
- `V0` -> Applied voltage; MATLAB: `model.param.set('V0', value, description)`
- `mstar` -> Effective mass; MATLAB: `model.param.set('mstar', value, description)`
- `d` -> Depth of tunneling layer; MATLAB: `model.param.set('d', value, description)`
...
- Sweep `Na` around the learned value `2e19[cm^-3]` and export target outputs to CSV.
- Sweep `Nd` around the learned value `2e19[cm^-3]` and export target outputs to CSV.
- Sweep `V0` around the learned value `0[V]` and export target outputs to CSV.
- Sweep `mstar` around the learned value `2*me_const/(1/0.044+1/0.036)` and export target outputs to CSV.
- Sweep `d` around the learned value `5[nm]` and export target outputs to CSV.
...
- Summary: 已学习 tunnel_diode 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=9 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 9 条；几何=1，物理场=0，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：tunnel_diode.m, tunnel_diode.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 tunnel_diode，训练阶段为 modeling_logic_learning_ready。
...
- `tunnel_diode.mph` (comsol_mph, 3725480 bytes)
- `tunnel_diode.m` (matlab_livelink, 41139 bytes)
- `tunnel_diode.java` (comsol_java, 38623 bytes)
- `tunnel_diode_summary.json` (json, 1285 bytes)
- `models.semicond.tunnel_diode.pdf` (pdf_document, 250461 bytes)
