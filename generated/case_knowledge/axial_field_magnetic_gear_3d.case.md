# axial_field_magnetic_gear_3d

- Case directory: `D:\桌面\codex\案例下载\电气\轴向磁场磁齿轮三维模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:55:39.667145+00:00`
...
- `f0_ls` = `2[Hz]` - Frequency of low speed rotor
- `f0_hs` = `5[Hz]` - Frequency of high speed rotor
- `Ri` = `30[mm]` - Inner radius of axial gear
- `Ro` = `50[mm]` - Outer radius of axial gear
- `IT_hs` = `5[mm]` - Iron thickness of high speed rotor
- `MT_hs` = `5[mm]` - Magnet thickness of high speed rotor
- `Ag_hs` = `3[mm]` - Airgap on high speed rotor
- `Ag_ls` = `3[mm]` - Airgap on low speed rotor
- `T_sp` = `10[mm]` - Thickness of stationary poles
- `MT_ls` = `5[mm]` - Magnet thickness of low speed rotor
- `IT_ls` = `8[mm]` - Iron thickness of low speed rotor
- `air_height` = `20[mm]` - External air domain height on top and bottom
- `air_radius` = `Ro*1.5` - External air domain radius

## 内容级读取结果

- 模型树证据数量: 23
- 理论关键词: 电磁
- geometry: wp1 / WorkPlane, c1 / Circle, c2 / Circle
...
- `axial_field_magnetic_gear_3d.java`: 参数=13, 几何=3, 物理场=0, 研究=1, 结果=0
- `axial_field_magnetic_gear_3d_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `axial_field_magnetic_gear_3d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `axial_field_magnetic_gear_3d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: axial_field_magnetic_gear_3d 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Magnetic Fields，并采用 Stationary 研究。
...
- PDF keywords: 电磁
- `f0_ls` -> Frequency of low speed rotor; MATLAB: `model.param.set('f0_ls', value, description)`
- `f0_hs` -> Frequency of high speed rotor; MATLAB: `model.param.set('f0_hs', value, description)`
- `Ri` -> Inner radius of axial gear; MATLAB: `model.param.set('Ri', value, description)`
- `Ro` -> Outer radius of axial gear; MATLAB: `model.param.set('Ro', value, description)`
- `IT_hs` -> Iron thickness of high speed rotor; MATLAB: `model.param.set('IT_hs', value, description)`
- `MT_hs` -> Magnet thickness of high speed rotor; MATLAB: `model.param.set('MT_hs', value, description)`
- `Ag_hs` -> Airgap on high speed rotor; MATLAB: `model.param.set('Ag_hs', value, description)`
- `Ag_ls` -> Airgap on low speed rotor; MATLAB: `model.param.set('Ag_ls', value, description)`
- `T_sp` -> Thickness of stationary poles; MATLAB: `model.param.set('T_sp', value, description)`
- `MT_ls` -> Magnet thickness of low speed rotor; MATLAB: `model.param.set('MT_ls', value, description)`
- `IT_ls` -> Iron thickness of low speed rotor; MATLAB: `model.param.set('IT_ls', value, description)`
- `air_height` -> External air domain height on top and bottom; MATLAB: `model.param.set('air_height', value, description)`
...
- 已识别几何证据：wp1 / WorkPlane；c1 / Circle；c2 / Circle
- 已识别物理场证据：Magnetic Fields
- 已识别参数：f0_ls、f0_hs、Ri、Ro、IT_hs、MT_hs、Ag_hs、Ag_ls、T_sp、MT_ls、IT_ls、air_height
...
- Sweep `f0_ls` around the learned value `2[Hz]` and export target outputs to CSV.
- Sweep `f0_hs` around the learned value `5[Hz]` and export target outputs to CSV.
...
- Sweep `IT_hs` around the learned value `5[mm]` and export target outputs to CSV.
- Sweep `MT_hs` around the learned value `5[mm]` and export target outputs to CSV.
- Sweep `Ag_hs` around the learned value `3[mm]` and export target outputs to CSV.
- Sweep `Ag_ls` around the learned value `3[mm]` and export target outputs to CSV.
...
- Summary: 已学习 axial_field_magnetic_gear_3d 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=13；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=3，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：axial_field_magnetic_gear_3d.m, axial_field_magnetic_gear_3d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 axial_field_magnetic_gear_3d，训练阶段为 modeling_logic_learning_ready。
...
- `axial_field_magnetic_gear_3d.mph` (comsol_mph, 33626179 bytes)
- `axial_field_magnetic_gear_3d.m` (matlab_livelink, 79325 bytes)
- `axial_field_magnetic_gear_3d.java` (comsol_java, 79864 bytes)
- `axial_field_magnetic_gear_3d_summary.json` (json, 1376 bytes)
