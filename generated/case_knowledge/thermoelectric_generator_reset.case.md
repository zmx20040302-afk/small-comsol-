...
- materials: mat2, mat3
- mesh: mesh1
- boundary_conditions: blk1 / size, blk1 / layername, blk1 / layertop, temp1 / 10, temp2 / 3, gnd1 / 3, fp1 / 10

### 文件内容证据

- `thermoelectric_generator_reset.java`: 参数=1, 几何=4, 物理场=2, 研究=0, 结果=0
- `thermoelectric_generator_reset.m`: 参数=1, 几何=4, 物理场=0, 研究=0, 结果=0
- `thermoelectric_generator_reset.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `thermoelectric_generator_reset_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: thermoelectric_generator_reset 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- Summary: 已学习 thermoelectric_generator_reset 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=1；内容证据=17 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 17 条；几何=4，物理场=2，材料=2，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：thermoelectric_generator_reset.m, thermoelectric_generator_reset.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 thermoelectric_generator_reset，训练阶段为 modeling_logic_learning_ready。
...
- `thermoelectric_generator_reset.mph` (comsol_mph, 3745634 bytes)
- `thermoelectric_generator_reset.m` (matlab_livelink, 33083 bytes)
- `thermoelectric_generator_reset.java` (comsol_java, 32442 bytes)
- `thermoelectric_generator_reset_summary.json` (json, 2152 bytes)
