...
- Case directory: `D:\桌面\codex\案例下载\电气\沟槽栅 IGBT 二维模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:40:04.793725+00:00`
...
- `W` = `16[um]` - Device width
- `S` = `1[um]` - Mesa width
- `DT` = `2[um]` - Trench depth
- `WT` = `0.33[um]` - Trench width
- `Wewin` = `0.16[um]` - Emitter contact window
- `Dp` = `1.4[um]` - p-base depth
- `Dn` = `0.13[um]` - n+ emitter depth
- `tOX` = `33[nm]` - Oxide thickness
- `Dnb` = `120[um]` - n-base depth
- `Ln` = `1.5[um]` - n+ emitter length
- `Lp` = `1.5[um]` - p+ emitter length
- `Nab` = `3.8e17[cm^-3]` - p-base peak doping
- `Ndb` = `8.5e13[cm^-3]` - n-base doping
- `Ndbf` = `9e15[cm^-3]` - n-buffer doping
- `Nac` = `3.7e18[cm^-3]` - p+ collector doping
- `Nae` = `1e20[cm^-3]` - p+ emitter doping
- `Nde` = `1e20[cm^-3]` - n+ emitter doping
- `tau0` = `10[us]` - Carrier lifetime
- `tnbf` = `5[um]` - n-buffer thickness
- `Dsub` = `10[um]` - p+ collector thickness
- `t0` = `Dp+Dnb+tnbf+Dsub` - Device thickness
- `d0` = `(Ln+Lp)/2` - Out-of-plan thickness
- `rhoCC` = `1.6e-4[ohm*cm^2]` - p+ substrate equivalent resistivity
- `rhoCE` = `4.7e-6[ohm*cm^2]` - Emitter contact resistivity
- `T0` = `300[K]` - Lattice temperature
- `Eg0` = `1.1743[V]` - Band gap Shigyo
- `Nv_` = `(T0/300[K])^(3/2)*1.04e19[1/cm^3]` - Valence band DOS COMSOL
- `Nc_` = `(T0/300[K])^(3/2)*2.8e19[1/cm^3]` - Conduction band DOS COMSOL
- `ni0` = `1e10[cm^-3]` - Intrinsic carrier concentration
- `Nvcfac` = `sqrt(ni0^2/Nv_/Nc_/exp(-Eg0*e_const/T0/k_B_const))` - DOS factor to match ni

## 内容级读取结果

- 模型树证据数量: 32
- 理论关键词: 传热, 流体, 电磁, 优化/参数扫描
- studies: Stationary, Parametric Sweep

### 文件内容证据

- `trench_gate_igbt_2d.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.semicond.trench_gate_igbt_2d.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `trench_gate_igbt_2d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `trench_gate_igbt_2d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `trench_gate_igbt_2d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.trench_gate_igbt_2d.pdf`: models.semicond.trench_gate_igbt_2d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；collector voltage agrees reasonably well with the published result.

## PDF Summary and Physics Judgement

- Simple summary: trench_gate_igbt_2d 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: collector voltage agrees reasonably well with the published result.
- PDF: simulation result by comparing 2D and 3D models of trench-gate IGBTs. They found that
- PDF: while the 3D model agrees well with measured result, the 2D model is off by the factor of
...
- PDF keywords: 传热, 流体, 电磁, 优化/参数扫描, models.semicond.trench_gate_igbt_2d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；collector voltage agrees reasonably well with the published result.
- `W` -> Device width; MATLAB: `model.param.set('W', value, description)`
- `S` -> Mesa width; MATLAB: `model.param.set('S', value, description)`
- `DT` -> Trench depth; MATLAB: `model.param.set('DT', value, description)`
- `WT` -> Trench width; MATLAB: `model.param.set('WT', value, description)`
- `Wewin` -> Emitter contact window; MATLAB: `model.param.set('Wewin', value, description)`
- `Dp` -> p-base depth; MATLAB: `model.param.set('Dp', value, description)`
- `Dn` -> n+ emitter depth; MATLAB: `model.param.set('Dn', value, description)`
- `tOX` -> Oxide thickness; MATLAB: `model.param.set('tOX', value, description)`
- `Dnb` -> n-base depth; MATLAB: `model.param.set('Dnb', value, description)`
- `Ln` -> n+ emitter length; MATLAB: `model.param.set('Ln', value, description)`
- `Lp` -> p+ emitter length; MATLAB: `model.param.set('Lp', value, description)`
- `Nab` -> p-base peak doping; MATLAB: `model.param.set('Nab', value, description)`
...
- 已识别参数：W、S、DT、WT、Wewin、Dp、Dn、tOX、Dnb、Ln、Lp、Nab
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 流体, 电磁, 优化/参数扫描?
...
- Sweep `W` around the learned value `16[um]` and export target outputs to CSV.
- Sweep `S` around the learned value `1[um]` and export target outputs to CSV.
- Sweep `DT` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `WT` around the learned value `0.33[um]` and export target outputs to CSV.
- Sweep `Wewin` around the learned value `0.16[um]` and export target outputs to CSV.
- Sweep `Dp` around the learned value `1.4[um]` and export target outputs to CSV.
- Sweep `Dn` around the learned value `0.13[um]` and export target outputs to CSV.
- Sweep `tOX` around the learned value `33[nm]` and export target outputs to CSV.
...
- Summary: 已学习 trench_gate_igbt_2d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=30；内容证据=32 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 32 条；几何=0，物理场=0，材料=0，网格=0，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：trench_gate_igbt_2d.m, trench_gate_igbt_2d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 trench_gate_igbt_2d，训练阶段为 modeling_logic_learning_ready。
...
- `trench_gate_igbt_2d.mph` (comsol_mph, 7772180 bytes)
- `trench_gate_igbt_2d.m` (matlab_livelink, 75123 bytes)
- `trench_gate_igbt_2d.java` (comsol_java, 68892 bytes)
- `trench_gate_igbt_2d_summary.json` (json, 2026 bytes)
- `models.semicond.trench_gate_igbt_2d.pdf` (pdf_document, 884930 bytes)
