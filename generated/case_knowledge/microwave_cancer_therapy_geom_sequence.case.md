...
- 模型树证据数量: 44
- 理论关键词: 电磁, 传热, 多孔介质/裂隙
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, r4 / Rectangle, r5 / Rectangle, pol1 / Polygon, uni1 / Union, dif1 / Difference, geom1 / r1 / size, geom1 / r2 / size, geom1 / r2 / pos, geom1 / r3 / size
- physics: Heat Transfer, Magnetic Fields, Electromagnetic Waves
- mesh: mesh1
- studies: Time Dependent, Frequency Domain
- boundary_conditions: r1 / size, r2 / size, r2 / pos, r3 / size, r3 / pos, r3 / selresult, r4 / size, r4 / pos, r5 / size, r5 / pos, r5 / selresult, pol1 / x

### 文件内容证据

- `microwave_cancer_therapy_geom_sequence.m`: 参数=0, 几何=20, 物理场=0, 研究=0, 结果=0
- `microwave_cancer_therapy_geom_sequence.java`: 参数=0, 几何=20, 物理场=0, 研究=0, 结果=0
...
- `microwave_cancer_therapy_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `microwave_cancer_therapy_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: microwave_cancer_therapy_geom_sequence 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF keywords: 电磁, 传热, 多孔介质/裂隙, models.heat.microwave_cancer_therapy.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields、Electromagnetic Waves，研究类型线索为 Time Dependent、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；suited for modeling in COMSOL Multiphysics because of its multiphysics capabilities., models.rf.microwave_cancer_therapy.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields、Electromagnetic Waves，研究类型线索为 Time Dependent、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；suited for modeling in COMSOL Multiphysics because of its multiphysics capabilities.
...
- Summary: 已学习 microwave_cancer_therapy_geom_sequence 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=44 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 44 条；几何=23，物理场=3，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：microwave_cancer_therapy_geom_sequence.m, microwave_cancer_therapy_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 microwave_cancer_therapy_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `microwave_cancer_therapy_geom_sequence.mph` (comsol_mph, 171942 bytes)
- `microwave_cancer_therapy_geom_sequence.m` (matlab_livelink, 5193 bytes)
- `microwave_cancer_therapy_geom_sequence.java` (comsol_java, 4980 bytes)
- `microwave_cancer_therapy_geom_sequence_summary.json` (json, 672 bytes)
