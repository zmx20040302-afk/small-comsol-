...
- Case directory: `D:\桌面\codex\案例下载\电气\微波炉`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T08:34:02.104309+00:00`
...
- `wo` = `267[mm]` - Oven width
- `do` = `270[mm]` - Oven depth
- `ho` = `188[mm]` - Oven height
- `wg` = `50[mm]` - Waveguide width
- `dg` = `78[mm]` - Waveguide depth
- `hg` = `18[mm]` - Waveguide height
- `rp` = `113.5[mm]` - Glass plate radius
- `hp` = `6[mm]` - Glass plate height
- `bp` = `15[mm]` - Glass plate base
- `rpot` = `31.5[mm]` - Potato radius
- `T0` = `8[degC]` - Initial potato temperature
- `full_geometry` = `1` - Symmetry flag

## 内容级读取结果

- 模型树证据数量: 43
- 理论关键词: 传热, 电磁, 流体, 多孔介质/裂隙, 优化/参数扫描
- geometry: blk1 / Block, blk2 / Block, cyl1 / Cylinder, sph1 / Sphere, uni1 / Union, geom1 / blk1 / size, geom1 / blk1 / pos, geom1 / blk2 / size, geom1 / blk2 / pos, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / pos
- physics: Heat Transfer, Electromagnetic Waves, Magnetic Fields
- mesh: mesh1
- studies: Stationary, Frequency Domain, Time Dependent, Parametric Sweep
- boundary_conditions: blk1 / size, blk1 / pos, blk2 / size, blk2 / pos, cyl1 / r, cyl1 / h, cyl1 / pos, sph1 / r, sph1 / pos

### 文件内容证据

- `microwave_oven.java`: 参数=12, 几何=14, 物理场=0, 研究=0, 结果=0
- `models.rf.microwave_oven.pdf`: 参数=0, 几何=0, 物理场=3, 研究=4, 结果=0
- `microwave_oven.pptx`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `microwave_oven.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `microwave_oven.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `microwave_oven_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.rf.microwave_oven.pdf`: models.rf.microwave_oven.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields、Electromagnetic Waves，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；potato, and plate. Figure 1 below shows both the full and reduced size geometry.

## PDF Summary and Physics Judgement

- Simple summary: microwave_oven 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: potato, and plate. Figure 1 below shows both the full and reduced size geometry.
- PDF: Figure 1: Geometry of the microwave oven, potato, and waveguide feed. Full size (left) and
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: Heat Transfer, Electromagnetic Waves, Magnetic Fields
- 识别到的研究类型: Stationary, Frequency Domain, Time Dependent, Parametric Sweep
- 理论关键词: 传热, 电磁, 流体, 多孔介质/裂隙, 优化/参数扫描

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 传热, 电磁, 流体, 多孔介质/裂隙, 优化/参数扫描, models.rf.microwave_oven.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields、Electromagnetic Waves，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；potato, and plate. Figure 1 below shows both the full and reduced size geometry.
- `wo` -> Oven width; MATLAB: `model.param.set('wo', value, description)`
- `do` -> Oven depth; MATLAB: `model.param.set('do', value, description)`
- `ho` -> Oven height; MATLAB: `model.param.set('ho', value, description)`
- `wg` -> Waveguide width; MATLAB: `model.param.set('wg', value, description)`
- `dg` -> Waveguide depth; MATLAB: `model.param.set('dg', value, description)`
- `hg` -> Waveguide height; MATLAB: `model.param.set('hg', value, description)`
- `rp` -> Glass plate radius; MATLAB: `model.param.set('rp', value, description)`
- `hp` -> Glass plate height; MATLAB: `model.param.set('hp', value, description)`
- `bp` -> Glass plate base; MATLAB: `model.param.set('bp', value, description)`
- `rpot` -> Potato radius; MATLAB: `model.param.set('rpot', value, description)`
- `T0` -> Initial potato temperature; MATLAB: `model.param.set('T0', value, description)`
- `full_geometry` -> Symmetry flag; MATLAB: `model.param.set('full_geometry', value, description)`
...
- 已识别几何证据：blk1 / Block；blk2 / Block；cyl1 / Cylinder；sph1 / Sphere；uni1 / Union；geom1 / blk1 / size
- 已识别物理场证据：Heat Transfer；Electromagnetic Waves；Magnetic Fields
- 已识别参数：wo、do、ho、wg、dg、hg、rp、hp、bp、rpot、T0、full_geometry
...
- Transfer recognized physics interfaces to similar requirements: Heat Transfer, Electromagnetic Waves, Magnetic Fields.
- Reuse geometry construction patterns when new requirements share shape, import, array, sweep, or boolean operations.
- Use the learned study types as solver starting points: Stationary, Frequency Domain, Time Dependent, Parametric Sweep.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 电磁, 流体, 多孔介质/裂隙, 优化/参数扫描?
...
- Sweep `wo` around the learned value `267[mm]` and export target outputs to CSV.
- Sweep `do` around the learned value `270[mm]` and export target outputs to CSV.
- Sweep `ho` around the learned value `188[mm]` and export target outputs to CSV.
- Sweep `wg` around the learned value `50[mm]` and export target outputs to CSV.
- Sweep `dg` around the learned value `78[mm]` and export target outputs to CSV.
- Sweep `hg` around the learned value `18[mm]` and export target outputs to CSV.
- Sweep `rp` around the learned value `113.5[mm]` and export target outputs to CSV.
- Sweep `hp` around the learned value `6[mm]` and export target outputs to CSV.
...
- Summary: 已学习 microwave_oven 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=43 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 43 条；几何=14，物理场=3，材料=0，网格=1，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：microwave_oven.m, microwave_oven.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 microwave_oven，训练阶段为 modeling_logic_learning_ready。
...
- `microwave_oven.mph` (comsol_mph, 318299253 bytes)
- `microwave_oven.m` (matlab_livelink, 60528 bytes)
- `microwave_oven.java` (comsol_java, 61905 bytes)
- `microwave_oven_summary.json` (json, 2517 bytes)
- `microwave_oven.pptx` (presentation_document, 1590896 bytes)
- `models.rf.microwave_oven.pdf` (pdf_document, 2907723 bytes)
