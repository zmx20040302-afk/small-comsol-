# negative_surface_discharge

- Case directory: `D:\桌面\codex\案例下载\电气\负极气-固界面的表面放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:36:34.501800+00:00`
...
- `Rcurv` = `25[um]` - Radius of curvature
- `a` = `1/Rcurv/2*1[cm]` - Parabola parameter in cm
- `d_air` = `500[um]` - Air gap
- `d_glass` = `800[um]` - Thickness of the glass layer
- `d_BSO` = `160[um]` - Thickness of the BSO layer
- `gap` = `d_air+d_BSO+d_glass` - Electrode gap

## 内容级读取结果

- 模型树证据数量: 30
- 理论关键词: 电磁, 流体
- geometry: r1 / Rectangle, r2 / Rectangle, pc1 / ParametricCurve, geom1 / r1 / layername, geom1 / r1 / layer, geom1 / r2 / size, geom1 / r2 / layername, geom1 / r2 / layerleft, geom1 / r2 / layerbottom, geom1 / pc1 / parmax, geom1 / pc1 / coord
- physics: Electrostatics
- mesh: mesh1, mesh2
- studies: Time Dependent
- boundary_conditions: r1 / layername, r1 / layer, r2 / size, r2 / layername, r2 / layerleft, r2 / layerbottom, pc1 / parmax, pc1 / coord

### 文件内容证据

- `negative_surface_discharge.java`: 参数=7, 几何=11, 物理场=0, 研究=0, 结果=0
- `models.edis.negative_surface_discharge.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `negative_surface_discharge.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `negative_surface_discharge.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `negative_surface_discharge_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.negative_surface_discharge.pdf`: models.edis.negative_surface_discharge.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The simulation results, including discharge current and surface charge distribution at the

## PDF Summary and Physics Judgement

- Simple summary: negative_surface_discharge 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electrostatics，并采用 Time Dependent 研究。
- Primary physics: `Electrostatics`
- Study type: `Time Dependent`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Electrostatics，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Time Dependent 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: The simulation results, including discharge current and surface charge distribution at the
- PDF: discharge and the resulting electrical characteristics.
- PDF: The first study examines the fast surface charging caused by streamer bursts when a high
...
- PDF keywords: 电磁, 流体, models.edis.negative_surface_discharge.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The simulation results, including discharge current and surface charge distribution at the
- `V0` -> Applied voltage; MATLAB: `model.param.set('V0', value, description)`
- `Rcurv` -> Radius of curvature; MATLAB: `model.param.set('Rcurv', value, description)`
- `a` -> Parabola parameter in cm; MATLAB: `model.param.set('a', value, description)`
- `d_air` -> Air gap; MATLAB: `model.param.set('d_air', value, description)`
- `d_glass` -> Thickness of the glass layer; MATLAB: `model.param.set('d_glass', value, description)`
- `d_BSO` -> Thickness of the BSO layer; MATLAB: `model.param.set('d_BSO', value, description)`
- `gap` -> Electrode gap; MATLAB: `model.param.set('gap', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；pc1 / ParametricCurve；geom1 / r1 / layername；geom1 / r1 / layer；geom1 / r2 / size
- 已识别物理场证据：Electrostatics
- 已识别参数：V0、Rcurv、a、d_air、d_glass、d_BSO、gap
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 流体?
...
- Sweep `V0` around the learned value `-2.5[kV]` and export target outputs to CSV.
- Sweep `Rcurv` around the learned value `25[um]` and export target outputs to CSV.
- Sweep `a` around the learned value `1/Rcurv/2*1[cm]` and export target outputs to CSV.
- Sweep `d_air` around the learned value `500[um]` and export target outputs to CSV.
- Sweep `d_glass` around the learned value `800[um]` and export target outputs to CSV.
- Sweep `d_BSO` around the learned value `160[um]` and export target outputs to CSV.
- Sweep `gap` around the learned value `d_air+d_BSO+d_glass` and export target outputs to CSV.
...
- Summary: 已学习 negative_surface_discharge 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=30 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 30 条；几何=11，物理场=1，材料=0，网格=2，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：negative_surface_discharge.m, negative_surface_discharge.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 negative_surface_discharge，训练阶段为 modeling_logic_learning_ready。
...
- `negative_surface_discharge.mph` (comsol_mph, 103681618 bytes)
- `negative_surface_discharge.m` (matlab_livelink, 58731 bytes)
- `negative_surface_discharge.java` (comsol_java, 54949 bytes)
- `negative_surface_discharge_summary.json` (json, 1412 bytes)
- `models.edis.negative_surface_discharge.pdf` (pdf_document, 990947 bytes)
