...
- Case directory: `D:\桌面\codex\案例下载\电气\圆偏振平面波的反射`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:22:24.619954+00:00`
...
- `n_slab` = `1.5` - Refractive index, slab
- `lda0` = `1[um]` - Wavelength
- `a0` = `0.3*lda0` - Domain width
- `b0` = `1.2*lda0` - Domain height
- `alpha` = `45[deg]` - Angle of incidence
- `beta` = `asin(n_air*sin(alpha)/n_slab)` - Refraction angle
- `alpha_Brewster` = `atan(n_slab/n_air)` - Brewster angle
- `R_rhcp` = `abs((r_s+r_p)/2)^2` - Reflectance, RHCP
- `R_lhcp` = `abs((r_s-r_p)/2)^2` - Reflectance, LHCP
- `T_rhcp` = `n_slab*cos(beta)/(n_air*cos(alpha))*abs((t_s+t_p)/2)^2` - Transmittance, RHCP
- `T_lhcp` = `n_slab*cos(beta)/(n_air*cos(alpha))*abs((t_s-t_p)/2)^2` - Transmittance, LHCP

## 内容级读取结果

- 模型树证据数量: 20
- 理论关键词: 传热, 电磁
- geometry: blk1 / Block, geom1 / blk1 / size, geom1 / blk1 / layername
- physics: Electromagnetic Waves
- mesh: mesh1
- studies: Frequency Domain
- boundary_conditions: blk1 / size, blk1 / layername

### 文件内容证据

- `circular_polarization.java`: 参数=12, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.woptics.circular_polarization.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `circular_polarization.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `circular_polarization.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `circular_polarization_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.circular_polarization.pdf`: models.woptics.circular_polarization.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；comparisons show excellent agreement between the computed results and analytical

## PDF Summary and Physics Judgement

- Simple summary: circular_polarization 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- PDF: comparisons show excellent agreement between the computed results and analytical
- PDF: polarizations are different. This is described in more detail in the Fresnel Equations model.
- PDF: physics) and y is the unit vector in the y-direction. The minus sign is used for the excitation
...
- PDF keywords: 传热, 电磁, models.woptics.circular_polarization.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；comparisons show excellent agreement between the computed results and analytical
- `n_air` -> Refractive index, air; MATLAB: `model.param.set('n_air', value, description)`
- `n_slab` -> Refractive index, slab; MATLAB: `model.param.set('n_slab', value, description)`
- `lda0` -> Wavelength; MATLAB: `model.param.set('lda0', value, description)`
- `a0` -> Domain width; MATLAB: `model.param.set('a0', value, description)`
- `b0` -> Domain height; MATLAB: `model.param.set('b0', value, description)`
...
- `alpha_Brewster` -> Brewster angle; MATLAB: `model.param.set('alpha_Brewster', value, description)`
- `R_rhcp` -> Reflectance, RHCP; MATLAB: `model.param.set('R_rhcp', value, description)`
- `R_lhcp` -> Reflectance, LHCP; MATLAB: `model.param.set('R_lhcp', value, description)`
- `T_rhcp` -> Transmittance, RHCP; MATLAB: `model.param.set('T_rhcp', value, description)`
- `T_lhcp` -> Transmittance, LHCP; MATLAB: `model.param.set('T_lhcp', value, description)`
...
- 已识别几何证据：blk1 / Block；geom1 / blk1 / size；geom1 / blk1 / layername
- 已识别物理场证据：Electromagnetic Waves
- 已识别参数：n_air、n_slab、lda0、a0、b0、alpha、beta、alpha_Brewster、R_rhcp、R_lhcp、T_rhcp、T_lhcp
...
- Sweep `n_air` around the learned value `1` and export target outputs to CSV.
- Sweep `n_slab` around the learned value `1.5` and export target outputs to CSV.
- Sweep `lda0` around the learned value `1[um]` and export target outputs to CSV.
- Sweep `a0` around the learned value `0.3*lda0` and export target outputs to CSV.
- Sweep `b0` around the learned value `1.2*lda0` and export target outputs to CSV.
- Sweep `alpha` around the learned value `45[deg]` and export target outputs to CSV.
- Sweep `beta` around the learned value `asin(n_air*sin(alpha)/n_slab)` and export target outputs to CSV.
- Sweep `alpha_Brewster` around the learned value `atan(n_slab/n_air)` and export target outputs to CSV.
...
- Summary: 已学习 circular_polarization 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=20 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=3，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：circular_polarization.m, circular_polarization.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 circular_polarization，训练阶段为 modeling_logic_learning_ready。
...
- `circular_polarization.mph` (comsol_mph, 18753929 bytes)
- `circular_polarization.m` (matlab_livelink, 71555 bytes)
- `circular_polarization.java` (comsol_java, 71166 bytes)
- `circular_polarization_summary.json` (json, 3087 bytes)
- `models.woptics.circular_polarization.pdf` (pdf_document, 1760502 bytes)
