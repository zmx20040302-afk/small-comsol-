...
- Created at: `2026-07-06T09:43:44.382954+00:00`
...
- boundary_conditions: temp1 / 1, sar1 / 2, lngr1 / 1, temp1 / T0, sar1 / epsilon_rad_mat, sar1 / epsilon_rad, sar1 / Tamb, solid1 / k_mat, init1 / Tinit, lngr1 / xdata, lngr1 / xdataexpr, lngr1 / data

### 文件内容证据

- `heat_radiation_1d.mph`: 参数=0, 几何=1, 物理场=1, 研究=3, 结果=2
- `heat_radiation_1d.java`: 参数=0, 几何=1, 物理场=1, 研究=3, 结果=2
- `heat_radiation_1d.m`: 参数=0, 几何=1, 物理场=1, 研究=2, 结果=2
- `heat_radiation_1d.zh_CN.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mph.heat_radiation_1d.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mph.heat_radiation_1d.zh_CN.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mph.heat_radiation_1d.pdf`: models.mph.heat_radiation_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.heat_radiation_1d.zh_CN.pdf`: models.mph.heat_radiation_1d.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 稳态辐射传热 - 一维 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF keywords: 传热, models.mph.heat_radiation_1d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.heat_radiation_1d.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
...
- Summary: 已学习 稳态辐射传热 - 一维 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=24 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 24 条；几何=1，物理场=1，材料=0，网格=1，研究=3，结果=2。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：heat_radiation_1d.java, heat_radiation_1d.m
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 稳态辐射传热 - 一维，训练阶段为 modeling_logic_learning_ready。
...
- `稳态辐射传热 - 一维` score=0.2342; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=7
...
- `heat_radiation_1d.java` (comsol_java, 3769 bytes)
- `heat_radiation_1d.m` (matlab_livelink, 6706 bytes)
- `heat_radiation_1d.mph` (comsol_mph, 1595249 bytes)
- `heat_radiation_1d.zh_CN.mph` (comsol_mph, 1609563 bytes)
