...
- Case directory: `D:\桌面\codex\案例下载\电气\石墨烯超材料完美吸收器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:56:53.453911+00:00`
...
- `T` = `300[K]` - Temperature
- `Ef` = `0.2[eV]` - Fermi energy
- `tau` = `1e-13[s]` - Relexation time
- `d_eff` = `1[nm]` - Effective thickness of graphene
- `a` = `15[um]` - Unit cell length
- `b` = `2[um]` - Geometric parameter 1
- `d_sub` = `17.6[um]` - Substrate thickness
- `d_domain` = `40[um]` - Simulation domain height
- `w` = `12[um]` - Geometric parameter 2
- `n_bg` = `1.53` - Substrate refractive index
...
- `graphene_metamaterial_perfect_absorber.java`: 参数=10, 几何=2, 物理场=0, 研究=0, 结果=0
- `graphene_metamaterial_perfect_absorber.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `models.woptics.graphene_metamaterial_perfect_absorber.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `graphene_metamaterial_perfect_absorber.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `graphene_metamaterial_perfect_absorber.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `graphene_metamaterial_perfect_absorber_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `graphene_metamaterial_perfect_absorber.pdf`: graphene_metamaterial_perfect_absorber.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Created in COMSOL Multiphysics 6.0；TOPAS is 1.53 in THz domain. The geometric parameters for the patterned graphene can
- `models.woptics.graphene_metamaterial_perfect_absorber.pdf`: models.woptics.graphene_metamaterial_perfect_absorber.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；TOPAS is 1.53 in THz domain. The geometric parameters for the patterned graphene can

## PDF Summary and Physics Judgement

- Simple summary: graphene_metamaterial_perfect_absorber 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- PDF: Created in COMSOL Multiphysics 6.0
- PDF: TOPAS is 1.53 in THz domain. The geometric parameters for the patterned graphene can
...
- PDF keywords: 传热, 结构力学, 电磁, 优化/参数扫描, graphene_metamaterial_perfect_absorber.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Created in COMSOL Multiphysics 6.0；TOPAS is 1.53 in THz domain. The geometric parameters for the patterned graphene can, models.woptics.graphene_metamaterial_perfect_absorber.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；TOPAS is 1.53 in THz domain. The geometric parameters for the patterned graphene can
- `T` -> Temperature; MATLAB: `model.param.set('T', value, description)`
- `Ef` -> Fermi energy; MATLAB: `model.param.set('Ef', value, description)`
- `tau` -> Relexation time; MATLAB: `model.param.set('tau', value, description)`
- `d_eff` -> Effective thickness of graphene; MATLAB: `model.param.set('d_eff', value, description)`
- `a` -> Unit cell length; MATLAB: `model.param.set('a', value, description)`
- `b` -> Geometric parameter 1; MATLAB: `model.param.set('b', value, description)`
- `d_sub` -> Substrate thickness; MATLAB: `model.param.set('d_sub', value, description)`
- `d_domain` -> Simulation domain height; MATLAB: `model.param.set('d_domain', value, description)`
- `w` -> Geometric parameter 2; MATLAB: `model.param.set('w', value, description)`
- `n_bg` -> Substrate refractive index; MATLAB: `model.param.set('n_bg', value, description)`
...
- 已识别参数：T、Ef、tau、d_eff、a、b、d_sub、d_domain、w、n_bg
...
- Sweep `T` around the learned value `300[K]` and export target outputs to CSV.
- Sweep `Ef` around the learned value `0.2[eV]` and export target outputs to CSV.
- Sweep `tau` around the learned value `1e-13[s]` and export target outputs to CSV.
- Sweep `d_eff` around the learned value `1[nm]` and export target outputs to CSV.
- Sweep `a` around the learned value `15[um]` and export target outputs to CSV.
- Sweep `b` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `d_sub` around the learned value `17.6[um]` and export target outputs to CSV.
- Sweep `d_domain` around the learned value `40[um]` and export target outputs to CSV.
...
- Summary: 已学习 graphene_metamaterial_perfect_absorber 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=17 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 17 条；几何=2，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：graphene_metamaterial_perfect_absorber.m, graphene_metamaterial_perfect_absorber.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 graphene_metamaterial_perfect_absorber，训练阶段为 modeling_logic_learning_ready。
...
- `graphene_metamaterial_perfect_absorber.mph` (comsol_mph, 130622800 bytes)
- `graphene_metamaterial_perfect_absorber.m` (matlab_livelink, 28105 bytes)
- `graphene_metamaterial_perfect_absorber.java` (comsol_java, 17480 bytes)
- `graphene_metamaterial_perfect_absorber_summary.json` (json, 1147 bytes)
- `graphene_metamaterial_perfect_absorber.pdf` (pdf_document, 1250175 bytes)
- `models.woptics.graphene_metamaterial_perfect_absorber.pdf` (pdf_document, 789522 bytes)
