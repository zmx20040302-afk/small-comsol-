# piezoelectric_hysteresis_parametric_sweep

- Case directory: `D:\桌面\codex\案例下载\电气\压电陶瓷的滞回现象`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T07:05:02.043128+00:00`
...
- `t` = `0[s]` - Time parameter
- `W` = `1.5[in]` - Actiator width
- `D` = `0.25[in]` - Actuator depth
- `H` = `0.015[in]` - Actuator height
- `alpha` = `4.2e6[m/F]` - Interdomain coupling
- `a` = `6.4e5[V/m]` - Domain wall density
- `c` = `0.2` - Polarization reversibility
- `k` = `1e6[V/m]` - Pinning loss
- `Ps` = `0.425[C/m^2]` - Saturation polarization
- `Q11` = `3.579e-2[m^4/C^2]` - Electrostriction coupling parameter
- `Q12` = `-5.335e-3[m^4/C^2]` - Electrostriction coupling parameter
- `Q44` = `1.923e-2[m^4/C^2]` - Electrostriction coupling parameter
- `Vmax` = `1200[V]` - Maximum applied voltage
- `F0` = `0[MPa]` - Applied mechanical load

## 内容级读取结果

- 模型树证据数量: 31
- 理论关键词: 电磁, 结构力学, 传热
- geometry: blk1 / Block, geom1 / blk1 / size
- physics: solid / SolidMechanics, es / Electrostatics, Electrostatics, Solid Mechanics
- materials: mat1
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: sym1 / [1 2 3], bndl1 / [4], blk1 / size, sym1 / 1, 2, 3, bndl1 / 4, gnd1 / 4, pot1 / 3

### 文件内容证据

- `piezoelectric_hysteresis_parametric_sweep.java`: 参数=14, 几何=2, 物理场=3, 研究=0, 结果=0
- `piezoelectric_hysteresis_parametric_sweep.m`: 参数=14, 几何=2, 物理场=3, 研究=0, 结果=0
- `models.mems.piezoelectric_hysteresis.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `models.sme.piezoelectric_hysteresis.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `piezoelectric_hysteresis_parametric_sweep.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoelectric_hysteresis_parametric_sweep_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.piezoelectric_hysteresis.pdf`: models.mems.piezoelectric_hysteresis.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；available in COMSOL Multiphysics for modeling ferroelectric hysteresis. It assumes that
- `models.sme.piezoelectric_hysteresis.pdf`: models.sme.piezoelectric_hysteresis.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；available in COMSOL Multiphysics for modeling ferroelectric hysteresis. It assumes that

## PDF Summary and Physics Judgement

- Simple summary: piezoelectric_hysteresis_parametric_sweep 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: available in COMSOL Multiphysics for modeling ferroelectric hysteresis. It assumes that
- PDF: Because of the symmetry, it is sufficient to model one quarter of the actual geometry.
- PDF: Figure 1: Model geometry.
- 脚本/文本识别到的物理场: solid / SolidMechanics, es / Electrostatics, Electrostatics, Solid Mechanics
...
- PDF keywords: 电磁, 结构力学, 传热, models.mems.piezoelectric_hysteresis.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；available in COMSOL Multiphysics for modeling ferroelectric hysteresis. It assumes that, models.sme.piezoelectric_hysteresis.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；available in COMSOL Multiphysics for modeling ferroelectric hysteresis. It assumes that
- `t` -> Time parameter; MATLAB: `model.param.set('t', value, description)`
- `W` -> Actiator width; MATLAB: `model.param.set('W', value, description)`
- `D` -> Actuator depth; MATLAB: `model.param.set('D', value, description)`
- `H` -> Actuator height; MATLAB: `model.param.set('H', value, description)`
- `alpha` -> Interdomain coupling; MATLAB: `model.param.set('alpha', value, description)`
- `a` -> Domain wall density; MATLAB: `model.param.set('a', value, description)`
- `c` -> Polarization reversibility; MATLAB: `model.param.set('c', value, description)`
- `k` -> Pinning loss; MATLAB: `model.param.set('k', value, description)`
- `Ps` -> Saturation polarization; MATLAB: `model.param.set('Ps', value, description)`
- `Q11` -> Electrostriction coupling parameter; MATLAB: `model.param.set('Q11', value, description)`
- `Q12` -> Electrostriction coupling parameter; MATLAB: `model.param.set('Q12', value, description)`
- `Q44` -> Electrostriction coupling parameter; MATLAB: `model.param.set('Q44', value, description)`
...
- 已识别几何证据：blk1 / Block；geom1 / blk1 / size
- 已识别物理场证据：solid / SolidMechanics；es / Electrostatics；Electrostatics；Solid Mechanics
- 已识别参数：t、W、D、H、alpha、a、c、k、Ps、Q11、Q12、Q44
...
- Transfer recognized physics interfaces to similar requirements: solid / SolidMechanics, es / Electrostatics, Electrostatics, Solid Mechanics.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 结构力学, 传热?
...
- Sweep `t` around the learned value `0[s]` and export target outputs to CSV.
- Sweep `W` around the learned value `1.5[in]` and export target outputs to CSV.
- Sweep `D` around the learned value `0.25[in]` and export target outputs to CSV.
- Sweep `H` around the learned value `0.015[in]` and export target outputs to CSV.
- Sweep `alpha` around the learned value `4.2e6[m/F]` and export target outputs to CSV.
- Sweep `a` around the learned value `6.4e5[V/m]` and export target outputs to CSV.
- Sweep `c` around the learned value `0.2` and export target outputs to CSV.
- Sweep `k` around the learned value `1e6[V/m]` and export target outputs to CSV.
...
- Summary: 已学习 piezoelectric_hysteresis_parametric_sweep 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=14；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=2，物理场=4，材料=1，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：piezoelectric_hysteresis_parametric_sweep.m, piezoelectric_hysteresis_parametric_sweep.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 piezoelectric_hysteresis_parametric_sweep，训练阶段为 modeling_logic_learning_ready。
...
- `piezoelectric_hysteresis_parametric_sweep.mph` (comsol_mph, 121885560 bytes)
- `piezoelectric_hysteresis_parametric_sweep.m` (matlab_livelink, 39329 bytes)
- `piezoelectric_hysteresis_parametric_sweep.java` (comsol_java, 42179 bytes)
- `piezoelectric_hysteresis_parametric_sweep_summary.json` (json, 1474 bytes)
- `models.mems.piezoelectric_hysteresis.pdf` (pdf_document, 510945 bytes)
- `models.sme.piezoelectric_hysteresis.pdf` (pdf_document, 510960 bytes)
