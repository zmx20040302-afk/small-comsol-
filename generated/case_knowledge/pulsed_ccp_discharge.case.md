...
- Case directory: `D:\桌面\codex\案例下载\电气\脉冲 CCP 放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:12:01.212756+00:00`
...
- `f0` = `13.56[MHz]` - Base frequency
- `As` = `0.25*pi*(0.3[m])^2` - Wafer surface area
- `p0` = `0.1[torr]` - Pressure
- `T0` = `300[K]` - Temperature
- `Prf` = `1[W]` - Single pulse power
- `Non` = `10` - Number of periods the pulse is on
- `Noff` = `100` - Number of periods the pulse is off
- `Nrampup` = `1` - Number of cycles to ramp to on state
- `Nrampdown` = `1` - Number of cycles to ramp to off state
- `Nelon` = `30` - Number of elements per period for the on period
- `Neloff` = `100` - Number of elements for the off period
- `Ncycles` = `Non+Noff` - Total number of cycles
- `period_single` = `Ncycles/f0` - Total period
...
- physics: ptp / ColdPlasmaTimePeriodic
- mesh: mesh1
- studies: Time Dependent
- boundary_conditions: gnd1 / 2, mct1 / 1, i1 / coord

### 文件内容证据

- `pulsed_ccp_discharge.java`: 参数=13, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.plasma.pulsed_ccp_discharge.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `pulsed_ccp_discharge.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pulsed_ccp_discharge.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pulsed_ccp_discharge_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.pulsed_ccp_discharge.pdf`: models.plasma.pulsed_ccp_discharge.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；independent variable, which must be discretized in space using a mesh. In order to resolve

## PDF Summary and Physics Judgement

- Simple summary: pulsed_ccp_discharge 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: independent variable, which must be discretized in space using a mesh. In order to resolve
- PDF: the RF modulation within the on period, at least 30 mesh elements are required per RF
- PDF: then off for another 100 cycles (7.8 us). During the on-cycle, we need to use 30 mesh
- 脚本/文本识别到的物理场: ptp / ColdPlasmaTimePeriodic
...
- PDF keywords: 传热, 流体, 结构力学, 电磁, models.plasma.pulsed_ccp_discharge.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；independent variable, which must be discretized in space using a mesh. In order to resolve
- `f0` -> Base frequency; MATLAB: `model.param.set('f0', value, description)`
- `As` -> Wafer surface area; MATLAB: `model.param.set('As', value, description)`
- `p0` -> Pressure; MATLAB: `model.param.set('p0', value, description)`
- `T0` -> Temperature; MATLAB: `model.param.set('T0', value, description)`
- `Prf` -> Single pulse power; MATLAB: `model.param.set('Prf', value, description)`
- `Non` -> Number of periods the pulse is on; MATLAB: `model.param.set('Non', value, description)`
- `Noff` -> Number of periods the pulse is off; MATLAB: `model.param.set('Noff', value, description)`
- `Nrampup` -> Number of cycles to ramp to on state; MATLAB: `model.param.set('Nrampup', value, description)`
- `Nrampdown` -> Number of cycles to ramp to off state; MATLAB: `model.param.set('Nrampdown', value, description)`
- `Nelon` -> Number of elements per period for the on period; MATLAB: `model.param.set('Nelon', value, description)`
- `Neloff` -> Number of elements for the off period; MATLAB: `model.param.set('Neloff', value, description)`
- `Ncycles` -> Total number of cycles; MATLAB: `model.param.set('Ncycles', value, description)`
...
- 已识别物理场证据：ptp / ColdPlasmaTimePeriodic
- 已识别参数：f0、As、p0、T0、Prf、Non、Noff、Nrampup、Nrampdown、Nelon、Neloff、Ncycles
...
- 识别到物理场证据：ptp / ColdPlasmaTimePeriodic
...
- Transfer recognized physics interfaces to similar requirements: ptp / ColdPlasmaTimePeriodic.
...
- Sweep `f0` around the learned value `13.56[MHz]` and export target outputs to CSV.
- Sweep `As` around the learned value `0.25*pi*(0.3[m])^2` and export target outputs to CSV.
- Sweep `p0` around the learned value `0.1[torr]` and export target outputs to CSV.
- Sweep `T0` around the learned value `300[K]` and export target outputs to CSV.
- Sweep `Prf` around the learned value `1[W]` and export target outputs to CSV.
- Sweep `Non` around the learned value `10` and export target outputs to CSV.
- Sweep `Noff` around the learned value `100` and export target outputs to CSV.
- Sweep `Nrampup` around the learned value `1` and export target outputs to CSV.
...
- Summary: 已学习 pulsed_ccp_discharge 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=13；内容证据=21 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 21 条；几何=2，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pulsed_ccp_discharge.m, pulsed_ccp_discharge.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pulsed_ccp_discharge，训练阶段为 modeling_logic_learning_ready。
...
- `pulsed_ccp_discharge.mph` (comsol_mph, 4313007 bytes)
- `pulsed_ccp_discharge.m` (matlab_livelink, 48957 bytes)
- `pulsed_ccp_discharge.java` (comsol_java, 51306 bytes)
- `pulsed_ccp_discharge_summary.json` (json, 1524 bytes)
- `models.plasma.pulsed_ccp_discharge.pdf` (pdf_document, 267510 bytes)
