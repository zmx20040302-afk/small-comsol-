...
- mesh: mesh1
- studies: Stationary
- boundary_conditions: wp1 / unite, sq1 / size, sq1 / base, sq1 / pos, sq1 / layerleft, sq1 / layerright, sq1 / layertop, sq2 / size, sq2 / base, r1 / size, r1 / base

### 文件内容证据

- `piezoresistive_pressure_sensor.java`: 参数=0, 几何=5, 物理场=0, 研究=0, 结果=0
...
- `piezoresistive_pressure_sensor.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoresistive_pressure_sensor.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoresistive_pressure_sensor_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: piezoresistive_pressure_sensor 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- Summary: 已学习 piezoresistive_pressure_sensor 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=20 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：piezoresistive_pressure_sensor.m, piezoresistive_pressure_sensor.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 piezoresistive_pressure_sensor，训练阶段为 modeling_logic_learning_ready。
...
- `piezoresistive_pressure_sensor.mph` (comsol_mph, 15044577 bytes)
- `piezoresistive_pressure_sensor.m` (matlab_livelink, 62002 bytes)
- `piezoresistive_pressure_sensor.java` (comsol_java, 61096 bytes)
- `piezoresistive_pressure_sensor_summary.json` (json, 1248 bytes)
