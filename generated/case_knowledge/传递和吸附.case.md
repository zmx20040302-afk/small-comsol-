...
- Created at: `2026-07-06T09:36:43.233420+00:00`
...
- `transport_and_adsorption.mph`: 参数=8, 几何=5, 物理场=2, 研究=2, 结果=0
...
- `models.mph.transport_and_adsorption.pdf`: models.mph.transport_and_adsorption.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.transport_and_adsorption.zh_CN.pdf`: models.mph.transport_and_adsorption.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 传递和吸附 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Transport of Diluted Species，并采用 Transient 研究。
- Primary physics: `Transport of Diluted Species`
- Study type: `Transient`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Transport of Diluted Species，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Transient 确认。

### Judgement Evidence

- 脚本/文本识别到的物理场: tds / DilutedSpecies, gb / GeneralFormBoundaryPDE
...
- PDF keywords: 流体, models.mph.transport_and_adsorption.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.transport_and_adsorption.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `c0` -> \u521d\u59cb\u6d53\u5ea6; MATLAB: `model.param.set('c0', value, description)`
- `k_ads` -> \u6b63\u53cd\u5e94\u901f\u7387\u5e38\u6570; MATLAB: `model.param.set('k_ads', value, description)`
- `k_des` -> \u9006\u53cd\u5e94\u901f\u7387\u5e38\u6570; MATLAB: `model.param.set('k_des', value, description)`
- `Gamma_s` -> \u6d3b\u6027\u4f4d\u6d53\u5ea6; MATLAB: `model.param.set('Gamma_s', value, description)`
- `Ds` -> \u8868\u9762\u6269\u6563\u7cfb\u6570; MATLAB: `model.param.set('Ds', value, description)`
- `D` -> \u6c14\u4f53\u6269\u6563\u7cfb\u6570; MATLAB: `model.param.set('D', value, description)`
- `v_max` -> \u6700\u5927\u901f\u5ea6; MATLAB: `model.param.set('v_max', value, description)`
- `delta` -> \u901a\u9053\u5bbd\u5ea6; MATLAB: `model.param.set('delta', value, description)`
...
- 已识别几何证据：r1 / Rectangle；pt1 / Point；pt2 / Point；geom1 / r1 / size；geom1 / r1 / pos
- 已识别物理场证据：tds / DilutedSpecies；gb / GeneralFormBoundaryPDE
- 已识别参数：c0、k_ads、k_des、Gamma_s、Ds、D、v_max、delta
...
- 识别到物理场证据：gb / GeneralFormBoundaryPDE
...
- Transfer recognized physics interfaces to similar requirements: tds / DilutedSpecies, gb / GeneralFormBoundaryPDE.
...
- Sweep `c0` around the learned value `1000[mol/m^3]` and export target outputs to CSV.
- Sweep `k_ads` around the learned value `1e-6[m^3/(mol*s)]` and export target outputs to CSV.
- Sweep `k_des` around the learned value `1e-9[1/s]` and export target outputs to CSV.
- Sweep `Gamma_s` around the learned value `1000[mol/m^2]` and export target outputs to CSV.
- Sweep `Ds` around the learned value `1e-11[m^2/s]` and export target outputs to CSV.
- Sweep `D` around the learned value `1e-9[m^2/s]` and export target outputs to CSV.
- Sweep `v_max` around the learned value `1[mm/s]` and export target outputs to CSV.
- Sweep `delta` around the learned value `0.1[mm]` and export target outputs to CSV.
