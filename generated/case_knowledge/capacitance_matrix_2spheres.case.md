# capacitance_matrix_2spheres

- Case directory: `D:\桌面\codex\案例下载\电气\两个球体的电容矩阵`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:47:39.988961+00:00`
...
- `a` = `0.1` - Radius of sphere 1
- `b` = `0.3` - Radius of sphere 2
- `c` = `0.5` - Distance of spheres
- `N` = `10`
- `PortName` = `1`
- `C11` = `4*pi*epsilon0_const*a*1[m]*1[1/pF]`
- `C22` = `4*pi*epsilon0_const*b*1[m]*1[1/pF]`

## 内容级读取结果

- 模型树证据数量: 29
- 理论关键词: 电磁, 优化/参数扫描
- geometry: sph1 / Sphere, sph2 / Sphere, sph3 / Sphere, pt1 / Point, geom1 / sph1 / r, geom1 / sph1 / pos, geom1 / sph2 / r, geom1 / sph2 / pos, geom1 / sph3 / r, geom1 / sph3 / layername, geom1 / pt1 / p
- physics: Electrostatics
- mesh: mesh1
- studies: Stationary, Parametric Sweep
- boundary_conditions: sph1 / r, sph1 / pos, sph2 / r, sph2 / pos, sph3 / r, sph3 / layername, pt1 / p

### 文件内容证据

- `capacitance_matrix_2spheres.java`: 参数=7, 几何=11, 物理场=0, 研究=0, 结果=0
- `Capacitance_Matrix_2Spheres.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `Capacitance_Matrix_2Spheres.pptx`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `capacitance_matrix_2spheres.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `capacitance_matrix_2spheres.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `capacitance_matrix_2spheres_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `Capacitance_Matrix_2Spheres.pdf`: Capacitance_Matrix_2Spheres.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为： Numerical results of COMSOL are；and c chosen in the model.

## PDF Summary and Physics Judgement

- Simple summary: capacitance_matrix_2spheres 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Optimization，并采用 Stationary 研究。
...
- PDF:  Numerical results of COMSOL are
- PDF: and c chosen in the model.
- PDF:  There are two options to realize a study:
- PDF: Numerical Settings and Mesh
...
- PDF keywords: 电磁, 优化/参数扫描, Capacitance_Matrix_2Spheres.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为： Numerical results of COMSOL are；and c chosen in the model.
- `a` -> Radius of sphere 1; MATLAB: `model.param.set('a', value, description)`
- `b` -> Radius of sphere 2; MATLAB: `model.param.set('b', value, description)`
- `c` -> Distance of spheres; MATLAB: `model.param.set('c', value, description)`
...
- `C11` -> 案例参数/扫描变量; MATLAB: `model.param.set('C11', value, description)`
- `C22` -> 案例参数/扫描变量; MATLAB: `model.param.set('C22', value, description)`
...
- 已识别几何证据：sph1 / Sphere；sph2 / Sphere；sph3 / Sphere；pt1 / Point；geom1 / sph1 / r；geom1 / sph1 / pos
- 已识别物理场证据：Electrostatics
- 已识别参数：a、b、c、N、PortName、C11、C22
...
- Sweep `a` around the learned value `0.1` and export target outputs to CSV.
- Sweep `b` around the learned value `0.3` and export target outputs to CSV.
- Sweep `c` around the learned value `0.5` and export target outputs to CSV.
...
- Sweep `C11` around the learned value `4*pi*epsilon0_const*a*1[m]*1[1/pF]` and export target outputs to CSV.
- Sweep `C22` around the learned value `4*pi*epsilon0_const*b*1[m]*1[1/pF]` and export target outputs to CSV.
...
- Summary: 已学习 capacitance_matrix_2spheres 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=29 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 29 条；几何=11，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：capacitance_matrix_2spheres.m, capacitance_matrix_2spheres.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 capacitance_matrix_2spheres，训练阶段为 modeling_logic_learning_ready。
...
- `capacitance_matrix_2spheres.mph` (comsol_mph, 30103160 bytes)
- `capacitance_matrix_2spheres.m` (matlab_livelink, 37288 bytes)
- `capacitance_matrix_2spheres.java` (comsol_java, 35555 bytes)
- `capacitance_matrix_2spheres_summary.json` (json, 1211 bytes)
- `Capacitance_Matrix_2Spheres.pptx` (presentation_document, 10706838 bytes)
- `Capacitance_Matrix_2Spheres.pdf` (pdf_document, 1578273 bytes)
