...
- Case directory: `D:\桌面\codex\案例下载\电气\高斯光束的二次谐波产生（波动光学）`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:37:22.575716+00:00`
...
- `w0` = `2[um]` - Minimum spot radius of laser beam
- `lda0` = `1.06[um]` - Wavelength of input laser beam
- `E0` = `30[kV/m]` - Peak electric field
- `x0` = `pi*w0^2/lda0` - Rayleigh range
- `k0` = `2*pi/lda0` - Propagation constant
- `omega0` = `k0*c_const` - Angular frequency
- `t0` = `25[fs]` - Pulse time delay
- `dt` = `10[fs]` - Pulse width
- `d33` = `1e-17[F/V]` - Matrix element for second harmonic generation
- `lda2` = `lda0/2` - Wavelength for second harmonic
- `hmax` = `lda2/6` - Maximum mesh element size
- `CFL` = `0.15` - CFL number
- `tstep` = `CFL*hmax/c_const` - Time step
...
- `second_harmonic_generation.java`: 参数=13, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.woptics.second_harmonic_generation.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `second_harmonic_generation.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `second_harmonic_generation.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `second_harmonic_generation_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.second_harmonic_generation.pdf`: models.woptics.second_harmonic_generation.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；simulation, but rather a 2D model. The model uses COMSOL Multiphysics’ standard 2D

## PDF Summary and Physics Judgement

- Simple summary: second_harmonic_generation 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: simulation, but rather a 2D model. The model uses COMSOL Multiphysics’ standard 2D
- PDF: geometry1 can be approximated by the following analytical solution of the paraxial wave
- PDF: requires a much longer propagation distance, resulting in a large computational problem.
...
- PDF keywords: 电磁, 结构力学, models.woptics.second_harmonic_generation.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；simulation, but rather a 2D model. The model uses COMSOL Multiphysics’ standard 2D
- `w0` -> Minimum spot radius of laser beam; MATLAB: `model.param.set('w0', value, description)`
- `lda0` -> Wavelength of input laser beam; MATLAB: `model.param.set('lda0', value, description)`
- `E0` -> Peak electric field; MATLAB: `model.param.set('E0', value, description)`
- `x0` -> Rayleigh range; MATLAB: `model.param.set('x0', value, description)`
- `k0` -> Propagation constant; MATLAB: `model.param.set('k0', value, description)`
- `omega0` -> Angular frequency; MATLAB: `model.param.set('omega0', value, description)`
- `t0` -> Pulse time delay; MATLAB: `model.param.set('t0', value, description)`
- `dt` -> Pulse width; MATLAB: `model.param.set('dt', value, description)`
- `d33` -> Matrix element for second harmonic generation; MATLAB: `model.param.set('d33', value, description)`
- `lda2` -> Wavelength for second harmonic; MATLAB: `model.param.set('lda2', value, description)`
- `hmax` -> Maximum mesh element size; MATLAB: `model.param.set('hmax', value, description)`
- `CFL` -> CFL number; MATLAB: `model.param.set('CFL', value, description)`
...
- 已识别参数：w0、lda0、E0、x0、k0、omega0、t0、dt、d33、lda2、hmax、CFL
...
- Sweep `w0` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `lda0` around the learned value `1.06[um]` and export target outputs to CSV.
- Sweep `E0` around the learned value `30[kV/m]` and export target outputs to CSV.
- Sweep `x0` around the learned value `pi*w0^2/lda0` and export target outputs to CSV.
- Sweep `k0` around the learned value `2*pi/lda0` and export target outputs to CSV.
- Sweep `omega0` around the learned value `k0*c_const` and export target outputs to CSV.
- Sweep `t0` around the learned value `25[fs]` and export target outputs to CSV.
- Sweep `dt` around the learned value `10[fs]` and export target outputs to CSV.
...
- Summary: 已学习 second_harmonic_generation 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=13；内容证据=21 条。
...
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：second_harmonic_generation.m, second_harmonic_generation.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 second_harmonic_generation，训练阶段为 modeling_logic_learning_ready。
...
- `second_harmonic_generation.mph` (comsol_mph, 3593778 bytes)
- `second_harmonic_generation.m` (matlab_livelink, 31227 bytes)
- `second_harmonic_generation.java` (comsol_java, 26983 bytes)
- `second_harmonic_generation_summary.json` (json, 1167 bytes)
- `models.woptics.second_harmonic_generation.pdf` (pdf_document, 548084 bytes)
