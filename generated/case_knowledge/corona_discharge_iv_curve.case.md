# corona_discharge_iv_curve

- Case directory: `D:\桌面\codex\案例下载\电气\线对线电晕放电的电流-电压特性`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:40:28.985239+00:00`
...
- `V0` = `10[kV]` - Applied voltage
- `d` = `10[cm]` - Out-of-plane thickness

## 内容级读取结果

- 模型树证据数量: 32
- 理论关键词: 电磁, 流体
- geometry: c1 / Circle, c2 / Circle, r1 / Rectangle, r2 / Rectangle, dif1 / Difference, geom1 / c1 / r, geom1 / c2 / r, geom1 / c2 / pos, geom1 / r1 / size, geom1 / r1 / base, geom1 / r1 / pos, geom1 / r2 / size
- physics: edis / ElectricDischarge, Electric Currents, Electrostatics
- materials: mat1
- mesh: mesh1
- studies: Stationary
- boundary_conditions: ece1 / 9, 10, 11, 12, c1 / r, c2 / r, c2 / pos, r1 / size, r1 / base, r1 / pos, r2 / size, r2 / base, r2 / pos

### 文件内容证据

- `corona_discharge_iv_curve.java`: 参数=2, 几何=14, 物理场=1, 研究=0, 结果=0
- `models.edis.corona_discharge_iv_curve.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `corona_discharge_iv_curve_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `corona_discharge_iv_curve.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `corona_discharge_iv_curve.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.corona_discharge_iv_curve.pdf`: models.edis.corona_discharge_iv_curve.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；that can be easily adapted to study other discharge configurations and types, facilitating a

## PDF Summary and Physics Judgement

- Simple summary: corona_discharge_iv_curve 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electric Currents，并采用 Stationary 研究。
...
- PDF: that can be easily adapted to study other discharge configurations and types, facilitating a
- PDF: wide range of applications in high-voltage engineering and plasma physics.
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: edis / ElectricDischarge, Electric Currents, Electrostatics
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 流体
...
- PDF keywords: 电磁, 流体, models.edis.corona_discharge_iv_curve.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；that can be easily adapted to study other discharge configurations and types, facilitating a
...
- 已识别几何证据：c1 / Circle；c2 / Circle；r1 / Rectangle；r2 / Rectangle；dif1 / Difference；geom1 / c1 / r
- 已识别物理场证据：edis / ElectricDischarge；Electric Currents；Electrostatics
- 已识别参数：V0、d
...
- Transfer recognized physics interfaces to similar requirements: edis / ElectricDischarge, Electric Currents, Electrostatics.
...
- Sweep `V0` around the learned value `10[kV]` and export target outputs to CSV.
- Sweep `d` around the learned value `10[cm]` and export target outputs to CSV.
...
- Summary: 已学习 corona_discharge_iv_curve 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=32 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 32 条；几何=14，物理场=3，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：corona_discharge_iv_curve.m, corona_discharge_iv_curve.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 corona_discharge_iv_curve，训练阶段为 modeling_logic_learning_ready。
...
- `corona_discharge_iv_curve.mph` (comsol_mph, 12660127 bytes)
- `corona_discharge_iv_curve.m` (matlab_livelink, 20338 bytes)
- `corona_discharge_iv_curve.java` (comsol_java, 17414 bytes)
- `corona_discharge_iv_curve_summary.json` (json, 924 bytes)
- `models.edis.corona_discharge_iv_curve.pdf` (pdf_document, 327848 bytes)
