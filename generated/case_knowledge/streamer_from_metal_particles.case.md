# streamer_from_metal_particles

- Case directory: `D:\桌面\codex\案例下载\电气\悬浮金属颗粒引发的流注放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:51:32.934504+00:00`
...
- `V0` = `50[kV]` - Applied voltage

## 内容级读取结果

- 模型树证据数量: 22
- 理论关键词: 电磁, 结构力学, 流体
- geometry: r1 / Rectangle, c1 / Circle, c2 / Circle, dif1 / Difference, geom1 / r1 / layername, geom1 / r1 / layerleft, geom1 / r1 / layerbottom, geom1 / c1 / r, geom1 / c1 / pos, geom1 / c2 / r, geom1 / c2 / pos
- materials: mat1
- mesh: mesh1
- studies: Time Dependent
- boundary_conditions: r1 / layername, r1 / layerleft, r1 / layerbottom, c1 / r, c1 / pos, c2 / r, c2 / pos

### 文件内容证据

- `streamer_from_metal_particles.java`: 参数=1, 几何=11, 物理场=0, 研究=0, 结果=0
- `models.edis.streamer_from_metal_particles.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `streamer_from_metal_particles.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `streamer_from_metal_particles.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `streamer_from_metal_particles_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.streamer_from_metal_particles.pdf`: models.edis.streamer_from_metal_particles.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；results provide a detailed framework for evaluating particle-induced breakdown

## PDF Summary and Physics Judgement

- Simple summary: streamer_from_metal_particles 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: results provide a detailed framework for evaluating particle-induced breakdown
...
- PDF keywords: 电磁, 结构力学, 流体, models.edis.streamer_from_metal_particles.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；results provide a detailed framework for evaluating particle-induced breakdown
...
- 已识别几何证据：r1 / Rectangle；c1 / Circle；c2 / Circle；dif1 / Difference；geom1 / r1 / layername；geom1 / r1 / layerleft
- 已识别参数：V0
...
- Summary: 已学习 streamer_from_metal_particles 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=1；内容证据=22 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 22 条；几何=11，物理场=0，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：streamer_from_metal_particles.m, streamer_from_metal_particles.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 streamer_from_metal_particles，训练阶段为 modeling_logic_learning_ready。
...
- `streamer_from_metal_particles.mph` (comsol_mph, 28061904 bytes)
- `streamer_from_metal_particles.m` (matlab_livelink, 39002 bytes)
- `streamer_from_metal_particles.java` (comsol_java, 36432 bytes)
- `streamer_from_metal_particles_summary.json` (json, 1016 bytes)
- `models.edis.streamer_from_metal_particles.pdf` (pdf_document, 1280942 bytes)
