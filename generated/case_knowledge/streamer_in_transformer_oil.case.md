# streamer_in_transformer_oil

- Case directory: `D:\桌面\codex\案例下载\电气\变压器油中的正流注传播`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:14:09.339829+00:00`
...
- 模型树证据数量: 32
- 理论关键词: 结构力学, 流体, 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, e1 / Ellipse, r3 / Rectangle, geom1 / r1 / size, geom1 / r1 / layername, geom1 / r1 / layerleft, geom1 / r1 / layerbottom, geom1 / r2 / size, geom1 / r2 / pos, geom1 / e1 / semiaxes, geom1 / e1 / pos
- mesh: mesh1
- studies: Time Dependent
- boundary_conditions: r1 / size, r1 / layername, r1 / layerleft, r1 / layerbottom, r2 / size, r2 / pos, e1 / semiaxes, e1 / pos, r3 / size, r3 / pos, r3 / layername, r3 / layerleft

### 文件内容证据

- `streamer_in_transformer_oil.java`: 参数=0, 几何=17, 物理场=0, 研究=0, 结果=0
- `models.edis.streamer_in_transformer_oil.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `streamer_in_transformer_oil.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `streamer_in_transformer_oil.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `streamer_in_transformer_oil_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.streamer_in_transformer_oil.pdf`: models.edis.streamer_in_transformer_oil.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion

## PDF Summary and Physics Judgement

- Simple summary: streamer_in_transformer_oil 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF keywords: 结构力学, 流体, 电磁, models.edis.streamer_in_transformer_oil.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；e1 / Ellipse；r3 / Rectangle；geom1 / r1 / size；geom1 / r1 / layername
...
- Summary: 已学习 streamer_in_transformer_oil 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=32 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 32 条；几何=17，物理场=0，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：streamer_in_transformer_oil.m, streamer_in_transformer_oil.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 streamer_in_transformer_oil，训练阶段为 modeling_logic_learning_ready。
...
- `streamer_in_transformer_oil.mph` (comsol_mph, 19344498 bytes)
- `streamer_in_transformer_oil.m` (matlab_livelink, 25575 bytes)
- `streamer_in_transformer_oil.java` (comsol_java, 23273 bytes)
- `streamer_in_transformer_oil_summary.json` (json, 887 bytes)
- `models.edis.streamer_in_transformer_oil.pdf` (pdf_document, 882891 bytes)
