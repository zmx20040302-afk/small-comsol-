...
- Case directory: `D:\桌面\codex\案例下载\电气\萨格纳克干涉仪`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:13:09.785580+00:00`
...
- `sagnac_interferometer.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.roptics.sagnac_interferometer.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `sagnac_interferometer.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `sagnac_interferometer.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `sagnac_interferometer_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.roptics.sagnac_interferometer.pdf`: models.roptics.sagnac_interferometer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics，研究类型线索为 Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；it can still be observed in a pure geometrical optics simulation when the frame of reference

## PDF Summary and Physics Judgement

- Simple summary: sagnac_interferometer 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Parametric Sweep 研究。
...
- PDF: it can still be observed in a pure geometrical optics simulation when the frame of reference
- PDF: splitter arranged in a triangle. The entire modeling domain rotates; as a result, the rays
- PDF: The model geometry is shown in Figure 1. The two mirrors and the beam splitter form an
...
- PDF keywords: 流体, 结构力学, 电磁, 优化/参数扫描, models.roptics.sagnac_interferometer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Ray Optics，研究类型线索为 Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；it can still be observed in a pure geometrical optics simulation when the frame of reference
- `Omega` -> Interferometer angular velocity; MATLAB: `model.param.set('Omega', value, description)`
- `lam0` -> Vacuum wavelength; MATLAB: `model.param.set('lam0', value, description)`
- `R` -> Ring radius; MATLAB: `model.param.set('R', value, description)`
- `th` -> Out-of-plane thickness; MATLAB: `model.param.set('th', value, description)`
- `A` -> Triangle area; MATLAB: `model.param.set('A', value, description)`
- `P` -> Triangle perimeter; MATLAB: `model.param.set('P', value, description)`
- `phi0` -> Initial ray angle; MATLAB: `model.param.set('phi0', value, description)`
- `xc` -> Center of rotation, x-coordinate; MATLAB: `model.param.set('xc', value, description)`
- `yc` -> Center of rotation, y-coordinate; MATLAB: `model.param.set('yc', value, description)`
- `wm` -> Mirror width; MATLAB: `model.param.set('wm', value, description)`
- `hm` -> Mirror height; MATLAB: `model.param.set('hm', value, description)`
- `xm1` -> Lower right mirror center, x-coordinate; MATLAB: `model.param.set('xm1', value, description)`
...
- 已识别物理场证据：Ray Optics
- 已识别参数：Omega、lam0、R、th、A、P、phi0、xc、yc、wm、hm、xm1
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 结构力学, 电磁, 优化/参数扫描?
...
- Sweep `Omega` around the learned value `10[deg/h]` and export target outputs to CSV.
- Sweep `lam0` around the learned value `632.8[nm]` and export target outputs to CSV.
- Sweep `R` around the learned value `10[cm]` and export target outputs to CSV.
- Sweep `th` around the learned value `3[cm]` and export target outputs to CSV.
- Sweep `A` around the learned value `0.5*(1.5*R)*(R*sqrt(3))` and export target outputs to CSV.
- Sweep `P` around the learned value `3*(R*sqrt(3))` and export target outputs to CSV.
- Sweep `phi0` around the learned value `30[deg]` and export target outputs to CSV.
- Sweep `xc` around the learned value `R` and export target outputs to CSV.
...
- Summary: 已学习 sagnac_interferometer 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=34；内容证据=33 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 33 条；几何=0，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：sagnac_interferometer.m, sagnac_interferometer.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 sagnac_interferometer，训练阶段为 modeling_logic_learning_ready。
...
- `sagnac_interferometer.mph` (comsol_mph, 30446062 bytes)
- `sagnac_interferometer.m` (matlab_livelink, 21070 bytes)
- `sagnac_interferometer.java` (comsol_java, 17475 bytes)
- `sagnac_interferometer_summary.json` (json, 2039 bytes)
- `models.roptics.sagnac_interferometer.pdf` (pdf_document, 626592 bytes)
