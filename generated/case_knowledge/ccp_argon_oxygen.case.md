...
- Case directory: `D:\桌面\codex\案例下载\电气\氩氧电容耦合等离子体反应器模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:37:24.136690+00:00`
...
- physics: ptp / ColdPlasmaTimePeriodic, Laminar Flow
- mesh: mesh1
- boundary_conditions: mct1 / 1, gnd1 / 2, in1 / 1, out1 / 2, i1 / coord

### 文件内容证据

- `ccp_argon_oxygen.java`: 参数=2, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.plasma.ccp_argon_oxygen.pdf`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `ccp_argon_oxygen.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ccp_argon_oxygen.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ccp_argon_oxygen_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.ccp_argon_oxygen.pdf`: models.plasma.ccp_argon_oxygen.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Laminar Flow；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；excited states can be lumped into a single species, which results in a chemical mechanism

## PDF Summary and Physics Judgement

- Simple summary: ccp_argon_oxygen 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow。
- Primary physics: `Laminar Flow`
- Study type: `unknown`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Laminar Flow，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型需要继续从研究步骤确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: excited states can be lumped into a single species, which results in a chemical mechanism
- PDF: in the model. If no surface reactions that leads to the lost of a given species at a surface are
- PDF: completely wrong results can be obtained if too much diffusion is used (the tuning
- 脚本/文本识别到的物理场: ptp / ColdPlasmaTimePeriodic, Laminar Flow
- 理论关键词: 传热, 结构力学, 流体, 电磁

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 传热, 结构力学, 流体, 电磁, models.plasma.ccp_argon_oxygen.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Laminar Flow；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；excited states can be lumped into a single species, which results in a chemical mechanism
- `xO2` -> 案例参数/扫描变量; MATLAB: `model.param.set('xO2', value, description)`
- `xAr` -> 案例参数/扫描变量; MATLAB: `model.param.set('xAr', value, description)`
...
- 已识别物理场证据：ptp / ColdPlasmaTimePeriodic；Laminar Flow
- 已识别参数：xO2、xAr
...
- Transfer recognized physics interfaces to similar requirements: ptp / ColdPlasmaTimePeriodic, Laminar Flow.
...
- Sweep `xAr` around the learned value `1-xO2` and export target outputs to CSV.
...
- Summary: 已学习 ccp_argon_oxygen 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=12 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 12 条；几何=2，物理场=2，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：ccp_argon_oxygen.m, ccp_argon_oxygen.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 ccp_argon_oxygen，训练阶段为 modeling_logic_learning_ready。
...
- `ccp_argon_oxygen.mph` (comsol_mph, 4630736 bytes)
- `ccp_argon_oxygen.m` (matlab_livelink, 78041 bytes)
- `ccp_argon_oxygen.java` (comsol_java, 76304 bytes)
- `ccp_argon_oxygen_summary.json` (json, 1227 bytes)
- `models.plasma.ccp_argon_oxygen.pdf` (pdf_document, 248087 bytes)
