...
- Case directory: `D:\桌面\codex\案例下载\电气\偶极微波等离子体源`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T03:58:35.974755+00:00`
...
- `r0` = `0.12` - Plasma source radius
- `z0` = `0.24` - Plasma source height
- `Pin` = `20[W]` - Input power

## 内容级读取结果

- 模型树证据数量: 40
- 理论关键词: 传热, 结构力学, 流体, 电磁, 多孔介质/裂隙
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, r4 / Rectangle, r5 / Rectangle, r6 / Rectangle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r2 / pos, geom1 / r3 / size, geom1 / r3 / pos
- physics: Magnetic Fields, Electromagnetic Waves
- mesh: mesh1, adaptmesh1, mesh2
- studies: Stationary, Frequency Domain
- boundary_conditions: r1 / size, r1 / pos, r2 / size, r2 / pos, r3 / size, r3 / pos, r4 / size, r4 / pos, r5 / size, r5 / pos, r6 / size, r6 / pos

### 文件内容证据

- `dipolar_ecr_source.java`: 参数=3, 几何=18, 物理场=0, 研究=0, 结果=0
- `models.plasma.dipolar_ecr_source.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `dipolar_ecr_source.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `dipolar_ecr_source.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `dipolar_ecr_source_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.dipolar_ecr_source.pdf`: models.plasma.dipolar_ecr_source.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Electromagnetic Waves，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Multiphysics unique, including:

## PDF Summary and Physics Judgement

- Simple summary: dipolar_ecr_source 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: Multiphysics unique, including:
- PDF: • Infinite elements for the magnetostatic model.
- PDF:  Functional-based mesh adaptation to create a fine mesh on the ECR surface.
...
- PDF keywords: 传热, 结构力学, 流体, 电磁, 多孔介质/裂隙, models.plasma.dipolar_ecr_source.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Electromagnetic Waves，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Multiphysics unique, including:
- `r0` -> Plasma source radius; MATLAB: `model.param.set('r0', value, description)`
- `z0` -> Plasma source height; MATLAB: `model.param.set('z0', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；r4 / Rectangle；r5 / Rectangle；r6 / Rectangle
...
- Sweep `r0` around the learned value `0.12` and export target outputs to CSV.
- Sweep `z0` around the learned value `0.24` and export target outputs to CSV.
- Sweep `Pin` around the learned value `20[W]` and export target outputs to CSV.
...
- Summary: 已学习 dipolar_ecr_source 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=40 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 40 条；几何=18，物理场=2，材料=0，网格=3，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：dipolar_ecr_source.m, dipolar_ecr_source.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 dipolar_ecr_source，训练阶段为 modeling_logic_learning_ready。
...
- `dipolar_ecr_source.mph` (comsol_mph, 63685493 bytes)
- `dipolar_ecr_source.m` (matlab_livelink, 73781 bytes)
- `dipolar_ecr_source.java` (comsol_java, 76957 bytes)
- `dipolar_ecr_source_summary.json` (json, 1517 bytes)
- `models.plasma.dipolar_ecr_source.pdf` (pdf_document, 2380710 bytes)
