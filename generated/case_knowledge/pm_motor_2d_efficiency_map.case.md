...
- Case directory: `D:\桌面\codex\案例下载\电气\永磁电机效率图`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:43:48.336153+00:00`
...
- `L` = `50[mm]` - Out-of-plane thickness of motor
- `init_ang` = `198[deg]` - Initial current angle
- `w_rot` = `12000[rpm]` - Shaft speed
- `f_el` = `w_rot*(Np/2)` - Electrical frequency
- `Ipk` = `30[A]` - Phase current peak value
- `Nturn` = `10` - Number of turns per coil
- `ff_slot` = `0.8` - Slot filling factor
- `lam_rho` = `7500[kg/m^3]` - Laminated core density
- `Nframes` = `120` - Number of time frames
- `Cu_rho0` = `1.667e-8[ohm*m]` - Copper reference resistivity
- `Cu_alpha` = `3.862e-3[1/K]` - Copper resistivity temperature coefficient
- `Cu_Tref` = `293.15[K]` - Copper reference temperature
- `Nsec` = `gcd(Np,Ns)` - Number of modeled sectors (1 = full circle)
- `PM_Br_ref` = `1.31[T]` - Magnet reference remanence flux density
- `PM_alpha` = `-0.12[%/K]` - Magnet remanence temperature coefficient
- `PM_Tref` = `20[degC]` - Magnet reference temperature
...
- `pm_motor_2d_efficiency_map.java`: 参数=18, 几何=1, 物理场=0, 研究=0, 结果=0
- `models.acdc.pm_motor_2d_efficiency_map.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `pm_motor_2d_efficiency_map.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pm_motor_2d_efficiency_map.m`: 参数=1, 几何=0, 物理场=0, 研究=0, 结果=0
- `pm_motor_2d_efficiency_map_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.pm_motor_2d_efficiency_map.pdf`: models.acdc.pm_motor_2d_efficiency_map.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；focus of this tutorial is on the multiphysics coupling and so the electromagnetic and

## PDF Summary and Physics Judgement

- Simple summary: pm_motor_2d_efficiency_map 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: focus of this tutorial is on the multiphysics coupling and so the electromagnetic and
- PDF: interface with the Electromagnetic Heating multiphysics coupling. In return the
- PDF: The motor geometry is reduced to the smallest sector which can represent the spatial
...
- PDF keywords: 传热, 电磁, 流体, 优化/参数扫描, models.acdc.pm_motor_2d_efficiency_map.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；focus of this tutorial is on the multiphysics coupling and so the electromagnetic and
...
- `init_ang` -> Initial current angle; MATLAB: `model.param.set('init_ang', value, description)`
- `w_rot` -> Shaft speed; MATLAB: `model.param.set('w_rot', value, description)`
- `f_el` -> Electrical frequency; MATLAB: `model.param.set('f_el', value, description)`
- `Ipk` -> Phase current peak value; MATLAB: `model.param.set('Ipk', value, description)`
- `Nturn` -> Number of turns per coil; MATLAB: `model.param.set('Nturn', value, description)`
- `ff_slot` -> Slot filling factor; MATLAB: `model.param.set('ff_slot', value, description)`
- `lam_rho` -> Laminated core density; MATLAB: `model.param.set('lam_rho', value, description)`
- `Nframes` -> Number of time frames; MATLAB: `model.param.set('Nframes', value, description)`
- `Cu_rho0` -> Copper reference resistivity; MATLAB: `model.param.set('Cu_rho0', value, description)`
...
- 已识别参数：Np、Ns、L、init_ang、w_rot、f_el、Ipk、Nturn、ff_slot、lam_rho、Nframes、Cu_rho0
...
- Sweep `L` around the learned value `50[mm]` and export target outputs to CSV.
- Sweep `init_ang` around the learned value `198[deg]` and export target outputs to CSV.
- Sweep `w_rot` around the learned value `12000[rpm]` and export target outputs to CSV.
- Sweep `f_el` around the learned value `w_rot*(Np/2)` and export target outputs to CSV.
- Sweep `Ipk` around the learned value `30[A]` and export target outputs to CSV.
- Sweep `Nturn` around the learned value `10` and export target outputs to CSV.
...
- Summary: 已学习 pm_motor_2d_efficiency_map 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=18；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=1，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pm_motor_2d_efficiency_map.m, pm_motor_2d_efficiency_map.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pm_motor_2d_efficiency_map，训练阶段为 modeling_logic_learning_ready。
...
- `pm_motor_2d_efficiency_map.mph` (comsol_mph, 2301169269 bytes)
- `pm_motor_2d_efficiency_map.m` (matlab_livelink, 110657 bytes)
- `pm_motor_2d_efficiency_map.java` (comsol_java, 116661 bytes)
- `pm_motor_2d_efficiency_map_summary.json` (json, 9330 bytes)
- `models.acdc.pm_motor_2d_efficiency_map.pdf` (pdf_document, 1439911 bytes)
