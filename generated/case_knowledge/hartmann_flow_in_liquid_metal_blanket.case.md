# hartmann_flow_in_liquid_metal_blanket

- Case directory: `D:\桌面\codex\案例下载\电气\液态金属包层中耦合传热的哈特曼流`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:37:58.392365+00:00`
...
- `d` = `1[cm]` - Half-distance between planes
- `U0` = `0.05[m/s]` - Average inlet velocity
- `B0` = `0.02[T]` - Magnetic flux density
...
- physics: mef / ElectricInductionCurrents, spf / LaminarFlow, ht / HeatTransferInFluids, Heat Transfer, Laminar Flow, Magnetic Fields
- materials: mat1
- mesh: mesh1
- studies: Stationary
- boundary_conditions: inl1 / 1, out1 / 6, temp1 / 1, 2, blk1 / size, blk1 / pos

### 文件内容证据

- `hartmann_flow_in_liquid_metal_blanket.java`: 参数=3, 几何=3, 物理场=3, 研究=0, 结果=0
- `models.acdc.hartmann_flow_in_liquid_metal_blanket.pdf`: 参数=0, 几何=0, 物理场=3, 研究=1, 结果=0
- `hartmann_flow_in_liquid_metal_blanket_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `hartmann_flow_in_liquid_metal_blanket.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `hartmann_flow_in_liquid_metal_blanket.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.hartmann_flow_in_liquid_metal_blanket.pdf`: models.acdc.hartmann_flow_in_liquid_metal_blanket.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Magnetic Fields，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The model is set up in 3D using the Magnetic and Electric Fields and Laminar Flow physics

## PDF Summary and Physics Judgement

- Simple summary: hartmann_flow_in_liquid_metal_blanket 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- PDF: The model is set up in 3D using the Magnetic and Electric Fields and Laminar Flow physics
- PDF: interfaces coupled via a Magnetohydrodynamics multiphysics node, as well as with the Heat
- PDF: Transfer in Fluids interface coupled to Laminar Flow via a Nonisothermal Flow multiphysics
- 脚本/文本识别到的物理场: mef / ElectricInductionCurrents, spf / LaminarFlow, ht / HeatTransferInFluids, Heat Transfer, Laminar Flow, Magnetic Fields
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 流体, 电磁
...
- PDF keywords: 传热, 流体, 电磁, models.acdc.hartmann_flow_in_liquid_metal_blanket.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Magnetic Fields，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The model is set up in 3D using the Magnetic and Electric Fields and Laminar Flow physics
- `d` -> Half-distance between planes; MATLAB: `model.param.set('d', value, description)`
...
- `HeatTransfer` -> 传热物理场; MATLAB: `model.component('comp1').physics.create('ht','HeatTransfer','geom1')`
- `LaminarFlow` -> 层流流体物理场; MATLAB: `model.component('comp1').physics.create('spf','LaminarFlow','geom1')`
...
- 已识别几何证据：blk1 / Block；geom1 / blk1 / size；geom1 / blk1 / pos
- 已识别物理场证据：mef / ElectricInductionCurrents；spf / LaminarFlow；ht / HeatTransferInFluids；Heat Transfer；Laminar Flow；Magnetic Fields
- 已识别参数：d、U0、B0
...
- 识别到物理场证据：mef / ElectricInductionCurrents
- 识别到物理场证据：spf / LaminarFlow
...
- Transfer recognized physics interfaces to similar requirements: mef / ElectricInductionCurrents, spf / LaminarFlow, ht / HeatTransferInFluids, Heat Transfer, Laminar Flow, Magnetic Fields.
...
- Sweep `B0` around the learned value `0.02[T]` and export target outputs to CSV.
...
- Summary: 已学习 hartmann_flow_in_liquid_metal_blanket 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=20 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=3，物理场=6，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：hartmann_flow_in_liquid_metal_blanket.m, hartmann_flow_in_liquid_metal_blanket.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 hartmann_flow_in_liquid_metal_blanket，训练阶段为 modeling_logic_learning_ready。
...
- `hartmann_flow_in_liquid_metal_blanket.mph` (comsol_mph, 19345746 bytes)
- `hartmann_flow_in_liquid_metal_blanket.m` (matlab_livelink, 40442 bytes)
- `hartmann_flow_in_liquid_metal_blanket.java` (comsol_java, 36662 bytes)
- `hartmann_flow_in_liquid_metal_blanket_summary.json` (json, 2076 bytes)
- `models.acdc.hartmann_flow_in_liquid_metal_blanket.pdf` (pdf_document, 1549765 bytes)
