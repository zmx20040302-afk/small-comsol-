...
- mesh: mesh1
- boundary_conditions: r1 / size, r2 / size, r2 / pos, r3 / size, r3 / pos, pol1 / source, pol1 / table, qb1 / p, qb1 / w, pol2 / source, pol2 / table, qb2 / p

### 文件内容证据

- `high_voltage_insulator_geom_sequence.m`: 参数=0, 几何=9, 物理场=0, 研究=0, 结果=0
- `high_voltage_insulator_geom_sequence.java`: 参数=0, 几何=9, 物理场=0, 研究=0, 结果=0
- `models.acdc.high_voltage_insulator.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `high_voltage_insulator_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `high_voltage_insulator_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: high_voltage_insulator_geom_sequence 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics。
...
- Summary: 已学习 high_voltage_insulator_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=9，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：high_voltage_insulator_geom_sequence.m, high_voltage_insulator_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 high_voltage_insulator_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `high_voltage_insulator_geom_sequence.mph` (comsol_mph, 351484 bytes)
- `high_voltage_insulator_geom_sequence.m` (matlab_livelink, 9831 bytes)
- `high_voltage_insulator_geom_sequence.java` (comsol_java, 10225 bytes)
- `high_voltage_insulator_geom_sequence_summary.json` (json, 653 bytes)
