...
- boundary_conditions: blk1 / size, blk1 / pos, blk1 / layername, blk1 / layer, blk1 / layerfront, blk1 / layerback, blk1 / layerbottom, wp1 / quickplane, wp1 / quicky, wp1 / unite, r1 / size, r1 / pos

### 文件内容证据

- `biased_resonator_3d_geom_sequence.java`: 参数=0, 几何=16, 物理场=0, 研究=0, 结果=0
- `biased_resonator_3d_geom_sequence.m`: 参数=0, 几何=14, 物理场=0, 研究=0, 结果=0
...
- `biased_resonator_3d_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=3, 结果=0
- `biased_resonator_3d_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: biased_resonator_3d_geom_sequence 主要研究结构受力、变形或强度响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenfrequency 研究。
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 声学, 结构力学, 电磁, 优化/参数扫描?
...
- Summary: 已学习 biased_resonator_3d_geom_sequence 的 8 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=40 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 40 条；几何=16，物理场=2，材料=0，网格=1，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：biased_resonator_3d_geom_sequence.m, biased_resonator_3d_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 biased_resonator_3d_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `biased_resonator_3d_geom_sequence.mph` (comsol_mph, 1038782 bytes)
- `biased_resonator_3d_geom_sequence.m` (matlab_livelink, 19484 bytes)
- `biased_resonator_3d_geom_sequence.java` (comsol_java, 20020 bytes)
- `biased_resonator_3d_geom_sequence_summary.json` (json, 701 bytes)
