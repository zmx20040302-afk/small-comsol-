...
- Created at: `2026-08-16T09:52:41.520331+00:00`
...
- `d_eff` = `0.345[nm]` - Effective graphene thickness
- `f` = `30[THz]` - Frequency
...
- `graphene_spp.m`: 参数=5, 几何=4, 物理场=0, 研究=0, 结果=0
- `graphene_spp.java`: 参数=5, 几何=4, 物理场=0, 研究=0, 结果=0
- `graphene_spp.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `graphene_spp_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: graphene_spp 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- Sweep `d_eff` around the learned value `0.345[nm]` and export target outputs to CSV.
- Sweep `f` around the learned value `30[THz]` and export target outputs to CSV.
...
- Summary: 已学习 graphene_spp 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=15 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 15 条；几何=4，物理场=0，材料=2，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：graphene_spp.m, graphene_spp.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 graphene_spp，训练阶段为 modeling_logic_learning_ready。
...
- `graphene_spp.mph` (comsol_mph, 1644509 bytes)
- `graphene_spp.m` (matlab_livelink, 47797 bytes)
- `graphene_spp.java` (comsol_java, 53803 bytes)
- `graphene_spp_summary.json` (json, 1073 bytes)
