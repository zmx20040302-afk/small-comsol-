...
- Created at: `2026-08-20T09:30:24.517270+00:00`
...
- 模型树证据数量: 38
- 理论关键词: 电磁, 结构力学, 流体
- geometry: r1 / Rectangle, arr1 / Array, r2 / Rectangle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r1 / layername, geom1 / r1 / layer, geom1 / r1 / layerleft, geom1 / r1 / layerbottom, geom1 / arr1 / fullsize, geom1 / arr1 / displ, geom1 / r2 / size
- physics: Solid Mechanics, Electrostatics
- mesh: mesh1
- studies: Stationary
- boundary_conditions: r1 / size, r1 / pos, r1 / layername, r1 / layer, r1 / layerleft, r1 / layerbottom, arr1 / fullsize, arr1 / displ, r2 / size, r2 / pos

### 文件内容证据

- `piezoelectric_valve.java`: 参数=11, 几何=13, 物理场=0, 研究=0, 结果=0
- `models.mems.piezoelectric_valve.pdf`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `piezoelectric_valve.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoelectric_valve.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `piezoelectric_valve_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: piezoelectric_valve 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- 已识别几何证据：r1 / Rectangle；arr1 / Array；r2 / Rectangle；geom1 / r1 / size；geom1 / r1 / pos；geom1 / r1 / layername
- 已识别物理场证据：Solid Mechanics；Electrostatics
- 已识别参数：t0、ID、OD、n、ts、w0、w1、w2、h0、deltaz、V0
...
- Summary: 已学习 piezoelectric_valve 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=38 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 38 条；几何=13，物理场=2，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：piezoelectric_valve.m, piezoelectric_valve.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 piezoelectric_valve，训练阶段为 modeling_logic_learning_ready。
...
- `piezoelectric_valve.mph` (comsol_mph, 16288904 bytes)
- `piezoelectric_valve.m` (matlab_livelink, 50461 bytes)
- `piezoelectric_valve.java` (comsol_java, 47754 bytes)
- `piezoelectric_valve_summary.json` (json, 1530 bytes)
