...
- Case directory: `D:\桌面\codex\案例下载\电气\MOSFET 击穿`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:29:39.809102+00:00`
...
- `Vg` = `3[V]` - Gate voltage
- `Vbreak` = `11.4[V]` - Breakdown voltage
...
- `mosfet_breakdown.java`: 参数=3, 几何=6, 物理场=0, 研究=0, 结果=0
- `models.semicond.mosfet_breakdown.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `mosfet_breakdown.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mosfet_breakdown.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mosfet_breakdown_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.mosfet_breakdown.pdf`: models.semicond.mosfet_breakdown.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；MOSFET as a result of impact ionization. MOSFETs typically operate in three regimes

## PDF Summary and Physics Judgement

- Simple summary: mosfet_breakdown 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF: MOSFET as a result of impact ionization. MOSFETs typically operate in three regimes
- PDF: The geometry and operation of the device are discussed in the DC Characteristics of a
- PDF: MOS Transistor (MOSFET) model.
...
- PDF keywords: 电磁, models.semicond.mosfet_breakdown.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；MOSFET as a result of impact ionization. MOSFETs typically operate in three regimes
- `Vd` -> Drain voltage; MATLAB: `model.param.set('Vd', value, description)`
- `Vg` -> Gate voltage; MATLAB: `model.param.set('Vg', value, description)`
- `Vbreak` -> Breakdown voltage; MATLAB: `model.param.set('Vbreak', value, description)`
...
- Sweep `Vg` around the learned value `3[V]` and export target outputs to CSV.
- Sweep `Vbreak` around the learned value `11.4[V]` and export target outputs to CSV.
...
- Summary: 已学习 mosfet_breakdown 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=15 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 15 条；几何=6，物理场=0，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mosfet_breakdown.m, mosfet_breakdown.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mosfet_breakdown，训练阶段为 modeling_logic_learning_ready。
...
- `mosfet_breakdown.mph` (comsol_mph, 26923611 bytes)
- `mosfet_breakdown.m` (matlab_livelink, 57885 bytes)
- `mosfet_breakdown.java` (comsol_java, 53817 bytes)
- `mosfet_breakdown_summary.json` (json, 1318 bytes)
- `models.semicond.mosfet_breakdown.pdf` (pdf_document, 379256 bytes)
