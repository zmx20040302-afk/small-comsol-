# induction_heating_curie_movement

- Case directory: `D:\桌面\codex\案例下载\电气\包含居里温度和运动的感应加热`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T06:49:02.034085+00:00`
...
- `del_sat` = `1/sqrt(pi*mu0_const*sig*f)` - Skin depth
- `sig` = `1e7[S/m]` - Conductivity for skin depth calculation
- `f` = `1[kHz]` - AC frequency
- `vel` = `3[mm/s]` - Prescribed velocity
- `t` = `0` - Time (required for postprocessing of stationary study)
- `Tc` = `900[degC]` - Curie point
- `I0` = `25[kA]` - AC amplitude

## 内容级读取结果

- 模型树证据数量: 13
- 理论关键词: 流体, 传热, 结构力学, 电磁
- physics: Heat Transfer, Magnetic Fields
- mesh: mesh1
- studies: Stationary, Time Dependent, Frequency Domain

### 文件内容证据

- `induction_heating_curie_movement.java`: 参数=7, 几何=0, 物理场=0, 研究=1, 结果=0
- `models.acdc.induction_heating_curie_movement.pdf`: 参数=0, 几何=0, 物理场=2, 研究=3, 结果=0
- `induction_heating_curie_movement.mph`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `induction_heating_curie_movement_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `induction_heating_curie_movement.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.induction_heating_curie_movement.pdf`: models.acdc.induction_heating_curie_movement.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The problem is set up in a 3D modeling space and the geometry sequence is imported

## PDF Summary and Physics Judgement

- Simple summary: induction_heating_curie_movement 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: The problem is set up in a 3D modeling space and the geometry sequence is imported
- PDF: The geometry in this model uses an assembly, rather than a union. This is to accommodate
- PDF: assembly consists of two parts with disjoint meshes. A Moving Mesh is used to apply the
- 脚本/文本识别到的物理场: Heat Transfer, Magnetic Fields
...
- PDF keywords: 流体, 传热, 结构力学, 电磁, models.acdc.induction_heating_curie_movement.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The problem is set up in a 3D modeling space and the geometry sequence is imported
- `del_sat` -> Skin depth; MATLAB: `model.param.set('del_sat', value, description)`
- `sig` -> Conductivity for skin depth calculation; MATLAB: `model.param.set('sig', value, description)`
- `f` -> AC frequency; MATLAB: `model.param.set('f', value, description)`
- `vel` -> Prescribed velocity; MATLAB: `model.param.set('vel', value, description)`
- `t` -> Time (required for postprocessing of stationary study); MATLAB: `model.param.set('t', value, description)`
- `Tc` -> Curie point; MATLAB: `model.param.set('Tc', value, description)`
- `I0` -> AC amplitude; MATLAB: `model.param.set('I0', value, description)`
...
- 已识别物理场证据：Heat Transfer；Magnetic Fields
- 已识别参数：del_sat、sig、f、vel、t、Tc、I0
...
- Transfer recognized physics interfaces to similar requirements: Heat Transfer, Magnetic Fields.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 传热, 结构力学, 电磁?
...
- Sweep `del_sat` around the learned value `1/sqrt(pi*mu0_const*sig*f)` and export target outputs to CSV.
- Sweep `sig` around the learned value `1e7[S/m]` and export target outputs to CSV.
- Sweep `f` around the learned value `1[kHz]` and export target outputs to CSV.
- Sweep `vel` around the learned value `3[mm/s]` and export target outputs to CSV.
- Sweep `t` around the learned value `0` and export target outputs to CSV.
- Sweep `Tc` around the learned value `900[degC]` and export target outputs to CSV.
- Sweep `I0` around the learned value `25[kA]` and export target outputs to CSV.
...
- Summary: 已学习 induction_heating_curie_movement 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=13 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 13 条；几何=0，物理场=2，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：induction_heating_curie_movement.m, induction_heating_curie_movement.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 induction_heating_curie_movement，训练阶段为 modeling_logic_learning_ready。
...
- `induction_heating_curie_movement.mph` (comsol_mph, 203422913 bytes)
- `induction_heating_curie_movement.m` (matlab_livelink, 102193 bytes)
- `induction_heating_curie_movement.java` (comsol_java, 103995 bytes)
- `induction_heating_curie_movement_summary.json` (json, 2634 bytes)
- `models.acdc.induction_heating_curie_movement.pdf` (pdf_document, 2864265 bytes)
