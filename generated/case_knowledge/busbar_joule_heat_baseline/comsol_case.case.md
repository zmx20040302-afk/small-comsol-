...
- Created at: `2026-07-29T08:49:46.250357+00:00`
...
- `user_prompt_as_pdf_summary`: 用户提示词替代 PDF 摘要：?????????????????-??????????-??????????CSV ???????????????

## PDF Summary and Physics Judgement

- Simple summary: ?????????? 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- Summary: 已学习 ?????????? 的 15 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=27 条。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 ??????????，训练阶段为 modeling_logic_learning_ready。
...
- `管式反应器代理模型 App` score=0.1803; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `热执行器代理模型 App` score=0.1472; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42
- `集群设置验证` score=0.1432; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=1

### Official Documentation Checks

- `IntroductionToLiveLinkForMATLAB.pdf` page 12; module=LiveLink_for_MATLAB; score=0.1251
- `IntroductionToCOMSOLMultiphysics.pdf` page 170; module=COMSOL_Multiphysics; score=0.1141
