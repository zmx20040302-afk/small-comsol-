...
- 模型树证据数量: 19
- 理论关键词: 未自动识别
- physics: ge / GlobalEquations
- studies: std1 / time / Transient, std1
- boundary_conditions: ge1 / name, ge1 / initialValueUt, ge1 / description, ge1 / equation, ge1 / initialValueU
...
- Summary: 已学习 用 Hodgkin-Huxley 模型模拟动作电位 的 3 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=19 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 19 条；几何=0，物理场=1，材料=0，网格=0，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：Hodgkin_Huxley_simulation_app.java, Hodgkin_Huxley_simulation_app.m
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 用 Hodgkin-Huxley 模型模拟动作电位，训练阶段为 modeling_logic_learning_ready。
...
- `用 Hodgkin-Huxley 模型模拟动作电位` score=0.2528; fields=physics, geometry, materials, mesh, solver, results; parameters=21
- `管式反应器代理模型 App` score=0.1607; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `热执行器代理模型 App` score=0.1497; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42

### Official Documentation Checks

- `IntroductionToLiveLinkForMATLAB.pdf` page 5; module=LiveLink_for_MATLAB; score=0.1102
- `IntroductionToCOMSOLMultiphysics.pdf` page 262; module=COMSOL_Multiphysics; score=0.1069
- `IntroductionToLiveLinkForMATLAB.pdf` page 3; module=LiveLink_for_MATLAB; score=0.1037
...
- `Hodgkin_Huxley_simulation_app.java` (comsol_java, 16908 bytes)
- `Hodgkin_Huxley_simulation_app.m` (matlab_livelink, 15745 bytes)
- `Hodgkin_Huxley_simulation_app.mph` (comsol_mph, 5841586 bytes)
