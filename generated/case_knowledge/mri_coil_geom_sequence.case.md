# mri_coil_geom_sequence

- Case directory: `D:\桌面\codex\案例下载\电气\MRI 笼式线圈`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:01:41.890181+00:00`
...
- `f0` = `63.87[MHz]` - Larmor frequency
- `lda0` = `c_const/f0` - Wavelength corresponding to Larmor frequency
- `c_value` = `28.5[pF]` - Capacitance used on the rungs
- `r_coil` = `0.24[m]` - Radius of the coil
- `h_coil` = `0.3[m]` - Height of the coil
- `l_element` = `0.01[m]` - Length of the capacitive elements
- `t_ring` = `0.015[m]`
- `l_element_1` = `0.01[m]`
- `h_coil_1` = `0.3[m]`
...
- 模型树证据数量: 40
- 理论关键词: 电磁, 优化/参数扫描, 结构力学, 多孔介质/裂隙
- geometry: cyl1 / Cylinder, wp1 / WorkPlane, c1 / Circle, ccur1 / ConvertToCurve, del1 / Delete, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / pos, geom1 / cyl1 / layername, geom1 / cyl1 / layerside, geom1 / cyl1 / layerbottom, geom1 / cyl1 / layertop
- physics: Electromagnetic Waves
- mesh: mesh1
- studies: Frequency Domain, Parametric Sweep
- boundary_conditions: cyl1 / r, cyl1 / h, cyl1 / pos, cyl1 / layername, cyl1 / layerside, cyl1 / layerbottom, cyl1 / layertop, wp1 / quickz, wp1 / unite, c1 / r, c1 / angle, c1 / rot

### 文件内容证据

- `mri_coil_geom_sequence.java`: 参数=9, 几何=15, 物理场=0, 研究=0, 结果=0
- `mri_coil_geom_sequence.m`: 参数=9, 几何=14, 物理场=0, 研究=0, 结果=0
- `models.rf.mri_coil.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `mri_coil.pptx`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `mri_coil_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mri_coil_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.rf.mri_coil.pdf`: models.rf.mri_coil.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Figure 1 shows the geometry of the example, which consists of a bird cage coil placed

## PDF Summary and Physics Judgement

- Simple summary: mri_coil_geom_sequence 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Frequency Domain 研究。
...
- PDF: Figure 1 shows the geometry of the example, which consists of a bird cage coil placed
- PDF: The automatic mesh control option in the Electromagnetic Waves, Frequency Domain
- PDF: interface is used with the maximum mesh element size as 1/6 of free space wavelength.
...
- PDF keywords: 电磁, 优化/参数扫描, 结构力学, 多孔介质/裂隙, models.rf.mri_coil.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Figure 1 shows the geometry of the example, which consists of a bird cage coil placed
- `f0` -> Larmor frequency; MATLAB: `model.param.set('f0', value, description)`
- `lda0` -> Wavelength corresponding to Larmor frequency; MATLAB: `model.param.set('lda0', value, description)`
- `c_value` -> Capacitance used on the rungs; MATLAB: `model.param.set('c_value', value, description)`
- `r_coil` -> Radius of the coil; MATLAB: `model.param.set('r_coil', value, description)`
- `h_coil` -> Height of the coil; MATLAB: `model.param.set('h_coil', value, description)`
- `l_element` -> Length of the capacitive elements; MATLAB: `model.param.set('l_element', value, description)`
- `t_ring` -> 案例参数/扫描变量; MATLAB: `model.param.set('t_ring', value, description)`
- `l_element_1` -> 案例参数/扫描变量; MATLAB: `model.param.set('l_element_1', value, description)`
- `h_coil_1` -> 案例参数/扫描变量; MATLAB: `model.param.set('h_coil_1', value, description)`
...
- `Extrude` -> 二维截面拉伸成三维结构; MATLAB: `model.component('comp1').geom('geom1').create(tag, 'Extrude')`
...
- 已识别几何证据：cyl1 / Cylinder；wp1 / WorkPlane；c1 / Circle；ccur1 / ConvertToCurve；del1 / Delete；geom1 / cyl1 / r
- 已识别物理场证据：Electromagnetic Waves
- 已识别参数：f0、lda0、c_value、r_coil、h_coil、l_element、t_ring、l_element_1、h_coil_1
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 优化/参数扫描, 结构力学, 多孔介质/裂隙?
...
- Sweep `f0` around the learned value `63.87[MHz]` and export target outputs to CSV.
- Sweep `lda0` around the learned value `c_const/f0` and export target outputs to CSV.
- Sweep `c_value` around the learned value `28.5[pF]` and export target outputs to CSV.
- Sweep `r_coil` around the learned value `0.24[m]` and export target outputs to CSV.
- Sweep `h_coil` around the learned value `0.3[m]` and export target outputs to CSV.
- Sweep `l_element` around the learned value `0.01[m]` and export target outputs to CSV.
- Sweep `t_ring` around the learned value `0.015[m]` and export target outputs to CSV.
- Sweep `l_element_1` around the learned value `0.01[m]` and export target outputs to CSV.
...
- Summary: 已学习 mri_coil_geom_sequence 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=40 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 40 条；几何=15，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mri_coil_geom_sequence.m, mri_coil_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mri_coil_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `mri_coil_geom_sequence.mph` (comsol_mph, 623458 bytes)
- `mri_coil_geom_sequence.m` (matlab_livelink, 8982 bytes)
- `mri_coil_geom_sequence.java` (comsol_java, 9230 bytes)
- `mri_coil_geom_sequence_summary.json` (json, 864 bytes)
- `mri_coil.pptx` (presentation_document, 3347797 bytes)
- `models.rf.mri_coil.pdf` (pdf_document, 1807751 bytes)
