# mutual_inductance_multiturn

- Case directory: `D:\桌面\codex\案例下载\电气\单导线和均匀螺旋线圈的自感和互感`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:49:40.019469+00:00`
...
- `N` = `20` - Number of turns in secondary
- `M` = `N*(mu0_const*pi*R2^2)/(2*R1)` - Analytic mutual inductance in secondary
...
- 模型树证据数量: 35
- 理论关键词: 电磁, 多孔介质/裂隙
- geometry: c1 / Circle, c2 / Circle, r1 / Rectangle, fil1 / Fillet, geom1 / c1 / r, geom1 / c1 / angle, geom1 / c1 / rot, geom1 / c1 / layername, geom1 / c2 / r, geom1 / c2 / pos, geom1 / r1 / size, geom1 / r1 / base
- physics: Magnetic Fields
- mesh: mesh1
- studies: Stationary, Frequency Domain
- boundary_conditions: c1 / r, c1 / angle, c1 / rot, c1 / layername, c2 / r, c2 / pos, r1 / size, r1 / base, r1 / pos, fil1 / radius

### 文件内容证据

- `mutual_inductance_multiturn.java`: 参数=7, 几何=14, 物理场=0, 研究=0, 结果=0
- `models.acdc.mutual_inductance_multiturn (1).pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `models.acdc.mutual_inductance_multiturn.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `mutual_inductance_multiturn.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mutual_inductance_multiturn.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mutual_inductance_multiturn_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.mutual_inductance_multiturn (1).pdf`: models.acdc.mutual_inductance_multiturn (1).pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；domain model. The secondary coil is modeled using a homogenized approach, which does
- `models.acdc.mutual_inductance_multiturn.pdf`: models.acdc.mutual_inductance_multiturn.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Created in COMSOL Multiphysics 5.6；domain model. The secondary coil is modeled using a homogenized approach, which does

## PDF Summary and Physics Judgement

- Simple summary: mutual_inductance_multiturn 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Magnetic Fields，并采用 Stationary 研究。
- Primary physics: `Magnetic Fields`
- Study type: `Stationary`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Magnetic Fields，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Stationary 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: domain model. The secondary coil is modeled using a homogenized approach, which does
- PDF: not explicitly consider each turn of the coil. Static and AC results are compared one against
...
- PDF keywords: 电磁, 多孔介质/裂隙, models.acdc.mutual_inductance_multiturn (1).pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；domain model. The secondary coil is modeled using a homogenized approach, which does, models.acdc.mutual_inductance_multiturn.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Created in COMSOL Multiphysics 5.6；domain model. The secondary coil is modeled using a homogenized approach, which does
...
- `N` -> Number of turns in secondary; MATLAB: `model.param.set('N', value, description)`
- `M` -> Analytic mutual inductance in secondary; MATLAB: `model.param.set('M', value, description)`
...
- 已识别几何证据：c1 / Circle；c2 / Circle；r1 / Rectangle；fil1 / Fillet；geom1 / c1 / r；geom1 / c1 / angle
- 已识别物理场证据：Magnetic Fields
- 已识别参数：r_wire、R1、R2、N、M、I1、I2
...
- Sweep `N` around the learned value `20` and export target outputs to CSV.
- Sweep `M` around the learned value `N*(mu0_const*pi*R2^2)/(2*R1)` and export target outputs to CSV.
...
- Summary: 已学习 mutual_inductance_multiturn 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=35 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 35 条；几何=14，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mutual_inductance_multiturn.m, mutual_inductance_multiturn.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mutual_inductance_multiturn，训练阶段为 modeling_logic_learning_ready。
...
- `mutual_inductance_multiturn.mph` (comsol_mph, 10477910 bytes)
- `mutual_inductance_multiturn.m` (matlab_livelink, 34133 bytes)
- `mutual_inductance_multiturn.java` (comsol_java, 31700 bytes)
- `mutual_inductance_multiturn_summary.json` (json, 1236 bytes)
- `models.acdc.mutual_inductance_multiturn (1).pdf` (pdf_document, 1664376 bytes)
- `models.acdc.mutual_inductance_multiturn.pdf` (pdf_document, 1503367 bytes)
