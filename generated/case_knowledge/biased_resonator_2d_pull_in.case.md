...
- Created at: `2026-08-22T04:13:03.340848+00:00`
...
- `vrel` = `-0.3` - Relative vertical displacement

## 内容级读取结果

- 模型树证据数量: 33
- 理论关键词: 结构力学, 电磁, 声学, 优化/参数扫描
...
- `biased_resonator_2d_pull_in.java`: 参数=6, 几何=11, 物理场=0, 研究=0, 结果=0
...
- `biased_resonator_2d_pull_in_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=3, 结果=0
- `biased_resonator_2d_pull_in.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `biased_resonator_2d_pull_in.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: biased_resonator_2d_pull_in 主要研究结构受力、变形或强度响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenfrequency 研究。
...
- `vrel` -> Relative vertical displacement; MATLAB: `model.param.set('vrel', value, description)`
...
- 已识别参数：Vdc、l_b、t_b、l_e、gap、vrel
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 结构力学, 电磁, 声学, 优化/参数扫描?
...
- Sweep `vrel` around the learned value `-0.3` and export target outputs to CSV.
...
- Summary: 已学习 biased_resonator_2d_pull_in 的 8 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=33 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 33 条；几何=11，物理场=3，材料=1，网格=1，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：biased_resonator_2d_pull_in.m, biased_resonator_2d_pull_in.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 biased_resonator_2d_pull_in，训练阶段为 modeling_logic_learning_ready。
...
- `biased_resonator_2d_pull_in.mph` (comsol_mph, 9219743 bytes)
- `biased_resonator_2d_pull_in.m` (matlab_livelink, 38226 bytes)
- `biased_resonator_2d_pull_in.java` (comsol_java, 37382 bytes)
- `biased_resonator_2d_pull_in_summary.json` (json, 1267 bytes)
