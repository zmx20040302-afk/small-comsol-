...
- Created at: `2026-07-06T09:41:32.394328+00:00`
...
- 模型树证据数量: 21
- 理论关键词: 未自动识别
- geometry: c1 / Circle, pt1 / Point
- physics: lpeq / LaplaceEquation
- mesh: mesh1
- studies: std1 / stat / Stationary, std1, Stationary
- results: pg1, surf1, pg2, lngr1, int1 / IntSurface
- boundary_conditions: ptsrc1 / 3, stat / errestandadap, stat / meshadaptmethod, surf1 / expr, lngr1 / markerpos, lngr1 / linewidth, lngr1 / evaluationsettings, lngr1 / expr, ptsrc1 / [3]

### 文件内容证据

- `point_source.mph`: 参数=0, 几何=2, 物理场=1, 研究=3, 结果=5
- `point_source.java`: 参数=0, 几何=2, 物理场=1, 研究=3, 结果=5
- `point_source.m`: 参数=0, 几何=2, 物理场=1, 研究=2, 结果=5
- `models.mph.point_source.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mph.point_source.zh_CN.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `point_source.zh_CN.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mph.point_source.pdf`: models.mph.point_source.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.point_source.zh_CN.pdf`: models.mph.point_source.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 点源实现 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 LaplaceEquation，并采用 Stationary 研究。
- Primary physics: `LaplaceEquation`
- Study type: `Stationary`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 LaplaceEquation，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Stationary 确认。

### Judgement Evidence

- 脚本/文本识别到的物理场: lpeq / LaplaceEquation
...
- PDF keywords: models.mph.point_source.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.point_source.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
...
- 已识别几何证据：c1 / Circle；pt1 / Point
- 已识别物理场证据：lpeq / LaplaceEquation
...
- Transfer recognized physics interfaces to similar requirements: lpeq / LaplaceEquation.
...
- Use detected result nodes as candidate validation and training targets: pg1, surf1, pg2, lngr1, int1 / IntSurface.
...
- Summary: 已学习 点源实现 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=21 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 21 条；几何=2，物理场=1，材料=0，网格=1，研究=3，结果=5。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 点源实现，训练阶段为 modeling_logic_learning_ready。
...
- `热执行器代理模型 App` score=0.155; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42
- `集群设置验证` score=0.1488; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=1

### Official Documentation Checks

- `IntroductionToLiveLinkForMATLAB.pdf` page 3; module=LiveLink_for_MATLAB; score=0.1086
- `IntroductionToCOMSOLMultiphysics.pdf` page 262; module=COMSOL_Multiphysics; score=0.1086
- `LiveLinkForSimulinkUsersGuide.pdf` page 20; module=LiveLink_for_Simulink; score=0.104
