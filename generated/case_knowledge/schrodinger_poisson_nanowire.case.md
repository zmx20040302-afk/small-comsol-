...
- Case directory: `D:\桌面\codex\案例下载\电气\砷化镓纳米线的自洽薛定谔-泊松结果`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:32:21.342299+00:00`
...
- `R0` = `50[nm]` - Radius of nanowire
- `m` = `0` - Azimuthal quantum number
- `meff` = `0.067*me_const` - Electron effective mass of GaAs
- `epsr` = `12.9` - Dielectric constant of GaAs
- `Nd` = `2*10^18[cm^-3]` - Doping
- `T` = `10[K]` - Temperature
- `kT` = `k_B_const*T` - Thermal energy
- `beta` = `1/kT` - 1/kT
- `N0TF` = `1/4*(2*meff/(pi*beta*hbar_const^2))^(3/2)` - Effective DOS, Thomas-Fermi approx
- `Ef` = `0[eV]` - Fermi energy level
- `V_R0` = `-(Ef+0.7[eV])/e_const` - Electric potential at surface of nanowire

## 内容级读取结果

- 模型树证据数量: 18
- 理论关键词: 传热, 电磁, 量子/特征值, 结构力学, 流体
...
- `schrodinger_poisson_nanowire.java`: 参数=11, 几何=2, 物理场=0, 研究=0, 结果=0
- `models.semicond.schrodinger_poisson_nanowire.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `schrodinger_poisson_nanowire.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `schrodinger_poisson_nanowire_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `schrodinger_poisson_nanowire.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.schrodinger_poisson_nanowire.pdf`: models.semicond.schrodinger_poisson_nanowire.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results for a GaAs Nanowire

## PDF Summary and Physics Judgement

- Simple summary: schrodinger_poisson_nanowire 主要研究量子能级或波函数分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Eigenvalue 研究。
...
- PDF: Results for a GaAs Nanowire
- PDF: 2 | SELF-CONSISTENT SCHRÖDINGER–POISSON RESULTS FOR A GAAS NANOWIRE
- PDF: predefined Schrödinger–Poisson multiphysics coupling feature is combined with the
...
- PDF keywords: 传热, 电磁, 量子/特征值, 结构力学, 流体, models.semicond.schrodinger_poisson_nanowire.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results for a GaAs Nanowire
- `R0` -> Radius of nanowire; MATLAB: `model.param.set('R0', value, description)`
- `m` -> Azimuthal quantum number; MATLAB: `model.param.set('m', value, description)`
- `meff` -> Electron effective mass of GaAs; MATLAB: `model.param.set('meff', value, description)`
- `epsr` -> Dielectric constant of GaAs; MATLAB: `model.param.set('epsr', value, description)`
- `Nd` -> Doping; MATLAB: `model.param.set('Nd', value, description)`
- `T` -> Temperature; MATLAB: `model.param.set('T', value, description)`
- `kT` -> Thermal energy; MATLAB: `model.param.set('kT', value, description)`
- `beta` -> 1/kT; MATLAB: `model.param.set('beta', value, description)`
- `N0TF` -> Effective DOS, Thomas-Fermi approx; MATLAB: `model.param.set('N0TF', value, description)`
- `Ef` -> Fermi energy level; MATLAB: `model.param.set('Ef', value, description)`
- `V_R0` -> Electric potential at surface of nanowire; MATLAB: `model.param.set('V_R0', value, description)`
...
- 已识别参数：R0、m、meff、epsr、Nd、T、kT、beta、N0TF、Ef、V_R0
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 电磁, 量子/特征值, 结构力学, 流体?
...
- Sweep `R0` around the learned value `50[nm]` and export target outputs to CSV.
- Sweep `m` around the learned value `0` and export target outputs to CSV.
- Sweep `meff` around the learned value `0.067*me_const` and export target outputs to CSV.
- Sweep `epsr` around the learned value `12.9` and export target outputs to CSV.
- Sweep `Nd` around the learned value `2*10^18[cm^-3]` and export target outputs to CSV.
- Sweep `T` around the learned value `10[K]` and export target outputs to CSV.
- Sweep `kT` around the learned value `k_B_const*T` and export target outputs to CSV.
- Sweep `beta` around the learned value `1/kT` and export target outputs to CSV.
...
- Summary: 已学习 schrodinger_poisson_nanowire 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=18 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 18 条；几何=2，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：schrodinger_poisson_nanowire.m, schrodinger_poisson_nanowire.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 schrodinger_poisson_nanowire，训练阶段为 modeling_logic_learning_ready。
...
- `schrodinger_poisson_nanowire.mph` (comsol_mph, 5755992 bytes)
- `schrodinger_poisson_nanowire.m` (matlab_livelink, 57457 bytes)
- `schrodinger_poisson_nanowire.java` (comsol_java, 51502 bytes)
- `schrodinger_poisson_nanowire_summary.json` (json, 1322 bytes)
- `models.semicond.schrodinger_poisson_nanowire.pdf` (pdf_document, 428125 bytes)
