# rotating_machinery_3d_tutorial_geom_sequence

- Case directory: `D:\桌面\codex\案例下载\电气\旋转机械三维建模教程`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:37:32.745359+00:00`
...
- `R1` = `1[cm]` - Geometry external radius
- `R2` = `6[mm]` - Positioning of sliding interface
- `R3` = `5[mm]` - Rotating object radius
- `H1` = `5[mm]` - Height of magnet and rotating disk

## 内容级读取结果

- 模型树证据数量: 45
- 理论关键词: 结构力学, 流体, 电磁
- geometry: cyl1 / Cylinder, cyl2 / Cylinder, cyl3 / Cylinder, dif1 / Difference, cyl4 / Cylinder, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / pos, geom1 / cyl2 / r, geom1 / cyl2 / h, geom1 / cyl2 / pos, geom1 / cyl3 / r
- physics: Electric Currents
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: cyl1 / r, cyl1 / h, cyl1 / pos, cyl2 / r, cyl2 / h, cyl2 / pos, cyl3 / r, cyl3 / h, cyl3 / pos, cyl4 / r, cyl4 / h, cyl4 / pos

### 文件内容证据

- `rotating_machinery_3d_tutorial_geom_sequence.java`: 参数=4, 几何=20, 物理场=0, 研究=0, 结果=0
- `rotating_machinery_3d_tutorial_geom_sequence.m`: 参数=4, 几何=19, 物理场=0, 研究=0, 结果=0
- `models.acdc.rotating_machinery_3d_tutorial.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `rotating_machinery_3d_tutorial_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rotating_machinery_3d_tutorial_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.rotating_machinery_3d_tutorial.pdf`: models.acdc.rotating_machinery_3d_tutorial.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electric Currents，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；representing an optional lamination. Figure 1 shows the geometry with the rotor and

## PDF Summary and Physics Judgement

- Simple summary: rotating_machinery_3d_tutorial_geom_sequence 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: representing an optional lamination. Figure 1 shows the geometry with the rotor and
- PDF: This COMSOL Multiphysics application is a time-dependent 3D problem. It is a true
- PDF: condition between the stator and rotor geometries. For the solid (nonlaminated) rotor the
...
- PDF keywords: 结构力学, 流体, 电磁, models.acdc.rotating_machinery_3d_tutorial.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electric Currents，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；representing an optional lamination. Figure 1 shows the geometry with the rotor and
- `R1` -> Geometry external radius; MATLAB: `model.param.set('R1', value, description)`
- `R2` -> Positioning of sliding interface; MATLAB: `model.param.set('R2', value, description)`
- `R3` -> Rotating object radius; MATLAB: `model.param.set('R3', value, description)`
- `H1` -> Height of magnet and rotating disk; MATLAB: `model.param.set('H1', value, description)`
...
- 已识别几何证据：cyl1 / Cylinder；cyl2 / Cylinder；cyl3 / Cylinder；dif1 / Difference；cyl4 / Cylinder；geom1 / cyl1 / r
...
- Sweep `R1` around the learned value `1[cm]` and export target outputs to CSV.
- Sweep `R2` around the learned value `6[mm]` and export target outputs to CSV.
- Sweep `R3` around the learned value `5[mm]` and export target outputs to CSV.
- Sweep `H1` around the learned value `5[mm]` and export target outputs to CSV.
...
- Summary: 已学习 rotating_machinery_3d_tutorial_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=45 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 45 条；几何=22，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：rotating_machinery_3d_tutorial_geom_sequence.m, rotating_machinery_3d_tutorial_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 rotating_machinery_3d_tutorial_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `rotating_machinery_3d_tutorial_geom_sequence.mph` (comsol_mph, 334719 bytes)
- `rotating_machinery_3d_tutorial_geom_sequence.m` (matlab_livelink, 7365 bytes)
- `rotating_machinery_3d_tutorial_geom_sequence.java` (comsol_java, 7283 bytes)
- `rotating_machinery_3d_tutorial_geom_sequence_summary.json` (json, 762 bytes)
- `models.acdc.rotating_machinery_3d_tutorial.pdf` (pdf_document, 1048584 bytes)
