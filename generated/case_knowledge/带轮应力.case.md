...
- Created at: `2026-07-06T09:38:55.419867+00:00`
...
- `stresses_in_pulley.java`: 参数=8, 几何=3, 物理场=1, 研究=3, 结果=0
- `stresses_in_pulley.mph`: 参数=8, 几何=3, 物理场=1, 研究=3, 结果=0
- `stresses_in_pulley.m`: 参数=8, 几何=3, 物理场=1, 研究=2, 结果=0
- `stresses_in_pulley.zh_CN.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `stresses_in_pulley_parameters.txt`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `stresses_in_pulley_parameters.zh_CN.txt`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mph.stresses_in_pulley.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mph.stresses_in_pulley.zh_CN.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `stresses_in_pulley_variables.txt`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `stresses_in_pulley_variables.zh_CN.txt`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mph.stresses_in_pulley.pdf`: models.mph.stresses_in_pulley.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.stresses_in_pulley.zh_CN.pdf`: models.mph.stresses_in_pulley.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 带轮应力 主要研究结构受力、变形或强度响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF keywords: 传热, 结构力学, 流体, 电磁, 多孔介质/裂隙, models.mph.stresses_in_pulley.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.stresses_in_pulley.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `mu` -> Coefficient of friction, pulley/belt; MATLAB: `model.param.set('mu', value, description)`
- `beta` -> Contact angle, pulley/belt; MATLAB: `model.param.set('beta', value, description)`
- `d` -> Pulley thickness; MATLAB: `model.param.set('d', value, description)`
- `P` -> Power at pulley shaft; MATLAB: `model.param.set('P', value, description)`
- `n` -> Revolutions per minute; MATLAB: `model.param.set('n', value, description)`
- `omega` -> Angular velocity; MATLAB: `model.param.set('omega', value, description)`
- `M` -> Torque at pulley shaft; MATLAB: `model.param.set('M', value, description)`
...
- Sweep `mu` around the learned value `0.5` and export target outputs to CSV.
- Sweep `beta` around the learned value `170[deg]` and export target outputs to CSV.
- Sweep `d` around the learned value `5[cm]` and export target outputs to CSV.
- Sweep `P` around the learned value `185[kW]` and export target outputs to CSV.
- Sweep `n` around the learned value `1000` and export target outputs to CSV.
- Sweep `omega` around the learned value `2*pi[rad]*n/1[min]` and export target outputs to CSV.
- Sweep `M` around the learned value `P/omega` and export target outputs to CSV.
- Sweep `R` around the learned value `125[mm]` and export target outputs to CSV.
...
- Summary: 已学习 带轮应力 的 10 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=19 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 19 条；几何=3，物理场=1，材料=1，网格=1，研究=3，结果=0。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 带轮应力，训练阶段为 modeling_logic_learning_ready。
...
- `管式反应器代理模型 App` score=0.1817; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `热执行器代理模型 App` score=0.1573; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42
- `集群设置验证` score=0.1479; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=1

### Official Documentation Checks

- `IntroductionToLiveLinkForMATLAB.pdf` page 5; module=LiveLink_for_MATLAB; score=0.1069
- `IntroductionToCOMSOLMultiphysics.pdf` page 262; module=COMSOL_Multiphysics; score=0.103
- `IntroductionToLiveLinkForMATLAB.pdf` page 3; module=LiveLink_for_MATLAB; score=0.0985
