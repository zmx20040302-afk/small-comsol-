...
- Case directory: `D:\桌面\codex\案例下载\电气\双极晶体管的三维分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:59:04.355024+00:00`
...
- `V_C` = `0.5[V]` - Applied voltage: collector
...
- 模型树证据数量: 39
- 理论关键词: 传热, 电磁
- geometry: blk1 / Block, blk2 / Block, wp1 / WorkPlane, r1 / Rectangle, geom1 / blk1 / size, geom1 / blk2 / size, geom1 / blk2 / pos, geom1 / wp1 / quickz, geom1 / wp1 / unite
- mesh: mesh1
- studies: Stationary
- boundary_conditions: blk1 / size, blk2 / size, blk2 / pos, wp1 / quickz, wp1 / unite

### 文件内容证据

- `bipolar_transistor_3d.java`: 参数=20, 几何=9, 物理场=0, 研究=0, 结果=0
- `models.semicond.bipolar_transistor_3d.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `bipolar_transistor_3d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `bipolar_transistor_3d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `bipolar_transistor_3d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.bipolar_transistor_3d.pdf`: models.semicond.bipolar_transistor_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；extend semiconductor modeling into 3D using COMSOL Multiphysics. As in the 2D

## PDF Summary and Physics Judgement

- Simple summary: bipolar_transistor_3d 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: extend semiconductor modeling into 3D using COMSOL Multiphysics. As in the 2D
- PDF: regime. A voltage-driven study is computed to characterize the current–voltage response
- PDF: The model geometry is shown in Figure 2. Due to the symmetry of the device only one
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 电磁
...
- PDF keywords: 传热, 电磁, models.semicond.bipolar_transistor_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；extend semiconductor modeling into 3D using COMSOL Multiphysics. As in the 2D
...
- `l_E` -> Emitter length; MATLAB: `model.param.set('l_E', value, description)`
...
- `l_cB` -> Base contact length; MATLAB: `model.param.set('l_cB', value, description)`
...
- 已识别几何证据：blk1 / Block；blk2 / Block；wp1 / WorkPlane；r1 / Rectangle；geom1 / blk1 / size；geom1 / blk2 / size
- 已识别参数：T0、w_BJT、d_BJT、l_BJT、w_E、l_E、w_EB、w_cE、w_cB、l_cB、w_cC、d_E
...
- Sweep `l_E` around the learned value `3[um]` and export target outputs to CSV.
...
- Summary: 已学习 bipolar_transistor_3d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=23；内容证据=39 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 39 条；几何=9，物理场=0，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：bipolar_transistor_3d.m, bipolar_transistor_3d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 bipolar_transistor_3d，训练阶段为 modeling_logic_learning_ready。
...
- `bipolar_transistor_3d.mph` (comsol_mph, 105473047 bytes)
- `bipolar_transistor_3d.m` (matlab_livelink, 47370 bytes)
- `bipolar_transistor_3d.java` (comsol_java, 47794 bytes)
- `bipolar_transistor_3d_summary.json` (json, 1695 bytes)
- `models.semicond.bipolar_transistor_3d.pdf` (pdf_document, 1635917 bytes)
