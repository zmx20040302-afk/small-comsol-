...
- Case directory: `D:\桌面\codex\案例下载\电气\静电吸盘`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T06:59:13.969295+00:00`
...
- `d_wafer` = `100[mm]` - Wafer diameter
- `t_wafer` = `0.5[mm]` - Wafer thickness
- `r_electrode` = `45[mm]` - Electrode radius
- `t_insulator` = `0.6[mm]` - Insulator thickness
- `gap` = `40[um]` - Gap between wafer and chuck
- `d_pin` = `1[mm]` - Pin width
- `x_pin` = `49[mm]` - Pin location
- `d_inlet` = `4[mm]` - Inlet diameter
- `w_outlet` = `2[mm]` - Outlet width
- `x_outlet` = `30[mm]` - Outlet location
- `L_inlet` = `1[mm]` - Inlet length
- `L_outlet` = `1[mm]` - Outlet Length
- `x_gap3` = `48[mm]` - gap_3 location
- `w_gap3` = `0.25[mm]` - gap_3 width

## 内容级读取结果

- 模型树证据数量: 21
- 理论关键词: 传热, 流体, 电磁, 结构力学, 优化/参数扫描
- physics: Electrostatics, Heat Transfer, Solid Mechanics, Laminar Flow
- studies: Stationary, Time Dependent, Parametric Sweep

### 文件内容证据

- `electrostatic_chuck.java`: 参数=14, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mems.electrostatic_chuck.pdf`: 参数=0, 几何=0, 物理场=4, 研究=3, 结果=0
- `electrostatic_chuck.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `electrostatic_chuck_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `electrostatic_chuck.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.electrostatic_chuck.pdf`: models.mems.electrostatic_chuck.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Solid Mechanics、Laminar Flow，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；resulting from the couplings of electrostatic force, gas flow, heat transfer, and solid

## PDF Summary and Physics Judgement

- Simple summary: electrostatic_chuck 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Time Dependent 研究。
...
- PDF: resulting from the couplings of electrostatic force, gas flow, heat transfer, and solid
- PDF: the e-chuck that are explicitly included in the geometry. The silicon wafer (diameter =
- PDF: depending on the resultant force on it. The wafer’s center is suspended above the e-chuck
- 脚本/文本识别到的物理场: Electrostatics, Heat Transfer, Solid Mechanics, Laminar Flow
...
- PDF keywords: 传热, 流体, 电磁, 结构力学, 优化/参数扫描, models.mems.electrostatic_chuck.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Solid Mechanics、Laminar Flow，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；resulting from the couplings of electrostatic force, gas flow, heat transfer, and solid
- `d_wafer` -> Wafer diameter; MATLAB: `model.param.set('d_wafer', value, description)`
- `t_wafer` -> Wafer thickness; MATLAB: `model.param.set('t_wafer', value, description)`
- `r_electrode` -> Electrode radius; MATLAB: `model.param.set('r_electrode', value, description)`
- `t_insulator` -> Insulator thickness; MATLAB: `model.param.set('t_insulator', value, description)`
- `gap` -> Gap between wafer and chuck; MATLAB: `model.param.set('gap', value, description)`
- `d_pin` -> Pin width; MATLAB: `model.param.set('d_pin', value, description)`
- `x_pin` -> Pin location; MATLAB: `model.param.set('x_pin', value, description)`
- `d_inlet` -> Inlet diameter; MATLAB: `model.param.set('d_inlet', value, description)`
- `w_outlet` -> Outlet width; MATLAB: `model.param.set('w_outlet', value, description)`
- `x_outlet` -> Outlet location; MATLAB: `model.param.set('x_outlet', value, description)`
- `L_inlet` -> Inlet length; MATLAB: `model.param.set('L_inlet', value, description)`
- `L_outlet` -> Outlet Length; MATLAB: `model.param.set('L_outlet', value, description)`
...
- 已识别物理场证据：Electrostatics；Heat Transfer；Solid Mechanics；Laminar Flow
- 已识别参数：d_wafer、t_wafer、r_electrode、t_insulator、gap、d_pin、x_pin、d_inlet、w_outlet、x_outlet、L_inlet、L_outlet
...
- Transfer recognized physics interfaces to similar requirements: Electrostatics, Heat Transfer, Solid Mechanics, Laminar Flow.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 流体, 电磁, 结构力学, 优化/参数扫描?
...
- Sweep `d_wafer` around the learned value `100[mm]` and export target outputs to CSV.
- Sweep `t_wafer` around the learned value `0.5[mm]` and export target outputs to CSV.
- Sweep `r_electrode` around the learned value `45[mm]` and export target outputs to CSV.
- Sweep `t_insulator` around the learned value `0.6[mm]` and export target outputs to CSV.
- Sweep `gap` around the learned value `40[um]` and export target outputs to CSV.
- Sweep `d_pin` around the learned value `1[mm]` and export target outputs to CSV.
- Sweep `x_pin` around the learned value `49[mm]` and export target outputs to CSV.
- Sweep `d_inlet` around the learned value `4[mm]` and export target outputs to CSV.
...
- Summary: 已学习 electrostatic_chuck 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=14；内容证据=21 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 21 条；几何=0，物理场=4，材料=0，网格=0，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：electrostatic_chuck.m, electrostatic_chuck.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 electrostatic_chuck，训练阶段为 modeling_logic_learning_ready。
...
- `electrostatic_chuck.mph` (comsol_mph, 1332639903 bytes)
- `electrostatic_chuck.m` (matlab_livelink, 105430 bytes)
- `electrostatic_chuck.java` (comsol_java, 105526 bytes)
- `electrostatic_chuck_summary.json` (json, 4389 bytes)
- `models.mems.electrostatic_chuck.pdf` (pdf_document, 1291065 bytes)
