...
- Case directory: `D:\桌面\codex\案例下载\电气\双极晶体管`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:22:50.587559+00:00`
...
- mesh: mesh1
- studies: Stationary
- boundary_conditions: r1 / size, r1 / pos, pt1 / p, pt2 / p

### 文件内容证据

- `bipolar_transistor.java`: 参数=20, 几何=7, 物理场=0, 研究=0, 结果=0
- `bipolar_transistor.m`: 参数=5, 几何=0, 物理场=0, 研究=0, 结果=0
- `bipolar_transistor.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.semicond.bipolar_transistor.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `bipolar_transistor_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.bipolar_transistor.pdf`: models.semicond.bipolar_transistor.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；labeled. The red line shows the axis of symmetry used to simplified the model geometry. Bottom:

## PDF Summary and Physics Judgement

- Simple summary: bipolar_transistor 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: labeled. The red line shows the axis of symmetry used to simplified the model geometry. Bottom:
- PDF: regions (n+, n, p) correspond with those in the geometry cross-section.
- PDF: current output under both voltage and current driven configurations. First, a study sweeps
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 电磁
...
- PDF keywords: 传热, 电磁, models.semicond.bipolar_transistor.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；labeled. The red line shows the axis of symmetry used to simplified the model geometry. Bottom:
...
- Summary: 已学习 bipolar_transistor 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=21；内容证据=35 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 35 条；几何=7，物理场=0，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：bipolar_transistor.m, bipolar_transistor.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 bipolar_transistor，训练阶段为 modeling_logic_learning_ready。
...
- `bipolar_transistor.mph` (comsol_mph, 36518938 bytes)
- `bipolar_transistor.m` (matlab_livelink, 45673 bytes)
- `bipolar_transistor.java` (comsol_java, 46124 bytes)
- `bipolar_transistor_summary.json` (json, 1637 bytes)
- `models.semicond.bipolar_transistor.pdf` (pdf_document, 1158169 bytes)
