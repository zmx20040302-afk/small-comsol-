...
- Case directory: `D:\桌面\codex\案例下载\电气\永磁电机三维模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T08:35:34.341531+00:00`
...
- `pm_motor_3d.java`: 参数=20, 几何=0, 物理场=0, 研究=1, 结果=0
- `models.acdc.pm_motor_3d.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `pm_motor_3d.mph`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `pm_motor_3d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pm_motor_3d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.pm_motor_3d.pdf`: models.acdc.pm_motor_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；substantial as illustrated in this model.

## PDF Summary and Physics Judgement

- Simple summary: pm_motor_3d 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: substantial as illustrated in this model.
- PDF: Rotation is modeled using the ready-made physics interface for rotating machinery. The
- PDF: central part of the geometry, containing the rotor and part of the air-gap, is modeled as
...
- PDF keywords: 传热, 结构力学, 电磁, 多孔介质/裂隙, models.acdc.pm_motor_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；substantial as illustrated in this model.
- `n_sectors` -> Number of sectors; MATLAB: `model.param.set('n_sectors', value, description)`
- `n_pairs` -> Number of coil pairs; MATLAB: `model.param.set('n_pairs', value, description)`
- `sector_angle` -> Sector angle; MATLAB: `model.param.set('sector_angle', value, description)`
- `rpm` -> Motor RPM; MATLAB: `model.param.set('rpm', value, description)`
- `time_one_cycle` -> The time for rotation for one sector; MATLAB: `model.param.set('time_one_cycle', value, description)`
- `omega_rotor` -> Angular frequency of rotor; MATLAB: `model.param.set('omega_rotor', value, description)`
- `freq` -> Supply frequency; MATLAB: `model.param.set('freq', value, description)`
- `omega` -> Supply angular frequency; MATLAB: `model.param.set('omega', value, description)`
- `I_RMS` -> Coil Ampere-turns, RMS; MATLAB: `model.param.set('I_RMS', value, description)`
- `I0` -> Coil Ampere-turns, peak value; MATLAB: `model.param.set('I0', value, description)`
- `t` -> Set the time t to zero for the stationary study; MATLAB: `model.param.set('t', value, description)`
- `r_outer` -> outer radius of stator; MATLAB: `model.param.set('r_outer', value, description)`
...
- 已识别参数：n_sectors、n_pairs、sector_angle、rpm、time_one_cycle、omega_rotor、freq、omega、I_RMS、I0、t、r_outer
...
- Sweep `n_sectors` around the learned value `18` and export target outputs to CSV.
- Sweep `n_pairs` around the learned value `n_sectors/2` and export target outputs to CSV.
- Sweep `sector_angle` around the learned value `360[deg]/n_sectors` and export target outputs to CSV.
- Sweep `rpm` around the learned value `3000[rpm]` and export target outputs to CSV.
- Sweep `time_one_cycle` around the learned value `1/(rpm*n_sectors)` and export target outputs to CSV.
- Sweep `omega_rotor` around the learned value `2*pi*rpm` and export target outputs to CSV.
- Sweep `freq` around the learned value `n_pairs*rpm` and export target outputs to CSV.
- Sweep `omega` around the learned value `2*pi*freq` and export target outputs to CSV.
...
- Summary: 已学习 pm_motor_3d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=30；内容证据=34 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 34 条；几何=0，物理场=2，材料=0，网格=0，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pm_motor_3d.m, pm_motor_3d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pm_motor_3d，训练阶段为 modeling_logic_learning_ready。
...
- `pm_motor_3d.mph` (comsol_mph, 351200817 bytes)
- `pm_motor_3d.m` (matlab_livelink, 82510 bytes)
- `pm_motor_3d.java` (comsol_java, 80605 bytes)
- `pm_motor_3d_summary.json` (json, 2748 bytes)
- `models.acdc.pm_motor_3d.pdf` (pdf_document, 3923902 bytes)
