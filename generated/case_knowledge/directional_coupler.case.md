...
- Case directory: `D:\桌面\codex\案例下载\电气\定向耦合器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T08:26:57.363206+00:00`
...
- `lda0` = `1.15[um]` - Wavelength
- `f0` = `c_const/lda0` - Frequency
- `a` = `3[um]` - Side of waveguide cross section
- `d` = `3[um]` - Distance between the waveguides
- `len` = `2.1[mm]` - Waveguide length
- `width` = `6*a` - Width of calculation domain
- `height` = `4*a` - Height of calculation domain
- `ncl` = `3.47` - Refractive index of GaAs
- `dn` = `0.005` - Refractive index increase in waveguide core
- `nco` = `ncl+dn` - Refractive index in waveguide core

## 内容级读取结果

- 模型树证据数量: 33
- 理论关键词: 电磁
- geometry: blk1 / Block, blk2 / Block, blk3 / Block, geom1 / blk1 / size, geom1 / blk1 / base, geom1 / blk2 / size, geom1 / blk2 / base, geom1 / blk2 / pos, geom1 / blk3 / size, geom1 / blk3 / base, geom1 / blk3 / pos
- physics: Electromagnetic Waves
- mesh: mesh1, mesh2
- studies: Frequency Domain
- boundary_conditions: blk1 / size, blk1 / base, blk2 / size, blk2 / base, blk2 / pos, blk3 / size, blk3 / base, blk3 / pos

### 文件内容证据

- `directional_coupler.java`: 参数=10, 几何=11, 物理场=0, 研究=0, 结果=0
- `models.woptics.directional_coupler.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `directional_coupler.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `directional_coupler.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `directional_coupler_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.directional_coupler.pdf`: models.woptics.directional_coupler.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；preserve the aspect ratio for the geometry.

## PDF Summary and Physics Judgement

- Simple summary: directional_coupler 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electromagnetic Waves，并采用 Frequency Domain 研究。
...
- PDF: preserve the aspect ratio for the geometry.
- PDF: as the mesh does not need to resolve the wave on a wavelength scale, but rather the beating
- PDF: beat length is well resolved in the model.
...
- PDF keywords: 电磁, models.woptics.directional_coupler.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；preserve the aspect ratio for the geometry.
- `lda0` -> Wavelength; MATLAB: `model.param.set('lda0', value, description)`
- `f0` -> Frequency; MATLAB: `model.param.set('f0', value, description)`
- `a` -> Side of waveguide cross section; MATLAB: `model.param.set('a', value, description)`
- `d` -> Distance between the waveguides; MATLAB: `model.param.set('d', value, description)`
- `len` -> Waveguide length; MATLAB: `model.param.set('len', value, description)`
- `width` -> Width of calculation domain; MATLAB: `model.param.set('width', value, description)`
- `height` -> Height of calculation domain; MATLAB: `model.param.set('height', value, description)`
- `ncl` -> Refractive index of GaAs; MATLAB: `model.param.set('ncl', value, description)`
- `dn` -> Refractive index increase in waveguide core; MATLAB: `model.param.set('dn', value, description)`
- `nco` -> Refractive index in waveguide core; MATLAB: `model.param.set('nco', value, description)`
...
- 已识别参数：lda0、f0、a、d、len、width、height、ncl、dn、nco
...
- Sweep `lda0` around the learned value `1.15[um]` and export target outputs to CSV.
- Sweep `f0` around the learned value `c_const/lda0` and export target outputs to CSV.
- Sweep `a` around the learned value `3[um]` and export target outputs to CSV.
- Sweep `d` around the learned value `3[um]` and export target outputs to CSV.
- Sweep `len` around the learned value `2.1[mm]` and export target outputs to CSV.
- Sweep `width` around the learned value `6*a` and export target outputs to CSV.
- Sweep `height` around the learned value `4*a` and export target outputs to CSV.
- Sweep `ncl` around the learned value `3.47` and export target outputs to CSV.
...
- Summary: 已学习 directional_coupler 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=33 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 33 条；几何=11，物理场=1，材料=0，网格=2，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：directional_coupler.m, directional_coupler.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 directional_coupler，训练阶段为 modeling_logic_learning_ready。
...
- `directional_coupler.mph` (comsol_mph, 185006019 bytes)
- `directional_coupler.m` (matlab_livelink, 61015 bytes)
- `directional_coupler.java` (comsol_java, 57912 bytes)
- `directional_coupler_summary.json` (json, 1984 bytes)
- `models.woptics.directional_coupler.pdf` (pdf_document, 4303883 bytes)
