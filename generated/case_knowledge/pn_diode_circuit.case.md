...
- Case directory: `D:\桌面\codex\案例下载\电气\P-N 二极管电路`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T03:10:50.252107+00:00`
...
- 模型树证据数量: 34
- 理论关键词: 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, fil1 / Fillet, pt1 / Point, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r2 / pos, geom1 / fil1 / radius, geom1 / pt1 / p
- materials: mat1
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: r1 / size, r1 / pos, r2 / size, r2 / pos, fil1 / radius, pt1 / p

### 文件内容证据

- `pn_diode_circuit.java`: 参数=14, 几何=10, 物理场=0, 研究=0, 结果=0
- `pn_diode_circuit.m`: 参数=6, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.semicond.pn_diode_circuit.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `pn_diode_circuit.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pn_diode_circuit_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.pn_diode_circuit.pdf`: models.semicond.pn_diode_circuit.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；In this example, a full level device simulation is made by connecting a 2D meshed p–n

## PDF Summary and Physics Judgement

- Simple summary: pn_diode_circuit 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF: In this example, a full level device simulation is made by connecting a 2D meshed p–n
- PDF: rectifier circuit is displayed in Figure 1). In order to validate the results, the outputs of the
- PDF: meshed). A Shockley–Read–Hall recombination is also added to the model in order to
...
- PDF keywords: 电磁, models.semicond.pn_diode_circuit.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；In this example, a full level device simulation is made by connecting a 2D meshed p–n
- `l_diode` -> Diode length; MATLAB: `model.param.set('l_diode', value, description)`
- `d_diode` -> Diode thickness; MATLAB: `model.param.set('d_diode', value, description)`
- `w_diode` -> Diode width; MATLAB: `model.param.set('w_diode', value, description)`
- `d_p` -> Depth of the p-region; MATLAB: `model.param.set('d_p', value, description)`
- `w_anode` -> Anode width; MATLAB: `model.param.set('w_anode', value, description)`
- `Nd_max` -> Max donor doping; MATLAB: `model.param.set('Nd_max', value, description)`
- `Na_max` -> Max acceptor doping; MATLAB: `model.param.set('Na_max', value, description)`
- `Nd_back` -> Background donor doping; MATLAB: `model.param.set('Nd_back', value, description)`
- `eta` -> Ideality factor; MATLAB: `model.param.set('eta', value, description)`
- `I0` -> Saturation current; MATLAB: `model.param.set('I0', value, description)`
- `Vac` -> Voltage amplitude; MATLAB: `model.param.set('Vac', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；fil1 / Fillet；pt1 / Point；geom1 / r1 / size；geom1 / r1 / pos
- 已识别参数：l_diode、d_diode、w_diode、d_p、w_anode、Nd_max、Na_max、Nd_back、eta、I0、Vac、f
...
- Sweep `l_diode` around the learned value `10[um]` and export target outputs to CSV.
- Sweep `d_diode` around the learned value `7[um]` and export target outputs to CSV.
- Sweep `w_diode` around the learned value `10[um]` and export target outputs to CSV.
- Sweep `d_p` around the learned value `1[um]` and export target outputs to CSV.
- Sweep `w_anode` around the learned value `4[um]` and export target outputs to CSV.
- Sweep `Nd_max` around the learned value `2e17[1/cm^3]` and export target outputs to CSV.
- Sweep `Na_max` around the learned value `1e17[1/cm^3]` and export target outputs to CSV.
- Sweep `Nd_back` around the learned value `1e15[1/cm^3]` and export target outputs to CSV.
...
- Summary: 已学习 pn_diode_circuit 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=14；内容证据=34 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 34 条；几何=10，物理场=0，材料=1，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pn_diode_circuit.m, pn_diode_circuit.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pn_diode_circuit，训练阶段为 modeling_logic_learning_ready。
...
- `pn_diode_circuit.mph` (comsol_mph, 50799592 bytes)
- `pn_diode_circuit.m` (matlab_livelink, 42460 bytes)
- `pn_diode_circuit.java` (comsol_java, 42392 bytes)
- `pn_diode_circuit_summary.json` (json, 1388 bytes)
- `models.semicond.pn_diode_circuit.pdf` (pdf_document, 334632 bytes)
