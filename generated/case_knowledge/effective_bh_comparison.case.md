# effective_bh_comparison

- Case directory: `D:\桌面\codex\案例下载\电气\有效 H-BB-H 曲线验证模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T07:20:00.514962+00:00`
...
- 模型树证据数量: 33
- 理论关键词: 未自动识别
- geometry: sq1 / Square, sq2 / Square, sq3 / Square, r1 / Rectangle, fil1 / Fillet, geom1 / sq1 / size, geom1 / sq1 / base, geom1 / sq2 / size, geom1 / sq2 / base, geom1 / sq3 / size, geom1 / sq3 / base, geom1 / r1 / size
- mesh: mesh1
- boundary_conditions: sq1 / size, sq1 / base, sq2 / size, sq2 / base, sq3 / size, sq3 / base, r1 / size, r1 / base, r1 / pos, fil1 / radius, fin / repairtoltype

### 文件内容证据

- `effective_bh_comparison.java`: 参数=5, 几何=16, 物理场=0, 研究=0, 结果=0
- `effective_bh_comparison.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `effective_bh_comparison.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `effective_bh_comparison_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: effective_bh_comparison 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- `core_width` -> 案例参数/扫描变量; MATLAB: `model.param.set('core_width', value, description)`
- `f0` -> 案例参数/扫描变量; MATLAB: `model.param.set('f0', value, description)`
- `Nturn` -> 案例参数/扫描变量; MATLAB: `model.param.set('Nturn', value, description)`
- `Ipeak` -> 案例参数/扫描变量; MATLAB: `model.param.set('Ipeak', value, description)`
...
- 已识别几何证据：sq1 / Square；sq2 / Square；sq3 / Square；r1 / Rectangle；fil1 / Fillet；geom1 / sq1 / size
- 已识别参数：core_width、f0、Nturn、Ipeak、t
...
- Sweep `core_width` around the learned value `15[mm]` and export target outputs to CSV.
- Sweep `f0` around the learned value `1000[Hz]` and export target outputs to CSV.
- Sweep `Nturn` around the learned value `1000` and export target outputs to CSV.
- Sweep `Ipeak` around the learned value `0.01[A]` and export target outputs to CSV.
...
- Summary: 已学习 effective_bh_comparison 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=33 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 33 条；几何=16，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：effective_bh_comparison.m, effective_bh_comparison.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 effective_bh_comparison，训练阶段为 modeling_logic_learning_ready。
...
- `effective_bh_comparison.mph` (comsol_mph, 88373956 bytes)
- `effective_bh_comparison.m` (matlab_livelink, 79865 bytes)
- `effective_bh_comparison.java` (comsol_java, 85343 bytes)
- `effective_bh_comparison_summary.json` (json, 1557 bytes)
