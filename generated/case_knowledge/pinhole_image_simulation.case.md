# pinhole_image_simulation

- Case directory: `D:\桌面\codex\案例下载\电气\日食期间针孔相机的光线追踪`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:32:10.141308+00:00`
...
- `aper` = `0.02[mm]` - Aperture radius
- `sun_dist_actual` = `1.5E8[m]` - Actual sun distance
- `moon_dist_actual` = `0.24E6[m]` - Actual moon distance
- `sun_plane_dist` = `1.5[m]` - Distance of sun plane
- `moon_plane_dist` = `0.5[m]` - Distance of moon plane
- `moon_ang_diam` = `0.515[deg]` - Moon ang. diameter
- `sun_ang_diam` = `0.533[deg]` - Sun ang. diameter
- `sun_plane_radius` = `sun_plane_dist*tan(sun_ang_diam/2)` - Radius of sun plane
- `moon_plane_radius` = `moon_plane_dist*tan(moon_ang_diam/2)` - Radius of moon plane
- `moon_displ` = `moon_plane_radius/2` - Moon plane displacement
- `image_plane_dist` = `14[in]` - Image plane distance
- `image_size` = `image_plane_dist*tan(sun_ang_diam/2)` - Image radius
- `image_plane_size` = `2*image_size` - Image plane radius
- `N_ray` = `N_ray_large` - Number of rays
- `N_ray_smol` = `1E4` - Small ray count (for testing)
- `N_ray_large` = `1E6` - Large ray count (for generating nice plots)

## 内容级读取结果

- 模型树证据数量: 34
- 理论关键词: 结构力学
- geometry: blk1 / Block, cyl1 / Cylinder, geom1 / blk1 / type, geom1 / blk1 / size, geom1 / blk1 / base, geom1 / blk1 / pos, geom1 / cyl1 / r, dif1 / Difference, del1 / Delete, geom1 / cyl1 / h
- mesh: mesh1, mesh2
- boundary_conditions: blk1 / type, blk1 / size, blk1 / base, blk1 / pos, cyl1 / r, cyl1 / h

### 文件内容证据

- `pinhole_image_simulation.java`: 参数=16, 几何=9, 物理场=0, 研究=0, 结果=0
- `pinhole_image_simulation.m`: 参数=16, 几何=7, 物理场=0, 研究=0, 结果=0
- `pinhole_image_simulation.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pinhole_image_simulation_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: pinhole_image_simulation 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics。
...
- PDF keywords: 结构力学
- `aper` -> Aperture radius; MATLAB: `model.param.set('aper', value, description)`
- `sun_dist_actual` -> Actual sun distance; MATLAB: `model.param.set('sun_dist_actual', value, description)`
- `moon_dist_actual` -> Actual moon distance; MATLAB: `model.param.set('moon_dist_actual', value, description)`
- `sun_plane_dist` -> Distance of sun plane; MATLAB: `model.param.set('sun_plane_dist', value, description)`
- `moon_plane_dist` -> Distance of moon plane; MATLAB: `model.param.set('moon_plane_dist', value, description)`
- `moon_ang_diam` -> Moon ang. diameter; MATLAB: `model.param.set('moon_ang_diam', value, description)`
- `sun_ang_diam` -> Sun ang. diameter; MATLAB: `model.param.set('sun_ang_diam', value, description)`
- `sun_plane_radius` -> Radius of sun plane; MATLAB: `model.param.set('sun_plane_radius', value, description)`
- `moon_plane_radius` -> Radius of moon plane; MATLAB: `model.param.set('moon_plane_radius', value, description)`
- `moon_displ` -> Moon plane displacement; MATLAB: `model.param.set('moon_displ', value, description)`
- `image_plane_dist` -> Image plane distance; MATLAB: `model.param.set('image_plane_dist', value, description)`
- `image_size` -> Image radius; MATLAB: `model.param.set('image_size', value, description)`
...
- 已识别几何证据：blk1 / Block；cyl1 / Cylinder；geom1 / blk1 / type；geom1 / blk1 / size；geom1 / blk1 / base；geom1 / blk1 / pos
- 已识别参数：aper、sun_dist_actual、moon_dist_actual、sun_plane_dist、moon_plane_dist、moon_ang_diam、sun_ang_diam、sun_plane_radius、moon_plane_radius、moon_displ、image_plane_dist、image_size
...
- Sweep `aper` around the learned value `0.02[mm]` and export target outputs to CSV.
- Sweep `sun_dist_actual` around the learned value `1.5E8[m]` and export target outputs to CSV.
- Sweep `moon_dist_actual` around the learned value `0.24E6[m]` and export target outputs to CSV.
- Sweep `sun_plane_dist` around the learned value `1.5[m]` and export target outputs to CSV.
- Sweep `moon_plane_dist` around the learned value `0.5[m]` and export target outputs to CSV.
- Sweep `moon_ang_diam` around the learned value `0.515[deg]` and export target outputs to CSV.
- Sweep `sun_ang_diam` around the learned value `0.533[deg]` and export target outputs to CSV.
- Sweep `sun_plane_radius` around the learned value `sun_plane_dist*tan(sun_ang_diam/2)` and export target outputs to CSV.
...
- Summary: 已学习 pinhole_image_simulation 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=16；内容证据=34 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 34 条；几何=10，物理场=0，材料=0，网格=2，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pinhole_image_simulation.m, pinhole_image_simulation.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pinhole_image_simulation，训练阶段为 modeling_logic_learning_ready。
...
- `pinhole_image_simulation.mph` (comsol_mph, 1333530 bytes)
- `pinhole_image_simulation.m` (matlab_livelink, 35615 bytes)
- `pinhole_image_simulation.java` (comsol_java, 37520 bytes)
- `pinhole_image_simulation_summary.json` (json, 1879 bytes)
