...
- Created at: `2026-07-06T09:36:53.948134+00:00`
...
- `pid_control.mph`: 参数=10, 几何=20, 物理场=3, 研究=2, 结果=0
- `pid_control_geom_sequence.java`: 参数=0, 几何=20, 物理场=0, 研究=0, 结果=0
- `pid_control_geom_sequence.mph`: 参数=0, 几何=20, 物理场=0, 研究=0, 结果=0
...
- `models.mph.pid_control.pdf`: models.mph.pid_control.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.pid_control.zh_CN.pdf`: models.mph.pid_control.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 使用 PID 控制器的过程控制 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Transient 研究。
- Primary physics: `Laminar Flow`
- Study type: `Transient`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Laminar Flow，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Transient 确认。

### Judgement Evidence

- 脚本/文本识别到的物理场: spf / LaminarFlow, tds / DilutedSpecies, ge1 / GlobalEquations
...
- PDF keywords: 流体, models.mph.pid_control.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.pid_control.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `v_in_top` -> 案例参数/扫描变量; MATLAB: `model.param.set('v_in_top', value, description)`
- `c_in_top` -> 案例参数/扫描变量; MATLAB: `model.param.set('c_in_top', value, description)`
- `c_in_inlet` -> 案例参数/扫描变量; MATLAB: `model.param.set('c_in_inlet', value, description)`
- `c00` -> 案例参数/扫描变量; MATLAB: `model.param.set('c00', value, description)`
- `D` -> 案例参数/扫描变量; MATLAB: `model.param.set('D', value, description)`
- `c_set` -> 案例参数/扫描变量; MATLAB: `model.param.set('c_set', value, description)`
- `k_P_ctrl` -> 案例参数/扫描变量; MATLAB: `model.param.set('k_P_ctrl', value, description)`
- `k_I_ctrl` -> 案例参数/扫描变量; MATLAB: `model.param.set('k_I_ctrl', value, description)`
- `k_D_ctrl` -> 案例参数/扫描变量; MATLAB: `model.param.set('k_D_ctrl', value, description)`
- `k_D_ctrl_temp_param` -> 案例参数/扫描变量; MATLAB: `model.param.set('k_D_ctrl_temp_param', value, description)`
...
- 已识别几何证据：ca1 / CircularArc；pol1 / Polygon；ca2 / CircularArc；ls1 / LineSegment；ca3 / CircularArc；pol2 / Polygon
- 已识别物理场证据：spf / LaminarFlow；tds / DilutedSpecies；ge1 / GlobalEquations
- 已识别参数：v_in_top、c_in_top、c_in_inlet、c00、D、c_set、k_P_ctrl、k_I_ctrl、k_D_ctrl、k_D_ctrl_temp_param
...
- Transfer recognized physics interfaces to similar requirements: spf / LaminarFlow, tds / DilutedSpecies, ge1 / GlobalEquations.
...
- Sweep `v_in_top` around the learned value `0.01[m/s]` and export target outputs to CSV.
- Sweep `c_in_top` around the learned value `1[mol/m^3]` and export target outputs to CSV.
- Sweep `c_in_inlet` around the learned value `0.2[mol/m^3]` and export target outputs to CSV.
- Sweep `c00` around the learned value `0.5[mol/m^3]` and export target outputs to CSV.
- Sweep `D` around the learned value `1e-4[m^2/s]` and export target outputs to CSV.
- Sweep `c_set` around the learned value `0.5[mol/m^3]` and export target outputs to CSV.
- Sweep `k_P_ctrl` around the learned value `-0.5[m^4/(mol*s)]` and export target outputs to CSV.
- Sweep `k_I_ctrl` around the learned value `-1[m^4/(mol*s^2)]` and export target outputs to CSV.
