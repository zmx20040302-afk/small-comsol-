...
- Case directory: `D:\桌面\codex\案例下载\电气\MOSCAP 的界面陷阱效应`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:42:03.441691+00:00`
...
- `t_epi` = `10[um]` - Epi layer thickness
- `t_sub` = `2[um]` - Substrate thickness
- `d_g` = `3.8e-2[cm]` - Gate diameter
- `area_g` = `pi*(d_g/2)^2` - Gate area
- `r_epi` = `0.75[ohm*cm]` - Epi layer resistivity
- `r_sub` = `0.005[ohm*cm]` - Substrate resistivity
- `Nd_epi` = `1/(e_const*1450[cm^2/V/s]*r_epi)` - Epi layer doping
- `Nd_sub` = `1/(e_const*1450[cm^2/V/s]*r_sub)` - Substrate doping
- `epsr_ox` = `3.9` - Oxide dielectric constant
- `t_ox` = `60[nm]` - Oxide thickness
- `C_ox` = `epsr_ox*epsilon0_const/t_ox*area_g` - Oxide capacitance
- `rhos_ox` = `e_const*9e11[cm^-2]` - Oxide fixed charge
- `Nss` = `2e11[cm^-2*eV^-1]` - Surface state density
- `Ew0` = `0.2[V]` - Width of continuous trapping levels
- `f0` = `50[Hz]` - AC frequency
- `phiM` = `4.5[V]` - Metal work function
- `v_th` = `1e7[cm/s]` - Thermal velocity
- `sigma_n` = `1e-15[cm^2]` - Electron capture cross section
- `sigma_p` = `2.2e-16[cm^2]` - Hole capture cross section
- `Vg` = `0[V]` - Gate voltage
- `Vac` = `1[mV]` - AC voltage
...
- `moscap_1d_interface_traps.java`: 参数=20, 几何=2, 物理场=0, 研究=0, 结果=0
- `models.semicond.moscap_1d_interface_traps.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `moscap_1d_interface_traps.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `moscap_1d_interface_traps.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `moscap_1d_interface_traps_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.moscap_1d_interface_traps.pdf`: models.semicond.moscap_1d_interface_traps.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion

## PDF Summary and Physics Judgement

- Simple summary: moscap_1d_interface_traps 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: 2 In the Select Physics tree, select Semiconductor > Semiconductor (semi).
- PDF: 4 Click Study.
...
- PDF keywords: 传热, 流体, 电磁, models.semicond.moscap_1d_interface_traps.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion
- `t_epi` -> Epi layer thickness; MATLAB: `model.param.set('t_epi', value, description)`
- `t_sub` -> Substrate thickness; MATLAB: `model.param.set('t_sub', value, description)`
- `d_g` -> Gate diameter; MATLAB: `model.param.set('d_g', value, description)`
- `area_g` -> Gate area; MATLAB: `model.param.set('area_g', value, description)`
- `r_epi` -> Epi layer resistivity; MATLAB: `model.param.set('r_epi', value, description)`
- `r_sub` -> Substrate resistivity; MATLAB: `model.param.set('r_sub', value, description)`
- `Nd_epi` -> Epi layer doping; MATLAB: `model.param.set('Nd_epi', value, description)`
- `Nd_sub` -> Substrate doping; MATLAB: `model.param.set('Nd_sub', value, description)`
- `epsr_ox` -> Oxide dielectric constant; MATLAB: `model.param.set('epsr_ox', value, description)`
- `t_ox` -> Oxide thickness; MATLAB: `model.param.set('t_ox', value, description)`
- `C_ox` -> Oxide capacitance; MATLAB: `model.param.set('C_ox', value, description)`
- `rhos_ox` -> Oxide fixed charge; MATLAB: `model.param.set('rhos_ox', value, description)`
...
- 已识别参数：t_epi、t_sub、d_g、area_g、r_epi、r_sub、Nd_epi、Nd_sub、epsr_ox、t_ox、C_ox、rhos_ox
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 流体, 电磁?
...
- Sweep `t_epi` around the learned value `10[um]` and export target outputs to CSV.
- Sweep `t_sub` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `d_g` around the learned value `3.8e-2[cm]` and export target outputs to CSV.
- Sweep `area_g` around the learned value `pi*(d_g/2)^2` and export target outputs to CSV.
- Sweep `r_epi` around the learned value `0.75[ohm*cm]` and export target outputs to CSV.
- Sweep `r_sub` around the learned value `0.005[ohm*cm]` and export target outputs to CSV.
- Sweep `Nd_epi` around the learned value `1/(e_const*1450[cm^2/V/s]*r_epi)` and export target outputs to CSV.
- Sweep `Nd_sub` around the learned value `1/(e_const*1450[cm^2/V/s]*r_sub)` and export target outputs to CSV.
...
- Summary: 已学习 moscap_1d_interface_traps 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=21；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=2，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：moscap_1d_interface_traps.m, moscap_1d_interface_traps.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 moscap_1d_interface_traps，训练阶段为 modeling_logic_learning_ready。
...
- `moscap_1d_interface_traps.mph` (comsol_mph, 7849802 bytes)
- `moscap_1d_interface_traps.m` (matlab_livelink, 65130 bytes)
- `moscap_1d_interface_traps.java` (comsol_java, 60297 bytes)
- `moscap_1d_interface_traps_summary.json` (json, 1976 bytes)
- `models.semicond.moscap_1d_interface_traps.pdf` (pdf_document, 355256 bytes)
