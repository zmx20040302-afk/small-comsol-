# ehd_flow_dielectric_liquid

- Case directory: `D:\桌面\codex\案例下载\电气\介电液体中的电流体动力学流动`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:56:26.113532+00:00`
...
- `sigma0` = `3E-8[S/m]` - Zero-field electric conductivity
- `rho_0` = `750[kg/m^3]` - Liquid density
- `eta` = `1.34e-3[Pa*s]` - Liquid viscosity
- `mu_i` = `2.8E-9[m^2/(V*s)]` - Ions mobility
- `alpha` = `2*mu_i*e_const/(epsilon0_const*eps_r)` - Recombination rate
- `V0` = `1000[V]` - Applied voltage
- `n0` = `sigma0/2/mu_i/e_const` - Zero-field ions number density

## 内容级读取结果

- 模型树证据数量: 28
- 理论关键词: 电磁, 流体, 传热, 结构力学
- geometry: sq1 / Square, c1 / Circle, uni1 / Union, del1 / Delete, geom1 / sq1 / size, geom1 / sq1 / layername, geom1 / sq1 / layerleft, geom1 / c1 / r, geom1 / c1 / pos, geom1 / c1 / layername
- physics: Electric Currents, Laminar Flow
- mesh: mesh1
- studies: Stationary
- boundary_conditions: sq1 / size, sq1 / layername, sq1 / layerleft, c1 / r, c1 / pos, c1 / layername

### 文件内容证据

- `ehd_flow_dielectric_liquid.java`: 参数=8, 几何=10, 物理场=0, 研究=0, 结果=0
- `models.edis.ehd_flow_dielectric_liquid.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `ehd_flow_dielectric_liquid_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `ehd_flow_dielectric_liquid.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `ehd_flow_dielectric_liquid.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.ehd_flow_dielectric_liquid.pdf`: models.edis.ehd_flow_dielectric_liquid.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Laminar Flow，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the resulting Coulomb-driven flows.

## PDF Summary and Physics Judgement

- Simple summary: ehd_flow_dielectric_liquid 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- PDF: the resulting Coulomb-driven flows.
- PDF: Simulation results show strong agreement with experimental data (Ref. 1).
- PDF: Results and Discussion
- 脚本/文本识别到的物理场: Electric Currents, Laminar Flow
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 流体, 传热, 结构力学
...
- PDF keywords: 电磁, 流体, 传热, 结构力学, models.edis.ehd_flow_dielectric_liquid.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Laminar Flow，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；the resulting Coulomb-driven flows.
- `eps_r` -> Relative permittivity; MATLAB: `model.param.set('eps_r', value, description)`
- `sigma0` -> Zero-field electric conductivity; MATLAB: `model.param.set('sigma0', value, description)`
- `rho_0` -> Liquid density; MATLAB: `model.param.set('rho_0', value, description)`
- `eta` -> Liquid viscosity; MATLAB: `model.param.set('eta', value, description)`
- `mu_i` -> Ions mobility; MATLAB: `model.param.set('mu_i', value, description)`
- `alpha` -> Recombination rate; MATLAB: `model.param.set('alpha', value, description)`
- `V0` -> Applied voltage; MATLAB: `model.param.set('V0', value, description)`
- `n0` -> Zero-field ions number density; MATLAB: `model.param.set('n0', value, description)`
...
- 已识别几何证据：sq1 / Square；c1 / Circle；uni1 / Union；del1 / Delete；geom1 / sq1 / size；geom1 / sq1 / layername
- 已识别物理场证据：Electric Currents；Laminar Flow
- 已识别参数：eps_r、sigma0、rho_0、eta、mu_i、alpha、V0、n0
...
- Transfer recognized physics interfaces to similar requirements: Electric Currents, Laminar Flow.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 流体, 传热, 结构力学?
...
- Sweep `eps_r` around the learned value `2` and export target outputs to CSV.
- Sweep `sigma0` around the learned value `3E-8[S/m]` and export target outputs to CSV.
- Sweep `rho_0` around the learned value `750[kg/m^3]` and export target outputs to CSV.
- Sweep `eta` around the learned value `1.34e-3[Pa*s]` and export target outputs to CSV.
- Sweep `mu_i` around the learned value `2.8E-9[m^2/(V*s)]` and export target outputs to CSV.
- Sweep `alpha` around the learned value `2*mu_i*e_const/(epsilon0_const*eps_r)` and export target outputs to CSV.
- Sweep `V0` around the learned value `1000[V]` and export target outputs to CSV.
- Sweep `n0` around the learned value `sigma0/2/mu_i/e_const` and export target outputs to CSV.
...
- Summary: 已学习 ehd_flow_dielectric_liquid 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=10，物理场=2，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：ehd_flow_dielectric_liquid.m, ehd_flow_dielectric_liquid.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 ehd_flow_dielectric_liquid，训练阶段为 modeling_logic_learning_ready。
...
- `ehd_flow_dielectric_liquid.mph` (comsol_mph, 47608272 bytes)
- `ehd_flow_dielectric_liquid.m` (matlab_livelink, 38856 bytes)
- `ehd_flow_dielectric_liquid.java` (comsol_java, 34697 bytes)
- `ehd_flow_dielectric_liquid_summary.json` (json, 1300 bytes)
- `models.edis.ehd_flow_dielectric_liquid.pdf` (pdf_document, 2025208 bytes)
