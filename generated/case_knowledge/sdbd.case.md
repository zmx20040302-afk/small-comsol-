...
- Case directory: `D:\桌面\codex\案例下载\电气\表面介质阻挡放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:11:58.278761+00:00`
...
- `V0` = `-15[kV]` - Applied voltage
- `z0` = `50[mm]` - Out-of-plane thickness

## 内容级读取结果

- 模型树证据数量: 32
- 理论关键词: 电磁, 传热, 流体
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, ls1 / LineSegment, mce1 / MeshControlEdges, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r1 / layername, geom1 / r1 / layer, geom1 / r2 / size, geom1 / r3 / size, geom1 / r3 / pos
- materials: mat1
- mesh: mesh1
- studies: Time Dependent
- boundary_conditions: r1 / size, r1 / pos, r1 / layername, r1 / layer, r2 / size, r3 / size, r3 / pos, ls1 / specify1, ls1 / coord1, ls1 / specify2, ls1 / coord2

### 文件内容证据

- `sdbd.java`: 参数=2, 几何=16, 物理场=0, 研究=0, 结果=0
- `models.edis.sdbd.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `sdbd.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `sdbd.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `sdbd_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.sdbd.pdf`: models.edis.sdbd.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion

## PDF Summary and Physics Judgement

- Simple summary: sdbd 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF keywords: 电磁, 传热, 流体, models.edis.sdbd.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion
- `V0` -> Applied voltage; MATLAB: `model.param.set('V0', value, description)`
- `z0` -> Out-of-plane thickness; MATLAB: `model.param.set('z0', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；ls1 / LineSegment；mce1 / MeshControlEdges；geom1 / r1 / size
- 已识别参数：V0、z0
...
- Sweep `V0` around the learned value `-15[kV]` and export target outputs to CSV.
- Sweep `z0` around the learned value `50[mm]` and export target outputs to CSV.
...
- Summary: 已学习 sdbd 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=32 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 32 条；几何=16，物理场=0，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：sdbd.m, sdbd.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 sdbd，训练阶段为 modeling_logic_learning_ready。
...
- `sdbd.mph` (comsol_mph, 109612227 bytes)
- `sdbd.m` (matlab_livelink, 33290 bytes)
- `sdbd.java` (comsol_java, 29346 bytes)
- `sdbd_summary.json` (json, 1000 bytes)
- `models.edis.sdbd.pdf` (pdf_document, 844684 bytes)
