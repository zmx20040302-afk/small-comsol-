...
- Case directory: `D:\桌面\codex\案例下载\电气\沟槽栅 IGBT 三维模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:14:48.231956+00:00`
...
- `Nv0` = `Nvcfac*Nv_` - Valence band DOS Shigyo
- `Nc0` = `Nvcfac*Nc_` - Conduction band DOS Shigyo
- `Nref0` = `6.5e16[cm^-3]` - Band gap narrowing reference concentration Shigyo
...
- `trench_gate_igbt_3d.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.semicond.trench_gate_igbt_3d.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `trench_gate_igbt_3d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `trench_gate_igbt_3d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `trench_gate_igbt_3d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.trench_gate_igbt_3d.pdf`: models.semicond.trench_gate_igbt_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；voltage agrees reasonably well with the published result.

## PDF Summary and Physics Judgement

- Simple summary: trench_gate_igbt_3d 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: voltage agrees reasonably well with the published result.
...
- PDF keywords: 传热, 电磁, models.semicond.trench_gate_igbt_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；voltage agrees reasonably well with the published result.
...
- Summary: 已学习 trench_gate_igbt_3d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=33；内容证据=31 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 31 条；几何=0，物理场=0，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：trench_gate_igbt_3d.m, trench_gate_igbt_3d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 trench_gate_igbt_3d，训练阶段为 modeling_logic_learning_ready。
...
- `trench_gate_igbt_3d.mph` (comsol_mph, 38996968 bytes)
- `trench_gate_igbt_3d.m` (matlab_livelink, 142606 bytes)
- `trench_gate_igbt_3d.java` (comsol_java, 144933 bytes)
- `trench_gate_igbt_3d_summary.json` (json, 2763 bytes)
- `models.semicond.trench_gate_igbt_3d.pdf` (pdf_document, 1654531 bytes)
