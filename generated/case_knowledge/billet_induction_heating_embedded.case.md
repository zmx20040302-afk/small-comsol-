# billet_induction_heating_embedded

- Case directory: `D:\桌面\codex\案例下载\电气\钢坯感应加热`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T15:25:31.538057+00:00`
...
- `h_coil` = `15[cm]` - Length of the coil
- `r_coil` = `10[cm]` - Inner coil radius
- `R_coil` = `13[cm]` - Outer coil radius
- `T_in` = `400[degC]` - Initial billet temperature
- `N_coil` = `10000` - Number of coil turns
- `u_trans` = `4[cm/s]` - Billet translational velocity
- `T_amb` = `20[degC]` - Ambient temperature
- `emissivity` = `0.5` - Billet surface emissivity
- `dx_part` = `0[m]` - Displacement (x)
- `dy_part` = `0[m]` - Displacement (y)
- `rot_part` = `0` - Rotation
- `f0` = `50[Hz]` - AC current frequency
- `a` = `12[cm]` - geometry input parameter
- `b` = `12[cm]` - geometry input parameter
- `r` = `6[cm]` - geometry input parameter

## 内容级读取结果

- 模型树证据数量: 58
- 理论关键词: 传热, 结构力学, 流体, 电磁
- geometry: part1 / Part, part2 / Part, part3 / Part, r1 / Rectangle, c1 / Circle, cyl1 / Cylinder, cyl2 / Cylinder, part1 / r1 / size, part1 / r1 / base, part1 / r1 / selresult, part1 / r1 / color, part1 / r1 / customcolor
- physics: Magnetic Fields, Heat Transfer
- mesh: mesh1, mesh2
- studies: Stationary, Frequency Domain
- boundary_conditions: r1 / size, r1 / base, r1 / selresult, r1 / color, r1 / customcolor, c1 / r, c1 / selresult, c1 / color, c1 / customcolor, cyl1 / r, cyl1 / h, cyl1 / axis

### 文件内容证据

- `billet_induction_heating_embedded.m`: 参数=15, 几何=20, 物理场=0, 研究=0, 结果=0
- `billet_induction_heating_embedded.java`: 参数=15, 几何=20, 物理场=0, 研究=0, 结果=0
- `applications.billet_induction_heating.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `billet.pptx`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `billet.pdf`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `billet_induction_heating_embedded.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `billet_induction_heating_embedded_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `applications.billet_induction_heating.pdf`: applications.billet_induction_heating.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Application created in COMSOL Multiphysics 6.4；• The Reset button restores the application inputs and geometry to the default (initial)
- `billet.pdf`: billet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields；核心内容可概括为： A steel billet travels through；three 10.000-turn coils

## PDF Summary and Physics Judgement

- Simple summary: billet_induction_heating_embedded 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: • The Reset button restores the application inputs and geometry to the default (initial)
- PDF:  The Geometry button rebuilds the complete 3D geometry, including the billet, the coil,
- PDF:  The Cross Section button rebuilds the 2D cross section geometry.
...
- PDF keywords: 传热, 结构力学, 流体, 电磁, applications.billet_induction_heating.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Application created in COMSOL Multiphysics 6.4；• The Reset button restores the application inputs and geometry to the default (initial), billet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields；核心内容可概括为： A steel billet travels through；three 10.000-turn coils
- `h_coil` -> Length of the coil; MATLAB: `model.param.set('h_coil', value, description)`
- `r_coil` -> Inner coil radius; MATLAB: `model.param.set('r_coil', value, description)`
- `R_coil` -> Outer coil radius; MATLAB: `model.param.set('R_coil', value, description)`
- `T_in` -> Initial billet temperature; MATLAB: `model.param.set('T_in', value, description)`
- `N_coil` -> Number of coil turns; MATLAB: `model.param.set('N_coil', value, description)`
- `u_trans` -> Billet translational velocity; MATLAB: `model.param.set('u_trans', value, description)`
- `T_amb` -> Ambient temperature; MATLAB: `model.param.set('T_amb', value, description)`
- `emissivity` -> Billet surface emissivity; MATLAB: `model.param.set('emissivity', value, description)`
- `dx_part` -> Displacement (x); MATLAB: `model.param.set('dx_part', value, description)`
- `dy_part` -> Displacement (y); MATLAB: `model.param.set('dy_part', value, description)`
- `rot_part` -> Rotation; MATLAB: `model.param.set('rot_part', value, description)`
- `f0` -> AC current frequency; MATLAB: `model.param.set('f0', value, description)`
...
- 已识别几何证据：part1 / Part；part2 / Part；part3 / Part；r1 / Rectangle；c1 / Circle；cyl1 / Cylinder
- 已识别物理场证据：Magnetic Fields；Heat Transfer
- 已识别参数：h_coil、r_coil、R_coil、T_in、N_coil、u_trans、T_amb、emissivity、dx_part、dy_part、rot_part、f0
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 结构力学, 流体, 电磁?
...
- Sweep `h_coil` around the learned value `15[cm]` and export target outputs to CSV.
- Sweep `r_coil` around the learned value `10[cm]` and export target outputs to CSV.
- Sweep `R_coil` around the learned value `13[cm]` and export target outputs to CSV.
- Sweep `T_in` around the learned value `400[degC]` and export target outputs to CSV.
- Sweep `N_coil` around the learned value `10000` and export target outputs to CSV.
- Sweep `u_trans` around the learned value `4[cm/s]` and export target outputs to CSV.
- Sweep `T_amb` around the learned value `20[degC]` and export target outputs to CSV.
- Sweep `emissivity` around the learned value `0.5` and export target outputs to CSV.
...
- Summary: 已学习 billet_induction_heating_embedded 的 7 个文件；训练阶段=modeling_logic_learning_ready；参数数量=15；内容证据=58 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 58 条；几何=22，物理场=2，材料=0，网格=2，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：billet_induction_heating_embedded.m, billet_induction_heating_embedded.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 billet_induction_heating_embedded，训练阶段为 modeling_logic_learning_ready。
...
- `billet_induction_heating_embedded.mph` (comsol_mph, 73132354 bytes)
- `billet_induction_heating_embedded.m` (matlab_livelink, 52801 bytes)
- `billet_induction_heating_embedded.java` (comsol_java, 56259 bytes)
- `billet_induction_heating_embedded_summary.json` (json, 2821 bytes)
- `billet.pptx` (presentation_document, 799848 bytes)
- `applications.billet_induction_heating.pdf` (pdf_document, 342311 bytes)
- `billet.pdf` (pdf_document, 95580 bytes)
