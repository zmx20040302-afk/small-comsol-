...
- `mur_magn` = `3.5` - Relative permittivity of magnetite
- `c_magn` = `0.25` - Magnetite concentration in ore
- `Mr_magn` = `60[A/m]` - Remanent magnetization of magnetite
- `H0` = `48163[nT]/mu0_const` - Geomagnetic field
- `Incl` = `59.357[deg]` - Local inclination
- `Decl` = `12.275[deg]` - Local declination

## 内容级读取结果

- 模型树证据数量: 23
- 理论关键词: 电磁, 多孔介质/裂隙, 结构力学
- geometry: imp1 / Import, elp1 / Ellipsoid, geom1 / imp1 / type, geom1 / imp1 / filename, geom1 / elp1 / semiaxes, geom1 / elp1 / pos
- physics: mfnc / MagnetostaticsNoCurrents, Electric Currents, Magnetic Fields
- materials: mat1
- mesh: mesh1
- studies: Stationary
- boundary_conditions: mfcs1 / 3, imp1 / type, imp1 / filename, elp1 / semiaxes, elp1 / pos

### 文件内容证据

- `magnetic_prospecting.java`: 参数=6, 几何=6, 物理场=1, 研究=0, 结果=0
- `models.acdc.magnetic_prospecting.pdf`: 参数=0, 几何=0, 物理场=2, 研究=1, 结果=0
- `magnetic_prospecting.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetic_prospecting.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetic_prospecting_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: magnetic_prospecting 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- 脚本/文本识别到的物理场: mfnc / MagnetostaticsNoCurrents, Electric Currents, Magnetic Fields
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 多孔介质/裂隙, 结构力学
...
- `mur_magn` -> Relative permittivity of magnetite; MATLAB: `model.param.set('mur_magn', value, description)`
- `c_magn` -> Magnetite concentration in ore; MATLAB: `model.param.set('c_magn', value, description)`
- `Mr_magn` -> Remanent magnetization of magnetite; MATLAB: `model.param.set('Mr_magn', value, description)`
- `H0` -> Geomagnetic field; MATLAB: `model.param.set('H0', value, description)`
- `Incl` -> Local inclination; MATLAB: `model.param.set('Incl', value, description)`
- `Decl` -> Local declination; MATLAB: `model.param.set('Decl', value, description)`
...
- 已识别几何证据：imp1 / Import；elp1 / Ellipsoid；geom1 / imp1 / type；geom1 / imp1 / filename；geom1 / elp1 / semiaxes；geom1 / elp1 / pos
- 已识别物理场证据：mfnc / MagnetostaticsNoCurrents；Electric Currents；Magnetic Fields
- 已识别参数：mur_magn、c_magn、Mr_magn、H0、Incl、Decl
...
- Transfer recognized physics interfaces to similar requirements: mfnc / MagnetostaticsNoCurrents, Electric Currents, Magnetic Fields.
...
- Sweep `mur_magn` around the learned value `3.5` and export target outputs to CSV.
- Sweep `c_magn` around the learned value `0.25` and export target outputs to CSV.
- Sweep `Mr_magn` around the learned value `60[A/m]` and export target outputs to CSV.
- Sweep `H0` around the learned value `48163[nT]/mu0_const` and export target outputs to CSV.
- Sweep `Incl` around the learned value `59.357[deg]` and export target outputs to CSV.
- Sweep `Decl` around the learned value `12.275[deg]` and export target outputs to CSV.
...
- Summary: 已学习 magnetic_prospecting 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=23 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 23 条；几何=6，物理场=3，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：magnetic_prospecting.m, magnetic_prospecting.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 magnetic_prospecting，训练阶段为 modeling_logic_learning_ready。
...
- `magnetic_prospecting.mph` (comsol_mph, 14202309 bytes)
- `magnetic_prospecting.m` (matlab_livelink, 15417 bytes)
- `magnetic_prospecting.java` (comsol_java, 11884 bytes)
- `magnetic_prospecting_summary.json` (json, 959 bytes)
