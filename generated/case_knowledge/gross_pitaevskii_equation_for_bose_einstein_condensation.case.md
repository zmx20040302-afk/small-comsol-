...
- Case directory: `D:\桌面\codex\案例下载\电气\玻色-爱因斯坦凝聚 Gross-Pitaevskii 方程`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:43:53.715320+00:00`
...
- `w0` = `2*pi*10[Hz]` - Trap frequency
- `ma` = `86.909[g/mol]/N_A_const` - Atomic mass of Rb-87
- `as` = `5.2[nm]` - Scattering length
- `U0` = `4*pi*hbar_const^2*as/ma` - Interaction strength
- `N0` = `1` - Number of atoms
- `wr` = `w0*sqrt(3)` - Transverse trap frequency
- `Rr0` = `(15*U0*w0*N0/(4*pi*ma*wr^3))^0.2` - Transverse T-F size
- `Rz0` = `(15*U0*wr^2*N0/(4*pi*ma*w0^4))^0.2` - Longitudinal T-F size
- `rho0` = `15*N0/(8*pi*Rr0^2*Rz0)` - T-F peak density

## 内容级读取结果

- 模型树证据数量: 20
- 理论关键词: PDE/方程建模, 流体
- geometry: c1 / Circle, geom1 / c1 / r, geom1 / c1 / angle, geom1 / c1 / rot
- physics: schr / SchrodingerEquation
- mesh: mesh1
- studies: Stationary, Eigenvalue
- boundary_conditions: c1 / r, c1 / angle, c1 / rot

### 文件内容证据

- `gross_pitaevskii_equation_for_bose_einstein_condensation.java`: 参数=9, 几何=4, 物理场=1, 研究=0, 结果=0
- `models.semicond.gross_pitaevskii_equation_for_bose_einstein_condensation.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `gross_pitaevskii_equation_for_bose_einstein_condensation_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `gross_pitaevskii_equation_for_bose_einstein_condensation.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `gross_pitaevskii_equation_for_bose_einstein_condensation.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.gross_pitaevskii_equation_for_bose_einstein_condensation.pdf`: models.semicond.gross_pitaevskii_equation_for_bose_einstein_condensation.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 流体，研究类型线索为 Stationary、Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Einstein condensate in a harmonic trap, using the Schrödinger Equation physics interface

## PDF Summary and Physics Judgement

- Simple summary: gross_pitaevskii_equation_for_bose_einstein_condensation 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Coefficient Form PDE，并采用 Eigenvalue 研究。
...
- PDF: Einstein condensate in a harmonic trap, using the Schrödinger Equation physics interface
- PDF: particle density. The eigenvalue study is not suitable for solving this kind of nonlinear
- PDF: eigenvalue problems. Instead, a stationary study is used with a global equation enforcing
- 脚本/文本识别到的物理场: schr / SchrodingerEquation
- 识别到的研究类型: Stationary, Eigenvalue
- 理论关键词: PDE/方程建模, 流体

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: PDE/方程建模, 流体, models.semicond.gross_pitaevskii_equation_for_bose_einstein_condensation.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 流体，研究类型线索为 Stationary、Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Einstein condensate in a harmonic trap, using the Schrödinger Equation physics interface
- `w0` -> Trap frequency; MATLAB: `model.param.set('w0', value, description)`
- `ma` -> Atomic mass of Rb-87; MATLAB: `model.param.set('ma', value, description)`
- `as` -> Scattering length; MATLAB: `model.param.set('as', value, description)`
- `U0` -> Interaction strength; MATLAB: `model.param.set('U0', value, description)`
- `N0` -> Number of atoms; MATLAB: `model.param.set('N0', value, description)`
- `wr` -> Transverse trap frequency; MATLAB: `model.param.set('wr', value, description)`
- `Rr0` -> Transverse T-F size; MATLAB: `model.param.set('Rr0', value, description)`
- `Rz0` -> Longitudinal T-F size; MATLAB: `model.param.set('Rz0', value, description)`
- `rho0` -> T-F peak density; MATLAB: `model.param.set('rho0', value, description)`
...
- 已识别几何证据：c1 / Circle；geom1 / c1 / r；geom1 / c1 / angle；geom1 / c1 / rot
- 已识别物理场证据：schr / SchrodingerEquation
- 已识别参数：w0、ma、as、U0、N0、wr、Rr0、Rz0、rho0
...
- Transfer recognized physics interfaces to similar requirements: schr / SchrodingerEquation.
- Reuse geometry construction patterns when new requirements share shape, import, array, sweep, or boolean operations.
- Use the learned study types as solver starting points: Stationary, Eigenvalue.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: PDE/方程建模, 流体?
...
- Sweep `w0` around the learned value `2*pi*10[Hz]` and export target outputs to CSV.
- Sweep `ma` around the learned value `86.909[g/mol]/N_A_const` and export target outputs to CSV.
- Sweep `as` around the learned value `5.2[nm]` and export target outputs to CSV.
- Sweep `U0` around the learned value `4*pi*hbar_const^2*as/ma` and export target outputs to CSV.
- Sweep `N0` around the learned value `1` and export target outputs to CSV.
- Sweep `wr` around the learned value `w0*sqrt(3)` and export target outputs to CSV.
- Sweep `Rr0` around the learned value `(15*U0*w0*N0/(4*pi*ma*wr^3))^0.2` and export target outputs to CSV.
- Sweep `Rz0` around the learned value `(15*U0*wr^2*N0/(4*pi*ma*w0^4))^0.2` and export target outputs to CSV.
...
- Summary: 已学习 gross_pitaevskii_equation_for_bose_einstein_condensation 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=20 条。
...
- 9 extracted parameters with values/descriptions.
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=4，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：gross_pitaevskii_equation_for_bose_einstein_condensation.m, gross_pitaevskii_equation_for_bose_einstein_condensation.java
  - Judgement: 脚本中的 COMSOL API 调用可用于恢复模型树顺序，并辅助生成 LiveLink MATLAB 自动建模脚本。
- **4. 参数与约束提取**
  - Evidence: 从文本、MATLAB 或 Java 证据中提取到 9 个参数。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 gross_pitaevskii_equation_for_bose_einstein_condensation，训练阶段为 modeling_logic_learning_ready。
...
- `gross_pitaevskii_equation_for_bose_einstein_condensation.mph` (comsol_mph, 3196447 bytes)
- `gross_pitaevskii_equation_for_bose_einstein_condensation.m` (matlab_livelink, 33761 bytes)
- `gross_pitaevskii_equation_for_bose_einstein_condensation.java` (comsol_java, 27036 bytes)
- `gross_pitaevskii_equation_for_bose_einstein_condensation_summary.json` (json, 1285 bytes)
- `models.semicond.gross_pitaevskii_equation_for_bose_einstein_condensation.pdf` (pdf_document, 619709 bytes)
