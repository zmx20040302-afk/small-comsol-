# magnetic_coupling_radial

- Case directory: `D:\桌面\codex\案例下载\电气\径向磁耦合器三维模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:56:00.740762+00:00`
...
- `alpha` = `0[deg]` - Rotation angle
- `gap` = `9[mm]` - Gap between rings
- `r_in` = `30[mm]` - Inner radius of inner ring
- `r_thick` = `10[mm]` - Ring thickness
- `r_out` = `50 [mm]` - Outer radius of outer ring
- `B_r_norm` = `1.42[T]` - Remanent flux density
- `phi_sec` = `22.5[deg]` - Halbach array angular period
- `height` = `15[mm]` - Height of the magnets
- `no_segs` = `32` - Number of segments of Halbach Array
...
- mesh: mesh1
- boundary_conditions: wp1 / quickz, wp1 / unite, c1 / r, c1 / angle, c2 / r, c2 / angle

### 文件内容证据

- `magnetic_coupling_radial.java`: 参数=9, 几何=6, 物理场=0, 研究=0, 结果=0
- `magnetic_coupling_radial.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetic_coupling_radial.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `magnetic_coupling_radial_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: magnetic_coupling_radial 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF keywords: 电磁
- `alpha` -> Rotation angle; MATLAB: `model.param.set('alpha', value, description)`
- `gap` -> Gap between rings; MATLAB: `model.param.set('gap', value, description)`
- `r_in` -> Inner radius of inner ring; MATLAB: `model.param.set('r_in', value, description)`
- `r_thick` -> Ring thickness; MATLAB: `model.param.set('r_thick', value, description)`
- `r_out` -> Outer radius of outer ring; MATLAB: `model.param.set('r_out', value, description)`
- `B_r_norm` -> Remanent flux density; MATLAB: `model.param.set('B_r_norm', value, description)`
- `phi_sec` -> Halbach array angular period; MATLAB: `model.param.set('phi_sec', value, description)`
- `height` -> Height of the magnets; MATLAB: `model.param.set('height', value, description)`
- `no_segs` -> Number of segments of Halbach Array; MATLAB: `model.param.set('no_segs', value, description)`
...
- 已识别参数：alpha、gap、r_in、r_thick、r_out、B_r_norm、phi_sec、height、no_segs
...
- Sweep `gap` around the learned value `9[mm]` and export target outputs to CSV.
- Sweep `r_in` around the learned value `30[mm]` and export target outputs to CSV.
- Sweep `r_thick` around the learned value `10[mm]` and export target outputs to CSV.
- Sweep `r_out` around the learned value `50 [mm]` and export target outputs to CSV.
- Sweep `B_r_norm` around the learned value `1.42[T]` and export target outputs to CSV.
- Sweep `phi_sec` around the learned value `22.5[deg]` and export target outputs to CSV.
- Sweep `height` around the learned value `15[mm]` and export target outputs to CSV.
...
- Summary: 已学习 magnetic_coupling_radial 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=9；内容证据=22 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 22 条；几何=6，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：magnetic_coupling_radial.m, magnetic_coupling_radial.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 magnetic_coupling_radial，训练阶段为 modeling_logic_learning_ready。
...
- `magnetic_coupling_radial.mph` (comsol_mph, 76517100 bytes)
- `magnetic_coupling_radial.m` (matlab_livelink, 50830 bytes)
- `magnetic_coupling_radial.java` (comsol_java, 48883 bytes)
- `magnetic_coupling_radial_summary.json` (json, 1134 bytes)
