# drift_diffusion_tutorial

- Case directory: `D:\桌面\codex\案例下载\电气\漂移扩散教学模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:41:30.642467+00:00`
...
- `drift_diffusion_tutorial.java`: 参数=0, 几何=2, 物理场=0, 研究=0, 结果=0
- `drift_diffusion_tutorial.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `drift_diffusion_tutorial.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `drift_diffusion_tutorial_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: drift_diffusion_tutorial 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- Summary: 已学习 drift_diffusion_tutorial 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=4 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 4 条；几何=2，物理场=0，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：drift_diffusion_tutorial.m, drift_diffusion_tutorial.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 drift_diffusion_tutorial，训练阶段为 modeling_logic_learning_ready。
...
- `drift_diffusion_tutorial.mph` (comsol_mph, 5131310 bytes)
- `drift_diffusion_tutorial.m` (matlab_livelink, 16782 bytes)
- `drift_diffusion_tutorial.java` (comsol_java, 13624 bytes)
- `drift_diffusion_tutorial_summary.json` (json, 773 bytes)
