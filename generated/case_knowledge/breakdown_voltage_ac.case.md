# breakdown_voltage_ac

- Case directory: `D:\桌面\codex\案例下载\电气\空气中平行电极的交流击穿电压`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:36:28.567634+00:00`
...
- `V0` = `50[kV]` - Maximum voltage
- `d` = `1[cm]` - Gap length
- `f` = `50[Hz]` - Frequency
- `t` = `1/f/100` - Time
- `Vapp` = `V0*(t*f/10)*sin(2*pi*f*t)` - Applied voltage
...
- physics: edis / ElectricDischarge, Electric Currents
- materials: mat1
- mesh: mesh1, edg1
- studies: Time Dependent, Parametric Sweep
- boundary_conditions: ece1 / 1, ece2 / 2, i1 / coord

### 文件内容证据

- `breakdown_voltage_ac.java`: 参数=5, 几何=2, 物理场=1, 研究=0, 结果=0
- `models.edis.breakdown_voltage_ac.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `breakdown_voltage_ac_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `breakdown_voltage_ac.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `breakdown_voltage_ac.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.breakdown_voltage_ac.pdf`: models.edis.breakdown_voltage_ac.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Time Dependent、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；this process, the model conducts a transient study with an AC voltage whose amplitude is

## PDF Summary and Physics Judgement

- Simple summary: breakdown_voltage_ac 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: this process, the model conducts a transient study with an AC voltage whose amplitude is
- PDF: Results and Discussion
- PDF: three frequencies. The results indicate that the breakdown voltage are largely independent
- 脚本/文本识别到的物理场: edis / ElectricDischarge, Electric Currents
...
- PDF keywords: 电磁, 传热, 结构力学, 流体, 优化/参数扫描, models.edis.breakdown_voltage_ac.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、结构力学、流体，研究类型线索为 Time Dependent、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；this process, the model conducts a transient study with an AC voltage whose amplitude is
- `V0` -> Maximum voltage; MATLAB: `model.param.set('V0', value, description)`
- `d` -> Gap length; MATLAB: `model.param.set('d', value, description)`
...
- 已识别物理场证据：edis / ElectricDischarge；Electric Currents
- 已识别参数：V0、d、f、t、Vapp
...
- Transfer recognized physics interfaces to similar requirements: edis / ElectricDischarge, Electric Currents.
...
- Sweep `V0` around the learned value `50[kV]` and export target outputs to CSV.
...
- Sweep `t` around the learned value `1/f/100` and export target outputs to CSV.
- Sweep `Vapp` around the learned value `V0*(t*f/10)*sin(2*pi*f*t)` and export target outputs to CSV.
...
- Summary: 已学习 breakdown_voltage_ac 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=17 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 17 条；几何=2，物理场=2，材料=1，网格=2，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：breakdown_voltage_ac.m, breakdown_voltage_ac.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 breakdown_voltage_ac，训练阶段为 modeling_logic_learning_ready。
...
- `breakdown_voltage_ac.mph` (comsol_mph, 8833498 bytes)
- `breakdown_voltage_ac.m` (matlab_livelink, 32069 bytes)
- `breakdown_voltage_ac.java` (comsol_java, 28654 bytes)
- `breakdown_voltage_ac_summary.json` (json, 1032 bytes)
- `models.edis.breakdown_voltage_ac.pdf` (pdf_document, 219363 bytes)
