...
- Case directory: `D:\桌面\codex\案例下载\COMSOL\球对称传递`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-07-06T09:42:13.528046+00:00`
...
- physics: g / GeneralFormPDE, General Form PDE, Heat Transfer
- mesh: mesh1, sca1, edg1, mesh1 / sca1 / scale
- studies: std1 / time / Transient, std1
- results: pg1, lngr1
- boundary_conditions: flux1 / 1, flux2 / 2, lngr1 / 1, gfeq1 / Ga, init1 / T, sca1 / scale, time / tlist, lngr1 / xdata, lngr1 / xdataexpr, lngr1 / expr, flux1 / [1], flux2 / [2]

### 文件内容证据

- `spherically_symmetric_transport.mph`: 参数=8, 几何=1, 物理场=3, 研究=2, 结果=2
- `spherically_symmetric_transport.java`: 参数=8, 几何=1, 物理场=3, 研究=2, 结果=2
- `spherically_symmetric_transport.m`: 参数=8, 几何=1, 物理场=3, 研究=1, 结果=2
- `models.mph.spherically_symmetric_transport.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mph.spherically_symmetric_transport.zh_CN.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `spherically_symmetric_transport.zh_CN.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mph.spherically_symmetric_transport.pdf`: models.mph.spherically_symmetric_transport.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.spherically_symmetric_transport.zh_CN.pdf`: models.mph.spherically_symmetric_transport.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 球对称传递 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Transient 研究。
...
- 脚本/文本识别到的物理场: g / GeneralFormPDE, General Form PDE, Heat Transfer
...
- `rho` -> 案例参数/扫描变量; MATLAB: `model.param.set('rho', value, description)`
- `cp` -> 案例参数/扫描变量; MATLAB: `model.param.set('cp', value, description)`
- `k` -> 案例参数/扫描变量; MATLAB: `model.param.set('k', value, description)`
- `Rp` -> 案例参数/扫描变量; MATLAB: `model.param.set('Rp', value, description)`
- `Qs` -> 案例参数/扫描变量; MATLAB: `model.param.set('Qs', value, description)`
- `hs` -> 案例参数/扫描变量; MATLAB: `model.param.set('hs', value, description)`
- `Text` -> 案例参数/扫描变量; MATLAB: `model.param.set('Text', value, description)`
- `Tinit` -> 案例参数/扫描变量; MATLAB: `model.param.set('Tinit', value, description)`
...
- 已识别几何证据：i1 / Interval
- 已识别物理场证据：g / GeneralFormPDE；General Form PDE；Heat Transfer
- 已识别参数：rho、cp、k、Rp、Qs、hs、Text、Tinit
...
- Transfer recognized physics interfaces to similar requirements: g / GeneralFormPDE, General Form PDE, Heat Transfer.
...
- Use detected result nodes as candidate validation and training targets: pg1, lngr1.
...
- Sweep `rho` around the learned value `2000` and export target outputs to CSV.
- Sweep `cp` around the learned value `300` and export target outputs to CSV.
- Sweep `k` around the learned value `0.5` and export target outputs to CSV.
- Sweep `Rp` around the learned value `0.005` and export target outputs to CSV.
- Sweep `Qs` around the learned value `0` and export target outputs to CSV.
- Sweep `hs` around the learned value `1000` and export target outputs to CSV.
- Sweep `Text` around the learned value `368` and export target outputs to CSV.
- Sweep `Tinit` around the learned value `298` and export target outputs to CSV.
...
- Summary: 已学习 球对称传递 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=33 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 33 条；几何=1，物理场=3，材料=0，网格=4，研究=2，结果=2。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：spherically_symmetric_transport.java, spherically_symmetric_transport.m
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 球对称传递，训练阶段为 modeling_logic_learning_ready。
...
- `球对称传递` score=0.2977; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=29
- `管式反应器代理模型 App` score=0.1812; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `热执行器代理模型 App` score=0.1521; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42

### Official Documentation Checks

- `IntroductionToLiveLinkForMATLAB.pdf` page 5; module=LiveLink_for_MATLAB; score=0.1117
- `IntroductionToCOMSOLMultiphysics.pdf` page 262; module=COMSOL_Multiphysics; score=0.1076
...
- `models.mph.spherically_symmetric_transport.pdf` (pdf_document, 421597 bytes)
- `models.mph.spherically_symmetric_transport.zh_CN.pdf` (pdf_document, 1769898 bytes)
- `spherically_symmetric_transport.java` (comsol_java, 5596 bytes)
- `spherically_symmetric_transport.m` (matlab_livelink, 7596 bytes)
- `spherically_symmetric_transport.mph` (comsol_mph, 752385 bytes)
- `spherically_symmetric_transport.zh_CN.mph` (comsol_mph, 766870 bytes)
