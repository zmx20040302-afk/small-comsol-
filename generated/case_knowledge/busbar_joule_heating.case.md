...
- `I_dc` = `1[kA]` - Busbar current (DC)
- `I_rms` = `I_dc` - Busbar current (AC, RMS value)
- `I_ac` = `I_rms*sqrt(2)` - Busbar current (AC, peak value)
- `Freq` = `50[Hz]` - AC excitation frequency
- `hfc_air` = `50[W/(m^2*K)]` - Heat transfer coefficient, forced convection

## 内容级读取结果

- 模型树证据数量: 28
- 理论关键词: 传热, 电磁
- geometry: imp1 / Import, sel1 / ExplicitSelection, comsel1 / ComplementSelection, unisel1 / UnionSelection, blk1 / Block, uni1 / Union, comsel2 / ComplementSelection, geom1 / imp1 / type, geom1 / imp1 / filename, geom1 / comsel1 / input, geom1 / unisel1 / input, geom1 / blk1 / size
- physics: Heat Transfer
- mesh: mesh1
- boundary_conditions: imp1 / type, imp1 / filename, comsel1 / input, unisel1 / input, blk1 / size, blk1 / base, comsel2 / input

### 文件内容证据

- `busbar_joule_heating.m`: 参数=5, 几何=14, 物理场=1, 研究=0, 结果=0
- `busbar_joule_heating.java`: 参数=5, 几何=13, 物理场=1, 研究=0, 结果=0
- `busbar_joule_heating.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `busbar_joule_heating_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: busbar_joule_heating 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF keywords: 传热, 电磁
- `I_dc` -> Busbar current (DC); MATLAB: `model.param.set('I_dc', value, description)`
- `I_rms` -> Busbar current (AC, RMS value); MATLAB: `model.param.set('I_rms', value, description)`
- `I_ac` -> Busbar current (AC, peak value); MATLAB: `model.param.set('I_ac', value, description)`
- `Freq` -> AC excitation frequency; MATLAB: `model.param.set('Freq', value, description)`
- `hfc_air` -> Heat transfer coefficient, forced convection; MATLAB: `model.param.set('hfc_air', value, description)`
...
- 已识别几何证据：imp1 / Import；sel1 / ExplicitSelection；comsel1 / ComplementSelection；unisel1 / UnionSelection；blk1 / Block；uni1 / Union
- 已识别物理场证据：Heat Transfer
- 已识别参数：I_dc、I_rms、I_ac、Freq、hfc_air
...
- Sweep `I_dc` around the learned value `1[kA]` and export target outputs to CSV.
- Sweep `I_rms` around the learned value `I_dc` and export target outputs to CSV.
- Sweep `I_ac` around the learned value `I_rms*sqrt(2)` and export target outputs to CSV.
- Sweep `Freq` around the learned value `50[Hz]` and export target outputs to CSV.
- Sweep `hfc_air` around the learned value `50[W/(m^2*K)]` and export target outputs to CSV.
...
- Summary: 已学习 busbar_joule_heating 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=14，物理场=1，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：busbar_joule_heating.m, busbar_joule_heating.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 busbar_joule_heating，训练阶段为 modeling_logic_learning_ready。
...
- `busbar_joule_heating.mph` (comsol_mph, 29953410 bytes)
- `busbar_joule_heating.m` (matlab_livelink, 54023 bytes)
- `busbar_joule_heating.java` (comsol_java, 57249 bytes)
- `busbar_joule_heating_summary.json` (json, 2236 bytes)
