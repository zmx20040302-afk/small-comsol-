...
- Created at: `2026-08-20T07:16:59.697043+00:00`
...
- 模型树证据数量: 15
- 理论关键词: 结构力学, 声学, 传热, 流体, 电磁
- geometry: i1 / Interval, i2 / Interval, geom1 / i1 / coord, geom1 / i2 / coord
- physics: plas / ColdPlasma, Solid Mechanics, Electrostatics
- mesh: mesh1
- studies: Eigenvalue, Eigenfrequency, Time Dependent
- boundary_conditions: gnd1 / 1, i1 / coord, i2 / coord

### 文件内容证据

- `streamer_1d.java`: 参数=1, 几何=4, 物理场=1, 研究=0, 结果=0
...
- `streamer_1d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `streamer_1d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `streamer_1d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: streamer_1d 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Eigenvalue 研究。
...
- 脚本/文本识别到的物理场: plas / ColdPlasma, Solid Mechanics, Electrostatics
...
- 已识别几何证据：i1 / Interval；i2 / Interval；geom1 / i1 / coord；geom1 / i2 / coord
- 已识别物理场证据：plas / ColdPlasma；Solid Mechanics；Electrostatics
- 已识别参数：Efield
...
- Transfer recognized physics interfaces to similar requirements: plas / ColdPlasma, Solid Mechanics, Electrostatics.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 结构力学, 声学, 传热, 流体, 电磁?
...
- Sweep `Efield` around the learned value `-100[kV/cm]` and export target outputs to CSV.
...
- Summary: 已学习 streamer_1d 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=1；内容证据=15 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 15 条；几何=4，物理场=3，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：streamer_1d.m, streamer_1d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 streamer_1d，训练阶段为 modeling_logic_learning_ready。
...
- `streamer_1d.mph` (comsol_mph, 3373834 bytes)
- `streamer_1d.m` (matlab_livelink, 37017 bytes)
- `streamer_1d.java` (comsol_java, 30187 bytes)
- `streamer_1d_summary.json` (json, 807 bytes)
