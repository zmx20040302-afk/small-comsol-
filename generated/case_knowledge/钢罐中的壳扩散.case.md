...
- Created at: `2026-07-06T09:45:27.564977+00:00`
...
- `shell_diffusion.mph`: 参数=2, 几何=13, 物理场=1, 研究=3, 结果=4
- `shell_diffusion_geom_sequence.m`: 参数=0, 几何=13, 物理场=0, 研究=0, 结果=0
- `shell_diffusion_geom_sequence.mph`: 参数=0, 几何=13, 物理场=0, 研究=0, 结果=0
- `shell_diffusion_geom_sequence.java`: 参数=0, 几何=13, 物理场=0, 研究=0, 结果=0
- `shell_diffusion.java`: 参数=2, 几何=0, 物理场=1, 研究=3, 结果=4
- `shell_diffusion.m`: 参数=2, 几何=0, 物理场=1, 研究=2, 结果=4
...
- `models.mph.shell_diffusion.pdf`: models.mph.shell_diffusion.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
- `models.mph.shell_diffusion.zh_CN.pdf`: models.mph.shell_diffusion.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 钢罐中的壳扩散 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 CoefficientFormBoundaryPDE，并采用 Stationary 研究。
- Primary physics: `CoefficientFormBoundaryPDE`
- Study type: `Stationary`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 CoefficientFormBoundaryPDE，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Stationary 确认。
...
- PDF keywords: 电磁, models.mph.shell_diffusion.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber., models.mph.shell_diffusion.zh_CN.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
...
- 已识别几何证据：wp1 / WorkPlane；pol1 / Polygon；fil1 / Fillet；wp2 / WorkPlane；c1 / Circle；par1 / Partition
...
- Use detected result nodes as candidate validation and training targets: pg1, surf1, arws1, pg2.
...
- Sweep `sigma` around the learned value `4.032e6[S/m]` and export target outputs to CSV.
- Sweep `d` around the learned value `1[mm]` and export target outputs to CSV.
...
- Summary: 已学习 钢罐中的壳扩散 的 10 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=49 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 49 条；几何=13，物理场=1，材料=0，网格=1，研究=3，结果=4。
