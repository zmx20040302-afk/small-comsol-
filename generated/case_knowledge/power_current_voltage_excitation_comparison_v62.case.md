# power_current_voltage_excitation_comparison_v62

- Case directory: `D:\桌面\codex\案例下载\电气\射频热损耗公式比较`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:14:57.237119+00:00`
...
- `I0` = `10[mA]`
- `eps_insulator` = `1.75`
- `Z0_coax` = `(Z0_const/(2*pi*sqrt(eps_insulator)))*log(r_outer/r_inner)`
- `Z_small` = `Z0_coax/1000`

## 内容级读取结果

- 模型树证据数量: 25
- 理论关键词: 电磁
- geometry: wp1 / WorkPlane, r1 / Rectangle, r2 / Rectangle, geom1 / wp1 / quickplane, geom1 / wp1 / unite, r3 / Rectangle
- physics: Electromagnetic Waves
- mesh: mesh1
- boundary_conditions: wp1 / quickplane, wp1 / unite, r1 / size, r1 / layerbottom, r2 / size, r2 / pos, r2 / layername, r2 / layerbottom, r2 / layertop

### 文件内容证据

- `power_current_voltage_excitation_comparison_v62.m`: 参数=8, 几何=5, 物理场=0, 研究=0, 结果=0
- `power_current_voltage_excitation_comparison_v62.java`: 参数=8, 几何=6, 物理场=0, 研究=0, 结果=0
- `power_current_voltage_excitation_comparison_v62_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `power_current_voltage_excitation_comparison_v62.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: power_current_voltage_excitation_comparison_v62 主要研究电流、电势或电磁场分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electromagnetic Waves。
...
- PDF keywords: 电磁
- `r_inner` -> 案例参数/扫描变量; MATLAB: `model.param.set('r_inner', value, description)`
- `r_outer` -> 案例参数/扫描变量; MATLAB: `model.param.set('r_outer', value, description)`
...
- `eps_insulator` -> 案例参数/扫描变量; MATLAB: `model.param.set('eps_insulator', value, description)`
- `Z0_coax` -> 案例参数/扫描变量; MATLAB: `model.param.set('Z0_coax', value, description)`
- `Z_small` -> 案例参数/扫描变量; MATLAB: `model.param.set('Z_small', value, description)`
...
- 已识别几何证据：wp1 / WorkPlane；r1 / Rectangle；r2 / Rectangle；geom1 / wp1 / quickplane；geom1 / wp1 / unite；r3 / Rectangle
- 已识别物理场证据：Electromagnetic Waves
- 已识别参数：r_inner、r_outer、V0、P0、I0、eps_insulator、Z0_coax、Z_small
...
- Sweep `r_inner` around the learned value `0.25[mm]` and export target outputs to CSV.
- Sweep `r_outer` around the learned value `0.75[mm]` and export target outputs to CSV.
...
- Sweep `I0` around the learned value `10[mA]` and export target outputs to CSV.
- Sweep `eps_insulator` around the learned value `1.75` and export target outputs to CSV.
- Sweep `Z0_coax` around the learned value `(Z0_const/(2*pi*sqrt(eps_insulator)))*log(r_outer/r_inner)` and export target outputs to CSV.
- Sweep `Z_small` around the learned value `Z0_coax/1000` and export target outputs to CSV.
...
- Summary: 已学习 power_current_voltage_excitation_comparison_v62 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=8；内容证据=25 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 25 条；几何=6，物理场=1，材料=0，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：power_current_voltage_excitation_comparison_v62.m, power_current_voltage_excitation_comparison_v62.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 power_current_voltage_excitation_comparison_v62，训练阶段为 modeling_logic_learning_ready。
...
- `power_current_voltage_excitation_comparison_v62.mph` (comsol_mph, 3409881 bytes)
- `power_current_voltage_excitation_comparison_v62.m` (matlab_livelink, 70068 bytes)
- `power_current_voltage_excitation_comparison_v62.java` (comsol_java, 75370 bytes)
- `power_current_voltage_excitation_comparison_v62_summary.json` (json, 1596 bytes)
