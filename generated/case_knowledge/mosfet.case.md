...
- Case directory: `D:\桌面\codex\案例下载\电气\MOS 晶体管 (MOSFET) 的直流特性`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T15:36:53.275913+00:00`
...
- `mosfet.java`: 参数=2, 几何=6, 物理场=0, 研究=0, 结果=0
- `models.semicond.mosfet.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `mosfet.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mosfet.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mosfet_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.mosfet.pdf`: models.semicond.mosfet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁、优化/参数扫描，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；is controlled by the gate voltage. Typically a larger gate voltage results in wider channel

## PDF Summary and Physics Judgement

- Simple summary: mosfet 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: is controlled by the gate voltage. Typically a larger gate voltage results in wider channel
- PDF: Figure 3 shows the model geometry, indicating how the geometry elements correspond
- PDF: and the voltages applied to the drain and the gate are varied. In the first study a small
...
- PDF keywords: 电磁, 传热, 优化/参数扫描, models.semicond.mosfet.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁、优化/参数扫描，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；is controlled by the gate voltage. Typically a larger gate voltage results in wider channel
...
- Summary: 已学习 mosfet 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=15 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 15 条；几何=6，物理场=0，材料=1，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mosfet.m, mosfet.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mosfet，训练阶段为 modeling_logic_learning_ready。
...
- `mosfet.mph` (comsol_mph, 13768867 bytes)
- `mosfet.m` (matlab_livelink, 44550 bytes)
- `mosfet.java` (comsol_java, 42789 bytes)
- `mosfet_summary.json` (json, 1228 bytes)
- `models.semicond.mosfet.pdf` (pdf_document, 1482617 bytes)
