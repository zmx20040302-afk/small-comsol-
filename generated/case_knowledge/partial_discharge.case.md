...
- Case directory: `D:\桌面\codex\案例下载\电气\固体电介质内的局部放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:47:17.812024+00:00`
...
- 模型树证据数量: 25
- 理论关键词: 电磁, 流体
- geometry: r1 / Rectangle, c1 / Circle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / c1 / r, geom1 / c1 / angle, geom1 / c1 / rot
- physics: edis / ElectricDischarge, Electrostatics
- materials: mat1
- mesh: mesh1
- studies: Time Dependent
- boundary_conditions: gas1, solid1, ece1 / 6, ece2 / 2, r1 / size, r1 / pos, c1 / r, c1 / angle, c1 / rot

### 文件内容证据

- `partial_discharge.java`: 参数=4, 几何=7, 物理场=1, 研究=0, 结果=0
- `models.edis.partial_discharge.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `partial_discharge.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `partial_discharge.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `partial_discharge_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.partial_discharge.pdf`: models.edis.partial_discharge.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；dielectrics. As a result, small voids, cracks, or imperfections within solid insulation can give

## PDF Summary and Physics Judgement

- Simple summary: partial_discharge 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 ElectricDischarge，并采用 Time Dependent 研究。
- Primary physics: `ElectricDischarge`
- Study type: `Time Dependent`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 ElectricDischarge，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Time Dependent 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: dielectrics. As a result, small voids, cracks, or imperfections within solid insulation can give
- PDF: applied voltage cycle. These results can be compared directly with experimental
- PDF: measurements, offering a valuable tool for studying the mechanisms of partial discharge
- 脚本/文本识别到的物理场: edis / ElectricDischarge, Electrostatics
...
- PDF keywords: 电磁, 流体, models.edis.partial_discharge.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；dielectrics. As a result, small voids, cracks, or imperfections within solid insulation can give
...
- 已识别几何证据：r1 / Rectangle；c1 / Circle；geom1 / r1 / size；geom1 / r1 / pos；geom1 / c1 / r；geom1 / c1 / angle
- 已识别物理场证据：edis / ElectricDischarge；Electrostatics
- 已识别参数：V0、f、t、Va
...
- 识别到物理场证据：edis / ElectricDischarge
...
- Transfer recognized physics interfaces to similar requirements: edis / ElectricDischarge, Electrostatics.
...
- Sweep `V0` around the learned value `12[kV]` and export target outputs to CSV.
- Sweep `f` around the learned value `50[Hz]` and export target outputs to CSV.
- Sweep `t` around the learned value `0[s]` and export target outputs to CSV.
- Sweep `Va` around the learned value `V0*sin(2*pi*f*t)` and export target outputs to CSV.
...
- Summary: 已学习 partial_discharge 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=25 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 25 条；几何=7，物理场=2，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：partial_discharge.m, partial_discharge.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 partial_discharge，训练阶段为 modeling_logic_learning_ready。
...
- `partial_discharge.mph` (comsol_mph, 9980032 bytes)
- `partial_discharge.m` (matlab_livelink, 29478 bytes)
- `partial_discharge.java` (comsol_java, 24293 bytes)
- `partial_discharge_summary.json` (json, 1017 bytes)
- `models.edis.partial_discharge.pdf` (pdf_document, 1455081 bytes)
