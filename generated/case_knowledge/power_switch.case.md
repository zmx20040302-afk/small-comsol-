...
- Case directory: `D:\桌面\codex\案例下载\电气\电源开关的电动力学`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:15:11.261532+00:00`
...
- `a_coil` = `1[mm^2]` - Coil wire section
- `filling` = `0.9` - Coil filling factor
- `mass` = `400[g]` - Plunger mass
- `k0` = `2[N/mm]` - Spring constant
...
- `power_switch.java`: 参数=18, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.acdc.power_switch.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `power_switch.pptx`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `power_switch.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `power_switch.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `power_switch_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.power_switch.pdf`: models.acdc.power_switch.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Global ODEs and DAEs，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry of the electromechanical device is shown in Figure 1. Two bulk E-shaped

## PDF Summary and Physics Judgement

- Simple summary: power_switch 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF keywords: 结构力学, 电磁, 流体, 多孔介质/裂隙, models.acdc.power_switch.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields、Global ODEs and DAEs，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry of the electromechanical device is shown in Figure 1. Two bulk E-shaped
...
- Summary: 已学习 power_switch 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=17；内容证据=22 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 22 条；几何=0，物理场=2，材料=0，网格=0，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：power_switch.m, power_switch.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 power_switch，训练阶段为 modeling_logic_learning_ready。
...
- `power_switch.mph` (comsol_mph, 68981179 bytes)
- `power_switch.m` (matlab_livelink, 81008 bytes)
- `power_switch.java` (comsol_java, 69150 bytes)
- `power_switch_summary.json` (json, 1480 bytes)
- `power_switch.pptx` (presentation_document, 2397398 bytes)
- `models.acdc.power_switch.pdf` (pdf_document, 3293308 bytes)
