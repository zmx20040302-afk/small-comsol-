...
- `zset` = `100[nm]` - Set point z-coordinate

## 内容级读取结果

- 模型树证据数量: 37
- 理论关键词: 电磁, 声学, 结构力学, 优化/参数扫描
- geometry: blk1 / Block, wp1 / WorkPlane, r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, geom1 / blk1 / size, geom1 / blk1 / pos, geom1 / blk1 / layername, geom1 / blk1 / layer, geom1 / blk1 / layerfront, geom1 / blk1 / layerback, geom1 / blk1 / layerbottom
...
- `biased_resonator_3d_pull_in.java`: 参数=2, 几何=15, 物理场=0, 研究=0, 结果=0
...
- `biased_resonator_3d_pull_in_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=3, 结果=0
- `biased_resonator_3d_pull_in.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `biased_resonator_3d_pull_in.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: biased_resonator_3d_pull_in 主要研究结构受力、变形或强度响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenfrequency 研究。
...
- `zset` -> Set point z-coordinate; MATLAB: `model.param.set('zset', value, description)`
...
- 已识别几何证据：blk1 / Block；wp1 / WorkPlane；r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；geom1 / blk1 / size
...
- Sweep `zset` around the learned value `100[nm]` and export target outputs to CSV.
...
- Summary: 已学习 biased_resonator_3d_pull_in 的 8 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=37 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 37 条；几何=15，物理场=2，材料=0，网格=1，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：biased_resonator_3d_pull_in.m, biased_resonator_3d_pull_in.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 biased_resonator_3d_pull_in，训练阶段为 modeling_logic_learning_ready。
...
- `biased_resonator_3d_pull_in.mph` (comsol_mph, 52105756 bytes)
- `biased_resonator_3d_pull_in.m` (matlab_livelink, 71442 bytes)
- `biased_resonator_3d_pull_in.java` (comsol_java, 73295 bytes)
- `biased_resonator_3d_pull_in_summary.json` (json, 1364 bytes)
