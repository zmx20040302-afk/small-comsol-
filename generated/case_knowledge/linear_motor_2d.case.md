...
- Case directory: `D:\桌面\codex\案例下载\电气\直线电机二维模型`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:08:00.296915+00:00`
...
- `Np` = `10` - Nr of poles on moving part/rotor
- `Ns` = `12` - Nr of stator slots
- `v_lin` = `0.5[m/s]` - Linear motion velocity
- `I_ph` = `2.5[A]` - Phase current rms
- `Nturn` = `120` - Nr of turns per coil
- `r_inc` = `25[mm]` - Incision/ computational plane radius (axial flux motor)
- `s_rot` = `v_lin/(2*pi*r_inc)` - Rotational speed (axial flux motor)
- `f_el` = `s_rot*Np/2` - Electrical frequency
- `ang_el_init` = `90[deg]` - Initial current angle (max force/ torque)
- `L_mag` = `20[mm]` - Length into plane/ radial extent of magnetic materials
...
- `linear_motor_2d.java`: 参数=11, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.acdc.linear_motor_2d.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `linear_motor_2d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `linear_motor_2d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `linear_motor_2d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.linear_motor_2d.pdf`: models.acdc.linear_motor_2d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Figure 2: 2D geometry of linear motor with magnetic steel materials in grey, coils in orange

## PDF Summary and Physics Judgement

- Simple summary: linear_motor_2d 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: Figure 2: 2D geometry of linear motor with magnetic steel materials in grey, coils in orange
- PDF: In terms of a linear motor, the geometry represents a periodic section of the machine. In
- PDF: terms of an axial flux motor, the geometry can represent the entire circular cross section.
...
- PDF keywords: 流体, 电磁, 结构力学, models.acdc.linear_motor_2d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Figure 2: 2D geometry of linear motor with magnetic steel materials in grey, coils in orange
- `Np` -> Nr of poles on moving part/rotor; MATLAB: `model.param.set('Np', value, description)`
- `Ns` -> Nr of stator slots; MATLAB: `model.param.set('Ns', value, description)`
- `v_lin` -> Linear motion velocity; MATLAB: `model.param.set('v_lin', value, description)`
- `I_ph` -> Phase current rms; MATLAB: `model.param.set('I_ph', value, description)`
- `Nturn` -> Nr of turns per coil; MATLAB: `model.param.set('Nturn', value, description)`
- `r_inc` -> Incision/ computational plane radius (axial flux motor); MATLAB: `model.param.set('r_inc', value, description)`
- `s_rot` -> Rotational speed (axial flux motor); MATLAB: `model.param.set('s_rot', value, description)`
- `f_el` -> Electrical frequency; MATLAB: `model.param.set('f_el', value, description)`
- `ang_el_init` -> Initial current angle (max force/ torque); MATLAB: `model.param.set('ang_el_init', value, description)`
- `L_mag` -> Length into plane/ radial extent of magnetic materials; MATLAB: `model.param.set('L_mag', value, description)`
- `t` -> Time variable alias; MATLAB: `model.param.set('t', value, description)`
...
- 已识别参数：Np、Ns、v_lin、I_ph、Nturn、r_inc、s_rot、f_el、ang_el_init、L_mag、t
...
- Sweep `v_lin` around the learned value `0.5[m/s]` and export target outputs to CSV.
- Sweep `I_ph` around the learned value `2.5[A]` and export target outputs to CSV.
- Sweep `Nturn` around the learned value `120` and export target outputs to CSV.
- Sweep `r_inc` around the learned value `25[mm]` and export target outputs to CSV.
- Sweep `s_rot` around the learned value `v_lin/(2*pi*r_inc)` and export target outputs to CSV.
- Sweep `f_el` around the learned value `s_rot*Np/2` and export target outputs to CSV.
...
- Summary: 已学习 linear_motor_2d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=11；内容证据=14 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 14 条；几何=0，物理场=1，材料=0，网格=0，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：linear_motor_2d.m, linear_motor_2d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 linear_motor_2d，训练阶段为 modeling_logic_learning_ready。
...
- `linear_motor_2d.mph` (comsol_mph, 36022080 bytes)
- `linear_motor_2d.m` (matlab_livelink, 69883 bytes)
- `linear_motor_2d.java` (comsol_java, 68764 bytes)
- `linear_motor_2d_summary.json` (json, 3089 bytes)
- `models.acdc.linear_motor_2d.pdf` (pdf_document, 858004 bytes)
