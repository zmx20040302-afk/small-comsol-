...
- Created at: `2026-07-06T09:42:41.282225+00:00`
...
- `cell_thermal_runaway.mph`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `cell_thermal_runaway.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: 电芯热失控 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF keywords: 传热, 电磁
- `d_can` -> Thickness of battery canister; MATLAB: `model.param.set('d_can', value, description)`
- `r_batt` -> Battery radius; MATLAB: `model.param.set('r_batt', value, description)`
- `h_batt` -> Battery height; MATLAB: `model.param.set('h_batt', value, description)`
- `L_neg` -> Length of negative electrode; MATLAB: `model.param.set('L_neg', value, description)`
- `L_sep` -> Length of separator; MATLAB: `model.param.set('L_sep', value, description)`
- `L_pos` -> Length of positive electrode; MATLAB: `model.param.set('L_pos', value, description)`
- `L_neg_cc` -> Negative current collector thickness; MATLAB: `model.param.set('L_neg_cc', value, description)`
- `L_pos_cc` -> Positive current collector thickness; MATLAB: `model.param.set('L_pos_cc', value, description)`
- `L_batt` -> Cell thickness; MATLAB: `model.param.set('L_batt', value, description)`
- `kT_pos` -> Positive electrode thermal conductivity; MATLAB: `model.param.set('kT_pos', value, description)`
- `kT_neg` -> Negative electrode thermal conductivity; MATLAB: `model.param.set('kT_neg', value, description)`
- `kT_pos_cc` -> Positive current collector thermal conductivity; MATLAB: `model.param.set('kT_pos_cc', value, description)`
...
- 已识别参数：d_can、r_batt、h_batt、L_neg、L_sep、L_pos、L_neg_cc、L_pos_cc、L_batt、kT_pos、kT_neg、kT_pos_cc
...
- Sweep `d_can` around the learned value `0.5[mm]` and export target outputs to CSV.
- Sweep `r_batt` around the learned value `9[mm]` and export target outputs to CSV.
- Sweep `h_batt` around the learned value `65[mm]` and export target outputs to CSV.
- Sweep `L_neg` around the learned value `55e-6[m]` and export target outputs to CSV.
- Sweep `L_sep` around the learned value `30e-6[m]` and export target outputs to CSV.
- Sweep `L_pos` around the learned value `55e-6[m]` and export target outputs to CSV.
- Sweep `L_neg_cc` around the learned value `7[um]` and export target outputs to CSV.
- Sweep `L_pos_cc` around the learned value `10[um]` and export target outputs to CSV.
