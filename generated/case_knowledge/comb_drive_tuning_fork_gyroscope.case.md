...
- Case directory: `D:\桌面\codex\案例下载\电气\梳驱动音叉式微速率陀螺仪`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T03:06:39.900533+00:00`
...
- `comb_drive_tuning_fork_gyroscope.java`: 参数=20, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mems.comb_drive_tuning_fork_gyroscope.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `comb_drive_tuning_fork_gyroscope.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `comb_drive_tuning_fork_gyroscope.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `comb_drive_tuning_fork_gyroscope_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.comb_drive_tuning_fork_gyroscope.pdf`: models.mems.comb_drive_tuning_fork_gyroscope.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；geometry, extensive use of selection features, implementation of analytic formulas for the

## PDF Summary and Physics Judgement

- Simple summary: comb_drive_tuning_fork_gyroscope 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Stationary 研究。
...
- PDF: geometry, extensive use of selection features, implementation of analytic formulas for the
- PDF: electromechanical forces and response estimation, and comparison of numerical results
- PDF: The geometry of the device is shown in the figure below. All dimensions and numbers of
...
- PDF keywords: 电磁, 结构力学, 优化/参数扫描, models.mems.comb_drive_tuning_fork_gyroscope.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；geometry, extensive use of selection features, implementation of analytic formulas for the
...
- Summary: 已学习 comb_drive_tuning_fork_gyroscope 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=26；内容证据=28 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 28 条；几何=0，物理场=1，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：comb_drive_tuning_fork_gyroscope.m, comb_drive_tuning_fork_gyroscope.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 comb_drive_tuning_fork_gyroscope，训练阶段为 modeling_logic_learning_ready。
...
- `comb_drive_tuning_fork_gyroscope.mph` (comsol_mph, 341590615 bytes)
- `comb_drive_tuning_fork_gyroscope.m` (matlab_livelink, 100813 bytes)
- `comb_drive_tuning_fork_gyroscope.java` (comsol_java, 102492 bytes)
- `comb_drive_tuning_fork_gyroscope_summary.json` (json, 3665 bytes)
- `models.mems.comb_drive_tuning_fork_gyroscope.pdf` (pdf_document, 3573283 bytes)
