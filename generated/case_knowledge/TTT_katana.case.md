...
- Case directory: `D:\桌面\codex\案例下载\电气\刀具差速淬火`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:15:55.327329+00:00`
...
- `Tpara` = `1[K]` - TTT temperature sweep parameter
...
- `TTT_katana.m`: 参数=1, 几何=0, 物理场=0, 研究=0, 结果=0
- `TTT_katana.java`: 参数=1, 几何=0, 物理场=0, 研究=0, 结果=0
- `TTT_katana.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `TTT_katana_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: TTT_katana 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- PDF keywords: 传热
- `Tpara` -> TTT temperature sweep parameter; MATLAB: `model.param.set('Tpara', value, description)`
...
- Sweep `Tpara` around the learned value `1[K]` and export target outputs to CSV.
...
- Summary: 已学习 TTT_katana 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=1；内容证据=1 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 1 条；几何=0，物理场=0，材料=0，网格=0，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：TTT_katana.m, TTT_katana.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 TTT_katana，训练阶段为 modeling_logic_learning_ready。
...
- `TTT_katana.mph` (comsol_mph, 8580361 bytes)
- `TTT_katana.m` (matlab_livelink, 31708 bytes)
- `TTT_katana.java` (comsol_java, 36227 bytes)
- `TTT_katana_summary.json` (json, 732 bytes)
