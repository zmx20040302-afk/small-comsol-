# effective_nonlinear_magnetic_curves_embedded

- Case directory: `D:\桌面\codex\案例下载\电气\有效非线性磁曲线计算器`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:27:30.365816+00:00`
...
- `Bmax` = `2.4` - Maximum value of the B field in the curve
- `Hmax` = `347507.836` - Maximum value of the H field in the curve
- `f0` = `1[Hz]` - Dummy frequency used for averaging
- `T` = `1/f0` - Dummy period used for averaging
- `tol` = `1e-5` - Integration tolerance
...
- `effective_nonlinear_magnetic_curves_embedded.java`: 参数=5, 几何=2, 物理场=1, 研究=0, 结果=0
- `effective_nonlinear_magnetic_curves_embedded.m`: 参数=5, 几何=2, 物理场=0, 研究=0, 结果=0
- `applications.effective_nonlinear_magnetic_curves.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `effective_nonlinear_magnetic_curves_embedded.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `effective_nonlinear_magnetic_curves_embedded_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `applications.effective_nonlinear_magnetic_curves.pdf`: applications.effective_nonlinear_magnetic_curves.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁、多孔介质/裂隙，研究类型线索为 Frequency Domain；核心内容可概括为：Application created in COMSOL Multiphysics 6.4；feature in COMSOL Multiphysics. This application can be used to compute such

## PDF Summary and Physics Judgement

- Simple summary: effective_nonlinear_magnetic_curves_embedded 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Coefficient Form PDE，并采用 Frequency Domain 研究。
...
- PDF: feature in COMSOL Multiphysics. This application can be used to compute such
- PDF: visualizes them in a plot. Finally, the results can be exported either as a text file, which can
- PDF: which can be loaded in COMSOL Multiphysics. These features can then be used when
...
- PDF keywords: 电磁, 多孔介质/裂隙, applications.effective_nonlinear_magnetic_curves.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁、多孔介质/裂隙，研究类型线索为 Frequency Domain；核心内容可概括为：Application created in COMSOL Multiphysics 6.4；feature in COMSOL Multiphysics. This application can be used to compute such
- `Bmax` -> Maximum value of the B field in the curve; MATLAB: `model.param.set('Bmax', value, description)`
- `Hmax` -> Maximum value of the H field in the curve; MATLAB: `model.param.set('Hmax', value, description)`
- `f0` -> Dummy frequency used for averaging; MATLAB: `model.param.set('f0', value, description)`
- `T` -> Dummy period used for averaging; MATLAB: `model.param.set('T', value, description)`
- `tol` -> Integration tolerance; MATLAB: `model.param.set('tol', value, description)`
...
- Sweep `Bmax` around the learned value `2.4` and export target outputs to CSV.
- Sweep `Hmax` around the learned value `347507.836` and export target outputs to CSV.
- Sweep `f0` around the learned value `1[Hz]` and export target outputs to CSV.
- Sweep `T` around the learned value `1/f0` and export target outputs to CSV.
- Sweep `tol` around the learned value `1e-5` and export target outputs to CSV.
...
- Summary: 已学习 effective_nonlinear_magnetic_curves_embedded 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=5；内容证据=11 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 11 条；几何=2，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：effective_nonlinear_magnetic_curves_embedded.m, effective_nonlinear_magnetic_curves_embedded.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 effective_nonlinear_magnetic_curves_embedded，训练阶段为 modeling_logic_learning_ready。
...
- `effective_nonlinear_magnetic_curves_embedded.mph` (comsol_mph, 1044096 bytes)
- `effective_nonlinear_magnetic_curves_embedded.m` (matlab_livelink, 33524 bytes)
- `effective_nonlinear_magnetic_curves_embedded.java` (comsol_java, 35823 bytes)
- `effective_nonlinear_magnetic_curves_embedded_summary.json` (json, 966 bytes)
- `applications.effective_nonlinear_magnetic_curves.pdf` (pdf_document, 374899 bytes)
