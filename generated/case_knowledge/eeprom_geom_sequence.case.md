...
- 模型树证据数量: 46
- 理论关键词: 传热, 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, uni1 / Union, r4 / Rectangle, r5 / Rectangle, r6 / Rectangle, r7 / Rectangle, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r2 / pos
- mesh: mesh1
- studies: Stationary, Time Dependent
- boundary_conditions: r1 / size, r1 / pos, r2 / size, r2 / pos, r3 / size, r3 / pos, uni1 / intbnd, r4 / size, r4 / pos, r5 / size, r5 / pos, r6 / size

### 文件内容证据

- `eeprom_geom_sequence.java`: 参数=0, 几何=20, 物理场=0, 研究=0, 结果=0
- `eeprom_geom_sequence.m`: 参数=0, 几何=20, 物理场=0, 研究=0, 结果=0
- `models.semicond.eeprom.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `eeprom_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `eeprom_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: eeprom_geom_sequence 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF keywords: 传热, 电磁, models.semicond.eeprom.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Erasable Programmable Read-Only Memory (EEPROM) device. A stationary study
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；uni1 / Union；r4 / Rectangle；r5 / Rectangle
...
- Summary: 已学习 eeprom_geom_sequence 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=46 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 46 条；几何=26，物理场=0，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：eeprom_geom_sequence.m, eeprom_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 eeprom_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `eeprom_geom_sequence.mph` (comsol_mph, 262725 bytes)
- `eeprom_geom_sequence.m` (matlab_livelink, 7742 bytes)
- `eeprom_geom_sequence.java` (comsol_java, 7823 bytes)
- `eeprom_geom_sequence_summary.json` (json, 641 bytes)
