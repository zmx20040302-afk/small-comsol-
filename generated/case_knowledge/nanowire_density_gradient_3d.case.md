...
- Case directory: `D:\桌面\codex\案例下载\电气\纳米线 MOSFET 的密度梯度三维仿真`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:19:45.578492+00:00`
...
- `Lgate` = `4[nm]` - Gate length
- `Wchannel` = `3.2[nm]` - Channel width
- `dOx` = `0.8[nm]` - Oxide thickness
- `Lsource` = `15[nm]` - Source length
- `Ldrain` = `15[nm]` - Drain length
- `Ltot` = `Lsource+Lgate+Ldrain` - Total length
- `Wtot` = `Wchannel+dOx*2` - Total width
...
- `nanowire_density_gradient_3d.java`: 参数=7, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.semicond.nanowire_density_gradient_3d.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `nanowire_density_gradient_3d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `nanowire_density_gradient_3d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `nanowire_density_gradient_3d_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.nanowire_density_gradient_3d.pdf`: models.semicond.nanowire_density_gradient_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；geometric domains, and quantum confinement at the silicon–oxide interface is accounted

## PDF Summary and Physics Judgement

- Simple summary: nanowire_density_gradient_3d 主要研究量子能级或波函数分布，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: geometric domains, and quantum confinement at the silicon–oxide interface is accounted
- PDF: Various selection utilities are used to simplify the assignment of physics settings and plot
- PDF: selections. The result matches well with the Id–Vg curves and electron density profiles
- 识别到的研究类型: Stationary
- 理论关键词: 传热, 电磁
...
- PDF keywords: 传热, 电磁, models.semicond.nanowire_density_gradient_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；geometric domains, and quantum confinement at the silicon–oxide interface is accounted
- `Lgate` -> Gate length; MATLAB: `model.param.set('Lgate', value, description)`
- `Wchannel` -> Channel width; MATLAB: `model.param.set('Wchannel', value, description)`
- `dOx` -> Oxide thickness; MATLAB: `model.param.set('dOx', value, description)`
- `Lsource` -> Source length; MATLAB: `model.param.set('Lsource', value, description)`
- `Ldrain` -> Drain length; MATLAB: `model.param.set('Ldrain', value, description)`
- `Ltot` -> Total length; MATLAB: `model.param.set('Ltot', value, description)`
- `Wtot` -> Total width; MATLAB: `model.param.set('Wtot', value, description)`
...
- 已识别参数：Lgate、Wchannel、dOx、Lsource、Ldrain、Ltot、Wtot
...
- Sweep `Lgate` around the learned value `4[nm]` and export target outputs to CSV.
- Sweep `Wchannel` around the learned value `3.2[nm]` and export target outputs to CSV.
- Sweep `dOx` around the learned value `0.8[nm]` and export target outputs to CSV.
- Sweep `Lsource` around the learned value `15[nm]` and export target outputs to CSV.
- Sweep `Ldrain` around the learned value `15[nm]` and export target outputs to CSV.
- Sweep `Ltot` around the learned value `Lsource+Lgate+Ldrain` and export target outputs to CSV.
- Sweep `Wtot` around the learned value `Wchannel+dOx*2` and export target outputs to CSV.
...
- Summary: 已学习 nanowire_density_gradient_3d 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=9 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 9 条；几何=0，物理场=0，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：nanowire_density_gradient_3d.m, nanowire_density_gradient_3d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 nanowire_density_gradient_3d，训练阶段为 modeling_logic_learning_ready。
...
- `nanowire_density_gradient_3d.mph` (comsol_mph, 23433650 bytes)
- `nanowire_density_gradient_3d.m` (matlab_livelink, 62318 bytes)
- `nanowire_density_gradient_3d.java` (comsol_java, 57944 bytes)
- `nanowire_density_gradient_3d_summary.json` (json, 1656 bytes)
- `models.semicond.nanowire_density_gradient_3d.pdf` (pdf_document, 631111 bytes)
