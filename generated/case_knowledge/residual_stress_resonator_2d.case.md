# residual_stress_resonator_2d

- Case directory: `D:\桌面\codex\案例下载\电气\薄膜谐振器中的残余应力 - 三维`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T16:05:34.698012+00:00`
...
- 模型树证据数量: 20
- 理论关键词: 传热, 结构力学, 声学, 优化/参数扫描
- physics: Solid Mechanics
- studies: Stationary, Eigenfrequency

### 文件内容证据

- `residual_stress_resonator_2d.java`: 参数=17, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mems.residual_stress_resonator_2d.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `models.mems.residual_stress_resonator_3d.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `residual_stress_resonator_2d.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `residual_stress_resonator_2d_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `residual_stress_resonator_2d.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.residual_stress_resonator_2d.pdf`: models.mems.residual_stress_resonator_2d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Almost all surface-micromachined thin films experience residual stress as a result of the
- `models.mems.residual_stress_resonator_3d.pdf`: models.mems.residual_stress_resonator_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Almost all surface-micromachined thin films experience residual stress as a result of the

## PDF Summary and Physics Judgement

- Simple summary: residual_stress_resonator_2d 主要研究结构受力、变形或强度响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Eigenfrequency 研究。
...
- PDF: Almost all surface-micromachined thin films experience residual stress as a result of the
- PDF: is subjected to a residual thermal stress. The results obtained are compared with analytical
- PDF: Eigenfrequency Study is used. This predefined study type first solves for a static thermal
...
- PDF keywords: 传热, 结构力学, 声学, 优化/参数扫描, models.mems.residual_stress_resonator_2d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Almost all surface-micromachined thin films experience residual stress as a result of the, models.mems.residual_stress_resonator_3d.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Almost all surface-micromachined thin films experience residual stress as a result of the
- `E1` -> Young's modulus; MATLAB: `model.param.set('E1', value, description)`
- `nu1` -> Poisson's ratio; MATLAB: `model.param.set('nu1', value, description)`
- `rho1` -> Density; MATLAB: `model.param.set('rho1', value, description)`
- `sigma` -> Residual stress; MATLAB: `model.param.set('sigma', value, description)`
- `epsilon` -> Residual strain; MATLAB: `model.param.set('epsilon', value, description)`
- `T1` -> Deposition temperature; MATLAB: `model.param.set('T1', value, description)`
- `T0` -> Room temperature; MATLAB: `model.param.set('T0', value, description)`
- `daT` -> Coefficient of thermal expansion; MATLAB: `model.param.set('daT', value, description)`
- `thickness` -> Thickness; MATLAB: `model.param.set('thickness', value, description)`
- `b` -> Width; MATLAB: `model.param.set('b', value, description)`
- `L_straight` -> Length of straight cantilever; MATLAB: `model.param.set('L_straight', value, description)`
- `Length_plate` -> Length of resonator plate; MATLAB: `model.param.set('Length_plate', value, description)`
...
- 已识别参数：E1、nu1、rho1、sigma、epsilon、T1、T0、daT、thickness、b、L_straight、Length_plate
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 结构力学, 声学, 优化/参数扫描?
...
- Sweep `E1` around the learned value `155[GPa]` and export target outputs to CSV.
- Sweep `nu1` around the learned value `0.23` and export target outputs to CSV.
- Sweep `rho1` around the learned value `2330[kg/m^3]` and export target outputs to CSV.
- Sweep `sigma` around the learned value `50[MPa]` and export target outputs to CSV.
- Sweep `epsilon` around the learned value `sigma*(1-nu1)/E1` and export target outputs to CSV.
- Sweep `T1` around the learned value `605[degC]` and export target outputs to CSV.
- Sweep `T0` around the learned value `25[degC]` and export target outputs to CSV.
- Sweep `daT` around the learned value `epsilon/(T1-T0)` and export target outputs to CSV.
...
- Summary: 已学习 residual_stress_resonator_2d 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=17；内容证据=20 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=0，物理场=1，材料=0，网格=0，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：residual_stress_resonator_2d.m, residual_stress_resonator_2d.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 residual_stress_resonator_2d，训练阶段为 modeling_logic_learning_ready。
...
- `residual_stress_resonator_2d.mph` (comsol_mph, 33827520 bytes)
- `residual_stress_resonator_2d.m` (matlab_livelink, 45694 bytes)
- `residual_stress_resonator_2d.java` (comsol_java, 44809 bytes)
- `residual_stress_resonator_2d_summary.json` (json, 2736 bytes)
- `models.mems.residual_stress_resonator_2d.pdf` (pdf_document, 1197699 bytes)
- `models.mems.residual_stress_resonator_3d.pdf` (pdf_document, 1093984 bytes)
