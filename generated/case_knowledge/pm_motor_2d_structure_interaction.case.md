...
- Case directory: `D:\桌面\codex\案例下载\电气\永磁电机的磁-结构相互作用`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:30:33.213475+00:00`
...
- `pm_motor_2d_structure_interaction.java`: 参数=15, 几何=1, 物理场=0, 研究=0, 结果=0
- `models.acdc.pm_motor_2d_structure_interaction.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `models.mbd.pm_motor_2d_structure_interaction.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `pm_motor_2d_structure_interaction.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pm_motor_2d_structure_interaction_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pm_motor_2d_structure_interaction.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.pm_motor_2d_structure_interaction.pdf`: models.acdc.pm_motor_2d_structure_interaction.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；2D. To model magnetic–structure coupling integrated with moving mesh, the
- `models.mbd.pm_motor_2d_structure_interaction.pdf`: models.mbd.pm_motor_2d_structure_interaction.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；2D. To model magnetic–structure coupling integrated with moving mesh, the

## PDF Summary and Physics Judgement

- Simple summary: pm_motor_2d_structure_interaction 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: 2D. To model magnetic–structure coupling integrated with moving mesh, the
- PDF: moving mesh. A time-dependent problem, computing the magnetic flux density and
...
- PDF keywords: 电磁, 结构力学, 流体, models.acdc.pm_motor_2d_structure_interaction.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；2D. To model magnetic–structure coupling integrated with moving mesh, the, models.mbd.pm_motor_2d_structure_interaction.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；2D. To model magnetic–structure coupling integrated with moving mesh, the
...
- `APnr` -> Number of air pockets in rotor; MATLAB: `model.param.set('APnr', value, description)`
- `APfct` -> Ratio of air pocket width to lamination width; MATLAB: `model.param.set('APfct', value, description)`
- `geom_scale` -> Geometry scale; MATLAB: `model.param.set('geom_scale', value, description)`
- `L` -> Out-of-plane thickness of motor; MATLAB: `model.param.set('L', value, description)`
- `Ipk` -> Phase current peak; MATLAB: `model.param.set('Ipk', value, description)`
...
- `t_ramp` -> Time for ramping speed; MATLAB: `model.param.set('t_ramp', value, description)`
- `N_tsteps` -> Number of time steps; MATLAB: `model.param.set('N_tsteps', value, description)`
...
- 已识别参数：Np、Ns、APnr、APfct、geom_scale、L、Ipk、init_ang、w_rot、f_el、t_ramp、N_tsteps
...
- Sweep `APnr` around the learned value `5` and export target outputs to CSV.
- Sweep `APfct` around the learned value `0.7` and export target outputs to CSV.
- Sweep `geom_scale` around the learned value `5` and export target outputs to CSV.
- Sweep `L` around the learned value `300[mm]` and export target outputs to CSV.
- Sweep `Ipk` around the learned value `10[A]` and export target outputs to CSV.
...
- Summary: 已学习 pm_motor_2d_structure_interaction 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=15；内容证据=19 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 19 条；几何=1，物理场=0，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pm_motor_2d_structure_interaction.m, pm_motor_2d_structure_interaction.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pm_motor_2d_structure_interaction，训练阶段为 modeling_logic_learning_ready。
...
- `pm_motor_2d_structure_interaction.mph` (comsol_mph, 107417842 bytes)
- `pm_motor_2d_structure_interaction.m` (matlab_livelink, 88235 bytes)
- `pm_motor_2d_structure_interaction.java` (comsol_java, 88153 bytes)
- `pm_motor_2d_structure_interaction_summary.json` (json, 2084 bytes)
- `models.acdc.pm_motor_2d_structure_interaction.pdf` (pdf_document, 1409599 bytes)
- `models.mbd.pm_motor_2d_structure_interaction.pdf` (pdf_document, 1409608 bytes)
