...
- Created at: `2026-07-06T09:46:02.425450+00:00`
...
- `ultrafast_heat_transfer.mph`: 参数=8, 几何=1, 物理场=2, 研究=2, 结果=0
...
- Simple summary: 飞秒激光加热引起的超快传热 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Transient 研究。
...
- PDF keywords: 传热
- `Phi` -> 案例参数/扫描变量; MATLAB: `model.param.set('Phi', value, description)`
- `G` -> 案例参数/扫描变量; MATLAB: `model.param.set('G', value, description)`
- `delta` -> 案例参数/扫描变量; MATLAB: `model.param.set('delta', value, description)`
- `C_l` -> 案例参数/扫描变量; MATLAB: `model.param.set('C_l', value, description)`
- `C_e` -> 案例参数/扫描变量; MATLAB: `model.param.set('C_e', value, description)`
- `K_e` -> 案例参数/扫描变量; MATLAB: `model.param.set('K_e', value, description)`
...
- 已识别几何证据：i1 / Interval
- 已识别物理场证据：ht / HeatTransfer；dode / DomainODE
- 已识别参数：Phi、G、delta、C_l、C_e、K_e、rho、t_p
...
- Transfer recognized physics interfaces to similar requirements: ht / HeatTransfer, dode / DomainODE.
...
- Sweep `Phi` around the learned value `10[mJ/cm^2]` and export target outputs to CSV.
- Sweep `G` around the learned value `2.6E16[W/m^3/K]` and export target outputs to CSV.
- Sweep `delta` around the learned value `15.3[nm]` and export target outputs to CSV.
- Sweep `C_l` around the learned value `2.5E6[J/m^3/K]` and export target outputs to CSV.
- Sweep `C_e` around the learned value `2.1E4[J/m^3/K]` and export target outputs to CSV.
- Sweep `K_e` around the learned value `315[W/m/K]` and export target outputs to CSV.
- Sweep `rho` around the learned value `10000[kg/m^3]` and export target outputs to CSV.
- Sweep `t_p` around the learned value `100[fs]` and export target outputs to CSV.
