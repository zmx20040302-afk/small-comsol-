...
- Case directory: `D:\桌面\codex\案例下载\电气\使用 k.p 法分析应变纤锌矿 GaN 能带结构`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T09:14:18.974299+00:00`
...
- `Del1` = `16[meV]` - Energy parameters
- `Delso` = `12[meV]`
- `Del2` = `Delso/3`
- `Del3` = `4[meV]`
- `Del` = `sqrt(2)*Del3`
- `A1` = `-6.56` - Valence band effective mass parameters
- `A2` = `-0.91`
- `A3` = `5.65`
- `A4` = `-2.83`
- `A5` = `-3.13`
- `A6` = `-4.86`
- `D1` = `0.7[eV]` - Deformation potentials
- `D2` = `2.1[eV]`
- `D3` = `1.4[eV]`
- `D4` = `-0.7[eV]`
- `C13` = `15.8e11[dyn/cm^2]` - Elastic stiffness constants
- `C33` = `26.7e11[dyn/cm^2]`

## 内容级读取结果

- 模型树证据数量: 20
- 理论关键词: 结构力学
- physics: Solid Mechanics, Global ODEs and DAEs
- studies: Eigenvalue

### 文件内容证据

- `k_dot_p_method_strained_wurtzite_gan_band_structure.java`: 参数=17, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.semicond.k_dot_p_method_strained_wurtzite_gan_band_structure.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `k_dot_p_method_strained_wurtzite_gan_band_structure_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `k_dot_p_method_strained_wurtzite_gan_band_structure.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `k_dot_p_method_strained_wurtzite_gan_band_structure.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.k_dot_p_method_strained_wurtzite_gan_band_structure.pdf`: models.semicond.k_dot_p_method_strained_wurtzite_gan_band_structure.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Global ODEs and DAEs，研究类型线索为 Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；For the Schrödinger Equation physics interface, if the wave function has more than one

## PDF Summary and Physics Judgement

- Simple summary: k_dot_p_method_strained_wurtzite_gan_band_structure 主要研究结构受力、变形或强度响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenvalue 研究。
...
- PDF: For the Schrödinger Equation physics interface, if the wave function has more than one
- PDF: elements exists and the result from the solution of the Schrödinger Equation can be
- PDF: convention adopted by all COMSOL physics interfaces: a plane wave is exp(−ikx+iωt),
- 脚本/文本识别到的物理场: Solid Mechanics, Global ODEs and DAEs
- 识别到的研究类型: Eigenvalue
- 理论关键词: 结构力学
...
- PDF keywords: 结构力学, models.semicond.k_dot_p_method_strained_wurtzite_gan_band_structure.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Global ODEs and DAEs，研究类型线索为 Eigenvalue；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；For the Schrödinger Equation physics interface, if the wave function has more than one
- `Del1` -> Energy parameters; MATLAB: `model.param.set('Del1', value, description)`
- `Delso` -> 案例参数/扫描变量; MATLAB: `model.param.set('Delso', value, description)`
- `Del2` -> 案例参数/扫描变量; MATLAB: `model.param.set('Del2', value, description)`
- `Del3` -> 案例参数/扫描变量; MATLAB: `model.param.set('Del3', value, description)`
- `Del` -> 案例参数/扫描变量; MATLAB: `model.param.set('Del', value, description)`
- `A1` -> Valence band effective mass parameters; MATLAB: `model.param.set('A1', value, description)`
- `A2` -> 案例参数/扫描变量; MATLAB: `model.param.set('A2', value, description)`
- `A3` -> 案例参数/扫描变量; MATLAB: `model.param.set('A3', value, description)`
- `A4` -> 案例参数/扫描变量; MATLAB: `model.param.set('A4', value, description)`
- `A5` -> 案例参数/扫描变量; MATLAB: `model.param.set('A5', value, description)`
- `A6` -> 案例参数/扫描变量; MATLAB: `model.param.set('A6', value, description)`
- `D1` -> Deformation potentials; MATLAB: `model.param.set('D1', value, description)`
...
- 已识别物理场证据：Solid Mechanics；Global ODEs and DAEs
- 已识别参数：Del1、Delso、Del2、Del3、Del、A1、A2、A3、A4、A5、A6、D1
...
- Transfer recognized physics interfaces to similar requirements: Solid Mechanics, Global ODEs and DAEs.
- Use the learned study types as solver starting points: Eigenvalue.
...
- Sweep `Del1` around the learned value `16[meV]` and export target outputs to CSV.
- Sweep `Delso` around the learned value `12[meV]` and export target outputs to CSV.
- Sweep `Del2` around the learned value `Delso/3` and export target outputs to CSV.
- Sweep `Del3` around the learned value `4[meV]` and export target outputs to CSV.
- Sweep `Del` around the learned value `sqrt(2)*Del3` and export target outputs to CSV.
- Sweep `A1` around the learned value `-6.56` and export target outputs to CSV.
- Sweep `A2` around the learned value `-0.91` and export target outputs to CSV.
- Sweep `A3` around the learned value `5.65` and export target outputs to CSV.
...
- Summary: 已学习 k_dot_p_method_strained_wurtzite_gan_band_structure 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=17；内容证据=20 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 20 条；几何=0，物理场=2，材料=0，网格=0，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：k_dot_p_method_strained_wurtzite_gan_band_structure.m, k_dot_p_method_strained_wurtzite_gan_band_structure.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 k_dot_p_method_strained_wurtzite_gan_band_structure，训练阶段为 modeling_logic_learning_ready。
...
- `k_dot_p_method_strained_wurtzite_gan_band_structure.mph` (comsol_mph, 6402800 bytes)
- `k_dot_p_method_strained_wurtzite_gan_band_structure.m` (matlab_livelink, 43724 bytes)
- `k_dot_p_method_strained_wurtzite_gan_band_structure.java` (comsol_java, 39902 bytes)
- `k_dot_p_method_strained_wurtzite_gan_band_structure_summary.json` (json, 2112 bytes)
- `models.semicond.k_dot_p_method_strained_wurtzite_gan_band_structure.pdf` (pdf_document, 948932 bytes)
