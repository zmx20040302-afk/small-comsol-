...
- Created at: `2026-08-22T05:24:57.906386+00:00`
...
- `nb` = `1.2` - Refractive index, dielectric
- `d` = `400[nm]` - Grating constant
- `lam0` = `441[nm]` - Vacuum wavelength
- `f0` = `c_const/lam0` - Frequency
- `alpha` = `0[deg]` - Angle of incidence

## 内容级读取结果

- 模型树证据数量: 27
- 理论关键词: 电磁
- geometry: r1 / Rectangle, r2 / Rectangle, c1 / Circle, uni1 / Union, del1 / Delete, geom1 / r1 / size, geom1 / r2 / size, geom1 / r2 / pos, geom1 / c1 / r, geom1 / c1 / pos
- physics: Electromagnetic Waves
- materials: mat1, mat2, mat3
- mesh: mesh1
- studies: Frequency Domain
- boundary_conditions: r1 / size, r2 / size, r2 / pos, c1 / r, c1 / pos

### 文件内容证据

- `plasmonic_wire_grating.java`: 参数=6, 几何=10, 物理场=0, 研究=0, 结果=0
...
- `plasmonic_wire_grating.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `plasmonic_wire_grating.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `plasmonic_wire_grating_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: plasmonic_wire_grating 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electromagnetic Waves，并采用 Frequency Domain 研究。
...
- `nb` -> Refractive index, dielectric; MATLAB: `model.param.set('nb', value, description)`
...
- Sweep `alpha` around the learned value `0[deg]` and export target outputs to CSV.
...
- Summary: 已学习 plasmonic_wire_grating 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=27 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 27 条；几何=10，物理场=1，材料=3，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：plasmonic_wire_grating.m, plasmonic_wire_grating.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 plasmonic_wire_grating，训练阶段为 modeling_logic_learning_ready。
...
- `plasmonic_wire_grating.mph` (comsol_mph, 8696042 bytes)
- `plasmonic_wire_grating.m` (matlab_livelink, 49122 bytes)
- `plasmonic_wire_grating.java` (comsol_java, 52067 bytes)
- `plasmonic_wire_grating_summary.json` (json, 1289 bytes)
