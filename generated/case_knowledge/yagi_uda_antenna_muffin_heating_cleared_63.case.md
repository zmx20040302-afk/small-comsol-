# yagi_uda_antenna_muffin_heating_cleared_63

- Case directory: `D:\桌面\codex\案例下载\电气\移动松饼的微波加热`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:36:06.678533+00:00`
...
- `arm_length` = `0.418*lda0/2` - Dipole antenna arm length
- `r_antenna` = `arm_length/20` - Dipole antenna radius
- `gap_size` = `arm_length/100` - Gap between arms
- `lr` = `0.48*lda0` - Reflector length
- `dr` = `0.19*lda0` - Distance between feeder and reflector
- `ld` = `0.35*lda0` - Director length
- `d` = `0.2*lda0` - Distance between director
- `w_slot` = `1.8[mm]` - Slot width
- `y_pos` = `0[mm]` - Muffins postion
- `y_min` = `-120[mm]` - Maximum postion of muffins
- `y_max` = `120[mm]` - Maximum postion of muffins
- `y_step` = `10[mm]` - Step size for changing position of muffins
- `v0` = `20[mm/s]` - Velocity of moving muffins
- `t_end` = `(y_max-y_min)/v0` - Time to cover from one end to another end for muffins
- `ab` = `10[mm]`
- `bc` = `-10[mm]+20[mm]`
...
- 模型树证据数量: 36
- 理论关键词: 传热, 流体, 电磁
- geometry: cyl1 / Cylinder, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / pos, geom1 / cyl1 / layername, geom1 / cyl1 / layerside, cyl2 / Cylinder, geom1 / cyl1 / layerbottom, geom1 / cyl1 / layertop
- mesh: mesh1, mesh2
- boundary_conditions: cyl1 / r, cyl1 / h, cyl1 / pos, cyl1 / layername, cyl1 / layerside, cyl1 / layerbottom, cyl1 / layertop

### 文件内容证据

- `yagi_uda_antenna_muffin_heating_cleared_63.java`: 参数=18, 几何=8, 物理场=0, 研究=0, 结果=0
- `yagi_uda_antenna_muffin_heating_cleared_63.m`: 参数=18, 几何=6, 物理场=0, 研究=0, 结果=0
- `yagi_uda_antenna_muffin_heating_cleared_63.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `yagi_uda_antenna_muffin_heating_cleared_63_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: yagi_uda_antenna_muffin_heating_cleared_63 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer。
...
- `y_pos` -> Muffins postion; MATLAB: `model.param.set('y_pos', value, description)`
- `y_min` -> Maximum postion of muffins; MATLAB: `model.param.set('y_min', value, description)`
...
- 已识别几何证据：cyl1 / Cylinder；geom1 / cyl1 / r；geom1 / cyl1 / h；geom1 / cyl1 / pos；geom1 / cyl1 / layername；geom1 / cyl1 / layerside
- 已识别参数：f0、lda0、arm_length、r_antenna、gap_size、lr、dr、ld、d、w_slot、y_pos、y_min
...
- Sweep `arm_length` around the learned value `0.418*lda0/2` and export target outputs to CSV.
...
- Sweep `dr` around the learned value `0.19*lda0` and export target outputs to CSV.
...
- Summary: 已学习 yagi_uda_antenna_muffin_heating_cleared_63 的 4 个文件；训练阶段=modeling_logic_learning_ready；参数数量=18；内容证据=36 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 36 条；几何=9，物理场=0，材料=0，网格=2，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：yagi_uda_antenna_muffin_heating_cleared_63.m, yagi_uda_antenna_muffin_heating_cleared_63.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 yagi_uda_antenna_muffin_heating_cleared_63，训练阶段为 modeling_logic_learning_ready。
...
- `yagi_uda_antenna_muffin_heating_cleared_63.mph` (comsol_mph, 8131820 bytes)
- `yagi_uda_antenna_muffin_heating_cleared_63.m` (matlab_livelink, 88822 bytes)
- `yagi_uda_antenna_muffin_heating_cleared_63.java` (comsol_java, 95357 bytes)
- `yagi_uda_antenna_muffin_heating_cleared_63_summary.json` (json, 3136 bytes)
