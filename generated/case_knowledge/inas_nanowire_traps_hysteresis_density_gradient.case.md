...
- Case directory: `D:\桌面\codex\案例下载\电气\InAs 纳米线场效应晶体管中的表面陷阱诱导滞后 - 密度梯度分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:28:26.127486+00:00`
...
- `R0` = `23[nm]` - Radius of nanowire
- `L0` = `1.6[um]` - Length of nanowire FET channel
- `mun0` = `4e4[cm^2/V/s]` - Electron mobility
- `mup0` = `5e2[cm^2/V/s]` - Hole mobility
- `epsr0` = `15.15` - Relative permittivity
- `Eg00` = `0.354[V]` - Band gap
- `chi00` = `4.9[V]` - Electron affinity
- `Nc0` = `8.7e16[cm^-3]` - Conduction band effective density of state
- `Nv0` = `6.6e18[cm^-3]` - Valence band effective density of state
- `T0` = `300[K]` - Temperature
- `d_ox0` = `100[nm]` - Thickness of gate oxide
- `d_ox` = `d_ox0*acosh((d_ox0+R0)/R0)` - Effective thickness of gate oxide
- `epsr_ox` = `2.12` - Effective relative permittivity of gate oxide
- `G_s` = `50[uS]` - Series conductance

## 内容级读取结果

- 模型树证据数量: 16
- 理论关键词: 传热, 电磁, 优化/参数扫描
- studies: Time Dependent, Parametric Sweep

### 文件内容证据

- `inas_nanowire_traps_hysteresis_density_gradient.java`: 参数=14, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.semicond.inas_nanowire_traps_hysteresis_density_gradient.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `inas_nanowire_traps_hysteresis_density_gradient.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `inas_nanowire_traps_hysteresis_density_gradient.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `inas_nanowire_traps_hysteresis_density_gradient_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.inas_nanowire_traps_hysteresis_density_gradient.pdf`: models.semicond.inas_nanowire_traps_hysteresis_density_gradient.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁、优化/参数扫描，研究类型线索为 Time Dependent、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；conditions agree well with simulation and experiment results found in the literature.

## PDF Summary and Physics Judgement

- Simple summary: inas_nanowire_traps_hysteresis_density_gradient 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: conditions agree well with simulation and experiment results found in the literature.
- PDF: The Semiconductor interface is used to describe the governing physics. This approach is
- PDF: the model. The density-gradient parameters are directly entered in the physics features.
- 识别到的研究类型: Time Dependent, Parametric Sweep
- 理论关键词: 传热, 电磁, 优化/参数扫描

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 传热, 电磁, 优化/参数扫描, models.semicond.inas_nanowire_traps_hysteresis_density_gradient.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、电磁、优化/参数扫描，研究类型线索为 Time Dependent、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；conditions agree well with simulation and experiment results found in the literature.
- `R0` -> Radius of nanowire; MATLAB: `model.param.set('R0', value, description)`
- `L0` -> Length of nanowire FET channel; MATLAB: `model.param.set('L0', value, description)`
- `mun0` -> Electron mobility; MATLAB: `model.param.set('mun0', value, description)`
- `mup0` -> Hole mobility; MATLAB: `model.param.set('mup0', value, description)`
- `epsr0` -> Relative permittivity; MATLAB: `model.param.set('epsr0', value, description)`
- `Eg00` -> Band gap; MATLAB: `model.param.set('Eg00', value, description)`
- `chi00` -> Electron affinity; MATLAB: `model.param.set('chi00', value, description)`
- `Nc0` -> Conduction band effective density of state; MATLAB: `model.param.set('Nc0', value, description)`
- `Nv0` -> Valence band effective density of state; MATLAB: `model.param.set('Nv0', value, description)`
- `T0` -> Temperature; MATLAB: `model.param.set('T0', value, description)`
- `d_ox0` -> Thickness of gate oxide; MATLAB: `model.param.set('d_ox0', value, description)`
- `d_ox` -> Effective thickness of gate oxide; MATLAB: `model.param.set('d_ox', value, description)`
...
- 已识别参数：R0、L0、mun0、mup0、epsr0、Eg00、chi00、Nc0、Nv0、T0、d_ox0、d_ox
...
- Use the learned study types as solver starting points: Time Dependent, Parametric Sweep.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 电磁, 优化/参数扫描?
...
- Sweep `R0` around the learned value `23[nm]` and export target outputs to CSV.
- Sweep `L0` around the learned value `1.6[um]` and export target outputs to CSV.
- Sweep `mun0` around the learned value `4e4[cm^2/V/s]` and export target outputs to CSV.
- Sweep `mup0` around the learned value `5e2[cm^2/V/s]` and export target outputs to CSV.
- Sweep `epsr0` around the learned value `15.15` and export target outputs to CSV.
- Sweep `Eg00` around the learned value `0.354[V]` and export target outputs to CSV.
- Sweep `chi00` around the learned value `4.9[V]` and export target outputs to CSV.
- Sweep `Nc0` around the learned value `8.7e16[cm^-3]` and export target outputs to CSV.
...
- Summary: 已学习 inas_nanowire_traps_hysteresis_density_gradient 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=14；内容证据=16 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 16 条；几何=0，物理场=0，材料=0，网格=0，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：inas_nanowire_traps_hysteresis_density_gradient.m, inas_nanowire_traps_hysteresis_density_gradient.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 inas_nanowire_traps_hysteresis_density_gradient，训练阶段为 modeling_logic_learning_ready。
...
- `inas_nanowire_traps_hysteresis_density_gradient.mph` (comsol_mph, 22041602 bytes)
- `inas_nanowire_traps_hysteresis_density_gradient.m` (matlab_livelink, 46802 bytes)
- `inas_nanowire_traps_hysteresis_density_gradient.java` (comsol_java, 38402 bytes)
- `inas_nanowire_traps_hysteresis_density_gradient_summary.json` (json, 2894 bytes)
- `models.semicond.inas_nanowire_traps_hysteresis_density_gradient.pdf` (pdf_document, 270863 bytes)
