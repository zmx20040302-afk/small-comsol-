...
- Case directory: `D:\桌面\codex\案例下载\电气\点-面结构大气直流电晕放电`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-21T16:08:06.755375+00:00`
...
- `models.plasma.point_to_plane_dc_corona.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `point_to_plane_dc_corona.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `point_to_plane_dc_corona.java`: 参数=1, 几何=0, 物理场=0, 研究=0, 结果=0
- `point_to_plane_dc_corona_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `point_to_plane_dc_corona.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.point_to_plane_dc_corona.pdf`: models.plasma.point_to_plane_dc_corona.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This tutorial model presents a study of a negative corona discharge in dry air at

## PDF Summary and Physics Judgement

- Simple summary: point_to_plane_dc_corona 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Time Dependent 研究。
...
- PDF: This tutorial model presents a study of a negative corona discharge in dry air at
- PDF: COMSOL Multiphysics can naturally capture these pulses, and in such cases a steady-state
- PDF: wall and gained due to secondary emission effects, resulting in the boundary condition for
...
- PDF keywords: 传热, 流体, 电磁, 结构力学, models.plasma.point_to_plane_dc_corona.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；This tutorial model presents a study of a negative corona discharge in dry air at
...
- Sweep `V0` around the learned value `-5[kV]` and export target outputs to CSV.
...
- Summary: 已学习 point_to_plane_dc_corona 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=1；内容证据=5 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 5 条；几何=0，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：point_to_plane_dc_corona.m, point_to_plane_dc_corona.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 point_to_plane_dc_corona，训练阶段为 modeling_logic_learning_ready。
...
- `point_to_plane_dc_corona.mph` (comsol_mph, 98657303 bytes)
- `point_to_plane_dc_corona.m` (matlab_livelink, 48586 bytes)
- `point_to_plane_dc_corona.java` (comsol_java, 44532 bytes)
- `point_to_plane_dc_corona_summary.json` (json, 1004 bytes)
- `models.plasma.point_to_plane_dc_corona.pdf` (pdf_document, 621594 bytes)
