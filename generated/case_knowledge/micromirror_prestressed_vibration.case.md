...
- Case directory: `D:\桌面\codex\案例下载\电气\预应力微镜振动：热黏性-热弹性力学耦合`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T07:08:25.726703+00:00`
...
- `sigma_pre` = `3[GPa]` - Initial normal stress
- `fc` = `13400[Hz]` - Typical frequency
- `dvisc` = `0.22[mm]*sqrt(100[Hz]/fc)` - Viscous boundary layer thickness at fc

## 内容级读取结果

- 模型树证据数量: 26
- 理论关键词: 结构力学, 传热, 流体, 声学
- geometry: wp1 / WorkPlane, sq1 / Square, sq2 / Square, cha1 / Chamfer, r1 / Rectangle, sq3 / Square, rot1 / Rotate, geom1 / wp1 / unite
- physics: Solid Mechanics, Heat Transfer, Pressure Acoustics
- mesh: mesh1
- studies: Stationary, Eigenfrequency, Frequency Domain
- boundary_conditions: wp1 / unite, sq2 / size, sq2 / pos, cha1 / dist, r1 / size, r1 / pos, sq3 / size, sq3 / pos

### 文件内容证据

- `micromirror_prestressed_vibration.java`: 参数=3, 几何=8, 物理场=0, 研究=0, 结果=0
- `models.aco.micromirror_prestressed_vibration.pdf`: 参数=0, 几何=0, 物理场=3, 研究=3, 结果=0
- `models.mems.micromirror_prestressed_vibration.pdf`: 参数=0, 几何=0, 物理场=3, 研究=3, 结果=0
- `micromirror_prestressed_vibration_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `micromirror_prestressed_vibration.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `micromirror_prestressed_vibration.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.aco.micromirror_prestressed_vibration.pdf`: models.aco.micromirror_prestressed_vibration.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Solid Mechanics、Pressure Acoustics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Thermoviscous Acoustics–Thermoelasticity Boundary multiphysics coupling.
- `models.mems.micromirror_prestressed_vibration.pdf`: models.mems.micromirror_prestressed_vibration.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Solid Mechanics、Pressure Acoustics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；multiphysics coupling.

## PDF Summary and Physics Judgement

- Simple summary: micromirror_prestressed_vibration 主要研究结构受力、变形或强度响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Eigenfrequency 研究。
...
- PDF: Thermoviscous Acoustics–Thermoelasticity Boundary multiphysics coupling.
- PDF: Physics Interfaces and Couplings
- PDF: This model uses the following interfaces and multiphysics couplings:
- 脚本/文本识别到的物理场: Solid Mechanics, Heat Transfer, Pressure Acoustics
...
- PDF keywords: 结构力学, 传热, 流体, 声学, models.aco.micromirror_prestressed_vibration.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Solid Mechanics、Pressure Acoustics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Thermoviscous Acoustics–Thermoelasticity Boundary multiphysics coupling., models.mems.micromirror_prestressed_vibration.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Solid Mechanics、Pressure Acoustics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；multiphysics coupling.
- `sigma_pre` -> Initial normal stress; MATLAB: `model.param.set('sigma_pre', value, description)`
- `fc` -> Typical frequency; MATLAB: `model.param.set('fc', value, description)`
- `dvisc` -> Viscous boundary layer thickness at fc; MATLAB: `model.param.set('dvisc', value, description)`
...
- 已识别几何证据：wp1 / WorkPlane；sq1 / Square；sq2 / Square；cha1 / Chamfer；r1 / Rectangle；sq3 / Square
- 已识别物理场证据：Solid Mechanics；Heat Transfer；Pressure Acoustics
- 已识别参数：sigma_pre、fc、dvisc
...
- Transfer recognized physics interfaces to similar requirements: Solid Mechanics, Heat Transfer, Pressure Acoustics.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 结构力学, 传热, 流体, 声学?
...
- Sweep `sigma_pre` around the learned value `3[GPa]` and export target outputs to CSV.
- Sweep `fc` around the learned value `13400[Hz]` and export target outputs to CSV.
- Sweep `dvisc` around the learned value `0.22[mm]*sqrt(100[Hz]/fc)` and export target outputs to CSV.
...
- Summary: 已学习 micromirror_prestressed_vibration 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=26 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 26 条；几何=8，物理场=3，材料=0，网格=1，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：micromirror_prestressed_vibration.m, micromirror_prestressed_vibration.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 micromirror_prestressed_vibration，训练阶段为 modeling_logic_learning_ready。
...
- `micromirror_prestressed_vibration.mph` (comsol_mph, 1037302464 bytes)
- `micromirror_prestressed_vibration.m` (matlab_livelink, 89988 bytes)
- `micromirror_prestressed_vibration.java` (comsol_java, 93764 bytes)
- `micromirror_prestressed_vibration_summary.json` (json, 3075 bytes)
- `models.aco.micromirror_prestressed_vibration.pdf` (pdf_document, 3260901 bytes)
- `models.mems.micromirror_prestressed_vibration.pdf` (pdf_document, 3260848 bytes)
