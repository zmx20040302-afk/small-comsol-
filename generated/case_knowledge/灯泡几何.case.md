...
- Created at: `2026-07-06T09:41:26.935774+00:00`
...
- 模型树证据数量: 25
- 理论关键词: 未自动识别
- geometry: pol1 / Polygon, cb1 / CubicBezier, ca1 / CircularArc, pol2 / Polygon, geom1 / pol1 / source, geom1 / pol1 / type, geom1 / ca1 / center, geom1 / ca1 / r, geom1 / ca1 / angle1, geom1 / ca1 / angle2, geom1 / ca1 / clockwise, geom1 / pol2 / source
- mesh: mesh1
- boundary_conditions: pol1 / source, pol1 / type, ca1 / center, ca1 / r, ca1 / angle1, ca1 / angle2, ca1 / clockwise, pol2 / source, pol2 / type, cc1 / type

### 文件内容证据

- `light_bulb_geom_sequence.java`: 参数=0, 几何=14, 物理场=0, 研究=0, 结果=0
- `light_bulb_geom_sequence.m`: 参数=0, 几何=14, 物理场=0, 研究=0, 结果=0
- `light_bulb_geom_sequence.mph`: 参数=0, 几何=14, 物理场=0, 研究=0, 结果=0
- `light_bulb_geom_sequence.zh_CN.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.mph.light_bulb_geometry.pdf`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mph.light_bulb_geometry.pdf`: models.mph.light_bulb_geometry.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.

## PDF Summary and Physics Judgement

- Simple summary: 灯泡几何 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF keywords: models.mph.light_bulb_geometry.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果；核心内容可概括为：PDF detected. Text extraction requires pypdf or pdfplumber.
...
- 已识别几何证据：pol1 / Polygon；cb1 / CubicBezier；ca1 / CircularArc；pol2 / Polygon；geom1 / pol1 / source；geom1 / pol1 / type
...
- Summary: 已学习 灯泡几何 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=0；内容证据=25 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 25 条；几何=14，物理场=0，材料=0，网格=1，研究=0，结果=0。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 灯泡几何，训练阶段为 modeling_logic_learning_ready。
...
- `管式反应器代理模型 App` score=0.1847; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `热执行器代理模型 App` score=0.155; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42
- `集群设置验证` score=0.1491; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=1

### Official Documentation Checks

- `IntroductionToCOMSOLMultiphysics.pdf` page 262; module=COMSOL_Multiphysics; score=0.1086
- `LiveLinkForSimulinkUsersGuide.pdf` page 20; module=LiveLink_for_Simulink; score=0.1041
- `LiveLinkForMATLABUsersGuide.pdf` page 198; module=LiveLink_for_MATLAB; score=0.1039
