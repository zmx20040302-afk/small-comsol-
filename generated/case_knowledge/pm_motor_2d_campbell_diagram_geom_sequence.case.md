...
- 模型树证据数量: 16
- 理论关键词: 传热, 结构力学, 流体, 电磁, 声学, 优化/参数扫描
- geometry: pi1 / PartInstance, pi2 / PartInstance, geom1 / pi1 / selkeepnoncontr, geom1 / pi2 / part
...
- boundary_conditions: pi1 / selkeepnoncontr, pi2 / part
...
- `pm_motor_2d_campbell_diagram_geom_sequence.m`: 参数=2, 几何=4, 物理场=0, 研究=0, 结果=0
- `pm_motor_2d_campbell_diagram_geom_sequence.java`: 参数=2, 几何=4, 物理场=0, 研究=0, 结果=0
- `pm_motor_2d_campbell_diagram_geom_sequence.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `pm_motor_2d_campbell_diagram_geom_sequence_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
...
- Simple summary: pm_motor_2d_campbell_diagram_geom_sequence 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Eigenfrequency 研究。
...
- 已识别几何证据：pi1 / PartInstance；pi2 / PartInstance；geom1 / pi1 / selkeepnoncontr；geom1 / pi2 / part
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 结构力学, 流体, 电磁, 声学, 优化/参数扫描?
...
- Summary: 已学习 pm_motor_2d_campbell_diagram_geom_sequence 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=16 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 16 条；几何=4，物理场=3，材料=0，网格=1，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：pm_motor_2d_campbell_diagram_geom_sequence.m, pm_motor_2d_campbell_diagram_geom_sequence.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 pm_motor_2d_campbell_diagram_geom_sequence，训练阶段为 modeling_logic_learning_ready。
...
- `pm_motor_2d_campbell_diagram_geom_sequence.mph` (comsol_mph, 3028930 bytes)
- `pm_motor_2d_campbell_diagram_geom_sequence.m` (matlab_livelink, 11036 bytes)
- `pm_motor_2d_campbell_diagram_geom_sequence.java` (comsol_java, 11191 bytes)
- `pm_motor_2d_campbell_diagram_geom_sequence_summary.json` (json, 708 bytes)
