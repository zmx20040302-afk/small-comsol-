...
- Case directory: `D:\桌面\codex\案例下载\电气\自聚焦`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T05:45:39.672171+00:00`
...
- `lda0` = `1.064[um]` - Wavelength
- `w0` = `100*lda0` - Nominal spot radius
- `n0` = `1.52` - Refractive index of BK-7 glass
- `x0` = `pi*n0*w0^2/lda0` - Rayleigh range
- `k` = `2*pi*n0/lda0` - Propagation constant
- `I0` = `2.5[GW/cm^2]` - Nominal peak intensity
- `E0` = `sqrt(2*I0/n0*sqrt(mu0_const/epsilon0_const))` - Nominal peak electric field
- `length` = `4*x0` - Length of computation domain
- `radius` = `2.5*w0*sqrt(1+(length/(2*x0))^2)` - Radius of computation domain
- `gamma` = `4e-16[cm^2/W]` - Nonlinear refractive index coefficient
- `delta_n` = `gamma*I0` - Nominal refractive index change
- `P_cr` = `(1.22*pi)^2*lda0^2/(8*pi*n0*gamma)` - Critical power

## 内容级读取结果

- 模型树证据数量: 30
- 理论关键词: 电磁, 优化/参数扫描
- geometry: cyl1 / Cylinder, blk1 / Block, int1 / Intersection, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / pos, geom1 / cyl1 / axis, geom1 / blk1 / size, geom1 / blk1 / pos
- physics: Electromagnetic Waves
- mesh: mesh1
- studies: Parametric Sweep
- boundary_conditions: cyl1 / r, cyl1 / h, cyl1 / pos, cyl1 / axis, blk1 / size, blk1 / pos

### 文件内容证据

- `self_focusing.java`: 参数=12, 几何=9, 物理场=0, 研究=0, 结果=0
- `models.woptics.self_focusing.pdf`: 参数=0, 几何=0, 物理场=1, 研究=1, 结果=0
- `self_focusing.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `self_focusing.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `self_focusing_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.self_focusing.pdf`: models.woptics.self_focusing.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry for the model is simple — just a cylinder.

## PDF Summary and Physics Judgement

- Simple summary: self_focusing 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Optimization，并采用 Parametric Sweep 研究。
- Primary physics: `Optimization`
- Study type: `Parametric Sweep`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 Optimization，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型可由 Parametric Sweep 确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: The geometry for the model is simple — just a cylinder.
- PDF: Results and Discussion
- PDF: results as the Gaussian beam expression in Equation 2. However, with increasing
...
- PDF keywords: 电磁, 优化/参数扫描, models.woptics.self_focusing.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves，研究类型线索为 Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry for the model is simple — just a cylinder.
- `lda0` -> Wavelength; MATLAB: `model.param.set('lda0', value, description)`
- `w0` -> Nominal spot radius; MATLAB: `model.param.set('w0', value, description)`
- `n0` -> Refractive index of BK-7 glass; MATLAB: `model.param.set('n0', value, description)`
...
- `I0` -> Nominal peak intensity; MATLAB: `model.param.set('I0', value, description)`
- `E0` -> Nominal peak electric field; MATLAB: `model.param.set('E0', value, description)`
- `length` -> Length of computation domain; MATLAB: `model.param.set('length', value, description)`
- `radius` -> Radius of computation domain; MATLAB: `model.param.set('radius', value, description)`
- `gamma` -> Nonlinear refractive index coefficient; MATLAB: `model.param.set('gamma', value, description)`
- `delta_n` -> Nominal refractive index change; MATLAB: `model.param.set('delta_n', value, description)`
- `P_cr` -> Critical power; MATLAB: `model.param.set('P_cr', value, description)`
...
- 已识别几何证据：cyl1 / Cylinder；blk1 / Block；int1 / Intersection；geom1 / cyl1 / r；geom1 / cyl1 / h；geom1 / cyl1 / pos
- 已识别物理场证据：Electromagnetic Waves
- 已识别参数：lda0、w0、n0、x0、k、I0、E0、length、radius、gamma、delta_n、P_cr
...
- Sweep `lda0` around the learned value `1.064[um]` and export target outputs to CSV.
- Sweep `w0` around the learned value `100*lda0` and export target outputs to CSV.
- Sweep `n0` around the learned value `1.52` and export target outputs to CSV.
- Sweep `x0` around the learned value `pi*n0*w0^2/lda0` and export target outputs to CSV.
- Sweep `k` around the learned value `2*pi*n0/lda0` and export target outputs to CSV.
- Sweep `I0` around the learned value `2.5[GW/cm^2]` and export target outputs to CSV.
- Sweep `E0` around the learned value `sqrt(2*I0/n0*sqrt(mu0_const/epsilon0_const))` and export target outputs to CSV.
- Sweep `length` around the learned value `4*x0` and export target outputs to CSV.
...
- Summary: 已学习 self_focusing 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=12；内容证据=30 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 30 条；几何=9，物理场=1，材料=0，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：self_focusing.m, self_focusing.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 self_focusing，训练阶段为 modeling_logic_learning_ready。
...
- `self_focusing.mph` (comsol_mph, 20644779 bytes)
- `self_focusing.m` (matlab_livelink, 31937 bytes)
- `self_focusing.java` (comsol_java, 29141 bytes)
- `self_focusing_summary.json` (json, 1314 bytes)
- `models.woptics.self_focusing.pdf` (pdf_document, 985783 bytes)
