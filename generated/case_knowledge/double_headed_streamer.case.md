# double_headed_streamer

- Case directory: `D:\桌面\codex\案例下载\电气\平行板电极中的双头流注`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:30:46.642086+00:00`
...
- boundary_conditions: ece1 / 2, 5, ece2 / 3, 6, map1 / 1, r1 / layername, r1 / layerleft, r1 / layerbottom

### 文件内容证据

- `double_headed_streamer.java`: 参数=1, 几何=4, 物理场=1, 研究=0, 结果=0
- `models.edis.double_headed_streamer.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `double_headed_streamer.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `double_headed_streamer.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `double_headed_streamer_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.double_headed_streamer.pdf`: models.edis.double_headed_streamer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；often high in energy, can result from drift, diffusion, photoionization, or other

## PDF Summary and Physics Judgement

- Simple summary: double_headed_streamer 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Time Dependent 研究。
...
- PDF: often high in energy, can result from drift, diffusion, photoionization, or other
- PDF: This case study investigates a double-headed streamer between parallel-plate electrodes.
- PDF: density consistent with simulation results in Ref. 1.
...
- PDF keywords: 电磁, 结构力学, 流体, models.edis.double_headed_streamer.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 结构力学、流体、电磁，研究类型线索为 Time Dependent；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；often high in energy, can result from drift, diffusion, photoionization, or other
...
- Summary: 已学习 double_headed_streamer 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=1；内容证据=17 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 17 条；几何=4，物理场=1，材料=1，网格=3，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：double_headed_streamer.m, double_headed_streamer.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 double_headed_streamer，训练阶段为 modeling_logic_learning_ready。
...
- `double_headed_streamer.mph` (comsol_mph, 14247779 bytes)
- `double_headed_streamer.m` (matlab_livelink, 28364 bytes)
- `double_headed_streamer.java` (comsol_java, 24070 bytes)
- `double_headed_streamer_summary.json` (json, 901 bytes)
- `models.edis.double_headed_streamer.pdf` (pdf_document, 822623 bytes)
