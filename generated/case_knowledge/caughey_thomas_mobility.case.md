...
- Case directory: `D:\桌面\codex\案例下载\电气\Caughey-Thomas 迁移率`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T11:51:59.492565+00:00`
...
- `Wfin` = `10[nm]` - Height
- `Lg` = `100[nm]` - Width
- `tox` = `1[nm]` - Gate oxide thickness
- `Vds` = `0[V]` - Drain source voltage
- `Vgs` = `0[V]` - Gate source voltage
- `ds` = `0` - Continuation parameter
...
- `caughey_thomas_mobility.java`: 参数=6, 几何=8, 物理场=0, 研究=0, 结果=0
- `models.semicond.caughey_thomas_mobility.pdf`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `caughey_thomas_mobility.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `caughey_thomas_mobility.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `caughey_thomas_mobility_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.caughey_thomas_mobility.pdf`: models.semicond.caughey_thomas_mobility.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；already highly nonlinear even more nonlinear. The Semiconductor physics interface

## PDF Summary and Physics Judgement

- Simple summary: caughey_thomas_mobility 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: already highly nonlinear even more nonlinear. The Semiconductor physics interface
- PDF: drain and source are located on the left and right side of the geometry, respectively; see
- PDF: coupled manner. The Semiconductor physics interface automatically creates a suggested
- 识别到的研究类型: Stationary
- 理论关键词: 电磁, 传热, 流体
...
- PDF keywords: 电磁, 传热, 流体, models.semicond.caughey_thomas_mobility.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 传热、流体、电磁，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；already highly nonlinear even more nonlinear. The Semiconductor physics interface
- `Wfin` -> Height; MATLAB: `model.param.set('Wfin', value, description)`
- `Lg` -> Width; MATLAB: `model.param.set('Lg', value, description)`
- `tox` -> Gate oxide thickness; MATLAB: `model.param.set('tox', value, description)`
- `Vds` -> Drain source voltage; MATLAB: `model.param.set('Vds', value, description)`
- `Vgs` -> Gate source voltage; MATLAB: `model.param.set('Vgs', value, description)`
- `ds` -> Continuation parameter; MATLAB: `model.param.set('ds', value, description)`
...
- Sweep `Wfin` around the learned value `10[nm]` and export target outputs to CSV.
- Sweep `Lg` around the learned value `100[nm]` and export target outputs to CSV.
- Sweep `tox` around the learned value `1[nm]` and export target outputs to CSV.
- Sweep `Vds` around the learned value `0[V]` and export target outputs to CSV.
- Sweep `Vgs` around the learned value `0[V]` and export target outputs to CSV.
- Sweep `ds` around the learned value `0` and export target outputs to CSV.
...
- Summary: 已学习 caughey_thomas_mobility 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=6；内容证据=22 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 22 条；几何=8，物理场=0，材料=1，网格=1，研究=1，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：caughey_thomas_mobility.m, caughey_thomas_mobility.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 caughey_thomas_mobility，训练阶段为 modeling_logic_learning_ready。
...
- `caughey_thomas_mobility.mph` (comsol_mph, 3536756 bytes)
- `caughey_thomas_mobility.m` (matlab_livelink, 44993 bytes)
- `caughey_thomas_mobility.java` (comsol_java, 42354 bytes)
- `caughey_thomas_mobility_summary.json` (json, 1196 bytes)
- `models.semicond.caughey_thomas_mobility.pdf` (pdf_document, 488330 bytes)
