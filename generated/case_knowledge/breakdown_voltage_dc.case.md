# breakdown_voltage_dc

- Case directory: `D:\桌面\codex\案例下载\电气\空气中平行电极的直流击穿电压`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T09:52:18.486552+00:00`
...
- physics: Electric Currents, Electrostatics
- materials: sw1
- mesh: mesh1
...
- `breakdown_voltage_dc.java`: 参数=2, 几何=2, 物理场=0, 研究=0, 结果=0
- `models.edis.breakdown_voltage_dc.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `breakdown_voltage_dc_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `breakdown_voltage_dc.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `breakdown_voltage_dc.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.breakdown_voltage_dc.pdf`: models.edis.breakdown_voltage_dc.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion

## PDF Summary and Physics Judgement

- Simple summary: breakdown_voltage_dc 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Electric Currents，并采用 Stationary 研究。
...
- PDF: three different material datasets. The simulated results exhibit good agreement with
- PDF: 1. Y.P. Raizer, Gas Discharges Physics , Springer, 1997.
- 脚本/文本识别到的物理场: Electric Currents, Electrostatics
...
- PDF keywords: 电磁, 流体, 优化/参数扫描, models.edis.breakdown_voltage_dc.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electrostatics，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion
...
- 已识别物理场证据：Electric Currents；Electrostatics
- 已识别参数：V0、d
...
- Transfer recognized physics interfaces to similar requirements: Electric Currents, Electrostatics.
...
- Summary: 已学习 breakdown_voltage_dc 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=11 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 11 条；几何=2，物理场=2，材料=1，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：breakdown_voltage_dc.m, breakdown_voltage_dc.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 breakdown_voltage_dc，训练阶段为 modeling_logic_learning_ready。
...
- `breakdown_voltage_dc.mph` (comsol_mph, 37721399 bytes)
- `breakdown_voltage_dc.m` (matlab_livelink, 48258 bytes)
- `breakdown_voltage_dc.java` (comsol_java, 47649 bytes)
- `breakdown_voltage_dc_summary.json` (json, 924 bytes)
- `models.edis.breakdown_voltage_dc.pdf` (pdf_document, 291969 bytes)
