...
- Created at: `2026-07-06T09:45:56.403476+00:00`

## File Summary

- count: 9
- kinds: {'comsol_java': 1, 'matlab_livelink': 1, 'comsol_mph': 1, 'text': 6}
- matlab_files: 1
- java_files: 1
- pdf_files: 0
- mph_files: 1
...
- boundary_conditions: wallbc2 / 3, prpc1 / 1, wallbc2 / SlidingWall, wallbc2 / uvw, dis1 / type, dis1 / elemcount, dis1 / elemratio, dis1 / growthrate, dis1 / symmetric, stat / useparam, wallbc2 / [3], prpc1 / [1]

### 文件内容证据

- `lid_driven_cavity.mph`: 参数=7, 几何=1, 物理场=1, 研究=3, 结果=0
- `lid_driven_cavity.java`: 参数=7, 几何=1, 物理场=1, 研究=3, 结果=0
- `lid_driven_cavity.m`: 参数=7, 几何=1, 物理场=1, 研究=2, 结果=0
- `lid_driven_cavity_literature1 (1).txt`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `lid_driven_cavity_literature1.txt`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `lid_driven_cavity_literature2 (1).txt`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `lid_driven_cavity_literature2.txt`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `lid_driven_cavity_parameters (1).txt`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `lid_driven_cavity_parameters.txt`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: 顶盖驱动方腔流 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- Summary: 已学习 顶盖驱动方腔流 的 9 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=27 条。
...
- **1. 文件证据读取**
  - Evidence: 共读取 9 个案例文件；MATLAB=1，Java=1，PDF=0，MPH=1，CSV=0。
  - Judgement: 这些文件被合并为一个 COMSOL 案例证据包，用于同时学习建模流程、参数和训练可行性。
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 27 条；几何=1，物理场=1，材料=1，网格=2，研究=3，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：lid_driven_cavity.java, lid_driven_cavity.m
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 顶盖驱动方腔流，训练阶段为 modeling_logic_learning_ready。
...
- `管式反应器代理模型 App` score=0.1744; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `热执行器代理模型 App` score=0.1556; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42
- `热执行器` score=0.1362; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=30

### Official Documentation Checks

- `IntroductionToLiveLinkForMATLAB.pdf` page 5; module=LiveLink_for_MATLAB; score=0.1056
- `IntroductionToCOMSOLMultiphysics.pdf` page 262; module=COMSOL_Multiphysics; score=0.1024
- `IntroductionToCOMSOLMultiphysics.pdf` page 260; module=COMSOL_Multiphysics; score=0.0998
...
- `lid_driven_cavity.java` (comsol_java, 16245 bytes)
- `lid_driven_cavity.m` (matlab_livelink, 15435 bytes)
- `lid_driven_cavity.mph` (comsol_mph, 4339257 bytes)
- `lid_driven_cavity_literature1 (1).txt` (text, 1070 bytes)
- `lid_driven_cavity_literature1.txt` (text, 1070 bytes)
- `lid_driven_cavity_literature2 (1).txt` (text, 1066 bytes)
- `lid_driven_cavity_literature2.txt` (text, 1066 bytes)
- `lid_driven_cavity_parameters (1).txt` (text, 288 bytes)
- `lid_driven_cavity_parameters.txt` (text, 283 bytes)
