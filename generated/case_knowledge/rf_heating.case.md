...
- Case directory: `D:\桌面\codex\案例下载\电气\射频加热`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T03:59:06.506755+00:00`
...
- 模型树证据数量: 18
- 理论关键词: 电磁, 传热, 结构力学, 流体, 多孔介质/裂隙
- geometry: imp1 / Import, geom1 / imp1 / type, geom1 / imp1 / filename
- physics: Electromagnetic Waves, Heat Transfer, Magnetic Fields
...
- `rf_heating.java`: 参数=3, 几何=3, 物理场=0, 研究=0, 结果=0
- `models.rf.rf_heating.pdf`: 参数=0, 几何=0, 物理场=3, 研究=2, 结果=0
- `rf_heating_summary.json`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `rf_heating.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `rf_heating.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.rf.rf_heating.pdf`: models.rf.rf_heating.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields、Electromagnetic Waves，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion

## PDF Summary and Physics Judgement

- Simple summary: rf_heating 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- PDF: 4 Click Study.
- 脚本/文本识别到的物理场: Electromagnetic Waves, Heat Transfer, Magnetic Fields
...
- PDF keywords: 电磁, 传热, 结构力学, 流体, 多孔介质/裂隙, models.rf.rf_heating.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Magnetic Fields、Electromagnetic Waves，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Results and Discussion
...
- 已识别物理场证据：Electromagnetic Waves；Heat Transfer；Magnetic Fields
- 已识别参数：f0、lda0、h_max
...
- Transfer recognized physics interfaces to similar requirements: Electromagnetic Waves, Heat Transfer, Magnetic Fields.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 传热, 结构力学, 流体, 多孔介质/裂隙?
...
- Sweep `f0` around the learned value `10[GHz]` and export target outputs to CSV.
...
- Summary: 已学习 rf_heating 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=3；内容证据=18 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 18 条；几何=3，物理场=3，材料=4，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：rf_heating.m, rf_heating.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 rf_heating，训练阶段为 modeling_logic_learning_ready。
...
- `rf_heating.mph` (comsol_mph, 76254946 bytes)
- `rf_heating.m` (matlab_livelink, 46753 bytes)
- `rf_heating.java` (comsol_java, 45442 bytes)
- `rf_heating_summary.json` (json, 2275 bytes)
- `models.rf.rf_heating.pdf` (pdf_document, 3637754 bytes)
