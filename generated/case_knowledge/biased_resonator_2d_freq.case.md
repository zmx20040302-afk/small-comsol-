# biased_resonator_2d_freq

- Case directory: `D:\桌面\codex\案例下载\电气\二维偏压谐振器：稳态，特征频率，频域和吸合分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-22T04:12:11.610725+00:00`
...
- `Vdc` = `35[V]` - DC bias voltage
- `l_b` = `22.5[um]` - Length of beam
- `t_b` = `2[um]` - Thickness of beam
- `l_e` = `10[um]` - Length of electrode
- `gap` = `0.1985[um]` - Air gap
- `alpha` = `4189[Hz]` - Damping parameter - alpha
- `beta` = `8.29e-13[s]` - Damping parameter - beta

## 内容级读取结果

- 模型树证据数量: 34
- 理论关键词: 电磁, 声学, 结构力学, 优化/参数扫描
- geometry: r1 / Rectangle, r2 / Rectangle, r3 / Rectangle, mir1 / Mirror, geom1 / r1 / size, geom1 / r1 / pos, geom1 / r2 / size, geom1 / r2 / pos, geom1 / r3 / size, geom1 / r3 / pos, geom1 / mir1 / keep
- physics: Solid Mechanics, Electric Currents, Electrostatics
- materials: mat1
- mesh: mesh1
- studies: Stationary, Eigenfrequency, Frequency Domain, Parametric Sweep
- boundary_conditions: r1 / size, r1 / pos, r2 / size, r2 / pos, r3 / size, r3 / pos, mir1 / keep

### 文件内容证据

- `biased_resonator_2d_freq.java`: 参数=7, 几何=11, 物理场=0, 研究=0, 结果=0
- `models.mems.biased_resonator_2d_modes.pdf`: 参数=0, 几何=0, 物理场=2, 研究=3, 结果=0
- `models.mems.biased_resonator_2d_pull_in.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `models.mems.biased_resonator_2d_basic.pdf`: 参数=0, 几何=0, 物理场=3, 研究=1, 结果=0
- `models.mems.biased_resonator_2d_freq.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `biased_resonator_2d_freq_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=3, 结果=0
- `biased_resonator_2d_freq.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `biased_resonator_2d_freq.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.mems.biased_resonator_2d_basic.pdf`: models.mems.biased_resonator_2d_basic.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electric Currents、Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；of a Biased Resonator — 3D documentation. For this 2D study, a simplified version of the
- `models.mems.biased_resonator_2d_freq.pdf`: models.mems.biased_resonator_2d_freq.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry, fabrication, and operation of the device are discussed for the Stationary
- `models.mems.biased_resonator_2d_modes.pdf`: models.mems.biased_resonator_2d_modes.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry, fabrication, and operation of the device are discussed for the “Stationary
- `models.mems.biased_resonator_2d_pull_in.pdf`: models.mems.biased_resonator_2d_pull_in.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry, fabrication, and operation of the device are discussed for the Stationary

## PDF Summary and Physics Judgement

- Simple summary: biased_resonator_2d_freq 主要研究结构受力、变形或强度响应，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Solid Mechanics，并采用 Eigenfrequency 研究。
...
- PDF: The geometry, fabrication, and operation of the device are discussed for the “Stationary
- PDF: Analysis of a Biased Resonator” model.
- PDF: so the prestressed eigenfrequency solver needs no adjustment to the physics settings. To
- 脚本/文本识别到的物理场: Solid Mechanics, Electric Currents, Electrostatics
...
- PDF keywords: 电磁, 声学, 结构力学, 优化/参数扫描, models.mems.biased_resonator_2d_basic.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electric Currents、Electrostatics，研究类型线索为 Stationary；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；of a Biased Resonator — 3D documentation. For this 2D study, a simplified version of the, models.mems.biased_resonator_2d_freq.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry, fabrication, and operation of the device are discussed for the Stationary, models.mems.biased_resonator_2d_modes.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Eigenfrequency；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry, fabrication, and operation of the device are discussed for the “Stationary, models.mems.biased_resonator_2d_pull_in.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Solid Mechanics、Electrostatics，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；The geometry, fabrication, and operation of the device are discussed for the Stationary
- `Vdc` -> DC bias voltage; MATLAB: `model.param.set('Vdc', value, description)`
- `l_b` -> Length of beam; MATLAB: `model.param.set('l_b', value, description)`
- `t_b` -> Thickness of beam; MATLAB: `model.param.set('t_b', value, description)`
- `l_e` -> Length of electrode; MATLAB: `model.param.set('l_e', value, description)`
- `gap` -> Air gap; MATLAB: `model.param.set('gap', value, description)`
- `alpha` -> Damping parameter - alpha; MATLAB: `model.param.set('alpha', value, description)`
- `beta` -> Damping parameter - beta; MATLAB: `model.param.set('beta', value, description)`
...
- 已识别几何证据：r1 / Rectangle；r2 / Rectangle；r3 / Rectangle；mir1 / Mirror；geom1 / r1 / size；geom1 / r1 / pos
- 已识别物理场证据：Solid Mechanics；Electric Currents；Electrostatics
- 已识别参数：Vdc、l_b、t_b、l_e、gap、alpha、beta
...
- Transfer recognized physics interfaces to similar requirements: Solid Mechanics, Electric Currents, Electrostatics.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 电磁, 声学, 结构力学, 优化/参数扫描?
...
- Sweep `Vdc` around the learned value `35[V]` and export target outputs to CSV.
- Sweep `l_b` around the learned value `22.5[um]` and export target outputs to CSV.
- Sweep `t_b` around the learned value `2[um]` and export target outputs to CSV.
- Sweep `l_e` around the learned value `10[um]` and export target outputs to CSV.
- Sweep `gap` around the learned value `0.1985[um]` and export target outputs to CSV.
- Sweep `alpha` around the learned value `4189[Hz]` and export target outputs to CSV.
- Sweep `beta` around the learned value `8.29e-13[s]` and export target outputs to CSV.
...
- Summary: 已学习 biased_resonator_2d_freq 的 8 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=34 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 34 条；几何=11，物理场=3，材料=1，网格=1，研究=4，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：biased_resonator_2d_freq.m, biased_resonator_2d_freq.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 biased_resonator_2d_freq，训练阶段为 modeling_logic_learning_ready。
...
- `biased_resonator_2d_freq.mph` (comsol_mph, 26911224 bytes)
- `biased_resonator_2d_freq.m` (matlab_livelink, 25615 bytes)
- `biased_resonator_2d_freq.java` (comsol_java, 23375 bytes)
- `biased_resonator_2d_freq_summary.json` (json, 1248 bytes)
- `models.mems.biased_resonator_2d_basic.pdf` (pdf_document, 576350 bytes)
- `models.mems.biased_resonator_2d_freq.pdf` (pdf_document, 190785 bytes)
- `models.mems.biased_resonator_2d_modes.pdf` (pdf_document, 250007 bytes)
- `models.mems.biased_resonator_2d_pull_in.pdf` (pdf_document, 275417 bytes)
