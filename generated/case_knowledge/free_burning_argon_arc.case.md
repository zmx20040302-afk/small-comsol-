# free_burning_argon_arc

- Case directory: `D:\桌面\codex\案例下载\电气\自由燃烧氩弧仿真`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T10:48:16.173660+00:00`
...
- 模型树证据数量: 29
- 理论关键词: 流体, 电磁, 传热, 多孔介质/裂隙, 优化/参数扫描
...
- `free_burning_argon_arc.java`: 参数=2, 几何=12, 物理场=0, 研究=0, 结果=0
- `models.edis.free_burning_argon_arc.pdf`: 参数=0, 几何=0, 物理场=2, 研究=2, 结果=0
- `free_burning_argon_arc.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `free_burning_argon_arc.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `free_burning_argon_arc_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.edis.free_burning_argon_arc.pdf`: models.edis.free_burning_argon_arc.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；and provide initial conditions for a time-dependent model.

## PDF Summary and Physics Judgement

- Simple summary: free_burning_argon_arc 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- PDF: and provide initial conditions for a time-dependent model.
- PDF: The model is solved using a stationary study. A current of 100 A is set at the cathode and
- PDF: bring a better description of the physics). To make the model more stable, use a mesh that
...
- PDF keywords: 流体, 电磁, 传热, 多孔介质/裂隙, 优化/参数扫描, models.edis.free_burning_argon_arc.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow，研究类型线索为 Stationary、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；and provide initial conditions for a time-dependent model.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 流体, 电磁, 传热, 多孔介质/裂隙, 优化/参数扫描?
...
- Sweep `I0` around the learned value `100[A]` and export target outputs to CSV.
...
- Summary: 已学习 free_burning_argon_arc 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=2；内容证据=29 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 29 条；几何=12，物理场=2，材料=3，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：free_burning_argon_arc.m, free_burning_argon_arc.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 free_burning_argon_arc，训练阶段为 modeling_logic_learning_ready。
...
- `free_burning_argon_arc.mph` (comsol_mph, 6671823 bytes)
- `free_burning_argon_arc.m` (matlab_livelink, 60752 bytes)
- `free_burning_argon_arc.java` (comsol_java, 59385 bytes)
- `free_burning_argon_arc_summary.json` (json, 2558 bytes)
- `models.edis.free_burning_argon_arc.pdf` (pdf_document, 649621 bytes)
