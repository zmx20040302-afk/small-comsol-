# three_phase_motor_frequency

- Case directory: `D:\桌面\codex\案例下载\电气\三相电机的频域研究`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:31:58.268060+00:00`
...
- `f0` = `60[Hz]` - Supply frequency
- `w0` = `2*pi*f0` - Supply angular frequency
- `n0` = `1000` - Number of turns
- `L` = `1[m]` - Length of the motor
- `Omega` = `200[rad/s]` - Angular speed of the rotor
- `coil_wire_current` = `2045.175[A]*sqrt(2)/n0` - Current amplitude in coil wire
- `r1` = `2[cm]` - Outer radius of rotor steel
- `r2` = `3[cm]` - Outer radius of rotor aluminum
- `r3` = `3.2[cm]` - Inner radius of windings
- `r4` = `5.2[cm]` - Inner radius of stator steel
- `r5` = `5.7[cm]` - Outer radius of stator steel
- `win_angle` = `45[deg]` - Angular span of a winding
- `airgap` = `r3-r2` - Size of air gap

## 内容级读取结果

- 模型树证据数量: 34
- 理论关键词: 电磁, 传热, 流体, 多孔介质/裂隙, 优化/参数扫描
- geometry: c1 / Circle, c2 / Circle, c3 / Circle, c4 / Circle, dif1 / Difference, geom1 / c1 / r, geom1 / c2 / r, geom1 / c3 / r, geom1 / c3 / angle, geom1 / c4 / r, geom1 / c4 / angle
- physics: Magnetic Fields
- mesh: mesh1
- studies: Frequency Domain, Parametric Sweep
- boundary_conditions: c1 / r, c2 / r, c3 / r, c3 / angle, c4 / r, c4 / angle

### 文件内容证据

- `three_phase_motor_frequency.java`: 参数=13, 几何=11, 物理场=0, 研究=0, 结果=0
- `models.acdc.three_phase_motor_frequency.pdf`: 参数=0, 几何=0, 物理场=1, 研究=2, 结果=0
- `three_phase_motor_frequency_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=1, 结果=0
- `three_phase_motor_frequency.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `three_phase_motor_frequency.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.acdc.three_phase_motor_frequency.pdf`: models.acdc.three_phase_motor_frequency.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Frequency Domain Study of Three-Phase

## PDF Summary and Physics Judgement

- Simple summary: three_phase_motor_frequency 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Frequency Domain 研究。
...
- PDF: Frequency Domain Study of Three-Phase
- PDF: 2 | FREQUENCY DOMAIN STUDY OF THREE-PHASE MOTOR
- PDF: problem 30, “Induction Motor Analyses” (Ref. 1). The Magnetic Fields physics is used to
...
- PDF keywords: 电磁, 传热, 流体, 多孔介质/裂隙, 优化/参数扫描, models.acdc.three_phase_motor_frequency.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Magnetic Fields，研究类型线索为 Frequency Domain、Parametric Sweep；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Frequency Domain Study of Three-Phase
- `f0` -> Supply frequency; MATLAB: `model.param.set('f0', value, description)`
- `w0` -> Supply angular frequency; MATLAB: `model.param.set('w0', value, description)`
- `n0` -> Number of turns; MATLAB: `model.param.set('n0', value, description)`
- `L` -> Length of the motor; MATLAB: `model.param.set('L', value, description)`
- `Omega` -> Angular speed of the rotor; MATLAB: `model.param.set('Omega', value, description)`
- `coil_wire_current` -> Current amplitude in coil wire; MATLAB: `model.param.set('coil_wire_current', value, description)`
- `r1` -> Outer radius of rotor steel; MATLAB: `model.param.set('r1', value, description)`
- `r2` -> Outer radius of rotor aluminum; MATLAB: `model.param.set('r2', value, description)`
...
- `win_angle` -> Angular span of a winding; MATLAB: `model.param.set('win_angle', value, description)`
...
- 已识别几何证据：c1 / Circle；c2 / Circle；c3 / Circle；c4 / Circle；dif1 / Difference；geom1 / c1 / r
- 已识别物理场证据：Magnetic Fields
- 已识别参数：f0、w0、n0、L、Omega、coil_wire_current、r1、r2、r3、r4、r5、win_angle
...
- Sweep `Omega` around the learned value `200[rad/s]` and export target outputs to CSV.
- Sweep `coil_wire_current` around the learned value `2045.175[A]*sqrt(2)/n0` and export target outputs to CSV.
...
- Summary: 已学习 three_phase_motor_frequency 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=13；内容证据=34 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 34 条；几何=11，物理场=1，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：three_phase_motor_frequency.m, three_phase_motor_frequency.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 three_phase_motor_frequency，训练阶段为 modeling_logic_learning_ready。
...
- `three_phase_motor_frequency.mph` (comsol_mph, 11849209 bytes)
- `three_phase_motor_frequency.m` (matlab_livelink, 40473 bytes)
- `three_phase_motor_frequency.java` (comsol_java, 36659 bytes)
- `three_phase_motor_frequency_summary.json` (json, 1601 bytes)
- `models.acdc.three_phase_motor_frequency.pdf` (pdf_document, 1778529 bytes)
