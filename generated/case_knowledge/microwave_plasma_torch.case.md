...
- Case directory: `D:\桌面\codex\案例下载\电气\微波等离子体炬`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-16T11:21:41.365046+00:00`
...
- `WaveguideWidth` = `2*WaveguideHeight` - Waveguide width
- `CylincerCenter` = `-15[mm]` - Cylincer center
- `WaveguideLength` = `220[mm]` - Waveguide lenght
- `WaveguideHeight` = `43.18[mm]` - Waveguide height
- `TopCylinderFactor` = `0.15` - Top cylinder adjusting factor
- `RadiusOut` = `15[mm]` - Outer radius
- `RadiusIn` = `13[mm]` - Inner radius
- `CylinderHeight` = `200[mm]` - Cylinder Height
- `f0` = `2.45[GHz]` - Excitation frequency
- `P0` = `10[W]` - Input power
- `p0` = `20[torr]` - Reference pressure
- `T0` = `300[K]` - Reference temperature
- `Qsv` = `0.5[l/min]` - Gas flow rate in slm
- `htc` = `10[W/m^2/K]` - Heat transfer coefficient

## 内容级读取结果

- 模型树证据数量: 35
- 理论关键词: 传热, 流体, 结构力学, 电磁
- geometry: blk1 / Block, cyl1 / Cylinder, cyl2 / Cylinder, geom1 / blk1 / base, geom1 / cyl1 / r, geom1 / cyl1 / h, geom1 / cyl1 / layername, geom1 / cyl2 / r
- physics: Heat Transfer, Laminar Flow, Electrostatics, Magnetic Fields, Electromagnetic Waves
- mesh: mesh1
- studies: Stationary, Frequency Domain
- boundary_conditions: blk1 / base, cyl1 / r, cyl1 / h, cyl1 / layername, cyl2 / r

### 文件内容证据

- `microwave_plasma_torch.java`: 参数=14, 几何=8, 物理场=1, 研究=0, 结果=0
- `models.plasma.microwave_plasma_torch.pdf`: 参数=0, 几何=0, 物理场=5, 研究=2, 结果=0
- `microwave_plasma_torch.mph`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `microwave_plasma_torch.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `microwave_plasma_torch_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.plasma.microwave_plasma_torch.pdf`: models.plasma.microwave_plasma_torch.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Electrostatics，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；plasma approximation that is set in the Plasma Conductivity Coupling multiphysics feature:

## PDF Summary and Physics Judgement

- Simple summary: microwave_plasma_torch 主要研究流动速度、压力或输运过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Laminar Flow，并采用 Stationary 研究。
...
- 脚本/文本识别到的物理场: Heat Transfer, Laminar Flow, Electrostatics, Magnetic Fields, Electromagnetic Waves
...
- PDF keywords: 传热, 流体, 结构力学, 电磁, models.plasma.microwave_plasma_torch.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Heat Transfer、Laminar Flow、Electrostatics，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；plasma approximation that is set in the Plasma Conductivity Coupling multiphysics feature:
- `WaveguideWidth` -> Waveguide width; MATLAB: `model.param.set('WaveguideWidth', value, description)`
- `CylincerCenter` -> Cylincer center; MATLAB: `model.param.set('CylincerCenter', value, description)`
- `WaveguideLength` -> Waveguide lenght; MATLAB: `model.param.set('WaveguideLength', value, description)`
- `WaveguideHeight` -> Waveguide height; MATLAB: `model.param.set('WaveguideHeight', value, description)`
- `TopCylinderFactor` -> Top cylinder adjusting factor; MATLAB: `model.param.set('TopCylinderFactor', value, description)`
- `RadiusOut` -> Outer radius; MATLAB: `model.param.set('RadiusOut', value, description)`
- `RadiusIn` -> Inner radius; MATLAB: `model.param.set('RadiusIn', value, description)`
- `CylinderHeight` -> Cylinder Height; MATLAB: `model.param.set('CylinderHeight', value, description)`
- `f0` -> Excitation frequency; MATLAB: `model.param.set('f0', value, description)`
- `P0` -> Input power; MATLAB: `model.param.set('P0', value, description)`
- `p0` -> Reference pressure; MATLAB: `model.param.set('p0', value, description)`
...
- 已识别几何证据：blk1 / Block；cyl1 / Cylinder；cyl2 / Cylinder；geom1 / blk1 / base；geom1 / cyl1 / r；geom1 / cyl1 / h
- 已识别物理场证据：Heat Transfer；Laminar Flow；Electrostatics；Magnetic Fields；Electromagnetic Waves
- 已识别参数：WaveguideWidth、CylincerCenter、WaveguideLength、WaveguideHeight、TopCylinderFactor、RadiusOut、RadiusIn、CylinderHeight、f0、P0、p0、T0
...
- Transfer recognized physics interfaces to similar requirements: Heat Transfer, Laminar Flow, Electrostatics, Magnetic Fields, Electromagnetic Waves.
...
- How do the theory keywords change governing equations, assumptions, or validation targets: 传热, 流体, 结构力学, 电磁?
...
- Sweep `WaveguideWidth` around the learned value `2*WaveguideHeight` and export target outputs to CSV.
- Sweep `CylincerCenter` around the learned value `-15[mm]` and export target outputs to CSV.
- Sweep `WaveguideLength` around the learned value `220[mm]` and export target outputs to CSV.
- Sweep `WaveguideHeight` around the learned value `43.18[mm]` and export target outputs to CSV.
- Sweep `TopCylinderFactor` around the learned value `0.15` and export target outputs to CSV.
- Sweep `RadiusOut` around the learned value `15[mm]` and export target outputs to CSV.
- Sweep `RadiusIn` around the learned value `13[mm]` and export target outputs to CSV.
- Sweep `CylinderHeight` around the learned value `200[mm]` and export target outputs to CSV.
...
- Summary: 已学习 microwave_plasma_torch 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=14；内容证据=35 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 35 条；几何=8，物理场=5，材料=0，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：microwave_plasma_torch.m, microwave_plasma_torch.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 microwave_plasma_torch，训练阶段为 modeling_logic_learning_ready。
...
- `microwave_plasma_torch.mph` (comsol_mph, 82882156 bytes)
- `microwave_plasma_torch.m` (matlab_livelink, 124047 bytes)
- `microwave_plasma_torch.java` (comsol_java, 130663 bytes)
- `microwave_plasma_torch_summary.json` (json, 3419 bytes)
- `models.plasma.microwave_plasma_torch.pdf` (pdf_document, 867247 bytes)
