...
- Created at: `2026-07-06T09:43:22.132997+00:00`
...
- 模型树证据数量: 30
- 理论关键词: 传热, 电磁, 多孔介质/裂隙
- geometry: i1 / Interval, geom1 / i1 / p2, geom1 / i1 / p1, geom1 / i1 / intervals, geom1 / i1 / p
- physics: ht / HeatTransfer
- materials: mat1
- mesh: mesh1
- studies: std1 / stat / Stationary, std1, Stationary
- boundary_conditions: solid2 / new int[]{2}, i1 / p2, i1 / p1, i1 / intervals, i1 / p, solid2 / k_mat, solid2 / k, solid2 / rho_mat, solid2 / rho, solid2 / Cp_mat, solid2 / Cp, solid2 / [2]

### 文件内容证据

- `thermal_hysteresis_using_state_variables_v62.java`: 参数=7, 几何=5, 物理场=1, 研究=3, 结果=0
- `thermal_hysteresis_v62.java`: 参数=7, 几何=5, 物理场=1, 研究=3, 结果=0
- `thermal_hysteresis_using_state_variables_v62.m`: 参数=7, 几何=5, 物理场=1, 研究=2, 结果=0
- `thermal_hysteresis_v62.m`: 参数=7, 几何=5, 物理场=1, 研究=2, 结果=0
- `1D_thermal_hysteresis_using_state_variables_v62.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `1D_thermal_hysteresis_v62.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## PDF Summary and Physics Judgement

- Simple summary: 磁滞作用下的相变建模 主要研究温度场和热传递过程，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 Heat Transfer，并采用 Stationary 研究。
...
- `dT` -> 案例参数/扫描变量; MATLAB: `model.param.set('dT', value, description)`
- `T_melt` -> 案例参数/扫描变量; MATLAB: `model.param.set('T_melt', value, description)`
- `T_freeze` -> 案例参数/扫描变量; MATLAB: `model.param.set('T_freeze', value, description)`
- `T_top` -> 案例参数/扫描变量; MATLAB: `model.param.set('T_top', value, description)`
- `T_bot` -> 案例参数/扫描变量; MATLAB: `model.param.set('T_bot', value, description)`
...
- 已识别几何证据：i1 / Interval；geom1 / i1 / p2；geom1 / i1 / p1；geom1 / i1 / intervals；geom1 / i1 / p
- 已识别物理场证据：ht / HeatTransfer
- 已识别参数：T_init、dT、T_melt、T_freeze、T_top、T_bot、LH
...
- Sweep `T_init` around the learned value `20[degC]` and export target outputs to CSV.
- Sweep `dT` around the learned value `1[K]` and export target outputs to CSV.
- Sweep `T_melt` around the learned value `65[degC]` and export target outputs to CSV.
- Sweep `T_freeze` around the learned value `55[degC]` and export target outputs to CSV.
- Sweep `T_top` around the learned value `T_melt+dT/2` and export target outputs to CSV.
- Sweep `T_bot` around the learned value `T_freeze-dT/2` and export target outputs to CSV.
- Sweep `LH` around the learned value `100[kJ/kg]` and export target outputs to CSV.
...
- Summary: 已学习 磁滞作用下的相变建模 的 6 个文件；训练阶段=modeling_logic_learning_ready；参数数量=7；内容证据=30 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 30 条；几何=5，物理场=1，材料=1，网格=1，研究=3，结果=0。
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 磁滞作用下的相变建模，训练阶段为 modeling_logic_learning_ready。
...
- `管式反应器代理模型 App` score=0.1737; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=86
- `热执行器代理模型 App` score=0.1521; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results; parameters=42
- `热执行器` score=0.1338; fields=physics, geometry, materials, boundary_conditions, mesh, solver, results, applicability; parameters=30

### Official Documentation Checks

- `IntroductionToLiveLinkForMATLAB.pdf` page 5; module=LiveLink_for_MATLAB; score=0.1133
- `LiveLinkForSimulinkUsersGuide.pdf` page 20; module=LiveLink_for_Simulink; score=0.1094
