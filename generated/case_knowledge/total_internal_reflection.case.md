...
- Case directory: `D:\桌面\codex\案例下载\电气\全内反射`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T12:30:18.248084+00:00`
...
- `k0` = `2*pi/lda0` - Vacuum wave number
- `n` = `1.5` - Refractive index in waveguide
- `k` = `k0*n` - Wave number in waveguide
- `theta` = `10[deg]` - Incidence angle
- `k1x` = `k*cos(theta)` - Wave vector, first wave, x-component
- `k1y` = `k*sin(theta)` - Wave vector, first wave, y-component
- `d` = `350[um]` - Waveguide width
- `L` = `5*4*d/2/tan(theta)` - Waveguide length
- `w0` = `75[um]` - Beam radius
...
- physics: ewbe / ElectromagneticWavesBeamEnvelopes, Electromagnetic Waves
- materials: mat1
- mesh: mesh1
- boundary_conditions: mbc1 / 1, mbc2 / 4, imp1 / 2, 3, r1 / size, r1 / pos

### 文件内容证据

- `total_internal_reflection.java`: 参数=10, 几何=3, 物理场=1, 研究=0, 结果=0
- `models.woptics.total_internal_reflection.pdf`: 参数=0, 几何=0, 物理场=1, 研究=0, 结果=0
- `total_internal_reflection.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `total_internal_reflection.m`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `total_internal_reflection_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.woptics.total_internal_reflection.pdf`: models.woptics.total_internal_reflection.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Electromagnetic Waves, Beam Envelopes interface is used to reduce the number of mesh

## PDF Summary and Physics Judgement

- Simple summary: total_internal_reflection 主要研究模型中的主要物理现象和输出量，PDF 用来说明问题目标、基本假设和求解对象；从核心内容看，建模时应优先选择 ElectromagneticWavesBeamEnvelopes。
- Primary physics: `ElectromagneticWavesBeamEnvelopes`
- Study type: `unknown`
- Judgement rule: 若 PDF 的核心量和方程关键词指向 ElectromagneticWavesBeamEnvelopes，并且脚本/文本中出现同类接口，就把它作为主物理场；研究类型需要继续从研究步骤确认。

### Judgement Evidence

- PDF: Model created in COMSOL Multiphysics 6.4
- PDF: Electromagnetic Waves, Beam Envelopes interface is used to reduce the number of mesh
- PDF: Results and Discussion
- PDF: propagation direction looks much steeper than it actually is. The mesh was deliberately
- 脚本/文本识别到的物理场: ewbe / ElectromagneticWavesBeamEnvelopes, Electromagnetic Waves
- 理论关键词: 电磁

## PDF Keywords to Geometry/Parameter/MATLAB Mapping

- Summary: 将 PDF 关键词、案例建模内容和 MATLAB/Java API 证据整理为可复用的几何与参数建模知识。
- PDF keywords: 电磁, models.woptics.total_internal_reflection.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，主要物理场线索为 Electromagnetic Waves；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；Electromagnetic Waves, Beam Envelopes interface is used to reduce the number of mesh
- `lda0` -> Wavelength; MATLAB: `model.param.set('lda0', value, description)`
- `k0` -> Vacuum wave number; MATLAB: `model.param.set('k0', value, description)`
- `n` -> Refractive index in waveguide; MATLAB: `model.param.set('n', value, description)`
- `k` -> Wave number in waveguide; MATLAB: `model.param.set('k', value, description)`
- `theta` -> Incidence angle; MATLAB: `model.param.set('theta', value, description)`
- `k1x` -> Wave vector, first wave, x-component; MATLAB: `model.param.set('k1x', value, description)`
- `k1y` -> Wave vector, first wave, y-component; MATLAB: `model.param.set('k1y', value, description)`
- `d` -> Waveguide width; MATLAB: `model.param.set('d', value, description)`
- `L` -> Waveguide length; MATLAB: `model.param.set('L', value, description)`
- `w0` -> Beam radius; MATLAB: `model.param.set('w0', value, description)`
...
- 已识别物理场证据：ewbe / ElectromagneticWavesBeamEnvelopes；Electromagnetic Waves
- 已识别参数：lda0、k0、n、k、theta、k1x、k1y、d、L、w0
...
- 识别到物理场证据：ewbe / ElectromagneticWavesBeamEnvelopes
...
- Transfer recognized physics interfaces to similar requirements: ewbe / ElectromagneticWavesBeamEnvelopes, Electromagnetic Waves.
...
- Sweep `n` around the learned value `1.5` and export target outputs to CSV.
- Sweep `k` around the learned value `k0*n` and export target outputs to CSV.
- Sweep `theta` around the learned value `10[deg]` and export target outputs to CSV.
- Sweep `k1x` around the learned value `k*cos(theta)` and export target outputs to CSV.
- Sweep `k1y` around the learned value `k*sin(theta)` and export target outputs to CSV.
- Sweep `d` around the learned value `350[um]` and export target outputs to CSV.
...
- Summary: 已学习 total_internal_reflection 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=10；内容证据=22 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 22 条；几何=3，物理场=2，材料=1，网格=1，研究=0，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：total_internal_reflection.m, total_internal_reflection.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 total_internal_reflection，训练阶段为 modeling_logic_learning_ready。
...
- `total_internal_reflection.mph` (comsol_mph, 13696466 bytes)
- `total_internal_reflection.m` (matlab_livelink, 16832 bytes)
- `total_internal_reflection.java` (comsol_java, 11769 bytes)
- `total_internal_reflection_summary.json` (json, 1107 bytes)
- `models.woptics.total_internal_reflection.pdf` (pdf_document, 1263311 bytes)
