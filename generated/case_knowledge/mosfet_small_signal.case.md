...
- Case directory: `D:\桌面\codex\案例下载\电气\MOSFET 小信号分析`
- Training stage: `modeling_logic_learning_ready`
- Created at: `2026-08-20T10:25:17.294276+00:00`
...
- `Vd` = `2[V]` - Drain voltage
- `Vg` = `2[V]` - Gate voltage
- `Vac` = `1[mV]` - Small signal voltage
- `freq` = `10[MHz]` - Small signal frequency

## 内容级读取结果

- 模型树证据数量: 17
- 理论关键词: 电磁
- geometry: r1 / Rectangle, pol1 / Polygon, mce1 / MeshControlEdges, geom1 / r1 / size, geom1 / pol1 / type, geom1 / pol1 / source
- materials: mat1
- mesh: mesh1
- studies: Stationary, Frequency Domain
- boundary_conditions: r1 / size, pol1 / type, pol1 / source

### 文件内容证据

- `mosfet_small_signal.java`: 参数=4, 几何=6, 物理场=0, 研究=0, 结果=0
- `mosfet_small_signal.m`: 参数=4, 几何=0, 物理场=0, 研究=0, 结果=0
- `models.semicond.mosfet_small_signal.pdf`: 参数=0, 几何=0, 物理场=0, 研究=2, 结果=0
- `mosfet_small_signal.mph`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0
- `mosfet_small_signal_summary.json`: 参数=0, 几何=0, 物理场=0, 研究=0, 结果=0

## 每个 PDF 文件的简单总结

- `models.semicond.mosfet_small_signal.pdf`: models.semicond.mosfet_small_signal.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；this, the COMSOL Multiphysics model uses two study steps: a stationary step followed by

## PDF Summary and Physics Judgement

- Simple summary: mosfet_small_signal 的 PDF 可用于理解案例目的、假设和操作步骤；当前证据还不足以唯一确定物理场，后续应结合 MATLAB/Java 中的 physics.create 或 COMSOL 模型树再确认。
...
- PDF: this, the COMSOL Multiphysics model uses two study steps: a stationary step followed by
- PDF: a frequency domain perturbation study. The Stationary study is used to compute the
- PDF: point in the absence of any AC signals. The Frequency Domain Perturbation study step
...
- PDF keywords: 电磁, models.semicond.mosfet_small_signal.pdf: 该 PDF 主要用于说明案例背景、建模目标、理论假设和验证结果，理论关键词为 电磁，研究类型线索为 Stationary、Frequency Domain；核心内容可概括为：Model created in COMSOL Multiphysics 6.4；this, the COMSOL Multiphysics model uses two study steps: a stationary step followed by
- `Vd` -> Drain voltage; MATLAB: `model.param.set('Vd', value, description)`
- `Vg` -> Gate voltage; MATLAB: `model.param.set('Vg', value, description)`
- `Vac` -> Small signal voltage; MATLAB: `model.param.set('Vac', value, description)`
- `freq` -> Small signal frequency; MATLAB: `model.param.set('freq', value, description)`
...
- 已识别几何证据：r1 / Rectangle；pol1 / Polygon；mce1 / MeshControlEdges；geom1 / r1 / size；geom1 / pol1 / type；geom1 / pol1 / source
- 已识别参数：Vd、Vg、Vac、freq
...
- Sweep `Vd` around the learned value `2[V]` and export target outputs to CSV.
- Sweep `Vg` around the learned value `2[V]` and export target outputs to CSV.
- Sweep `Vac` around the learned value `1[mV]` and export target outputs to CSV.
- Sweep `freq` around the learned value `10[MHz]` and export target outputs to CSV.
...
- Summary: 已学习 mosfet_small_signal 的 5 个文件；训练阶段=modeling_logic_learning_ready；参数数量=4；内容证据=17 条。
...
- **2. 内容级模型树抽取**
  - Evidence: 抽取到模型树证据 17 条；几何=6，物理场=0，材料=1，网格=1，研究=2，结果=0。
  - Judgement: 模型已读取案例内容并抽取 COMSOL 模型树结构，可用于后续自动建模和代码生成。
- **3. 建模脚本识别**
  - Evidence: 检测到可复用的 MATLAB/Java 建模脚本：mosfet_small_signal.m, mosfet_small_signal.java
...
- **7. 记忆库补充**
  - Evidence: 案例标题为 mosfet_small_signal，训练阶段为 modeling_logic_learning_ready。
...
- `mosfet_small_signal.mph` (comsol_mph, 24517586 bytes)
- `mosfet_small_signal.m` (matlab_livelink, 48842 bytes)
- `mosfet_small_signal.java` (comsol_java, 49549 bytes)
- `mosfet_small_signal_summary.json` (json, 1377 bytes)
- `models.semicond.mosfet_small_signal.pdf` (pdf_document, 190216 bytes)
